...
#define PNIO_DEFAULT_NIC_ID    			0		// default Network Interface Controller ID is 0
#define PNIO_DEFAULT_PORT_ID            1       // default port ID (only single port supported)


#define PNIO_EV_TO_VALID	0x00000001U
#define PNIO_EV_TO			0x00000002U
#define PNIO_EV_TI			0x00000004U
#define PNIO_EV_TI_VALID	0x00000008U
#define PNIO_EV_CYCL		0x00000010U
...
#define PNIO_USE_SPI_FLASH_FOR_FW
	...
	extern int 	spi_flash_read		  ( unsigned int flash_strt_addr, unsigned char * receive_addr, signed int data_size, unsigned int * pError );
	extern int 	spi_flash_program	  ( unsigned int flash_strt_addr, unsigned char * transmit_addr, signed int data_size, unsigned int * pError );
	extern void spi_flash_init		  ( void );
	extern int	spi_flash_erase		  ( unsigned int flash_strt_addr, signed int data_size, unsigned int * pError );
	extern int	spi_flash_chip_erase  ( void );
	extern int	spi_flash_erase_verify( unsigned int flash_strt_addr, signed int data_size );
	extern int 	spi_flash_verify	  (unsigned char * pFlash, unsigned char * pBuf, int BufSize);
	...
	#define SPI_NV_ADDRESS																	0x403f0000
//	#define SPI_NV_ADDRESS																	0x43ff0000
    #define SPI_NV_ADDRESS_SEC_OFFSET                                                       (0x4000)
    #define SPI_NV_ADDRESS_SEC1                                                             (SPI_NV_ADDRESS + SPI_NV_ADDRESS_SEC_OFFSET * 0)
    #define SPI_NV_ADDRESS_SEC2                                                             (SPI_NV_ADDRESS + SPI_NV_ADDRESS_SEC_OFFSET * 1)
#else
	#define FLASH_INIT
...
// *---------------------------------------------------
    ...
    PNIO_EXTRN PNIO_IOXS    PNIO_cbf_data_write	// write data to IO stack (local ==> remote)
		    (
		     PNIO_UINT32    DevHndl,			// [in] Handle f�r Multidevice
...
// *---------------------------------------------------
    ...
    {
	    PNIO_UINT32		time_low;
	    PNIO_UINT16		time_mid;
	    PNIO_UINT16		time_hi_and_version;
	    PNIO_UINT8		clock_seq_hi_and_reserved;
	    PNIO_UINT8		clock_seq_low;
	    PNIO_UINT8		node[6];
    }  ATTR_PNIO_PACKED PNIO_UUID_TYPE ;

    #include "sys_unpck.h"


    typedef struct _PNPB_OWNSHIP
    {
    ...
    typedef enum _EXP_ENTITY_STATE
    {
        ELEM_FREE     = 0,
        ELEM_OCCUPIED = 1
    } EXP_ENTITY_STATE;

    typedef struct _PNIO_EXP_SUB
    {
    ...
    typedef struct _s_pns_arinfo
    {
    	PNIO_AR_TYPE    ArType;             // type of AR (see cm_ar_type_enum)
    	PNIO_UINT32     ArNum;              // AR number  (device access: ArNumber = 3)
    	PNIO_UINT16     ArSessionKey;       // AS session key
    	PNIO_UINT16     SendClock;          // sendclock
    	PNIO_UINT16     RedRatioIocrIn;     // reduction ratio of input IOCR
    	PNIO_UINT16     RedRatioIocrOut;    // reduction ratio of output IOCR
    	PNIO_UINT32     HostIp;             // ip address of host ( PN-Controller )
    	PNIO_UINT32     CycleFactor;
    }TS_PNS_ARINFO, *TS_PNS_ARINFO_PTR;
    ...
    //typedef enum
    //{
    //    DO_NOT_EXEC_CHECK_IND,  // stack performs submodule-check (i.e. calls check-indication, if expected submod != real submod)
    //    DO_EXEC_CHECK_IND       // no more submodule-check by stack, application has already set parameter isWrongSubmodule
    //} PNIO_CHECK_IND;
    ...
    typedef enum _PNIO_INP_UPDATE_STATE
    {
...
//--------------------------------------------------------------------------------------
//  statistic and link status information from EDD
//--------------------------------------------------------------------------------------
    typedef struct _PNIO_EDD_STATISTIC_TYPE
    {
	    PNIO_UINT32      PortID;         // port ID
	    PNIO_UINT32      RxTotalByte;    // total bytes received
	    PNIO_UINT32      RxTotalFrame;   // Total frames received
	    PNIO_UINT32      RxGoodFrame;    // Good frames received
	    PNIO_UINT32      RxUni;          // Unicast received
	    PNIO_UINT32      RxMul;          // Multicast received
	    PNIO_UINT32      RxBroad;        // Broadcast received
	    PNIO_UINT32      RxUnMul;        // Unknown Multicast received
	    PNIO_UINT32      RxUnUni;        // Unknown Unicast received
	    PNIO_UINT32      RxErrors;       // bad input frames
	    PNIO_UINT32      RxDiscard;      // dropped input frames

	    PNIO_UINT32      TxTotalByte;    // total bytes send
	    PNIO_UINT32      TxTotalFrame;   // total frames send
	    PNIO_UINT32      TxUni;          // Unicast send
	    PNIO_UINT32      TxMul;          // Multicast send
	    PNIO_UINT32      TxBroad;        // Broadcast send
	    PNIO_UINT32      TxCollision;    // Transmit collisions
	    PNIO_UINT32      TxErrors;       // bad output frames
	    PNIO_UINT32      TxDiscard;      // dropped output frames
    } PNIO_EDD_STATISTIC_TYPE;

    #define PNIO_EDD_STATISTICS_RESET_ALL   (LSA_UINT16)0xFFFF	  //  EDD_STATISTICS_RESET_ALL

    typedef struct _PNIO_EDD_LINKSTAT_TYPE
    {
	     PNIO_UINT32    PortID;         // port ID
         PNIO_UINT32	State;			// port link state EDD_LINK_UP, EDD_LINK_DOWN,...
         PNIO_UINT32 	Speed;			// port speed EDD_LINK_SPEED_10, EDD_LINK_SPEED_100,..
         PNIO_UINT32    Mode; 			// port link mode EDD_LINK_MODE_HALF, EDD_LINK_MODE_FULL,
    } PNIO_EDD_LINKSTAT_TYPE;


    #include "iodapi_rema.h"

// *---------------------------------------------------
// *    callback table (optional use)
// *---------------------------------------------------
// (no more supported in DK 4.x and newer)
...
#define PNIO_SUPPORT_IM12345  0x3e
    ...
    // *-------------------------------------------------------
    // * IM0 Filter Data structure must be filled for every
    // * submodule, that contains IM0 data.
    // * In this simple example we assume, that only the DAP
    // * supports IM0 Data. That means, we have only
    // * one subslot in the device, that supports IM0.
    // * More info you find in the pnio specification.
    // *-------------------------------------------------------
    typedef ATTR_PNIO_PACKED_PRE struct
    {
       REC_IO_BLOCKHDR   BlockHeader;	// BlockHeader
       PNIO_UINT16       NumOfApis;      // number of APIs
           // *---note: a list of APIs follows, here we have only one...
           PNIO_UINT32      Api;            // API number
           PNIO_UINT16      NumOfMod;       // number of submodules in the API
               // *---note: a list of slots follows, here we have only one...
               PNIO_UINT16      Slot;           // slot number
               PNIO_UINT32      ModId;          // module ID
               PNIO_UINT16      NumOfSubMod;    // number of submodules in the module
                   // *---note: a list of subslots follows, here we have only one...
                   PNIO_UINT16      SubSlot;        // subslot number
                   PNIO_UINT32      SubModId;       // submodule ID
    }  ATTR_PNIO_PACKED IM0_FILTER_DATA_STRUCT;
...
PNIO_EXTRN PNIO_UINT32 PNIO_setup
                            (PNIO_INT8	 *pStationName,     // unique station name
                             PNIO_UINT32 StationNameLen,    // length of station name (may not be zero terminated)
...
typedef struct
{
  PNIO_UINT32       Api;                  // number of entries in the list
  PNIO_ADDR_TYPE    Entry;                // Array of entries
...
typedef struct
{
    ...
    PNIO_IOCR_TYPE_ENUM   IoDir;        // IO direction (PNIO_IOCR_TYPE_INPUT, PNIO_IOCR_TYPE_OUTPUT)
    	...
    	PNIO_UINT32       IocrOffset;   // [in]  specifies offset in IOCR (for EB400 only), 0 = from start of iocr
	#endif
...
PNIO_VOID*  PNIO_rec_set_rsp_async (void);          // set asynchronous response mode (for 1 Request)
    ...
    // *** values for pSubstMode ***
    #define REC8029_SUBSTMOD_ZERO           0x0000  // substitute values are all ZERO
    #define REC8029_SUBSTMOD_LASTVAL        0x0001  // substitute values are last valid output data
    #define REC8029_SUBSTMOD_REPLACED       0x0002  // defined set of substitute values replace output data
...
#define PNIO_PACKID_ACP		    1
#define PNIO_PACKID_CLRPC	    2
#define PNIO_PACKID_CM		    3
#define PNIO_PACKID_DCP		    4
#define PNIO_PACKID_EDD		    5
#define PNIO_PACKID_GSY		    6
#define PNIO_PACKID_IOD		    7
#define PNIO_PACKID_LLDP	    8
#define PNIO_PACKID_MRP 	    9
#define PNIO_PACKID_NARE 	    10
#define PNIO_PACKID_OHA 	    11
#define PNIO_PACKID_OS		    12
#define PNIO_PACKID_SOCK	    13
#define PNIO_PACKID_BSPADAPT	14
#define PNIO_PACKID_REMA    	15
#define PNIO_PACKID_POF     	16
#define PNIO_PACKID_PNPB        17
#define PNIO_PACKID_PNDV	    18
#define PNIO_PACKID_LSAS	    19
#define PNIO_PACKID_TSKMA	    20
...
PNIO_EXTRN void PNIO_printf (	PNIO_INT8 * fmt, ...);

PNIO_EXTRN PNIO_UINT32 PNIO_sprintf(PNIO_UINT8* pBuf, PNIO_INT8 * fmt, ...);

PNIO_EXTRN PNIO_UINT32 PNIO_vsnprintf(PNIO_UINT8* pBuf, PNIO_UINT32 count, PNIO_INT8* fmt, PNIO_va_list argptr);

PNIO_EXTRN PNIO_UINT32 PNIO_snprintf (PNIO_UINT8* str, PNIO_UINT32 count, PNIO_UINT8* fmt, ...);

PNIO_EXTRN PNIO_UINT32 PNIO_vsprintf(PNIO_UINT8* pBuf, PNIO_INT8* fmt, PNIO_va_list argptr);

PNIO_EXTRN void PNIO_TrcPrintf(PNIO_INT8 * fmt, ...);

PNIO_EXTRN void PNIO_ConsolePrintf (PNIO_INT8 * fmt, ...);
PNIO_EXTRN void PNIO_ConsoleLog(PNIO_INT8 * fmt, ...);
...
PNIO_UINT32  PNIO_CP_set_opdone(void);

PNIO_UINT32  PNIO_get_HwCycInfoPtr  (PNIO_CYCLE_INFO_PTR* pHwCycInfo);
...
#ifndef SPI_IN_USE
#define SPI_IN_USE
extern unsigned char spi_in_use;
#endif
#endif
