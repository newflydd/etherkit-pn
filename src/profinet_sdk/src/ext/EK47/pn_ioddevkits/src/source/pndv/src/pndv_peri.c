...
/* contents:
    ...
    - pndv_in_peri_put_cfg_to_interface
    - pndv_in_peri_put_ds_to_interface
    - pndv_in_peri_def_para_one_slot
    - pndv_in_peri_def_para
    - pndv_in_peri_dial_quit
    - pndv_in_peri_pral_quit
    - pndv_in_peri_upal_quit
    - pndv_in_peri_ros_al_quit
    - pndv_in_peri_pull_al_quit
    - pndv_in_peri_plug_al_quit
...
{
    UnsignedOpt count;
    UnsignedOpt ar_cnt,elem_cnt;

    /* ---- INIT PNDV-PERI COUPLING DATA ---- */

  //  PNDV_MEMSET((void*)pndv_data.iface_ptr, 0, sizeof(PNDV_IFACE_STRUCT));
    ...
    for ( count = 0; count < (PNDV_MAX_ARS_RT + PNDV_MAX_ARS_DA) ; count++ )
    {
        ...
        pndv_data.iface_ptr->ar[count].red_int_info_used          = FALSE;
    }
...
void pndv_peri_sm( PNDV_SM_PERIBUS_EVENT event )
{
        ...
        case PNDV_SM_PERIBUS_STATE_UNKNOWN:
        {
            ...
            switch (event)
            {
                case PNDV_SM_PERIBUS_EVENT_OK:
                {
                    ...
                    pndv_ar_set_so_state(PNDV_SOL_EVENT_FLAG_PERIBUS_STATE, FALSE, PNDV_SOL_ALL_ARS);
                ...
                case PNDV_SM_PERIBUS_EVENT_ERROR:
                {
                    ...
                    pndv_ar_set_so_state(PNDV_SOL_EVENT_FLAG_PERIBUS_STATE, TRUE, PNDV_SOL_ALL_ARS);
        ...
        case PNDV_SM_PERIBUS_STATE_OK:
        {
            ...
            switch (event)
            {
                case PNDV_SM_PERIBUS_EVENT_ERROR:
                {
                    ...
                    tmp_ret_val = pndv_ar_set_so_state(PNDV_SOL_EVENT_FLAG_PERIBUS_STATE, TRUE, PNDV_SOL_ALL_ARS);
...
void pndv_in_peri_check_event ( PNDV_IFACE_CMD_ENTRY_T event)
{
    ...
    switch (event.cmd)
    {
        ...
        case PNDV_EV_TO_PNDV_PULL_AL:
        {
            ...
            pndv_pp_peri_submodule_remove(event.add_2, FALSE);
        ...
        case PNDV_EV_TO_PNDV_PULL_AL_MORE_FOLLOWS:
        {
            ...
            pndv_pp_peri_submodule_remove(event.add_2, TRUE);
            break;
        }
        ...
        case PNDV_EV_TO_PNDV_PLUG_AL:
        {
            ...
            pndv_pp_peri_submodule_add(event.add_2, FALSE);
        ...
        case PNDV_EV_TO_PNDV_PLUG_AL_MORE_FOLLOWS:
        {
            ...
            pndv_pp_peri_submodule_add(event.add_2, TRUE);
            break;
        }
        ...
        case PNDV_EV_TO_PNDV_CHANNEL_DIAG:
        {
            ...
            Unsigned16 entity_nr = event.add_2;
        ...
        case PNDV_EV_TO_PNDV_GENERIC_DIAG:
        {
            ...
            if ( PNDV_IFACE_SERVICE_NEW == pndv_data.iface_ptr->generic_diag_data[interface_index].state )
            {
                ...
                if ( TRUE == pndv_data.iface_ptr->generic_diag_data[interface_index].alm_kommend )
                {
        ...
        {
            // request to take ownership (TRUE) or to release ownership (FALSE)
            // add_1 = ar_index
            // add_2 = is_owner
...
PNDV_IFACE_CMD_ENTRY_T pndv_in_peri_read_coupl_event (void)
{
    ...
    if ( PNDV_SM_OPEN <= pndv_data.sm )
    {
        if ( pndv_data.iface_ptr->cmd.read_ptr_peri_to_pndv != pndv_data.iface_ptr->cmd.write_ptr_peri_to_pndv )
        {
            ...
            /* Field is 256 bytes long and index is Unsigned8 */
...
{
    UnsignedOpt write_ptr_next = pndv_data.iface_ptr->cmd.write_ptr_pndv_to_peri + 1;
...
void pndv_in_peri_put_ds_to_interface (CM_UPPER_RQB_PTR_TYPE ds_ptr )
{
    ...
    Unsigned16                  soll_slot,tmp_subslot_nr;
    ...
    else
    {
        ...
        if (   ( (Unsigned32)ds_ptr <  (Unsigned32)&glob_coupling_interface )
            || ( (Unsigned32)ds_ptr > ((Unsigned32)(&glob_coupling_interface) + sizeof(glob_coupling_interface)) )
           )
        {
        ...
        tmp_event.add_1 = pndv_ar_get_ar_idx_by_session_key(tmp_ds_ptr->args.pd.event->session_key);
...
void pndv_in_peri_dial_quit ( Unsigned16 entity_nr, Unsigned8 ret_val_to_peri )
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_in_peri_xdial_quit ( Unsigned16 entity_nr, Unsigned8 ret_val_to_peri )
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_in_peri_pral_quit (Unsigned16 entity_nr, Unsigned8 ret_val_to_peri)
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_in_peri_upal_quit ( Unsigned16 entity_nr )
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_in_peri_ural_quit ( Unsigned16 entity_nr )
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_in_peri_ros_al_quit ( Unsigned16 entity_nr )
{
    PNDV_IFACE_CMD_ENTRY_T tmp_event;
...
void pndv_peri_read_write_record (void *req_ptr)
{
    ...
    {
        record_to_peri = TRUE; /* send all records to peri */

        if ( TRUE == record_to_peri )
        {
               ...
               )
            {
                ...
                if ( 0 != data_length )
                {
                    ...
                    Unsigned32  length_rem;
                    Unsigned32  length_new;
                ...
                if( (new_prm) && (pndv_data.rema_station_prm_ptr != 0) )
                {
                    ...
                    if (PNDV_MAX_RECORD_GLOB_PARAM < (bytes_to_copy+sizeof(CM_EVENT_TYPE)+sizeof(CM_RQB_TYPE)))
                    {
                        /* the new data is too big, so we'd rather delete everything */
                        PNDV_WRITE_REMA_STATION_PRM_TO_FLASH(0);
                    }
                    else
                    {
                        PNDV_WRITE_REMA_STATION_PRM_TO_FLASH(bytes_to_copy+sizeof(CM_EVENT_TYPE)+sizeof(CM_RQB_TYPE));
                            /* Changed station parameters 
                               -> other existing ARs must be canceled in order to start up again 
                               with new station parameters */
                    }
