...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT16 OHA_FreeMgm(LSA_VOID)
{
...
#ifdef OHA_CFG_USE_DNS               /* only one DNS-channel supported! */
        OHA_GET_HDNS_PTR()->DnsState = OHA_HANDLE_STATE_FREE;
        OHA_GET_HDNS_PTR()->ThisHandle = (LSA_HANDLE_TYPE) (0 | OHA_HANDLE_TYPE_DNS);
#endif
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 oha_init_defvals(OHA_DB_IF_PTR_TYPE pDB)
{
    LSA_UINT16             Status = OHA_OK;
    LSA_UINT16             i,j;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_init_stats_info(OHA_OID mib_node)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 oha_undo_init(LSA_VOID)
{
    LSA_UINT16  Status;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#if 0
LSA_UINT16 OHA_UserCheckHDB(LSA_VOID)
{
    LSA_UINT16                Status  = OHA_OK;
    OHA_HDB_USER_PTR_TYPE     pHDB    = OHA_GET_HUSER_PTR(0);
    LSA_UINT32                i       = 0;
    ...
    while (i < OHA_CFG_MAX_CHANNELS)
    {
        if ( pHDB->State == OHA_HANDLE_STATE_OPEN )
        {
            if (OHA_IS_NOT_NULL(pHDB->Params.pDetail) &&
				pHDB->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
				pHDB->Params.pDetail->Params.User.DcpSetControl)  /* controlled by User ? */
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_DB_IF_PTR_TYPE OHA_DB_GET_INTERFACE(
    LSA_UINT16 NICId)
{
...
)
{
    ...
    while ( (! Found ) && ( i < OHA_CFG_MAX_NICS) )
    {
        if (!pDB->Existing)   /* free interface */
        {
            Found = LSA_TRUE;
            pDB->NicId = NICId; /* write NicId to interface */
            pDB->Existing = LSA_TRUE;
            Status = LSA_RET_OK;
        }
        else
        {
			if (pDB->NicId == NICId)
			{
				...
				OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "oha_init_interface failed: OHA_DB_WRITE_IP_SUITE failed (NICId: 0x%X already used).", NICId);
				break;
			}
            pDB++;
            i++;
	...
	if(Found)
	{
        ...
        Status = OHA_DB_WRITE_PROFINET_ROLE(NICId, ProfinetRole); /* 0 = no role, 1 = Profinet IO */
        if (Status != LSA_RET_OK)
        {
		...
		if (Status != LSA_RET_OK) { OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ); }
...
/*=============================================================================
* function name:  oha_init_dhcp_rtf
*
* function:       initialization of DHCP ResetToFactory parameters
*                 (options 61 and 255)
*
* parameters:     ...
*
...
*                              or LSA_RET_ERR_PARAM
*===========================================================================*/
LSA_UINT16 oha_init_dhcp_rtf(
	LSA_UINT16 NICId,
	LSA_UINT8  AccCtrl,       /* OHA_READ_WRITE, OHA_QUERY_WRITE */
	LSA_UINT8  DHCPOption,    /* see DHCPParameters record       */
							  /* OptionCode (Bit 0 ・6):         */
	                          /* Value 0x01: use MAC Address     */
							  /*       0x02: use NameOfStation   */
							  /*       0x03: use ClientID        */
							  /* DHCPDisabled (Bit 7):           */
							  /* Value 0x00: enable DHCP client  */
							  /*       0x01: disable DHCP client */
	OHA_COMMON_MEM_U8_PTR_TYPE ClientIDValue, /* Buffer for the ClientID */
	LSA_UINT16 ClientIDLength /* Length of ClientID, range 0 ・0xff */
)
{
	OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NICId);
	LSA_UINT16          Status = LSA_RET_OK;
	LSA_UINT8           OptionCode = DHCPOption & 0x7f; /* Bit 7: 0 ... use DHCP, 1 ... don't use DHCP */

	if (OHA_IS_NULL(pDB))
	{
		Status = LSA_RET_ERR_PARAM;
		OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "oha_init_dhcp_rtf failed. No database present (NIC-ID: 0x%X)", NICId);
		return(Status);
	}

	/* (i) check the rtf parameters */
	if (AccCtrl != OHA_READ_WRITE && AccCtrl != OHA_QUERY_WRITE)
	{
		Status = OHA_ERR_PARAM;
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_init_dhcp_rtf: AccCtrl (0x%X) failed (Status: 0x%X)", AccCtrl, Status);
		return(Status);
	}
	...
	{
		Status = OHA_ERR_PARAM;
		OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_init_dhcp_rtf: OptionCode (%u) out of range.", OptionCode);
		return(Status);
	}

	if ((OptionCode == 1 && ClientIDLength != 0) || /* use MAC Address */
		(OptionCode == 2 && ClientIDLength != 0) || /* use NameOfStation */
		(OptionCode == 3 && (ClientIDLength == 0 || ClientIDLength > 0xff)))   /* use ClientID */
	{
		Status = OHA_ERR_PARAM;
		OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_init_dhcp_rtf: Not matching of OptionCode (%u) and ClientIDLength (%u)!", OptionCode, ClientIDLength);
		return(Status);
	}

	if (ClientIDLength && OHA_IS_NULL(ClientIDValue))   /* use ClientID */
	{
		Status = OHA_ERR_PARAM;
		OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_init_dhcp_rtf: No ClientID (0x%x) given, ClientIDLength (%u)!", ClientIDValue, ClientIDLength);
		return(Status);
	}

	/* (ii) store the rtf parameters */
	pDB->DcpData.DHCPRtFParams.AccCtrl = AccCtrl;
	pDB->DcpData.DHCPRtFParams.DHCPOption = DHCPOption; /* see DHCPParameters record */
	pDB->DcpData.DHCPRtFParams.ClientIDLen = ClientIDLength; /* Length of ClientID, range 0 ・0xff */
	if (OHA_IS_NOT_NULL(pDB->DcpData.DHCPRtFParams.ClientIDValue))
	{
		LSA_UINT16 FreeStat;
		OHA_FREE_LOCAL_MEM(&FreeStat, pDB->DcpData.DHCPRtFParams.ClientIDValue);
		OHA_MEM_FREE_FAIL_CHECK(FreeStat);
		pDB->DcpData.DHCPRtFParams.ClientIDValue = LSA_NULL;
	}
	if (ClientIDLength)
	{
		OHA_LOCAL_MEM_PTR_TYPE  pVarTemp = LSA_NULL;    /* variable in octets */

		OHA_ALLOC_LOCAL_MEM(&pVarTemp, ClientIDLength);  /* alloc the new buffer for the variable */
		if (LSA_HOST_PTR_ARE_EQUAL(pVarTemp, LSA_NULL))  /* failed the allocation */
		{
			OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_init_dhcp_rtf(). Allocating memory failed.");
			Status = LSA_RET_ERR_RESOURCE;
			return Status;
		}
		pDB->DcpData.DHCPRtFParams.ClientIDValue = (OHA_COMMON_MEM_U8_PTR_TYPE)pVarTemp;
		OHA_MEMCPY(pDB->DcpData.DHCPRtFParams.ClientIDValue, ClientIDValue, ClientIDLength);
	}

	/* (iii) write the rtf parameters to the database */
	Status = oha_write_dhcp_rtf(NICId);

	return(Status);
}

/*=============================================================================
* function name:  oha_write_dhcp_rtf
*
* function:       writes the DHCP ResetToFactory parameters
*                 (options 61 and 255) to the database
*
* parameters:     ...
*
* return value:   LSA_UINT16:  LSA_RET_OK
...
)
{
	...
	{
		Status = LSA_RET_ERR_PARAM;
		OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "oha_write_dhcp_rtf failed. No database present (NIC-ID: 0x%X)", NICId);
		return(Status);
	}

	OHA_ASSERT(pDB->DcpData.DHCPRtFParams.AccCtrl == OHA_READ_WRITE ||
				pDB->DcpData.DHCPRtFParams.AccCtrl == OHA_QUERY_WRITE);
	{
		/* see DHCPParameters record:      */
		...
		LSA_UINT8      OptionCode = pDB->DcpData.DHCPRtFParams.DHCPOption & 0x7f; /* Bit 7: 0 ... use DHCP, 1 ... don't use DHCP */
		LSA_UINT16     ClientIDLen = pDB->DcpData.DHCPRtFParams.ClientIDLen;   /* Length of ClientID, range 0 ・0xff */
		...
		LSA_UINT16 DhcpDataLen = (LSA_UINT16)(3 + ClientIDLen);
		...
		{
			Status = LSA_RET_ERR_RESOURCE;
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_write_dhcp_rtf: Getting DhcpData memory failed (Status=0x%X).", Status);
			return(Status);
		}
		...
		*pDhcpData = (OptionCode == 1) ? (LSA_UINT8)1 : (LSA_UINT8)0;   /* Type  */
		...
		if (ClientIDLen)
		{                           /* Client-Identifier   */
			OHA_MEMCPY(pDhcpData, pDB->DcpData.DHCPRtFParams.ClientIDValue, ClientIDLen);
		}

		pDhcpData = (OHA_COMMON_MEM_U8_PTR_TYPE)pVarTemp; /* rewind */

		/* finally write the DHCP params to the database */
		Status = OHA_DB_WRITE_DCP_ELEMENT(NICId, OHA_DCP_OPTION_DHCP, 61, /* first byte = Id, suboption */
											pDhcpData, DhcpDataLen, pDB->DcpData.DHCPRtFParams.AccCtrl, LSA_FALSE);
		if (Status != LSA_RET_OK)
		{
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_write_dhcp_rtf: Setting DHCP-Params (61) failed (Status=%u).", Status);
		}
		...
		pDhcpData = (OHA_COMMON_MEM_U8_PTR_TYPE)pVarTemp; /* re-init */
		...
		*pDhcpData = (LSA_UINT8)1;     /* Len   */
		pDhcpData++;
		*pDhcpData = (pDB->DcpData.DHCPRtFParams.DHCPOption & 0x80)/*disable DHCP client*/ ? (LSA_UINT8)0/*don't use DHCP*/ : (LSA_UINT8)2;

		pDhcpData = (OHA_COMMON_MEM_U8_PTR_TYPE)pVarTemp; /* rewind again */

		/* finally write the next DHCP params to the database */
		Status = OHA_DB_WRITE_DCP_ELEMENT(NICId, OHA_DCP_OPTION_DHCP, 255, /* first byte = Id, suboption */
											pDhcpData, DhcpDataLen, pDB->DcpData.DHCPRtFParams.AccCtrl, LSA_FALSE);
		if (Status != LSA_RET_OK)
		{
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH, "oha_write_dhcp_rtf: Setting DHCP-Params (255) failed (Status=%u).", Status);
		}
	...
	/* this initial values will get active, if not they are overwritten otherwise */
	pDB->DcpData.DHCPRtFParamsActive = LSA_TRUE;
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_PORT_NAME(
    LSA_UINT16                    NicId,
    LSA_UINT16                    PortId,
...
)
{
    ...
    if (OHA_IS_NOT_NULL(pDB))
    {
        if ((NameLen && OHA_IS_NOT_NULL(NamePtr) && (NameLen <= OHA_MAX_PORT_STRLEN)) &&
            (PortId && PortId <= OHA_CFG_MAX_PORTS))
        {
            /* OHA_COMMON_MEM_U8_PTR_TYPE *ppName = &pDB->LldpData[PortId-1].pLocalPortName; */

            if (OHA_OK == oha_check_port_name((LSA_UINT16)NameLen, NamePtr) ? LSA_TRUE : LSA_FALSE)
            {
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_CHASSIS_NAME(
    OHA_COMMON_MEM_U8_PTR_TYPE    NamePtr,
    LSA_INT                       NameLen
)
{
    ...
    if (NameLen <= OHA_MAX_CHASSIS_STRLEN)
    {
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_CHASSIS_NAME(
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_SYS_NAME(
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_SYS_NAME(
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_SYS_DESCR(
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_IF_DESCR (
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
 ...
 * return value:    LSA_UINT16      Error:      != LSA_RET_OK if name cannot be written
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_SYS_DESCR (
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_SYS_CONTACT (
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_SYS_CONTACT(
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_SYS_LOCATION (
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_SYS_LOCATION(
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_SYS_CAP (
 ...
 * return value:    LSA_UINT16      Error:      != LSA_RET_OK if name cannot be written
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_IF_DESCR (
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
...
)
{
	...
	OHA_ASSERT_NULL_PTR(AddrPtrPtr);
 ...
 * return value:   LSA_INT:                             Length
 *===========================================================================*/
LSA_INT OHA_DB_READ_STATION_TYPE(
    LSA_UINT16                   NicId,
...
)
{
	...
	OHA_ASSERT_NULL_PTR(TypePtrPtr);
...
extern LSA_VOID EDDP_LL_SetStationNameInfo(LSA_UINT8 *pName,LSA_INT NameLen);
#endif /* IOD_INCLUDE_STORM_FILTER_DCP_SW */
LSA_INT OHA_DB_READ_STATION_NAME(
    LSA_UINT16  NicId,
...
#if IOD_INCLUDE_STORM_FILTER_DCP_SW // Renesas RZT2
        // Call to pre-check station name in EDD
        EDDP_LL_SetStationNameInfo(pType,Length);
#endif
        return Length;
    }
    *NamePtrPtr = LSA_NULL;
 ...
 *===========================================================================*/
LSA_UINT8 OHA_DB_READ_STATION_ID(
    LSA_UINT16                   NicId,
 ...
 *                                              if not present
 *===========================================================================*/
LSA_UINT8 OHA_DB_READ_OEM_DEVICE_ID(
    LSA_UINT16                   NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE * IdPtrPtr
)
{
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_TCPIP_STATUS(
    LSA_UINT16  NicId,
    LSA_UINT16  Status
)
{
 ...
 * return value:   LSA_INT:                             Length
 *===========================================================================*/
LSA_INT OHA_DB_READ_SUPPORTED_DATABASE(
    LSA_UINT16             NicId,
 ...
 *                                              0 if no role
 *===========================================================================*/
LSA_UINT8 OHA_DB_READ_PROFINET_ROLE(
    LSA_UINT16  NicId
)
{
    LSA_BOOL   Remanent;
 ...
 *                                          cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_DEVICE_INITIATIVE(
    LSA_UINT16      NicId,
 ...
 *              DCP_STATUS_TCPIP_RESERVED - 0x0003-0x00ff: Reserviert
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_TCPIP_STATUS(
    LSA_UINT16  NicId
)
{
    OHA_DB_IF_PTR_TYPE  pDB;
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_MAC_ADDR(
    LSA_UINT16   NicId,
    OHA_MAC_TYPE AddrPtr,
    LSA_INT      AddrLen,
    LSA_UINT8    AccessControl,
    LSA_BOOL     Remanent
)
{
    ...
    if (AddrLen == 0 || (OHA_IS_NOT_NULL(AddrPtr) && AddrLen == (LSA_INT)sizeof(OHA_MAC_TYPE)))
    {
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_PROFINET_ROLE(
    LSA_UINT16      NicId,
    LSA_UINT8       role
)
{
 ...
 *                                          cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_DEVICE_INITIATIVE(
    LSA_UINT16      NicId,
...
)
{
    ...
    switch (HelloMode & OHA_DCP_HELLO_MODE_MASK)
    {
        ...
        default:
			...
			OHA_PROGRAM_TRACE_01(pHDBDcp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_DB_WRITE_DEVICE_INITIATIVE failed. Invalid mode (HelloMode: 0x%X)",HelloMode);
            return Ret;
    }
    ...
    LSA_BOOL    Remanent )
{
    ...
    if (OHA_IS_NOT_NULL(pDB))
    {
        ...
        if (AddrLen == 0 || AddrLen == OHA_IP_ADDR_SIZE)
        {
            ...
            if (OHA_OK == oha_check_ip_address(OHA_GET32_NTOH(&IpSuite, 0),
 ...
 *
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_STATION_ID(
    LSA_UINT16   NicId,
    LSA_UINT8  * IdPtr,
    LSA_INT      IdLen,
    LSA_UINT8    AccessControl,
 ...
 * +---------------+------------------------------------------------------+
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_OEM_DEVICE_ID(
    LSA_UINT16   NicId,
    LSA_UINT8  * IdPtr,
    LSA_INT      IdLen,
    LSA_UINT8    AccessControl,
    ...
    if (NameLen <= OHA_MAX_STATION_NAME_LEN)
    {
        if (NameLen == 0)   /* AP00257533: write an empty string */
        {
            ...
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE,
                                  "OUT: OHA_DB_WRITE_STATION_NAME(NicId: 0x%X, RetVal: 0x%X)", NicId, Ret);
            return Ret;
        }

        OHA_ASSERT_NULL_PTR(NamePtr);
        OHA_MEMCPY(pConvertName, NamePtr, (LSA_UINT32)NameLen); /* AP00352120: store olny in lower case */
        ...
        if ((OHA_OK == oha_check_station_name((LSA_UINT16)NameLen, NamePtr, 
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_STATION_TYPE(
    LSA_UINT16                    NicId,
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_DHCP_OPTION(
    LSA_UINT16                  NicId,
...
)
{
	...
	OHA_ASSERT_NULL_PTR(OptionPtrPtr);
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_DHCP_OPTION(
    LSA_UINT16   NicId,
    LSA_UINT8  * OptionPtr
)
{
...
/*------------------------------------------------------------------------------------*/
static LSA_VOID oha_strreverse(OHA_COMMON_MEM_U8_PTR_TYPE begin, OHA_COMMON_MEM_U8_PTR_TYPE end)
{
	LSA_UINT8 aux;

	OHA_ASSERT_NULL_PTR(begin);
	OHA_ASSERT_NULL_PTR(end);
...
/*------------------------------------------------------------------------------------*/
static LSA_INT oha_ultoa(LSA_UINT32 value, OHA_COMMON_MEM_U8_PTR_TYPE str, LSA_UINT base, LSA_INT numbers)
{
	LSA_INT count = 0;
	static const char num[] = "0123456789abcdef";
	OHA_COMMON_MEM_U8_PTR_TYPE wstr=str;

	OHA_ASSERT_NULL_PTR(wstr);
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 oha_mac2a (const OHA_COMMON_MEM_U8_PTR_TYPE mac,
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 oha_ip2a (const OHA_COMMON_MEM_U8_PTR_TYPE ip,
...
/*----------------------------------------------------------------------------*/
LSA_INT oha_oid2a (const OHA_OID_PTR_TYPE oid, LSA_UINT16 oidlen,
...
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_ALIAS_NAME_CHECK(NicId: 0x%X, AliasPtr: 0x%X, AliasLen: 0x%X)",
                          NicId, AliasPtr, AliasLen);

    if (AliasLen == 0 || OHA_IS_NULL(AliasPtr)|| OHA_IS_NULL(pDB))
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                              "OUT: OHA_DB_ALIAS_NAME_CHECK(NicId: 0x%X, Found: 0x%X)", NicId, Found);
        return Found;
    }
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_DEVICE_INSTANCE(
    LSA_UINT16                    NicId,
    ...
    LSA_BOOL                      Remanent)
{
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_WRITE_DEVICE_INSTANCE(NicId: 0x%X, DeviceInstance: 0x%X, AccessControl: 0x%X, Remanent: 0x%X)",
                          NicId, DeviceInstance, AccessControl, Remanent);
 ...
 * +---------------------+---------------------------------------------------+
 *===========================================================================*/
LSA_UINT8 OHA_DB_READ_DEVICE_INSTANCE(
    LSA_UINT16                    NicId,
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_PROFIBUS_ADDITION(
    LSA_UINT16                      NicId,
 ...
 *                                          parameters cannot be changed
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_PROFIBUS_COMMON(
    LSA_UINT16                       NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE       ParamPtr,
	LSA_BOOL                         IsRema
)
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_PROFIBUS_COMMON(
    LSA_UINT16                 NicId,
...
)
{
	...
	OHA_ASSERT_NULL_PTR(ParamPtrPtr);
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_PROFIBUS_ADDITION(
    LSA_UINT16                NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE* ParamPtrPtr
)
{
...
)
{
    ...
    if (len == TypeLen)
    {
        Ret = OHA_MEMCMP(p_station_type, TypePtr, (LSA_UINT32)len);
    }
    /* DCP_SERVER_PBC */
    if (!Ret)
    {
 ...
 * return value:   LSA_INT:                             Length
 *===========================================================================*/
LSA_BOOL OHA_UPPER_CASES_ALLOWED(
    LSA_UINT16  NicId
)
{
