    ...
    unsigned    intsize)    /* IN - size of output buffer */
{
...
#if 1 /* check valid buffer length, see 2374779 */
	if (0 == *datalength)
	{
		return LSA_NULL;
	}
...
#if 1 /* check valid buffer length, see 2374779 */
    bufp = oha_asn_parse_length(bufp, *datalength - 1, &asn_length);
#else
    ...
    *datalength -= asn_length + (unsigned)(bufp - data);
    ...
    while (asn_length--)
        value = (LSA_INT32)((LSA_UINT32)value << 8) | *bufp++;
    ...
    unsigned    intsize) /* IN - size of *intp */
{
    ...
    if (intsize == sizeof (LSA_INT32))
    {
        ...
        {
            intsize--;
            integer = (LSA_INT32)((LSA_UINT32)integer << 8);
        }
    }
    else
    {
        integer = (LSA_INT32)((LSA_UINT32)integer << (8*(sizeof(LSA_INT32) - intsize)));
    }
    ...
    *datalength -= intsize;
    ...
    while (intsize--)
    {
        *data++ = (U_CHAR)(((U_LONG)integer & mask) >> (8 * (sizeof(LSA_INT32) - 1)));
        integer = (LSA_INT32)((LSA_UINT32)integer << 8);
    }
...
/* FUNCTION: asn_build_unsigned_int()
 *
 * asn_build_int - builds an ASN object containing an integer.
 * On entry, datalength is input as the number of valid bytes following
  */
U_CHAR * oha_asn_build_unsigned_int(U_CHAR* data,
                           unsigned* datalength,
                           U_CHAR type, const LSA_UINT32* intp, unsigned intsize)
{
   /*
    * ASN.1 integer ::= 0x02 asnlength byte {byte}*
    */
   ...
   /* mask = ((u_long) 0xFF) << (8 * (sizeof(LSA_INT32) - 1)); */
   /*
    * mask is 0xFF000000 on a big-endian machine
    */
    mask = 0xff000000L;  /* should work on all machines */
   if ((U_CHAR) ((integer & mask) >> (8 * (sizeof(LSA_INT32) - 1))) & 0x80)
   {
      /*
       * if MSB is set
       */
   ...
   else
   {
      ...
      */
       ...
       * mask is 0xFF800000 on a big-endian machine
       */
      mask = 0xFF800000;  /* should work on all machines */
    ...
    unsigned *  strlength)  /* IN/OUT - size of output buffer */
{
    ...
    *type = *bufp++;       /* type should contain the type (0x04 = octet string) */
    ...
    if (asn_length + (unsigned)(bufp - data) > *datalength)
    {
        OHA_SNMPERROR("oha_asn_parse_length(): overflow of message");
        OHA_SNMP_MIB_CTR(snmpInASNParseErrs);
        return LSA_NULL;
    }
    if (asn_length > *strlength)
    {
        OHA_SNMPERROR("oha_asn_parse_length(): I don't support such long strings");
        OHA_SNMP_MIB_CTR(snmpInASNParseErrs);
        return LSA_NULL;
    }
    OHA_MEMCPY((char *)string, (char *)bufp, (LSA_UINT32)asn_length);
    *strlength = asn_length;
    *datalength -= asn_length + (unsigned)(bufp - data);
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_asn_parse_length(bufp + asn_length: 0x%X)", (bufp + asn_length));
    ...
    unsigned *  strlength)  /* IN/OUT - size of output buffer */
{
    ...
    *type = *bufp++;
    ...
    OHA_MEMCPY((char *)string, (char *)bufp+1, (LSA_UINT32)asn_length-1);
    ...
    U_CHAR *    type)       /* OUT - ASN type of object */
{
    ...
    *type = *bufp;
#if 1 /* check valid buffer length, see 2374779 */
    bufp = oha_asn_parse_length(bufp + 1, *datalength -1, &asn_length);
#else
...
U_CHAR *  oha_asn_parse_length(
	...
	unsigned datalength, /* pass valid buffer length to parse function, see 2374779 */
...
#if 1 /* check valid buffer length, see 2374779 */
	  if (lengthbyte > datalength-1)
	  {
		OHA_SNMPERROR("oha_asn_parse_length(): insufficient valid buffer length");
		OHA_SNMP_MIB_CTR(snmpInASNParseErrs);
		return LSA_NULL;
	  }
#endif

        OHA_MEMCPY((char *)length, (char *)data + 1, lengthbyte);
...
#if 1 /* length cannot be longer than maximum packet length, see 2374779 */
		if (*length >= OHA_SNMP_SIZE)
		{
            OHA_SNMPERROR("oha_asn_parse_length(): length exceeds max SNMP packet length");
            OHA_SNMP_MIB_CTR(snmpInASNParseErrs);
            return LSA_NULL;
		}
#endif
    ...
    unsigned *  objidlength)   /* IN/OUT - number of sub-id's in objid */
{
    ...
    *datalength -= asn_length + (unsigned)(bufp - data);
    ...
    length = (LSA_INT32)asn_length;    /* number of bytes in encoded image of object Id */
    while (length > 0)
    {
        subidentifier = 0;
        do
        {
#if 1 /* prevent reading beyond OID end, see 2374779 */
		if (0 == length)
		{
			return LSA_NULL; /* bufp has passed end of malformed OID, see while clause below */
		}
#endif
    ...
    unsigned    cpsize)     /* IN - size of *cp  = 8 bytes */
{
    ...
    data = oha_asn_build_header(data, datalength, type, (unsigned)(8-startindex));
    if (data && ((int)*datalength > (8-startindex)))
    {
        OHA_MEMCPY((char *)data, &(cp->s[startindex]), (LSA_UINT32)(8-startindex));
        *datalength -= (unsigned)(8-startindex);
