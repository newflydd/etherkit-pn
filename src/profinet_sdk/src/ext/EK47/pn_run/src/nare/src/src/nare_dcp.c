...
/*                                forward declaration                        */
/*===========================================================================*/
static LSA_VOID NARE_DCPCancel_done(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_CONST_PTR_TYPE        pHDB);
...
static NARE_RSP NARE_DCPNotifyRQBProvide(
    NARE_HDB_DCP_PTR_TYPE         pHDB)
{
    ...
    Status             = NARE_RSP_OK;
    pHDB->NotifyDCPCnt = 0;
    UserId.uvar32      = 0; /* not really used */
    i = 0;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_HDB_DCP_PTR_TYPE NARE_DCPGetHandleByNIC(
    LSA_UINT16 NICId)
{
    NARE_HDB_DCP_PTR_TYPE pHDB;
    LSA_UINT32            i;
    ...
    LSA_SYS_PTR_TYPE       pSys)
{
    ...
    if ( Found )
    {
        ...
        NARE_ALLOC_LOCAL_MEM((NARE_LOCAL_MEM_PTR_TYPE *)&pHDB->DCPRCtrl.pTable,DSize);
        ...
        {
            /* preallocated RQB used with HELLO-requets */
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static NARE_DCP_RCTRL_PTR_TYPE NARE_DCPGetRCTRLByIdx(
    NARE_HDB_DCP_CONST_PTR_TYPE pHDB,
    LSA_UINT32            Idx)
{
    NARE_DCP_RCTRL_PTR_TYPE  pRCtrl;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_DCP_RCTRL_PTR_TYPE NARE_DCPGetFreeRCTRL(
    NARE_HDB_DCP_PTR_TYPE pHDB)
{
    NARE_DCP_RCTRL_PTR_TYPE  pRCtrl;
    ...
    NARE_DCP_RCTRL_PTR_TYPE   pRCtrl)
{
    ...
    switch (*pRCtrlUser->pRState)
    {
        case NARE_RCTRL_RSTATE_DCP_IDENT:
            ...
            NARE_ASSERT_NULL_PTR(pRCtrlUser->pRQB->args.Ident.pStationName);
            Status = NARE_DCPIdentify(pHDB,
                                    pRCtrl,
                                    pRCtrlUser->pRQB->args.Ident.pStationName,
                                    pRCtrlUser->pRQB->args.Ident.StationNameLen,
                                    pRCtrlUser->pRQB->args.Ident.RequestID);
            break;
        ...
        case NARE_RCTRL_RSTATE_DCP_SET:
            ...
            NARE_PROGRAM_TRACE_05(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "DCP[%X:%d]-Calling DCP_SET(). RequestID= %d [0x%X] [Mode: 0x%x]",
                pHDB->ThisHandle, pRCtrl->ThisIdx, pRCtrl->pRCtrl->pRQB->args.Set.RequestID, pRCtrl->pRCtrl->pRQB->args.Set.RequestID,
                pRCtrlUser->DCPSetMode);
            switch (pRCtrlUser->DCPSetMode)       /* set only IP-Params */
            {
                case NARE_RCTRL_DCP_SET_IP:
                    ...
                    Status = NARE_DCPSet(pHDB,
                        pRCtrl,
                        &pRCtrl->pRCtrl->pRQB->args.Set.MACAddr,
                        (NARE_UPPER_RQB_IP_CONST_PTR_TYPE)&pRCtrl->pRCtrl->pRQB->args.Set.IPAddr,
                        (NARE_UPPER_RQB_IP_CONST_PTR_TYPE)&pRCtrl->pRCtrl->pRQB->args.Set.SubMask,
                        (NARE_UPPER_RQB_IP_CONST_PTR_TYPE)&pRCtrl->pRCtrl->pRQB->args.Set.DefRouter,
                        LSA_NULL,
                        0,
                        pRCtrl->pRCtrl->pRQB->args.Set.SetIPRemanent == NARE_SET_IP_REMANENT_YES ? LSA_TRUE : LSA_FALSE,
                        pRCtrl->pRCtrl->pRQB->args.Set.RequestID);
                    break;

                case NARE_RCTRL_DCP_SET_STATIONNAME:      /* set only StationName */
                    ...
                    Status = NARE_DCPSet(pHDB,
                        ...
                        LSA_NULL,
                        LSA_NULL,
                        (NARE_UPPER_MEM_U8_PTR_TYPE)pRCtrl->pRCtrl->pRQB->args.Set.pStationName,
                        pRCtrl->pRCtrl->pRQB->args.Set.StationNameLen,
                        pRCtrl->pRCtrl->pRQB->args.Set.SetIPRemanent == NARE_SET_IP_REMANENT_YES ? LSA_TRUE : LSA_FALSE,
                        pRCtrl->pRCtrl->pRQB->args.Set.RequestID);
                    break;

                case NARE_RCTRL_DCP_SET_ALL:            /* set IP-Params and StationName */
                ...
                default:   /* nothing to set. Not expected  */
...
/*+               pending-queue.                                            +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID NARE_DCPCheckAndStartPending(
    NARE_HDB_DCP_PTR_TYPE     pHDB)
{
    NARE_USER_RCTRL_PTR_TYPE  pRCtrlUser;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_DCPOpenChannelDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_RSP                 Response;
...
static LSA_VOID NARE_DCPCloseChannelDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_RSP                 Response;
...
/*+  pIdentifiedStationName :   Pointer to identified StationName  (output) +*/
/*+  IdentifiedStationNameLen:  Length of identified StationName   (output) +*/
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL NARE_DCPAnalyseIdentifyRspTLV(
    DCP_USR_NODE_PTR                  pNode,
    LSA_UINT8                        *pMAC,
    LSA_UINT8                        *pIP,
    LSA_UINT8                        *pSubMask,
    LSA_UINT8                        *pDefRouter,
    NARE_UPPER_MEM_U8_CONST_PTR_TYPE  pStationName,
    LSA_UINT32                        StationNameLen,
    NARE_UPPER_MEM_U8_PTR_TYPE        pIdentifiedStationName,
    LSA_UINT8                        *IdentifiedStationNameLen,
    LSA_UINT32                       *pStationNameState,
            ...
            case DCP_TYPE_DEVICE_NAME:
                ...
                if ( ! (Found & 2))
                {
                        ...
                        (! LSA_HOST_PTR_ARE_EQUAL(pStationName, LSA_NULL)))  /* same length */
                    {
                        ...
                        StationOK = LSA_TRUE;
                        a = 0;
                        while ((a < StationNameLen ) && ( StationOK ))
                        {
                            if (NARE_TO_LOWER(pTLV->Data.NameOfStation.Name[a]) != NARE_TO_LOWER(pStationName[a]))
                            {
                                StationOK = LSA_FALSE;
                            }
                            a++;
                        ...
                        *IdentifiedStationNameLen = (LSA_UINT8)(Len - sizeof(pTLV->Status));
                        /* copy the returned station name */
                        for (a = 0; a < *IdentifiedStationNameLen; a++)
                        {
                            *((LSA_UINT8 *)pIdentifiedStationName + a) = pTLV->Data.NameOfStation.Name[a];
                        }
                    }
                    else
                    {
                        ...
                        else if (Len > (240 + sizeof(pTLV->Status)))  // station name length longer than 240 bytes
                        {
                            *IdentifiedStationNameLen = (LSA_UINT8)(Len - sizeof(pTLV->Status));
                            NARE_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "DCP-Identfiy: Identified station name length is longer than 240 Bytes!");
                            Result = LSA_FALSE;
                        }
                        else
                        {
                            NARE_PROGRAM_TRACE_00(0,LSA_TRACE_LEVEL_NOTE,"DCP-Identfiy: Identified station name is different!");
                            /* Station name set but different */
                            *pStationNameState = NARE_RCTRL_DCP_STATION_NAME_SET_DIFFERENT;
                            *IdentifiedStationNameLen = (LSA_UINT8)(Len - sizeof(pTLV->Status));
                            /* copy the returned station name */
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_DCPIdentifyDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_RSP                 Response;
    ...
    switch (NARE_DCP_RQB_GET_RESPONSE(pRQB))
    {
        case DCP_RSP_OK_CANCEL:
            NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Identify was canceled.", pHDB->ThisHandle, pRCtrl->ThisIdx);
        ...
        case DCP_RSP_OK_MORE:
            ...
            if (pRQB->args.Identify.NodeCnt == 0)
            {
                ...
                else
                {
                    ...
                    if ((pRCtrl->AliasUsed < pRCtrlUser->pRQB->args.Ident.AliasCnt) && (!pRCtrl->pRCtrl->DoCancel))
                    {
                        NARE_ASSERT_NULL_PTR(pRCtrlUser->pRQB->args.Ident.pStationNameAlias);
                        ...
                        Response = NARE_DCPIdentifyRQBSetup(pHDB,
                                    pRQB,
                                    (NARE_UPPER_MEM_U8_PTR_TYPE)pRCtrlUser->pRQB->args.Ident.pStationNameAlias[pRCtrl->AliasUsed].pName,
                                    pRCtrlUser->pRQB->args.Ident.pStationNameAlias[pRCtrl->AliasUsed].NameLen,
                                    LSA_TRUE,
                                    pRCtrlUser->pRQB->args.Ident.Notify,
                                    pRCtrl->ThisIdx,
                                    pRCtrlUser->pRQB->args.Ident.Tolerance,
                                    pRCtrlUser->pRQB->args.Ident.RequestID);
                                    pRCtrl->AliasUsed++;
                        if (Response == NARE_RSP_OK)
                        {   /* set the right handle. and NodeCnt back to 1  */
            ...
            {   /* ---------------------------------------------*/
                ...
                pRCtrlUser->pRQB->args.Ident.AliasUsed = pRCtrl->AliasUsed;
                ...
                if (NARE_DCPAnalyseIdentifyRspTLV(pRQB->args.Identify.NodePtr,
                    pRCtrlUser->pRQB->args.Ident.MACAddr,
                    pRCtrlUser->pRQB->args.Ident.IPAddr,
                    pRCtrlUser->pRQB->args.Ident.SubMask,
                    pRCtrlUser->pRQB->args.Ident.DefRouter,
                    (NARE_UPPER_MEM_U8_PTR_TYPE)pRCtrlUser->pRQB->args.Ident.pStationName,
                    pRCtrlUser->pRQB->args.Ident.StationNameLen,
                    pRCtrlUser->pRQB->args.Ident.IdentifiedStationName,
                    &pRCtrlUser->pRQB->args.Ident.IdentifiedStationNameLen,
                    ...
                    &remIDs))
                {
                    NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Identify successful1. IP: %d.%d.%d.%d",
                        pHDB->ThisHandle,
                        pRCtrl->ThisIdx,
                        pRCtrlUser->pRQB->args.Ident.IPAddr[0],
                        pRCtrlUser->pRQB->args.Ident.IPAddr[1],
                        pRCtrlUser->pRQB->args.Ident.IPAddr[2],
                        pRCtrlUser->pRQB->args.Ident.IPAddr[3]);
                    NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Identify successful2. SN: %d.%d.%d.%d",
                        pHDB->ThisHandle,
                        pRCtrl->ThisIdx,
                        pRCtrlUser->pRQB->args.Ident.SubMask[0],
                        pRCtrlUser->pRQB->args.Ident.SubMask[1],
                        pRCtrlUser->pRQB->args.Ident.SubMask[2],
                        pRCtrlUser->pRQB->args.Ident.SubMask[3]);
                    NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Identify successful3. DR: %d.%d.%d.%d",
                        pHDB->ThisHandle,
                        pRCtrl->ThisIdx,
                        pRCtrlUser->pRQB->args.Ident.DefRouter[0],
                        pRCtrlUser->pRQB->args.Ident.DefRouter[1],
                        pRCtrlUser->pRQB->args.Ident.DefRouter[2],
                        pRCtrlUser->pRQB->args.Ident.DefRouter[3]);
                    NARE_PROGRAM_TRACE_08(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Identify successful4. MAC: %02X-%02X-%02X-%02X-%02X-%02X",
                        pHDB->ThisHandle,
                        pRCtrl->ThisIdx,
                        pRCtrlUser->pRQB->args.Ident.MACAddr[0],
                        pRCtrlUser->pRQB->args.Ident.MACAddr[1],
                        pRCtrlUser->pRQB->args.Ident.MACAddr[2],
                        pRCtrlUser->pRQB->args.Ident.MACAddr[3],
                        pRCtrlUser->pRQB->args.Ident.MACAddr[4],
                        pRCtrlUser->pRQB->args.Ident.MACAddr[5]);
                    ...
                    {
                        if(remIDs.Foundstate != DEV_AND_INSTANCE_NOT_SET)
                        {
                            if (remIDs.Foundstate & DEV_VENDOR_SET)    /* vendor ids found */
                            {
                                pRCtrlUser->pRQB->args.Ident.VendorID = remIDs.VendorID;
                                pRCtrlUser->pRQB->args.Ident.DeviceID = remIDs.DeviceID;
                            }
                            else
                            {
                                pRCtrlUser->pRQB->args.Ident.VendorID = 0;
                                pRCtrlUser->pRQB->args.Ident.DeviceID = 0;
                            }
                            if (remIDs.Foundstate & INSTANCE_SET)      /* instance present */
                            {
                                pRCtrlUser->pRQB->args.Ident.InstanceID      = remIDs.InstanceID;
                                pRCtrlUser->pRQB->args.Ident.InstanceIDValid = 1;
                            }
                            else
                            {
                                pRCtrlUser->pRQB->args.Ident.InstanceID      = 0;
                                pRCtrlUser->pRQB->args.Ident.InstanceIDValid = 0;
                            }
                        }
                        else
                        {
                ...
                {
                    if (pRCtrlUser->pRQB->args.Ident.IdentifiedStationNameLen > 240)
                    {
                        pRCtrlUser->pRQB->args.Ident.IdentifiedStationNameLen = 240; // set to max value (size of IdentifiedStationName array)
                        NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP, "DCP[%X:%d]-ERROR: Name of identified station is longer than 240 bytes", pHDB->ThisHandle, pRCtrl->ThisIdx);
                    }
                    else
                    {
                    ...
                    Response = NARE_RSP_ERR_DCP_IDENT_TLV_ERROR; /* Error in DCP PDU */
                }
            }
            break;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_DCPSetDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_RSP                       Response;
    ...
    switch (NARE_DCP_RQB_GET_RESPONSE(pRQB))
    {
        case DCP_RSP_OK:
            ...
            if (pRQB->args.Set.OptCnt == pRCtrl->SetOptions.OptionCnt ) /* we only correct option cnt */
            {
                ...
                while (( a < pRCtrl->SetOptions.OptionCnt) && ( Response == NARE_RSP_OK))
                {
                    Len = NARE_NTOHS(pTLV->Len); /* Len of Value-field of TLV */
                    if ( ( pTLV->Type    == NARE_HTONS(DCP_TYPE_CTRL_RESULT) ) &&
                         ( pTLV->Status  == pRCtrl->SetOptions.OptionType[a] ) &&
...
static LSA_VOID NARE_DCPHelloDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_RSP                       Response;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_DCPNotifyDone(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_DCP_PTR_TYPE        pHDB)
{
    NARE_DCP_RCTRL_PTR_TYPE       pRCtrl;
        ...
        case NARE_HANDLE_STATE_OPEN:
            switch (NARE_DCP_RQB_GET_RESPONSE(pRQB))
            {
                case DCP_RSP_OK:
                    ...
                    pHDBUser = NAREGetUserHDBFromHandle(NARE_RQB_GET_HANDLE(pRCtrlUser->pRQB));
                    NARE_ASSERT_NULL_PTR(pHDBUser);
                    /* If we should notify (should be!) and we dont use an ALIAS and    */
                    /* a notify RQB is present and this is ther first indication.       */
                    if ((pRCtrlUser->pRQB->args.Ident.Notify == NARE_NOTIFY_DCP_RESPONSE_YES) && /* should be       */
                        (!pRCtrl->NotifyCnt) &&
                        (!LSA_HOST_PTR_ARE_EQUAL(pHDBUser->NotifyReq.pBottom, LSA_NULL)))
                    {
                        ...
                        if (NARE_DCPAnalyseIdentifyRspTLV(pRQB->args.Notify.NodePtr,
                                                          pRQBUser->args.IdentNotify.Ident.MACAddr,
                                                          pRQBUser->args.IdentNotify.Ident.IPAddr,
                                                          pRQBUser->args.IdentNotify.Ident.SubMask,
                                                          pRQBUser->args.IdentNotify.Ident.DefRouter,
                                                          (NARE_UPPER_MEM_U8_PTR_TYPE)pRCtrlUser->pRQB->args.Ident.pStationName,
                                                          pRCtrlUser->pRQB->args.Ident.StationNameLen,
                                                          pRCtrlUser->pRQB->args.Ident.IdentifiedStationName,
                                                          &pRCtrlUser->pRQB->args.Ident.IdentifiedStationNameLen,
                                                          ...
                                                          &remIDs))
                        {
                            NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Notify1. IP: %d.%d.%d.%d",
                                                  pHDB->ThisHandle,
                                                  pRCtrl->ThisIdx,
                                                  pRQBUser->args.IdentNotify.Ident.IPAddr[0],
                                                  pRQBUser->args.IdentNotify.Ident.IPAddr[1],
                                                  pRQBUser->args.IdentNotify.Ident.IPAddr[2],
                                                  pRQBUser->args.IdentNotify.Ident.IPAddr[3]);
                            NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Notify2. SN: %d.%d.%d.%d",
                                                  pHDB->ThisHandle,
                                                  pRCtrl->ThisIdx,
                                                  pRQBUser->args.IdentNotify.Ident.SubMask[0],
                                                  pRQBUser->args.IdentNotify.Ident.SubMask[1],
                                                  pRQBUser->args.IdentNotify.Ident.SubMask[2],
                                                  pRQBUser->args.IdentNotify.Ident.SubMask[3]);
                            NARE_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Notify3. DR: %d.%d.%d.%d",
                                                  pHDB->ThisHandle,
                                                  pRCtrl->ThisIdx,
                                                  pRQBUser->args.IdentNotify.Ident.DefRouter[0],
                                                  pRQBUser->args.IdentNotify.Ident.DefRouter[1],
                                                  pRQBUser->args.IdentNotify.Ident.DefRouter[2],
                                                  pRQBUser->args.IdentNotify.Ident.DefRouter[3]);
                            NARE_PROGRAM_TRACE_08(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Notify4. MAC: %02X-%02X-%02X-%02X-%02X-%02X",
                                                  pHDB->ThisHandle,
                                                  pRCtrl->ThisIdx,
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[0],
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[1],
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[2],
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[3],
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[4],
                                                  pRQBUser->args.IdentNotify.Ident.MACAddr[5]);
                            ...
                            NARE_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "DCP[%X:%d]-Notify5. StationOK: %d",
                                                  ...
                                                  pRCtrl->ThisIdx,
                                                  DCPStatioNameState);
                            /* Now check if Parameters equal to configured ones. Only possible if    */
                            /* we have an IP address to be checked (CheckIPAdd)                      */
                            if (DCPStatioNameState == NARE_RCTRL_DCP_STATION_NAME_SET_EQUAL)
                            {
                                ...
                                /* Copy NotifyID from IDENT-Request */
                                pRQBUser->args.IdentNotify.NotifyID = pRCtrlUser->pRQB->args.Ident.NotifyID;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_DCPRQBCopyLLErrorLocal(
    LSA_RQB_LOCAL_ERROR_TYPE const * pLLError)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID nare_dcp_request_lower_done(
    NARE_DCP_LOWER_RQB_PTR_TYPE  pRQB)
{
    NARE_HDB_DCP_PTR_TYPE   pHDB;
...
/*+  Notify               : NARE_NOTIFY_DCP_RESPONSE_YES .. Notify identfiy responses +*/
/*+                         NARE_NOTIFY_DCP_RESPONSE_NO ... dont Notify               +*/
/*+  NotifyID             : NotifyID (only needed if Notify NARE_NOTIFY_DCP_RESPONSE_YES) +*/
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_RSP NARE_DCPIdentifyRQBSetup(
    NARE_HDB_DCP_CONST_PTR_TYPE      pHDB,
    NARE_DCP_LOWER_RQB_PTR_TYPE      pRQB,
    NARE_UPPER_MEM_U8_CONST_PTR_TYPE pStationName,
    ...
    LSA_UINT32                   RequestID)
{
    ...
    pRQB->args.Identify.RequestId = RequestID;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_RSP NARE_DCPIdentify(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    NARE_DCP_RCTRL_CONST_PTR_TYPE  pRCtrl,
    NARE_UPPER_MEM_PTR_TYPE      pStationName,
    LSA_UINT8                    StationNameLen,
    LSA_UINT32                   RequestID)
{
    ...
    NARE_DCP_ALLOC_LOWER_RQB(&pRQB, UserId, sizeof(DCP_RQB_TYPE), pSys);
    if (!LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL))
    {   /* we use User-ID as NotifyID */
        Result = NARE_DCPIdentifyRQBSetup(pHDB,
            pRQB,
            (NARE_UPPER_MEM_U8_PTR_TYPE)pStationName,
            StationNameLen,
            LSA_FALSE,
            pRCtrl->pRCtrl->pRQB->args.Ident.Notify,
            pRCtrl->ThisIdx,
            pRCtrl->pRCtrl->pRQB->args.Ident.Tolerance,
            RequestID);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static NARE_RSP NARE_DCPSetRQBSetup(
    NARE_HDB_DCP_CONST_PTR_TYPE      pHDB,
    NARE_DCP_LOWER_RQB_PTR_TYPE      pRQB,
    NARE_DCP_RCTRL_PTR_TYPE          pRCtrl,
    NARE_UPPER_RQB_MAC_PTR_TYPE      pMAC,
    NARE_UPPER_RQB_IP_CONST_PTR_TYPE pIPAddr,
    NARE_UPPER_RQB_IP_CONST_PTR_TYPE pSubnetMask,
    NARE_UPPER_RQB_IP_CONST_PTR_TYPE pDefRouter,
    NARE_UPPER_MEM_U8_CONST_PTR_TYPE pStationName,
    LSA_UINT32                       StationNameLen,
    ...
    LSA_UINT32                   RequestID)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        NARE_DCP_RQB_SET_USERID_UVAR32(pRQB,pRCtrl->ThisIdx);
        pRQB->args.Set.RequestId = RequestID;
        Result = NARE_DCPSetRQBSetup(pHDB,pRQB,pRCtrl,pMAC,(NARE_UPPER_RQB_IP_CONST_PTR_TYPE)pIPAddr,(NARE_UPPER_RQB_IP_CONST_PTR_TYPE)pSubnetMask,(NARE_UPPER_RQB_IP_CONST_PTR_TYPE)pDefRouter,pStationName,StationNameLen,SetIPRemanent);
        if ( Result == NARE_RSP_OK)
        {
            if (! LSA_HOST_PTR_ARE_EQUAL(pIPAddr, LSA_NULL))
            {
...
/*+  Description: Issues HELLO Request to DCP to Stop sending HELLO         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_RSP NARE_DCPCancel(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    LSA_UINT32                   RequestID)
{
    NARE_DCP_LOWER_RQB_PTR_TYPE   pRQB;
    LSA_SYS_PTR_TYPE              pSys;
    NARE_RSP                      Result = NARE_RSP_OK;
    LSA_USER_ID_TYPE              UserId;

    NARE_ASSERT_NULL_PTR(pHDB);
    NARE_FUNCTION_TRACE_01(pHDB->TraceIdx,LSA_TRACE_LEVEL_CHAT,"IN : NARE_DCPCancel(pHandle: 0x%X)",pHDB);
    pSys   = pHDB->Params.pSys;
    UserId.uvar32 = 0;
    NARE_DCP_ALLOC_LOWER_RQB(&pRQB,UserId,sizeof(DCP_RQB_TYPE),pSys);
    if(!(LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL)))
    {
        NARE_DCP_RQB_SET_OPCODE(pRQB,DCP_OPC_CANCEL_REQ);
        NARE_DCP_RQB_SET_HANDLE(pRQB,pHDB->DCPHandle);
        pRQB->args.CancelReq.RequestId = RequestID;
        NARE_DCP_REQUEST_LOWER(pRQB,pSys);
    }
    else
    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    NARE_DCPCancel_done                         +*/
/*+  Input/Output          :    NARE_HDB_DCP_PTR_TYPE        pHDB           +*/
