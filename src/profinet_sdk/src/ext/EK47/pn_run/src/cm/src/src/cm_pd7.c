...
#define LTRC_ACT_MODUL_ID  807
#define CM_MODULE_ID       807
	...
	)
{
	...
	if (cm_is_not_null (pdprm->read) || (pdprm->wait_for & CM_PD_PRM_WF_READ_CNF) != 0)
	{
		CM_FATAL();
	}
...
#if CM_CFG_USE_GSY
	/* CM_PD_SCID_GSY */
	else if (scid == CM_PD_SCID_GSY)
	{
		rc = cm_gsy_prm_read (pdprm->channel
			, port_id, slot_nr, subslot_nr, record_index
			, record_data_length, record_data
			);
		if (rc != CM_OK) {
	...
	else if (scid == CM_PD_SCID_BBUF) /* see CM_OPC_SV_AR_PDEVPRM_READ */
	{
		...
		{
			CM_PD_RECORD_PTR_TYPE rec = cm_pd_reclist_read(pdprm->channel, CM_PD_RECLIST_REGULAR, slot_nr, subslot_nr, record_index);

			if (cm_is_null (rec)) /* not found */
			{
	...
	else if (scid == CM_PD_SCID_CMPD)
	{
		...
		else if (record_index == 0xB08F /* CM: StartupStatisticsData */)
		{
			response = cm_pd_prm_read_StartupStatisticsData (pdprm->channel, & record_data_length, record_data);
		}
		else
#endif
	...
	)
{
	...
	case MRP_RSP_ERR_PRM_NOT_CONFIGURED:
		CM_PD_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE /*AP00586614:no error trace if mrp is not configured*/
			, "mrp-rsp(%u/not-configured)", CM_MRP_GET_RESPONSE (mrp)
			);
	...
	)
{
	...
	if (cm_is_null (read) || (pdprm->wait_for & CM_PD_PRM_WF_READ_CNF) == 0)
	{
		CM_FATAL();
	}
