...
*/
	...
	#define GLOB_FW_VERSION_FUNCTION                  4
	#define GLOB_FW_VERSION_RELEASE                   7
	#define GLOB_FW_VERSION_SERVICE_PACK              0

	/* developments point of version view */
	#if ('B' != GLOB_FW_VERSION_LETTER)
		/* official view: developments point of version view */
	  ...
	  #define GLOB_FW_VERSION_INTEGRATION               3  /* increment when generating a new version ! */
	  #define GLOB_FW_VERSION_GENERATION                1  /* fix used to check for additional compatibility issues*/
	#else
		/* internal view: every build passed to the integration test has to have a unique build number - do not forget */
	  #define GLOB_FW_VERSION_PROJECT                   ((10 * GLOB_FW_VERSION_FUNCTION)+ GLOB_FW_VERSION_RELEASE) /* fixed as long as we work on the same customer version above */
	...
	#define GLOB_FW_VERSION_LETTER                    'V'
	#define GLOB_FW_VERSION_FUNCTION                  4
	#define GLOB_FW_VERSION_RELEASE                   2
	#define GLOB_FW_VERSION_SERVICE_PACK              1

	/* developments point of version view */
	#if ('B' != GLOB_FW_VERSION_LETTER)
		/* V,R,... official view: developments point of version view */
	  #define GLOB_FW_VERSION_PROJECT                   ((10 * GLOB_FW_VERSION_FUNCTION)+ GLOB_FW_VERSION_RELEASE) /* fixed as long as we work on the same customer version above */
	  #define GLOB_FW_VERSION_INCREMENT                 1  /* should be increased on major changes during development that cause incompatibilities */
	  #define GLOB_FW_VERSION_INTEGRATION               4  /* increment when generating a new version ! */
	  #define GLOB_FW_VERSION_GENERATION                1  /* fix used to check for additional compatibility issues*/
	#else
...
#elif ( (PNIOD_PLATFORM & (PNIOD_PLATFORM_ECOS_EB200P | PNIOD_PLATFORM_RZT2_EDDP) ) || (PNIOD_PLATFORM & PNIOD_PLATFORM_POSIX_EB200P) )

	#define GLOB_FW_VERSION_LETTER                    'V'
	#define GLOB_FW_VERSION_FUNCTION                  4
	#define GLOB_FW_VERSION_RELEASE                   3
	#define GLOB_FW_VERSION_SERVICE_PACK              0
...
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_DKSW)
	#define DATE_VERSION_STR       "2015-09-16 12:00"
#elif (PNIOD_PLATFORM & PNIOD_PLATFORM_ECOS_EB200_400)
	#define DATE_VERSION_STR       "2015-08-20 09:00"
#elif ( (PNIOD_PLATFORM & (PNIOD_PLATFORM_ECOS_EB200P |PNIOD_PLATFORM_RZT2_EDDP)) || (PNIOD_PLATFORM & PNIOD_PLATFORM_POSIX_EB200P) )
