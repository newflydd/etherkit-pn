/*
 * v3.1 March 13, 2012 ip/ip_reasm.c (bugfix, see ip_reasm_chk_consistency())
 * v3.1 August 30, 2011 ip/ip_reasm.c
...
int ip_reasm_check_mem_useage (u_long increment); /* INICHE_TCIP_ITGR u_short->u_long to avoid casts in callers */ 
int ip_reasm_incr_mem_useage (u_long increment); /* INICHE_TCIP_ITGR u_short->u_long to avoid casts in callers */ 
int ip_reasm_decr_mem_useage (u_long decrement); /* INICHE_TCIP_ITGR u_short->u_long to avoid casts in callers */ 
...
void ip_reasm_proc_complete_rcvd_pkt (PACKET p);
...
#ifdef IPSEC
extern void ipsec_inbound (PACKET *p);
#endif /* IPSEC */
...
/* FUNCTION: ip_reasm_proc_complete_rcvd_pkt ()

This function initiates processing on the reassembled packet. If 
the packet has been secured/authenticated, it is sent to the IPSec 
module for decapsulation, and the resulting packet is then de-
multiplexed to the appropriate protocol module (e.g., UDP, TCP, 
etc.).  Otherwise, the packet is directly sent for demultiplexing.

INPUTS: (1) Pointer to the reassembled packet
...
void ip_reasm_proc_complete_rcvd_pkt (PACKET p)
{
   struct ip * pip; /* pointer to IPv4 header */
   ...
   switch (pip->ip_prot)
      ...
      case AH_PROTOCOL:
         ipsec_inbound (&p);
         /* return if packet has been consumed by IPSec */
         if (!p) return;
         break;
#endif
...
/* AP01331226 new check function */
/* FUNCTION: ip_reasm_chk_consistency()
 * 
 * This function performs length-based consistency checks on the just-
 * received fragment and queued sibling fragments.
 * 
 * PARAM1: PACKET             just-received packet
 * PARAM2: IP_FRAGTYPE        type of fragment (first, middle, or last)
 * PARAM3: IREP               pointer to matching IPv4 reassembly entry
 * 
 * RETURNS: int               IPREASM_ERROR, if the fragments failed the
 *                            consistency check; otherwise, it returns 
 *                            IPREASM_OK.
 */
int
ip_reasm_chk_consistency(PACKET p, IP_FRAGTYPE ftype, IREP irep)
{
   RFQP rfqp;
   u_short lplen, clen, nlen, tlen; 
   struct ip * pip;
   int i;
   ...
   /* compute the length of packet based on an examination of all 
    * received fragments (this value may be less than the actual
    * length of the original, unfragmented datagram) */
   do
      {
      for (i = 0; i < IPR_MAX_FRAGS; ++i)
         {
         if ((p = rfqp->bufp [i]) != 0)
            {
            clen = rfqp->frag_offset [i] + rfqp->bufp [i]->nb_plen;
            if (clen > lplen)
               lplen = clen;
            }
         else
   ...
   /* 'tlen' may be zero if the last fragment has not been received yet */
   tlen = irep->length;
   /* compute the length of the packet (based on information extracted 
    * from just-received packet).  This value may be less than the actual
    * length of the original, unfragmented datagram. */
   nlen = (((ntohs(pip->ip_flgs_foff)) & IP_EXTRACT_FOFF) << 3) + ((ntohs(pip->ip_len)) - ip_hlen(pip));

   /* check for consistency in length between various received fragments */
   if (ftype == IP_LF)
      {
      /* just received a last fragment */
      if (tlen == 0)
         {
         if (nlen <= lplen)
            {
            /* a just-received last fragment indicates a packet length shorter
             * than or equal to that indicated from a previously received first 
             * or middle fragment */
            return (IPREASM_ERROR);
            }
         }
      else
         {
         if (nlen != tlen)
            {
            /* two last fragments, each indicating a different length */
            return (IPREASM_ERROR);
            }
         }
      }
      ...
      {
      /* just received a first or middle fragment */
      if (tlen && nlen >= tlen)
         {
         /* a just-received first or middle fragment indicates a packet 
          * length equal to or longer than that indicated from a previously 
          * received last fragment */
         return (IPREASM_ERROR);
         }
      }

   /* received set of fragments is consistent (wrt length) */
   return (IPREASM_OK);
}
...
int ip_reasm_process_subsequent_fragments (PACKET p, IREP irep)
{
   ...
   pip = ip_head(p);
   ftype = ip_reasm_determine_type_of_frag (pip);
   rc = ip_reasm_chk_consistency (p, ftype, irep); /* AP01331226 */
   if (rc)
      {
      LOCK_NET_RESOURCE (FREEQ_RESID);
      pk_free (p);
   ...
   if ((rc2 = ip_reasm_compute_overlap (p, irep, &index, &rfqp, &frag_offset, &hole_created)) != IPREASM_ACCEPT_FRAG)
      ...
      switch (rc2)
         ...
         case IPREASM_DROP_FRAG_BAD_PARAM:
            ...
            return ENP_PARAM;
         default: /* INICHE_TCIP_ITGR AP01308013, prevent GNU compiler warning */
            break;
         }
      }
   ...
   if (irep->length != 0)
      ...
      if (irep->rcvd == irep->length)
         ...
         if (reassy_pkt == 0)
             ...
             * the IP header. */
            ++ip_mib.ipReasmOKs;
            ip_reasm_proc_complete_rcvd_pkt (reassy_pkt);
            }
         }
      else
         {
...
#ifdef NET_STATS /* INICHE_TCIP_ITGR added #ifdef */
int ipr_stats(void * pio)
{
