...
/* contents:
    ...
    - pndv_in_init_offline
    - pndv_in_control_bf_led_with_offline_ind
    - pndv_in_cm_ar_offline_ind
    - pndv_trigger_offline_ind_done
    - pndv_in_cm_ar_offline_ind_done
    - pndv_in_ar_abort_req

    - pndv_in_put_all_module
    - pndv_in_rmv_all_module
    - pndv_in_cm_module_add
    - pndv_in_cm_module_add_done

    - pndv_in_cm_plug_next_submodule
    - pndv_in_cm_submodule_add
    - pndv_in_cm_submodule_add_done

    - pndv_in_cm_module_remove
    - pndv_in_cm_module_remove_done
    - pndv_in_cm_submodule_remove_done

    - pndv_in_check_ind
    - pndv_in_check_rsp
    - pndv_in_store_ist_cfg
    - pndv_ar_reset_set_cfg
    - pndv_in_set_cfg
...
#if 0 // bad overview in doxy if this is enabled
//! @defgroup pndv_ar_connect           Connnect related functions
//! @defgroup pndv_ar_man               AR Management
//! @defgroup pndv_ar_connect           Service: connect.ind related functions
//! @defgroup pndv_ar_disconnect        Service: disconnect.ind and abort related functions
//! @defgroup pndv_ar_ownership         Service: ownership.ind related functions
//! @defgroup pndv_ar_prm_end           Service: prm_end.ind related functions
//! @defgroup pndv_ar_appl_ready        Service: appl-Ready.req related functions
//! @defgroup pndv_ar_input_update      Service: input update notification
//! @defgroup pndv_ar_in_data           Service: in_data.ind related functions
//! @defgroup pndv_ar_rir               Service: rir.ind related functions
#endif
...
void pndv_ar_cm_connect_ind(CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    Boolean                          is_io_memory_allocated = FALSE;
    ...
    switch (ret_val)
    {
        ...
        case PNDV_CONNECT_IND_RET_ERROR_RESOURCE:
        {
            ...
            pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_CHK_IND_AR_ABORT, (Unsigned16)__LINE__, 0);
...
PNDV_CONNECT_IND_RET_VAL_T pndv_ar_tool_connect_ind_find_ar_idx(UnsignedOpt *ar_idx_ptr, CM_UPPER_EVENT_PTR_TYPE event_ptr)
{
        ...
        case PNDV_CM_AR_NO:
        {
            ...
            switch (event_ptr->u.sv.ar_connect->ar_type)
            {
                ...
                case CM_AR_TYPE_SUPERVISOR:
                {
                       ...
                       )
                    {
                        ...
                        if( (PNDV_MAX_ARS_RT + PNDV_MAX_ARS_DA) <= (*ar_idx_ptr) )
                        {
                            ...
                            pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_CHK_IND_AR_ABORT, (Unsigned16)__LINE__, (*ar_idx_ptr));
                        }
                    }
                    else
                    {
                ...
                default:
                {
                    ...
                    pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_CHK_IND_AR_ABORT, (Unsigned16)__LINE__, event_ptr->u.sv.ar_connect->ar_type);
    ...
    pndv_in_write_debug_buffer_3__( PNDV_DC_CM_CONNECT_IND,
                                    ...
                                    event_ptr->session_key,
                                      ((((Unsigned32)(event_ptr->u.sv.ar_connect->ar_type)      ) << 24 ) & 0xFF000000)
                                    | ((((Unsigned32)(event_ptr->u.sv.ar_connect->ar_properties)) <<  8 ) & 0x00FFFF00)
                                    | (( (Unsigned32)(*ar_idx_ptr)                                ) & 0x000000FF)
                                  );
...
void pndv_ar_tool_connect_ind_store_ar_data(UnsignedOpt ar_idx, CM_UPPER_EVENT_PTR_TYPE event_ptr)
{
    ...
    if (PNDV_AR_SM_OFFLINE == pndv_data.ar[ar_idx].sm_state )
    {
        ...
        */
        ...
        pndv_data.ar[ar_idx].ar_set_trigger_running      = FALSE;
        ...
        {
            pndv_ds_check_arfsu_data_adjust((Unsigned8 *)event_ptr->u.sv.ar_connect->ARFSUBlock_ptr, &pndv_data.ar[ar_idx].ar_fsu_enable, pndv_data.ar[ar_idx].ar_fsu_uuid);
        }
 ...
 *
 */
void pndv_ar_tool_connect_ind_process_parked()
{
    CM_UPPER_RQB_PTR_TYPE tmp_rqb_ptr;
    Unsigned32      ret_val;

    if (pndv_data.serv.con_serv_run == TRUE)
    {
        ...
        pndv_data.serv.con_serv_run = FALSE;
 ...
 *
 */
void pndv_ar_sys_connect_ind_timeout()
{
...
void pndv_ar_cm_ar_disconnect_ind(CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_AR_DISCONNECT_IND,
                                         rqb_ptr->args.pd.event->u.sv.ar_disconnect->disconnect.reason_code, ar_idx);
...
{
    //! nothing to do
    //pndv_data.ar[ar_idx].ar_abort_running = FALSE; /* AR-Abort ist durch */
    ...
    if ( ar_idx < PNDV_MAX_ARS_RT )
    {
        ...
        for (local_ar_idx=0;local_ar_idx<PNDV_MAX_ARS_RT ;local_ar_idx++ )
        {
            ...
            pndv_ar_set_so_state( ((Unsigned32)(1 << ar_idx)), FALSE, local_ar_idx );
        }
    }

    pndv_data.ar[ar_idx].no_prm_end_ind         = FALSE;
    pndv_data.ar[ar_idx].ar_abort_requested     = FALSE;
    ...
    if(pndv_data.cfg.akt_isom_ar_idx == ar_idx)
    {
...
#ifdef PNDV_CFG_ISOM_NO_SHARED
        {
            ...
            for (local_ar_idx=0;local_ar_idx<PNDV_MAX_ARS_RT ;local_ar_idx++ )
            {
                ...
                pndv_ar_set_so_state( PNDV_SOL_EVENT_FLAG_LOCKED_BY_ISOM, FALSE, local_ar_idx );
            }
        }
#endif
    }
...
void pndv_ar_tool_disconnect_ind_control_bf_led ( void )
{
    ...
    if (   (FALSE == ar_in_data )
        && (TRUE == pndv_data.ar_in_data_led_used )
       )
    {

        PNDV_LED_CTRL_DATA_AR_OFFLINE();

        pndv_data.ar_in_data_led_used = FALSE;
    }
}
...
void pndv_ar_tool_disconnect_ind_cleanup(Unsigned32 ar_idx)
{
    ...
    if ( PNDV_MAX_ARS_RT > ar_idx  ) /* we have a payload AR */
    {
        ...
        /* by resetting current_rt_class = PNDV_RT_NONE
           will be called (sometime) pndv_in_cm_ar_offline_ind_done
           the RQB will only be returned there */
        pndv_data.iface_ptr->ar[ar_idx].current_rt_class = PNDV_RT_NONE;
        pndv_data.iface_ptr->ar[ar_idx].red_int_info_used = FALSE;
        ...
        */
        if (    ( TRUE                    == pndv_data.cfg.mode_isom )    /* taktsync. RTC3-AR */
    ...
    pndv_data.ar[ar_idx].empty_prm_end_ind = FALSE;
    pndv_data.ar[ar_idx].pdev_8000_locked = FALSE;
    pndv_data.ar[ar_idx].pdev_9000_locked = FALSE;
        ...
        if(ar_is_owner == TRUE)
        {
            ...
            else
            {
                ...
                if (local_sub_res->ar_set_nr == pndv_data.ar[ar_idx].ar_set_nr)
                {
                    ...
                    {
                        /* EnRa: if there is another AR (sr_remains = TRUE) and the dismantled AR was owner (ar_is_owner == TRUE) 
                        ...
                        pndv_in_write_debug_buffer_3__(PNDV_DC_COMMON_MODUL_INFO,
                                                       ...
                                                       ((Unsigned32)local_sub_res->sub_module.mod_ident)|((Unsigned32)local_sub_res->sub_module.sub_ident)<<16
                                                       );
 ...
 *
 */
void pndv_ar_tool_disconnect_ind_reset_set_cfg( Unsigned32 ar_idx )
{
    ...
    for ( entity_nr = 0; entity_nr < PNDV_CM_SV_SUBSLOT_COUNT; entity_nr++ )
    {
        ...
        PNDV_MEMSET((void *)&pndv_data.iface_ptr->set_cfg[ar_idx][entity_nr],0, sizeof(PNDV_SET_CFG_T));
    }
...
/**
 *  @brief interface funktion to indicate freeing communication memory has finished
 *  @ingroup pndv_ar_disconnect
 *
 *  @param[in]     ar_idx AR index
 *
 *  Interface function to system. This function is used to indicate that the freeing of
 *  communication memory for one ar has been finished.
 *
 */
void pndv_ar_sys_disconnect_ind_free_iocr_mem_done (UnsignedOpt ar_idx)
{
    //! no need to do that async now
#if 0
    CM_UPPER_RQB_PTR_TYPE tmp_rqb_ptr;
    ...
    tmp_rqb_ptr = pndv_data.rqb.stored_ar_offline_ind_rqb_ptr[ar_idx];
    pndv_data.rqb.stored_ar_offline_ind_rqb_ptr[ar_idx] = 0;

    LSA_RQB_SET_METHOD_PTR(tmp_rqb_ptr, pndv_ar_cm_disconnect_ind_done);

    PNDV_REQUEST_LOCAL(tmp_rqb_ptr);
#endif
}
...
UnsignedOpt pndv_ar_abort_req( UnsignedOpt ar_idx, UnsignedOpt call_line)
{
       ...
       )
    {
        ...
        else
        {
            ...
            pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_AR_ABORT, ar_idx, call_line);

            PNDV_RQB_SET_HANDLE( &pndv_data.rqb.ar_abort[ar_idx], pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
...
void pndv_ar_cm_ar_abort_done (CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    pndv_in_write_debug_buffer_all__(PNDV_DC_CM_AR_ABORT_DONE, ar_idx);
...
void pndv_ar_abort_shared_ar (void)
{
    ...
    for ( ar_idx = 0; ar_idx < PNDV_MAX_ARS_RT ; ar_idx++ )
    {
        ...
        {
            pndv_ar_abort_req( ar_idx, (UnsignedOpt)__LINE__ );
        }
    }
}
//@}
...
void pndv_ar_cm_sv_ownership_ind( CM_UPPER_RQB_PTR_TYPE rqb_ptr )
{
    ...
    if (pndv_data.serv.own_serv_run == FALSE)
    {
        ...
        switch (local_ar_ownership->ar_context)
        {
            ...
            case CM_SV_AR_CONTEXT_PLUG:
            {
                ...
                for (elem_cnt = 0; elem_cnt < local_ar_ownership->nr_of_elems; elem_cnt++)
                {
                    ...
                    PNDV_COPY_BYTE((void *)&set_cfg_elem_ptr->elem.own_info, (void *)act_elem_ptr, sizeof(CM_SV_AR_OWNERSHIP_ELEMENT_TYPE));

                    //! mark the submodule as being projected
                    set_cfg_elem_ptr->elem.ownership_passed = TRUE;
                    ...
                    else
                    {
                        ...
                        set_cfg_elem_ptr->flags &=  ~( (Unsigned32) PNDV_SET_CFG_FLAGS_OWN);
                    }

                    // submodule ist ready by default, peri can force arp by setting the bit
                    set_cfg_elem_ptr->flags &=  ~( (Unsigned32) PNDV_SET_CFG_FLAGS_SET_ARP);

                    //! mark service to be in use
                    pndv_data.serv.own_serv_run = TRUE;

                    set_cfg_elem_ptr->elem.own_state = PNDV_IFACE_SERVICE_NEW;
                    //! prepare interface command
                    tmp_event.add_1 = (Unsigned8)ar_idx;
                    ...
                    pndv_in_write_debug_buffer_3__(PNDV_DC_COMMON_MODUL_INFO,
                                                   ...
                                                   ((Unsigned32)pndv_data.iface_ptr->set_cfg[ar_idx][entity_nr].elem.own_info.mod_ident)|((Unsigned32)pndv_data.iface_ptr->set_cfg[ar_idx][entity_nr].elem.own_info.sub_ident)<<16
                                                   );
                }
            ...
            case CM_SV_AR_CONTEXT_DISC:
            {
                ...
                PNDV_RQB_SET_HANDLE( pndv_data.ar[ar_idx].own_ind_rqb_ptr, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
                PNDV_RQB_SET_OPCODE( pndv_data.ar[ar_idx].own_ind_rqb_ptr, CM_OPC_SV_AR_OWNERSHIP_RSP);

                PNDV_CM_REQUEST( pndv_data.ar[ar_idx].own_ind_rqb_ptr);
 ...
 *  @param[in]  more_follows TRUE = more responses follow, FALSE = last response
 *
 *  ownership responses have to come back in order of request
 *
 */
void pndv_ar_peri_ownership_ind_done(Unsigned8 ar_idx, Unsigned16 entity_nr, Boolean more_follows)    /*lint -esym(661,elem_cnt) */
{
    CM_UPPER_RQB_PTR_TYPE   rqb_ptr;
    UnsignedOpt             elem_cnt;
    UnsignedOpt             ar_is_second_of_r1_set;
    CM_UPPER_SV_AR_OWNERSHIP_PTR_TYPE local_ar_ownership;
        ...
        case PNDV_CMP_RES_ERROR:            /* check for PDEV error -> SOL in case of error */
        {
               ...
               )
            {
            ...
            #endif
                {
                    ...
                    pndv_ar_set_so_state(PNDV_SOL_EVENT_FLAG_MOD_ID_HEAD, TRUE, ar_idx);
                }
#ifdef PNDV_CFG_DISABLE_SOL_BY_PDEV
                else
                {
                    ...
                    if ((local_ar_ownership_elem->subslot_nr & 0xF000) == 0x8000)
                    {
                        pndv_data.ar[ar_idx].pdev_8000_locked = TRUE;
                    }
                    else
                    {
                        pndv_data.ar[ar_idx].pdev_9000_locked = TRUE;
                    }
...
#ifdef PNDV_CFG_DISABLE_SOL_BY_PDEV
    if (pndv_data.ar[ar_idx].pdev_8000_locked == TRUE)
    {
        if (  (  (local_ar_ownership_elem->slot_nr == PNDV_IM_SLOT_NO)
               ||(local_ar_ownership_elem->slot_nr == PNDV_IM_SLOT_NO_PARTNER)
               )
    ...
    if (pndv_data.ar[ar_idx].pdev_9000_locked == TRUE)
    {
        if (  ( (local_ar_ownership_elem->slot_nr == PNDV_IM_SLOT_NO)
               ||(local_ar_ownership_elem->slot_nr == PNDV_IM_SLOT_NO_PARTNER)
               )
        ...
        )
    {
        ...
        else
        {
            #ifdef PNDV_CFG_ENABLE_R1
            if (  (pndv_data.ar[ar_idx].ar_set_nr != 0)
                &&(pndv_data.sr[pndv_ar_get_sr_idx_by_ar_set_nr(pndv_data.ar[ar_idx].ar_set_nr)].sm == PNDV_SR_STATE_ESTABLISH)
                &&(pndv_r1_bridge_enable_bridge())
                )
            {
                /* this ar is the second ar of a set */
                ar_is_second_of_r1_set = LSA_TRUE;
                pndv_in_write_debug_buffer_3__(PNDV_DC_CM_AR_R1_OWNERSHIP_PASSIVE, 0, 0, 0);
            }
            else
            #endif
            {
    ...
    if (more_follows == FALSE)
    {
        ...
        switch (local_ar_ownership->ar_context)
        {
            case CM_SV_AR_CONTEXT_CONN:
            {
                ...
                pndv_data.ar[ar_idx].no_prm_end_ind = TRUE;
                ...
                if (pndv_data.ar[ar_idx].no_prm_end_ind == TRUE)
                {
            ...
            case CM_SV_AR_CONTEXT_PLUG:
            {
                ...
                pndv_data.serv.own_serv_run = FALSE;
...
{
    Unsigned8  projected_port_subslots      = 0;
    Unsigned8  projected_interface_subslots = 0;
    Unsigned16 local_cnt;
...
#endif
    {
        ...
        pndv_ar_set_so_state( PNDV_SOL_EVENT_FLAG_PDEV_FAULT, TRUE, ar_idx );
...
Unsigned32 pndv_ar_tool_ownership_ind_check_plausibility(CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    Unsigned32                          ret_val = PNDV_OK;
    Unsigned32                          ar_wants_primary = LSA_FALSE;
    Unsigned32                          ar_wants_pdev = LSA_FALSE;
    Unsigned32                          ar_old_proj = LSA_FALSE;
    Unsigned16                          elem_cnt;
    UnsignedOpt                         ar_idx;
    CM_UPPER_SV_AR_OWNERSHIP_PTR_TYPE   own_ptr;
    UnsignedOpt                         subslot_error;
    PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T local_sub_res;
    Unsigned32                          pr_owner_ar = PNDV_AR_IDX_NOT_USED;
    Unsigned32                          pr_owner_mod_ident = 0;
    Unsigned32                          if_owner_ar = PNDV_AR_IDX_NOT_USED;
    ...
    for (port_count = 1; port_count <= pndv_data.cfg.pd.port_count_used; port_count++)
    {
        pndv_get_submod_resource_ptr(&local_sub_res, PNDV_IM_SLOT_NO/*slot_nr*/, pndv_data.cfg.pd.interface_subslot_nr + port_count/*subslot_nr*/, &subslot_error);
        if(subslot_error)
        {
            // slot/subslot not possible, must be an implementation error
            pndv_in_fatal_error( PNDV_MODULE, __LINE__, 0);
    ...
    if ( ret_val == PNDV_OK)
    {
        switch (ar_share_type)
        {
            ...
            {
                  ret_val = PNDV_ERR_RESOURCE;
               // pndv_in_write_debug_buffer_all_add__(PNDV_DC_CM_CHK_IND_AR_ABORT, (Unsigned16)__LINE__, modul_ptr->mod_ident);
                break;
            }
        }
    }
    ...
    /* ---------------------------------------------------------------------
        PART 5:
            check for remanence of global params
       --------------------------------------------------------------------- */

#ifdef PNDV_CFG_USE_DFC
    if (PNDV_OK == ret_val)
    {
        if (  (ar_share_type == PNDV_AR_TYPE_SHARED_WITH_PDEV)
            ||(ar_share_type == PNDV_AR_TYPE_SHARED_WITHOUT_PDEV)
            )
        {
            Unsigned32                          data_length = 0;
            /* Retentivity of the station parameters is only read if the ar was not currently locked */
            PNDV_GET_REMA_STATION_PRM_LEN ( &data_length );

            if (   ( 0 != data_length )
                && ( sizeof(CM_RQB_TYPE) >= data_length ) //// if the data is larger, the next memory grease is available here
               )
            {
                /* there are retentive station parameters */
                //////// -> copy the rqb to local pndv_data ////////
                PNDV_COPY_BYTE( &pndv_data.rema_local_station_prm.rqb,
                                 pndv_data.rema_station_prm_ptr,
                                 data_length );

                pndv_data.rema_local_station_prm.rqb.args.pd.event                    = &pndv_data.rema_local_station_prm.event_type;
                pndv_data.rema_local_station_prm.rqb.args.pd.event->u.sv.ar_rec.data  = (CM_COMMON_MEM_U8_PTR_TYPE) &pndv_data.rema_local_station_prm.record_data;
                pndv_data.rema_local_station_prm.event_type.u.sv.ar_rec.cm_pnio_err   = CM_PNIO_ERR_NONE;

                /* Request an DFC */

                PNDV_DFC_DS_REQ( &pndv_data.rema_local_station_prm.rqb,
                                  PNDV_DFC_REQ_ID_PNDV_DS1,
                                  pndv_data.rema_local_station_prm.event_type.ar_nr );
            }
            else if(pndv_data.rema_station_prm_ptr != 0)
            {
                /* no retentive station parameters
                 -> as long as there are no station parameters, there are no shared AR */

                ret_val = PNDV_ERR_RESOURCE;
                pndv_in_write_debug_buffer_2__(PNDV_DC_LINE_INFO, PNDV_MODULE, __LINE__);
                pndv_ar_set_so_state(PNDV_SOL_EVENT_FLAG_SHARED_NO_REMA, TRUE, ar_idx );
            }
            else
            {
                /* this device has no  head module parameters */

                /* only the first AR is allowed without  head module parameters, every further one is already locked here (see above) */
...
void pndv_ar_cm_sv_event_prm_end_ind (CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    if (pndv_data.serv.prm_serv_run == FALSE)
    {
        ar_idx = pndv_ar_get_ar_idx_by_session_key( rqb_ptr->args.sv.ar_event->session_key);
        ...
        switch (local_ar_prm_end_ptr->ar_context)
        {
            case CM_SV_AR_CONTEXT_CONN:
            {
                ...
                if (local_ar_prm_end_ptr->nr_of_elems == 0)
                {
                    ...
                    PNDV_RQB_SET_HANDLE( pndv_data.ar[ar_idx].prm_end_ind_rqb_ptr, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
                    ...
                    if (pndv_data.ar[ar_idx].ar_type != CM_AR_TYPE_SINGLE_RTC3)
                    {
                        ...
                        pndv_data.serv.prm_serv_run = TRUE;

                        pndv_data.ar[ar_idx].empty_prm_end_ind = TRUE;
                    ...
                    else
                    {
                        ...
                        PNDV_CM_REQUEST( pndv_data.ar[ar_idx].prm_end_ind_rqb_ptr);
    ...
    if(local_sub_res != 0)
    {
        ...
        if (local_ar_prm_end_ptr->ar_context == CM_SV_AR_CONTEXT_PLUG)
        {
            if (pndv_data.ar[ar_idx].ar_set_nr != 0)
            {
                ...
                {
                    // this ar is the primary, send prm-end to partner
                    #ifdef PNDV_CFG_ENABLE_R1
                    wait_for_r1 = pndv_r1_sendPrmEndData(slot_nr, subslot_nr, ar_idx, more_follows);
                    //#else : wait_for_r1 = false;
                    #endif
                }
            }
        }
...
void pndv_ar_peri_prm_end_ind_done_continue(Unsigned8 ar_idx, Unsigned16 slot_nr, Unsigned16 subslot_nr, Boolean more_follows)
{
    ...
    UnsignedOpt             subslot_error;
    UnsignedOpt             sr_set_idx;
    Unsigned32              cfg_flags;
    CM_UPPER_SV_AR_PRM_END_PTR_TYPE local_ar_prm_end_ptr;
    PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T local_sub_res;
    Boolean                 is_submodule_present = TRUE;
    ...
    if((local_sub_res == 0) || (entity_nr >= PNDV_MAX_SV_ENTITY))
    {
        is_submodule_present = FALSE;
    }
            ...
            if (! (PNDV_IS_PDEV(local_ar_prm_end_ptr->elem[elem_cnt].slot_nr, local_ar_prm_end_ptr->elem[elem_cnt].subslot_nr)))
            {
                if (pndv_data.ar[ar_idx].ar_type == CM_AR_TYPE_SINGLE_RTC3)
                {
                    ...
                    if(   (local_sub_res->para_error)
                        ||(cfg_flags & PNDV_SET_CFG_FLAGS_SET_ARP)
                       )
                    {
    ...
    if (more_follows == FALSE)
    {
        ...
        local_ar_prm_end_ptr->nr_of_elems -= pndv_data.ar[ar_idx].prm_end_rsp_deleted;
        ...
        switch (local_ar_prm_end_ptr->ar_context)
        {
            ...
            case CM_SV_AR_CONTEXT_PLUG:
            {
                ...
                pndv_data.serv.prm_serv_run = FALSE;
 ...
 *
 */
void pndv_ar_tool_prm_end_check_empty_request(Unsigned32 ar_idx)
{
       ...
       )
    {
        ...
        if (pndv_data.ar[ar_idx].empty_prm_end_ind == TRUE)
        {
...
void pndv_ar_do_appl_ready_req ( UnsignedOpt ar_idx )
{
    ...
    Unsigned16                          elem_cnt, i, skip_cnt;
    ...
    if (elem_cnt)
    {
        ...
        {
            Unsigned32  tmp_api;
            Unsigned16  tmp_slot, tmp_sub, entity_nr;
            UnsignedOpt tmp_subs_error;
            PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T local_sub_res;
...
void pndv_ar_cm_sv_appl_ready_done (CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
       ...
       )
    {
        if (response == CM_ERR_ELEM)
        {
            ...
            if (act_elem_ptr)
            {
                Unsigned16 entity_nr = PNDV_GET_ENTITY_NR(act_elem_ptr->api, act_elem_ptr->slot_nr, act_elem_ptr->subslot_nr, ar_idx);
                pndv_in_fatal_error( PNDV_MODULE, __LINE__, entity_nr);
            }
            else
            {
...
void pndv_ar_cm_ar_in_data_ind (CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    if (pndv_data.serv.ida_serv_run == FALSE)
    {
        ar_idx = pndv_ar_get_ar_idx_by_session_key( rqb_ptr->args.pd.event->session_key );
        ...
        switch ((pndv_data.ar[ar_idx].sm_state) )
        {
            ...
            case PNDV_AR_SM_IN_DATA_RTC3:
            {
                ...
                if (  local_ar_in_data->nr_of_elems == 0 ) // all modules pulled or not owned
                {
                    ...
                    pndv_ar_peri_in_data_done(ar_idx, 0 , LSA_FALSE);
                ...
                else
                {
                    for (elem_cnt = 0; elem_cnt < local_ar_in_data->nr_of_elems; elem_cnt++)
                    {
                        ...
                        //! mark service to be in use
                        pndv_data.serv.ida_serv_run = TRUE;
 ...
 *  @param[in]  more_follows TRUE = more responses follow, FALSE = last response
 *
 *  in_data indication has been processed by peri. more_follows == TRUE indicates
...
void pndv_ar_peri_in_data_done(Unsigned8 ar_idx, Unsigned16 entity_nr, Boolean more_follows)
{
        ...
        )
    {
        ...
        if ( !( CM_AR_PROP_DEVICE_ACCESS & ar_properties ))
        {
           ...
           pndv_data.ar_in_data_led_used = TRUE;
    ...
    PNDV_RQB_SET_HANDLE( pndv_data.ar[ar_idx].indata_ind_rqb_ptr, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
    PNDV_RQB_SET_OPCODE( pndv_data.ar[ar_idx].indata_ind_rqb_ptr, CM_OPC_SV_AR_IN_DATA_RSP);

    PNDV_CM_REQUEST( pndv_data.ar[ar_idx].indata_ind_rqb_ptr);

    pndv_data.ar[ar_idx].indata_ind_rqb_ptr = 0;

    pndv_data.serv.ida_serv_run = FALSE;
    //! check for waiting prm_end requests
...
void pndv_ar_cm_sv_event_ar_rir_ind(CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    pndv_data.iface_ptr->ar[ar_idx].red_int_info_used                   = TRUE;
#endif
 ...
 *
 *  @param[in]  ar_idx ar index
 *  @param[in]  is_primary indicates a primary edge if TRUE, otherwise a backup edge
 *
...
{
    PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T local_sub_res;
    Unsigned16 sr_set_idx = PNDV_SR_SET_IDX_NOT_USED;
   ...
   /*having edge event while in PNDV_AR_SM_IN_DATA is OK
...
#endif
	{
        ...
        if (is_primary)
        {
            ...
            pndv_data.sr[sr_set_idx].primary_ar_idx = ar_idx; // set the new primary ar the leading one
        ...
        if (pndv_data.ar[ar_idx].ar_set_trigger_running  == FALSE)
        {
            pndv_data.ar[ar_idx].ar_set_trigger_running  = TRUE;
            ...
            ptr.rqb->args.sv.arset_trigger->is_primary   = is_primary;
        ...
        else
        {
            ...
            pndv_data.ar[ar_idx].ar_set_trigger_new_event        = TRUE;
...
void pndv_ar_cm_sr_ar_set_trigger_cnf(CM_UPPER_RQB_PTR_TYPE rqb_ptr)
{
    ...
    ar_idx = pndv_ar_get_ar_idx_by_session_key(session_key);
    if (ar_idx >= PNDV_CM_AR_NO)
    {
        // must not be here
        pndv_in_fatal_error( PNDV_MODULE, __LINE__, 0);
    ...
    tmp_event.add_1 = ar_idx;
    tmp_event.add_2 = rqb_ptr->args.sv.arset_trigger->is_primary;
    ...
    if (pndv_data.ar[ar_idx].ar_set_trigger_new_event == TRUE)
    {
        pndv_data.ar[ar_idx].ar_set_trigger_new_event = FALSE;
 ...
 */
void pndv_pevent_serialization_new_event_to_user(PNDV_PERI_EVENT_SERIALIZATION_T *ev_queue, PNDV_IFACE_CMD_ENTRY_T event, Unsigned8 ar_idx)
{
    ...
    ev_queue->event_array[ar_idx].in_use = TRUE;
    ev_queue->in_use_count++;
    if(ev_queue->in_use_count == 1)
    {
        ev_queue->event_array[ar_idx].sent_to_user = TRUE;
        pndv_in_peri_write_coupl_event( event );
    }
    else
    {
        ev_queue->event_array[ar_idx].sent_to_user = FALSE;
    }
}
 ...
 * @param ar_idx        ar_idx of peri-event
 */
void pndv_pevent_serialization_event_done(PNDV_PERI_EVENT_SERIALIZATION_T *ev_queue, Unsigned8 ar_idx)
{
    if(ar_idx >= sizeof(ev_queue->event_array) / sizeof(ev_queue->event_array[0]))
    {
        pndv_in_fatal_error( PNDV_MODULE, __LINE__, ar_idx);
    ...
    ev_queue->event_array[ar_idx].in_use        = FALSE;
    ev_queue->event_array[ar_idx].sent_to_user  = FALSE;
    ev_queue->in_use_count--;
    if(ev_queue->in_use_count > 0)
    {
        Unsigned8 other_ar_idx;
        for(other_ar_idx = 0; other_ar_idx < PNDV_CM_AR_NO; other_ar_idx++)
        {
            if(ev_queue->event_array[other_ar_idx].in_use)
            {
                ev_queue->event_array[other_ar_idx].sent_to_user = TRUE;
 ...
 * @return  TRUE if the event for <ar_idx> is currently active&processed by user
 */
Boolean pndv_pevent_serialization_is_event_processing(PNDV_PERI_EVENT_SERIALIZATION_T *ev_queue, Unsigned8 ar_idx)
{
    Boolean is_event_processing = FALSE;
    if(ev_queue->event_array[ar_idx].in_use && ev_queue->event_array[ar_idx].sent_to_user)
    {
        is_event_processing = TRUE;
    }
 ...
 */
Boolean pndv_pevent_serialization_is_event_queued(PNDV_PERI_EVENT_SERIALIZATION_T *ev_queue, Unsigned8 ar_idx)
{
    Boolean is_event_queued = FALSE;
    if(ev_queue->event_array[ar_idx].in_use && !ev_queue->event_array[ar_idx].sent_to_user)
    {
        is_event_queued = TRUE;
    }
 ...
 */
void pndv_pevent_serialization_remove_event(PNDV_PERI_EVENT_SERIALIZATION_T *ev_queue, Unsigned8 ar_idx)
{
    if(ar_idx >= sizeof(ev_queue->event_array) / sizeof(ev_queue->event_array[0]))
    {
        pndv_in_fatal_error( PNDV_MODULE, __LINE__, ar_idx);
        ...
        case PNDV_AR_SM_SESSION_START:
        {
            switch (event)
            {
                ...
                case PNDV_AR_SM_EVENT_CONNECT_IND_ERROR_DONE:
                {
                    ...
                    PNDV_RQB_SET_HANDLE( pndv_data.ar[ar_idx].con_ind_rqb_ptr, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
                    PNDV_RQB_SET_OPCODE( pndv_data.ar[ar_idx].con_ind_rqb_ptr, CM_OPC_SV_AR_CONNECT_RSP);

                    PNDV_CM_REQUEST( pndv_data.ar[ar_idx].con_ind_rqb_ptr);
                    break;
                }
            ...
            switch (event)
            {
                ...
                case PNDV_AR_SM_EVENT_DISCONNECT_IND:
                {
...

                    {
                        ...
                        {
                            tmp_event.add_2 = (Unsigned16)pndv_data.ar[ar_idx].disconnect_ind_rqb_ptr->args.pd.event->u.sv.ar_disconnect->disconnect.reason_code;
                        }
                        else
                        {
                            tmp_event.add_2 = 0;
                ...
                case PNDV_AR_SM_EVENT_PRM_END_IND_DONE:
                {
                    ...
                    if(pndv_data.ar[ar_idx].ar_type != CM_AR_TYPE_SINGLE_RTC3)
                    {
                        ...
                        if (pndv_data.ar[ar_idx].nr_of_iocrs != 0)
                        {
                            ...
                            //pndv_in_peri_write_coupl_event( tmp_event );
                            pndv_pevent_serialization_new_event_to_user( &pndv_data.ev_queue_ready_for_input_update, tmp_event, ar_idx );
                        }
                    }
        ...
        case PNDV_AR_SM_W_INPUT_UPDATE_RTC3:
        {
            switch (event)
            {
                case PNDV_AR_SM_EVENT_FIRST_INPUT_UPDATE_DONE:
                {
                    ...
                    pndv_pevent_serialization_event_done(&pndv_data.ev_queue_ready_for_input_update, ar_idx);
        ...
        case PNDV_AR_SM_W_DISCONNECT_IND_DONE:
        {
            switch (event)
            {
                case PNDV_AR_SM_EVENT_DISCONNECT_IND_DONE:
                {
                    ...
                    PNDV_RQB_SET_HANDLE( pndv_data.ar[ar_idx].disconnect_ind_rqb_ptr, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
                    PNDV_RQB_SET_OPCODE( pndv_data.ar[ar_idx].disconnect_ind_rqb_ptr, CM_OPC_SV_AR_DISCONNECT_RSP);

                    PNDV_CM_REQUEST( pndv_data.ar[ar_idx].disconnect_ind_rqb_ptr);
            ...
            switch (event)
            {
                case PNDV_SR_EVENT_ADD_TO_ARSET:
                {
                    ...
                    *sr_sm_state_ptr = PNDV_SR_STATE_ESTABLISH;
                    ...
                    pndv_data.sr[sr_set_idx].primary_ar_idx = ar_idx;
 ...
 *  @param[in] lock   TRUE == set event, FALSE == reset event
...
{
    UnsignedOpt     ar_idx_local;
    UnsignedOpt     tmp_ret_val;
    UnsignedOpt     ret_val;
    UnsignedOpt     one_ar;
    UnsignedOpt     i;
    Unsigned32      event_dez_trace;

    static const Unsigned32 bit_test[] = {0xAAAAAAAA, 0xCCCCCCCC, 0xF0F0F0F0, 0xFF00FF00, 0xFFFF0000};
    ...
    for (i = 4; i > 0; i--)
    {
        event_dez_trace |= (Unsigned32)((event & bit_test[i]) != 0) << i;
    }
    ...
    for(/*initialized above*/;ar_idx_local < PNDV_MAX_ARS_RT; ar_idx_local++)
    {
        ...
        if (lock)
        {
            ...
            if (old_flags)
            {
                ...
                if ( old_flags != pndv_data.ar_so_locked_state[ar_idx_local] )
                {
                    ...
                    pndv_in_write_debug_buffer_all_add__(PNDV_DC_SO_LOCKED,
                            (Unsigned16)event_dez_trace,
                            ( ( ( pndv_data.ar_so_locked_state[ar_idx_local]) <<  8 ) & 0xFFFFFF00 )
                          | ( ( ( (Unsigned32)(lock) )                        <<  5 ) & 0x00000020 )
                          | ( (  1                                            <<  4 ) & 0x00000010 )
            ...
            else
            {
                ...
                /* last parameter:
                    0xppppppti   p: still active flags 0..15 t_1: incoming / outgoing TRUE / FALSE t_0: new SO_Locked_State TRUE / FALSE i: ar_idx  */
                pndv_in_write_debug_buffer_all_add__(PNDV_DC_SO_LOCKED,
                        (Unsigned16)event_dez_trace,
                        ( ( ( pndv_data.ar_so_locked_state[ar_idx_local]) <<  8 ) & 0xFFFFFF00 )
                      | ( ( ( (Unsigned32)(lock) )                        <<  5 ) & 0x00000020 )
                ...
                else
                {
                    ...
                    tmp_ret_val = pndv_ar_abort_req( ar_idx_local, (UnsignedOpt)__LINE__ );
 ...
 */
Unsigned8 pndv_ar_get_num_connected_ars()
{
    Unsigned8 tmp_ar_idx;
    Unsigned8 num_connected_ars = 0;
...
int pndv_ar_get_ar_type(int ar_num)
{
	int ar_type = 0;
	if( (0U < ar_num) && (ar_num <= PNDV_CM_AR_NO) )
	{
		ar_type = pndv_data.ar[(ar_num -1)].ar_type;
	}
	return ar_type;
}
