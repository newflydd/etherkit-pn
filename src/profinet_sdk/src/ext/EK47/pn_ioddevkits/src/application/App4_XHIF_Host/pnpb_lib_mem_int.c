    ...
    while(PNPB_TRUE)
    {
        ...
        else    /* Cyclical data */
        {
            ...
            if(PNPB_XHIF_DIRECTION_INOUT == pnpb_submodule_params[submodule_no]->Direction)
            {
                ...
                {
#if 0
                    /* Do not report - problem in TIA project, parameterization will fail and there will be module diff block */
                    printf("Error: Invalid IO data length for INOUT 0x%x != 0x%x for sl %x su %x pos %d\n",
                       data_size, pnpb_submodule_params[submodule_no]->OutData_size, slot_no, subslot_no, submodule_no);
#endif
                }
            }
            ...
            else if( 0 != (pnpb_submodule_params[submodule_no]->Direction & PNPB_XHIF_DIRECTION_OUT))    /* direction OUT or INOUT */
            {
                ...
                if(data_size != pnpb_submodule_params[submodule_no]->OutData_size)
                {
#if 0
                    /* Do not report - problem in TIA project, parameterization will fail and there will be module diff block */
                    printf("Error: Invalid IO data length 0x%x = %d for sl %x su %x pos %d\n",
                            data_size, data_size,
                            slot_no, subslot_no, submodule_no);
#endif
                }
                else    /* Data present => Copy them to Ertec buffer*/
                {
                    Addr.Geo.Slot = slot_no;
                    Addr.Geo.Subslot = subslot_no;
