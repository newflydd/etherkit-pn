...
static PNIO_IM0_LIST_ENTRY Im0List[] = { // * --------------------------------------------
		...
		{ 0,                          // PNIO_UINT32     Api;                 // api number
				...
				{ IOD_CFG_VENDOR_ID,     // PNIO_UINT16     VendorId;            // VendorIDHigh, VendorIDLow
						...
						0x03,                   // PNIO_UINT16     ProfSpecTyp;         // IM_Profile_Spec_Type (e.g. 5="interface module")
...
//#define DEV_ANNOTATION_ORDER_ID     "6ES7 195-3BE00-0YA0"
//static PNIO_IM0_LIST_ENTRY Im0List[] = { // * --------------------------------------------
//										 // * IM0 Data for the DAP
//										 // * (optional)
//										 // * --------------------------------------------
...
//						DEV_ANNOTATION_ORDER_ID,                   // PNIO_UINT8      OrderId [20];        // Order_ID, visible, must be 20 bytes here (fill with blanks)
//						"1234567890      ",     // PNIO_UINT8      SerNum  [16];        // IM_Serial_Number, visible string, must be 16 bytes here (fill with blanks)
//						DEVKIT_HW_REVISION,     // PNIO_UINT16     HwRevision;          // IM_Hardware_Revision
//						{ DEVKIT_VERSION_PREFIX,  // PNIO_UINT8      SwRevPrefix;         // software revision prefix
//								DEVKIT_VERSION_HH,      // PNIO_UINT8      SwRevFuncEnhanc;     // IM_SW_Revision_Funct_Enhancement
//								DEVKIT_VERSION_H,      // PNIO_UINT8      SwRevBugFix;         // IM_SW_Revision_Bug_Fix
...
//						0x01,                   // PNIO_UINT8      VersMaj;             // IM_Version major
//						0x01,                   // PNIO_UINT8      VersMin;             // IM_Version minor
//						0x1e } },                 // PNIO_UINT16     ImSupported;         // IM_Supported, IM1...4 supported (bit list, bit 1...bit4, here: 0 ==> IM1..4  supported)
//
//				// * --------------------------------------------
//				// * IM0 Data for the Interface
//				// * (mandatory, because proxy for device)
//				// * --------------------------------------------
//				{ 0,                          // PNIO_UINT32     Api;                 // api number
//						0,                          // PNIO_UINT32     Slot;                // slot number (1..0x7fff)
...
//								0x01,                   // PNIO_UINT8      VersMaj;             // IM_Version major
//								0x01,                   // PNIO_UINT8      VersMin;             // IM_Version minor
//								0x00 } },                 // PNIO_UINT16     ImSupported;         // IM_Supported, IM1...4 supported (bit list, bit 1...bit4)
//
//				// * --------------------------------------------
//				// * IM0 Data for the slot 1 (64 byte input)
//				// * (mandatory, because proxy for device)
...
//								0x01,                   // PNIO_UINT8      VersMaj;             // IM_Version major (must be 1, do not change !!)
//								0x01,                   // PNIO_UINT8      VersMin;             // IM_Version minor (must be 1, do not change !!)
//								0x00 } }                  // PNIO_UINT16     ImSupported;         // IM_Supported, IM1...4 supported (bit list, bit 1...bit4, here: 0 ==> IM1..15 not supported)
//		};
...
static void PrintHelp(void) {
	...
	PNIO_ConsolePrintf("'0'      Perform System Reboot\n");
		...
		#endif //(PNIO_TRACE != PNIO_TRACE_NONE)
#endif //(PNIOD_PLATFORM  == PNIOD_PLATFORM_RZT2_EDDP)
	PNIO_ConsolePrintf("\n");
}
...

{
	...
	// 								  Api  Slot	Subslot,    ModId,     SubId, InLen, OutLen, I&M0 support
	PNIO_SUB_LIST_ENTRY tempEntry0 = { 0, 0, 1, 0x20000, 0x2, 0, 0, PNIO_IM0_SUBMODULE + PNIO_IM0_DEVICE};
	PNIO_SUB_LIST_ENTRY tempEntry1 = { 0, 0, 0x8000, 0x20000, 0x3, 0, 0, PNIO_IM0_SUBMODULE };
	PNIO_SUB_LIST_ENTRY tempEntry2 = { 0, 0, 0x8001, 0x20000, 0x4, 0, 0, PNIO_IM0_SUBMODULE };
	PNIO_SUB_LIST_ENTRY tempEntry3 = { 0, 0, 0x8002, 0x20000, 0x4, 0, 0, PNIO_IM0_SUBMODULE };
	PNIO_SUB_LIST_ENTRY tempEntry4 = { 0, 1, 1, 0xf001, 0x1, 16, 12, PNIO_IM0_SUBMODULE };
	PNIO_SUB_LIST_ENTRY tempEntry5 = { 0, 2, 1, 0xf080, 0x80, 4, 0, PNIO_IM0_SUBMODULE };
	...
	Status = PnUsr_DeviceSetup(&PnUsrDev,                                  // device specific configuration
			IoSubList,	                              // list of plugged submodules (real configuration), including PDEV
			IoSubListNum, 								// number of entries in IoSubList
	...
	while (exitAppl == PNIO_FALSE) {
		...
		{
                    /* Output */
                    if(OutDatIops[2][1] == PNIO_S_GOOD)
                    {
                        /* Get Output1Byte module data */
                        uint8_t u8Output1Byte = *((uint8_t*)&OutData[2][1][0]);
                        /* LED0 Update */
                    ...
                    *((uint8_t*)&InData[1][1][0]) = (uint8_t)(0xA0 | u8SW2 << 1 | u8SW1 << 0);
                }
                OsWait_ms(1);
