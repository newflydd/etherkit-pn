...
static CLRPC_SCALL_PTR_TYPE
clrpc_sv_scall_alloc (
	LSA_UINT32 * error_code_ptr,
	CLRPC_SERVER_PTR_TYPE server,
	CLRPC_UUID_CONST_PTR_TYPE actuid,
	LSA_UINT32 seq
	);
...
//----------------------------------------------------------------------------*/
static CLRPC_SERVER_PTR_TYPE
clrpc_epm_find_server (
	LSA_UINT16 find_method,
	CLRPC_UUID_CONST_PTR_TYPE if_id,
	LSA_UINT16 if_vers_major,
	LSA_UINT16 if_vers_minor,
	CLRPC_UUID_CONST_PTR_TYPE obj_id
	)
{
	CLRPC_SERVER_PTR_TYPE server;
	LSA_UINT16 i;
	...
	*/
	for (i = 0; i < CLRPC_CFG_MAX_SERVERS; ++i)
	{
		...
		// TODO: by dingding, make node equal, mandatory
		server->obj.object_uuid.node[1] = obj_id->node[1];
		server->obj.object_uuid.node[2] = obj_id->node[2];
		server->obj.object_uuid.node[3] = obj_id->node[3];
	...
	)
{
	...
	sock_open->reuse_addr                    = USE_CHANNEL_SETTING;
	...
	)
{
	...
	sock_close->graceful_shutdown = USE_CHANNEL_SETTING;
...
static LSA_UINT16
clrpc_pkt_is_sane (
	CLRPC_HEADER_CONST_PTR_TYPE pkt,
	LSA_UINT32 recv_len
	)
{
	...
	)
{
	...
	pkt_send->server_boot = boot_time;
	...
	)
{
	...
	pkt_send->server_boot = server->server_boot;
...
clrpc_sv_provide_dequeue (
	CLRPC_SERVER_CONST_PTR_TYPE server
	)
{
	CLRPC_UPPER_RQB_PTR_TYPE upper;
...
/*----------------------------------------------------------------------------*/
CLRPC_IF_OBJECT_PTR_TYPE
clrpc_sv_if_obj_lookup (
	CLRPC_SERVER_PTR_TYPE server,
	CLRPC_UUID_CONST_PTR_TYPE object_uuid
	)
{
	...
	)
{
		...
		*/
				 ...
				 clrpc_uuid_equal (&pkt->if_id, &clrpc_conversation_manager_uuid))
		{
			...
			if (CLRPC_PTR_IS_NULL (if_obj))
			{
				...
				clrpc_sv_send_reject (server, lower, CLRPC_NCA_UNKNOWN_IF, 0);
				return CLRPC_PROVIDE_LOWER;
			}
		}
		else
		{
			...
			*/
			...
			if (server_forward->scall_cnt >= CLRPC_CFG_MAX_SCALLS)
			{
				...
				clrpc_sv_send_reject (server, lower, CLRPC_NCA_SERVER_TOO_BUSY, 0);
				return CLRPC_PROVIDE_LOWER;
			}
			...
			if (CLRPC_PTR_IS_NOT_NULL (scall->upper))
			{
				...
				clrpc_if_obj_opnum_count_incr (&server_forward->obj
					, server_forward->pnio_signature, scall->upper->args.sv.call->opnum
					);
				clrpc_if_obj_opnum_count_decr (&server->obj
					, server->pnio_signature, scall->upper->args.sv.call->opnum
					);
			}
	...
	else
	{
		...
		if (pkt->ptype == CLRPC_PT_PING)
		{
			...
			{
				CLRPC_SV_TRACE_00 (server->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "wrong uuid - drop!");
				return CLRPC_PROVIDE_LOWER;
			}
			...
			/* do not check server_boot - TIA 1985613 */
		...
		if (! clrpc_uuid_equal (&server->if_uuid, &pkt->if_id))
		{
			...
			/* do not forward to server who just quit  TIA 1956472 */
			if (server_forward->do_unregister)
			{
				CLRPC_SV_TRACE_02 (server->trace_idx, LSA_TRACE_LEVEL_NOTE_HIGH
					, "forwarding server sid(%u) does do_unregister(%u), reprovide"
					, server_forward->id, server_forward->do_unregister
					);
				return CLRPC_PROVIDE_LOWER;
			}
		...
		if (CLRPC_PTR_IS_NULL (if_obj))
		{
			CLRPC_SV_TRACE_02 (server->trace_idx, LSA_TRACE_LEVEL_WARN
				, "obj_uuid not found in sid(%u) running at UDP-port(%u)."
				, server->id, CLRPC_HTONS (server->port)
				);
		...
		if (CLRPC_PTR_IS_NULL (scall))
		{
			...
			clrpc_sv_send_reject (server, lower, error_code, server->server_boot);
			return CLRPC_PROVIDE_LOWER;
		}
	...
	)
{
	...
	/* Note: Special case: the lower resource must be returned to original pool. See clrpc_sv_event_recv (server_forward).
	clrpc_sv_lower_done() must have done this things
...
# error "check CLRPC_CFG_MAX_SERVERS"
	...
	*/
	...
	{
		CLRPC_FATAL1 ((LSA_UINT32)LSA_NULL);
	}
...
/*----------------------------------------------------------------------------*/
static LSA_UINT32 clrpc_build_server_boot_time ()
{
	...
	)
{
	...
	{
		CLRPC_FATAL1 ((LSA_UINT32)rb_register);
	...
	if (alloc_len > 0xFFFF + (CLRPC_PNIO_OFFSET + 64U))
	{
		CLRPC_SV_TRACE_01 (trace_idx, LSA_TRACE_LEVEL_ERROR
			, "not supported: alloc_len(%u) > 0xFFFF + (CLRPC_PNIO_OFFSET + 64U)", alloc_len
			);
		return CLRPC_ERR_PARAM;
	}
	...
	)
{
	...
	if (CLRPC_PTR_IS_NULL (orig_upper_channel))
	{
		CLRPC_FATAL1 ((LSA_UINT32)server->upper);
		return;
	}
	...
	if (response != SOCK_RSP_OK)
	{
		...
		if (server->is_endpoint_mapper > 0)
		{
			...
			while (CLRPC_PTR_IS_NOT_NULL (next))
			{
				...
				if (CLRPC_PTR_IS_NULL (orig_upper_channel))
				{
					CLRPC_FATAL1 ((LSA_UINT32)orig_upper_rb);
				}
