...
static struct communityId   oha_communities[NUM_COMMUNITIES]  =
{
    {"private", OHA_SNMP_RWRITE},
    {"public", OHA_SNMP_RONLY},
};

static U_CHAR   oha_sidbuf[OHA_MAX_COMMUNITY_SIZE  +  1];   /* Community string buffer */
static unsigned oha_sidlen = 0;

/* these can't be global in a re-entrant system */
static const U_CHAR * oha_packet_end  =  LSA_NULL; /* end of built packet data */
static int oha_community;  /* access code for current packet's community string */

/*===========================================================================*/
/*                                external functions                         */
/*===========================================================================*/

static int oha_snmp_create_identical(
    U_CHAR * snmp_in, U_CHAR * snmp_out,
    unsigned snmp_length, LSA_INT32 errstat, LSA_INT32 errindex);

static int oha_snmp_create_identical_no_var_bind(
    U_CHAR * snmp_in,
    U_CHAR * snmp_out,
...
static int oha_snmp_access(
    U_SHORT acl,
    unsigned community,
    unsigned rw);

/*  int snmp_get_community(U_CHAR*); */
static int oha_snmp_get_community(
    U_CHAR*, unsigned);

static int oha_snmp_parse_var_op_list(
	OHA_OID,
    U_CHAR*,
    unsigned,
    U_CHAR*,
...
static U_CHAR * oha_snmp_auth_parse(
    U_CHAR*,
    unsigned*,
    U_CHAR*,
    unsigned*,
...
static U_CHAR * oha_snmp_auth_build(
    U_CHAR*,
    unsigned*,
    U_CHAR*,
    unsigned*,
    ...
    LSA_BOOL *  oha_oid)       /* OUT/True: it's an LLDP-Oid, False: no lldp-oid */
{
    ...
    switch ((short)outerrstat)
    {
        case OHA_SNMP_ERR_NOERROR:
             ...
             */
            header_shift = 0;
            out_length = (LSA_UINT)(oha_packet_end - out_reqid);
            if (out_length >= 0x80)
            {
                header_shift++;
                if (out_length > 0xFF)
            ...
            auth_shift = 0;
            out_length = (LSA_UINT)((oha_packet_end - out_auth) - 2 + header_shift);
            if (out_length >= 0x80)
            {
                auth_shift++;
                if (out_length > 0xFF)
            ...
            out_data = out_auth;
            out_length = (LSA_UINT)(oha_packet_end - out_auth);
            out_data = oha_snmp_auth_build(out_data, &out_length, oha_sidbuf, &oha_sidlen,
                                           &version, (unsigned)(oha_packet_end - out_header));
    ...
    int         version)
{
    ...
    *index = 1;
    ...
    while ((int)length > 0)
    {
        ...
        if (!oha_snmp_access(acl, (unsigned)oha_community, (unsigned)exact))
        {
        ...
        if (msgtype == SET_REQ_MSG)
        {
            ...
            if (oha_snmp_goodValue(var_val,var_val_type, (int)var_val_len,
            ...
            if (msgtype == SET_REQ_MSG)
            {
                ...
                if (oha_set_parms.access_method)
                {
                    ...
                    err = (*oha_set_parms.access_method)(OHA_HANDLE_TYPE_SOCK, var_val, var_val_type, (int)var_val_len,
                                                         var_name, var_name_len);
                }
                else     /* generic set of variable */
                {
                    err = oha_snmp_setVariable(var_val, var_val_type, statP, (int)statLen);
     ...
     */
    header_shift = 0;
    out_length = (unsigned)(oha_packet_end - var_list_start);
    if (out_length >= 0x80)
    {
        header_shift++;
        if (out_length > 0xFF)
    ...
    dummyLen = (unsigned)(oha_packet_end - var_list_start);
...
static int  oha_snmp_get_community( /* snmp_get_community(U_CHAR * rcvdcomm)  */
    U_CHAR * rcvdcomm,
    unsigned sidlen)
{
    int   count;
    ...
    for (count = 0; count < NUM_COMMUNITIES; count++)
    {
		...
		{
			/* Note: The length of string rcvdcomm is known(sidlen), OHA_MEMCMP compares the first    */
			/*       sidlen characters of both strings and they are equal. To be sure the two strings */
			/*       are completely equal, they must have the same length.                            */
			/*       Therefore the other string must have a '\0' on sidlen position! (RQ 4505757)     */
			if ((sidlen < OHA_MAX_COMMUNITY_SIZE) && (oha_communities[count].name[sidlen] == '\0'))
			{
				break;
			}
		}
    ...
    LSA_INT32 *   version)
{
    ...
    OHA_ASSERT_NULL_PTR(slen);
    OHA_ASSERT_NULL_PTR(sid);
