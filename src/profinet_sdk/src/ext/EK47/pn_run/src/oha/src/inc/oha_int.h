...
#ifdef OHA_INVALID_NIC_ID
# error "OHA_INVALID_NIC_ID is defined. Please remove the define from oha_cfg.h"
#endif
...
#define OHA_IS_NULL(ptr_)           (LSA_HOST_PTR_ARE_EQUAL((ptr_), LSA_NULL))
#define OHA_IS_NOT_NULL(ptr_)       (! LSA_HOST_PTR_ARE_EQUAL((ptr_), LSA_NULL))
#define OHA_ARE_EQUAL(ptr1_, ptr2_) (LSA_HOST_PTR_ARE_EQUAL((ptr1_), (ptr2_)))
...
#define OHA_MODULE_ID_OHA_DNS              0x13
...
/*===========================================================================*/
#include "sys_pck1.h"
typedef OHA_IP_TYPE * OHA_LOCAL_MEM_IP_PTR_TYPE;
...
typedef volatile OHA_INTERNAL_REQUEST_TYPE * OHA_INTERNAL_REQUEST_PTR_TYPE;
...
typedef OHA_STATS_INFO_TYPE * OHA_STATS_INFO_PTR_TYPE;
...
typedef OHA_MRP_STATS_INFO_TYPE * OHA_MRP_STATS_INFO_PTR_TYPE;
...
typedef LSA_VOID LSA_FCT_PTR(/*ATTR*/, OHA_LOCAL_CALLBACK_FCT_PTR_TYPE)(OHA_INTERNAL_REQUEST_PTR_TYPE pIntRQB);
...
typedef struct oha_timer_info_tag * OHA_TIMER_INFO_PTR_TYPE;
...
typedef struct oha_peer_tag * OHA_UPPER_PEER_PTR_TYPE;
...
typedef struct oha_adjust_tag * OHA_ADJUST_PTR_TYPE;
...
typedef struct oha_db_topo_tag * OHA_DB_TOPO_PTR_TYPE;
...
typedef struct oha_db_topo_tag
{
    ...
    OHA_PEER_TYPE       PeerSender;   /* TIA 741027: only 1 allowed */
} OHA_DB_TOPO_TYPE;
...
typedef struct
{
	...
	LSA_UINT16          LastSpecifier_Gateway_Inactive;
...
#define OHA_TOPO_MAX_INTERFACE_IND_ENTRIES            5
...
#define OHA_DIAG_GATEWAY_INACTIVE   5            /* If (GateWay Inactive)    */

typedef struct oha_last_prm_ind_data_tag * OHA_LAST_PRM_IND_DATA_PTR_TYPE;
...
#define OHA_HANDLE_STATE_OPEN_DNS_RUNNING              60
#define OHA_HANDLE_STATE_CLOSE_DNS_RUNNING             61
#define OHA_HANDLE_STATE_CANCEL_ERROR_DNS_RUNNING      64  /* cancel because of open error */
#define OHA_HANDLE_STATE_CLOSE_ERROR_DNS_RUNNING       65  /* close because of open error  */
...
typedef struct oha_sock_rqb_link_tag  * OHA_SOCK_RQB_LINK_PTR_TYPE;

typedef struct oha_hdb_sock_tag * OHA_HDB_SOCK_PTR_TYPE;
...
typedef struct oha_hdb_agnt_tag * OHA_HDB_AGNT_PTR_TYPE;

typedef struct oha_hdb_route_tag
{
	OHA_SOCK_RQB_LINK_TYPE     RQBs;               /* RQB's for SetRouter           */
	SockIF_IPAddr              Gateway;            /* SOCK associated gateway addr. */
	LSA_UINT32                 RQBCnt;             /* Number of pending Router RQBs */
	LSA_BOOL                   IsNotSet;           /* True: Set was not successful  */
	LSA_UINT32                 ExpectedGW;         /* Router address is to set      */
	LSA_UINT32                 ActivatedGW;        /* Router address, which was set */
	LSA_BOOL                   RemoveGatewayDiags; /* remove "Gateway Inactive / Mismatch" */
} OHA_HDB_ROUTE_TYPE;
...
typedef struct oha_hdb_sock_tag
{
    ...
    LSA_UINT32                       TraceIdx;     /* for indexed trace             */
    ...
    LSA_HANDLE_TYPE                  ControlUser;  /* user for SNMP activation     */
    /* ------------------------------*/
	...
	LSA_UINT32                       TcIpRQBCnt;   /* Number of pending SOCK RQBs (TimeParams) */
    OHA_USER_RQB_LINK_TYPE           TcIpUserReq;  /* TCIP user Requests (KeepAlive) */
    LSA_UINT32                       ClusterRQBCnt;/* Number of pending cluster RQBBs */
    OHA_USER_RQB_LINK_TYPE           ClusterRQBs;  /* Queued Cluster-RQBs (Config or Control) */
    LSA_UINT32                       ForwardRQBCnt;/* Number of pending Ip forward RQBs */
    OHA_USER_RQB_LINK_TYPE           ForwardRQBs;  /* Queued Ip Forward-RQBs        */

	OHA_HDB_ROUTE_TYPE               Router;       /* unique gateway */
} OHA_HDB_SOCK_TYPE;

/* --------------------------------------------------------------------------*/
/* for DNS-Systemchannel (only one DNS-Channel possible).                    */
/*                                                                           */
/* for handling all requests to DNS. We have no queue which holds            */
/* user-request blocks awaiting service. i.e. need a request to DNS.         */
/* If a free DNS-Request nlock is present pending user-requests are          */
/* removed and assotiated with a DNS-Request block.                          */
/*                                                                           */
/* --------------------------------------------------------------------------*/
#ifdef OHA_CFG_USE_DNS

/* Agent-Request-States: */
#define OHA_DNS_STATE_STOP              1  /* initial, after OpenChannel */
#define OHA_DNS_STATE_WF_STOP           2  /* DNS_STOP is active         */
#define OHA_DNS_STATE_START             3  /* DNS_START is done          */
#define OHA_DNS_STATE_WF_START          4  /* DNS_START is active        */

/* LowerLayer (DNS) Request control block  */
#ifndef OHA_DNS_RQB_PTR_TYPE
#define OHA_DNS_RQB_PTR_TYPE          DNS_UPPER_RQB_PTR_TYPE
#endif

typedef struct oha_dns_rqb_link_tag
{
    OHA_DNS_RQB_PTR_TYPE                pBottom;
    OHA_DNS_RQB_PTR_TYPE                pTop;
} OHA_DNS_RQB_LINK_TYPE;

typedef struct oha_dns_rqb_link_tag * OHA_DNS_RQB_LINK_PTR_TYPE;

typedef struct oha_hdb_dns_tag * OHA_HDB_DNS_PTR_TYPE;

typedef struct oha_hdb_dns_tag
{
    LSA_UINT8                        DnsState;     /* State of handle               */
    LSA_HANDLE_TYPE                  ThisHandle;   /* this handle                   */
    OHA_HANDLE_PARAMS_TYPE           Params;       /* general handle parameters     */
    LSA_UINT32                       TraceIdx;     /* for indexed trace             */
    OHA_UPPER_RQB_PTR_TYPE           pOCReq;       /* Open or Close Request         */
    ...
    LSA_UINT16                       UserCnt;      /* Number of User-Channels using */
                                                   /* this DNS-channel.             */
    LSA_UINT16                       RequestState; /* request state (DNS act/deact) */
    LSA_HANDLE_TYPE                  DnsHandle;    /* If InUse. the DNS handle      */
} OHA_HDB_DNS_TYPE;
#endif
...
typedef struct oha_nare_rqb_link_tag * OHA_NARE_RQB_LINK_PTR_TYPE;
...
typedef struct oha_check_info_tag * OHA_CHECK_INFO_PTR_TYPE;
...
typedef struct oha_hdb_nare_tag * OHA_HDB_NARE_PTR_TYPE;
...
typedef struct oha_hdb_user_tag * OHA_HDB_USER_PTR_TYPE;
...
typedef struct oha_hdb_user_tag
{
    ...
    OHA_USER_RQB_LINK_TYPE         DnsUserReq;        /* User Requests for DNS Set     */
    OHA_USER_RQB_LINK_TYPE         DnsGetReq;         /* User Requests for DNS Get     */
    ...
    OHA_UPPER_RQB_PTR_TYPE         pSnmpInd;         /* Indication Request for SNMP (de)activate */
...
typedef struct oha_lldp_rqb_link_tag * OHA_LLDP_RQB_LINK_PTR_TYPE;
...
typedef struct oha_hdb_lldp_tag * OHA_HDB_LLDP_PTR_TYPE;
...
typedef struct oha_frame_offset_tag * OHA_FRAME_OFFSET_PTR_TYPE;
...
typedef struct oha_edd_rqb_link_tag * OHA_EDD_RQB_LINK_PTR_TYPE;

typedef struct oha_hdb_edd_tag * OHA_HDB_EDD_PTR_TYPE;
...
typedef struct oha_hdb_edd_tag
{
    ...
    LSA_BOOL                         PendingPortIdActive; /* TFS 3730601 */

    /* 4259675 Multiple interface � Not possible to set same IP for X1 and X2 interfaces */
	OHA_SOCK_LOWER_RQB_PTR_TYPE      IpPendingRQB; /* SetIP (SockSetAddrInfo) conflicting (local) */
	SOCK_IP_SUITE_TYPE               IpPendingActive;/* conflicting IP (local)      */
	OHA_SOCK_LOWER_RQB_PTR_TYPE      SetNameAddrRQB; /* SetName (SockSetAddrInfo)   */
} OHA_HDB_EDD_TYPE;
...
typedef struct oha_mrp_rqb_link_tag * OHA_MRP_RQB_LINK_PTR_TYPE;
#endif

typedef struct oha_hdb_mrp_tag * OHA_HDB_MRP_PTR_TYPE;
...
typedef OHA_PRM_RECORD_TYPE * OHA_PRM_RECORD_PTR_TYPE;


typedef struct oha_hdb_rema_tag * OHA_HDB_REMA_PTR_TYPE;
...
typedef struct oha_sync_data_tag * OHA_SYNC_DATA_PTR_TYPE;
...
typedef struct oha_hdb_gsy_tag * OHA_HDB_GSY_PTR_TYPE;
...
typedef struct oha_hello_data_tag * OHA_HELLO_DATA_PTR_TYPE;
...
typedef struct oha_dcp_rqb_link_tag * OHA_DCP_RQB_LINK_PTR_TYPE;
...
typedef struct oha_hdb_dcp_tag * OHA_HDB_DCP_PTR_TYPE;
...
typedef struct oha_hdb_dcp_tag
{
    ...
    OHA_DCP_LOWER_RQB_PTR_TYPE  pLowerDcpRspRqb; /* DcpSet response            */
...
typedef struct oha_glob_tag * OHA_GLOB_PTR_TYPE;
...
#ifdef OHA_CFG_USE_DNS                     /* we have only one DNS channel!   */
    OHA_HDB_DNS_TYPE      HDns;                         /* DNS-Handle Mgm     */
#endif
...
#define OHA_GET_HDNS_PTR(  )  (&(OHA_Data.HDns))
...
#define OHA_HANDLE_TYPE_DNS   0xC0
...
#define OHA_HANDLE_IS_DNS(Handle)    ((OHA_HANDLE_GET_TYPE(Handle) == OHA_HANDLE_TYPE_DNS)  ? LSA_TRUE:LSA_FALSE)
...
#define OHA_EDD_DELAY_SUPPORTED(_hw_ytpe) ((_hw_ytpe & EDD_HW_TYPE_FEATURE_DELAY_MSK) == EDD_HW_TYPE_FEATURE_DELAY_SUPPORT)
...
LSA_EXTERN OHA_DB_IF_PTR_TYPE OHA_GetIf(
    LSA_UINT16 IfIndex);

/* OHA_db.c */

LSA_EXTERN LSA_VOID OHA_InitDatabase(LSA_VOID);

LSA_EXTERN LSA_VOID OHA_FreeDataBase(LSA_VOID);

LSA_EXTERN LSA_UINT16 oha_init_defvals(OHA_DB_IF_PTR_TYPE pDB);

LSA_EXTERN LSA_VOID oha_init_stats_info(OHA_OID mib_node);

LSA_EXTERN LSA_UINT16 oha_update_supported_database(
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 oha_check_access_control (
    LSA_UINT8       AccessControl );

LSA_EXTERN LSA_VOID OHA_RequestGetConfig(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestGetAddress(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestAddressIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_AddressIndicateEvent(
    LSA_UINT16              NICId,
    OHA_HDB_USER_PTR_TYPE   pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_WriteAddrInfo(
    LSA_UINT16                              NICId,
    OHA_UPPER_ADDR_INFO_PTR_TYPE            pAddress);

LSA_EXTERN LSA_VOID OHA_AddressIndicateEventAllUser(
    LSA_UINT16              NICId);

LSA_EXTERN LSA_VOID OHA_RequestSetAddress(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoSetAddressRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoWriteAddressRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoRtfRequest(
   LSA_UINT16               NICId);

LSA_EXTERN LSA_VOID OHA_RequestSetIdent(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoSetIdentRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);
...
LSA_EXTERN LSA_UINT16 oha_db_store_temp_ip_address(
    LSA_UINT16                     NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpAddrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE    NetMaskPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE    RouterPtr,
    LSA_INT                        AddrLen);

LSA_EXTERN LSA_UINT16 oha_db_read_temp_ip_address(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE      pAccCtrl,
    LSA_BOOL                      * pRemanent );

LSA_EXTERN LSA_UINT16 oha_db_store_expected_ip_suite(
    LSA_UINT16                     NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr,
    LSA_INT                        AddrLen);

LSA_EXTERN LSA_UINT16 oha_db_read_expected_ip_suite(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE      pAccCtrl,
    LSA_BOOL                      * pRemanent );

LSA_EXTERN LSA_VOID oha_db_delete_expected_ip_suite(
    LSA_UINT16                    NicId);

LSA_EXTERN LSA_UINT16 oha_db_store_emergency_ip_suite(
    LSA_UINT16                     NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr,
    LSA_INT                        AddrLen);

LSA_EXTERN LSA_UINT16 oha_db_read_emergency_ip_suite(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE      pAccCtrl,
    LSA_BOOL                      * pRemanent );

LSA_EXTERN LSA_VOID oha_db_delete_emergency_ip_suite(
    LSA_UINT16                    NicId);

LSA_EXTERN LSA_UINT16 oha_db_set_real_ip_suite(
    LSA_UINT16                  NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE  pIpAddr,
    OHA_COMMON_MEM_U8_PTR_TYPE  pSubnetAddr,
    OHA_COMMON_MEM_U8_PTR_TYPE  pRouterAddr);

LSA_EXTERN LSA_UINT16 oha_db_delete_real_ip_suite(
    LSA_UINT16                    NicId);

LSA_EXTERN LSA_UINT16 oha_db_read_ip_conflict_status(
    LSA_UINT16                  NicId,
    LSA_BOOL *                  ConflictStatus);

LSA_EXTERN LSA_VOID oha_db_delete_profibus_params(
    LSA_UINT16                    NicId);

LSA_EXTERN LSA_VOID oha_db_write_ip_conflict_status(
    LSA_UINT16                  NicId,
    LSA_BOOL                    ConflictStatus);

LSA_EXTERN LSA_VOID oha_db_write_factory_reset_status(
    LSA_UINT16                  NicId,
    LSA_BOOL                    ConflictStatus);

LSA_EXTERN LSA_UINT16 oha_db_store_temp_station_name(
    LSA_UINT16                    NicId,
    LSA_UINT8                   * NamePtr,
    LSA_UINT16                    NameLen);

LSA_EXTERN LSA_INT oha_db_read_temp_station_name(
    LSA_UINT16  NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE        * NamePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE          pAccCtrl,
    LSA_BOOL                          * pRemanent );

LSA_EXTERN LSA_UINT16 oha_db_delete_temp_station_name(
    LSA_UINT16                  NicId);

LSA_EXTERN LSA_UINT16 oha_db_delete_station_name(
    LSA_UINT16                    NicId);
...
LSA_EXTERN LSA_VOID OHA_UnlockDbUser(
    OHA_HDB_USER_PTR_TYPE       pHDBUser);

LSA_EXTERN LSA_UINT16 oha_db_delete_temp_ip_address(
    LSA_UINT16                  NicId);

LSA_EXTERN LSA_UINT16 oha_db_store_rema_ip_address(
    LSA_UINT16                     NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr);

LSA_EXTERN LSA_UINT16 oha_db_read_rema_ip_address(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr);

LSA_EXTERN LSA_UINT16 oha_db_delete_rema_ip_address(
    LSA_UINT16                  NicId);

LSA_EXTERN LSA_UINT16 oha_db_update_rema_ip_address(
    LSA_UINT16                  NicId);

LSA_EXTERN LSA_UINT16 oha_db_write_snmp_variable (
...
LSA_EXTERN LSA_UINT16 oha_db_read_snmp_variable (
	OHA_LIST_TYPED_ENTRY_TYPE(oha_db_snmp_elem_tag) * pAnchor,
...
LSA_EXTERN LSA_BOOL oha_db_is_dhcp_disabled(
	const OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) *pAnchor);

/* OHA_db_l.c */

LSA_EXTERN LSA_VOID OHA_RequestReadLldpElement(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);
...
LSA_EXTERN LSA_UINT16 oha_db_lldp_read_remanence(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,
...
LSA_EXTERN LSA_BOOL oha_db_lldp_compare_remanence(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,
    OHA_COMMON_MEM_U8_PTR_TYPE  pSubType,
...
LSA_EXTERN LSA_UINT16 oha_db_write_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,
    LSA_UINT8                   Type,       /* type of variable */
...
LSA_EXTERN LSA_UINT16 oha_db_read_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,
...
LSA_EXTERN LSA_UINT16 oha_db_read_loc_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,        /* type of variable, option */
...
LSA_EXTERN LSA_UINT16 oha_db_delete_loc_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
...
LSA_EXTERN LSA_UINT16 oha_db_delete_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,
    LSA_UINT8                   Type,       /* type of variable, option */
...
LSA_UINT16 oha_db_delete_rem_lldp_pno_others(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId);

LSA_UINT16 oha_db_delete_rem_lldp_orgspec_others(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId);

LSA_UINT16 oha_db_delete_rem_lldp_unknown_optionals(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId);

LSA_EXTERN LSA_UINT32 oha_db_count_rem_lldp_elements(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
    LSA_UINT32                  OUI);       /* OUI, if subtype=127 */

LSA_EXTERN LSA_UINT16 oha_db_read_rem_chassis_name(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
...
LSA_EXTERN LSA_UINT16 oha_db_read_rem_port_name(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,   /* index + 1 to RemoteSender[] - for SNMP */
...
LSA_EXTERN LSA_BOOL oha_port_names_valid(
    LSA_UINT16  NicId);

LSA_EXTERN LSA_VOID OHA_RequestReadLldpOptions(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestWriteLldpOption(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpAccess(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpConfigIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpConfigResponse(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_VOID OHA_RequestLldpPortConfigIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpPortConfigResponse(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);
...
LSA_EXTERN LSA_UINT16 oha_db_read_temp_lldp_mib_value(
    LSA_UINT32                  Options,
    OHA_COMMON_MEM_U32_PTR_TYPE *ppValue,
    OHA_COMMON_MEM_U16_PTR_TYPE pLength);

LSA_EXTERN LSA_UINT16 oha_db_read_temp_lldp_mib_if_value(
    LSA_UINT16                  NicId,
    LSA_UINT32                  IfOptions,
    OHA_COMMON_MEM_U8_PTR_TYPE  *ppValue,
    OHA_COMMON_MEM_U16_PTR_TYPE pLength);

LSA_EXTERN LSA_UINT16 oha_db_read_temp_lldp_mib_port_value(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
...
LSA_EXTERN LSA_UINT16 oha_db_write_temp_lldp_mib_values(
    LSA_UINT32                   Options,
	OHA_LLDP_IF_PARAM_TYPE *pConfig);

LSA_EXTERN LSA_UINT32 OHA_GetGlobQueryOptions(LSA_VOID);

LSA_EXTERN LSA_UINT32 OHA_GetPortQueryOptions(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId);

LSA_EXTERN LSA_UINT16 oha_db_write_temp_lldp_mib_port_values(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT32                  Options,
	OHA_LLDP_PORT_PARAM_TYPE *pConfig);

/* OHA_sys.c */

/* LSA_EXTERN LSA_UINT16 OHA_UserCheckHDB(LSA_VOID);  */

#define oha_db_read_loc_lldp_element OHA_DB_READ_LLDP_ELEMENT

LSA_EXTERN LSA_UINT16 oha_write_dhcp_rtf(
	LSA_UINT16                  NICId);
...
LSA_EXTERN LSA_VOID OHA_CallCbf(
    OHA_UPPER_CALLBACK_FCT_PTR_TYPE Cbf,
    OHA_UPPER_RQB_PTR_TYPE      pRQB,
...
LSA_EXTERN LSA_VOID OHA_UserRequestFinish(
    OHA_HDB_USER_PTR_TYPE      pHDB,
    OHA_UPPER_RQB_PTR_TYPE     pRQB,
    LSA_UINT16                 Status);
...
LSA_EXTERN OHA_HDB_USER_PTR_TYPE OHA_GetUserHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN OHA_HDB_DCP_PTR_TYPE OHAGetDcpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN OHA_HDB_EDD_PTR_TYPE OHAGetEddHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN OHA_HDB_SOCK_PTR_TYPE OHAGetSockHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN OHA_HDB_AGNT_PTR_TYPE OHA_SOCKIsAgentActive(LSA_HANDLE_TYPE Agent);

LSA_EXTERN LSA_VOID OHA_SOCKFreeAgent(OHA_HDB_AGNT_PTR_TYPE  pSubAgnt,
    LSA_SYS_PTR_TYPE       pSys);

#ifdef OHA_CFG_USE_DNS
LSA_EXTERN OHA_HDB_DNS_PTR_TYPE OHAGetDnsHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);
#endif

#ifndef OHA_CFG_NO_NARE
LSA_EXTERN OHA_HDB_NARE_PTR_TYPE OHAGetNareHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN LSA_UINT16 OHA_CancelCyclicNare(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish);
#endif

LSA_EXTERN OHA_HDB_LLDP_PTR_TYPE OHAGetLldpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN LSA_VOID OHA_CancelDcpControlUser(
    OHA_HDB_USER_PTR_TYPE         pHDBUser);

LSA_EXTERN LSA_BOOL OHA_CheckDcpControlUser(
    OHA_HDB_USER_PTR_TYPE         pHDBUser);


LSA_EXTERN LSA_UINT16 OHA_TOPOGetNicByInterface(
...
LSA_EXTERN LSA_BOOL OHA_IsAnyLocalCtrlClusterIpActive(
	LSA_HANDLE_TYPE UserHandle);

LSA_EXTERN LSA_BOOL OHA_IsAnyLocalIpForwardingActive(
	LSA_HANDLE_TYPE UserHandle);

LSA_EXTERN LSA_VOID OHA_DoLocalSetAddress(
    LSA_UINT16 NicID);

LSA_EXTERN LSA_VOID OHA_FreeTopoDB(
    OHA_DB_TOPO_PTR_TYPE pTopoDataBase);

LSA_EXTERN LSA_VOID OHA_InitTopo(
    OHA_HDB_USER_PTR_TYPE pHDBUser);

LSA_VOID OHA_TopoInitPort(
    OHA_HDB_USER_PTR_TYPE pHDBUser, LSA_UINT16 PortId);

LSA_EXTERN LSA_VOID OHA_FreeAllTopoDBs(
    OHA_HDB_USER_PTR_TYPE pHDBUser);

LSA_EXTERN LSA_VOID OHA_FreeTopoPeers(
    OHA_DB_TOPO_PTR_TYPE pTopoDataBase);

LSA_EXTERN LSA_UINT16 OHA_CopyTopoData(
    OHA_DB_TOPO_PTR_TYPE pDestTopoData, OHA_DB_TOPO_PTR_TYPE pSrcTopoData);

LSA_EXTERN LSA_UINT16 OHA_CopyAdjustData(
    OHA_HDB_GSY_PTR_TYPE  pHDBGsy,
    OHA_HDB_USER_PTR_TYPE pHDBUser, LSA_UINT16 PortId);

LSA_EXTERN LSA_UINT16 OHA_AdjustAdminStatus(
    LSA_UINT16 NICId,
    LSA_UINT16 PortId,
    LSA_UINT32 PeerToPeerBoundary);

LSA_EXTERN LSA_VOID OHA_UpdateFrameOffset(
    LSA_UINT16 NICId,
    LSA_UINT16 PortId,
    LSA_BOOL OnlySyncForwarding);

LSA_EXTERN LSA_UINT16 OHA_CopySyncData(
    OHA_HDB_GSY_PTR_TYPE  pHDBGsy,
    OHA_HDB_USER_PTR_TYPE pHDBUser);
...
LSA_EXTERN LSA_UINT16 OHA_CopyTopoPeersToDB(
    OHA_DB_TOPO_PTR_TYPE        pTopoDB,
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData);

LSA_EXTERN LSA_UINT16 OHA_CheckTopoPeers(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT16                  NicID,
    LSA_UINT32                  *err_offset);

LSA_EXTERN LSA_BOOL OHA_CheckDomainUUID(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData);

LSA_EXTERN LSA_BOOL OHA_CheckMRP_PortStatus(
    LSA_UINT32 MRP_PortStatus);

LSA_EXTERN LSA_BOOL OHA_CheckTopoMauType(
    LSA_UINT16                  MauType,
    LSA_BOOL                    allow_non_pnio_mautypes);

LSA_EXTERN LSA_UINT16 OHA_GetCountOfRealPeers(
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortId);

LSA_EXTERN LSA_UINT16 OHA_GetCountOfInterfaces(LSA_VOID);

LSA_EXTERN LSA_UINT16 OHA_GetUsedUserCnt(
    LSA_UINT16                  NicID);

LSA_EXTERN OHA_HDB_USER_PTR_TYPE OHA_USERIsAgentActive(LSA_HANDLE_TYPE Agent,
													   LSA_BOOL *pIsRema);

LSA_EXTERN LSA_UINT16 OHA_AllocTopoPortInd(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId);
...
LSA_EXTERN LSA_UINT16 OHA_GetSpeedFromSpeedMode(
    LSA_UINT8                   SpeedMode);

LSA_EXTERN LSA_UINT16 OHA_GetModeFromSpeedMode(
    LSA_UINT8                   SpeedMode);

LSA_EXTERN LSA_UINT16 OHA_GetAdvertisedCapFromParams(
	LSA_UINT32				AutonegCapAdvertised);

LSA_EXTERN LSA_UINT16 OHA_GetLinkStatus(
                            LSA_UINT16 Status,
...
LSA_EXTERN LSA_UINT16 OHA_GetLinkStatusChange(
    LSA_UINT16              PrevStatus,
    LSA_UINT16              ActualStatus);

LSA_EXTERN LSA_UINT16 OhaLldpSetPortDelays(
...
LSA_EXTERN LSA_UINT16 OhaLldpSetPortStatus(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
    OHA_UPPER_MEM_U8_PTR_TYPE   pPortStatus);

LSA_EXTERN LSA_UINT16 OhaLldpSetChassisMacAdr(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
    OHA_UPPER_MEM_U8_PTR_TYPE   ChassisMacAdr);

LSA_EXTERN LSA_UINT16 OhaLldpSetPortStatusMrp(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
    OHA_UPPER_MEM_U8_PTR_TYPE   pDomainUUID,
    LSA_UINT16                  MrpPortStatus);

LSA_EXTERN LSA_UINT16 OhaLldpSetPtcpStatus(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
...
LSA_EXTERN LSA_UINT16 OhaLldpSetChassisPortTTL(
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength,
    LLDP_TLV_SET_INFO_TYPE      *pTLVInfo,
	LSA_UINT32                   MultipleIfMode);

LSA_EXTERN LSA_UINT16 OHA_LldpGetMgmAddr(
...
LSA_EXTERN LSA_UINT16 OhaLldpSetUnknownOptionals(
    LSA_UINT16                              NicId,
    LSA_UINT16                              PortId,
    OHA_UPPER_MEM_PTR_TYPE                 *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE              pTLength);

LSA_EXTERN LSA_UINT16 OhaLldpSetPNOSpecifics(
    LSA_UINT16                              NicId,
    LSA_UINT16                              PortId,
    OHA_UPPER_MEM_PTR_TYPE                 *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE              pTLength);

LSA_EXTERN LSA_UINT16 OhaLldpSetIEEE802_1Extensions(
    LSA_UINT16                              NicId,
    LSA_UINT16                              PortId,
    OHA_UPPER_MEM_PTR_TYPE                 *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE              pTLength);

LSA_EXTERN LSA_UINT16 OhaLldpSetIEEE802_3Extensions(
    LSA_UINT16                              NicId,
    LSA_UINT16                              PortId,
    OHA_UPPER_MEM_PTR_TYPE                 *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE              pTLength);
...
LSA_EXTERN LSA_UINT16 OhaLldpSetOrgDefOthers(
    LSA_UINT16                              NicId,
    LSA_UINT16                              PortId,
    OHA_UPPER_MEM_PTR_TYPE                 *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE              pTLength);
...
LSA_EXTERN OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN OHA_HDB_REMA_PTR_TYPE OHA_REMAGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN OHA_HDB_GSY_PTR_TYPE OHA_GSYGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_DbStoreLocalTTL(
    LSA_UINT16 NICId,
    LSA_UINT16 TxInterval,
    LSA_UINT16 TxHoldMultiplier);

LSA_EXTERN LSA_UINT16 OHA_DbGetTxInterval(
    LSA_UINT16 NICId,
    OHA_COMMON_MEM_U16_PTR_TYPE pTxInterval);

LSA_EXTERN LSA_UINT16 OHA_DbStoreMgmAddress(
    LSA_UINT16 NICId,
    OHA_COMMON_MEM_U8_PTR_TYPE pIpaddr);
...
LSA_EXTERN LSA_UINT16 OHA_AdminStatusToSendMode(
    LSA_UINT32                  AdminStatus);

LSA_EXTERN LSA_BOOL oha_is_this_router_to_set(
	LSA_UINT16 NICId,
	LSA_UINT32 router);

LSA_EXTERN LSA_UINT16 OHA_CountGateways(LSA_UINT16 NICId, LSA_UINT16 What,
	OHA_COMMON_MEM_U32_PTR_TYPE Gateway);

LSA_EXTERN LSA_VOID OhaGetConflictIp(
    LSA_UINT16                      NicId,
...
LSA_EXTERN LSA_VOID OhaSetLocalPendingIp(
	LSA_UINT16                      NicId,
	SOCK_UPPER_IP_SUITE_PTR_TYPE    PendingIp
);

LSA_EXTERN LSA_VOID OhaResetLocalPendingIp(
	LSA_UINT16                      NicId
);

LSA_EXTERN LSA_BOOL OHA_IsIpLocalPending(
	LSA_UINT16                      NicId,
	OHA_COMMON_MEM_U8_PTR_TYPE      IpAddrPtr
);

LSA_EXTERN LSA_BOOL OHA_IsIpOnAnyOtherIfLocalPending(
	LSA_UINT16                      NICId,
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr);

LSA_EXTERN LSA_BOOL OHA_IsIpOnAnyOtherIfActive(
	LSA_UINT16                      NICId,
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr);

LSA_EXTERN LSA_UINT32 OHA_GetIfLocalPending(
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr);

LSA_EXTERN LSA_VOID OHA_UserSetAddrPending (
    OHA_HDB_USER_PTR_TYPE pHDBUser,
    LSA_UINT16 Status);

LSA_EXTERN LSA_UINT32 OHA_DbGetGlobalIfMode(
    OHA_COMMON_MEM_U16_PTR_TYPE  pCountOfMode_0,
    OHA_COMMON_MEM_U16_PTR_TYPE  pCountOfMode_1);

LSA_EXTERN LSA_BOOL OHA_CheckIpEnabled(LSA_UINT16 NICId);

/* OHA_usr.c */

LSA_EXTERN LSA_VOID OHA_USERCloseChannelDone(
    OHA_HDB_USER_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_VOID OHA_USERChannelDone(
    OHA_HDB_USER_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_UserGetFreeHDB(
    OHA_HDB_USER_PTR_TYPE *pHDBUser);

LSA_EXTERN LSA_VOID OHA_UserReleaseHDB(
    OHA_HDB_USER_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID OHA_UserChannelStateMachine(
    OHA_HDB_USER_PTR_TYPE  pHDB,
    LSA_UINT16             Response);

LSA_EXTERN LSA_VOID OHA_RequestDbControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestDcpControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestDcpIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestDcpResponse(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestPtcpWriteData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestLldpIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestGetNeighbour(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestControlSnmpOid(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestWriteDcpElement(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestReadDcpElement(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

/* OHA_topo.c */

LSA_EXTERN LSA_UINT16 OHA_TopoIndicateEvent(
    OHA_LAST_PRM_IND_DATA_PTR_TYPE          ThisInd,
    OHA_HDB_USER_PTR_TYPE                   pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_TopoIndicateEmptyEvent(
    LSA_UINT16                              PortId,
    OHA_HDB_USER_PTR_TYPE                   pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_RequestAddPrm(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
...
LSA_EXTERN LSA_UINT16 OHA_RequestAddPrmAdjust(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
    LSA_UINT16                  PortId,
    OHA_HDB_USER_PTR_TYPE       pHDBUser,
...
LSA_EXTERN LSA_UINT16 OHA_RequestAddPrmSync(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_UINT16 OHA_RequestAddIRData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
    LSA_UINT32                  RecordIndex,
    OHA_HDB_USER_PTR_TYPE       pHDBUser,
...
LSA_EXTERN LSA_UINT16 OHA_RequestAddIfAdjustData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
    LSA_UINT32                  RecordIndex,
    OHA_HDB_USER_PTR_TYPE       pHDBUser,
...
LSA_EXTERN LSA_UINT16 OHA_RequestAddPnIdentData(
    OHA_UPPER_RQB_PTR_TYPE      pRQB,
    OHA_HDB_USER_PTR_TYPE       pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_RequestAddFastStartUp(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
    LSA_UINT32                  RecordIndex,
    OHA_HDB_USER_PTR_TYPE       pHDBUser,
...
LSA_EXTERN LSA_UINT16 OHA_TopoIndicatePort(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL                CheckPeers,
    LSA_BOOL                CheckLinkStatus);

LSA_EXTERN LSA_UINT16 OHA_TopoIndicateInterface(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL                BuildEmptyEvent);

LSA_EXTERN LSA_VOID OHA_UpdateTopoInterfaces(LSA_VOID);

LSA_EXTERN LSA_VOID OHA_FreeTopoPortInd(
    OHA_HDB_USER_PTR_TYPE pHDBUser,
    LSA_UINT16   PortId);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_Chassis(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_Ports(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_MRP_domain(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_None(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_PTCP_Status(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_MAU_Type(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_RT_Class3(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_CableDelay(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare,
    LSA_UINT32          *   pExtCHannelAddValue);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_PreambleLength(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoPeer_Fragmentation(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoDelay(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoSpeed(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoMode(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoInterface_Gateway(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoInterface_NoS(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoInterface_NoD(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopoInterface_IP(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_CompareTopo_GatewayInactive(
	OHA_HDB_USER_PTR_TYPE   pHDBUser,
	LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_PrepareTopoIndication(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
...
LSA_EXTERN LSA_VOID OHA_TOPORequestHandler(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              NICId,
    LSA_UINT16              Response);

LSA_EXTERN LSA_UINT16 OHA_RequestReadMrpPrm(
    OHA_UPPER_RQB_PTR_TYPE  pRQB,
    LSA_UINT16              NICId,
    LSA_UINT16              record_index);

LSA_EXTERN LSA_VOID OHA_IndicateEventAllUsers(
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortId);

LSA_EXTERN LSA_BOOL oha_update_mrp_port_status(
    OHA_HDB_MRP_PTR_TYPE        pHDBMrp,
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortID);

LSA_EXTERN LSA_BOOL oha_update_ptcp_status(
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortID,
    LSA_BOOL                    OnlySyncForwarding);

LSA_EXTERN LSA_BOOL oha_update_port_status(
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortID,
    LSA_BOOL                    OnlySyncForwarding);

LSA_EXTERN LSA_BOOL OHA_IsWirelessPort(
    LSA_UINT16                  NICId,
    LSA_UINT16                  PortID);
...
LSA_EXTERN LSA_VOID OHA_TOPOUserCheckPending(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response);

LSA_EXTERN LSA_UINT16 OHA_DoRemotePortState(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId);

/* OHA_edd.c */

LSA_EXTERN LSA_VOID OHA_EDDChannelDone(
    OHA_HDB_EDD_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

/* LSA_EXTERN LSA_UINT16 OHA_EDDCloseChannel(    */
/*  OHA_HDB_EDD_PTR_TYPE        pHDB);                              */
LSA_EXTERN LSA_UINT16 OHA_EDDCloseChannel(
    EDD_HANDLE_LOWER_TYPE EDDHandle, /* TIA 2709127 */
    LSA_SYS_PTR_TYPE      pSys);
LSA_EXTERN LSA_VOID OHA_EDDCloseChannelDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

/* LSA_EXTERN LSA_UINT16 OHA_EDDOpenChannel(     */
/*  OHA_HDB_EDD_PTR_TYPE        pHDB);                              */
LSA_EXTERN LSA_UINT16 OHA_EDDOpenChannel(
    LSA_HANDLE_TYPE       OHAHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);
LSA_EXTERN LSA_VOID OHA_EDDOpenChannelDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_EDDGetParams(
    OHA_HDB_EDD_PTR_TYPE        pHDB,
	LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_EDDGetParamsDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_EDDReleaseHDB(
    OHA_HDB_EDD_PTR_TYPE pHDB);

LSA_EXTERN OHA_HDB_EDD_PTR_TYPE OHA_EDDGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_BOOL OHA_EDDCheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_EDDGetFreeHDB(
    OHA_HDB_EDD_PTR_TYPE *pHDBEdd);

LSA_EXTERN LSA_UINT16 OHA_EDDLinkIndProvide(
    OHA_HDB_EDD_PTR_TYPE     pHDB,
    LSA_UINT16               PortCnt);

LSA_EXTERN LSA_VOID OHA_EDDLinkIndDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE      pRQB,
    OHA_HDB_EDD_PTR_TYPE            pHDB);

LSA_EXTERN LSA_VOID OHA_EddIndicateEventAllUsers(
    OHA_HDB_EDD_PTR_TYPE        pHDBEdd,
    LSA_UINT16                  PortId);

LSA_EXTERN LSA_UINT16 OHA_EDDGetPortParams(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_EDDGetPortParamsDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_EDDControlSignal(
    LSA_UINT16  NicId,
    LSA_UINT8 Frequency);

LSA_EXTERN LSA_VOID OHA_EDDControlSignalDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_BOOL OHA_EDDIsAtLeastOnePortUp(
    LSA_UINT16                  NICId);

LSA_EXTERN LSA_UINT16 OHA_EDDSetRemotePortState(
    LSA_UINT16                  NicId,
    EDD_RQB_SET_REMOTE_PORT_STATE_TYPE RemotePortState);

LSA_EXTERN LSA_VOID OHA_EDDSetRemotePortStateDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_EDDSetArpFilter(
    LSA_UINT16                  NICId,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_UINT16 OHA_EDDSetIp(
    LSA_UINT16                  NICId,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_EDDSetArpFilterDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_EDDSetIpDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_EDD_SetFilter(
    LSA_UINT16	                NICId,
    LSA_UINT8                   Mode,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_EDD_SetFilterDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB);

/* OHA_dcp.c */

LSA_EXTERN LSA_UINT16 OHA_DCPOpenChannel(
    LSA_HANDLE_TYPE       NAREHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_DCPOpenChannelDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DCPCloseChannel(
    LSA_HANDLE_TYPE       DCPHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_DCPCloseChannelDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_DCPChannelDone(
    OHA_HDB_DCP_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_DCPCancelReceive(
    LSA_HANDLE_TYPE       DCPHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_DCPCancelReceiveDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB);

LSA_EXTERN OHA_HDB_DCP_PTR_TYPE OHA_DCPGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_BOOL OHA_DCPCheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_DCPGetFreeHDB(
    OHA_HDB_DCP_PTR_TYPE *pHDBDcp);

LSA_EXTERN LSA_VOID OHA_DCPReleaseHDB(
    OHA_HDB_DCP_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID OHA_DCPRecvDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_DCPSendDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DCPRecvProvide(
    OHA_HDB_DCP_PTR_TYPE     pHDB,
    LSA_UINT32               Cnt,
    LSA_UINT8                ServiceId);

LSA_EXTERN LSA_UINT16 oha_dcp_Get(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE         pHDB);

LSA_EXTERN LSA_UINT16 oha_dcp_Identify(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE         pHDB);

LSA_EXTERN LSA_UINT16 oha_dcp_Set(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE         pHDB,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pSetInd);

LSA_EXTERN LSA_UINT16 oha_dcp_Hello (
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE         pHDB);

LSA_EXTERN LSA_UINT16 OHA_DcpIndicateEvent(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE    pEvent);

LSA_EXTERN LSA_VOID OHA_DcpIndicateCheckIpEvent(
	LSA_UINT16                  NicId);

LSA_EXTERN LSA_VOID OHA_DcpIndicateEventAllUsers(
    OHA_HDB_DCP_PTR_TYPE         pHDBDcp,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE    pEvent);

LSA_EXTERN LSA_VOID OHA_DcpIndicateEventAllOtherUsers(
    OHA_UPPER_RQB_PTR_TYPE       pRQB,
    LSA_UINT16                   NicId);

LSA_EXTERN LSA_VOID OHA_DcpHelloEventAllUsers(
    OHA_UPPER_HELLO_PTR_TYPE    pEvent,
    LSA_UINT16                  NicId);

LSA_EXTERN OHA_DCP_LOWER_RQB_PTR_TYPE  oha_dcp_alloc_rsp_rqb(
    OHA_HDB_DCP_PTR_TYPE       pHDB);

LSA_EXTERN LSA_VOID oha_dcp_free_rsp_rqb(
    OHA_HDB_DCP_PTR_TYPE        pHDB,
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB);

LSA_EXTERN LSA_VOID oha_dcp_free_rcv_rqb(
    OHA_HDB_DCP_PTR_TYPE        pHDB,
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB);

LSA_EXTERN LSA_VOID OHA_DcpSetAllNics(
    LSA_UINT8                   Mode,
    LSA_UINT32                  Options);

LSA_EXTERN LSA_VOID OHA_DCPRequestHandler(
    OHA_HDB_DCP_PTR_TYPE        pHDBDcp,
    OHA_UPPER_RQB_PTR_TYPE      pRQBUser,
    LSA_UINT16                  Response);

LSA_EXTERN LSA_UINT16 OHA_DCPUpdateFilter(
    LSA_UINT16 NICId,
    LSA_UINT32 ControlStatus,
    LSA_UINT16 Orginator);

LSA_EXTERN LSA_UINT16 OHA_DcpWriteAccessControl(
    LSA_UINT16 NicId,
    LSA_UINT32 options,
    LSA_UINT8 control);

LSA_EXTERN LSA_UINT16 OHA_DcpGetReason(LSA_UINT8 reason);

LSA_EXTERN LSA_UINT16 OHA_DcpWriteUserControl(
	LSA_UINT16      NicId,
    LSA_HANDLE_TYPE UserHandle,
    OHA_UPPER_DCP_CONTROL_PTR_TYPE control);

LSA_EXTERN LSA_UINT16 oha_dcp_ParseSet(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    const OHA_HDB_DCP_TYPE * const pHDB,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pSetInd);

LSA_EXTERN LSA_UINT16 oha_dcp_QuerySet(
    OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_DCP_PTR_TYPE         pHDB,
...
LSA_EXTERN LSA_VOID OHA_RequestHelloIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);
...
LSA_EXTERN LSA_UINT16 OHA_DoDcpResponseRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoDcpSetAddr(
    OHA_HDB_DCP_PTR_TYPE    pHDBDcp);

LSA_EXTERN LSA_VOID OHA_FinishDcpResponse(
    OHA_HDB_DCP_PTR_TYPE     pHDBDcp,
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               Response);

LSA_EXTERN LSA_BOOL  oha_is_valid_dhcp_client_id(
    LSA_UINT16                  Length,
    OHA_COMMON_MEM_U8_PTR_TYPE  pOption);

LSA_EXTERN LSA_BOOL  oha_is_valid_dhcp_usage(
	LSA_UINT16                  Length,
	OHA_COMMON_MEM_U8_PTR_TYPE  pOption);

LSA_EXTERN LSA_VOID OHA_DcpIndicateEventExtUsers(
    LSA_UINT16                          NicId,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE   pEvent);

/* OHA_lldu.c */

LSA_EXTERN LSA_VOID OHA_LLDPUserCheckPending(
    OHA_HDB_LLDP_PTR_TYPE   pHDBLldp,
    LSA_UINT16              Response);

LSA_EXTERN LSA_VOID OHA_RequestLldpStartSend(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_VOID OHA_SNMPUserCheckPending(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response);

LSA_EXTERN LSA_UINT16 OHA_CancelLldpUser(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish);

LSA_EXTERN LSA_VOID OHA_RequestSnmpIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_SnmpIndicateEvent(
    OHA_HDB_USER_PTR_TYPE                   pHDB,
    LSA_UINT32                              SNMPActivation);

LSA_EXTERN OHA_HDB_USER_PTR_TYPE OHA_SnmpGetEventUser(LSA_VOID);

LSA_EXTERN LSA_UINT32 OHA_SNMPGetOidElement(
    OHA_UPPER_GET_OID_PTR_TYPE pSnmpOid,
	OHA_OID                    Node);

/* OHA_lldp.c */

LSA_EXTERN LSA_VOID OHA_LLDPChannelDone(
    OHA_HDB_LLDP_PTR_TYPE      pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_LLDPOpenChannel(
    LSA_HANDLE_TYPE       NAREHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_LLDPOpenChannelDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPCloseChannel(
    LSA_HANDLE_TYPE       LLDPHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_LLDPCloseChannelDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPRecvProvide(
    OHA_HDB_LLDP_PTR_TYPE    pHDB,
    LSA_UINT32               Cnt);

LSA_EXTERN LSA_VOID OHA_LLDPRecvDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPGetConfig(
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_LLDPGetConfigDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPSetGlobals(
    LSA_UINT16              NicId,
    LSA_UINT16              Orginator,
...
LSA_EXTERN LSA_VOID OHA_LLDPSetGlobalsDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN OHA_HDB_LLDP_PTR_TYPE OHA_LLDPGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_BOOL OHA_LLDPCheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_LLDPGetFreeHDB(
    OHA_HDB_LLDP_PTR_TYPE *pHDBEdd);

LSA_EXTERN LSA_VOID OHA_LLDPReleaseHDB(
    OHA_HDB_LLDP_PTR_TYPE pHDB);

LSA_UINT16 OHA_LLDPSet(
    OHA_HDB_LLDP_PTR_TYPE        pHDB,
    LSA_UINT16                   PortId,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPSetDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);
...
LSA_EXTERN LSA_UINT16 OHA_LLDPStart(
    OHA_HDB_LLDP_PTR_TYPE        pHDB,
    LSA_UINT16                   PortId,
    LSA_UINT16                   AdminStatus,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_UINT16 OHA_LLDPStartAll(
    OHA_HDB_LLDP_PTR_TYPE        pHDB,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPStartDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPStop(
    OHA_HDB_LLDP_PTR_TYPE        pHDB,
    LSA_UINT16                   PortId,
    LSA_UINT16                   AdminStatus,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPStopDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPGetStats(
    OHA_HDB_LLDP_PTR_TYPE        pHDB,
    LSA_UINT16                   PortId,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPGetStatsDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_LLDPUpdateGlobals(
    LSA_UINT16                   NicID,
	LSA_UINT32                   SetIfOptions,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPSetMibOptions(
	LSA_UINT32                   IfSetOptions,
	LSA_BOOL					 IsRema);

LSA_EXTERN LSA_UINT16 OHA_LLDPSetTempIfOptions(
    LSA_UINT16                  NicId,
	LSA_UINT32                  IfSetOptions);

LSA_EXTERN LSA_UINT16 OHA_LldpIndicateEvent(
    OHA_HDB_LLDP_PTR_TYPE        pHDBLldp,
    OHA_HDB_USER_PTR_TYPE        pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_LldpIndicateConfigEvent(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    LSA_UINT32                   Options,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LldpIndicateEventAllUsers(
    OHA_HDB_LLDP_PTR_TYPE        pHDBLldp,
    LSA_UINT16                   PortId);

LSA_EXTERN LSA_VOID OHA_LldpIndicateConfigEventAllUsers(
    OHA_HDB_LLDP_PTR_TYPE        pHDBLldp,
    LSA_UINT32                   Options,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_VOID OHA_LldpIndicatePortConfigEventAllUsers(
    OHA_HDB_LLDP_PTR_TYPE        pHDBLldp,
    LSA_UINT16                   PortId,
    LSA_UINT32                   Options,
    LSA_UINT16                   Orginator);

LSA_EXTERN LSA_UINT16 OHA_LLDPUpdateSend(
    LSA_UINT16 NICId,
    LSA_UINT32 ControlStatus,
    LSA_UINT16 Orginator);

LSA_EXTERN LSA_VOID OHA_LLDPUpdateSendGlobal(LSA_VOID);

OHA_HDB_LLDP_PTR_TYPE OHA_LLDPGetHandleByIf(
    LSA_UINT16 IfId);

OHA_HDB_LLDP_PTR_TYPE OHA_LLDPGetHandleByIndex(
    LSA_UINT16 Index);
...
LSA_EXTERN LSA_VOID OHA_FreeRemoteSender(
    OHA_LLDP_SENDER_PTR_TYPE    pRemoteSender,
    OHA_LOCAL_MEM_U16_PTR_TYPE  pCountOfDeletedElem);

LSA_EXTERN LSA_VOID OHA_InitRemoteSender(
    OHA_LLDP_SENDER_PTR_TYPE pRemoteSender);

LSA_EXTERN LSA_VOID OHA_RequestReadQueryElements(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OhaLldpGetParams(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,
...
LSA_EXTERN LSA_UINT16 OHA_LLDPDoStats(
    LSA_UINT16                  Orginator,
	LSA_UINT8                   MessageType,
	LSA_BOOL                    IsRema);

LSA_EXTERN LSA_UINT16 OHA_LLDPPrepareStats(
    LSA_UINT16                   Orginator,
	OHA_COMMON_MEM_U16_PTR_TYPE  pCountOfQueries,
	LSA_BOOL                     MakeRema);

LSA_EXTERN LSA_UINT16 OHA_LLDPUpdateStats(
    OHA_DB_IF_PTR_TYPE          pDB,
    LSA_UINT16                  PORTId,
    LSA_UINT16                  SenderId,
    LSA_UINT16                  Event);

LSA_EXTERN LSA_UINT16 OHA_LldpQueryConfigEvent(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    LSA_UINT32                   Options,
	LSA_BOOL                     MakeRema);

LSA_EXTERN LSA_UINT16 OHA_LldpQueryPortConfigEvent(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
	LSA_UINT16					 PortId,
    LSA_UINT32                   Options);

LSA_EXTERN LSA_UINT16 OHA_LldpIndicatePortConfigEvent(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
	LSA_UINT16					 PortId,
    LSA_UINT32                   Options,
    LSA_UINT16                   Orginator);
...
#ifndef OHA_CFG_NO_MRP
LSA_EXTERN OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByIf(
    LSA_UINT32 IfId);

LSA_EXTERN OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByIndex(
    LSA_UINT32 Index);

LSA_EXTERN OHA_HDB_MRP_PTR_TYPE OHAGetMrpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN LSA_BOOL OHA_MRPCheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_MRPGetFreeHDB(
    OHA_HDB_MRP_PTR_TYPE *pHDBMrp);

LSA_EXTERN LSA_VOID OHA_MRPReleaseHDB(
    OHA_HDB_MRP_PTR_TYPE pHDB);

LSA_EXTERN LSA_UINT16 OHA_MRPOpenChannel(
    LSA_HANDLE_TYPE       OHAHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_MRPOpenChannelDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_MRPCloseChannel(
    LSA_HANDLE_TYPE       MRPHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_MRPCloseChannelDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_MRPRecvProvide(
    OHA_HDB_MRP_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_MRPRecvDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_MRPGetStatus(
    OHA_HDB_MRP_PTR_TYPE        pHDB,
	LSA_UINT16                  Orginator,
    LSA_UINT16                  Instance);

LSA_EXTERN LSA_VOID OHA_MRPGetStatusDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_MRPSetStatus(
    OHA_HDB_MRP_PTR_TYPE       pHDB,
    LSA_UINT16                  Orginator,
    LSA_UINT16                  Instance);

LSA_EXTERN LSA_VOID OHA_MRPSetStatusDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_MrpWriteData(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_MRPChannelDone(
    OHA_HDB_MRP_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

/* OHA_mrpr.c */

LSA_EXTERN LSA_UINT16 OHA_MRPCountStats(
    LSA_UINT16                   Orginator,
	LSA_UINT8                   MessageType);
...
LSA_EXTERN OHA_HDB_SOCK_PTR_TYPE OHA_SOCKGetHandleGlobal(
    LSA_VOID);

LSA_EXTERN LSA_UINT16 OHA_SOCKGetFreeHDB(
    OHA_HDB_SOCK_PTR_TYPE *pHDBSock);

LSA_EXTERN LSA_VOID OHA_SOCKReleaseHDB(
    OHA_HDB_SOCK_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID OHA_SOCKChannelDone(
    OHA_HDB_SOCK_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_SOCKOpenChannel(
    LSA_HANDLE_TYPE       SOCKHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_SOCKOpenChannelDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKCloseChannel(
    LSA_HANDLE_TYPE       SOCKHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_SOCKCloseChannelDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKRecvProvide(
    OHA_HDB_SOCK_PTR_TYPE    pHDB,
	OHA_OID                  MibNode,
    LSA_UINT32               Cnt);

LSA_EXTERN LSA_VOID OHA_SOCKRecvDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_SOCKParseDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRxRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_SNMPSockCheckPending(
    OHA_HDB_SOCK_PTR_TYPE   pHDBSock,
	OHA_OID                 MibNode,
    LSA_UINT16              Response);

LSA_EXTERN LSA_UINT16 OHA_SOCKOpenPort(
    OHA_HDB_SOCK_PTR_TYPE    pHDB,
	OHA_OID                  MibNode);

LSA_EXTERN LSA_VOID OHA_SOCKOpenPortDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKSetMib2Status(
    LSA_UINT16              NicID,
    LSA_UINT16              PortID,
    LSA_UINT16              Orginator);

LSA_EXTERN LSA_UINT16 OHA_SOCKUpdateMib2Params(
    LSA_UINT16              Orginator,
...
LSA_EXTERN LSA_VOID OHA_SOCKSetMib2StatusDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKSetMib2Params(
    LSA_UINT16              Orginator);

LSA_EXTERN LSA_VOID OHA_SOCKSetMib2ParamsDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKMib2ParamsInd(
    LSA_UINT16              Orginator);

LSA_EXTERN LSA_VOID OHA_SOCKMib2ParamsIndDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKClosePort(
    OHA_HDB_SOCK_PTR_TYPE    pHDB,
	OHA_OID                  MibNode);

LSA_EXTERN LSA_VOID OHA_SOCKClosePortDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);
...
LSA_EXTERN LSA_VOID OHA_SOCKRequestHandler(
    OHA_HDB_SOCK_PTR_TYPE       pHDBSock,
    OHA_OID                     mib_node,
    LSA_UINT16                  Response);

LSA_EXTERN LSA_VOID OHA_RequestSockControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestControlTcipData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestControlTcipDataDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_SOCKUserSetIpPending(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response);

LSA_EXTERN LSA_UINT16 OHA_SOCKSetIpAddress(
    LSA_UINT16                      NicID,
    LSA_UINT16                      Orginator,
    OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr,
...
LSA_EXTERN LSA_UINT16 OHA_SOCKSetRouter(
	LSA_UINT16                      NicID,
	LSA_UINT16                      Orginator,
	LSA_UINT32                      GateWay);

LSA_EXTERN LSA_UINT16 OHA_SOCKControlTcipData(
    LSA_UINT16                      Orginator,
	LSA_OPCODE_TYPE                 Opcode,
    LSA_UINT16                      KeepAliveTime);

LSA_EXTERN LSA_UINT16 OHA_SOCKIPv4Forwarding(
	LSA_OPCODE_TYPE                 Opcode,
	LSA_UINT16                      Orginator,
	LSA_UINT8                       State);

LSA_EXTERN LSA_VOID OHA_SOCKIPv4ForwardingDone(
	OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
	OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKSetClusterIPControl(
	LSA_UINT32                      InterfaceID,
	LSA_UINT16                      Orginator,
	LSA_UINT32                      ClusterIpRole);

LSA_EXTERN LSA_UINT16 OHA_SOCKSetClusterIPv4Suite(
	LSA_UINT32                      InterfaceID,
	LSA_UINT16                      Orginator,
...
LSA_EXTERN LSA_VOID OHA_SOCKSetClusterIPv4SuiteDone(
	OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
	OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKGetClusterIP(
	LSA_UINT32                      InterfaceID,
	LSA_UINT16                      Orginator);

LSA_EXTERN LSA_VOID OHA_SOCKGetClusterIPDone(
	OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
	OHA_HDB_SOCK_PTR_TYPE        pHDB);
...
LSA_EXTERN LSA_VOID OHA_SOCKSetIpAddressDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_SOCKSetRouterDone(
	OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
	OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_SOCKShutDown(
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_CancelSockUser(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish);

LSA_EXTERN LSA_BOOL OHA_CheckSockUser(
    OHA_HDB_USER_PTR_TYPE         pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_SOCKGetMib2Status(
    LSA_UINT16                  NicID,
	LSA_UINT16                  PortID,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_SOCKGetMib2StatusDone(
    OHA_SOCK_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_SOCK_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_InitAddrInfo(
    LSA_UINT16                  NicID);
...
LSA_EXTERN LSA_VOID OHA_RequestDns(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN OHA_HDB_DNS_PTR_TYPE OHA_DNSGetHandle(
    LSA_VOID);

LSA_EXTERN LSA_UINT16 OHA_DNSGetFreeHDB(
    OHA_HDB_DNS_PTR_TYPE *pHDBDns);

LSA_EXTERN LSA_VOID OHA_DNSReleaseHDB(
    OHA_HDB_DNS_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID OHA_DNSChannelDone(
    OHA_HDB_DNS_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_DNSOpenChannel(
    LSA_HANDLE_TYPE       DNSHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_DNSOpenChannelDone(
    OHA_DNS_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DNS_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DNSCloseChannel(
    LSA_HANDLE_TYPE       DNSHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_DNSCloseChannelDone(
    OHA_DNS_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DNS_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DNSSet(
    OHA_HDB_DNS_PTR_TYPE    pHDB,
    LSA_UINT16              Orginator,
    OHA_IP_TYPE            *pIpAddrList);

LSA_EXTERN LSA_VOID OHA_DNSSetDone(
    OHA_DNS_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DNS_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID OHA_DNSGetDone(
    OHA_DNS_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DNS_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DNSGetServerData(
    LSA_UINT16               Orginator);
...
LSA_EXTERN OHA_HDB_NARE_PTR_TYPE OHA_NAREGetHandleByNIC(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_BOOL OHA_NARECheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN LSA_UINT16 OHA_NAREGetFreeHDB(
    OHA_HDB_NARE_PTR_TYPE *pHDBNare);

LSA_EXTERN LSA_VOID OHA_NAREReleaseHDB(
    OHA_HDB_NARE_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID OHA_NAREChannelDone(
    OHA_HDB_NARE_PTR_TYPE       pHDB,
    LSA_UINT16                 Response);

LSA_EXTERN LSA_UINT16 OHA_NAREOpenChannel(
    LSA_HANDLE_TYPE       NAREHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_NAREOpenChannelDone(
    OHA_NARE_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_NARE_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_NARECloseChannel(
    LSA_HANDLE_TYPE       NAREHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN LSA_VOID OHA_NARECloseChannelDone(
    OHA_NARE_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_NARE_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_NARETestIp(
    LSA_UINT16                   NicID,
    OHA_COMMON_MEM_U8_PTR_TYPE   IpAddrPtr,
    LSA_UINT                     AddrLen,
    LSA_UINT16                   Orginator,
...
LSA_EXTERN LSA_VOID OHA_NARECheckIpDone(
    OHA_NARE_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_NARE_PTR_TYPE        pHDB);
...
LSA_EXTERN LSA_VOID OHA_NARETestIpCancelDone(
    OHA_NARE_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_NARE_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_FreeCyclicNareRqbs(
    OHA_HDB_USER_PTR_TYPE        pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_NAREHelloSend(
    LSA_UINT16                  NICId,
    LSA_UINT32                  HelloRetry,
    LSA_UINT32                  HelloInterval,
    LSA_UINT32                  HelloDelay);

LSA_EXTERN LSA_UINT16 OHA_NAREHelloStop(
    LSA_UINT16                  NICId,
    LSA_UINT16                  Orginator);

LSA_EXTERN LSA_VOID OHA_NAREHelloDone(
    OHA_NARE_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_NARE_PTR_TYPE        pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoHello(
    LSA_UINT16                  NICId,
    LSA_UINT16                  AdditionalDelay);

LSA_EXTERN LSA_UINT16 OHA_NARECheckAddress(
    LSA_UINT16                  NicID,
    LSA_UINT32                  CheckOptions,
    LSA_UINT32                  Options);

LSA_EXTERN LSA_BOOL OHA_NARECheckAddressCancel(
    LSA_UINT16                  NicID,
    LSA_UINT32                  CheckOptions,
    LSA_UINT16                  Orginator);
#endif
...
LSA_EXTERN LSA_VOID OHA_RequestPrmPrepare(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_DoPrmPrepare(
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_VOID OHA_RequestPrmWrite(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWritePDEV(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteStation(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteSNMP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteBUS(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteDNS(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteSNMPControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmIPForwarding(
	OHA_UPPER_RQB_PTR_TYPE   pRQB,
	OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteClusterIP(
	OHA_UPPER_RQB_PTR_TYPE   pRQB,
	OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteLLDP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmControlTCP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmWriteDHCP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoPrmWriteStation(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_DoPrmWriteSNMP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestPrmEnd(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestPrmChangePort(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestPrmCommit(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_PrmCommit(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_VOID OHA_RequestPrmRead(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestPrmIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_Check_PdPortData(
    OHA_HDB_USER_PTR_TYPE    pHDBUser,
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_BOOL                 IsWireless,
    LSA_UINT32                  * err_offset);

LSA_EXTERN LSA_UINT16 OHA_Check_PdPortDataAdjust(
    OHA_HDB_USER_PTR_TYPE    pHDBUser,
    OHA_UPPER_RQB_PTR_TYPE   pRQB);

LSA_EXTERN LSA_UINT16 OHA_Check_PdSyncData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB);

LSA_EXTERN LSA_UINT16 OHA_Check_PdIRData(
    OHA_HDB_USER_PTR_TYPE    pHDBUser,
    OHA_UPPER_RQB_PTR_TYPE   pRQB);

LSA_EXTERN LSA_UINT16 OHA_Check_FastStartUp(
    OHA_UPPER_RQB_PTR_TYPE   pRQB);

LSA_EXTERN LSA_UINT16 OHA_RequestReadRealPrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestReadRealInterfacePrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestReadPDInterfacePrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestReadPNIdentPrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestReadFastStartUpPrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestReadExpectedPrm(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_UINT16 OHA_RequestReadAdditionalInfo(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestPrmReadStation(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_PrmReadDHCP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_VOID OHA_PrmReadDNS(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser);

LSA_EXTERN LSA_VOID OHA_PrmReadClusterIP(
	OHA_UPPER_RQB_PTR_TYPE   pRQB,
	OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_UINT16 OHA_RequestPrmReadSNMP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestPrmReadBus(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_RequestPrmReadTCP(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_PrmReadLLDPIfData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               NicId);

LSA_EXTERN LSA_UINT16 OHA_Put_PrmLLDPPortData(
    LSA_UINT16                  NICId,
	LSA_UINT16                  PortId,
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
...
LSA_EXTERN LSA_UINT16 OHA_CompareTopoMAU(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare);

LSA_EXTERN LSA_UINT16 OHA_GetExt_PdPortDataExpected(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecordData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_UINT16 OHA_GetExt_PdPortDataAdjusted(
...
LSA_EXTERN LSA_UINT16 OHA_Get_PdIRData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    LSA_UINT32                  RecordLen,
    OHA_COMMON_MEM_U8_PTR_TYPE  * ppIRDataUUID,
    LSA_UINT32                  * err_offset);

LSA_EXTERN LSA_UINT16 OHA_Get_PdSyncData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_UINT16 OHA_Get_NameOfStationData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_UINT16 OHA_Get_SNMPSysData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_UINT16 OHA_Get_IpSuiteData(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    LSA_UINT32                  RecordLen,
...
LSA_EXTERN LSA_VOID OHA_RequestRemaIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestRemaControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_BOOL OHA_IsRemaResponsePending(LSA_UINT32 InterfaceId);

LSA_EXTERN LSA_UINT16 oha_rema_prm_prepare(
    LSA_UINT16 NicID,
    LSA_BOOL is_commit);

LSA_EXTERN LSA_UINT16 oha_rema_prm_write(
    LSA_UINT16              NicID,
    OHA_UPPER_RQB_PTR_TYPE  rqb);

LSA_EXTERN LSA_UINT16 oha_rema_prm_end(
    LSA_UINT16              NICId);

LSA_EXTERN LSA_UINT16 oha_rema_prm_commit(
    LSA_UINT16             NicID,
    LSA_HANDLE_TYPE        ChannelHandle,
    LSA_BOOL               do_rtf);

LSA_EXTERN LSA_UINT16 oha_rema_station_update(
    LSA_UINT16             NicID);

LSA_EXTERN LSA_UINT16 oha_rema_dhcp_update(
    LSA_UINT16             NicID);

LSA_EXTERN LSA_UINT16 oha_rema_snmp_update(
    LSA_UINT16             NicID);

LSA_EXTERN LSA_VOID oha_rema_snmp_update_global( LSA_VOID );

LSA_EXTERN LSA_UINT16 oha_rema_bus_update(
    LSA_UINT16             NicID);

LSA_EXTERN LSA_VOID oha_rema_init(LSA_VOID); /* AP01039458 */

LSA_EXTERN LSA_VOID oha_PDEV_undo_init(
    LSA_UINT16  NicID);

LSA_EXTERN LSA_VOID oha_station_undo_init(
    LSA_UINT16  NicID);

LSA_EXTERN LSA_VOID oha_snmp_undo_init(
    LSA_UINT16  NicID);

LSA_EXTERN LSA_VOID oha_snmp_undo_init_global( LSA_VOID );

LSA_EXTERN LSA_VOID oha_bus_undo_init(
    LSA_UINT16  NicID);

LSA_EXTERN LSA_VOID oha_rema_prm_free_recordset_a(
    LSA_UINT16  NICId);

LSA_EXTERN LSA_VOID OHA_RequestRemaRead(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

LSA_EXTERN LSA_VOID OHA_RequestRemaWrite(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB);

/* oha_tim.c */

LSA_EXTERN LSA_UINT16 OHA_TimerAlloc(
...
LSA_EXTERN LSA_UINT16 OHA_TimerFree(LSA_UINT16 OHATimerID);
LSA_EXTERN LSA_VOID OHA_TimerMgmSetup(LSA_VOID);

LSA_EXTERN LSA_VOID OHA_TimerStop(
    LSA_UINT16 OHATimerID);

LSA_EXTERN LSA_VOID OHA_TimerStart(
    LSA_UINT16 OHATimerID,
    LSA_UINT16 Time);

LSA_EXTERN LSA_VOID OHA_TimerRQBFree(
    LSA_UINT16 OHATimerID);

LSA_EXTERN LSA_VOID OHA_TimerCB(
    OHA_INTERNAL_REQUEST_PTR_TYPE   pIntRQB);

LSA_EXTERN LSA_VOID OHA_RequestTimer(
    OHA_UPPER_RQB_PTR_TYPE pRQB);

LSA_EXTERN LSA_VOID OHA_RequestCyclicTimer(
    OHA_UPPER_RQB_PTR_TYPE pRQB);

LSA_EXTERN LSA_VOID OHA_RequestCyclicCheck(
    OHA_UPPER_RQB_PTR_TYPE pRQB);

LSA_EXTERN LSA_VOID OHA_RequestPtcpTimer(
    OHA_UPPER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId);

LSA_EXTERN LSA_UINT16 OHA_CancelPtcpTimer(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish);

LSA_EXTERN LSA_UINT16 OHA_CancelCyclicTimer(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish);
...
LSA_EXTERN LSA_VOID OHA_SOCKUserSetMib2ParamsPending(
	OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response);

LSA_EXTERN LSA_VOID OHA_SOCKUserCheckPending(
    OHA_HDB_SOCK_PTR_TYPE   pHDBSock,
    LSA_UINT16              Response);
...
/*                                   macros                                  */
/*===========================================================================*/
/*lint -function(exit, OHA_FatalError) */
#define OHA_FATAL(Error) \
	OHA_FatalError( Error, OHA_MODULE_ID, __LINE__)
...
#error "OHA_UPPER_IN_FCT_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_UPPER_OUT_FCT_ATTR
#error "OHA_UPPER_OUT_FCT_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_SYSTEM_IN_FCT_ATTR
#error "OHA_SYSTEM_IN_FCT_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_SYSTEM_OUT_FCT_ATTR
#error "OHA_SYSTEM_OUT_FCT_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_LOCAL_FCT_ATTR
#error "OHA_LOCAL_FCT_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_UPPER_RQB_ATTR
#error "OHA_UPPER_RQB_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_UPPER_MEM_ATTR
#error "OHA_UPPER_MEM_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_SYSTEM_MEM_ATTR
#error "OHA_SYSTEM_MEM_ATTR defined in file oha_cfg.h"
#endif

#if defined OHA_LOCAL_MEM_ATTR
#error "OHA_LOCAL_MEM_ATTR defined in file oha_cfg.h"
#endif
