	...
	)
{
	...
	if (CLRPC_PTR_IS_NULL (rb)) {
		...
		CLRPC_FATAL1 ((LSA_UINT32)rb); /* may return!? */
		return;
	}
	...
	for (my_handle = 0; my_handle < CLRPC_CFG_MAX_CHANNELS; ++my_handle)
	{
		...
		if (channel->state == CLRPC_ST_FREE)
		{
#if CLRPC_DEBUG
			//CLRPC_MEMSET(&clrpc_channels[my_handle], 0xCC, sizeof(clrpc_channels[my_handle])); /* to find uninitialized things */
			CLRPC_MEMSET(channel, 0xCC, sizeof(*channel)); /* to find uninitialized things */
#endif
			channel->state = CLRPC_ST_IDLE; /* in use now */
			break;
		}
	}
...
)
{
    ...
    LSA_UINT32 trace_idx = rqb_ptr->args.channel.set_interface_param->trace_idx;
    CLRPC_ASSERT (CLRPC_PTR_IS_NULL (channel->upper));
    ...
    if (! clrpc_system_channel_is_ready ())
    {
        CLRPC_UPPER_TRACE_00 (trace_idx, LSA_TRACE_LEVEL_ERROR
    ...
    else
    {
        ...
        if (rqb_ptr->args.channel.set_interface_param->client_receive_resources != 0)
        {
            ...
            {
                CLRPC_CL_TRACE_00(trace_idx, LSA_TRACE_LEVEL_ERROR
            ...
            else
            {
                ...
                if( channel->cl.lower.state == CLRPC_CL_LOWER_CLOSED )
                {
                    ...
                    else
                    {
                        ...
                        CLRPC_CL_TRACE_00(trace_idx, LSA_TRACE_LEVEL_CHAT
		                ...
		                param->reuse_addr = USE_CHANNEL_SETTING;
                ...
                {
                    CLRPC_CL_TRACE_01 (trace_idx, LSA_TRACE_LEVEL_ERROR
                ...
                CLRPC_CL_TRACE_00 (trace_idx, LSA_TRACE_LEVEL_ERROR
                    , "do not enable clients without defining CLRPC_CFG_MAX_CLIENTS."
                    );
...
)
{
    ...
    switch (channel->cl.lower.state)
    {
        case CLRPC_CL_LOWER_READY:
        {
			...
			param->graceful_shutdown = USE_CHANNEL_SETTING;
