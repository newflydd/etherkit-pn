	...
	)
{
	...
	{/*lint -save -esym(550,u) Symbol 'u' not accessed*/
		union {
			CLRPC_RQB_TYPE x;
			CLRPC_LIST_ENTRY_TYPE y;
		} u;

		if ((void*)&CLRPC_RQB_GET_NEXT_RQB_PTR(&u.x) != (void*)&u.y.Flink )
		{
			CLRPC_FATAL1(0);
		}

		if ((void*)&CLRPC_RQB_GET_PREV_RQB_PTR(&u.x) != (void*)&u.y.Blink )
		{
			CLRPC_FATAL1(0);
		}
		/*lint -restore */
...
static LSA_FATAL_ERROR_TYPE clrpc_fatal_var; /* for some debuggers that have problems with the stack-trace */
