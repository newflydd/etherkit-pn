/*
 * v3.1 August 30, 2011 ip/iface.c
...
{
   u_long currentIfOperState = ifp->n_mib->ifOperStatus; /* INICHE_TCIP_ITGR save */
   int   err;
   ...
   else /* INICHE_TCIP_ITGR added "else" */
   {
      ifp->n_netbr = 0;
#if 0 /* INICHE_TCIP_ITGR see 957200 */
      ifp->n_netbr42 = 0;
   ...
   if(NI_UP == currentIfOperState) /* INICHE_TCIP_ITGR restart iface only if it was UP */
   {
...
#if 1 /* INICHE_TCIP_ITGR added fuction ni_set_ipsuite() */

int ni_set_ipsuite(NET ifp, u_long Ip_addr, u_long Subnet_mask, u_long Gateway)
{
	/* note: compare to ni_set_config() */

#ifdef TCIP_CFG_CHECK_GATEWAY_ADDRESS
	if( Gateway == Ip_addr ) /* legacy coding of "no gateway" (see too AP00285731) */
	{
		Gateway = 0;
	}
#endif
	 ...
	 */
	if( ifp->n_defgw == Gateway
	    && ifp->n_ipaddr == Ip_addr
	    && ifp->snmask == Subnet_mask )
	{
		return 0; 
	}
...
#endif   /* INCLUDE_TCP */
#if 0 /* INICHE_TCIP_ITGR see tcip_set_ip_suite */
	del_route(0, 0, if_netnumber(ifp));    /* delete any IP routes */
#endif /*INICHE_TCIP_ITGR*/
...
ni_set_state(NET ifp, int opcode)
{
   ...
   if (opcode == NI_DOWN)  /* clean up system tables */
   {
      ...
      if_killsocks(ifp);      /* kill this iface's sockets */
#endif   /* INCLUDE_TCP */
#ifdef IP_ROUTING /* INICHE_TCIP_ITGR added #ifdef */
      del_route(0, 0, if_netnumber(ifp));    /* delete any IP routes */
#endif
      clear_arp_entries(NULLIP, ifp);        /* delete any ARP entries */
   }
...
#else /* INICHE_TCIP_ITGR see AP01482837 */
	  LSA_UNUSED_ARG(ifc);
#endif


#if 1 /* INICHE_TCIP_ITGR test for broadcast and multicast, see caller (MIB) */
   if((addr[0] & 0x01) == 0)
   {
      return(FALSE);
   }
