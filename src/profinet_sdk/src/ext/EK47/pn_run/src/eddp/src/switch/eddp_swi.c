    ...
    EDDP_UPPER_DPB_PTR_TYPE     pDPB)
{
    ...
    EDDP_SWI_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "[H:--] IN: EDDP_SWIDeviceOpen(pDDB=0x%X, pDPB=0x%X)", 
        pDDB, pDPB);
    ...
    // check and store parameters for "IRQx_Control" register
    if (   (pDPB->HWCfg.IRQx_Control.IRQ2_Length > EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_MAX)
        || (pDPB->HWCfg.IRQx_Control.IRQ3_Length > EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_MAX)
       )
    {
        Response = EDD_STS_ERR_PARAM;
        EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
        EDDP_SWI_TRACE_03(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceOpen(): ERROR -> For register IRQx_Control: IRQ2_Length(%d) or IRQ3_Length(%d) > IRQx_Length_Max(%d)", 
            pDPB->HWCfg.IRQx_Control.IRQ2_Length, pDPB->HWCfg.IRQx_Control.IRQ3_Length, EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_MAX);
    }
    else
    {
        switch (pDDB->Hardware)
        {
            ...
            {
                // only this combination is valid: FPGA and IRQx_Length=7
                if (   (pDPB->HWCfg.IRQx_Control.IRQ2_Length != EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_FPGA_DEFAULT)
                    || (pDPB->HWCfg.IRQx_Control.IRQ3_Length != EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_FPGA_DEFAULT)
                   )
                {
                    Response = EDD_STS_ERR_PARAM;
                    EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
                    EDDP_SWI_TRACE_03(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceOpen(): ERROR -> For register IRQx_Control: IRQ2_Length(%d) or IRQ3_Length(%d) != IRQx_Length_Default(%d)", 
                        pDPB->HWCfg.IRQx_Control.IRQ2_Length, pDPB->HWCfg.IRQx_Control.IRQ3_Length, EDDP_HW_CFG_IRQX_CONTROL_IRQ_LENGTH_FPGA_DEFAULT);
                }
            }
            break;
            ...
            {
                // check minimum of IRQ3_Length, see EDDP_HW_CFG_IRQX_CONTROL_IRQ3_LENGTH_MIN_REV3
                if (pDPB->HWCfg.IRQx_Control.IRQ3_Length < EDDP_HW_CFG_IRQX_CONTROL_IRQ3_LENGTH_MIN_REV3)
                {
                    Response = EDD_STS_ERR_PARAM;
                    EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
                    EDDP_SWI_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceOpen(): ERROR -> For register IRQx_Control: IRQ3_Length(%d) too small, min. value = %d", 
                        pDPB->HWCfg.IRQx_Control.IRQ3_Length, EDDP_HW_CFG_IRQX_CONTROL_IRQ3_LENGTH_MIN_REV3);
                }
            }
            break;
            ...
            default:
            {
                Response = EDD_STS_ERR_PARAM;
                EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
                EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceOpen(): ERROR -> Hardware(0x%X) is not supported!", 
                    pDDB->Hardware);
            }
            break;
        }
    ...
    if (Response == EDD_STS_OK)
    {
        if( (EDDP_SWI_LARGEST_POSSIBLE_PHY_ADDRESS - pDDB->SWI.PortCnt) < pDPB->SWI.PhyAddressOffset)
        {
            Response = EDD_STS_ERR_PARAM;
            EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
            EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, 
                              "[H:--] EDDP_SWIDeviceOpen(): ERROR -> PhyAddressOffset(0x%X) to large!", 
                              pDPB->SWI.PhyAddressOffset);
        }
        else
        {
    ...
    if (Response == EDD_STS_OK)
    {
        ...
        case EDDP_HW_HERA_FPGA:
        {
            pDDB->SWI.PTCP_RxDelay = EDDP_SWI_PTCP_RX_DELAY_FPGA;
            pDDB->SWI.PTCP_TxDelay = EDDP_SWI_PTCP_TX_DELAY_FPGA;
        }
        break;
    ...
    EDDP_UPPER_DSB_PTR_TYPE     pDSB)
{
    ...
    // set default PHY config for Ertec200p-PHY and FPGA
    if ((pDDB->HardwareType & EDD_HW_TYPE_USED_MSK) == EDD_HW_TYPE_USED_ERTEC_200P)
    { 
        /* third parameter is not used with INIT_ALL cmd therefor it can be 0 */
        EDDP_ERTEC200P_INIT_PHY_CONFIG_REG(pDDB->hSysDev, EDDP_PHY_CONFIG_CMD_INIT_ALL, 0/*HWPortID is irrelevant here*/);
    }

    // check if the configured hardware is equal to the present hardware
    reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_IP_DEVELOPMENT);
    reg_val = EDDP_GET_BIT_VALUE(reg_val, PNIP_A0_REG_IP_DEVELOPMENT__MSK_TYPE, PNIP_A0_REG_IP_DEVELOPMENT_SHFT_TYPE);
    ...
    {         
        ...
        // set up MAC_SA_low_Px register
        reg_val = ( (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[5])       |
                    (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[4]) << 8  |
                    (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[3]) << 16 |
                    (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[2]) << 24  );
        reg_address = (HwPortID==1 ? PNIP_REG_MAC_SA_LOW_P1 : 
                      (HwPortID==2 ? PNIP_REG_MAC_SA_LOW_P2 : 
                      (HwPortID==3 ? PNIP_REG_MAC_SA_LOW_P3 : 
                      (HwPortID==4 ? PNIP_REG_MAC_SA_LOW_P4 : 0))));
        EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);

        // set up MAC_SA_high_Px register
        reg_address = (HwPortID==1 ? PNIP_REG_MAC_SA_HIGH_P1 : 
                      (HwPortID==2 ? PNIP_REG_MAC_SA_HIGH_P2 : 
                      (HwPortID==3 ? PNIP_REG_MAC_SA_HIGH_P3 : 
                      (HwPortID==4 ? PNIP_REG_MAC_SA_HIGH_P4 : 0))));
        reg_val = EDDP_HAL_REG32_READ(pDDB, reg_address);
        reg_val |= ( (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[1])      |
                     (LSA_UINT32)(pDDB->SWI.PortParams[HwPortID].Port_MAC_Address.MacAdr[0]) << 8  );
        EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);
        ...
        #pragma EDDP_PRAGMA_MESSAGE("Workaround for PNIP-Revision(1) - AP01403271: Tx_RedGuard_DP2 deaktivieren (ERTEC200P Step1)")
        ...
        if (   ( EDDP_HAL_HW_PNIP_REV2 == EDDP_HALGetPnipRevision(pDDB) )
    ...
    switch (EDDP_HALGetPnipRevision(pDDB))
    {
        ...
        default:
        {
            EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWIDeviceSetup1(): FATAL -> Version(%d) of PNIP is unknown",
    ...
    #ifdef EDDP_MESSAGE
    #ifdef EDDP_PRAGMA_MESSAGE
    #pragma EDDP_PRAGMA_MESSAGE("Workaround for PNIP-Revision(1) - AP01357789: Fragmentierung führt zu Interrupt Phasecrossed (SafetyMargin is depending PNIP revision)")
    #endif /* EDDP_PRAGMA_MESSAGE */
    #endif /* EDDP_MESSAGE */
    ...
    switch (EDDP_HALGetPnipRevision(pDDB))
    {
        ...
        case EDDP_HAL_HW_PNIP_REV1:
        {
            EDDP_SET_BIT_VALUE32(reg_val, EDDP_SWI_REG_SAFETYMARGIN_100M_INIT_PNIP_REV1, PNIP_REG_SAFETYMARGIN__MSK_SAFETYMARGIN_100M, PNIP_REG_SAFETYMARGIN_SHFT_SAFETYMARGIN_100M);
        }
        break;
        #endif
    ...
    switch (EDDP_HALGetPnipRevision(pDDB))
    {
        ...
        default:
        {
            EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWIDeviceSetup1(): FATAL -> Version(%d) of PNIP is unknown on setting up PNIP_REG_PHY_CONTROL", 
                EDDP_HALGetPnipRevision(pDDB));
            EDDP_FATAL(EDDP_FATAL_ERR_NOT_SUPPORTED);
        }
        break;
    ...
    switch(EDDP_HALGetChipVersion(pDDB))
    {
        ...
        default:
            EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWIDeviceSetup1(): FATAL -> Hardware not supported EDDP_HALGetChipVersion(pDDB)=%d", 
    ...
    switch (pDDB->Hardware)
    {
        ...
        default:
        {
            Response = EDD_STS_ERR_PARAM;
            EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
            EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceSetup1(): ERROR -> HardwareType(0x%X) not supported!", 
                pDDB->Hardware);
        }
        break;
    }
    ...
    if (Response == EDD_STS_OK)
    {
        ...
        Response = EDDP_SWI_Set_HOL_Barriers(pDDB);
    ...
    if(EDD_STS_OK == Response)
    {
        if( (EDDP_HAL_HW_HERA == EDDP_HALGetChipVersion(pDDB)) && (EDDP_HAL_HW_IF_A == EDDP_HALGetChipInterface(pDDB)) )
        {
            if (  (EDD_FEATURE_ENABLE == pDSB->PortParams[0].GBitSupport) ||
                  (EDD_FEATURE_ENABLE == pDSB->PortParams[0].GBitSupport) ||
                  (EDD_FEATURE_ENABLE == pDSB->PortParams[0].GBitSupport) ||
                  (EDD_FEATURE_ENABLE == pDSB->PortParams[0].GBitSupport)   )
            {
                Response = EDD_STS_ERR_PARAM;
        ...
        if( (EDDP_HAL_HW_HERA == EDDP_HALGetChipVersion(pDDB)) && (EDDP_HAL_HW_IF_B == EDDP_HALGetChipInterface(pDDB)) )
        {
            ...
            if( ( (EDD_FEATURE_ENABLE != pDSB->PortParams[0].GBitSupport) && (EDD_FEATURE_DISABLE != pDSB->PortParams[0].GBitSupport) ) ||
                ( (EDD_FEATURE_ENABLE != pDSB->PortParams[0].GBitSupport) && (EDD_FEATURE_DISABLE != pDSB->PortParams[0].GBitSupport) ) ||
                ( (EDD_FEATURE_ENABLE != pDSB->PortParams[0].GBitSupport) && (EDD_FEATURE_DISABLE != pDSB->PortParams[0].GBitSupport) ) ||
                ( (EDD_FEATURE_ENABLE != pDSB->PortParams[0].GBitSupport) && (EDD_FEATURE_DISABLE != pDSB->PortParams[0].GBitSupport) )   )
            ...
            // HERADevMark RZ hier die richtige Verwendung der PortID prüfen
	...
	EDDP_LOCAL_DDB_PTR_TYPE     pDDB)
{
	...
	LSA_UINT32 PX_Mapping[EDDP_MAX_PORT_CNT];
	LSA_UINT32 PhyAddressX[EDDP_MAX_PORT_CNT];
	LSA_UINT32 HWPortID_X[EDDP_MAX_PORT_CNT];

	LSA_UINT32 AsicSorted[EDDP_MAX_PORT_CNT + 1];
	LSA_UINT32 HardwareSorted[EDDP_MAX_PORT_CNT + 1];

	LSA_UINT8  portIterator;
	LSA_UINT8  portIterator2;

	EDD_RSP    Response = EDD_STS_OK;

	for (PortID = 1; PortID <= EDDP_MAX_PORT_CNT; PortID++)
	{
		PX_Mapping[PortID - 1] = 0x7;  // no PHY
		PhyAddressX[PortID - 1] = 0x0;
		HWPortID_X[PortID - 1] = 0x0;

		AsicSorted[PortID] = 0xFFFFFFFF;
		HardwareSorted[PortID] = 0xFFFFFFFF;
	}

	AsicSorted[0] = 0;
	HardwareSorted[0] = 0;

	// sort values of PortID_x_to_HardwarePort_y and PortID_x_to_ASICPort_y
	for (portIterator = 0; portIterator < pDDB->SWI.PortCnt; portIterator++)
	{
		for (portIterator2 = 0; portIterator2 < pDDB->SWI.PortCnt; portIterator2++)
		{
			if (EDDP_PORT_NOT_CONNECTED == pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator2])
			{
				break;
			}

			if ((pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator2] > HardwareSorted[portIterator]) &&
				(pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator2] < HardwareSorted[portIterator + 1]))
			{
				HardwareSorted[portIterator + 1] = pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator2];
			}

			if ((pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[portIterator2] > AsicSorted[portIterator]) &&
				(pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[portIterator2] < AsicSorted[portIterator + 1]))
			{
				AsicSorted[portIterator + 1] = pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[portIterator2];
			}
	...
	for (PortID = 1; PortID <= pDDB->SWI.PortCnt; PortID++)
	{
		...
		if (EDDP_PORT_NOT_CONNECTED != HwPortID)
		{
			LSA_UINT8 foundHere;
			LSA_UINT8 isFound;
			...
			// calculate values for register Port_Mapping
			PX_Mapping[HwPortID - 1] = pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[PortID - 1] - 1;

			// search in sorted table after related values 
			for (foundHere = 1; foundHere <= pDDB->SWI.PortCnt; foundHere++)
			{
				if (AsicSorted[foundHere] == pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[HwPortID - 1])
				{
					isFound = 1;
					break;
				}
			...
			if (0 == isFound)
			{
				Response = EDD_STS_ERR_PARAM;
				EDDP_DBG_SET_DETAIL_ERR(pDDB, Response);
				EDDP_SWI_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWIDeviceSetup1: ERROR -> PortID_x_to_ASICPort_y[HwPortID=%d]=%d was not found in AsicSorted[] !",
					HwPortID, pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[HwPortID - 1]);
			}

			// search related value in PortID_x_to_HardwarePort_y
			for (portIterator = 0; portIterator < pDDB->SWI.PortCnt; portIterator++)
			{
				if (HardwareSorted[foundHere] == pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator])
				{
					PhyAddressX[HwPortID - 1] = portIterator + pDDB->SWI.PHYAddressOffset;
					break;
				}
			}

			// search SMI value in PortID_x_to_ASICPort_y
			for (portIterator = 0; portIterator < pDDB->SWI.PortCnt; portIterator++)
			{
				if (pDDB->SWI.PortMap.PortID_x_to_SMIPort_y[HwPortID - 1] == pDDB->SWI.PortMap.PortID_x_to_ASICPort_y[portIterator])
				{
					HWPortID_X[HwPortID - 1] = pDDB->SWI.PortMap.PortID_x_to_HardwarePort_y[portIterator] - 1;
					break;
				}
			}
		}
	...
	// write calculated value to registers of pnip
	reg_address = PNIP_R3_REG_PORT_MAPPING;
	reg_val = 0;
	EDDP_SET_BIT_VALUE32(reg_val, PX_Mapping[0], PNIP_R3_REG_PORT_MAPPING__MSK_P1_MAPPING, PNIP_R3_REG_PORT_MAPPING_SHFT_P1_MAPPING);
	EDDP_SET_BIT_VALUE32(reg_val, PX_Mapping[1], PNIP_R3_REG_PORT_MAPPING__MSK_P2_MAPPING, PNIP_R3_REG_PORT_MAPPING_SHFT_P2_MAPPING);
	EDDP_SET_BIT_VALUE32(reg_val, PX_Mapping[2], PNIP_R3_REG_PORT_MAPPING__MSK_P3_MAPPING, PNIP_R3_REG_PORT_MAPPING_SHFT_P3_MAPPING);
	EDDP_SET_BIT_VALUE32(reg_val, PX_Mapping[3], PNIP_R3_REG_PORT_MAPPING__MSK_P4_MAPPING, PNIP_R3_REG_PORT_MAPPING_SHFT_P4_MAPPING);

	EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);

	reg_address = PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING;
	reg_val = 0;
	EDDP_SET_BIT_VALUE32(reg_val, PhyAddressX[0], PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING__MSK_PHY_ADDRESS_1, PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_PHY_ADDRESS_1);
	EDDP_SET_BIT_VALUE32(reg_val, PhyAddressX[1], PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING__MSK_PHY_ADDRESS_2, PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_PHY_ADDRESS_2);
	EDDP_SET_BIT_VALUE32(reg_val, PhyAddressX[2], PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING__MSK_PHY_ADDRESS_3, PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_PHY_ADDRESS_3);
	EDDP_SET_BIT_VALUE32(reg_val, PhyAddressX[3], PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING__MSK_PHY_ADDRESS_4, PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_PHY_ADDRESS_4);

	EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[0], PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_1, PNIP_R3_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_1);

	if (EDDP_HAL_HW_TYPE_ASIC == EDDP_HALGetChipType(pDDB))
	{
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[1], PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_2, PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_2);
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[2], PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_3, PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_3);
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[3], PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_4, PNIP_R3A0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_4);
	}
	else if (EDDP_HAL_HW_TYPE_FPGA == EDDP_HALGetChipType(pDDB))
	{
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[1], PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_2, PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_2);
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[2], PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_3, PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_3);
		EDDP_SET_BIT_VALUE32(reg_val, HWPortID_X[3], PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING__MSK_HWPORTID_4, PNIP_R3F0_REG_SMI_PHY_ADDRESS_MAPPING_SHFT_HWPORTID_4);
	}
    ...
    EDDP_UPPER_DSB_PTR_TYPE	    pDSB)
{
    EDD_RSP         Response = EDD_STS_OK;
    #ifdef EDDP_CFG_HW_PNIP_REV1_SUPPORT
    LSA_UINT32      WaitTime = 0;
    LSA_UINT32      reg_val = 0;
    LSA_UINT32      UC_Learning_Enable = 0, UC_Learning_Enable_R = 0;
    LSA_UINT32      MC_Learning_Enable = 0, MC_Learning_Enable_R = 0;
    LSA_UINT32      IRT_Learning_Enable = 0;
    #endif
    ...
    #ifdef EDDP_MESSAGE
    #ifdef EDDP_PRAGMA_MESSAGE
    #pragma EDDP_PRAGMA_MESSAGE("Workaround for PNIP-Revision(1) - AP01274025: FDB-Kommandos und Lernen")
    #endif /* EDDP_PRAGMA_MESSAGE */
    #endif /* EDDP_MESSAGE */
    /* ---------------------------------------------------------------------------- */
    ...
    if (EDDP_HAL_HW_PNIP_REV1 == EDDP_HALGetPnipRevision(pDDB))
    {
        ...
        UC_Learning_Enable      = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
        UC_Learning_Enable_R    = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
        EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
        EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_UC_CONTROL, reg_val);
        // MC_Control
        reg_val                 = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_MC_CONTROL);
        MC_Learning_Enable      = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
        MC_Learning_Enable_R    = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
        EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
        EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_MC_CONTROL, reg_val);
        // IRT_Control
        reg_val                 = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_IRT_CONTROL);
        IRT_Learning_Enable     = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
        EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_IRT_CONTROL, reg_val);

        // wait 125µs
        WaitTime = (EDDP_HAL_100US_TO + EDDP_HAL_10US_TO + EDDP_HAL_10US_TO + 5000);
        EDDP_HALWaitNsTicks (pDDB, WaitTime);
    }
    #endif
    ...
    #ifdef EDDP_CFG_HW_PNIP_REV1_SUPPORT
    // restore "Learning" settings
    if (EDDP_HAL_HW_PNIP_REV1 == EDDP_HALGetPnipRevision(pDDB))
    {
        // UC_Control
        reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_UC_CONTROL);
        EDDP_SET_BIT_VALUE32(reg_val, UC_Learning_Enable,   PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
        EDDP_SET_BIT_VALUE32(reg_val, UC_Learning_Enable_R, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_UC_CONTROL, reg_val);
        // MC_Control
        reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_MC_CONTROL);
        EDDP_SET_BIT_VALUE32(reg_val, MC_Learning_Enable,   PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
        EDDP_SET_BIT_VALUE32(reg_val, MC_Learning_Enable_R, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_MC_CONTROL, reg_val);
        // IRT_Control
        reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_IRT_CONTROL);
        EDDP_SET_BIT_VALUE32(reg_val, IRT_Learning_Enable, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
        EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_IRT_CONTROL, reg_val);
    }
    /* ---------------------------------------------------------------------------- */
    ...
    EDDP_LOCAL_DDB_PTR_TYPE     pDDB)
{
	...
	default:
	{
		EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWIDeviceShutdown(): FATAL -> Version(%d) of PNIP is unknown on disabling PNIP_REG_PHY_CONTROL__MSK_PHY_ACTIVATE",
			EDDP_HALGetPnipRevision(pDDB));
		EDDP_FATAL(EDDP_FATAL_ERR_NOT_SUPPORTED);
	}
	break;
        ...
        {   
            ...
            else
            {
                ...
                if (EDDP_HAL_HW_PNIP_REV1 == EDDP_HALGetPnipRevision(pDDB))
                {
                    ...
                    // UC_Control
                    reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_UC_CONTROL);
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.UC_Learning_Enable 
                        = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.UC_Learning_Enable_R 
                        = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
                    EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
                    EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
                    EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_UC_CONTROL, reg_val);
                    // MC_Control
                    reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_MC_CONTROL);
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.MC_Learning_Enable 
                        = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.MC_Learning_Enable_R 
                        = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
                    EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
                    EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
                    EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_MC_CONTROL, reg_val);
                    // IRT_Control
                    reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_IRT_CONTROL);
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.IRT_Learning_Enable 
                        = EDDP_GET_BIT_VALUE(reg_val, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
                    EDDP_SET_BIT_VALUE32(reg_val, 0, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
                    EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_IRT_CONTROL, reg_val);

                    // lock accesses to these registers
                    pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.isCmdRunning = LSA_TRUE;

                    EDDP_SWI_TRACE_05(pDDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "[H:--] EDDP_SWIRequest(): store Learning settings - UC_Learning_Enable(0x%X), UC_Learning_Enable_R(0x%X), MC_Learning_Enable(0x%X), MC_Learning_Enable_R(0x%X), IRT_Learning_Enable(0x%X)", 
                        pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.UC_Learning_Enable, 
                        pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.UC_Learning_Enable_R, 
                        pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.MC_Learning_Enable, 
                        pDDB->SWI.CommandParams.ClearUC_Table.param_union.Fdb.ClearUCTable.MC_Learning_Enable_R, 
    ...
    EDD_RSP                     ResponseCmd)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* ---------------------------------------------------------------------------- */
            #ifdef EDDP_CFG_HW_PNIP_REV1_SUPPORT
            // restore "Learning" settings, which were stored before the command
            if (EDDP_HAL_HW_PNIP_REV1 == EDDP_HALGetPnipRevision(pDDB))
            {
                // UC_Control
                reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_UC_CONTROL);
                EDDP_SET_BIT_VALUE32(reg_val, pParam->param_union.Fdb.ClearUCTable.UC_Learning_Enable,   PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE,   PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE);
                EDDP_SET_BIT_VALUE32(reg_val, pParam->param_union.Fdb.ClearUCTable.UC_Learning_Enable_R, PNIP_REG_UC_CONTROL__MSK_UC_LEARNING_ENABLE_R, PNIP_REG_UC_CONTROL_SHFT_UC_LEARNING_ENABLE_R);
                EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_UC_CONTROL, reg_val);
                // MC_Control
                reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_MC_CONTROL);
                EDDP_SET_BIT_VALUE32(reg_val, pParam->param_union.Fdb.ClearUCTable.MC_Learning_Enable,   PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE,   PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE);
                EDDP_SET_BIT_VALUE32(reg_val, pParam->param_union.Fdb.ClearUCTable.MC_Learning_Enable_R, PNIP_REG_MC_CONTROL__MSK_MC_LEARNING_ENABLE_R, PNIP_REG_MC_CONTROL_SHFT_MC_LEARNING_ENABLE_R);
                EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_MC_CONTROL, reg_val);
                // IRT_Control
                reg_val = EDDP_HAL_REG32_READ(pDDB, PNIP_REG_IRT_CONTROL);
                EDDP_SET_BIT_VALUE32(reg_val, pParam->param_union.Fdb.ClearUCTable.IRT_Learning_Enable, PNIP_REG_IRT_CONTROL__MSK_IRT_LEARNING_ENABLE, PNIP_REG_IRT_CONTROL_SHFT_IRT_LEARNING_ENABLE);
                EDDP_HAL_REG32_WRITE(pDDB, PNIP_REG_IRT_CONTROL, reg_val);

                // unlock accesses to registers: UC_Control, MC_Control and IRT_Control
                pParam->param_union.Fdb.ClearUCTable.isCmdRunning = LSA_FALSE;

                EDDP_SWI_TRACE_05(pDDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "[H:--] EDDP_SWI_CommandDoneCbf_FDB(): restore Learning settings - UC_Learning_Enable(0x%X), UC_Learning_Enable_R(0x%X), MC_Learning_Enable(0x%X), MC_Learning_Enable_R(0x%X), IRT_Learning_Enable(0x%X)", 
                    pParam->param_union.Fdb.ClearUCTable.UC_Learning_Enable, 
                    pParam->param_union.Fdb.ClearUCTable.UC_Learning_Enable_R, 
                    pParam->param_union.Fdb.ClearUCTable.MC_Learning_Enable, 
                    pParam->param_union.Fdb.ClearUCTable.MC_Learning_Enable_R, 
                    pParam->param_union.Fdb.ClearUCTable.IRT_Learning_Enable);
            }
            #endif
            /* ---------------------------------------------------------------------------- */
            /* Software workaround for PNIP Learning bug    -> END                          */
                ...
                case EDD_SRV_SENDCLOCK_CHANGE:
                {
                    if (pParam->CmdOpcode == EDDP_CMD_GSYNC_UPDATE_COMCYCL)
                    {
                        ...
                        switch (EDDP_HALGetPnipRevision(pDDB))
                        {
                            ...
                            default:
                            {
                                EDDP_SWI_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWI_CommandDoneCbf_GSYNC(): FATAL -> Version(%d) of PNIP is unknown", 
                                    EDDP_HALGetPnipRevision(pDDB));
                                EDDP_FATAL(EDDP_FATAL_ERR_NOT_SUPPORTED);
                            }
                            break;
    ...
    LSA_BOOL                    bKeepPhyStatus)
{
    ...
    reg_val = (InitData.LinkStatusRegAddress | 0x00000020/*Polling enable*/ | (InitData.LinkStatusBitMask << 16));
    // select the right register address for current HwPortID "i"
    reg_address = (i==1 ? PNIP_REG_PHYSTATUS1_SELECT_P1 : 
                  (i==2 ? PNIP_REG_PHYSTATUS1_SELECT_P2 : 
                  (i==3 ? PNIP_REG_PHYSTATUS1_SELECT_P3 : 
                  (i==4 ? PNIP_REG_PHYSTATUS1_SELECT_P4 : 0))));
    EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);

    // set PHYStatus2_Select_PX. 
    reg_val = (InitData.SpeedRegAddress | 0x00000020/*Polling enable*/ | (InitData.SpeedBitMask << 16) );
    // select the right register address for current HwPortID "i"
    reg_address = (i==1 ? PNIP_REG_PHYSTATUS2_SELECT_P1 : 
                  (i==2 ? PNIP_REG_PHYSTATUS2_SELECT_P2 : 
                  (i==3 ? PNIP_REG_PHYSTATUS2_SELECT_P3 : 
                  (i==4 ? PNIP_REG_PHYSTATUS2_SELECT_P4 : 0))));
    EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);
    
    // set PHYStatus3_Select_PX. 
    reg_val = (InitData.DuplexityRegAddress | 0x00000020/*Polling enable*/ | (InitData.DuplexityBitMask << 16) );
    // select the right register address for current HwPortID "i"
    reg_address = (i==1 ? PNIP_REG_PHYSTATUS3_SELECT_P1 : 
                  (i==2 ? PNIP_REG_PHYSTATUS3_SELECT_P2 : 
                  (i==3 ? PNIP_REG_PHYSTATUS3_SELECT_P3 : 
                  (i==4 ? PNIP_REG_PHYSTATUS3_SELECT_P4 : 0))));
    EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);
    ...
    LSA_UINT32                  EventNr)
{
    ...
    switch (pDDB->Hardware)
    {
        ...
        case EDDP_HW_ERTEC200P_FPGA:
        {
            ...
            LSA_UINT32          RegMacStatusP1, RegMacStatusP2;
    ...
    LSA_UINT32                  EventNr)
{
    ...
    switch (pDDB->Hardware)
    {
        ...
        case EDDP_HW_ERTEC200P_FPGA:
        {
            LSA_UINT32          RegFrameAgingStatusP1;
            LSA_UINT32          RegFrameAgingStatusP2;
...
LSA_VOID EDDP_LOCAL_FCT_ATTR EDDP_SWI_ISR_QCW_Empty(
    ...
    LSA_UINT32                  EventNr)
{
    LSA_UINT32      RegHolStatus, RegResLackCounter;
...
LSA_VOID EDDP_LOCAL_FCT_ATTR EDDP_SWI_ISR_FragmentationFault(
    ...
    LSA_UINT32                  EventNr)
{
    LSA_UINT32          RegFragmentationStatus, RegFragmentationDebug, RegCycleLength;
    ...
    EDD_RSP*                    pResponse)
{
    ...
    _EDDP_SWI_ReadPhyStatusRegs (pDDB, HwPortID, &PhyLinkStatus);
    ...
    EDD_RSP*                    pResponse)
{
    ...
    // get PHY address, vendor register addresses and "unequal compare value" for PHY start-up 
    *pResponse = EDDP_PHY_GetPhyAddress (pDDB->hDDB, pDDB->hSysDev, HwPortID,pDDB->SWI.PHYAddressOffset, &PhyAddress, &OUI1RegAddress, &OUI2RegAddress, &OUIUnequalValue);

    if (*pResponse != EDD_STS_OK)
    {
        *pResponse = EDD_STS_ERR_PARAM;
        EDDP_DBG_SET_DETAIL_ERR(pDDB, *pResponse);
        EDDP_SWI_TRACE_00(EDDP_UNDEF_TRACE_IDX, LSA_TRACE_LEVEL_ERROR, "[H:--] EDDP_SWI_GEN_PhySTS_StateMachine_Transition_Stop(): ERROR -> Call of EDDP_PHY_GetPhyAddress() is not Ok!");
    }

    if (   ((pDDB->HardwareType & EDD_HW_TYPE_USED_MSK) == EDD_HW_TYPE_USED_ERTEC_200P)
        || ((pDDB->HardwareType & EDD_HW_TYPE_USED_MSK) == EDD_HW_TYPE_USED_HERA) )
    { 
        // set PHYCommand_PX | OUI1/2 polling is now active; when PHY is ready an interrupt is fired and EDDP_SWI_FinishPHYSetup() is called
        // select the right register address for current port number
        reg_address = (HwPortID==1 ? PNIP_REG_PHYCOMMAND_P1 : 
        ...
        reg_val = reg_val | PhyAddress | 0x00001000; /*power up reset enable*/

        reg_val = reg_val & (~0x00000020); /*Polling disable*/

        EDDP_HAL_REG32_WRITE(pDDB, reg_address, reg_val);

        // reset PHY 
        EDDP_ERTEC200P_INIT_PHY_CONFIG_REG(pDDB->hSysDev, EDDP_PHY_CONFIG_CMD_DISABLE_PHY, HwPortID);

        // wait 100us
        EDDP_WAIT_10_NS(pDDB->hSysDev, 100UL * 100UL);
        ...
        switch (MediaType)
        {
            ...
            {
                EDDP_ERTEC200P_INIT_PHY_CONFIG_REG(pDDB->hSysDev, EDDP_PHY_CONFIG_CMD_SET_COPPER_MODE, HwPortID);
            }
            break;

            case EDD_MEDIATYPE_FIBER_OPTIC_CABLE:
            {
                EDDP_ERTEC200P_INIT_PHY_CONFIG_REG(pDDB->hSysDev, EDDP_PHY_CONFIG_CMD_SET_FX_MODE, HwPortID);
            }
            break;
            default:
            {
                EDDP_SWI_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "[H:--] EDDP_SWI_GEN_PhySTS_StateMachine_Transition_Stop(): Mediatype not supported");
                *pResponse = EDD_STS_ERR_NOT_SUPPORTED;
                EDDP_DBG_SET_DETAIL_ERR(pDDB, *pResponse);
            }
            break;
        }
        ...
        // activate phy polling: PHYCommand_Px.Enable = 1
        {
            ...
            reg_val = reg_val | PhyAddress | 0x00000020;
        ...
        EDDP_ERTEC200P_INIT_PHY_CONFIG_REG(pDDB->hSysDev, EDDP_PHY_CONFIG_CMD_ENABLE_PHY, HwPortID);
    }
    else
    {
        //other HWTypes are not supported in this version
        EDDP_SWI_TRACE_01(EDDP_UNDEF_TRACE_IDX, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_SWI_GEN_PhySTS_StateMachine_Transition_Stop(): FATAL -> unknown HardwareType 0x%X. DDB possibly corrupted?", 
            pDDB->HardwareType);
        EDDP_FATAL(EDDP_FATAL_ERR_INCONSISTENCE);
    }
