...
/*===========================================================================*/
/* DCP-SET-Trace */
static LSA_UINT32 oha_dcp_ParseOptTcp  (LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT32 oha_dcp_ParseOptDhcp (LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT32 oha_dcp_ParseOptIdent(LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT32 oha_dcp_ParseOptPbus (LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT32 oha_dcp_ParseOptControl(LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);

static LSA_UINT32 oha_dcp_QueryOptTcp  (LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT32 oha_dcp_QueryOptIdent(LSA_UINT16 NicId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
...
/*****************************************************************************/
static OHA_EDD_LOWER_MEM_PTR_TYPE oha_dcp_NameOfStationFilter(
    LSA_UINT16              NICId,
...
)
{
    ...
    if (str_len)   /* there's a NameOfStation existing */
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            if (str_len != 0)
            {
                OHA_MEMCPY(&FilterPtr->Data, NamePtr, (LSA_UINT32)str_len);
            }
...
/* Lower Channel IDENTIFY-Filter will be set.                                */
/*****************************************************************************/
static OHA_EDD_LOWER_MEM_PTR_TYPE oha_dcp_AliasNameFilter(
    LSA_UINT16              NICId,
    LSA_UINT16              PortId,
    LSA_UINT16              SenderId,
	OHA_COMMON_MEM_U16_PTR_TYPE pTLVLen
)
{
	...
	{
	     OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL,"oha_dcp_AliasNameFilter(NICId: 0x%X): Invalid PortId (%Xh)",NICId,PortId);
	     OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		return(MemPtr);
	}
    if(!(SenderId && (SenderId <= LLDP_MAX_PORT_SENDER)))
	{
	     OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL,"oha_dcp_AliasNameFilter(NICId: 0x%X): Invalid SenderId (%Xh)",NICId,SenderId);
	     OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		return(MemPtr);
	}
        ...
        SenderStatus == OHA_STATUS_DATA_INVALID)
    {
        ...
        if (Length != 0 && OHA_IS_NOT_NULL(NamePtr) &&
            (OHA_OK == oha_check_station_name(Length, NamePtr,
                                              ...
                                              OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE))
        {
			...
			if (SubType == OHA_CHASSIS_ID_LOCALLY_ASSIGNED)
			{
				...
				if (PortLength != 0 && OHA_IS_NOT_NULL(PortPtr) &&
					(OHA_OK == oha_check_port_name(PortLength, PortPtr) ? LSA_TRUE : LSA_FALSE))
				{
					OHA_EDD_ALLOC_LOWER_MEM(&MemPtr, UserId,
					...
					if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
					{
						...
						/* TFS 3968125: convert the whole string to lowercase letters */
						OHA_STRNTOLOWER((OHA_COMMON_MEM_U8_PTR_TYPE)&FilterPtr->Data, Length + 1 + PortLength);
					}
					if(OHA_IS_NOT_NULL(pTLVLen))
					{
...
/*****************************************************************************/
#if 0
static OHA_EDD_LOWER_MEM_PTR_TYPE oha_dcp_PeerNameOfStationFilter(
    LSA_UINT16              NICId,
    LSA_UINT16              PortId,
    LSA_UINT16              SenderId,
	OHA_COMMON_MEM_U16_PTR_TYPE pTLVLen
    ...
    OHA_COMMON_MEM_U16_PTR_TYPE  Entries)
{
    ...
    *Entries = 0;
    ...
    if ( !LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
    {
		...
		if(Mode == OHA_SET_FILTER)
		{
			...
			/* if OHA has to set the filter type AllSelector */
			{
				...
				OHA_EDD_ALLOC_LOWER_MEM((OHA_EDD_LOWER_MEM_PTR_TYPE *)&FilterPtr, UserId,
	...
	{
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"DCP-Allocating FilterTable failed (NICId: 0x%X)!", NICId);
	}
    ...
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pSetInd)
{
    ...
    if (pRecv->OptCnt > 0)
    {
        ...
        while (RestLen > 0)
        {
            ...
            if (RestLen >= ReadLen)
            {
				...
				if (WriteLen > DCP_MAX_RSP_DATA_SIZE)
				{
					...
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Too many datasets (OptCnt: %d) requested from the Ethenet!", pRecv->OptCnt);
                ...
                switch (DbaseID)
                {
                    ...
                    case DCP_DBID_PBC:          /* PBC-Profil (OrangeBox) 128 */
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OrangeBox (DbaseID = 0x%X) not supported anymore.", DbaseID);
                        break;
                    ...
                    default:                    /* Unknown Database */
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Database (DbaseID = 0x%X) not existing in the directory.", DbaseID);
                        break;
                }
            }
            else
            {                   /* remaining length too short for a further dataset */
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Remaining length too short for this dataset!");
            }
        ...
        if (pSetInd->Options)      /* are there parameters to set - QUERY the SYSTEM first */
        {
            ...
            if (QueryOptions)  /* are there Query-Options */
            {
                ...
                {
                    OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - no query-user existing !");
                    pSetInd->Event = OHA_EVENT_REMOTE_SET;
                ...
                (pSetInd->Options & OHA_CTRL_IP_SUITE))
            {
                ...
                {
                    OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - a user set is active !");
                    pSetInd->Event = OHA_EVENT_REMOTE_SET;
                    (LSA_VOID)OHA_REPORT_DCP_SET_IND(NicId, pSetInd, (DCP_USR_BYTE_PTR)ReqPtr, ReqLen); /* report */
                    pSetInd->Event = OHA_MODE_NO_SET;
				...
				pSetInd->Options & OHA_CTRL_RESET_COMMUNICATION)
            {
                ...
                {
                    OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - a REMA Set (DCP or PrmCommit is active !");
                    pSetInd->Event = OHA_EVENT_REMOTE_SET;
                    (LSA_VOID)OHA_REPORT_DCP_SET_IND(NicId, pSetInd, (DCP_USR_BYTE_PTR)ReqPtr, ReqLen); /* report */
                    pSetInd->Event = OHA_MODE_NO_SET;
                    pSetInd->Reason = OHA_DCP_REASON_NOT_APPLICABLE;
                ...
                {
                    OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - no set, OHA_OPC_PRM_COMMIT is active !");
                    pSetInd->Event = OHA_EVENT_REMOTE_SET;
                    (LSA_VOID)OHA_REPORT_DCP_SET_IND(NicId, pSetInd, (DCP_USR_BYTE_PTR)ReqPtr, ReqLen); /* report */
                    pSetInd->Event = OHA_MODE_NO_SET;
                    pSetInd->Reason = OHA_DCP_REASON_NOT_APPLICABLE;
            ...
            OHA_FUNCTION_TRACE_04(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT,
				...
				pSetInd->Options & OHA_CTRL_RESET_COMMUNICATION)
            {
				...
				if (pSetInd->Options & OHA_CTRL_STATION_NAME)
				{
					...
					{
						OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - RtF and NoS, not a valid set !");
				...
				if (pSetInd->Options & OHA_CTRL_IP_SUITE)
				{
					...
					if(size == OHA_IP_SUITE_SIZE && OHA_IS_NOT_NULL(IpSuitePtr)) /* only IpSuite={0} is valid within RTF */
					{
						...
						{
							OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OUT: oha_dcp_ParseSet() - RtF and IP, not a valid set !");
							pSetInd->Reason = OHA_DCP_REASON_DS_NOT_SET;
							pSetInd->Event = OHA_MODE_NO_SET;
							RetVal = OHA_ERR_PARAM;
							OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_dcp_ParseSet(RetVal: 0x%X)",RetVal);
            ...
            if (QueryOptions)    /* are there Options for QUERY the USER */
            {
                if (pHDB->pHDBUserCtrl) /* user-control for a DcpSet ? */
                {
                    ...
                    {
                        OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_dcp_ParseSet() - no UserIndRqbs found !");
                        pSetInd->Event = OHA_MODE_NO_SET;
                        pSetInd->Reason = OHA_DCP_REASON_NO_RESOURCE;
                        RetVal = OHA_ERR_RESOURCE;
                    }
    ...
    else
    {
        ...
        pSetInd->Reason = OHA_DCP_REASON_NO_DATASET;
        RetVal = OHA_ERR_PROTOCOL;
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"No Dataset requested from the Ethenet!");
    }
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_ParseOptControl(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_ParseOptPbus(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
...
/* will be stored locally                                                    */
/*****************************************************************************/
static LSA_UINT32                oha_dcp_ParseOptIdent(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    /* OHA_IMPROVE: no check necessary, if output function will be changed to LSA_UINT16 */
    if (DsLen < 256)
    ...
    switch (DsPtr->Hdr.Id)
    {
        ...
        case DCP_DSID_IDNAME:       /* NameOfStation */
            ...
            if (AccessControl)
            {
                ...
                if (DataLen && (OHA_ERR_PARAM == oha_check_station_name(DataLen, DsPtr->Data.IdentName.String,
                                                                        OHA_UPPER_CASES_ALLOWED(NicId) ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO,
                                                                        OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE))
                {
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_QueryOptIdent(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_ParseOptDhcp(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    if (DsLen >= 2)
    {
		...
		if (DsPtr->Hdr.Id == 61)
		{
			if(oha_is_valid_dhcp_client_id(DsLen, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp))
			{
				...
				{
					QueryOptions |= OHA_CTRL_DHCP_CLIENT_ID;
				}
				if (Status & DCP_MASK_DS_REMANENT)
				{
			...
			{
			    OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "Invalid or inconsistent DHCP option 61.");
			}
		}
		else if (DsPtr->Hdr.Id == 255)
		{
			if (oha_is_valid_dhcp_usage(DsLen, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp))
			{
				if (oha_db_read_query_set_access(NicId, DCP_DBID_DHCP, DsPtr->Hdr.Id/*DsPtr->Data.Dhcp.OptionCode*/)) /* is it locked ? */
				{
					QueryOptions |= OHA_CTRL_DHCP_USAGE;
				}
				if (Status & DCP_MASK_DS_REMANENT)
				{
			...
			{
				OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "Invalid or inconsistent DHCP option 255.");
			}
		}
		else
		{
		    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "DHCP options (0x%X) invalid or not supported.", DsPtr->Hdr.Id);
		}
    }
    pInd->Type = DsPtr->Hdr.Opt;
    pInd->SubType = DsPtr->Hdr.Id;
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_ParseOptTcp(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
            ...
            if (DsLen == DCP_DS_TCPIP_SIZE)
            {
                ...
                if (AccessControl)
                {
                    ...
                    LSA_UINT16 RetVal = oha_db_store_temp_ip_address (NicId,
                                                                      ...
                                                                      DsPtr->Data.TcpIp.SubnetMask,
                                                                      DsPtr->Data.TcpIp.DefaultRouter,
                                                                      OHA_IP_ADDR_SIZE);
                    if (RetVal != LSA_RET_OK)
                    {
                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"oha_dcp_ParseOptTcp: oha_db_store_temp_ip_address failed (NicId: 0x%X, RetVal: 0x%X).", NicId, RetVal);
                        break;
                    }
...
/*****************************************************************************/
static LSA_UINT32                oha_dcp_QueryOptTcp(
    LSA_UINT16              NicId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
...
/*****************************************************************************/
LSA_VOID oha_dcp_free_rcv_rqb(OHA_HDB_DCP_PTR_TYPE pHDB,
                              OHA_DCP_LOWER_RQB_PTR_TYPE pRQB)
{
    ...
    OHA_QUERY_ELEM_PTR_TYPE      pQueryElem)
{
    ...
    LSA_UINT16              NICId;
    LSA_UINT16              ElemSize;
    OHA_DCP_ELEM_PTR_TYPE   pElemList; /* array (list) of DCP elements */
            ...
            if (RestLen >= ReadLen)
            {
                ...
                if (SetInd.Count)   /* element found ? */
                {
                    if (pQueryElem->Count < ElemSize)
                    {
                        ...
                        OHA_MEMSET(pElemList, 0, sizeof(OHA_DCP_ELEM_TYPE));
                        pElemList->Type     = DsPtr->Hdr.Opt;
                        pElemList->SubType  = DsPtr->Hdr.Id;
                        pElemList->Remanent = ((Status & DCP_MASK_DS_REMANENT) == 0) ? OHA_REMANENT_NO : OHA_REMANENT_YES;
    ...
    {
        RetVal = OHA_ERR_PROTOCOL;
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"No Dataset from Ethenet demanded!");
    }
...
// validate DHCP option 61
     ...
     *
//----------------------------------------------------------------------------*/
LSA_BOOL  oha_is_valid_dhcp_client_id(
    LSA_UINT16 Length,
...
)
{
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_is_valid_dhcp_client_id(length: 0x%X, data: 0x%X)", Length, pOption);
	...
	if (DhcpCode != 61)
	{
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_client_id(invalid DHCP Code %x): LSA_FALSE", DhcpCode);
        return LSA_FALSE;
    }
	
	if(DhcpLen == 0) /* note: must not test for >= 2 for historical reasons */
	{
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_client_id(invalid DHCP Len %x): LSA_FALSE",DhcpLen);
        return LSA_FALSE;
    }

	if (1/*Code*/ + 1/*Len*/ + DhcpLen > Length) /* note: must not test for equality for historical reasons */
	{
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_client_id(DHCP Len %x not fitting into DCP, Len %x): LSA_FALSE", DhcpLen, Length);
        return LSA_FALSE;
    }
	...
	if (DhcpType > 1)
	{
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_client_id(invalid DHCP Type %x): LSA_FALSE", DhcpType);
        return LSA_FALSE;
    }
	if (DhcpType == 1 && DhcpLen != 1)
	{
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_client_id(invalid DHCP Type %x with Len %x): LSA_FALSE", DhcpType, DhcpLen);
        return LSA_FALSE;
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_valid_dhcp_client_id(): LSA_TRUE");
    return LSA_TRUE; /* ok */
}

/*------------------------------------------------------------------------------
// validate DHCP option 255
*
* syntax:
*
* Code = 255		valid
* Len != 1	        not valid
* Data = 0 		    valid, don't use DHCP (default)
* Data = 1			valid, don't use DHCP, set all DHCP options to RtF values
* Data = 2			valid, use DHCP with the given objects
*
//----------------------------------------------------------------------------*/
LSA_BOOL oha_is_valid_dhcp_usage(
	LSA_UINT16 Length,
	OHA_COMMON_MEM_U8_PTR_TYPE pOption
)
{
	LSA_UINT8			DhcpCode;		/* ID of the DHCP option of list		*/
	LSA_UINT8			DhcpLen;		/* Length of the DHCP option of list	*/
	LSA_UINT8			DhcpData;		/* Data of the DHCP option of list		*/

	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
		"IN : oha_is_valid_dhcp_usage(length: 0x%X, data: 0x%X)", Length, pOption);
	...
	if (DhcpCode != 255)
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_usage(invalid DHCP Code %x): LSA_FALSE", DhcpCode);
		return LSA_FALSE;
	}

	if (DhcpLen != 1)
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_usage(invalid DHCP Len %x): LSA_FALSE", DhcpLen);
		return LSA_FALSE;
	}

	if (1/*Code*/ + 1/*Len*/ + DhcpLen > Length) /* note: must not test for equality for historical reasons */
	{
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_usage(DHCP Len %x not fitting into DCP, Len %x): LSA_FALSE", DhcpLen, Length);
		return LSA_FALSE;
	}
	...
	if (DhcpData > 2)
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_is_valid_dhcp_usage(invalid DATA Type %x): LSA_FALSE", DhcpData);
		return LSA_FALSE;
	}

	OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_valid_dhcp_usage(): LSA_TRUE");
	return LSA_TRUE; /* ok */
}
