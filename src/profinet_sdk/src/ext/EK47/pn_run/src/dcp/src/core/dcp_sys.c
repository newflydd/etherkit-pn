...
/*  2014-11-21  P06.00.00.00_00.01.48.01 1126404: SET-Statemachine gemäß IEC-*/
...
LSA_VOID)
{
	...
	*/
    ...
    //AP01503994
    if (DCP_MAX_CHANNELS > 63)	//lint !e506 !e774 HM 12/01/2016 define-based behaviour
	{
		...
		*/
		for(ChId = 0; ChId < DCP_MAX_CHANNELS; ChId++)
		{
			...
			*/
			...
			ChPtr->Cancel.Queue.FirstPtr = LSA_NULL;
			ChPtr->Cancel.Queue.LastPtr = LSA_NULL;
			...
			ChPtr->Get.RetryTimerPtr = LSA_NULL;
		}
...
/*****************************************************************************/
static DCP_TIMER_PTR_TYPE  DCP_LOCAL_FCT_ATTR  dcp_TimerTabFindId(
DCP_CHA_PTR		ChPtr,
LSA_UINT16		TimerId)
{
...
/*****************************************************************************/
/* Function: dcp_TimerTabFindRqb()                                           */
/* Find a timer block in 100ms queue of LSA timers by RQB pointer            */
/*****************************************************************************/
DCP_TIMER_PTR_TYPE  DCP_LOCAL_FCT_ATTR  dcp_TimerTabFindRqb(
DCP_CHA_PTR				ChPtr,
DCP_UPPER_RQB_PTR_TYPE	RbPtr)
{
	DCP_TIMER_PTR_TYPE	TimerPtr;

	DCP_FUNCTION_TRACE_02(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  > dcp_TimerTabFindRqb() ChPtr=%X RbPtr=%X", 
						ChPtr, RbPtr);

	TimerPtr = dcp_data.Timer100ms.Queue.FirstPtr;
	while (!LSA_HOST_PTR_ARE_EQUAL(TimerPtr, LSA_NULL))
	{
		if (TimerPtr->RbPtr == RbPtr)
		{
			break;
		}
		TimerPtr = TimerPtr->NextPtr;
	...
	DCP_FUNCTION_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerTabFindRqb() TimerPtr=%X", 
						TimerPtr);
	return(TimerPtr);
}
...
/*****************************************************************************/
static LSA_UINT16  DCP_LOCAL_FCT_ATTR  dcp_TimerTabAdd(
DCP_TIMER_PTR_TYPE			TimerPtr)
{
...
/*****************************************************************************/
static LSA_UINT16  DCP_LOCAL_FCT_ATTR  dcp_TimerTabRemove(
DCP_TIMER_PTR_TYPE			TimerPtr)
{
	DCP_TIMER_TAB	*TabPtr;
	LSA_UINT16		RetVal = LSA_RET_OK;
	...
	Found = dcp_TimerDeque(&TabPtr->Queue, TimerPtr);
	...
	DCP_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerTabRemove() RetVal=0x%x Base=%u Queue.Count=%u Found=%d", 
						RetVal, TimerPtr->Base, TabPtr->Queue.Count, Found);
	return(RetVal);
}
...
DCP_TIMER_PTR_TYPE 			TimerPtr)
{
	...
	DCP_FUNCTION_TRACE_02(0 /*TraceIdx*/, LSA_TRACE_LEVEL_NOTE, "  > dcp_timeout_int() TimerPtr=%X TimerId=%u", 
					TimerPtr, TimerPtr->Id);
	...
	if (!LSA_HOST_PTR_ARE_EQUAL(ChPtr, LSA_NULL))
	{
		...
		*/
		case DCP_TIMER_LOCK:
			...
			*/
			DCP_FUNCTION_TRACE_05(TraceIdx, LSA_TRACE_LEVEL_NOTE, "--- dcp_timeout_int() ChPtr=%X ServerSetWait=%u: ClientHoldTime end for ...%02X-%02X-%02X", 
						ChPtr, ChPtr->ServerSetWait, ChPtr->ClientHoldAddr[3], ChPtr->ClientHoldAddr[4], ChPtr->ClientHoldAddr[5]);
			if (!ChPtr->ServerSetWait)	//1126404: ignore in WACK state
			{
		...
		*/
		...
		if (TimerPtr->Running)
		{
            LSA_UINT16 TTRRetVal = LSA_RET_OK;
			if (!TimerPtr->Ext.Cyclic)
			{
				/* Remove one shot timer from timer queue
				*/
				TTRRetVal = dcp_TimerTabRemove(TimerPtr);
			...
			else if (!LSA_HOST_PTR_ARE_EQUAL(TimerPtr->RbPu.VoidPtr, LSA_NULL))
			{
				...
				*/
				...
				{
					DCP_ERROR_TRACE_05(TraceIdx, LSA_TRACE_LEVEL_ERROR, "*** dcp_timeout_int() ChPtr=%X TimerPtr=%X Id=%d: channel state=0x%x TTRRetVal=0x%X", 
									ChPtr, TimerPtr, TimerPtr->Id, ChPtr->State, TTRRetVal);
	...
	else
	{
		...
		*/
		DCP_ERROR_TRACE_02(0 /*TraceIdx*/, LSA_TRACE_LEVEL_NOTE, "*** dcp_timeout_int() TimerPtr=%X: Timeout without channel TimerId=%d", 
						TimerPtr, TimerPtr->Id);
	}
...
DCP_UINT16			Base)
{
	...
	{
		RetVal = LSA_RET_ERR_RESOURCE;
		DCP_ERROR_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_ERROR,"*** dcp_TimerAlloc() ChPtr=%X DCP_ALLOC_LOCAL_MEM length=%u failed RetVal=0x%x",
				ChPtr, sizeof(DCP_TIMER), RetVal);
	}
	else
	{
		...
		*/
		...
		default:
			...
			*/
			...
			DCP_ERROR_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_ERROR, "*** dcp_TimerAlloc() ChPtr=%X: Invalid timer Base=%u RetVal=0x%x", 
						ChPtr, Base, RetVal);
			dcp_ErrorInternal(DCP_MODULE_ID, __LINE__, DCP_ERR_TIMER, LSA_NULL, Base);
		}

		if (LSA_RET_OK == RetVal)   //lint !e774 Boolean within 'if' always evaluates to True, because dcp_ErrorInternal() is an exit, HM 13.01.2016
		{
...
DCP_UPPER_RQB_PTR_TYPE	RbPtr)
{
	...
	else
	{
		...
		*/
		...
		{
			if ((!TimerPtr->Running)
			&&  ((ChPtr->DetailPtr->ClientCh == DCP_CDB_SERVER_CHANNEL)
	...
	DCP_FUNCTION_TRACE_04(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerGet() ChPtr=%X RbuPtr=%X TimerPtr=%X Found=%X", 
						ChPtr, RbPtr, TimerPtr, Found);
	return (TimerPtr);
}
...
DCP_TIMER_QUEUE			*QueuePtr)
{
	...
	if (!LSA_HOST_PTR_ARE_EQUAL(TimerPtr, LSA_NULL))
	{
		if (!LSA_HOST_PTR_ARE_EQUAL(QueuePtr, LSA_NULL))
		{
			...
			*/
			Found = dcp_TimerDeque(QueuePtr, TimerPtr);
		}
	...
	DCP_FUNCTION_TRACE_04(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerPut() ChPtr=%X ChTimerPtr=%X Queue.count=%u Found=%X", 
						ChPtr, ChPtr->TimerPtr, ChPtr->TimerQueue.Count, Found);
}
...
/*****************************************************************************/
/* Internal access function: dcp_TimerFindRqb()                              */
/* Find DCP internal timer with given RQB pointer in queue                   */
/*****************************************************************************/
DCP_TIMER_PTR_TYPE  DCP_LOCAL_FCT_ATTR  dcp_TimerFindRqb(
DCP_CHA_PTR				ChPtr,
DCP_UPPER_RQB_PTR_TYPE	RbPtr,
DCP_TIMER_QUEUE			*QueuePtr)
{
	...
	DCP_FUNCTION_TRACE_06(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  > dcp_TimerFindRqb() ChPtr=%X RbPtr=%X QueuePtr=%X First=%X Last=%X Count=%u", 
						ChPtr, RbPtr, QueuePtr, QueuePtr->FirstPtr, QueuePtr->LastPtr, QueuePtr->Count);
	...
	*/
	...
	while (!LSA_HOST_PTR_ARE_EQUAL(TimerPtr, LSA_NULL))
	{
		if (LSA_HOST_PTR_ARE_EQUAL(TimerPtr->RbPtr, RbPtr))
		{
			/* Found
			*/
			break;
	...
	DCP_FUNCTION_TRACE_02(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerFindRqb() ChPtr=%X TimerPtr=%X", 
						ChPtr, TimerPtr);
	return(TimerPtr);
}
...
DCP_LOWER_RQB_PTR_TYPE	RblPu)
{
	...
	DCP_FUNCTION_TRACE_04(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerStart() ChPtr=%X TimerPtr=%X RetVal=0x%x Found=%X", 
						ChPtr, TimerPtr, RetVal, Found);
	return(RetVal);
}
...
/*****************************************************************************/
static LSA_VOID  DCP_LOCAL_FCT_ATTR  dcp_TimerFree(
DCP_CHA_PTR			ChPtr,
DCP_TIMER_PTR_TYPE	TimerPtr)
{
	LSA_UINT16 RetVal = LSA_RET_OK;
...
/*****************************************************************************/
static LSA_VOID  DCP_LOCAL_FCT_ATTR  dcp_TimerCleanAll(
DCP_CHA_PTR	ChPtr)
{
	DCP_TIMER_PTR_TYPE	TimerPtr;
	LSA_UINT8			TimerId;
	...
	*/ 
	...
	DCP_FUNCTION_TRACE_05(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_TimerCleanAll() ChPtr=%X TimerCount=%u Queue.Count=%u TimerPtr=%X Found=%X", 
						ChPtr, ChPtr->TimerCount, ChPtr->TimerQueue.Count, ChPtr->TimerPtr, Found);
}
		...
		&& (ChPtr->DetailPtr->LowerId == NewChPtr->DetailPtr->LowerId))
		{
			...
			*/
			...
			||  (DCP_DEV_FSU == ChPtr->DetailPtr->DevId))
			{
				...
				*/
				...
				else
				{
					...
					*/
					ChFound = LSA_FALSE;
					if (NewChPtr->DetailPtr->ClientCh == DCP_CDB_SERVER_CHANNEL)
					{
			...
			else
			{
				...
				else if (NewChPtr->DetailPtr->ClientCh == DCP_CDB_SERVER_CHANNEL)
				{
					/* New Server channel
					*/
	...
	for (ChId = 0; ChId < DCP_MAX_CHANNELS; ChId++)
	{
		...
		if (ChPtr->State == DCP_CHA_STATE_FREE)
		{
			...
			*/
			...
			ChPtr->ServerSetWait = LSA_FALSE;	//1126404
...
LSA_HANDLE_TYPE  Handle)
{
	...
	DCP_FUNCTION_TRACE_01(0 /*TraceIdx*/, LSA_TRACE_LEVEL_NOTE, "  > dcp_ChLowGet() Handle=%d", 
						Handle);
...
LSA_HANDLE_TYPE  Handle)
{
	...
	DCP_FUNCTION_TRACE_01(0 /*TraceIdx*/, LSA_TRACE_LEVEL_NOTE, "  > dcp_ChGet() Handle=%d", 
						Handle);
...
DCP_UPPER_RQB_PTR_TYPE RbuPtr)
{
    ...
    {
		DCP_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "  < dcp_DequeUpper() RbuPtr=%X ChPtr=%X", 
							RbuPtr, ChPtr);
    }
}
...
LSA_UINT16			Size)
{
	...
	dcp_data.Error.error_code[0] = (LSA_UINT32)ErrNum;
