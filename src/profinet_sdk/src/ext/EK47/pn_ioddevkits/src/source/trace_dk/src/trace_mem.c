...
#include "compiler.h"
	...
	static void TrcMemLog(PNIO_INT8* pMsgBuf, PNIO_UINT32 MsgSize)
	{
		//if (PnioLogDest == 2)
		{
    ...
    PNIO_UINT32     TrcInit	  (PNIO_UINT32 BufSize)
    {
        ...
        {
            Status = OsAllocF ((void**) &pBuf, TrcBufTotalSize);
            if (Status == PNIO_OK)
    ...
    PNIO_UINT32 TrcTraceMsgStore (PNIO_UINT32 ModId, PNIO_UINT32 Line, PNIO_UINT32 subsys, PNIO_UINT32 level, PNIO_INT8 * pMsgBuf, PNIO_UINT32 StrLen)
    {
    	PNIO_UINT32 TmpSize;
    	PNIO_UINT32 MsgSize;
        PNIO_UINT32 TimStampL = OsGetTime_ms();
        OsTakeSemB (BufSemId);
    ...
    // *
    // *  input :  void*        pBuf      pointer to the message buffer
    ...
    PNIO_UINT32 TrcPrintMsgStore (PNIO_INT8 * fmt, PNIO_va_list argptr)
    {
		...
		while (TempBufIndex > OFFS_MSG2)
		{
			...
			if (LineBreakIndex < TempBufIndex)
			{
				...
				if (TempBufIndex)
				{
					OsMemCpy (&TempBuf[OFFS_MSG2], &TempBuf[LineBreakIndex+1], TempBufIndex);
    ...
    void TrcMsg2Mem  (PNIO_INT8* pMsgBuf, PNIO_UINT32 MsgSize)
    {
        ...
        OsMemCpy (pBuf + BufInd, pMsgBuf, MsgSize);
        BufInd += MsgSize;
        if (TrcBufOverflow)
        {
