...
/* DCP-GET */
static LSA_INT oha_dcp_GetOptPbus  (LSA_UINT16 NICId, LSA_UINT16 Type, DCP_USR_DATASET_PTR DsPtr, LSA_UINT32 *DcpGetStatus);
static LSA_INT oha_dcp_GetOptTcp   (LSA_UINT16 NICId, LSA_UINT16 Type, DCP_USR_DATASET_PTR DsPtr, LSA_UINT32 *DcpGetStatus);
static LSA_INT oha_dcp_GetOptDhcp  (LSA_UINT16 NICId, LSA_UINT16 Type, DCP_USR_DATASET_PTR DsPtr, LSA_UINT32 *DcpGetStatus);
static LSA_INT oha_dcp_GetOptIdent (LSA_UINT16 NICId, LSA_UINT16 Type, DCP_USR_DATASET_PTR DsPtr, LSA_UINT32 *DcpGetStatus);
static LSA_INT oha_dcp_GetOptInitiative(LSA_UINT16 NICId, LSA_UINT16 Type, DCP_USR_DATASET_PTR DsPtr, LSA_UINT32 *DcpGetStatus);

/* DCP-IDENTIFY */
static LSA_INT oha_dcp_IdentifyFilterCheck(LSA_UINT16 NICId, DCP_USR_REQ_DATA_PTR ReqPtr, LSA_INT OptCnt,
                                    DCP_USR_RSP_DATA_PTR RspPtr, LSA_INT *OptCntPtr);
static LSA_INT oha_dcp_IdentifyRspSet (LSA_UINT16 NICId, DCP_USR_RSP_DATA_PTR DataPtr, LSA_INT DataLen,
                                LSA_INT *OptCntPtr);

static LSA_BOOL oha_dcp_FindType (DCP_USR_RSP_DATA_PTR DataPtr, LSA_INT DataLen, LSA_UINT16 Type);

/* DCP-SET */
static LSA_UINT8 oha_dcp_SetOptControl(LSA_UINT16 NICId, DCP_USR_DATASET_PTR  DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT8 oha_dcp_SetOptTcp (LSA_UINT16 NICId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT8 oha_dcp_SetOptDhcp(LSA_UINT16 NICId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT8 oha_dcp_SetOptIdent(LSA_UINT16 NICId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
static LSA_UINT8 oha_dcp_SetOptPbus(LSA_UINT16 NICId, DCP_USR_DATASET_PTR DsPtr, OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd);
...
/*****************************************************************************/
static LSA_VOID oha_dcp_confirm_remote_set_ip(
	LSA_UINT16 NICId,
	LSA_UINT32 Options,
	LSA_BOOL CurrentIPIsEmergency)
{
	...
	if(OHA_MEMCMP(IpAddrPtr, (OHA_COMMON_MEM_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE))
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: oha_dcp_confirm_remote_set_ip(NICId: 0x%X) - IP address is 0.0.0.0", NICId);
		return;
	}
	...
	if (CurrentIPIsEmergency) /* EmergencyIp := CurrentIp */
	{
		...
		{
			 OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR, "oha_dcp_confirm_remote_set_ip(NICId: 0x%X): Write Emergency IP failed (RetVal=%u).", NICId, RetVal);
		}
	}
	else    /* ExpectedIp := CurrentIp, EmergencyIp := 0.0.0.0 */
	{
		...
		if (RetVal != LSA_RET_OK)
		{
			 OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR, "oha_dcp_confirm_remote_set_i(NICId: 0x%X)p: Write Expected IP failed (RetVal=%u).", NICId, RetVal);
		}
    ...
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pSetInd)
{
    ...
    if (pSetInd->Options)           /* changes made ? */
    {
        ...
        if (pSetInd->Options & OHA_CTRL_IP_SUITE)
        {
            ...
            if (pSetInd->Event == OHA_MODE_EMERGENCY_SET) /* EmergencyIp := CurrentIp */
            {
				...
				if (RetVal != LSA_RET_OK)
				{
					 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_RequestSetAddress: Write Emergency IP failed (RetVal=%u).", RetVal);
				}
            ...
            else    /* ExpectedIp := CurrentIp, EmergencyIp := 0.0.0.0 */
            {
				...
				if (RetVal != LSA_RET_OK)
				{
					 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_RequestSetAddress: Write Expected IP failed (RetVal=%u).", RetVal);
				}
				...
				if (Remanent)	/* AP00909635 */
				{
					...
					if (RetVal != LSA_RET_OK)
					{
						 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_RequestSetAddress: Write REMA IP failed (RetVal=%u).", RetVal);
					}
				}
				else
				{
            ...
            if (RetVal != OHA_OK)
            {
                OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_dcp_confirm_remote_set - EDDSetArpFilter(RetVal: 0x%X)", RetVal);
            }
            RetVal = OHA_EDDSetIp(NICId, OHA_HANDLE_TYPE_DCP);
            if (RetVal != OHA_OK)
            {
                OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_dcp_confirm_remote_set - EDDSetIp(RetVal: 0x%X)", RetVal);
            }
        }
	...
	if(Options)
	{
		...
		if (RetVal != OHA_OK)
		{
			OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: oha_dcp_confirm_remote_set - LLDPUpdateSend(RetVal: 0x%X)", RetVal);
		}
	}

#ifndef OHA_CFG_NO_REMA
	 if ((Options & OHA_CTRL_DHCP_CLIENT_ID) ||
		 (Options & OHA_CTRL_DHCP_USAGE))
	 {
    ...
    /* is the IP-Suite to check (ethernet) */
    {
        ...
        {
            RetVal = OHA_NARECheckAddress(NICId, CheckOptions, pSetInd->Options);
            if (RetVal != OHA_OK)
            {
                OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_dcp_confirm_remote_set - OHA_NARECheckAddress failed (RetVal: 0x%X)", RetVal);
            }
        }

        /* no user query, no check */
    }
#endif
    ...
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pSetInd)
{
	...
	ReqPtr = (DCP_USR_REQ_DATA_PTR)(pRecv->DataPtr);
    ...
    if (pRecv->OptCnt > 0)
    {
        ...
        /* calculate length of the TLV-chain */
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(RbPtr, LSA_NULL) )
        {
            ...
            while (RestLen > 0)
            {
                ...
                if (RestLen >= ReadLen)
                {
					...
					if (SendLen + WriteLen > DCP_MAX_RSP_DATA_SIZE)
					{
						...
						OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"All requested data OptCnt: %d) do not fit in the DCP-Set-ResPDU!", pRecv->OptCnt);

						/* Requesting a Set service containing more "to be setted blocks" than could be acknowledged in the response is malformed. */
						OHA_FATAL(OHA_FATAL_ERR_DCP);  /* NOTREACHED */
						break;
					}
				...
				{LSA_UINT16 SwapValue = 1+2; ResultPtr->Hdr.Len = OHA_SWAP16(SwapValue);} /* lint 572 */
            ...
            pHDB->TxPendingCnt++;
            /* send Response */
            OHA_DCP_RQB_SET_HANDLE(RbPtr,pHDB->DcpHandle);  /* because of preallocation, see TFS 1126408 */
            OHA_DCP_REQUEST_LOWER(RbPtr,pHDB->Params.pSys); /* DCP_OPC_SEND_RSP */
			...
			if(pSetInd->Options & OHA_CTRL_IP_SUITE)
			{
				switch(pSetInd->Event)
				{
					...
					default:
			            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"DCP-Set failed.(Set-Event: 0x%X).",pSetInd->Event);
        ...
        else
        {
            ...
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"DCP-Getting lower RQB-memory failed!");
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
    else
    {
        ...
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"No dataset demanded from the ethenet!");
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
    }
    ...
    OHA_HDB_DCP_PTR_TYPE         pHDB)
{
    ...
    DCP_USR_REQ_DATA_PTR            ReqPtr;
    /* OHA_MAC_ADDR                 AddrPtr  = pRecv->AddrPtr;  */
    ...
    if (pRecv->OptCnt > 0)
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(RbPtr, LSA_NULL) )
        {
            ...
            while (DsetCnt < pRecv->OptCnt)
            {
				...
				if (SendLen + DsetLen > DCP_MAX_RSP_DATA_SIZE)
				{
                    ...
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"All requested data (DsetCnt: %d) do not fit in the DCP-Get-ResPDU!", DsetCnt);
					break;
				}
                ...
                if (Result != DCP_RESULT_OK) /* a Dcp-Get is not allowed */
                {
                    ...
                    ResultPtr->Hdr.Opt = DCP_DBID_CTRL;
                    ResultPtr->Hdr.Id = DCP_DSID_RESPONSE;
					{LSA_UINT16 SwapValue = 1+2; ResultPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
                    ResultPtr->Data.Result.Code = Result;
                    ResultPtr->Data.Result.AddOn = DCP_RESULT_RESERVED;
                    DsetLen = DCP_DS_RESULT_MIN_SIZE;
                }
        ...
        {
            RetVal = LSA_RET_ERR_RESOURCE;
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP-Allocating lower RQB-memory failed!");
        }
    }
    else
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"No Dataset demanded from the Ethenet!");
    }
...
/*****************************************************************************/
LSA_UINT16 oha_dcp_Identify (OHA_DCP_LOWER_RQB_PTR_TYPE   pRQB,
                             OHA_HDB_DCP_PTR_TYPE         pHDB)
{
...
/*                                                                           */
/*****************************************************************************/
static LSA_UINT16 oha_dcp_CheckHello(DCP_USR_BYTE_PTR DataPtr,
							LSA_INT	OptCnt)
{
    LSA_UINT16              RetVal;
    LSA_INT                 i;
...
)
{
	...
	if (RetVal != OHA_OK)
	{
		...
		RetVal = OHA_ERR_PROTOCOL;
		OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_WARN,"Wrong coding of the DCP-Hello-ReqPDU (RetVal: 0x%X)!", RetVal);
    ...
    if (pRecv->OptCnt > 0)
    {
            ...
            if (ReqLen >= ReadLen)
            {
                ...
                switch (DbaseID)
                {
                    ...
                    default:                    /* Unknown Dataset */
						Result |= DCP_RESULT_OK;
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Indication HELLO: An unknown option occurred (DbaseID: 0x%X)",DbaseID);
                        break;
                }
            }
        ...
        {
            RetVal = LSA_RET_ERR_RESOURCE;
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Got no Dataset from the Ethenet!");
        }
    }
    else
    {
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetOptPbus(
    LSA_UINT16              NICId,
    LSA_UINT16              Type,
    DCP_USR_DATASET_PTR     DsPtr,
    LSA_UINT32          *   DcpGetStatus )
{
	...
	OHA_ASSERT_NULL_PTR(DcpGetStatus);
        ...
        case DCP_DSID_PBCOMMON:         /* suboption common parameters */
            ...
            if (RetVal == LSA_RET_OK && OHA_IS_NOT_NULL(ParamPtr))
            {
                if(OHA_IS_NOT_NULL(DsPtr))
                {
					...
					{LSA_UINT16 SwapValue = DCP_DS_PBCOMMON_SIZE+2+3/*OUI*/; DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
        ...
        case DCP_DSID_PBADDITION:          /* suboption, additional parameters */
            ...
            if (RetVal == LSA_RET_OK && OHA_IS_NOT_NULL(ParamPtr))
            {
                if(OHA_IS_NOT_NULL(DsPtr))
                {
					...
					{LSA_UINT16 SwapValue = DCP_DS_PBADDITION_SIZE+2+3/*OUI*/; DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetTcpIpAddr(
    LSA_UINT16              NICId,
    DCP_USR_DATASET_PTR     DsPtr)
{
    LSA_UINT16      Status;
    ...
    if (OHA_IS_NOT_NULL(DsPtr))
    {
		...
		{LSA_UINT16 SwapValue = DCP_DS_TCPIP_SIZE+2; DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
    }

    DataLen = DCP_DS_HDR_SIZE+2 + DCP_DS_TCPIP_SIZE;

    OHA_FUNCTION_TRACE_02(pDB->TraceIdx, LSA_TRACE_LEVEL_CHAT,
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetOptTcp(
    LSA_UINT16              NICId,
    LSA_UINT16              Type,
    DCP_USR_DATASET_PTR     DsPtr,
    LSA_UINT32          *   DcpGetStatus )
        ...
        case DCP_DSID_MACADR:       /* MAC-Address */
        {
            ...
            if (Len == DCP_DS_MACADDR_SIZE && AddrPtr)
            {
                if(OHA_IS_NOT_NULL(DsPtr))
                {
					...
					{LSA_UINT16 SwapValue = DCP_DS_MACADDR_SIZE+2; DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
                }
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetOptDhcp(
    LSA_UINT16              NICId,
    LSA_UINT16              Type,
    DCP_USR_DATASET_PTR     DsPtr,
    LSA_UINT32          *   DcpGetStatus )
{
    ...
    if (RetVal == LSA_RET_OK)
    {
        if (!LSA_HOST_PTR_ARE_EQUAL(OptPtr, LSA_NULL)
            &&  (*OptPtr == DsID))  /* DHCP option existing (length <> 0 */
        {
				...
				DCP_DBID_DHCP, DsID)) /* is it locked ? */
			{
				if (DsID == 61)
				{
					(*DcpGetStatus) |= OHA_CTRL_DHCP_CLIENT_ID;
				}
				else if (DsID == 255)
				{
					(*DcpGetStatus) |= OHA_CTRL_DHCP_USAGE;
				}
			}

			/* if DHCP is disabled, the DHCP options, e.g. 61 should'nt be readable */
			if (DsID != 255 && oha_db_is_dhcp_disabled(&pDbIf->DcpData.pDhcpOptions))
			{
				return(0);
			}
            ...
            if(OHA_IS_NOT_NULL(DsPtr))
            {
				...
				/* if option 255 had the control value (1), the DHCP RtF values already have been set */
				/* so the state (value = 0) is to show */
				if (DsPtr->Data.Dhcp.OptionCode == 255 &&
					DsPtr->Data.Dhcp.OptionLen == 1 &&
					DsPtr->Data.Dhcp.OptionVal[0] == 1) /* don't use DHCP but set all DHCP options to RtF */
				{
					DsPtr->Data.Dhcp.OptionVal[0] = 0;
				}
            }
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetOptIdent(
    LSA_UINT16              NICId,
    LSA_UINT16              Type,
    DCP_USR_DATASET_PTR     DsPtr,
    LSA_UINT32          *   DcpGetStatus )
{
    ...
    switch (DsID)
    {
        ...
        case DCP_DSID_IDID:         /* Stationsidentification */
			...
			static LSA_UINT8 DevIdent[2*sizeof(LSA_UINT16)];
        ...
        case DCP_DSID_IDDEVINST:         /* DeviceInstance */
			...
			static LSA_UINT8 DevInst[sizeof(pDB->PNIdentValid.InstanceID)];
        ...
        case DCP_DSID_IDROLE:       /* ProfinetIO Role */
			{
				...
				if(OHA_IS_NOT_NULL(DsPtr))
				{
					...
					{LSA_UINT16 SwapValue = DCP_DS_IDROLE_SIZE+2; DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
				}
...
/*****************************************************************************/
LSA_VOID oha_dcp_free_rsp_rqb(OHA_HDB_DCP_PTR_TYPE pHDB,
                                                 OHA_DCP_LOWER_RQB_PTR_TYPE pRQB)
{
...
/*****************************************************************************/
/* Local function: oha_dcp_RspBufferOverflow()                               */
/* Check for requested DCP data, if they fully fit in the DCP-Get-ResPDU     */
/* LSA_TRUE: not fitting, LSA_FALSE: fit in the PDU                          */
/*****************************************************************************/
static LSA_BOOL oha_dcp_RspBufferOverflow(
	LSA_INT              Len_1,
	LSA_INT              Len_2,
	LSA_INT              OptCnt)
{
	LSA_INT Len = Len_1 + (Len_1 % 2) + Len_2 + (Len_2 % 2); /* fillbytes */

	if (Len > DCP_MAX_RSP_DATA_SIZE) /* not fitting in the response frame */
	{
		/* 2007741: The (further) requested data do not fit in the DCP-ResPDU ! */
		OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH, "All requested data (OptCnt: %d) do not fit in the DCP-ResPDU (Len: %d)!", OptCnt, Len);
		return LSA_TRUE;
	}
	return LSA_FALSE;
}
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_IdentifyFilterCheck(
    LSA_UINT16              NICId,
    DCP_USR_REQ_DATA_PTR    ReqPtr,
    LSA_INT                 OptCnt,
    ...
    LSA_INT             *   OptCntPtr)
{
    ...
    DCP_USR_BYTE_PTR        WritePtr; /* lint critical: possible creation of out-of-bounds pointer */
	...
	OHA_ASSERT_NULL_PTR(ReqPtr);
	OHA_ASSERT_NULL_PTR(RspPtr);
	OHA_ASSERT_NULL_PTR(OptCntPtr);

	WritePtr = RspPtr->Byte;
    /* get length of filter chain */
    /* BytePtr = &ReqPtr->Identify.Filter.Opt; */
    BytePtr = ReqPtr->Byte; /* lint critical: possible creation of out-of-bounds pointer */
    ...
    while (FilterMatch && (RestLen > 0))
    {
        ...
        else if ((DCP_DBID_IDENT == DsPtr->Opt) && (DCP_DSID_IDTYPE == DsPtr->Id))
        {
            ...
            if (FilterMatch)
            {
				...
				if (oha_dcp_RspBufferOverflow(RetLen, DsetLen + DCP_DS_HDR_SIZE + 2, OptCnt))
				{
					return(RetLen); /* not fitting in the response frame anymore */
				}
        ...
        else
        {
...

            {
                ...
                {
                    case DCP_DBID_TCP:          /* TCP/IP-Parameter */
						DsetLen = oha_dcp_GetOptTcp(NICId, Type, LSA_NULL, &DcpSetStatus);
						if (oha_dcp_RspBufferOverflow(RetLen, DsetLen, OptCnt))
						{
							return(RetLen); /* not fitting in the response frame anymore */
						}
                    ...
                    case DCP_DBID_DHCP:         /* DHCP-Parameter */
						DsetLen = oha_dcp_GetOptDhcp(NICId, Type, LSA_NULL, &DcpSetStatus);
						if (oha_dcp_RspBufferOverflow(RetLen, DsetLen, OptCnt))
						{
							return(RetLen); /* not fitting in the response frame anymore */
						}
                    ...
                    case DCP_DBID_IDENT:        /* general Device properties */
						DsetLen = oha_dcp_GetOptIdent(NICId, Type, LSA_NULL, &DcpSetStatus);
						if (oha_dcp_RspBufferOverflow(RetLen, DsetLen, OptCnt))
						{
							return(RetLen); /* not fitting in the response frame anymore */
						}
                    ...
                    case DCP_DBID_PBPARAM:      /* profibus parameters */
						DsetLen = oha_dcp_GetOptPbus(NICId, Type, LSA_NULL, &DcpSetStatus);
						if (oha_dcp_RspBufferOverflow(RetLen, DsetLen, OptCnt))
						{
							return(RetLen); /* not fitting in the response frame anymore */
						}
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_IdentifyRspSet(
    LSA_UINT16              NICId,
    ...
    LSA_INT             *   OptCntPtr)
{
    ...
    {
		Len = oha_dcp_GetOptIdent(NICId, Type, LSA_NULL, &DcpGetStatus);
		if (oha_dcp_RspBufferOverflow(DataSize, Len, DbaseCnt))
		{
			return(DataSize); /* not fitting in the response frame anymore */
		}
    ...
    if (!oha_dcp_FindType(DataPtr, DataLen, Type))
    {
		Len = oha_dcp_GetOptTcp(NICId, Type, LSA_NULL, &DcpGetStatus);
		if (oha_dcp_RspBufferOverflow(DataSize, Len, DbaseCnt))
		{
			return(DataSize); /* not fitting in the response frame anymore */
		}
        ...
        if (Len == 0)
        {
            ...
            OHA_MEMCPY(BytePtr, DefaultIpParam, (LSA_UINT32)Len);
    ...
    if (!oha_dcp_FindType(DataPtr, DataLen, Type))
    {
		Len = oha_dcp_GetOptInitiative(NICId, Type, LSA_NULL, &DcpGetStatus);
		if (oha_dcp_RspBufferOverflow(DataSize, Len, DbaseCnt))
		{
			return(DataSize); /* not fitting in the response frame anymore */
		}
...
/*****************************************************************************/
static LSA_BOOL oha_dcp_FindType(
    DCP_USR_RSP_DATA_PTR    DataPtr,
    LSA_INT                 DataLen,
    LSA_UINT16              Type)
{
    ...
    BytePtr = &DataPtr->Identify.IdentDs.Hdr.Opt;
    while (Count < DataLen)
    {
...
/*****************************************************************************/
static LSA_UINT8                               oha_dcp_SetOptControl(
    LSA_UINT16                              NICId,
    DCP_USR_DATASET_PTR                     DsPtr,
...
/*****************************************************************************/
static LSA_UINT8               oha_dcp_SetOptPbus(
    LSA_UINT16              NICId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: oha_dcp_SetOptPbus(), DsPtr: 0x%X, Result:0x%X", DsPtr, Result);
    return(Result);
}
...
/* of a SET-Request                                                          */
/*****************************************************************************/
static LSA_UINT8               oha_dcp_SetOptIdent(
    LSA_UINT16              NICId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    switch (DsPtr->Hdr.Id)
    {
        ...
        case DCP_DSID_IDNAME:       /* NameOfStation */
            ...
            if (AccessControl)
            {
                ...
                if (DataLen && (OHA_ERR_PARAM == oha_check_station_name((LSA_UINT16)DataLen, DsPtr->Data.IdentName.String,
                                                                        OHA_UPPER_CASES_ALLOWED(NICId) ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO,
                                                                        OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE)) //lint -e415 -e416
                {
                    Result =  DCP_RESULT_DS_NOT_SET;
                ...
                if (len == DataLen)
                {
                    Found = oha_namecmp(DsPtr->Data.IdentName.String, pName, (LSA_UINT32)len, OHA_UPPER_CASES_ALLOWED(NICId));
...
/*****************************************************************************/
static LSA_UINT8               oha_dcp_SetOptDhcp(
    LSA_UINT16              NICId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_dcp_SetOptDhcp(NICId: 0x%X, DsPtr: 0x%X, DsPtr->Hdr.Id: 0x%X)",
                          NICId, DsPtr, DsPtr->Hdr.Id);
    ...
    if (DsLen >= 2)
    {
		...
		/* TFS 857716: check the DHCP-Option client identifier */
		if (DsPtr->Hdr.Id == 61)
		{
			if(oha_is_valid_dhcp_client_id(DsLen, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp))
			{
				if (oha_db_read_set_access(NICId, DCP_DBID_DHCP, DsPtr->Hdr.Id/*DsPtr->Data.Dhcp.OptionCode*/)) /* is it locked ? */
				{
					LSA_UINT16 RetVal = OHA_DB_WRITE_DHCP_OPTION(NICId, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp);

					if (RetVal == LSA_RET_OK)
					{
						Result = DCP_RESULT_OK;
						pInd->Options |= OHA_CTRL_DHCP_CLIENT_ID;
						if ((Status & DCP_MASK_DS_REMANENT) != 0)
						{
							pInd->RemaOptions |= OHA_CTRL_DHCP_CLIENT_ID;
		...
		else if (DsPtr->Hdr.Id == 255)
		{
			if (oha_is_valid_dhcp_usage(DsLen, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp))
			{
				if (oha_db_read_set_access(NICId, DCP_DBID_DHCP, DsPtr->Hdr.Id/*DsPtr->Data.Dhcp.OptionCode*/)) /* is it locked ? */
				{
					LSA_UINT16 RetVal = OHA_DB_WRITE_DHCP_OPTION(NICId, (OHA_COMMON_MEM_U8_PTR_TYPE)&DsPtr->Data.Dhcp);
					...
					{
						Result = DCP_RESULT_OK;
						pInd->Options |= OHA_CTRL_DHCP_USAGE;
						if ((Status & DCP_MASK_DS_REMANENT) != 0)
						{
							pInd->RemaOptions |= OHA_CTRL_DHCP_USAGE;
							(LSA_VOID)oha_db_write_remanence(NICId, OHA_DCP_OPTION_DHCP, DsPtr->Hdr.Id, LSA_TRUE);
						}
						pInd->Count++;
					}
...
/*****************************************************************************/
static LSA_UINT8               oha_dcp_SetOptTcp(
    LSA_UINT16              NICId,
    DCP_USR_DATASET_PTR     DsPtr,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE pInd)
{
    ...
    switch (DsPtr->Hdr.Id)
    {
        ...
        case DCP_DSID_TCPIP:        /* IP-Parameter */
            ...
            if (DsLen == DCP_DS_TCPIP_SIZE)
            {
                ...
                IpIsValid = (OHA_OK == oha_check_ip_address(OHA_GET32_NTOH(&DsPtr->Data.TcpIp.IpAddr,0),
                                                            OHA_GET32_NTOH(&DsPtr->Data.TcpIp.SubnetMask,0),
                                                            OHA_GET32_NTOH(&DsPtr->Data.TcpIp.DefaultRouter,0),
                                                            OHA_IP_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE); /* 0.0.0.0 is also allowed by a remote dcp set (AP00521755) */
...
/*****************************************************************************/
static LSA_INT                 oha_dcp_GetOptInitiative(
    LSA_UINT16              NICId,
    LSA_UINT16              Type,
    DCP_USR_DATASET_PTR     DsPtr,
    LSA_UINT32          *   DcpGetStatus )
        ...
        case DCP_DSID_DEVINIT:      /* Device-Initiative */
        {
                ...
                OHA_IS_NOT_NULL(ValuePtr))
            {
				if (OHA_IS_NOT_NULL(DsPtr))
				{
                	...
                	DsPtr->Status = DCP_RESERVED;
					{LSA_UINT16 SwapValue = DCP_DS_DEVINIT_SIZE+2;DsPtr->Hdr.Len = OHA_SWAP16(SwapValue);}
				}
    ...
    LSA_INT             *   OptCntPtr)
{
    ...
    if (Len == 0)
    {
        ...
        OHA_MEMCPY(BytePtr, DefaultInitiative, (LSA_UINT32)Len);
    }
    if ((Len % 2) != 0)
    {
        /* fillbyte if length is odd */
