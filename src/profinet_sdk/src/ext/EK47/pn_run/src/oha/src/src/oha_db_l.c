...
static LSA_BOOL OHA_LldpIsPortOption(LSA_UINT8 Type, LSA_UINT16 PortId)
{
...
static LSA_BOOL OHA_LldpIsMib2Object(LSA_UINT8 Type)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestReadLldpElement(
    ...
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_ELEMENT failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NicId);
    }
    else if ( OHA_IS_NULL(pLldpElement->pElements) && pLldpElement->Count != 0)
    {
        ...
        OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_ELEMENT failed. No ArrayOfElements given(NIC-ID: 0x%X, Count: 0x%X)", NicId, pLldpElement->Count);
        pLldpElement->Count = 0;
    }
    else if ((!pLldpElement->PortId) || (pLldpElement->PortId > OHA_CFG_MAX_PORTS))
    {
        ...
        pLldpElement->Count = 0;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_ELEMENT failed. PortID is wrong (PORT-ID: 0x%X)", pLldpElement->PortId);
    }
             ...
             pLldpElement->OUI == 0)
    {
        ...
        pLldpElement->Count = 0;
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_ELEMENT failed. OUI is wrong (OUI: 0x%X)", pLldpElement->OUI);
    }
    else
    {
        ...
        if (SenderId && (SenderId <= LLDP_MAX_PORT_SENDER)) /* check it */
        {
            ...
            OHA_LLDP_OPTION_PTR_TYPE pUserList = (pLldpElement->Count) ? pLldpElement->pElements : LSA_NULL; /* array (list) of LLDP elements */

            OHA_ASSERT_NULL_PTR(pDbSender);
                ...
                if (pLldpElement->Type && pLldpElement->SubType)
                {
                                                                   ...
                                                                   &pVariable, &VarLength))
                    {
                        if (SubType == pLldpElement->SubType)
                        {
                            ...
                            {
                                OHA_MEMSET(pUserList, 0, sizeof(OHA_LLDP_OPTION_TYPE));
					            pUserList->PortId   = pLldpElement->PortId;
                    ...
                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestReadOneLldpElement(Response: 0x%X)", Response);
                    return; /*  ==========>>>>>>>>>>>>  */
                }
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestReadLldpElement(Response: 0x%X)", Response);
}
...
/*****************************************************************************/
static OHA_DB_LLDP_ELEM_PTR_TYPE oha_find_lldp_DbElement(
    const OHA_LIST_TYPED_ENTRY_TYPE(oha_db_lldp_elem_tag) * const pAnchor,  /* start of a list  */
    LSA_UINT8                   Type,       /* type of variable */
    LSA_UINT8                   SubType,    /* subtype of variable */
...
/*****************************************************************************/
static LSA_VOID oha_remove_lldp_DbElement (OHA_DB_LLDP_ELEM_PTR_TYPE pElement)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
...
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_delete_rem_lldp_pno_others(NicId: 0x%X, PortId: 0x%X, SenderId: 0x%X)",
...
)
{
    ...
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_get_rem_lldp_db_element(NicId: 0x%X, pElement: 0x%X)", NicId, pElement);
    ...
    {
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_get_rem_lldp_db_element(pElement: 0x%X)", pElement);
        return (pElement);           /* NULL */
    }
    ...
    if (OHA_IS_NOT_NULL(pElement) && OhaListIsNotInitialized(pElement))
    {
		...
		OHA_PROGRAM_TRACE_03(0, LSA_TRACE_LEVEL_FATAL,"oha_get_rem_lldp_db_element(pElement: 0x%X, Type: 0x%X, PortId: 0x%X): List not initalized",pElement,Type,PortId);
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_write_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,   /* index + 1 to RemoteSender[] - for SNMP */
    LSA_UINT8                   Type,       /* type of variable */
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_write_rem_lldp_element(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,   /* index + 1 to RemoteSender[] - for SNMP */
...
)
{
    ...
    /* ++ check the params ++ check the params ++ */
	OHA_ASSERT_NULL_PTR(ppVariable);
	OHA_ASSERT_NULL_PTR(pLength);
...
)
{
    ...
    if (!Type) /* invalid (sub)option or no element */
    {
        OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_get_loc_lldp_db_element(NicId: 0x%X, pElement: 0x%X, Type: 0x%X)", NicId, pElement, Type);
        return (pElement);           /* NULL */
    }
    ...
    if (OHA_LldpIsPortOption(Type, PortId))
    {
        ...
        {
            /* Error on getting port. Abort. */
            OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_get_loc_lldp_db_element(NicId: 0x%X, pElement: 0x%X, PortId:  0x%X)", NicId, pElement, PortId);
            return (pElement);           /* NULL */
        }
    }
    ...
    else
    {
        ...
        {
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_get_loc_lldp_db_element(NicId: 0x%X, pElement: 0x%X)", NicId, pElement);
            return (pElement);       /* NULL */
        }
    ...
    {
        /* OhaListInitialize(pElement); */
		OHA_PROGRAM_TRACE_03(0, LSA_TRACE_LEVEL_FATAL,"oha_get_loc_lldp_db_element(pElement: 0x%X, Type: 0x%X, PortId: 0x%X): List not initalized",pElement,Type,PortId);
		OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
    }
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_lldp_write_remanence(NicId: 0x%X, Type: 0x%X)", NicId, Type);
        return (RetVal);
    }
    ...
    if (OHA_LldpIsPortOption(Type, PortId))
    {
        if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
        {
            /* Error on getting port. Abort. */
	        RetVal = LSA_RET_ERR_PARAM;
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_lldp_write_remanence(NicId: 0x%X, PortId:  0x%X)", NicId, PortId);
            return (RetVal);
        }
    }
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_lldp_read_remanence(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
    OHA_COMMON_MEM_U8_PTR_TYPE  pSubType,   /* subtype of variable, suboption */
...
)
{
    ...
    /* ++ check the params ++ check the params ++ */
	OHA_ASSERT_NULL_PTR(RemaStatus);
	OHA_ASSERT_NULL_PTR(pSubType);
    if (!Type) /* invalid option */
    {
        *RemaStatus = 0;
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_lldp_read_remanence(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
 ...
 *===========================================================================*/
LSA_BOOL oha_db_lldp_compare_remanence(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
    OHA_COMMON_MEM_U8_PTR_TYPE  pSubType,   /* subtype of variable, suboption */
...
)
{
	...
	OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,"oha_db_lldp_compare_remanence: Remanence(0x%X) read",Remanence);
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_LLDP_ELEMENT(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable */
    LSA_UINT8                   SubType,    /* subtype of variable */
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DB_WRITE_LLDP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
    return RetVal;
}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_ELEMENT(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,        /* type of variable, option */
    OHA_COMMON_MEM_U8_PTR_TYPE  pSubType,    /* subtype of variable, suboption */
...
)
{
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_READ_LLDP_ELEMENT(Type: 0x%X, pSubType: 0x%X, pLength: 0x%X, OUI: 0x%X)",
    ...
    if (!Type) /* invalid option */
    {
        ...
        *pLength    = 0;
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DB_READ_LLDP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
    return RetVal;
}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_delete_loc_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_delete_loc_lldp_element(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_db_delete_loc_lldp_element(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
    return RetVal;
}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_delete_rem_lldp_element(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,
    LSA_UINT8                   Type,       /* type of variable, option */
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_delete_rem_lldp_element(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
...
/*+               Fills RQB with station data and confirms request.         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestReadLldpOptions(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE      pHDBEdd;
         ...
         LSA_HOST_PTR_ARE_EQUAL(pLldpOptions->pElements, LSA_NULL) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_OPTIONS failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NicId);
    }
    else if ((!pLldpOptions->PortId) || (pLldpOptions->PortId > OHA_CFG_MAX_PORTS))
    {
        ...
        pLldpOptions->Count = 0;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_OPTIONS failed. PortID is wrong (PORT-ID: 0x%X)", pLldpOptions->PortId);
    }
             ...
             pLldpOptions->OUI == 0)
    {
        Response = OHA_ERR_PARAM;  /* OUI is wrong */
        pLldpOptions->Count = 0;
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_READ_OPTIONS failed. OUI is wrong (OUI: 0x%X)", pLldpOptions->OUI);
    }
    else
    {
        OHA_LLDP_OPTION_PTR_TYPE pUserList = (pLldpOptions->Count) ? pLldpOptions->pElements : LSA_NULL; /* array (list) of LLDP elements */
        ...
        if (pLldpOptions->Type && pLldpOptions->SubType)
        {
                                                       ...
                                                       &pVariable, &VarLength))
            {
                if (SubType == pLldpOptions->SubType)
                {
                    ...
                    {
                        OHA_MEMSET(pUserList, 0, sizeof(OHA_LLDP_OPTION_TYPE));
					    pUserList->PortId   = pLldpOptions->PortId;
            ...
            OHA_UserRequestFinish(pHDB,pRQB,Response);
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestReadLldpOptions(Response: 0x%X)", Response);
            return; /*  ==========>>>>>>>>>>>>  */
        }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestWriteLldpOption(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE      pHDBEdd;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_WRITE_OPTION failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NicId);
    }
    else if (pLldpOption->PortId > OHA_CFG_MAX_PORTS)
    {
        Response = OHA_ERR_PARAM;  /* Mode is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_WRITE_OPTION failed. PortID is wrong (PORT-ID: 0x%X)", pLldpOption->PortId);
    }
    ...
    {
        Response = OHA_ERR_PARAM;  /* OUI is wrong */
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_WRITE_OPTION failed. OUI is wrong (OUI: 0x%X)", pLldpOption->OUI);
    }
    ...
    {
        Response = OHA_ERR_PARAM;  /* OUI is wrong */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_WRITE_OPTION failed. OUI is not allowed (OUI: 0x%X)", pLldpOption->OUI);
    }
    ...
    if (OHA_LldpIsMib2Object(pLldpOption->Type))
	{
		if (OHA_IsAnyLocalSetMib2Active(NicId)) /* SetMib2Params from any other user running ? */
		{
			...
			OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_WRITE_OPTION failed. Already one User-Request active for this NicId (NIC-ID: 0x%X)",NicId);
		}
	}
            ...
            Difference)
        {
            ...
            if (OHA_LldpIsPortOption(pLldpOption->Type, pLldpOption->PortId))
            {
					 ...
					 pHDBLldp->RequestState == OHA_LLDP_STATE_WF_START)) /* AP01070629, AP01137963 */
                {
                    ...
                    {
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_RequestWriteLldpOption: OHA_LLDPSet failed (Status: 0x%X).", Status);
                    }
                }
            }
            else
			...
			if (OHA_LldpIsMib2Object(pLldpOption->Type))
			{
				...
				{
					Status = OHA_SOCKSetMib2Params(OHA_RQB_GET_HANDLE(pRQB));
				}
				if (Status != OHA_OK)
				{
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_OPC_LLDP_WRITE_OPTION: Setting MIB2-Params failed (Status=%u).", Status);
				}
				else
				{
					/* queue the user-rqb */
					...
					OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestWriteLldpOption: 0x%X)", Response);
					return;
				}
			}
		}
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestWriteLldpOption(Response: 0x%X)", Response);
}
 ...
 *===========================================================================*/
LSA_UINT32 oha_db_count_rem_lldp_elements(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT8                   Type,       /* type of variable, option */
    LSA_UINT32                  OUI         /* OUI, if subtype=127 */
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_count_rem_lldp_elements(PortId: 0x%X, Type: 0x%X, OUI: 0x%X)",
                          PortId, Type, OUI);

    /* ++ check the params ++ check the params ++ */
    if (!Type) /* invalid option */
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_count_rem_lldp_elements(NicId: 0x%X, Count: 0x%X)", NicId, Count);
        return Count;
    }
 ...
 * return value:   LSA_UINT16:      Count
 *===========================================================================*/
LSA_UINT32 oha_db_count_all_rem_lldp_elements(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId
)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestLldpAccess(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    LSA_UINT16              Response = OHA_OK;
	...
	if (!OHA_EDDCheckNICUsed(NICId))     /* EDD System channel is opened */
	{
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_ACCESS_CONTROL failed. Must be an interface channel (NICId: 0x%X)", NICId);
	}
	else if (pLldpAccess->PortId > OHA_CFG_MAX_PORTS)
    {
        Response = OHA_ERR_PARAM;  /* PortId is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_ACCESS_CONTROL failed. PortID is wrong (PORT-ID: 0x%X)", pLldpAccess->PortId);
    }
    else if ( pLldpAccess->Options == 0 && pLldpAccess->PortOptions == 0 )
    {
        ...
        OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP, "OHA_OPC_LLDP_ACCESS_CONTROL failed. No Parameter to write (Options: 0x%X, PortOptions: 0x%X)",
			   ...
			   pLldpAccess->AccessMode == OHA_NO_ACCESS))
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_ACCESS_CONTROL failed. Invalid AccessMode (0x%X)", pLldpAccess->AccessMode);
	}
	else if (pLldpAccess->AccessMode == OHA_QUERY_WRITE && OHA_IS_NULL(pHDBLldp))
	{
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_LLDP_ACCESS_CONTROL failed. No LLDP channel for NicId (0x%X) existing", NICId);
	}
	...
	if(pLldpAccess->Options && pLldpAccess->AccessMode == OHA_QUERY_WRITE)
	{
		...
		if (OHA_IS_NOT_NULL(pHDBLldp->pHDBUserQueryIf)) /* check the if query-user */
		{
			...
			{
				OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Already a SNMP-Query If-User (0x%X) existing!", pHDBLldp->pHDBUserQueryIf);
			    OHA_UserRequestFinish(pHDB,pRQB,Response);
	...
	if(pLldpAccess->PortOptions && pLldpAccess->AccessMode == OHA_QUERY_WRITE)
	{
		...
		if (OHA_IS_NOT_NULL(pHDBLldp->pHDBUserQueryPort)) /* check the port query-user */
		{
			...
			{
				OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Already a SNMP-Query Port-User (0x%X) existing!", pHDBLldp->pHDBUserQueryPort);
			    OHA_UserRequestFinish(pHDB,pRQB,Response);
	...
	if(pLldpAccess->Options)
	{
		if (pLldpAccess->AccessMode == OHA_QUERY_WRITE)
		{
			...
			OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"Set SNMP-Query If-User successful");
	    }
	...
	if(pLldpAccess->PortOptions)
	{
		if (pLldpAccess->AccessMode == OHA_QUERY_WRITE)
		{
			...
			OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"Set SNMP-Query Port-User successful");
	    }
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_MIB_VALUE(
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_MIB_ACCESS(
    LSA_UINT32                  Options,    /* type of variable   */
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_MIB_PORT_VALUE(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
...
)
{
    ...
    if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
    {
		RetVal = OHA_ERR_PARAM;
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_VALUE(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( PortOptions == 0)
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_VALUE (No Parameter to read (PortOptions: 0x%X, pValue: 0x%X)",
							 PortOptions, pValue);
        return (RetVal);
    }
    ...
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_VALUE (No Parameter given (PortOptions: 0x%X, pValue: 0x%X)",
							 PortOptions, pValue);
        return (RetVal);
    }
	...
	switch (PortOptions)
	{
		...
		default:
			RetVal = OHA_ERR_PARAM;
			OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_VALUE (No valid option to read ( NicId: 0x%X, PortOptions: 0x%X)",
								 NicId, PortOptions);
		break;
	}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_MIB_PORT_ACCESS(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT32                  PortOptions,    /* type of variable   */
    OHA_COMMON_MEM_U8_PTR_TYPE  pAccCtrl    /* access control for variable */
)
{
    ...
    if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
    {
		RetVal = OHA_ERR_PARAM;
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_ACCESS(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( PortOptions == 0 )
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_ACCESS (No Parameter to read (PortOptions: 0x%X, pAccCtrl: 0x%X)",
							 PortOptions, pAccCtrl);
        return (RetVal);
    }
    ...
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_ACCESS (No Parameter given (PortOptions: 0x%X, pAccCtrl: 0x%X)",
							 PortOptions, pAccCtrl);
        return (RetVal);
    }
	...
	switch (PortOptions)  /* only one allowed */
	{
		...
		default:
			RetVal = OHA_ERR_PARAM;
			OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_LLDP_MIB_PORT_ACCESS (No valid option to read (PortOptions: 0x%X, pAccCtrl: 0x%X)",
								 PortOptions, pAccCtrl);
		break;
	}
 ...
 * return value:   LSA_UINT16:  LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_LLDP_MIB_VALUE(
    LSA_UINT32                  Options,    /* type of variable   */
 ...
 * return value:   LSA_UINT16:  LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_LLDP_MIB_ACCESS(
    LSA_UINT32                  Options,    /* type of variable   */
 ...
 * return value:   LSA_UINT16:  LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_LLDP_MIB_PORT_VALUE(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT32                  PortOptions,/* type of variable   */
    LSA_UINT32					Value,     /* variable in octets */
...
)
{
    ...
    {
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_VALUE(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( PortOptions == 0 )
    {
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_VALUE (No Parameter to write (PortOptions: 0x%X, Value: 0x%X)",
							 PortOptions, Value);
        return (RetVal);
    }
	...
	{
		/* Parameter Length is wrong */
		OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OUT: OHA_DB_WRITE_LLDP_MIB_PORT_VALUE. Invalid Length (0x%X)", Length);
		return (RetVal);
	}
 ...
 * return value:   LSA_UINT16:  LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT32                  PortOptions,/* type of variable   */
    LSA_UINT8                   AccCtrl    /* access control for variable */
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS(PortId: 0x%X, PortOptions: 0x%X, AccCtrl: 0x%X)",
    ...
    {
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( PortOptions == 0 )
    {
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS (No Parameter to write (PortOptions: 0x%X, AccCtrl: 0x%X)",
							 PortOptions, AccCtrl);
        return (RetVal);
    }
    ...
    {
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS (No valid Parameter for lldpPortConfigTLVsTxEnable (PortOptions: 0x%X, AccCtrl: 0x%X)",
							 PortOptions, AccCtrl);
        return (RetVal);
    }
    ...
    {
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS (No valid Parameter for lldpXdot3PortConfigTLVsTxEnable (PortOptions: 0x%X, AccCtrl: 0x%X)",
							 PortOptions, AccCtrl);
        return (RetVal);
    }
	...
	{
		/* Parameter AccessMode is wrong */
		OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OUT: OHA_DB_WRITE_LLDP_MIB_PORT_ACCESS. Invalid AccessMode (0x%X)", AccCtrl);
		return (RetVal);
	}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_temp_lldp_mib_value(
    LSA_UINT32                  Options,    /* type of variable   */
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_temp_lldp_mib_port_value(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT32                  PortOptions,/* type of variable   */
    OHA_COMMON_MEM_U32_PTR_TYPE *ppValue,   /* variable in octets */
...
)
{
    ...
    if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
    {
		RetVal = OHA_ERR_PARAM;
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_port_value(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( PortOptions == 0)
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_port_value (No Parameter to read (PortOptions: 0x%X, pValue: 0x%X)",
							 PortOptions, ppValue);
        return (RetVal);
    }
    ...
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_port_value (No Parameter given (PortOptions: 0x%X, ppValue: 0x%X)",
							 PortOptions, ppValue);
        return (RetVal);
    }
	...
	switch (PortOptions)
	{
		...
		break;
		case OHA_CTRL_LLDP_TLVS:    /* lldpPortConfigTable "lldpPortConfigTLVsTxEnable"           */
			*ppValue = (OHA_COMMON_MEM_U32_PTR_TYPE)&pPortConfig->TLVsTxEnable.TmpOctStr[0];
			Length   = pPortConfig->TLVsTxEnable.Length;
		break;
		case OHA_CTRL_DOT3_TLVS:    /* lldpXdot3PortConfigTable "lldpXdot3PortConfigTLVsTxEnable" */
			*ppValue = (OHA_COMMON_MEM_U32_PTR_TYPE)&pPortConfig->Dot3TLVsTxEnable.TmpOctStr[0];
			Length   = pPortConfig->Dot3TLVsTxEnable.Length;
		break;
		default:
			RetVal = OHA_ERR_PARAM;
			OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_port_value (No valid option to read ( NicId: 0x%X, PortOptions: 0x%X)",
								 NicId, PortOptions);
		break;
	}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_temp_lldp_mib_if_value(
    LSA_UINT16                  NicId,
...
)
{
    ...
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_if_value (No Parameter to read (IfOptions: 0x%X, pValue: 0x%X)",
							 IfOptions, ppValue);
        return (RetVal);
    }
    ...
    {
		RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_if_value (No Parameter given (IfOptions: 0x%X, ppValue: 0x%X)",
							 IfOptions, ppValue);
        return (RetVal);
    }
	...
	switch (IfOptions)
	{
		...
		default:
			RetVal = OHA_ERR_PARAM;
			OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_temp_lldp_mib_if_value (No valid option to read ( NicId: 0x%X, IfOptions: 0x%X)",
								 NicId, IfOptions);
		break;
	}
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_write_temp_lldp_mib_values(
    LSA_UINT32              Options,
 ...
 *===========================================================================*/
LSA_UINT32 OHA_GetPortQueryOptions(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId
)
{
    ...
    {
        /* Error on getting port. Abort. */
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_GetPortQueryOptions(Invalid PortId:  0x%X)", PortId);
        return (PortQueryOptions);
    }
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_write_temp_lldp_mib_port_values(
    LSA_UINT16                NicId,
    LSA_UINT16                PortId,
    LSA_UINT32                Options,
...
)
{
    ...
    if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
    {
        /* Error on getting port. Abort. */
		RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_write_temp_lldp_mib_port_values(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
    if ( Options == 0 || OHA_IS_NULL(pConfig))
    {
		RetVal = LSA_RET_ERR_PARAM;
        OHA_PROGRAM_TRACE_02(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_write_temp_lldp_mib_port_values (No Parameter given (Options: 0x%X, pConfig: 0x%X)",
							 Options, pConfig);
        return (RetVal);
    }
	...
	{
		pDbConfig->TLVsTxEnable.TmpOctStr[0] = pConfig->TLVsTxEnable;
	}
	if (Options & OHA_CTRL_DOT3_TLVS)
	{
		pDbConfig->Dot3TLVsTxEnable.TmpOctStr[0] = pConfig->Dot3TLVsTxEnable;
	}
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_PORT_ID(
    LSA_UINT16  NicId,
    LSA_UINT16  PortId,
...
)
{
	...
	if (!(PortId && PortId <= OHA_CFG_MAX_PORTS))
	{
		OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_DB_READ_LLDP_PORT_ID(): Invalid PortId (%Xh)",PortId);
		OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		return(0);
	}
    ...
    if (OHA_ERR_PARAM == oha_check_port_name(Len, pVar) ? LSA_TRUE : LSA_FALSE)
    {
        *NamePtrPtr = LSA_NULL;
 ...
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_LLDP_CHASSIS_ID(
    LSA_UINT16                  NicId,
...
)
{
	...
	if (Len != 0 && OHA_IS_NOT_NULL(pVar))	/* station name present */
	{
		...
		if (OHA_OK == oha_check_station_name(Len, pVar,
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_rem_chassis_name(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,      /* index + 1 to RemoteSender[] - for SNMP */
		...
		*pSubType == OHA_PORT_ID_LOCALLY_ASSIGNED)
	{
		...
		for(Len=0; Len < PortLen; Len++, pStr++)
		{
			if(*pStr == '.')
			{
				...
				if (OHA_OK == oha_check_port_name(Len, pPortID) ? LSA_TRUE : LSA_FALSE)
				{
					Len++;
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 oha_db_read_rem_port_name(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  SenderId,   /* index + 1 to RemoteSender[] - for SNMP */
...
)
{
	...
	if (OHA_IS_NOT_NULL(*ppPortName) && (*pPortLen <= OHA_MAX_PORT_STRLEN))
	{
		...
		{
			OHA_PROGRAM_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "~~~No (valid) LLDP_PortID present PortId: 0x%X, SenderId: 0x%X, PortID-Subtype: 0x%X",
		...
		for(Len=0; Len < *pPortLen; Len++, pStr++)
		{
			...
			{
				/* check, if PortID is valid */
				if (OHA_OK == oha_check_port_name(Len, *ppPortName) ? LSA_TRUE : LSA_FALSE)
				{
					*pPortLen = Len;
					break;
				}
