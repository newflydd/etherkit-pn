...
// *
// *-----------------------------------------------------------------
void pnpb_sub_exp_init()
{
	OsMemSet (&PnpbExp[0], 0, sizeof (PnpbExp));
}
...
#endif
{
	...
	if (MoreFollows == PNIO_FALSE)
	{
		...
		#if 0
			// * ------------------------------------------------------------
			// *  if requested from application, check the submodule
			// *  configuration and call PNIO_cbf_check_ind() in case of
			// *  discrepance (to adjust element isWrongSubmodule in PnpbExp)
			// * ------------------------------------------------------------
			if (FlagDoCheckInd == DO_EXEC_CHECK_IND)
			{
				DoCheckInd (ArInd);
			}
		#endif
...
PNIO_BOOL pnpb_remove_all_sub  (PNIO_UINT32 ArInd)
{
    ...
    OsMemSet (&PnpbExp[ArInd], 0, sizeof (PNIO_EXP) );
