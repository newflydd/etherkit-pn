...
static LSA_BOOL   sock_mib2_cmp_system_params(SOCK_MIB2_SYSGROUP_DATA_CONST_PTR_TYPE p_mib2_sys);
static LSA_UINT16 sock_mib2_copy_params_to_local_storage(SOCK_MIB2_SYSGROUP_DATA_CONST_PTR_TYPE p_mib2_sys);
static LSA_VOID   sock_mib2_mov_system_params_to_rqb(SOCK_MIB2_SYSGROUP_DATA_PTR_TYPE p_mib2_sys);
...
sock_mib2_get_ifindex(
	...
	ret = (TCIP_OK == tcip_get_mib2_ifindex(gif->InterfaceID, gif->PortID, &gif->IfIndex ) ? LSA_TRUE : LSA_FALSE);
#else
...
#if (SOCK_CFG_USE_TCIP == 1)
	ret = (TCIP_OK == tcip_set_mib2_ifgroup_data(ifd->InterfaceID, ifd->PortID, ifd->ifDescr.pName, ifd->ifDescr.Length ) ? LSA_TRUE : LSA_FALSE);
#else
...
sock_mib2_set_system_params(
	...
	if (sgd->sysName.Remanent != SOCK_NAME_REMANENT_NO && sgd->sysName.Remanent != SOCK_NAME_REMANENT_YES)
	{
		return SOCK_RSP_ERR_PARAM;
	}

	if (sgd->sysDescr.Remanent != SOCK_NAME_REMANENT_NO && sgd->sysDescr.Remanent != SOCK_NAME_REMANENT_YES)
	{
		return SOCK_RSP_ERR_PARAM;
	}

	if (sgd->sysContact.Remanent != SOCK_NAME_REMANENT_NO && sgd->sysContact.Remanent != SOCK_NAME_REMANENT_YES)
	{
		return SOCK_RSP_ERR_PARAM;
	}

	if (sgd->sysLocation.Remanent != SOCK_NAME_REMANENT_NO && sgd->sysLocation.Remanent != SOCK_NAME_REMANENT_YES)
	{
		return SOCK_RSP_ERR_PARAM;
	}

#if (SOCK_CFG_USE_TCIP == 1)
	ret = (TCIP_OK == tcip_set_mib2_sysgroup_data( sgd->sysName.pName, sgd->sysName.Length,
													 sgd->sysDescr.pName, sgd->sysDescr.Length,
													 sgd->sysContact.pName, sgd->sysContact.Length,
													 sgd->sysLocation.pName, sgd->sysLocation.Length  ) ? LSA_TRUE : LSA_FALSE);
#else
...
sock_mib2_copy_params_to_local_storage(
	SOCK_MIB2_SYSGROUP_DATA_CONST_PTR_TYPE p_mib2_sys
) {
