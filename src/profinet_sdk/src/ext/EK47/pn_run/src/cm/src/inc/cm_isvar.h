...
/*** defines for argr->cm.sv.ar_flags ***/
...
#define CM_SV_AR_FL_00001000          0x00001000 /* not used anymore, TIA 1475666 */
...
#define CM_SV_AR_FL_PRM_BACKUP        0x00200000 /* SR-AR, PRM sequence started in "backup" (TIA 2329293) */
...
#define CM_SV_AR_FL_WF_PRM_BEGIN      0x10000000
#define CM_SV_AR_FL_PDEV_INVALID      0x20000000 /* the AR's PDEV-parameterization is not active (if no PDIRData then must not activate RTC3 prov/cons, etc.) */
...
/*** defines for argr->cm.sv.pd_flags ***/

#define CM_SV_AR_PDFL_LOCAL_PDEV_INTERFACE   0x00000001 /* the AR contains the local pdev-interface */
#define CM_SV_AR_PDFL_REMOTE_PDEV_INTERFACE  0x00000002 /* the AR contains the remote pdev-interface */
#define CM_SV_AR_PDFL_REMOTE_PDEV_PRM_FAULT  0x00000004 /* result of parameterization of remote pdev */
#define CM_SV_AR_PDFL_PBE_PDEV_IF_LOCAL      0x00000008 /* the PRM-sequence contains the local pdev-interface */
#define CM_SV_AR_PDFL_PBE_PDEV_IF_REMOTE     0x00000010 /* the PRM-sequence contains the remote pdev-interface */
...
typedef struct cm_sv_ar_rec_tag { /* record state-machine */
	...
	LSA_UINT32 coc_offs; /* != 0 if is-CombinedObjectContainer (TIA 1092258) */
	LSA_UINT8* coc_data;
	LSA_UINT32 coc_size;

	/*CM_SV_ITERATOR_TYPE*/struct cm_sv_iterator_tag *iter_ptr; /* an iterator for the F8xx records */
	...
	LSA_BOOL is_rerun; /* introduced for "pdev_crc64 and rerun" (AP01290734); note: TIA 1092258 obsoleted AP01238541 ("pdev_crc64") */
...
#define CM_SV_AR_ALMI_IDLE                     0x0000 /* idle, all bits 0 */
#define CM_SV_AR_ALMI_ALARM_REQ                0x0001 /* alarm.REQ made (not idle) */
#define CM_SV_AR_ALMI_ALARM_CNF                0x0002 /* alarm.CNF seen */
#define CM_SV_AR_ALMI_ALARM_ACK                0x0004 /* alarm-ack.IND seen */
#define CM_SV_AR_ALMI_DO_SET2UNK               0x0008 /* do set2unk after alarm-ack.IND (plug, release or return-of-submodule alarm) */
#define CM_SV_AR_ALMI_IS_PLG_REL               0x0010 /* modifier of DO_SET2UNK: plug or release alarm (expecting prm-sequence) */
#define CM_SV_AR_ALMI_RTC3_ACTION              0x0020 /* rtc3-action running */
#define CM_SV_AR_ALMI_NWDT_ACTION              0x0040 /* new-data-action running */
#define CM_SV_AR_ALMI_DO_ARDY                  0x0080 /* call cm_sv_rpc_appl_ready_request() on alarm-ack.ind */
#define CM_SV_AR_ALMI_WF_R1_ALARM_BEGIN_POSTED 0x0100 /* R1 alarm begin pending */
#define CM_SV_AR_ALMI_WF_R1_ALARM_END_POSTED   0x0200 /* R1 alarm end pending */

typedef struct cm_sv_ar_almi_tag { /* alarm initiator */

	LSA_UINT16 sequence_number;

	LSA_UINT16 flags; /* see CM_SV_AR_ALMI_IDLE, ... */
	...
	CM_SV_AR_ALMI_R1B_TYPE r1b;

} CM_SV_AR_ALMI_TYPE;

typedef CM_SV_AR_ALMI_TYPE *CM_SV_AR_ALMI_PTR_TYPE;

typedef CM_SV_AR_ALMI_TYPE const *CM_SV_AR_ALMI_CONST_PTR_TYPE;
/*---------------------------------------------------------------------------*/
...
typedef struct cm_sv_ar_almr_tag { /* alarm responder */
	...
	CM_ACP_LOWER_RQB_PTR_TYPE acp_ind; /* the stored acp-alarm.IND */
...
typedef CM_SV_AR_ALMR_TYPE *CM_SV_AR_ALMR_PTR_TYPE;
...
typedef struct cm_sv_argr_tag {
	...
	/*CM_SV_ARSET_PTR_TYPE*/ struct cm_sv_arset_tag *arset; /* the AR-set if CM_AR_TYPE_SINGLE_SYSRED, else LSA_NULL */
	...
	LSA_UINT32 pd_flags; /* see CM_SV_AR_PDFL_LOCAL_PDEV_INTERFACE, ... */

	LSA_UINT32 r1_flags; /* see CM_SV_AR_R1FL_ORIGINAL, ... */
	...
	LSA_UINT16 nr_of_remote_pdev_submodules; /* R1 */

	/*CM_AR_GRAPH_SUBMODULE_PTR_TYPE*/ struct cm_ar_graph_submodule_tag * local_pdev_if;

	/*CM_AR_GRAPH_SUBMODULE_PTR_TYPE*/ struct cm_ar_graph_submodule_tag * remote_pdev_if;
	...
	LSA_BOOL set_mrp_off; /* AP00824058 */

	LSA_UINT16 net_data_length; /* TIA 1453731 */

	/***/

	CM_RPC_LOWER_RQB_PTR_TYPE curr_rpc_ind; /* current rpc indication, here until event-response */

	LSA_BOOL curr_rpc_ind_is_parked; /* TIA 1518690 */
	...
	CM_UPPER_SV_AR_RIR_PTR_TYPE ar_rir_mem; /* see cm_sv_ar_rir_alloc() */

	CM_UPPER_RQB_PTR_TYPE pdevprm_rdwr_rqb; /* for pdev-read or pdev-write (TIA 1193013) */

	CM_UPPER_RQB_PTR_TYPE pdev_begin_rqb;

	CM_UPPER_RQB_PTR_TYPE pdev_end_rqb; /* for pdev-prmend (AP00640250) */

	CM_PD_AR_PRM_END_TYPE prm_end_args; /* "pre-allocated" parameters for pdev_end_rqb */
	...
	CM_UPPER_RQB_PTR_TYPE plug_group_alarm_rqb; /* pre-allocated RQB for plug-group alarms (TIA 1327527) */

	CM_SV_ARGR_R1B_TYPE r1b; //R1 marker (AR-specific data for R1)
...
#define CM_SV_AR_SMFL_CIR          0x0800 /* CIR (TIA 906437) */
#define CM_SV_AR_SMFL_PULLPDEV     0x1000 /* Pull PDev (TIA 954400) */
#define CM_SV_AR_SMFL_R1_PP        0x2000 /* R1 (TIA 1564264) */
