...
#define DCP_SWAP16(C)  ((LSA_UINT16)((((LSA_UINT16)(C) & 0xFF00) >> 8) | \
                                     (((LSA_UINT16)(C) & 0x00FF) << 8)))

/* swaps a byte pattern of 01020304 to 04030201 */
#define DCP_SWAP32(C)  ((LSA_UINT32)((((LSA_UINT32)(C) & 0xFF000000) >> 24) | \
...
#define DCP_IS_REQUEST(PduPtr) \
    	((!LSA_HOST_PTR_ARE_EQUAL(LSA_NULL,PduPtr)) && \
...
#define DCP_IS_VALID_PTR(ptr)  if(LSA_HOST_PTR_ARE_EQUAL(LSA_NULL,(ptr)))\
                               {\
                                   dcp_ErrorInternal(DCP_MODULE_ID, __LINE__, DCP_ERR_FATAL, (ptr), 0);\
                               }
#else
#define DCP_IS_VALID_PTR(ptr) {}
#endif
...
typedef struct _DCP_CLIENT_DATA
{
	...
	DCP_TIMER_PTR_TYPE			RetryTimerPtr;	//1737982
	LSA_UINT16					Xid;
...
typedef struct _DCP_CANCEL_DATA
{
	DCP_UPPER_QUEUE				Queue;
	LSA_UINT32					Count;
...
typedef struct _DCP_CHANNEL_LOWER
{
	...
	LSA_UINT8					ChCount;
	LSA_HANDLE_TYPE				Handle;
	EDD_HANDLE_LOWER_TYPE		HandleLower; /* TIA 2709127 */
	LSA_INT						RecvRblAct;
...
typedef struct _DCP_CHANNEL
{
	...
	LSA_BOOL										ServerSetWait;			//1126404
...
*/
#define DCP_DS_SET_ADDR(_DstPtr, _SrcPtr, L)		\
		{LSA_INT I; for (I = 0; I < (L); I++)		\
...
DCP_TIMER_PTR_TYPE  DCP_LOCAL_FCT_ATTR dcp_TimerFindRqb(DCP_CHA_PTR,DCP_UPPER_RQB_PTR_TYPE,DCP_TIMER_QUEUE*);
...
DCP_TIMER_PTR_TYPE	DCP_LOCAL_FCT_ATTR dcp_TimerTabFindAddr(DCP_CHA_PTR,LSA_UINT32,const DCP_MAC_ADR);
DCP_TIMER_PTR_TYPE  DCP_LOCAL_FCT_ATTR dcp_TimerTabFindRqb(DCP_CHA_PTR,DCP_UPPER_RQB_PTR_TYPE);
...
*/
LSA_INT		DCP_LOCAL_FCT_ATTR dcp_RspHdrSet(DCP_CHA_PTR, DCP_LOWER_RQB_PTR_TYPE, const DCP_USR_BYTE_PTR, LSA_UINT8, LSA_UINT32, const DCP_USR_BYTE_PTR, LSA_INT);
