...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_TimerMgmSetup(LSA_VOID)
{
    LSA_UINT32 i;
    ...
    OHA_LOCAL_MEM_U16_PTR_TYPE     pOHATimerID)
{
	...
	OHA_ASSERT_NULL_PTR(pOHATimerID);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_TimerFree(LSA_UINT16 OHATimerID)
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_TimerRQBCheckFree(
    LSA_UINT16 OHATimerID)
    ...
    LSA_USER_ID_TYPE  user_id)
{
    ...
    if ( OHATimerID < OHA_CFG_MAX_TIMER )
    {
            ...
            ( pTimer->TimerID == timer_id ))
        {
            ...
            if ( pTimer->Cbf != 0 )  /* lint warning 611 “suspicious cast” */
            {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestTimer(OHA_UPPER_RQB_PTR_TYPE pRQB)
{
    LSA_UINT32    i;
    LSA_UINT16    j;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestCyclicTimer(OHA_UPPER_RQB_PTR_TYPE pRQB)
{
    ...
    ++OHA_GET_GLOB_PTR()->CyclicTimerCnt; /* TFS 1217823 */
    ...
    for (i=0; i<OHA_CFG_MAX_CHANNELS; i++, pHDB++)
    {
        ...
        if (OHA_IS_NULL(pHDB->Params.pDetail))
        {
            continue;
        }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestCyclicCheck(OHA_UPPER_RQB_PTR_TYPE pRQB)
{
                ...
                /* AP01522573: still one check: IP */
                {
                    ...
                    if ( pInfo->State == OHA_CHECK_STATE_START )
                    {
                        ...
                        if ( pInfo->NextSend == TimerCnt )
                        {
                            ...
                            pInfo->NextSend += OHA_CHECK_GRID;

                            if ( ! pInfo->Underway )
                            {
                                OHA_NARE_RQB_PTR_TYPE   pCheckRQB = pInfo->pCheckRQB;

                                /* only send if not active */
                                if ( OHA_IS_NOT_NULL(pCheckRQB) )
                                {
                                    LSA_UINT16              Orginator;
                                    LSA_OPCODE_TYPE         ChecxOpcode;
                                    ...
                                    Orginator = OHA_NARE_RQB_GET_USERID_UVAR16(pCheckRQB);
                                    ChecxOpcode = OHA_NARE_RQB_GET_OPCODE(pCheckRQB);
                                    ...
                                    OHA_NARE_RQB_SET_HANDLE(pCheckRQB,pHDBNare->NareHandle);

                                    switch (ChecxOpcode)
                                    {
                                        case NARE_OPC_TEST_IP:
											/* AP00842368 Checking the IP-address by NARE (Gratuitous ARP): Sender IP must be 0.0.0.0 */
											OHA_MEMCPY (pCheckRQB->args.TestIP.LocalIPAddr, (OHA_COMMON_MEM_PTR_TYPE)OHA_ZERO_IP, NARE_IP_ADDR_SIZE);
                                            break;
                                        default:
                                            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_RequestCyclicCheck(): Wrong NARE-Opcode (%u)", ChecxOpcode);
                                            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                                            break;
                                    }
                                    ...
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_LOW
                                                         , "Check Request for channel %u, opcode %u", (i+1), ChecxOpcode);
                                    OHA_NARE_REQUEST_LOWER(pCheckRQB,pHDBNare->Params.pSys);
                                }
                            }
                            else
                            {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CancelPtcpTimer(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
...
/*+               and the timer is started -> stop it.                      +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CancelCyclicTimer(
    OHA_HDB_USER_PTR_TYPE        pHDBUser,
    OHA_LOCAL_MEM_BOOL_PTR_TYPE  pFinish
)
{
