...
// *-----------------------------------------------------------------
	...
	#define PNPB_IO_ERTEC400_DEFINE_LOCK_OFS(_lock_ofs, _lock_len) 											\
		PNIO_UINT32 _lock_ofs = PNPB_IO_ERTEC400_CNS_INVALID_OFFSET;            /* not locked */ 			\
		PNIO_UINT32 _lock_len = PNPB_IO_ERTEC400_CNS_MAX_CONSISTENCY_BLOCK_LEN;

	#define PNPB_IO_ERTEC400_CNS_INVALID_OFFSET               0xFFFFFFFF     // invalid buffer offset
	#define PNPB_IO_ERTEC400_CNS_MAX_CONSISTENCY_BLOCK_LEN    255            // max. length of a consistent block lock for ERTEC400

	#define PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(_pdata, _ar_ind, _lock_ofs, _lock_len, _data_ofs, _data_len)             \
	{                                                                                                           \
		if (   (PNPB_IO_ERTEC400_CNS_INVALID_OFFSET == _lock_ofs)   /* buffer not locked */                     \
			|| (   (_data_ofs < _lock_ofs)      /* and new data is outside the locked range */                  \
				|| ((_data_ofs + _data_len) > (_lock_ofs + _lock_len))))                                        \
		{                                                                                                       \
			if (PNPB_IO_ERTEC400_CNS_INVALID_OFFSET != _lock_ofs)        /* buffer is locked */                 \
			{                                                                                                   \
				PnpbCopyRecReadIoData(_pdata, _ar_ind, _lock_ofs, _lock_len);									\
				PnpbProvUnlock(_ar_ind);                                                                        \
			}                                                                                                   \
			if (PNPB_IO_ERTEC400_CNS_INVALID_OFFSET != _data_ofs)        /* new range to be locked? */          \
			{                                                                                                   \
				/* is max consistency block length exceeding size of IOCR? */                                   \
				/* _lock_len = ((_data_ofs + PNPB_IO_ERTEC400_CNS_MAX_CONSISTENCY_BLOCK_LEN) > _iocr_len) ? 	\
						(_iocr_len - _data_ofs) : PNPB_IO_ERTEC400_CNS_MAX_CONSISTENCY_BLOCK_LEN;	*/			\
				_lock_len = _data_len;                                                                          \
				/* lock new consistency block */                                                                \
				PnpbProvLock(&_pdata, _ar_ind, (PNIO_UINT16)_data_ofs, _lock_len);                              \
				if (NULL == _pdata)                                                                             \
				{                                                                                               \
...
#else
	#define PNPB_IO_ERTEC400_DEFINE_LOCK_OFS(_lock_ofs, _lock_len)
	#define PNPB_IO_ERTEC400_CNS_INVALID_OFFSET
	#define PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(_pdata, _ar_ind, _lock_ofs, _lock_len, _data_ofs, _data_len)
#endif
	...
	switch (Status)
	 {
		...
		default:
		 ...
		 if (Rec8029_Req[ArInd].ExecConsRq  == IO_READ_STATE_EXEC)
		 {
			 if (Status == PNIO_OK)
			 {
				 OsMemCpy  (Rec8029_Req[ArInd].pDstIoDat,
...
PNPB_CODE_FAST PNIO_UINT32 PnpbProvLock(PNIO_UINT8** ppDatIn, PNIO_UINT16 ArInd, PNIO_UINT16 dataOffset, PNIO_UINT16 dataLength)
{
	*ppDatIn = NULL;
	...
	#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB400)
		iom_provider_lock_section((LSA_VOID**)ppDatIn, ArInd, dataOffset, dataLength);
	#else
...
PNIO_UINT32 PnpbCopyRecReadIoData(PNIO_UINT8* pDatIn, PNIO_UINT16 ArInd, PNIO_UINT16 dataOffset, PNIO_UINT16 dataLength)
{
	...
	if (pDatIn)
	{
			...
			{
				#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB400)
					if ( (Rec8028_Req[ArInd].SrcIoDatOffs >= dataOffset) &&
						((Rec8028_Req[ArInd].SrcIoDatOffs + Rec8028_Req[ArInd].IoDatLen -1) <= (dataOffset + dataLength-1)))
					{
						OsMemCpy  (Rec8028_Req[ArInd].pDstIoDat,
								pDatIn + Rec8028_Req[ArInd].SrcIoDatOffs,
								Rec8028_Req[ArInd].IoDatLen);
					}
					if ((Rec8028_Req[ArInd].SrcIopsOffs >= dataOffset) &&
						(Rec8028_Req[ArInd].SrcIopsOffs <= (dataOffset + dataLength-1)))
					{
						*(Rec8028_Req[ArInd].pDstIops) = *(pDatIn + Rec8028_Req[ArInd].SrcIopsOffs);
						// both InputData and IPS are valid here ... Rec8028 read can be finished
			...
			{
				#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB400)
					if ((Rec8029_Req[ArInd].SrcIocsOffs  >= dataOffset) &&
						(Rec8029_Req[ArInd].SrcIocsOffs  <= (dataOffset + dataLength-1)))
					{
...
// *        the physical output
// *------------------------------------------------------------------------
extern LSA_VOID EDDP_LL_DatXchOutput(LSA_UINT8 *pAddr, PNIO_UINT32 ArNum);
PNPB_CODE_FAST PNIO_UINT32 pnpb_initiate_data_read  (PNIO_UINT32   DevHndl)
{
	...
	for (ArInd = 0; ArInd < IOD_CFG_NUMOF_AR; ArInd++)
	{
		if (PnpbExp[ArInd].FirstParamEndValid)
		{
			...
			EDDP_LL_DatXchOutput(pData,ArInd);
...
// *        the physical output
// *------------------------------------------------------------------------
extern LSA_VOID EDDP_LL_DatXchInput(LSA_UINT8 *pAddr, PNIO_UINT32 ArNum);
extern LSA_VOID EDDP_LL_DatXchInputAPDU(PNIO_UINT32 ArNum);
PNPB_CODE_FAST PNIO_UINT32 pnpb_initiate_data_write (PNIO_UINT32   DevHndl)
{
	...
	PNPB_IO_ERTEC400_DEFINE_LOCK_OFS(LockOfs, LockLen);
		...
		if (PnpbExp[ArInd].Rdy4InpUpdateValid)
		{
			// **** get pointer to IOCR buffer ***
				...
				if (PnpbExp[ArInd].IocrLen < 44)
					OsMemSet (pData + PnpbExp[ArInd].IocrLen, 0, 44 - PnpbExp[ArInd].IocrLen);
			#endif
			...
			for (i = 0; i < PnpbExp[ArInd].NumOfPluggedSub; i++)
			{
				...
				if (pExpSub->OwnSessionKey && pExpSub->isPlugged)
				{
					...
					{   // ** submod has no IO data **
						PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, pExpSub->In.iops_offset, 1);
					...
					else
					{
						...
						{  // ** submod has input data **
							PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, pExpSub->In.data_offset, pExpSub->In.data_length+1);
						   // *** update (input-)provider state in input frame ***
							if ((!pExpSub->IsWrongSubmod) && pExpSub->ParamEndValid)
							{
#if(1 == IOD_USED_WITH_XHIF_HOST)
						...
						{  // ** submod has out put data **
							PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, pExpSub->Out.iocs_offset, 1);
							if ((!pExpSub->IsWrongSubmod) && pExpSub->ParamEndValid)
				...
				else // submod not owned or not plugged
				{
					...
					else
					{
						...
						{  // ** submod has input data **
							PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, pExpSub->In.data_offset, pExpSub->In.data_length+1);
							OsMemSet((void*)(pData + pExpSub->In.data_offset), 0x00, pExpSub->In.data_length);
							*(pData + pExpSub->In.iops_offset) = PNIO_S_BAD;
						}
						if (pExpSub->IoProp & PNIO_SUB_PROP_OUT)
						{  // ** submod has out put data **
			...
			EDDP_LL_DatXchInput(pData, ArInd);
			// **** free pointer to IOCR buffer ***
			#if (!(PNIOD_PLATFORM & PNIOD_PLATFORM_EB400))
			PnpbCopyRecReadIoData(pData, ArInd, 0, 0xFFFF);
			PnpbProvUnlock(ArInd);
			#endif
			PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, PNPB_IO_ERTEC400_CNS_INVALID_OFFSET, 0);

			PnpbExp[ArInd].IoUpdatePending = PNIO_FALSE;
		}
		EDDP_LL_DatXchInputAPDU(ArInd);
	}
	...
	for (ArInd = 0; ArInd < IOD_CFG_NUMOF_AR; ArInd++)
	{
		...
		if((pnpb_ar_prm_ok__(ArInd, Pnpb.ArState)) && (PnpbExp[ArInd].FirstParamEndValid)) 
		{
            ...
            if( PNIO_AR_TYPE_SINGLE_SYSRED == PnpbExp[ArInd].ArType )
			{
				...
				ArInd_other = PNPB_AR_OTHER(ArInd);
			    ...
			    if((*pTmpApduStat & REDUNDANT_PRIMARY_STATUS_BIT) && (PNPB_IO_DATA_STATE_P_B_BACKUP == pnpb_data.io.s2.primary_backup_state[ArInd]))
			    {
					      ...
					      (pnpb_data.io.s2.state[ArSet] == PNPB_S2_STATE_HOLD_O_DATA)
                    {
                	    ...
                	    PNPB_SYS_TRACE_00(LSA_TRACE_LEVEL_CHAT, "pnpb_initiate_s2_data_read rdht stopped");
						    LSAS_STOP_TIMER(&retval, pnpb_data.timer[ArSet].ident, pnpb_data.init.component_id);
                        ...
                        switch(retval)
                        {
                            ...
                            {
							    LSAS_RESET_TIMER(&retval, pnpb_data.timer[ArSet].ident, pnpb_data.init.component_id);
                                break;
                            }
                            default:
                            {
                    		     PnpbSetLastError (PNPB_ERR_TIMER_BAD_RETURN_VAL);
                    		     PNPB_SYS_TRACE_01(LSA_TRACE_LEVEL_ERROR, "bad return value from LSAS_STOP_TIMER", retval);
                    		     OsExitX (OS_MUTEX_PNPB_IO);
                    		     return (PNIO_NOT_OK);
                ...
                else if( !(*pTmpApduStat & REDUNDANT_PRIMARY_STATUS_BIT) && (PNPB_IO_DATA_STATE_P_B_PRIMARY == pnpb_data.io.s2.primary_backup_state[ArInd]) )
                {
                      ...
                      )
                    {
                        ...
                        /* start rdht timer */
						    LSAS_START_TIMER(&ret_val, pnpb_data.timer[ArSet].ident, user_id, pnpb_data.io.s2.rdht[ArSet], pnpb_data.init.component_id);
                        if( LSA_RET_ERR_PARAM == ret_val)
                        {
						     PnpbSetLastError (PNPB_ERR_TIMER_BAD_RETURN_VAL);
						     PNPB_SYS_TRACE_01(LSA_TRACE_LEVEL_ERROR, "bad return value from LSAS_START_TIMER", ret_val);
						     OsExitX (OS_MUTEX_PNPB_IO);
						     return (PNIO_NOT_OK);
                        }
...
PNPB_CODE_FAST PNIO_UINT32 pnpb_initiate_s2_data_write (PNIO_UINT32   DevHndl)
{
	...
	for (ArInd = 0; ArInd < IOD_CFG_NUMOF_AR; ArInd++)
	{
		if ( (PnpbExp[ArInd].Rdy4InpUpdateValid) )
		{
			...
			for (i = 0; i < PnpbExp[ArInd].NumOfPluggedSub; i++)
			{
				...
				if( PNIO_AR_TYPE_SINGLE_SYSRED == PnpbExp[ArInd].ArType )
				{
					...
					else	/*not owned by this ar or not plugged*/
					{
						...
						{
							PNPB_IO_ERTEC400_PROVIDER_LOCK_UNLOCK(pData, ArInd, LockOfs, LockLen, pExpSub->In.data_offset, pExpSub->In.data_length);
							OsMemSet((void*)(pData + pExpSub->In.data_offset), 0x00, pExpSub->In.data_length);
						}
						#endif
					}
			...
			#if ( ( PNIOD_PLATFORM & PNIOD_PLATFORM_EB400 ) && ( 1 == IOD_INCLUDE_REC8028_8029 ) )
				if (Rec8028_Req[ArInd].ExecProvRq  == IO_READ_STATE_EXEC)
				{
					Rec8028_Req[ArInd].ExecProvRq = IO_READ_STATE_FINISHED;
				}
...
void    pnpb_io_s2_ar_set_trigger_req    (Unsigned32 ar_idx)
{
    ...
    {
        pnpb_write_event_to_pndv(PNDV_EV_TO_PNDV_SR_EDGE_IND, ar_idx, 1, NULL);
        PNPB_TRIGGER_PNDV ();

    }
    else if(pnpb_data.io.s2.primary_backup_state[ar_idx] == PNPB_IO_DATA_STATE_P_B_BACKUP_PROCEEDING)
    {
        pnpb_write_event_to_pndv(PNDV_EV_TO_PNDV_SR_EDGE_IND, ar_idx, 0, NULL);
        PNPB_TRIGGER_PNDV ();
    }
}
...
void    pnpb_rdht_timeout    (PNIO_UINT32 ArSet)
{
    ...
    pnpb_data.timer[ArSet].rqb_in_use = FALSE;
...
void    pnpb_trigger_rdht_timeout (LSA_UINT16 timer_id, LSA_USER_ID_TYPE user_id)
{
    ...
    if(ArSet < PNPB_MAX_S2_AR_SETS)
    {
        ...
        pnpb_data.timer[ArSet].rqb_in_use = TRUE;
    }
}
...
PNIO_UINT8    pnpb_io_s2_ar_release (Unsigned32 ar_idx)
{
    ...
    Unsigned32 idx_other_ar = PNPB_AR_OTHER(ar_idx);
      ...
      )
    {
        ...
        {
        	if( (IOD_CFG_NUMOF_AR + 1) > idx_other_ar )
        	{
	...
	if(idx_other_ar <= IOD_CFG_NUMOF_AR)
	{
        if((IOD_CFG_NUMOF_AR == idx_other_ar) || (Pnpb.ArState[idx_other_ar] == PNPB_AR_OFFLINE))
        {
            ...
            switch(retval)
            {
                ...
                {
                     PnpbSetLastError (PNPB_ERR_TIMER_BAD_RETURN_VAL);
                     PNPB_SYS_TRACE_01(LSA_TRACE_LEVEL_ERROR, "bad return value from LSAS_STOP_TIMER in AR release", retval);
                     OsExitX (OS_MUTEX_PNPB_IO);
                     return (PNIO_NOT_OK);
                     break;
				}
						   ...
						   PNIO_UINT8  Iops)
{
	...
	{
		OsMemSet (&Rec8028_Req[0], 0, sizeof (Rec8028_Req));
		OsMemSet (&Rec8029_Req[0], 0, sizeof (Rec8029_Req));
#ifdef PNPB_SYSTEM_REDUNDANCY
		int i;
									 ...
									 PNIO_UINT8*    pIocsBuf)
	{
		...
		{
			OsWait_ms (READ_IO_POLLING_TIME_MS); // we poll here to avoid semaphore handling in time critical cycleIO-task
		}
