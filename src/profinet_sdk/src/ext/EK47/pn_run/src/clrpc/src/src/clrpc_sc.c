	...
	)
{
	...
	LSA_UINT8 flags;
	LSA_INT32 len;
	CLRPC_UPPER_SV_CALL_PTR_TYPE up_call = scall->upper->args.sv.call;
	...
	len   = (LSA_INT32)(up_call->ndr_len - scall->send.frag_curr_fragnum * CLRPC_MUST_RECV_BODY_SIZE);
	...
	if (len < 0)
	{
		CLRPC_FATAL1 ((LSA_UINT32)len);	/* wrong length */
	}
	...
	CLRPC_MEMCPY (body, up_call->ndr_data + scall->send.frag_curr_fragnum * CLRPC_MUST_RECV_BODY_SIZE, (LSA_UINT)len);
	...
	)
{
	...
	if (CLRPC_PTR_IS_NOT_NULL (scall->upper))
	{
		clrpc_pkt_init (pkt_send, scall->send.ptype,
			...
			scall->upper->args.sv.call->opnum, &scall->upper->args.sv.call->object_id,
	...
	case CLRPC_PT_REQUEST:	/* client -> server */
		...
		scall->send.serial   += 1;
		pkt_send->server_boot = scall->server->server_boot;
...
{
	CLRPC_SV_TRACE_03(scall->trace_idx, LSA_TRACE_LEVEL_CHAT
		, "cid(%u) send.ptype = PT_FAULT state(%u) nca_error(%u)"
		, scall->id
		, scall->state
		, nca_error
		);

	scall->send.ptype              = CLRPC_PT_FAULT;
...
{
	CLRPC_SV_TRACE_03(scall->trace_idx, LSA_TRACE_LEVEL_CHAT
		, "cid(%u) send.ptype = PT_REJECT state(%u) nca_error(%u)"
		, scall->id
		, scall->state
		, nca_error
		);

	scall->send.ptype = CLRPC_PT_REJECT;
	scall->send.reject_fault_error = nca_error;
	...
	)
{
	...
	if (scall->way.state == CLRPC_WAY_ST_UNKNOWN)
	{
		...
		if (cnt >= 10)
		{
			CLRPC_FATAL1 (cnt);	/* see AP00699788 */
			return;
		}
...
clrpc_if_obj_opnum_count_ok (
	CLRPC_IF_OBJECT_CONST_PTR_TYPE if_obj,
...
clrpc_sc_opnum_count_decr (
	CLRPC_SCALL_CONST_PTR_TYPE scall,
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
	)
{
	...
	)
{
	...
	if (scall->state == CLRPC_CST_INIT)
	{
		...
		if (pkt->opnum > scall->server->opnum_max)
		{
			...
			clrpc_sv_send_reject (scall->server, lower, CLRPC_NCA_OP_RANGE_ERR, scall->server->server_boot);
			return;
		}
		...
		{
			clrpc_sv_send_reject (scall->server, lower, CLRPC_NCA_SERVER_TOO_BUSY, scall->server->server_boot);
			return;
		}
...
# pragma CLRPC_MESSAGE("VV: drop this ugly up_call...")
		...
		up_call = scall->upper->args.sv.call;

		up_call->server_id = scall->server->id;
		up_call->scall_id  = scall;

		up_call->usr_id = if_obj->usr_id;
		up_call->host_ip = CLRPC_UDP_IP(udp_recv);
		up_call->udp_port = CLRPC_UDP_PORT(udp_recv);
		up_call->opnum = pkt->opnum;
		up_call->semantics = clrpc_pkt_get_semantics(pkt);
		up_call->drep.drep_char = pkt->drep[0] & 0x0F;
		up_call->drep.drep_int = pkt->drep[0] & 0xF0;
		up_call->drep.drep_float = pkt->drep[1];
		CLRPC_MEMCPY (&up_call->object_id, &pkt->object, sizeof (up_call->object_id));
		up_call->alloc_len   = 0;
		up_call->ndr_len     = 0;
		up_call->ndr_data    = LSA_NULL;
		up_call->pnio_status = CLRPC_PNIO_STATUS_INVALID;

		/* increment running opnum-count
		//	decrement with clrpc_sv_provide_enqeue (scall->upper)
		//----------------------------------------------------------------*/
		clrpc_if_obj_opnum_count_incr (if_obj
			, scall->server->pnio_signature, up_call->opnum
			);

		if (! scall->server->pnio_signature)
		{
			up_call->alloc_len = scall->server->alloc_len;
		}
		else
		{
			up_call->alloc_len = clrpc_sv_pnio_fetch_alloc_len (pkt);
			if (up_call->alloc_len == 0)
			{
				...
				CLRPC_ASSERT (up_call->alloc_len == 0);	/* must ! */
				clrpc_sc_send_error_fault (scall, CLRPC_NCA_FAULT_REMOTE_NO_MEMORY);
				return;
			}
			/* dynamically restrict to max allowed size */
			if (up_call->alloc_len > scall->server->alloc_len)
			{
				CLRPC_SV_TRACE_02 (scall->trace_idx, LSA_TRACE_LEVEL_UNEXP
		...
		if (up_call->alloc_len > 0)
		{
			CLRPC_ALLOC_UPPER_MEM (CLRPC_CAST_UPPER_MEM_PTR_PTR (&up_call->ndr_data), clrpc_null_user_id, up_call->alloc_len, scall->server->channel->sysptr);
			if (CLRPC_PTR_IS_NULL (up_call->ndr_data))
			{
				up_call->alloc_len = 0;
				CLRPC_SV_TRACE_00 (scall->trace_idx, LSA_TRACE_LEVEL_UNEXP
			...
			if (((LSA_UINT32)(up_call->ndr_data) & 3) != 0)
			{
		...
		CLRPC_SV_TRACE_07 (scall->trace_idx, LSA_TRACE_LEVEL_CHAT
			...
			, scall->id
			, CLRPC_NTOHL(up_call->host_ip)
			, CLRPC_NTOHS(up_call->udp_port)
			, up_call->opnum
			, (up_call->drep.drep_char & up_call->drep.drep_int)
			, up_call->semantics, up_call->alloc_len
			);
		CLRPC_SV_TRACE_12 (scall->trace_idx, LSA_TRACE_LEVEL_CHAT
			, "cid(%u) obj_id(%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x)"
			, scall->id
			, up_call->object_id.time_low, up_call->object_id.time_mid, up_call->object_id.time_hi_and_version
			, up_call->object_id.clock_seq_hi_and_reserved, up_call->object_id.clock_seq_low
			, up_call->object_id.node[0], up_call->object_id.node[1], up_call->object_id.node[2]
			, up_call->object_id.node[3], up_call->object_id.node[4], up_call->object_id.node[5]
			);
	}
...
/* AP00642069: because of lost FACK or rerun, first check fragnum, then check alloc_len */
	...
	if (up_call->ndr_len + pkt->len > up_call->alloc_len)
	{
		CLRPC_SV_TRACE_03 (scall->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "more data (hold(%u) + pkt.len(%u)) than alloc_len(%u)"
			, up_call->ndr_len, pkt->len, up_call->alloc_len
			);
		clrpc_sc_send_error_fault(scall, CLRPC_NCA_FAULT_REMOTE_NO_MEMORY);
		return;
	}
	...
	*/
	...
	else if (up_call->semantics == CLRPC_SEMANTICS_AT_MOST_ONCE
			 && scall->way.state == CLRPC_WAY_ST_UNKNOWN)
	{
	...
	CLRPC_MEMCPY (up_call->ndr_data + up_call->ndr_len, pkt + 1, pkt->len);
	up_call->ndr_len += pkt->len;
	...
	)
{
	...
	if (scall->upper->args.sv.call->semantics == CLRPC_SEMANTICS_AT_MOST_ONCE
	...
	)
{
	...
	if (up_call->semantics == CLRPC_SEMANTICS_AT_MOST_ONCE)
	{
	...
	if (scall->upper_wf == 0)
	{
		...
		is_maybe = (up_call->semantics == CLRPC_SEMANTICS_MAYBE || up_call->semantics == CLRPC_SEMANTICS_BROADCAST_MAYBE);
		...
		if (scall->server->pnio_signature)
		{
			...
			if (nca_status != 0)
			{
				...
				scall->upper_wf = 0;/* Request 1442389 */
				clrpc_sc_send_error_reject (scall, nca_status);
				return;
			}
		}
	...
	)
{
	...
	if (CLRPC_RQB_GET_RESPONSE (scall->upper) == CLRPC_ERR_REJECTED)
	{
		...
		To stop this storm of telegrams we use the FAULT with CLRPC_NCA_FAULT_UNSPECIFIC instead. */
		clrpc_sc_send_error_fault (scall, CLRPC_NCA_FAULT_UNSPECIFIC);
		return;
	}
	...
	/* epm server busy */
	if (CLRPC_RQB_GET_RESPONSE (scall->upper) == CLRPC_ERR_SERVER_BUSY)
	{
		CLRPC_SV_TRACE_01 (scall->trace_idx, LSA_TRACE_LEVEL_WARN
			, "cid(%u) epm server overload, send server busy"
			, scall->id
			);
		clrpc_sc_send_error_reject (scall, CLRPC_NCA_SERVER_TOO_BUSY);
		return;
	}
	...
	if (up_call->ndr_len <= CLRPC_MUST_RECV_BODY_SIZE)
	{
		CLRPC_SV_TRACE_01 (scall->trace_idx, LSA_TRACE_LEVEL_CHAT
			, "one pkt, len(%u)"
			, up_call->ndr_len
	...
	else
	{
		...
		scall->send.frag_last_fragnum = CLRPC_CAST_U16 (up_call->ndr_len / CLRPC_MUST_RECV_BODY_SIZE);
	}
	...
	)
{
	CLRPC_SV_TRACE_04 (scall->trace_idx, LSA_TRACE_LEVEL_CHAT
		...
		, scall->id, scall->state, scall->upper->args.sv.call->semantics, scall->send.frag_flags
		);
	...
	)
{
	...
	if (pkt->server_boot != scall->server->server_boot)
	{
		...
		clrpc_sv_send_reject (scall->server, lower, CLRPC_NCA_WRONG_BOOT_TIME, scall->server->server_boot);
		...
		*/
		...
		else if (scall->state == CLRPC_CST_INIT)
		{
			if (! (pkt->flags & CLRPC_PF_IDEMPOTENT))
			{
				...
				clrpc_sv_send_reject (scall->server, lower, CLRPC_NCA_PROTO_ERROR, scall->server->server_boot);
				return CLRPC_PROVIDE_LOWER;
			}
		}
	...
	)
{
	...
	/* clrpc_conversation_manager_uuid is checked on receive */
	...
	*/
	...
	clrpc_sc_send_error_reject (scall, CLRPC_NCA_WHO_ARE_YOU_FAILED);
	return CLRPC_PROVIDE_LOWER;
}
	...
	)
{
	...
	CLRPC_ASSERT (1 ==
		((CLRPC_PTR_IS_NULL (lower) ? 0 : 1) + (oneshot ? 1 : 0) + (CLRPC_PTR_IS_NULL (upper) ? 0 : 1) + (CLRPC_PTR_IS_NULL (send) ? 0 : 1)));
	...
	do
	{
		...
		else if (CLRPC_PTR_IS_NOT_NULL (send))
		{
			...
			if (clrpc_sc_send_prepare (scall, send) == CLRPC_OK)
			{
				...
				{
					static int done = 0;
					if (! done && scall->send.frag_curr_fragnum == 2 && scall->upper->args.sv.call->opnum == CLRPC_PNIO_OPNUM_READ)
					{
						//skip send
						done = 1;
		...
		/* A timeout can overtake a send job, if there the scall is stuck in the send-queue due to a lack of resources. Also see clrpc_sc_post_to_send_queue(). Test case: overloaded EB200. */
