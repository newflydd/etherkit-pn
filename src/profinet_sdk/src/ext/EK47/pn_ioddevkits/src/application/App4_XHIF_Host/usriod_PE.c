...
static PNIO_UINT8  ResponseDataAvailable[IOD_CFG_NUMOF_IO_AR] = {PNIO_FALSE,PNIO_FALSE,PNIO_FALSE,PNIO_FALSE};

#define RSP_MAXBUFSIZE  ( sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_GET_MODE ) + 20 )

PNIO_UINT8   PE_RspBuffer[ IOD_CFG_NUMOF_IO_AR ][ RSP_MAXBUFSIZE ];  /* buffer for PE response */
PNIO_UINT32  PE_RspLen[IOD_CFG_NUMOF_IO_AR];                       /* length of PE response (must be <= RSP_MAXBUFSIZE) */
    ...
    )
{
    ...
    printf( "Request not succesfull, responding with error code 0x%x\n", PE_ErrNum );

    /* ***adapt blockheader *** */
    pCmdRspHdr->BlockHdr.Type  = htons( 0x0801 );
    pCmdRspHdr->BlockHdr.Len   = htons( sizeof( PE_SUM_RSP_HDR ) + sizeof( PE_SDR_RSP_ERROR ) - 4 );
                                     ...
                                     PNIO_UINT8     DataStrucIdRs_ok)   /* [in]  RSP: data structure identifier response, of ok */
{
    ...
    memcpy( ( PNIO_VOID* ) pRspBuf, ( PNIO_VOID* ) pReqBuf, sizeof( PE_SUM_REQ_HDR ) );

    /* ***adapt blockheader *** */
    pCmdRspHdr->BlockHdr.Type  = htons( 0x0801 );
    pCmdRspHdr->BlockHdr.Len   = htons( BlockLen );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
        	...
        	printf ( "PROFIenergy command START_PAUSE, time = %dmsec  received\n", htonl( pRqStartP->PauseTime ) );
...
{
  // **** check block type  ***
  if (pBlockHdr->Type != htons(BlockType))
  {
        return (PNIO_NOT_OK);
  }

  // **** check block length  ***
  if (pBlockHdr->Len != htons ((PNIO_UINT16)BlockLen))
  {
        return (PNIO_NOT_OK);
  }

  // **** check block version   ***
  if (pBlockHdr->Version != htons(BlockVers))
  {
        return (PNIO_NOT_OK);
  }
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy command END_PAUSE received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy command QUERY_MODES received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        if( ErrCode == PE_RET_OK )
        {
            ...
            /* if ok, set data structure identifier RS and service data response values *** */
            printf( "PROFIenergy command GET_MODE received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy command PEM STATUS received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy command IDENTIFY received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            // * if ok, set data structure identifier RS and service data response values ***
            printf ( "PROFIenergy Get Measurement values received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy Get Measurement list received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
			// * if ok, set data structure identifier RS and service data response values ***
			printf ( "PROFIenergy Get Measurement values with objectnumber received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
			/* if ok, set data structure identifier RS and service data response values *** */
			printf ( "PROFIenergy Get Measurement list with object number received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf ( "PROFIenergy PEM Status with CTTO received\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
            /* if ok, set data structure identifier RS and service data response values *** */
            printf( "PROFIenergy Start Pause with time response\n" );
    ...
    )
{
    ...
    /*     BEGIN  user code */
        ...
        {
        	/* if ok, set data structure identifier RS and service data response values *** */
        	printf( "PROFIenergy Query Version response\n" );

        	/* used version V1.1 */
        	pRspIdent->major = 1;	/* major version number of PE profile */
          	pRspIdent->minor = 1;	/* minor version number of PE profile */
    ...
    )
{
   ...
   /* *********** check block header ********** */
    ...
    memcpy( ( PNIO_VOID* )&PE_RspBuffer[ ArNum ][ 0 ], ( PNIO_VOID* ) pSumReqHdr, sizeof( PE_SUM_REQ_HDR ) );
    ...
    )
{
    ...
    if( pAddr->Geo.Slot == 0 )  /* is DAP ? */
    {
        ...
        /* Indication that we did not save any bytes to buffer */
        *pBufLen = 0;
        return ( PNIO_NOT_OK );
    }
    ...
    {
        printf("PROFIenergy Response Length exceeds Buffer Length!\n");
    }
    else
    {
    ...
    if(NULL != pBuffer)
    {
        memcpy ( pBuffer, &PE_RspBuffer[ ArNum ][ 0 ], *pBufLen );
    }
    else
    {
