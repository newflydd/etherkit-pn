...
ENTER_XX_MEM;
 ...
 // * --------------------------------------------
 //OsMemSet(pFirstBuf, (int)mem_ini_value, (PNIO_UINT32)pDesc->size_aligned_without_desc);
...
// *
// ***************************************************************************/
XX_MEM_DESCRIPTOR_T   *g_pDesc = NULL;
PNIO_UINT32 g_Xmem = 0;
XX_MEM_DWORD  XX_MEM_CODE_ATTR  XX_MEM_alloc( void                       **p,
                                              XX_MEM_DWORD               len,
                                              XX_MEM_BYTE                *pBaseAdr )
{
    ...
    g_pDesc = (XX_MEM_DESCRIPTOR_T  *)pBaseAdr;
    *p = XX_MEM_NULL_PTR;
    ...
    if ( (h->buf_len == 0) && (h->free_len_to_next_buf >= buf_size) )
    {
        ...
        *p = p_own;
        ...
        {
            OsMemSet(*p, (int)pDesc->preset_value, (PNIO_UINT32)h->buf_len);
        }
        g_Xmem += buf_size;
    ...
    *p = p_own;
    ...
    if (pDesc->do_preset_buffer == XX_MEM_DO_PRESET )
    {
        OsMemSet(*p, (int)pDesc->preset_value, (PNIO_UINT32)act->buf_len);
    }
...
// *
// ***************************************************************************/
XX_MEM_DWORD XX_MEM_CODE_ATTR XX_MEM_free( void *p )
{
...
XX_MEM_DWORD XX_MEM_CODE_ATTR  XX_MEM_info( XX_MEM_INFO_T  *info)
{
    ...
    for ( ;;)
    {
        ...
        if ( (h->buf_len > 0) && (h->alloc_index >= info->IndexFirstBuf) )
        {
            ...
            #endif
            if ( info->pFirstBuf == (void *)0  )
            {
                info->pFirstBuf     = (void *)(h + 1);
...
XX_MEM_DWORD XX_MEM_CODE_ATTR  XX_MEM_infoPrint( XX_MEM_INFO_T  *info)
{
    ...
    for ( ;;)
    {
        ...
        {
            #ifdef XX_MEM_STATISTIC
                info->count_allocated_buffer++;    // first block is always present, but can be empty
            #endif
            if ( info->pFirstBuf == (void *)0  )
            {
                info->pFirstBuf     = (void *)(h + 1);
...
#if 0
        PNIO_printf ("Ind=%d  len=%d freelen=%d,  offs=0x%x, OfsN=0x%x,  OfsP=0x%x\n",
                 h->alloc_index,
                 h->buf_len,
                 h->free_len_to_next_buf,
                 h->offs_own,
                 h->offs_next_buf,
                 h->offs_prev_buf);
#endif
...
/*---------------------- end [subroutine] ---------------------------------*/
    ...
    LSA_UINT32 xx_mem_validateAll (void* pMemP)
    {
        PNIO_UINT32 Status;
