/*
 * v3.1 August 30, 2011 tcp/nptcp.c
...
#define LTRC_ACT_MODUL_ID 26 /* INICHE_TCIP_ITGR added for LSA traces */
...
#endif
      {
#if 0 /* INICHE_TCIP_ITGR no dtrap */
...
nptcp_init()
{
   ...
   tcpmib.tcpRtoAlgorithm = 4;     /* Van Jacobson's algorithm (see AP01352893) */
   tcpmib.tcpRtoMin = TCPTV_MIN * 1000;      /* PR_SLOWHZ */
   tcpmib.tcpRtoMax = TCPTV_REXMTMAX * 1000; /* PR_SLOWHZ */
   tcpmib.tcpMaxConn = -1; /* INICHE_TCIP_ITGR AP01332791 */
#endif
...
#if 1 /* INICHE_TCIP_ITGR -- be more tolerant */
   {
       u_short     chksum = tcp_cksum(bip);
       if(chksum != tcpp->th_sum)
       {
           if(!((chksum == 0) && (tcpp->th_sum == 0xffff)))
#else
   ...
   if ((data->m_next))
   {
      ...
      while (m2)  /* If we still have two or more buffers, more copying: */
      {
         ...
         { 
			...
			TCIP_INICHE_TRACE_02(0, LSA_TRACE_LEVEL_FATAL
				, "nptcp: MaxLnh:%d, e:%d"
				, MaxLnh, e
				);
#else
...
so_icmpdu(PACKET p, struct destun * pdp)
{
   ...
   fhost = pdp->dip.ip_dest;
   lhost = pdp->dip.ip_src;
   fport = *(unshort*)(&pdp->ddata[0]);
   lport = *(unshort*)(&pdp->ddata[2]);
   ...
   if (pdp->dip.ip_prot == TCPTP)
   {
      ...
      so->so_error = EMSGSIZE;  /* set error for socket owner */
   }   
   ...
   else if(pdp->dip.ip_prot == UDP_PROT)
   {
      ...
      {
         if ((tmp->u_fport == htons(fport) || tmp->u_fport == 0) &&
             (tmp->u_fhost == fhost) &&
             (tmp->u_lport == htons(lport)))
         {
...
tcp_tick()
{
   ...
   {
   tcp_fasttimo();
      nextfast = cticks + 1;   /* Once per ctick */
   }
#endif   /* DO_DELAY_ACKS */
