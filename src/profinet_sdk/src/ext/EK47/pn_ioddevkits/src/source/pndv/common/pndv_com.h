...
#define PNDV_ERR_FATAL                       ((UnsignedOpt)0x00)

#define PNDV_OK                              ((UnsignedOpt)0x01)
#define PNDV_OK_ASYNC                        ((UnsignedOpt)0x02)
#define PNDV_OK_DONE                         ((UnsignedOpt)0x03)
#define PNDV_OK_NONE                         ((UnsignedOpt)0x04)
#define PNDV_OK_CHANGE_ALL                   ((UnsignedOpt)0x05) /* legacy */
#define PNDV_OK_CHANGE_ID                    ((UnsignedOpt)0x06) /* legacy */
#define PNDV_OK_WAIT                         ((UnsignedOpt)0x07) /* legacy */
#define PNDV_OK_SUBSTITUTED                  ((UnsignedOpt)0x08) /* legacy */

#define PNDV_ERR_MASK                        ((UnsignedOpt)0x80)
#define PNDV_ERR_TIMER                       ((UnsignedOpt)0x82)
#define PNDV_ERR_NOT_SUPPORTED               ((UnsignedOpt)0x83)
#define PNDV_ERR_RESOURCE                    ((UnsignedOpt)0x84)
#define PNDV_ERR_APPL_RESOURCE               ((UnsignedOpt)0x85)
#define PNDV_ERR_PARAMETER                   ((UnsignedOpt)0x86)
#define PNDV_ERR_SEQUENCE                    ((UnsignedOpt)0x87)

#define PNDV_TO_PERI_OK                      ((Unsigned8)0x01)
#define PNDV_TO_PERI_OK_CANCELLED            ((Unsigned8)0x02)
#define PNDV_TO_PERI_OK_NO_ALARM             ((Unsigned8)0x03)
#define PNDV_TO_PERI_OK_R1_NOT_ACKED         ((Unsigned8)0x04)

#define PNDV_TO_PERI_UNREPORTED              ((Unsigned8)0x11)  /* modul state does not allow a diag yet */
...
#ifdef PNDV_USE_PSI
#define PNDV_INDEX_PATH_CMPDSV              0
#define PNDV_INDEX_PATH_ACP_CM              1

#define PNDV_MAX_CHANNEL                    2
#else
#define PNDV_INDEX_PATH_EDD_CLRPC_CM        0
#define PNDV_INDEX_PATH_CMPDSV              1
#define PNDV_INDEX_PATH_ACP_CM              2

#define PNDV_MAX_CHANNEL                    3
#endif

#define PNDV_DFC_IDX_NOT_USED               ((Unsigned16)0xffff)
#define PNDV_DFC_IDX_MAX                    ((Unsigned16)0xfffe)

#define PNDV_RECORD_MAX_RES                 1      /* rd_in_out.rqb_ds_res_ptr */
#define PNDV_EVENT_MASK                     0x00FF  /* low part is event info / high part is additional info (e.g. slot) */
...
typedef struct PNDV_GLOB_DATA_S
{
    ...
    Unsigned32                      led_info;
    Unsigned32                      led_maint_info;

    UnsignedOpt                     sf_on;
    UnsignedOpt                     maint_on;
    UnsignedOpt                     save_maint_on;

    Unsigned32                      sync_led_info;

    UnsignedOpt                     syn_led_on;
...
/* removed    PNDV_EV_TO_PERI_PARA_REQ_DONE             = 0x25, TRC_EXPLAIN("PERI_PARA_REQ_DONE") */
...
/* removed    PNDV_EV_TO_PNDV_PARA_REQ                       = 0x95, TRC_EXPLAIN("PNDV_PARA_REQ")*/
    ...
    PNDV_EV_TO_PNDV_EXT_CHANNEL_DIAG_RMV_ALL       = 0xc7, TRC_EXPLAIN("PNDV_EXT_CHANNEL_DIAG_RMV_ALL") /**< ar_idx in add_info 1 */
    PNDV_EV_TO_PNDV_EXT_GENERIC_DIAG               = 0xc8, TRC_EXPLAIN("PNDV_EXT_GENERIC_DIAG") /**< index to a subslot entity in add_info 2 */
...
{                                                            \
    ...
    {                                                       \
        ...
        case PNDV_EV_TO_PNDV_EXT_CHANNEL_DIAG_RMV_ALL           : _pstr5 = "EXT_CHANNEL_DIAG_RMV_ALL";         break; \
        case PNDV_EV_TO_PNDV_EXT_GENERIC_DIAG                   : _pstr5 = "EXT_GENERIC_DIAG";                 break; \
...
#define    PNDV_PERI_STATE_NOT_OK         (Unsigned8)0
#define    PNDV_PERI_STATE_OK             (Unsigned8)1
...
typedef struct
{
    ...
    Unsigned8       record_data[CM_RECORD_OFFSET + PNDV_MAX_RECORD_DATA_LEN];
...
typedef struct
{
    CM_RQB_TYPE     rqb;
    CM_EVENT_TYPE   event_type;
    Unsigned8       header[CM_RECORD_OFFSET];
    Unsigned8       data[PNDV_MAX_RECORD_DS1_LEN];
...
typedef struct
{
    Boolean     mapped;
    Unsigned16  slot;
}   PNDV_OH_TABLE_STRUCT;

typedef struct
{
    Boolean                 modul_used;
    Boolean                 reserve_modul_allowed;
    Boolean                 primary_module;
    PNDV_OH_TABLE_STRUCT    soll_ist;
    PNDV_OH_TABLE_STRUCT    ist_soll;
}   PNDV_OH_MAPP_DATA_STRUCT;

/*------------------------------------*/

typedef struct PNDV_CHANNEL_DIAG_S
{
    Unsigned8   alm_kommend;
    Unsigned8   kennung;
    Unsigned16  kanal;
...
typedef struct PNDV_GENERIC_DIAG_S
{
    ...
    Unsigned8           alm_kommend;
    Unsigned16          usi;
    Unsigned32          diag_tag; /* the diag tag must be provided by the PNPB */
    Unsigned16          length;
    Unsigned8           *data;
    // Unsigned16          properties; // &&&2do  for DK4.0
    // Unsigned16          channel;    // &&&2do  for DK4.0
...
{
    Unsigned8  data[PNDV_AL_PRAL_INFO_MAX_LEN];
...
{
    Unsigned8 data[PNDV_AL_UPAL_INFO_LEN];
} PNDV_UPAL_INFO_T;
...
{
    Unsigned8                   data[PNDV_AL_URAL_INFO_LEN];
    Unsigned16                  usi;
} PNDV_URAL_INFO_T;
...
{
    Unsigned8                   data[PNDV_AL_STAL_INFO_LEN];
    Unsigned16                  ar_idx;
    Unsigned16                  usi;
} PNDV_STAL_INFO_T;
...
{
    Unsigned32                  fdata;   //!< Bit0 = [0=parameter not configurable|1=parameter configurabele]
                                         //!< Bit1 =
}PNDV_REAL_FLAGS_T;
...
{
    Boolean                           ownership_passed;  //!< TRUE if this element got an ownership once. FALSE otherwise.
...
typedef struct PNDV_SET_CFG_S
{
    ...
    Unsigned32                      flags; //!< Bit0 = [0=not owner|1=owner]
                                           //!< ***** option handling *****
...
{
    Unsigned8       cmd;    /**< use commands acording @ref PNDV_EVENTS_E */
    Unsigned8       add_1;
    Unsigned16      add_2;
...
typedef struct PNDV_IFACE_CMD_S
{
    ...
    UnsignedOpt                 read_ptr_peri_to_pndv;               
    UnsignedOpt                 write_ptr_peri_to_pndv;

    /* events from profinet stack to peripheral bus */
    PNDV_IFACE_CMD_ENTRY_T      pndv_to_peri[PNDV_IFACE_CMD_CNT];

    UnsignedOpt                 read_ptr_pndv_to_peri;
    UnsignedOpt                 write_ptr_pndv_to_peri;
} PNDV_IFACE_CMD_T;
...
typedef struct PNDV_IFACE_STRUCT_S
{
    ...
    struct
    {
        ...
        Boolean                     suppress_error_if_para_ds;
    ...
    struct
    {
        Unsigned32                  previous_rt_class;      /* needed for internal decisions */
        Unsigned32                  current_rt_class;       /* tells rt class for slave processor */
        ...
        Unsigned16                  sr_ar_set_nr;           /* number of the ar set a sysred ar belongs to */
        ...
        Boolean                     red_int_info_used;      /**< if true, red_int_info is already filled up */
        Unsigned16                  ar_session_key;         /* holds the AR session key, can be used by upper components for key->AR index mapping*/
        ...
        Unsigned8                   ar_fsu_enable;          /* fsu enabled if true */
        Unsigned8                   ar_fsu_uuid[16];        /* onyl valid if ar_fsu_enable == true */
        #ifdef PNDV_CFG_ENABLE_RS_INFO_BLOCK_SUPPORT
        UnsignedOpt                 has_RSInfoBlock;        /* AR has RS info block */
        #endif
        struct
        {
            Unsigned32   RxEndNs;
            Unsigned32   RxLocalTransferEndNs;
            Unsigned32   TxEndNs;
            Unsigned32   TxLocalTransferStartNs;
            Unsigned32   TxLocalTransferEndNs;
...
typedef CM_UPPER_RQB_PTR_TYPE PNDV_DFC_REQ_PTR;

typedef void     PNDV_FUNC_PTR_NAME( PNDV_DFC_RESP_CALL)( void * );
typedef void     PNDV_FUNC_PTR_NAME( PNDV_DFC_RESP_ABORT)( PNDV_DFC_REQ_PTR );
...
typedef struct
{
    Unsigned32                  virt_mod_id;
    Unsigned32                  virt_submod_count;
    Unsigned32                  *virt_sub_mod_id_ptr;

} PNDV_VIRT_MOD_STRUCT;
...
typedef struct PNDV_PD_CFG_S
{
    ...
    struct
    {
        ...
        LSA_UINT32  im_0_bits;
    }port[PNDV_CFG_MAX_PORT_CNT+1];

}PNDV_PD_CFG_S_T;

typedef PNDV_VIRT_MOD_STRUCT * PNDV_VIRT_MOD_STRUCT_PTR;
...
typedef struct
{
    ...
    struct
    {
        Unsigned32               virtmod_used;
        PNDV_VIRT_MOD_STRUCT_PTR virtmod_ptr;
    }   virt_mod;
...
typedef enum
{
  PNDV_AR_ABORT_EXT_PERI    = 0,    /* peri bus fault */
  PNDV_AR_ABORT_EXT_SO_EVENT,       /* SO locked  */
  PNDV_AR_ABORT_EXT_RTF             /* power down ar before ResetToFactory */
...
#define PNDV_LSA_COMPONENT_ID                   0x0000
...
typedef struct
{
    Unsigned8 * trace_data_ptr;
    Unsigned32  trace_data_len;

    Unsigned8 * local_data_ptr;
    Unsigned32  local_data_len;
...
//!@ingroup grSYSTEM
//!@{
void        pndv_init                            (PNDV_IFACE_STRUCT_PTR  iface_ptr);
void        pndv_open                            (PNDV_SYS_PARAMETER_PTR parameter_ptr);
void        pndv_perform_services                (void);
Unsigned16  pndv_version                         (Unsigned16 version_len, PNDV_SYS_VERSION_PTR version_ptr);
void        pndv_debug_extern                    ( Unsigned16 detail_1, Unsigned16 detail_2);
void        pndv_debug_get_info                  (PNDV_SYS_DEBUG_INFO_PTR debug_info);
void        pndv_connect_ind_trigger_timeout     (Unsigned16 timer_id, PNDV_USER_ID_TYPE user_id);

void PNDV_SYSTEM_OUT_FCT_ATTR               pndv_request(PNDV_RQB_PTR_TYPE rqb_ptr);
//! @}
...
//!@ingroup grSYSTEM
//!@{
void        pndv_peri_read_write_record          (void *req_ptr);
//!@}
void        pndv_al_ps_state                     (Unsigned16 slot, Boolean mode );

void        pndv_ar_sys_disconnect_ind_free_iocr_mem_done        (UnsignedOpt ar_idx);
Unsigned8   pndv_ar_get_num_connected_ars                        (void);

void        pndv_peri_init_entity_admin(void);
void        pndv_peri_reset_entity_admin_by_ar_idx(UnsignedOpt ar_idx);
UnsignedOpt pndv_peri_get_entity_index(Unsigned16 slot, Unsigned16 subslot, UnsignedOpt ar_idx);
#ifdef PNDV_CFG_PERI_QUEUE_DS_REQ
UnsignedOpt pndv_peri_do_rqb_queue(CM_UPPER_RQB_PTR_TYPE rqb_ptr);
void        pndv_peri_rqb_queue_get_next(CM_UPPER_RQB_PTR_TYPE *rqb_ptr_ptr);
#endif

/* pndv_cfg.c*/


LSA_UINT32  pndv_cantor(LSA_UINT16 slot, LSA_UINT16 subslot);
void        pndv_maint_led_control               (UnsignedOpt sf_led_mode);
...
void pndv_r1_init(int im_position);
void pndv_r1_pppn_cbf(PPPN_RQB_PTR rqb_ptr);

void pndv_r1_bridge_pppn_cbf(PPPN_RQB_PTR rqb_ptr);
void pndv_r1_bridge_start(void);


void pndv_r1_backplane_state_active(void);
void pndv_r1_backplane_state_inactive(void);

typedef struct PNDV_R1_OPEN_DATA_S
{
    uint8_t  pppn_bridge_handle;      //!< handle for pppn-open-channel
    uint8_t  pppn_bridge_send_opc;    //!< prio for sending pppn-data

    uint8_t  pppn_handle;      //!< handle for pppn-open-channel
    uint8_t  pppn_send_opc;    //!< prio for sending pppn-data

}   PNDV_R1_OPEN_DATA_T;

typedef PNDV_R1_OPEN_DATA_T* PNDV_R1_OPEN_DATA_PTR;

void pndv_r1_open(PNDV_R1_OPEN_DATA_PTR open_data_ptr);
