...
/*                                                                           */
/*   TSKMA message handler for ORG task                                      */
/*                                                                           */
/*****************************************************************************/
...
#define TSKMA_MODULE_ID 	TSKMA_MODULE_ID_TSKMA_TASK_ORG
...
//*----------------------------------------------------------------------------
//*	tskma_task_org_request()
//*
//* Service EDD messages
//*----------------------------------------------------------------------------
void tskma_task_org_request(TSKMA_RQB_S_PTR_T rqb_ptr)
{
    switch(TSKMA_RQB_GET_OPCODE(rqb_ptr))
    {
        case TSKMA_OPC_ORG_REQ_RESERVED:
        {
...
//* TSKMA message handler for ORG task
//*----------------------------------------------------------------------------
OS_CODE_FAST void tskma_task_org (void)
{
	#if DEBUG_TASKCNT
	LSA_UINT32  taskID;
	#endif
    ...
    TSKMA_TASK_INIT_STATE(TSKMA_TASK_NUM_ORG);
    ...
    for(;;)
    {
        ...
        TSKMA_OS_READ_MAIL(&msg.void_ptr, TSKMA_TASK_NUM_ORG);
        ...
        TSKMA_WAKE_NEXT_TS_TASK(TSKMA_TASK_NUM_ORG);
    }
}
