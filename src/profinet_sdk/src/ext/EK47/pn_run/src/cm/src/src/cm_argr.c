...
)
{
	...
	ar_graph->ar_com.path_type = channel->path_type;
	...
	ar_graph->ar_com.autostop_cons_id = EDD_CONS_ID_INVALID;

	{
		/* CM-CL: block re-initialized by cm_ar_com_idle() */
	ar_graph->ar_com.down = CM_AR_REASON_NONE;
	ar_graph->ar_com.cycle_counter = 0;
	ar_graph->ar_com.down_pnio_status = CM_PNIO_ERR_NONE;
	CM_MEMSET (& ar_graph->ar_com.down_debug_data, 0, sizeof(ar_graph->ar_com.down_debug_data));

	ar_graph->ar_com.cr_is_up = LSA_FALSE;
	ar_graph->ar_com.cr_is_rir = LSA_FALSE;

	ar_graph->ar_com.user_apdu_status = 0; /* see cm_ar_graph_insert_io_ar() and cm_ar_com_idle() */
	}

	/* note: ar_graph->ar.ar_type, etc. is not yet valid */
	...
	case CM_PATH_TYPE_MULTICAST:
		/* nothing to initialize. See CM_AR_GRAPH_PTR_TYPE */
		ok = LSA_TRUE;
		break;
#endif
...
cm_ar_graph_insert_io_ar (
	...
	CM_ASSERT(cm_is_not_null(ar_graph));
	CM_ASSERT(cm_is_not_null((LSA_VOID_PTR_TYPE)ar_uuid));
	CM_ASSERT(cm_is_not_null(cmi_mac));
	CM_ASSERT(cm_is_not_null(cmi_obj_uuid));
	...
	*/
	...
	default:
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "ar_type(%u) out of range"
			, ar_type
			);
		return 4; /* ar-type out of range */
	}

	if( ar_type == CM_AR_TYPE_SINGLE_RTC3 ) {

		CM_EDD_CHANNEL_PARAMS_PTR_TYPE edd_params = cm_edd_channel_params(ar_graph->ar_com.channel);

		if( edd_params->ConsumerCntClass3 == 0 && edd_params->ProviderCntClass3 == 0 ) { /* note: "&&" because of CM-MC */

			CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
				, "ar_type(%u) not supported"
				, ar_type
				);
			return 4; /* ar-type not supported */
		}
	}
	...
	if( (ar_properties & CM_AR_PROP_RESERVED_24_28) != 0 ) {
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
	...
	if( (ar_properties & CM_AR_PROP_STARTUP_MODE_ADVANCED) != 0 ) {
		...
		if( (ar_properties & CM_AR_PROP_COMBINED_OBJECT_CONTAINER) != 0 ) {

			CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
				, "ar_properties.Legacy and ar_properties.CombinedObjectContainer not allowed"
				);
			return 9; /* 'CombinedObjectContainer' requires */
		}
	}
	...
	CM_ASSERT(cm_is_null(ar_graph->ar.cmi_station_name));

	if( cm_is_null(cmi_station_name) ) {
		...
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
			...
			if( cm_is_null(ar_graph->ar.cmi_station_name) ) {
	...
	if (ar_type == CM_AR_TYPE_SINGLE_SYSRED) {
		ar_graph->ar_com.user_apdu_status = CM_EVENT_APDU_STATUS_INITIAL_SYSRED_VALUE;
	}
	else {
		ar_graph->ar_com.user_apdu_status = CM_EVENT_APDU_STATUS_INITIAL_VALUE;
	}
...
cm_ar_graph_insert_ir_info (
	...
	if (cm_is_not_null (ar_graph->ir_info))
	{
		CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
...
cm_ar_graph_insert_ir_info_sub (
	...
	if (cm_is_null (ar_graph) || cm_is_null (ar_graph->ir_info))
	{
	...
	if (cm_is_not_null (ir_info_sub))
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR, "iocr_ref(%u) defined twice", iocr_ref);
		return 8;
	}

	if (! (6 <= subframe_offset && subframe_offset <= 0x059B))
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR, "subframe_offset(%u) out of range", subframe_offset);
		return 9;
	}
	...
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR, "subframe_data(0x%x) out of range", subframe_data);
		return 10;
	}
	...
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR, "no local-mem, size(%u)", sizeof (*ir_info_sub));
		return 255; /* no mem (for ir_info) */
	}
...
cm_ar_graph_update_io_ar (
	...
	if( ar_graph->ar_com.has_udp )
	{
		...
		if( cmr_udp_port != CM_IANA_PNIO_UDP_UNICAST_PORT_NBO )
		{
			/* note: cmr_udp_port must be cmi_udp_port. see EDD_SRV_SRT_PROVIDER_ADD */

			CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
				, "cmr_udp_port(0x%x) not edd_port(%u)"
				, CM_NTOH16(cmr_udp_port), CM_NTOH16(CM_IANA_PNIO_UDP_UNICAST_PORT_NBO)
				);
			return LSA_FALSE; /* cmr_udp_port not as expected */
		}
	}
...
cm_sv_ar_graph_check_io_ar(
	...
	LSA_UINT16 edd_port = CM_IANA_PNIO_UDP_UNICAST_PORT_NBO;
...
)
{
	...
	CM_ASSERT (cm_is_not_null (mac_addr));
	...
	*/
	...
	default:
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "unknown rt_class(0x%x)", rt_class
			);
		return 7; /* unknown rt-class */
	}

	{
	CM_AR_GRAPH_IO_CR_PTR_TYPE firstCr = CmListFirst(&ar_graph->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE);

	if( cm_is_not_null(firstCr) ) {

		if( (firstCr->iocr_properties & CM_IOCR_PROP_RT_CLASS_MASK) != (iocr_properties & CM_IOCR_PROP_RT_CLASS_MASK) ) {

			return 7; /* RTClass must match first CR's RTClass */
		}
	}
	}
	...
	if( ar_graph->ar.ar_type == CM_AR_TYPE_SINGLE_RTC3 ) {
		...
		/*
		 * note: No need to check if "not supported in GSDML" here.
		 *       See cm_ar_graph_insert_io_ar(). RTC3 support by EDD already checked there.
		 */
	...
	if( is_multicast || rt_class == CM_IOCR_PROP_RT_CLASS_3 ) {
		...
		case CM_PATH_TYPE_CLIENT:
			...
			frame_id = CM_EDD_INVALID_FRAME_ID; /* allocated below (sanity) */
			break;
	...
	is_power_of_two = cm_is_power_of_2 (send_clock);
	...
	*/
	...
	if( frame_send_offset != 0xFFFFFFFF ) { /* i.e. not "best effort" */
		...
		LSA_UINT32 send_cycle_time = send_clock * 31250; /* note: send_clock <= 32ms, see cm_edd_send_clock_range_ok_hw() */
	...
	if( is_multicast )
	{
		...
		case CM_PATH_TYPE_SERVER:
			if( is_consumer ) {
				...
				if( rt_class == CM_IOCR_PROP_RT_CLASS_3 ) {
					CM_FATAL(); /* RTC3-QUERVERKEHR + CM-SV needs review if re-enabled. see AP00774495 too. */
				}
				else {
					ptr->cm.cr_state = CM_AR_CR_IDLE;
				}
...
#if CM_CFG_MAX_SERVERS //R1 marker (special case representative-AR)
	if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER && ((ar_graph->cm.sv.r1_flags & CM_SV_AR_R1FL_REPRESENTATIVE) != 0) ) {
		ptr->cm.cr_state = CM_AR_CR_ALLOCATED;
	}
#endif
...
#endif
	ptr->cm.sv.sum_iodata = 0; /* sanity */
	ptr->cm.sv.sum_ioxs   = 0; /* sanity */
#endif
...
)
{
	...
	if( ! cm_edd_send_clock_range_ok_hw(ar_graph->ar_com.channel, send_clock) )
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "send_clock(%u) not supported", send_clock
			);
	...
	if (send_clock != local_clock)
	{
		CM_AR_TRACE_02 (ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "send_clock(%u) != pdev-send_clok(%u)", send_clock, local_clock
		);
		return 10; /* adapt.send-clock doesn't match pdev.send-clock*/
	}
...
{
	CM_AR_GRAPH_IO_CR_PTR_TYPE  iocr;
	LSA_UINT16 autostop_cons_id = EDD_CONS_ID_INVALID;
	LSA_UINT32 lowest_data_hold_time = ~(LSA_UINT32)0;
	...
	for (CmListForeach (iocr, & ar_graph->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE))
	{
		...
		{
			if ( CM_ACPID_ISVALID(iocr) ) /* add.cnf(+) */
			{
				LSA_UINT32 data_hold_time = iocr->reduction_ratio;
				data_hold_time *= iocr->data_hold_factor; /* note: no send_clock here, because it's const for all iocr's */

				if (data_hold_time < lowest_data_hold_time) /* TIA 1218136 */
				{
					lowest_data_hold_time = data_hold_time;
	...
	if (autostop_cons_id != EDD_CONS_ID_INVALID)
	{
...
static CM_AR_GRAPH_IOAPI_PTR_TYPE
...
)
{
	...
	CM_ASSERT (cm_is_not_null (iocr_ap));
	...
	)
{
	...
	case CM_MCR_PROP_PROTOCOL_DCP:
		break;
	case CM_MCR_PROP_PROTOCOL_DNS: /* DNS currently not supported */
	default:
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
	...
	if (cm_is_null (provider_name)) {
		CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "cm_is_null(provider_name)"
			);
...
cm_ar_graph_insert_alarm_cr (
	...
	if( cm_is_not_null (ar_graph->alarm_cr) ) {
		CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "more than one alcr"
			);
		CM_FATAL(); /* a bug in caller */
...
#if CM_CFG_MAX_SERVERS //R1 marker (special case representative-AR)
	if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER && ((ar_graph->cm.sv.r1_flags & CM_SV_AR_R1FL_REPRESENTATIVE) != 0) ) {
		ptr->cm.cr_state = CM_AR_CR_ERROR; /* see cm_ar_com_alarm_remove_next() */
	}
#endif

	local_alarm_ref = ACP_INVALID_ALARM_REF;

	if( ptr->cm.cr_state == CM_AR_CR_IDLE )
	{
		LSA_BOOL ok;
...
cm_ar_graph_insert_ap_module (
	...
	if( cm_is_not_null(exp_slot_nr_ptr) ) {
...
cm_ar_graph_insert_submodule (
	...
	if (cm_is_not_null (ptr)) {
		CM_AR_TRACE_01(argr->trace_idx, LSA_TRACE_LEVEL_UNEXP
	...
	if( subslot_nr == 0 ) { /* special case */
		...
		if( argr->ar.ar_type == CM_AR_TYPE_SINGLE_SYSRED ) { /* allow 0x9jpp for R1 */

			//R1 marker (subslot_nr 0x9jpp)

			if(subslot_nr > 0x9FFF) {
			...
			/* note: more checks in cm_sv_build_expsubmod() for CM-SV */
		}
		else {
	...
	*/
	...
	if( (sub_properties & CM_SUB_PROP_DISCARD_IOXS_MASK) == CM_SUB_PROP_DISCARD_IOXS_YES ) {
		...
		if( IOXS_Required ) {
			...
			return 12; /* sub_properties.discard_ioxs not supported (TIA 1218229) */
		}
	}
	...
	if (cm_is_null (ptr)) {
		CM_AR_TRACE_01(argr->trace_idx, LSA_TRACE_LEVEL_ERROR
			, "no local-mem, size(%u)", sizeof(*ptr)
			);
...
#if CM_CFG_MAX_CLIENTS
	ptr->cl.real_sub_ident = 0;
	ptr->cl.real_sub_state = 0; /* dummy-assignment, see cm_cl_marshal_connect() */
	ptr->cl.sub_coc_properties = 0;
	CmListInitialize (& ptr->cl.prm_record_list);
	CmListInitialize (& ptr->cl.prm_update_record_list);
#endif
	...
	ptr->sv.err_flags  = 0; /* see cm_sv_pd_prm_begin() */
	ptr->sv.err_index  = 0;
	ptr->sv.err_offset = 0;
#endif
...
)
{
	...
	CM_ASSERT (cm_is_not_null (submod));
...
cm_ar_graph_insert_sr_info(
	...
	if( cm_is_not_null(ar_graph->sr_info) ) {
		CM_FATAL(); /* see caller */
	}
	...
	if( (sr_properties & CM_SR_PROP_INP_VALID_ON_BACKUP_AR) != 0 ) {
		...
		/* note: check TIA 211661 when the feature will be supported */
...
cm_ar_graph_insert_arvendor (
	...
	if (cm_is_not_null(ptr)) {
		CM_AR_TRACE_02(argr->trace_idx, LSA_TRACE_LEVEL_ERROR
	...
	if (alloc_size > 0xFFFF) {
		CM_AR_TRACE_01(argr->trace_idx, LSA_TRACE_LEVEL_ERROR
			, "max block size exceeded, size(%u) > 0xFFFF", alloc_size
			);
		return 255; /* no mem */
	}
	else {
	...
	if (cm_is_null (ptr)) {
		CM_AR_TRACE_01(argr->trace_idx, LSA_TRACE_LEVEL_ERROR
			, "no local-mem, size(%u)", alloc_size
			);
		return 255; /* no mem */
	}
	...
	)
{
	...
	if (rs_info_block_len != CM_REQ_RSINFO_SIZE)
	{
		*block_field = 1; /* not enough data */
	}
	else
	{
		LSA_UINT16  block_type, block_length;
		LSA_UINT8   block_version_high, block_version_low;
		LSA_UINT16  padding;
		LSA_UINT32  rs_properties;

		/*
		//	RSInfoBlock ::=
		//		BlockHeader, [Padding* a], RS_Properties
		//
		//	RS_Properties ::= Unsigned32
		*/

		block_type = CM_GET16_NTOH (rs_info_block, CM_REQ_RSINFO_BlockType);
		block_length = CM_GET16_NTOH (rs_info_block, CM_REQ_RSINFO_BlockLength);
		block_version_high = CM_GET8_NTOH (rs_info_block, CM_REQ_RSINFO_BlockVersionHigh);
		block_version_low = CM_GET8_NTOH (rs_info_block, CM_REQ_RSINFO_BlockVersionLow);

		padding = CM_GET16_NTOH (rs_info_block, CM_REQ_RSINFO_Padding1);

		rs_properties = CM_GET32_NTOH (rs_info_block, CM_REQ_RSINFO_RSProperties);

		if (block_type != CM_BLOCK_TYPE_RSINFOBLOCK)
		{
			*block_field = 0; /* wrong BlockType */
		}
		else if (block_length != (rs_info_block_len - (2 + 2)))
		{
			*block_field = 1; /* wrong BlockLength */
		}
		else if (block_version_high != 0x01)
		{
			*block_field = 2; /* wrong BlockVersionHigh */
		}
		else if (block_version_low != 0x00)
		{
			*block_field = 3; /* wrong BlockVersionLow */
		}
		else if (padding != 0x0000)
		{
			*block_field = 4; /* non zero padding */
		}
		else if (rs_properties != 0)
		{
			*block_field = 5; /* wrong RS_Properties (only AlarmTransport=0 supported) */
		}
		else
		{
			CM_AR_GRAPH_RS_INFO_PTR_TYPE rs_info;

			CM_ALLOC_LOCAL_MEM (CM_CAST_LOCAL_MEM_PTR_PTR (&rs_info), sizeof (*rs_info));

			if (cm_is_null (rs_info))
			{
				CM_AR_TRACE_01 (argr->trace_idx, LSA_TRACE_LEVEL_ERROR
					, "no local-mem, size(%u)"
					, sizeof(*rs_info));

				*block_field = 255; /* no mem (for rs_info) */
				return LSA_FALSE;
			}
			else
			{
				rs_info->rs_info_data_len = CM_CAST_U16 (rs_info_block_len);
				CM_MEMCPY (rs_info->rs_info_data, rs_info_block, rs_info_block_len);

				argr->rs_info = rs_info;
	...
	CM_AR_TRACE_01 (argr->trace_idx, LSA_TRACE_LEVEL_UNEXP
		, "cm_ar_graph_insert_rs_info: parse RSInfo block failed, error_code2(0x%x)", *block_field);
	...
	for (CmListForeach(ar_ap, &ar_graph->ap_list, CM_AR_GRAPH_AP_PTR_TYPE)) {
		...
		for (CmListForeach(ar_mod, &ar_ap->mod_list, CM_AR_GRAPH_MODULE_PTR_TYPE)) {
			...
			for (CmListForeach(ar_sub, &ar_mod->submod_list, CM_AR_GRAPH_SUBMODULE_PTR_TYPE)) {
				...
				LSA_UINT16  ips_cnt = 0;
				LSA_UINT16  ics_cnt = 0;
				LSA_UINT16  ops_cnt = 0;
				LSA_UINT16  ocs_cnt = 0;
				...
				case CM_SUB_PROP_TYPE_NO_DATA:
				case CM_SUB_PROP_TYPE_INPUT:
					if (ips_cnt != 1 || ics_cnt != 1 || ops_cnt != 0 || ocs_cnt != 0) {
						return 10; /* wrong releated count */
					}
					break;

				case CM_SUB_PROP_TYPE_OUTPUT:
					if (ips_cnt != 0 || ics_cnt != 0 || ops_cnt != 1 || ocs_cnt != 1) {
						return 10; /* wrong releated count */
					}
					break;

				case CM_SUB_PROP_TYPE_INPUT_OUTPUT:
					if (ips_cnt != 1 || ics_cnt != 1 || ops_cnt != 1 || ocs_cnt != 1) {
						return 10; /* wrong releated count */
					}
					break;
...
cm_ar_graph_check_iocrs_against_expsubmods(
...
#if CM_CFG_MAX_SERVERS /* TIA 1453731 */
	LSA_UINT16 net_data_length_icr = 0; /* NDLI */
	LSA_UINT16 net_data_length_ocr = 0; /* NDLO */
	LSA_UINT16 net_data_length_ar  = 0; /* NDLAR */
#endif
	...
	for (CmListForeach (ar_iocr, & ar_graph->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE)) {
...
#if CM_CFG_MAX_SERVERS /* TIA 1453731, counting changed with TIA 2612940 */
		LSA_UINT16 net_data_length = 0; /* sum of data_length */
		LSA_UINT16 net_ioxs_length = 0; /* sum of iops_length/iocs_length */
		LSA_UINT16 notred_ioxs_length = 0; /* sum of "not reduced" iops_length/iocs_length */
#endif
		...
		for (CmListForeach (ar_ioapi, & ar_iocr->related_ap_list, CM_AR_GRAPH_IOAPI_PTR_TYPE)) {
			...
			for (CmListForeach (ar_iodata, &ar_ioapi->related_iodata_objects_list, CM_AR_GRAPH_IODATA_OBJECT_PTR_TYPE)) {
				...
				if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
					...
					net_data_length += ar_sub->descr[descr_dir].data_length;
					net_ioxs_length += ar_sub->descr[descr_dir].iops_length;

					if( ! reduce_ioxs ) {
						notred_ioxs_length += ar_sub->descr[descr_dir].iops_length;
					}
				}
#endif
			}
			...
			for (CmListForeach (ar_iocs, &ar_ioapi->related_iocs_list, CM_AR_GRAPH_IOCS_PTR_TYPE)) {
				...
				if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
					...
					net_ioxs_length += ar_sub->descr[descr_dir].iocs_length;

					if( ! reduce_ioxs ) {
						notred_ioxs_length += ar_sub->descr[descr_dir].iocs_length;
					}
				}
#endif
			}
		...
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
			...
			ar_iocr->cm.sv.sum_iodata = net_data_length; /* TIA 2612940 */
			ar_iocr->cm.sv.sum_ioxs   = net_ioxs_length; /* TIA 2612940 */

			net_data_length += notred_ioxs_length; /* TIA 1592153 (V6.0i2.7x) */

			/***/

			if( ar_iocr->iocr_type == CM_IOCR_TYPE_INPUT ) {

				if( net_data_length > net_data_length_icr ) {
					net_data_length_icr = net_data_length;
				}
			}

			if( ar_iocr->iocr_type == CM_IOCR_TYPE_OUTPUT ) {

				if( net_data_length > net_data_length_ocr ) {
					net_data_length_ocr = net_data_length;
				}
			}

			net_data_length_ar += net_data_length;
		}
#endif
		...
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
			...
			if( ar_iocr->cm.subframe_data != 0 ) {
				...
				if( cm_edd_can_partial_data(channel, (ar_iocr->iocr_properties & CM_IOCR_PROP_RT_CLASS_MASK)) ) {
					...
					if( ! cm_edd_bug_consumer_partial_data_length_allowed(channel, ar_iocr) ) { /* partial access not allowed */
	...
	if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
		...
		ar_graph->cm.sv.net_data_length = net_data_length_ar;

		/***/

		if( (ar_graph->cm.sv.r1_flags & CM_SV_AR_R1FL_REPRESENTATIVE) != 0 ) { //R1 marker

			/* GSD checks apply to local ARs only (TIA 1964489) */

			ar_graph->cm.sv.net_data_length = 0; /* no resources occupied */
		}
		else {

			CM_SV_DEVICE_PTR_TYPE dev = ar_graph->cm.sv.dev;
			LSA_UINT16 net_data_length_used = 0;
			LSA_UINT16 ar_nr;

			for( ar_nr = 1; ar_nr <= dev->explicit_ar.max_ARs_TOT; ++ar_nr ) {

				CM_AR_GRAPH_PTR_TYPE tmp_argr = cm_sv_ar_lookup2(dev, ar_nr);

				if( cm_is_not_null(tmp_argr) ) { /* note: one of the ARs is "ar_graph" */

					net_data_length_used += tmp_argr->cm.sv.net_data_length;
				}
			}

			if( net_data_length_icr > dev->gsd_MaxInputLength ) {
				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "GSDML violation: too much ICR net data (%u/%u)"
					, net_data_length_icr
					, dev->gsd_MaxInputLength
					);
				return 254;
			}

			if( net_data_length_ocr > dev->gsd_MaxOutputLength ) {
				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "GSDML violation: too much OCR net data (%u/%u)"
					, net_data_length_ocr
					, dev->gsd_MaxOutputLength
					);
				return 254;
			}

			if( net_data_length_used > dev->gsd_MaxDataLength ) {
				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "GSDML violation: too much AR net data (%u/%u)"
					, net_data_length_used
					, dev->gsd_MaxDataLength
					);
				return 254;
			}
		}
...
cm_cl_ar_graph_check_coc_properties (
	...
	LSA_UINT32  next_coc_group;
	LSA_BOOL  more_coc_group_exists;
	...
	do
	{
		...
		LSA_BOOL  coc_group_found = LSA_FALSE;
		LSA_UINT32  coc_ap_found = 0;

		next_coc_group += 1;
		more_coc_group_exists = LSA_FALSE;
		...
		for (CmListForeach (ap, & argr->ap_list, CM_AR_GRAPH_AP_PTR_TYPE))
		{
			for (CmListForeach (mod, & ap->mod_list, CM_AR_GRAPH_MODULE_PTR_TYPE))
			{
				...
				{
					LSA_UINT32  local_coc_group = (sub->cl.sub_coc_properties & CM_SUB_COC_GROUPID_MASK);
					if (local_coc_group != 0)
					{
						if (local_coc_group == next_coc_group)
						{
							coc_group_found = LSA_TRUE;
							if ((sub->cl.sub_coc_properties & CM_SUB_COC_IS_ACCESS_POINT) != 0)
							{
								coc_ap_found += 1;
								if (CM_IS_PDEV_PORT (sub->subslot_nr))
								{
									CM_AR_TRACE_03 (argr->trace_idx, LSA_TRACE_LEVEL_UNEXP
										, "dev(%u) ar(%u) access point(0x%x) of a pdev coc group must be the interface"
										, argr->device_nr, argr->ar_nr, sub->subslot_nr
										);
									return CM_PNIO_FAULTY_ExpectedSubmoduleBlockReq (12/*sub_properties*/);
								}
							}
						}
						else if (local_coc_group > next_coc_group)
						{
							more_coc_group_exists = LSA_TRUE;
						}
					}
				}
			}
		...
		if (coc_group_found)
		{
			if (coc_ap_found == 0  ||  coc_ap_found > 1)
			{
				CM_AR_TRACE_02 (argr->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "dev(%u) ar(%u) illegal count of CombinedObjectContainer AccessPoints"
	...
	)
{
	if( ar_graph->ar.ar_type == CM_AR_TYPE_SINGLE_RTC3
		...
		if (cm_is_null (ar_graph->ir_info))
		{
		...
		else
		{
			...
			if( subCount == 0 ) { /* no DFP */

				CM_AR_GRAPH_IO_CR_PTR_TYPE ar_iocr;
				for (CmListForeach (ar_iocr, &ar_graph->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE)) {

					if( ar_iocr->cm.subframe_data != 0 ) {
						return CM_PNIO_FAULTY_IRInfoBlock(7); /* CL: user-bug, SV: unreachable */
					}
				}
			}
			else if( subCount == 2 ) {
				...
				for( CmListForeach (ar_iocr, &ar_graph->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE) ) {
					...
					case CM_IOCR_TYPE_MULTICAST_CONSUMER:
					default:
						return CM_PNIO_FAULTY_IRInfoBlock(0); /* not supported */
					}

					if( ar_iocr->cm.subframe_data == 0 ) {
						return CM_PNIO_FAULTY_IRInfoBlock(8); /* invalid or duplicate iocr_ref  */
					}

					/***/

					{
					CM_AR_GRAPH_IR_INFO_SUB_PTR_TYPE ir_info_sub = cm_ar_graph_find_ir_info_sub(ar_graph, ar_iocr->iocr_ref);

					if( cm_is_null(ir_info_sub) ) {
						return CM_PNIO_FAULTY_IRInfoBlock(8); /* IOCRReference: no matching IOCRBlockReq or not unique */
					}

					if (ar_iocr->cm.subframe_data   != (ir_info_sub->subframe_data & 0xFF7F)
					||	ar_iocr->cm.subframe_offset != ir_info_sub->subframe_offset)
					{
						CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR
							, "subframe_data, cm-user has not set correctly, see cm_arcb_calc_iocr_buffer_size()"
							);
						return CM_PNIO_FAULTY_IRInfoBlock(0); /* CL: cm-user parameter error, SV: unreachable */
					}
					}
					...
					{
					LSA_UINT32 sf_len = ((ar_iocr->cm.subframe_data >> 8) & 0xFF);

					if( (ar_iocr->cm.subframe_data & 0x7F) == 0 ) {
						return CM_PNIO_FAULTY_IRInfoBlock(10); /* unreachable, see cm_ar_graph_insert_ir_info_sub() */
					}
					...
					if( ar_iocr->cm.subframe_offset + 2 /* SFCRC16 */ + 2 /* SFEndDelimiter */ >= ar_iocr->data_length ) {
						CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR
					...
					if( ar_iocr->cm.subframe_offset + sf_len + 2 /* SFCRC16 */ + 2 /* SFEndDelimiter */ > ar_iocr->data_length ) {
						CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR
				...
				if (!(inp == 1 && out == 1))
				{
					return CM_PNIO_FAULTY_IRInfoBlock(7); /* NumberOfIOCRs */
				}
			}
			else
			{
				return CM_PNIO_FAULTY_IRInfoBlock(7); /* NumberOfIOCRs: not 0 or 2, CL: user-bug, SV: unreachable */
			}
		}
	}
	else
...
cm_ar_graph_check_consistency(
	...
	if( cm_is_iosar_with_device_access(ar_graph) ) {
		...
		if( inp == 0 || out == 0 ) {
			...
			return CM_PNIO_FAULTY_Connect__CMRPC_IOCRMissing();
		}

		if( inp > 1 || out > 1 ) { /* N-IOCRs added with V7.0 (TIA 1684352) */

			if( (ar_graph->ar.ar_properties & CM_AR_PROP_STARTUP_MODE_ADVANCED) == 0 ) {

				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "ICR(%u)/OCR(%u), legacy ARs support only 1 input and 1 output", inp, out
					);

				return CM_PNIO_FAULTY_Connect__CMRPC_OutOfProviderConsumerOrAlarmResources(); /* legacy supports only 1 input and 1 output */
			}

			if( ar_graph->ar.ar_type == CM_AR_TYPE_SINGLE_SYSRED ) { //niocr: remove check with TIA 1702667

				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "ICR(%u)/OCR(%u), only 1 input and 1 output supported", inp, out
					);

				return CM_PNIO_FAULTY_Connect__CMRPC_OutOfProviderConsumerOrAlarmResources(); /* only 1 input and 1 output supported (AP00774504) */
			}
		}
		...
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER ) {
			...
			CM_SV_DEVICE_PTR_TYPE dev = ar_graph->cm.sv.dev;
			LSA_UINT16 ar_nr;
			LSA_UINT16 total_inp = 0;
			LSA_UINT16 total_out = 0;
			LSA_UINT16 total_mprov = 0;
			LSA_UINT16 total_mcons = 0;

			if( inp > 1 + dev->gsd_NumberOfAdditionalInputCR
				|| out > 1 + dev->gsd_NumberOfAdditionalOutputCR
				|| mprov > dev->gsd_NumberOfMulticastProviderCR
				|| mcons > dev->gsd_NumberOfMulticastConsumerCR
			) {

				CM_AR_TRACE_08(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "ICR(%u/%u)/OCR(%u/%u)/MPROV(%u/%u)/MCONS(%u/%u), too many CRs for GSDML"
					, inp, dev->gsd_NumberOfAdditionalInputCR
					, out, dev->gsd_NumberOfAdditionalOutputCR
					, mprov, dev->gsd_NumberOfMulticastProviderCR
					, mcons, dev->gsd_NumberOfMulticastConsumerCR
					);

				return CM_PNIO_FAULTY_Connect__CMRPC_OutOfProviderConsumerOrAlarmResources(); /* Not supported by GSDML */
			}

			for( ar_nr = 1; ar_nr <= dev->explicit_ar.max_ARs_IOX/*!*/; ++ar_nr ) {

				CM_AR_GRAPH_PTR_TYPE temp_ar = cm_sv_ar_lookup2(dev, ar_nr);

				if( cm_is_not_null(temp_ar) ) {

					CM_AR_GRAPH_IO_CR_PTR_TYPE temp_iocr;

					for( CmListForeach(temp_iocr, &temp_ar->io_cr_list, CM_AR_GRAPH_IO_CR_PTR_TYPE) ) {

						switch( temp_iocr->iocr_type ) {
			...
			if( total_inp > dev->gsd_NumberOfInputCR
				|| total_out > dev->gsd_NumberOfOutputCR
				|| total_mprov > dev->gsd_NumberOfMulticastProviderCR
				|| total_mcons > dev->gsd_NumberOfMulticastConsumerCR
			) {

				CM_AR_TRACE_08(ar_graph->trace_idx, LSA_TRACE_LEVEL_UNEXP
					, "ICR(total: %u, GSD: %u)/OCR(total: %u, GSD: %u)/MPROV(total: %u, GSD: %u)/MCONS(total: %u, GSD: %u), too many CRs for GSDML"
					, total_inp, dev->gsd_NumberOfInputCR
					, total_out, dev->gsd_NumberOfOutputCR
					, total_mprov, dev->gsd_NumberOfMulticastProviderCR
					, total_mcons, dev->gsd_NumberOfMulticastConsumerCR
				);
	...
	if( block_field != 0 ) {
		...
		case 254: /* NDLI > gsd_MaxInputLength or NDLO > gsd_MaxOutputLength */
			return CM_PNIO_FAULTY_IOCRBlockReq(8); /* V7.1i1.1x, TIA 2771931 updates TIA 1453731 */
	...
	else if( cm_is_null (ar_graph->alarm_cr) ) {
	...
	if( ar_graph->ar.ar_type == CM_AR_TYPE_SINGLE_SYSRED ) {
		...
		if( cm_is_null(ar_graph->sr_info) ) {

			CM_AR_TRACE_00(ar_graph->trace_idx, LSA_TRACE_LEVEL_ERROR
				, "ARType SYSRED needs an SRInfoBlock"
				);
			return CM_PNIO_FAULTY_Connect__CMRPC_UnknownBlocks(); /* note: there is no code2 for "missing blocks" */
		}
	}
	else {
...
cm_ar_graph_free (
	...
	if (cm_is_not_null(ar_graph->ar.cmi_station_name)) {
		CM_ASSERT(ar_graph->ar.cmi_station_name_length != 0);
...
#if CM_CFG_MAX_SERVERS //R1 marker (special case representative-AR)
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER && ((ar_graph->cm.sv.r1_flags & CM_SV_AR_R1FL_REPRESENTATIVE) != 0) ) {
			CM_ASSERT(iocr->cm.cr_state == CM_AR_CR_ALLOCATED); /* note: no CM_ARCQ_OPC_FRAMES_REMOVE */
			iocr->cm.cr_state = CM_AR_CR_ALLOCATED;
		}
#endif
	...
	if( cm_is_not_null(alarm_cr) ) { /* there is at most one (easy-supervisor has none) */

#if CM_CFG_MAX_SERVERS //R1 marker (special case representative-AR)
		if( ar_graph->ar_com.path_type == CM_PATH_TYPE_SERVER && ((ar_graph->cm.sv.r1_flags & CM_SV_AR_R1FL_REPRESENTATIVE) != 0) ) {
			CM_ASSERT(alarm_cr->cm.cr_state == CM_AR_CR_IDLE || alarm_cr->cm.cr_state == CM_AR_CR_ERROR); /* note: "ERROR" if could not create representative-AR */
			alarm_cr->cm.cr_state = CM_AR_CR_ALLOCATED;
		}
#endif

		if( alarm_cr->cm.cr_state != CM_AR_CR_ALLOCATED ) {
	...
	{
	CM_AR_GRAPH_RS_INFO_PTR_TYPE  rs_info = ar_graph->rs_info;
	ar_graph->rs_info = LSA_NULL;

	if( cm_is_not_null(rs_info) ) {

		CM_FREE_LOCAL_MEM (& ret_val, rs_info);
		CM_ASSERT(ret_val == LSA_RET_OK);
	}
	}
...
cm_ar_graph_find_iodata (
	...
	CM_ASSERT (cm_is_not_null (ioapi));
...
cm_ar_graph_find_module (
	...
	CM_ASSERT (cm_is_not_null (ar_ap));
...
cm_ar_graph_find_submodule (
	CM_AR_GRAPH_AP_CONST_PTR_TYPE  ap, /* in: if ap == NULL then mod must specified */
	...
	CM_ASSERT (! (cm_is_null (ap) && cm_is_null (mod)));
...
)
{
	...
	if (cm_is_not_null (ap_pp)) {
		*ap_pp = LSA_NULL;
	}
	if (cm_is_not_null (mod_pp)) {
		*mod_pp = LSA_NULL;
	}
	if (cm_is_not_null (sub_pp)) {
		*sub_pp = LSA_NULL;
	}
	...
	{
		mod = cm_ar_graph_find_module (ap, slot_nr);
		if (cm_is_not_null (mod) && cm_is_not_null (sub_pp))
		{
...
)
{
	...
	for (CmListForeach(ap, &ar_graph->ap_list, CM_AR_GRAPH_AP_PTR_TYPE)) {
		...
		for (CmListForeach(module, &ap->mod_list, CM_AR_GRAPH_MODULE_PTR_TYPE)) {
			...
			for (CmListForeach(submodule, &module->submod_list, CM_AR_GRAPH_SUBMODULE_PTR_TYPE)) {
...
#if CM_CFG_MAX_CLIENTS
				CM_AR_TRACE_02(ar_graph->trace_idx, LSA_TRACE_LEVEL_CHAT
					, "      cl.real_sub_ident(%u 0x%x)", submodule->cl.real_sub_ident, submodule->cl.real_sub_ident
					);
				CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_CHAT
					, "      cl.sub_state(0x%x)", submodule->cl.real_sub_state
					);
#endif
				...
				{
					CM_PD_RECORD_PTR_TYPE  record;

					for (CmListForeach(record, &submodule->cl.prm_record_list, CM_PD_RECORD_PTR_TYPE)) {
						CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_CHAT
	...
	if (cm_is_not_null (ar_graph->rs_info))
	{
		CM_AR_TRACE_01(ar_graph->trace_idx, LSA_TRACE_LEVEL_CHAT
			, "RSInfoBlock: rs_info(0x%08x)"
			, ar_graph->rs_info
			);
	}
...
LSA_UINT32 CmUnitTestHelper_cm_ar_graph_check_ir_info_block(
	CM_AR_GRAPH_CONST_PTR_TYPE ar_graph
) {
	return cm_ar_graph_check_ir_info_block(ar_graph);
}
