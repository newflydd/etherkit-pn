...
cm_oha_channel_init(
	...
	CM_ASSERT(channel->path_type == CM_PATH_TYPE_OHA);
...
cm_oha_channel_undo_init(
	...
	if( cm_is_not_null(channel->lower.channel[CM_PATH_TYPE_SERVER]) ) {
...
)
{
	...
	CM_OHA_LOWER_RQB_PTR_TYPE lower_rqb_ptr;
		...
		/* alloc the RQB */
	CM_OHA_ALLOC_LOWER_RQB (&lower_rqb_ptr, cm_null_user_id, sizeof (CM_OHA_LOWER_RQB_TYPE), oha_channel->sysptr);
...
#if CM_DEBUG
	CM_OHA_SET_NEXT_RQB_PTR (lower_rqb_ptr, LSA_NULL); /* sanity */
	CM_OHA_SET_PREV_RQB_PTR (lower_rqb_ptr, LSA_NULL); /* sanity */
#endif
...
)
{
	...
	CM_ASSERT (cm_oha_channel_get(channel) == channel);
...
)
{
	...
	CM_ASSERT (cm_is_not_null(channel->lower.prealloc.oha));
...
cm_oha_address_indication(
	...
	if( cm_is_null(oha) ) /* provide */
	{
		...
		CM_OHA_ALLOC_LOWER_MEM (&lower_mem
		...
		oha->args.AddrInfo.NoS.pName = (OHA_UPPER_MEM_U8_PTR_TYPE)lower_mem;
		...
		oha->args.AddrInfo.CurrentIPSuiteIsEmergency = (channel->usr.oha.is_IPSuite_emergency ? OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES : OHA_CURRENT_IPSUITE_IS_EMERGENCY_NO);
		oha->args.AddrInfo.CurrentIPSuiteConflictDetected = channel->usr.oha.is_IPSuite_conflicting ? OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES : OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_NO;
		...
		oha->args.AddrInfo.FactoryReset = OHA_FACTORY_RESET_NO;
	...
	else /* handle indication and do re-provide */
	{
		...
		else
		{
				...
				(CM_COMMON_MEM_U8_PTR_TYPE)(&addr->CurrentIPSuite), sizeof(addr->CurrentIPSuite)) != 0)
			{
				...
				if( CM_MEMCMP (channel->usr.oha.IPSuite.IpAddr, zeroIpAddr, sizeof(zeroIpAddr)) == 0 ) {
					channel->usr.oha.is_IPAddr_null = LSA_TRUE;
				}
				else {
			...
			if (channel->usr.oha.is_IPSuite_emergency != (addr->CurrentIPSuiteIsEmergency == OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES ? LSA_TRUE : LSA_FALSE))
			{
				CM_OHA_TRACE_02 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE
				...
				channel->usr.oha.is_IPSuite_emergency = (addr->CurrentIPSuiteIsEmergency == OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES ? LSA_TRUE : LSA_FALSE); /* update */
				ip_changed = LSA_TRUE;
			}

			if (channel->usr.oha.is_IPSuite_conflicting != (addr->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE))
			{
				CM_OHA_TRACE_02 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE
				...
				channel->usr.oha.is_IPSuite_conflicting = (addr->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE); /* update */
				ip_changed = LSA_TRUE;
			}
			...
			||  CM_MEMCMP (channel->usr.oha.name_of_station, addr->NoS.pName, addr->NoS.Length) != 0
			)
			{
				CM_OHA_TRACE_02 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE
			...
			if (addr->FactoryReset == OHA_FACTORY_RESET_YES)
			{
				...
				addr->FactoryReset = OHA_FACTORY_RESET_NO; /* TIA 1525038 */
			}
...
cm_oha_control_ip_suite(
	...
	if( cm_is_null(oha) || CM_OHA_GET_OPCODE(oha) != 0/*unused*/ ) {
...
cm_oha_write_ptcp_data(
	...
	oha->args.PtcpData.DoSync = do_sync_clock ? OHA_DO_SYNC_YES : OHA_DO_SYNC_NO;
	oha->args.PtcpData.IsSync = is_sync ? OHA_IS_SYNC_YES : OHA_IS_SYNC_NO;
...
cm_oha_register_client (
	...
	CM_ASSERT (oha_channel->path_type == CM_PATH_TYPE_OHA);

	if (cm_is_not_null (cl_channel)) {
		CM_ASSERT (cm_is_null (oha_channel->lower.channel[CM_PATH_TYPE_CLIENT]));
		oha_channel->lower.channel[CM_PATH_TYPE_CLIENT] = cl_channel; /* register */
	}
	else {
		CM_ASSERT (cm_is_not_null (oha_channel->lower.channel[CM_PATH_TYPE_CLIENT]));
...
cm_oha_hello_indication(
	...
	if( cm_is_null(oha) ) /* provide */
	{
		...
		for( cnt = 0; cnt < max; ++cnt )
		{
			...
			CM_ASSERT (channel->path_type == CM_PATH_TYPE_OHA); /* see sysptr in alloc/free */
			...
			oha->args.HelloInd.pStationName		= (OHA_UPPER_MEM_PTR_TYPE)lower_mem;
	...
	if (cm_is_null (oha)) { /* provide */
		...
		for (port_id = 0; port_id <= edd_params->MaxPortCnt; port_id++)
		{
			...
			for( cnt = 0; cnt < max; ++cnt )
			{
				...
				oha->args.PrmInd.diag_ptr = (OHA_UPPER_PRM_DIAG_ENTRY_PTR_TYPE)lower_mem;
...
cm_oha_prm_write (
	...
	oha->args.PrmData.mk_rema = mk_rema ? OHA_REMANENT_YES : OHA_REMANENT_NO;
...
cm_oha_prm_commit (
	...
	oha->args.PrmControl.is_local = is_local ? OHA_IS_LOCAL_YES : OHA_IS_LOCAL_NO;

	/* AP01504165: PrmControl.do_rtf removed, function parameter "do_reset_to_factory" removed */

	cm_pd_prm_set_prm_port_flags (channel, oha->args.PrmControl.ApplyDefault
		, sizeof (oha->args.PrmControl.ApplyDefault), sizeof (oha->args.PrmControl.ApplyDefault[0])
	...
	)
{
	...
	oha->args.PrmState.PortId = channel->usr.pd.pullplug.port->port_id;
	oha->args.PrmState.ModuleState = submodule_state;
	oha->args.PrmState.ApplyDefault = apply_default_portparams;

	cm_oha_request_lower (channel, OHA_OPC_PRM_CHANGE_PORT, 0, 0, oha);
}

/*----------------------------------------------------------------------------*/
static LSA_VOID
cm_oha_pullplug_change_port_done (
	CM_CHANNEL_PTR_TYPE  channel,
	CM_OHA_LOWER_RQB_CONST_PTR_TYPE  oha
	)
{
	if (cm_pd_sc_map_response(LSA_COMP_ID_OHA, CM_OHA_GET_RESPONSE (oha)) != CM_OK)
	{
		CM_FATAL1 (CM_OHA_GET_RESPONSE (oha));
	}

	cm_pd_pullplug_sm_trigger (channel, CM_PD_SCBF_OHA, oha->args.PrmState.NotApplicable);
}
...
cm_oha_request_lower_done (
	...
	case CM_PATH_TYPE_PD:
		...
		case OHA_OPC_PRM_CHANGE_PORT:
			cm_oha_pullplug_change_port_done (channel, oha);
			break;
