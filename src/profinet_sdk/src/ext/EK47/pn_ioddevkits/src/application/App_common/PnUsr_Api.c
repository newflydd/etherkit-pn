...
// *
// *  3) Note, that the hardware-based buffer lock mechanism on the ERTEC400 is limited to 255 byte,
// *     including all IO data, IOPS and IOCS of an IOCR.
// *
// *--------------------------------------------------------------------------
...
#if (EXAMPL_DEV_CONFIG_VERSION <= 9) || (21 == EXAMPL_DEV_CONFIG_VERSION)
#include "pniousrd.h"
...
PNIO_UINT32 NumOfAr = 0;
PNIO_UINT8 PNIO_WD_triggering = 0;
PNIO_UINT8 PNIO_dynamic_cfg = 0;
PNIO_UINT8 PNIO_automatic_alrms = 0;
PNIO_UINT8 PNIO_alrm_in_progress = 0;
// *----------------------------------------------------------------*
// *
// *  MainAppl(void)
...
void PnUsr_cbf_iodapi_event_varinit(PNIO_SUB_LIST_ENTRY *pIoSubList, PNIO_UINT32 NumOfSubListEntries);
...
// *------------ local data  ----------*
PNIO_UINT32 TskId_CycleIO = 0;        // task id for executing cyclic data exchange
PNIO_UINT32 Task_CycleIO_Active = 0;  // activate/deactivate cyclic data exchange
PNIO_UINT32 TransEndSemId = 0;        // semaphore to trigger io thread at newCycle
...
PNIO_ANNOTATION* UsrIod_BuildDeviceAnnotation(void) {
	...
	IOD_CFG_DEV_ANNOTATION_ORDER_ID, OsStrLen(IOD_CFG_DEV_ANNOTATION_ORDER_ID));

	OsMemCpy(DevAnnotation.SerialNumber,
	IOD_CFG_IM0_SERIAL_NUMBER, OsStrLen(IOD_CFG_IM0_SERIAL_NUMBER));
...
// *
// *----------------------------------------------------------------*
void PNS_CycleIoTimercallback(int timer_id, int user_id) {
	if (0 < TransEndSemId) {
#if (PNIOD_PLATFORM	 == PNIOD_PLATFORM_RZT2_EDDP)
		//	OsTaskNotify(TskId_CycleIO, PNIO_EV_CYCL);
		//	OsTaskNotify(TskId_CycleIO, PNIO_EV_CYCL|PNIO_EV_TI|PNIO_EV_TO);
#else
			OsGiveSemB (TransEndSemId);
#endif
	}
...
extern LSA_VOID PNS_IO_DatXchOutput(PNIO_UINT32 DevHndl);
extern LSA_VOID PNS_IO_DatXchInput(PNIO_UINT32 DevHndl);

OS_CODE_FAST static PNIO_INT32 Task_CycleIO(void) {
	PNIO_UINT16 IoTimerId = 0;
	PNIO_UINT32 u32Events = 0;
	// *----------------------------------------------------------
	// * Synchronization to parent process
	// *----------------------------------------------------------
	OsWaitOnEnable();		    // must be first call in every task

	OsAllocTimer((PNIO_UINT16*) &IoTimerId, LSA_TIMER_TYPE_CYCLIC, LSA_TIME_BASE_1MS, (PNIO_VOID*) PNS_CycleIoTimercallback);
	OsStartTimer(IoTimerId, TransEndSemId, 1);

	//    PnUsr_ActivateIoDatXch();   // start cyclic io data exchange

	while (1) {
		// **** receive a cyclic trigger, to start initiate data rd/wr ***
		u32Events = OsTaskNotifyWait(TskId_CycleIO);
		if (0U < ((PNIO_EV_TO_VALID | PNIO_EV_TO | PNIO_EV_TI_VALID | PNIO_EV_TI) & u32Events)) {
			PNIO_initiate_data_read(PNIO_SINGLE_DEVICE_HNDL);
			PNIO_initiate_data_write(PNIO_SINGLE_DEVICE_HNDL);
		}
	}
...
static PNIO_UINT32 PlugSubmodules(PNIO_SUB_LIST_ENTRY *pIoSubList,           // plugged submodules, including PDEV
		...
		PNIO_UINT32 NumOfIm0ListEntries)   // number of entries in pIm0List
{
	...
	// *--------------------------------------------------------------
	// TODO: changed by dingding
//         Status = Bsp_nv_data_restore (PNIO_NVDATA_DEVICENAME,  // data type
//                                       (PNIO_VOID**) &pNvDevName,             // data pointer (allocated by
//                                       &StationNameLen);
	StationNameLen = 8;
	pNvDevName = (PNIO_UINT8*) malloc(sizeof(PNIO_UINT8) * StationNameLen);
	memcpy(pNvDevName, "mcp-pn64", StationNameLen);
...
#define SNMP_SYSTEM_DESCRIPTION_ACCORDING_TO_SPH
		...
		/*******************************************************************/
		/* should match "OID : 1.0.8802.1.1.2.1.3.2.0 "lldpLocChassisId"". */
		/*******************************************************************/
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) "%-25s", IOD_CFG_DEVICE_TYPE); /* DeviceType VisibleString[25] */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) " "); /* Blank */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) "%-20s", DEVKIT_ORDERNUMBER); /* OrderID VisibleString[20] */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) " "); /* Blank */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) "%-16s", DEVKIT_PRODUCTSERIALNUMBER); /* IM_Serial_Number VisibleString[16] */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) " "); /* Blank */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) "%5d", DEVKIT_HW_REVISION); /* HWRevision(The range is from “<Blank><Blank><Blank><Blank>0” to “99999”.) VisibleString[5] */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) " "); /* Blank */
		SysDescrBuff_size += PNIO_sprintf(&SysDescrBuff[SysDescrBuff_size], (PNIO_INT8*) "%c  %d  %d  %d", DEVKIT_VERSION_PREFIX, /* SWRevisionPrefix */
		DEVKIT_VERSION_HH, /* SWRevision------------------------------------------------------------------------------------ */
		DEVKIT_VERSION_H, /* The value shall be set manufacturer specific using the character “0”-“9” and “<Blank>”.  */
		DEVKIT_VERSION_L); /* The range is from “<Blank><Blank>0<Blank><Blank>0<Blank><Blank>0” to “999999999”.          */
#endif
		...
		SnmpPar.PortNameLen2 = OsStrLen(IOD_CFG_SNMP_PORT2_NAME);
                ...
                SnmpPar.pPortName3   = IOD_CFG_SNMP_PORT3_NAME;
                SnmpPar.PortNameLen3 = OsStrLen (IOD_CFG_SNMP_PORT3_NAME);
            #endif

#if (IOD_CFG_PDEV_NUMOF_PORTS >= 4)
                SnmpPar.pPortName4   = IOD_CFG_SNMP_PORT4_NAME;
                SnmpPar.PortNameLen4 = OsStrLen (IOD_CFG_SNMP_PORT4_NAME);
            #endif
		...
		PNIO_setup((PNIO_INT8*) pNvDevName,   // device name, saved in persistent memory
				StationNameLen,            // size of station name (may be not zero terminated)
	...
	Bsp_EbSetLed(BSP_LED_BUS_RUN, 0);
	...
	// *    happened
	// *-----------------------------------------
	if (Status != PNIO_OK) { // **** user specific error handling ****
		PNIO_printf((PNIO_INT8*) "error in PROFINET IO Setup \n");
	}
		...
		PNIO_UINT32 NumOfIm0ListEntries) // number of entries in pIm0List
{
	...
	// *----------------------------------------------------------
	Status = OsCreateThread((void (*)(void)) Task_CycleIO, (PNIO_UINT8*) "Pnio_CycleIO", TASK_PRIO_APPL_CYCLE_IO, &TskId_CycleIO);
	...
	// *    to a PNIO controller.
	// *-----------------------------------------
	Status = StartupPnioDevice(pPnUsrDev, pIoSubList, NumOfSublistEntries, pIm0List, NumOfIm0ListEntries);
	...
	PrintDevkitVersion();
        ...
        {
            PNIO_ConsolePrintf ( (PNIO_INT8*) "ERTEC200P PARITY ERROR: COUNT=%d, SOURCE=0x%08X, EDC_EVENT=0x%08X\n",
            					  tskma_parity_error_count, tskma_parity_error_source, *((unsigned long*) U_SCRB__EDC_EVENT));
        }
        if (tskma_access_error_count)
        {
            PNIO_ConsolePrintf ( (PNIO_INT8*) "ERTEC200P ACCESS ERROR: COUNT=%d, SOURCE=0x%08X\n",
...
// *----------------------------------------------------------------*
PNIO_UINT32 PNIO_cbf_rec_read(PNIO_UINT32 DevHndl, PNIO_UINT32 Api, PNIO_UINT16 ArNum,			// ar - handle
		PNIO_UINT16 SessionKey,	    // ar session number
		PNIO_UINT32 SequenceNum, PNIO_DEV_ADDR *pAddr,			// [in] geographical or logical address
		PNIO_UINT32 RecordIndex, PNIO_UINT32 *pBufLen,		// [in,out] length to read, out: length, read by user
		PNIO_UINT8 *pBuffer,		// [in] buffer pointer
	...
	case 0x80a0:
		...
		// *----------------------------------------------*
	default:
		Status = (PnUsr_cbf_rec_read(DevHndl, Api, ArNum, SessionKey, SequenceNum, pAddr, RecordIndex, pBufLen, pTmpBuf, pPnioState));
		break;
	}
...
// *			*pBuf		    copied Record data
// *----------------------------------------------------------------*
PNIO_UINT32 PNIO_cbf_rec_write(PNIO_UINT32 DevHndl,        // device handle
		PNIO_UINT32 Api,            // application process identifier
	...
	case 0x8030:
		Status = PnUsr_cbf_rec_write(DevHndl, Api, ArNum, SessionKey, SequenceNum, pAddr, RecordIndex, pBufLen, pBuffer, pPnioState);
		break;
	default:
		if (RecordIndex <= 0x7fff) {  // *** transfer user specific indizes ***
			Status = PnUsr_cbf_rec_write(DevHndl, Api, ArNum, SessionKey, SequenceNum, pAddr, RecordIndex, pBufLen, pBuffer, pPnioState);
