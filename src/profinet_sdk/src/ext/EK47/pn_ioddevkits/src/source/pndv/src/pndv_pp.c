...
void pndv_pp_sm(PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T res_ptr, PNDV_CFG_SUBMODULE_RES_EVENT_T event)
{
            ...
            switch (event)
            {
                case PNDV_CFG_SUBMODULE_RES_EVENT_PERI_ADD_REQ:
                {
                    ...
                    pndv_in_write_debug_buffer_3__( PNDV_DC_COMMON_MODUL_INFO,
                                                    ...
                                                    ((Unsigned32)res_ptr->sub_module.mod_ident)|((Unsigned32)res_ptr->sub_module.sub_ident)<<16
                                                   );
                    break;
                }
                default:
    ...
    pndv_in_write_debug_buffer_3__(PNDV_DC_PNDV_PP_SM, event, ((Unsigned16)old_state) | (((Unsigned16)res_ptr->res_state)<<8), (Unsigned32)res_ptr);
}
 ...
 *  @param[in]  more_follows TRUE = more pulls will follow, FALSE = this is the last pulled submodule
 *
 *  long_description
 *
 */
 ...
 */
void pndv_pp_tool_submodule_add_remove_do_request()
{
    PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T local_sub_res;
    ...
    if(local_sub_res->res_state == PNDV_CFG_SUBMODULE_RES_STATE_W_ADD)
    {
        ...
        PNDV_RQB_SET_HANDLE(&pndv_data.rqb.sub_add_rem, pndv_data.cm_handle[PNDV_INDEX_PATH_ACP_CM]);
    ...
    PNDV_CM_REQUEST( &pndv_data.rqb.sub_add_rem );
}
 ...
 *
 *  @param[in]  entity Index of a submodule entity within interface
 *  @param[in]  more_follows TRUE = more plugs will follow, FALSE = this is the last plugged submodule
 *
 *  long_description
 *
 */
...
UnsignedOpt pndv_pp_tool_check_sub_is_local_pd(PNDV_STRUCT_CFG_SUBMODULE_RES_PTR_T res_ptr)
{
    ...
    {
        Unsigned16 local_subslot_nr = 0x8000;
        #ifdef PNDV_CFG_ENABLE_R1
        if( pndv_r1_bridge_enable_bridge() )
        {
            local_subslot_nr = 0x8000+(PNDV_IM_SLOT_NO*0x1000);     // connection between slot and subslot is R1-specific! ET200MP has subslot 0x8000 on slot 1
        }
        #endif
