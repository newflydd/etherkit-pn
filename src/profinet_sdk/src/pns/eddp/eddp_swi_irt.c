	...
	LSA_UINT32                  PortID)
{
    ...
    if ((PortID > 0) && (PortID <= pDDB->SWI.PortCnt))
    {
        ...
        switch (pDDB->SWI.IRTtopFwMode)
        {
            ...
            {
                /* ------------------------------------------------------------------------------------ */
                /* IRT_DelayTime = MaxBridgeDelay + (MaxLineRxDelay ELineRxDelay) - Compensation       */
                /* ------------------------------------------------------------------------------------ */
...
/*
    Bedingungen                                                | Preamble        | Expected?
       ...
       record present                                          |                 |
    -----------------------------------------------------------|-----------------|----------------
    a) EE                    don´t care          don´t care  | long Preamble   | OK
    b) EE                    no                  don´t care  | long Preamble   | NOT_OK
    c) EE                    don´t care          NOT_OK      | long Preamble   | NOT_OK
    d) EE                    yes                 OK          | short Preamble  | OK   (to a PNIP Port)
    e) EE                    yes                 OK          | long Preamble   | OK   (e.g. to a CP1616 Port)
*/
    if ((PortID > 0) && (PortID <= pDDB->SWI.PortCnt))
    {
