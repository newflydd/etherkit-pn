/*
 * v3.1 August 30, 2011 tcp/udpsock.c
...
#define LTRC_ACT_MODUL_ID 31 /* INICHE_TCIP_ITGR added for LSA traces */
...
#if 0 /* INICHE_TCIP_ITGR lock/unlock moved outside function */
   ...
   struct mbuf *  nam)
{
   ...
   case PRU_ATTACH:
      ...
      so->so_rcv.sb_hiwat = UDP_RCVBUF_HIWAT;
      ...
      {
         so->so_snd.sb_hiwat = udp_maxalloc();
        UNLOCK_NET_RESOURCE(NET_RESID);
      ...
      {
         so->so_snd.sb_hiwat = udp6_maxalloc();
        UNLOCK_NET_RESOURCE(NET_RESID);
        udpconn = udp6_open(0L, 0, udp_socket(), udp6_soinput, so);
        LOCK_NET_RESOURCE(NET_RESID);
      }
#endif
      return ((udpconn) ? 0 : EINVAL);
...
int udp4_sockbind(struct socket *so, struct mbuf *nam, int req ) 
{
  ...
  if (req == PRU_BIND)
  {
     ...
     * existing binding
     */
#if 1 /* INICHE_TCIP_ITGR see AP01549419 */
	if (!(so->so_options & SO_REUSEADDR))
	{
		for (udptmp = firstudp; udptmp; udptmp = udptmp->u_next)
		{
			if (udptmp != udpconn
				&& udptmp->u_lport == lport /* bind fails if existing socket has same port */
				&& (udptmp->u_lhost == lhost /* and if existing socket has same address */
				    || udptmp->u_lhost == INADDR_ANY /* or if existing socket has INADDR_ANY */
				    || lhost == INADDR_ANY /* or if new socket has INADDR_ANY */
					)
				)
			{
				return(EADDRINUSE);
		...
		struct mbuf *nam ) 
{
  ...
  else
  {
    ...
    else  /* use port already set in UDP connection */
    {
	  ...
	  {
		  m_free(m); /* Bugfix für RQ 3993434 */
		  return (EINVAL);
	  }
      fport = udpconn->u_fport;
    }
