...
struct cm_list_entry_tag {
	struct cm_list_entry_tag * Flink; /* forward link */
	struct cm_list_entry_tag * Blink; /* backward link */
};

typedef struct cm_list_entry_tag  CM_LIST_ENTRY_TYPE;

typedef struct cm_list_entry_tag  *CM_LIST_ENTRY_PTR_TYPE;

typedef struct cm_list_entry_tag  const *CM_LIST_ENTRY_CONST_PTR_TYPE;
...
typedef CM_LIST_EMPTY_TYPE * CM_LIST_EMPTY_PTR_TYPE;
...
/* not tested */
#define CmListRemoveForEntry(Temp_, ForLoopEntry_, Type_) {\
	Temp_  = ForLoopEntry_;\
	ForLoopEntry_ = CmListPrevOrHead((CM_LIST_ENTRY_PTR_TYPE)ForLoopEntry_, Type_);\
	CmListRemoveEntry((CM_LIST_ENTRY_PTR_TYPE)Temp_);\
	}
