...
LSA_VOID gdma_init()
{
    ...
    *GDMA_JC_EN = 0;
    ...
    LSA_MEMSET( (LSA_UINT8 *) GDMA_JOBX_CTRL(0), 0, GDMA_MAX_JOBS * 4 );
    ...
    *GDMA_IRQ_STATUS = 0xFFFFFFFF;
    ...
    LSA_MEMSET( (LSA_UINT8*) GDMA_LIST_RAM_START, 0, GDMA_LIST_RAM_END - GDMA_LIST_RAM_START );
    // init job stack ram
    // 32 jobs, one job = 4 registers * 32bit
    // 32 * 4 * 4byte = 512byte --> 0x0200
    LSA_MEMSET( (LSA_UINT8*) GDMA_JOB_STACK_RAM_START, 0, GDMA_JOB_STACK_RAM_END - GDMA_JOB_STACK_RAM_START );
#else
 //   gdma_init_hw();
#endif
    /* reset DMA meta data */
    LSA_MEMSET(gdma_data.gdma_used_jobs_map,                0, sizeof(gdma_data.gdma_used_jobs_map));
    LSA_MEMSET(gdma_data.gdma_disabled_jobs_map,            0, sizeof(gdma_data.gdma_disabled_jobs_map));
    LSA_MEMSET(gdma_data.gdma_transfer_list_memory_map,     0, sizeof(gdma_data.gdma_transfer_list_memory_map));
    LSA_MEMSET(gdma_data.gdma_software_triggered_jobs_map,  0, sizeof(gdma_data.gdma_software_triggered_jobs_map));

    #if GDMA_CFG_NUM_JOB_LISTS > 1
    LSA_MEMSET(gdma_data.gdma_ext_mem, 0, sizeof(gdma_data.gdma_ext_mem));
    #endif
    LSA_MEMSET(gdma_data.gdma_job_done_callbacks,       0, sizeof(gdma_data.gdma_job_done_callbacks));
    LSA_MEMSET(gdma_data.gdma_callback_args,            0, sizeof(gdma_data.gdma_callback_args));
...
LSA_INT16 gdma_add_job(LSA_UINT8 job_pool_index, gdma_job_descriptor_t* job_desc)
{
    ...
    GDMA_EXIT_RETURN(job_num + job_pool_index * GDMA_MAX_JOBS);       /* first job set : IDs 0-31, second job set 32-63, ... */
#endif
    return 0;
...
LSA_INT16 gdma_add_transfers_to_job(LSA_INT16 job_num_global, gdma_add_transfers_descriptor_t *add_transfers_desc)
{
    ...
    LSA_UINT32              job_control_regs_base_addr;
    gdma_jobx_ctrl_reg_t    job_reg;
    LSA_UINT16              job_pool_index = job_num_global / GDMA_MAX_JOBS;
    LSA_INT16               job_num;
    ...
    offset = job_reg.bits.transfer_ptr;
...
LSA_INT16 gdma_remove_job(LSA_INT16 job_num)
{
    ...
    /* remove transfer list entries and free allocated resources in the transfer list memory map */
    {
        ...
        LSA_MEMSET(&count_field_zero, 0, sizeof(gdma_trans_recx_cnt_reg_t));
...
LSA_INT16 gdma_switch_to_job_pool(LSA_UINT8 job_pool_index)
{
    ...
    LSA_MEMCPY(gdma_data.gdma_job_control_reg_set[gdma_data.gdma_active_job_pool_idx], (LSA_VOID*) GDMA_JOBX_CTRL(0), GDMA_MAX_JOBS * sizeof(gdma_jobx_ctrl_reg_t));

    /* copy the new set of job control registers to the hardware registers */
    LSA_MEMCPY((LSA_VOID*) GDMA_JOBX_CTRL(0), &gdma_data.gdma_job_control_reg_set[job_pool_index][0], GDMA_MAX_JOBS * sizeof(gdma_jobx_ctrl_reg_t));
...
/**
 * @brief   This function activates all jobs of the current job set. The jobs can be triggered via hardware or software afterwards.
 */
LSA_VOID gdma_enable_all_jobs()
{
    LSA_UINT16 job_num;

    GDMA_ENTER();

    /* enable all hardware triggers and cancel all software trigger blockades */
    for(job_num = 0; job_num < GDMA_MAX_JOBS; job_num++)
    {
        if(gdma_data.gdma_used_jobs_map[gdma_data.gdma_active_job_pool_idx][job_num / 32] & (1 << (job_num % 32)))
        {
            if((gdma_data.gdma_software_triggered_jobs_map[gdma_data.gdma_active_job_pool_idx][job_num / 32] & (1 << (job_num % 32))) == 0)
            {
                GDMA_JOBX_CTRL(job_num)->bits.hw_job_start_en = 1;     /* job may be triggered by hardware again */
            }
            gdma_data.gdma_disabled_jobs_map[gdma_data.gdma_active_job_pool_idx][job_num / 32]  &= ~(1UL << (job_num % 32));  /* job may be triggered by software again */
...
#if 0
/**
 * @brief   Using this function, you can update the user argument, that shall be passed
 *          with a job-done callback.
 * @param job_num       job identifier, that was returned by gdma_add_job()
 * @param new_argument  new user argument, that is passed with each each call to the corresponding job callback.
 */
LSA_INT16 gdma_update_job_done_cbf_arg(LSA_INT16 job_num, LSA_UINT32 new_argument)
{
    LSA_UINT16  job_pool_index = job_num / GDMA_MAX_JOBS;
    job_num     = job_num % GDMA_MAX_JOBS;
    ...
    gdma_data.gdma_callback_args[job_pool_index][job_num] = new_argument;
...
static LSA_INT16 gdma_setup_transfer_records(LSA_UINT32 transfer_list_base_addr, LSA_UINT16 offset, LSA_UINT16 num_transfers, gdma_transfer_descr_t *transfers)
{
    ...
    for(transfer_idx = 0; transfer_idx < num_transfers; transfer_idx++)
    {
        ...
        GDMA_ASSERT(( GDMA_TRANS_RECX_SRC(transfer_list_base_addr, offset + transfer_idx) ) != LSA_NULL, GDMA_ERR_MEM);
        *GDMA_TRANS_RECX_SRC(transfer_list_base_addr, offset + transfer_idx) = (LSA_UINT32) transfers[transfer_idx].src_addr;
        *GDMA_TRANS_RECX_DST(transfer_list_base_addr, offset + transfer_idx) = (LSA_UINT32) transfers[transfer_idx].dst_addr;
        ...
        *GDMA_TRANS_RECX_CTRL(transfer_list_base_addr, offset + transfer_idx) = transfers[transfer_idx].burst_mode << 18;
        ...
        GDMA_ASSERT(transfers[transfer_idx].transfer_count > 0 && transfers[transfer_idx].transfer_count <= 65536, GDMA_ERR_PARAMETER);     /* error case : transfer_count field in transfer descriptor is incorrectly set */
...
static LSA_VOID gdma_in_fatal_error( LSA_UINT8 error_module, LSA_UINT16 error_line, LSA_UINT16 error_code)
{
    gdma_error_descriptor.lsa_component_id = GDMA_LSA_COMPONENT_ID;
