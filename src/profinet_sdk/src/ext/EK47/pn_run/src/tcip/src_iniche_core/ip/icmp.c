...
icmprcv(PACKET p)      /* the incoming packet */
{
   ...
   struct ip * pip;
   struct ping *  e;
#if 0 /*INICHE_TCIP_ITGR see 2788816 */
   unsigned short osum;
#endif
   unsigned short xsum;
#ifdef FULL_ICMP
...
#if 1 /* INICHE_TCIP_ITGR see 2788816*/
...
#endif
   {
      ...
      if ((NDEBUG & UPCTRACE) && (NDEBUG & IPTRACE))
      {
		...
		TCIP_INICHE_TRACE_01(0, LSA_TRACE_LEVEL_ERROR
			, "ICMP: checksum error, xsum 0x%x instead 0xffff"
			, xsum
			);
#else
...
#endif
#endif
#if 0 /*INICHE_TCIP_ITGR ID 948168, moved to ipdemux.c */
...
#endif
#endif
#if 1 /* INICHE_TCIP_ITGR see 1664106, packet from receive cycle must not be reused for send */
	  {
		PACKET rep_p = ip_copypkt(p);

		struct ip * rep_pip = ip_head(rep_p);
		struct ping *rep_e = (struct ping *)ip_data(rep_pip);
		unsigned rep_len = rep_p->nb_plen - (ip_hlen(rep_pip));

		rep_e->ptype = ECHOREP;
		rep_e->pchksum = 0;

		if (rep_len&1)  /* pad odd length packets for checksum routine */
		{
		sav_ch = *(((char *) rep_e) + rep_len);
		((char *)rep_e)[rep_len] = 0;
		}

		rep_e->pchksum = ~cksum(rep_e, (rep_len+1)>>1);
		if (rep_len&1) *(((char *) rep_e) + rep_len) = sav_ch;
		...
		{
			rep_pip->ip_src = p->net->n_ipaddr;
		}
		else rep_pip->ip_src = pip->ip_dest;

		rep_pip->ip_dest = host;

		icmp_mib.icmpOutEchoReps++;
		icmp_mib.icmpOutMsgs++;

		rep_p->nb_prot += ip_hlen(rep_pip);      /* move pointer past IP to ICMP */
		rep_p->nb_plen = rep_len;

		i = ip_write(ICMP_PROT, rep_p); /* send replicated packet */

		LOCK_NET_RESOURCE(FREEQ_RESID);
		pk_free(p); /* free received packet */
		UNLOCK_NET_RESOURCE(FREEQ_RESID);
	  }
#else
      e->ptype = ECHOREP;
