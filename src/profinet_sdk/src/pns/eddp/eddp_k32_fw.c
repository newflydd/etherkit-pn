 ...
 *===========================================================================*/
extern LSA_VOID_PTR_TYPE EDDP_LL_K32_RRQB_GET(EDDP_LOCAL_DDB_PTR_TYPE  pDDB,
							 LSA_UINT32               EventNr );
...
{
	LSA_UINT32 Status;
	K32_RQB_PTR_TYPE pRQB = EDDP_LL_K32_RRQB_GET(pDDB, EventNr);

    LSA_UNUSED_ARG(EventNr);       // handle unused parameter

    while(LSA_NULL != pRQB)
    {
    	Status = pRQB->Hdr.Response;
    	switch(pRQB->Hdr.Opcode)
    	{
    	case K32_OPC_GSY_SYNC:
    	case K32_OPC_GSY_DELAY_REQ:
    		break;
    	case K32_OPC_START:
    		 if (K32_RSP_OK == Status)
			{
				Status = EDD_STS_OK;
			}
			else
		...
		*/
    	...
    	pRQB = EDDP_LL_K32_RRQB_GET(pDDB, EventNr);
    }
