    ...
    LLDP_UPPER_RQB_PTR_TYPE      pRQB)
{
    ...
    switch ( LLDP_RQB_GET_RESPONSE(pRQB))
    {
        ...
        case LLDP_RSP_OK_CANCEL:
            break;
        default:
            LLDP_UPPER_TRACE_04(pHDB->TraceIdx,LSA_TRACE_LEVEL_NOTE_HIGH,"LLDP-RQB(0x%08x) Response error [opcode: 0x%X, Handle: 0x%X, Resp: 0x%X]"
                , pRQB, LLDP_RQB_GET_OPCODE(pRQB), LLDP_RQB_GET_HANDLE(pRQB), LLDP_RQB_GET_RESPONSE(pRQB)
                );
            break;
    }
    if (Cbf != 0)
    {   /* AP00999034: call macro instead of: Cbf(pRQB); */
        ...
        if(pHDB->ReleaseSysPtr == LSA_TRUE)
        {
            ...
            LLDP_REQUEST_UPPER_DONE(Cbf, pRQB, SysPtr);
...
LSA_VOID LLDP_UserRequestFinish(
    LLDP_HDB_PTR_TYPE           pHDB,
    LLDP_UPPER_RQB_PTR_TYPE     pRQB,
    LLDP_RSP                    Response)
{
...
LLDP_HDB_PTR_TYPE LLDP_GetUserHDBFromHandle(
