...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_TOPOUserCheckPending(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_TOPORequestHandler(
    OHA_HDB_USER_PTR_TYPE           pHDBUser,
    LSA_UINT16                      NICId,
    LSA_UINT16                      Response)
{
    ...
    if (pHDBUser->PendingPortId > pHDBLldp->pDB->PortCnt)
    {
        ...
        if (Opcode == OHA_OPC_TOPO_CONTROL)
        {
            ...
            OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"TOPO-Control Register-Topo successful");
        }
#endif
        ...
        if (Opcode == OHA_OPC_PRM_COMMIT)
        {
            ...
            else
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Write REMA for Prm-Commit failed (Response: 0x%X)!", Response);
        ...
        else if (Opcode == OHA_OPC_PRM_END)
        {
            ...
            OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"Handle Prm-End successful");
        ...
        {
            OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_TOPORequestHandler: Invalid Opcode (0x%X)", Opcode);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
    else
    {
        ...
        {
            OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"EDDGetPortParams failed during Operation (Opcode (0x%X)!", Opcode);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT16 OHA_SetLastSpecifier(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
	...
	if (PortId == 0)	/* interface diagnosis */
	{
		switch (DiagIndex)
		{
			...
			case OHA_DIAG_INTERFACE_NOD:       /* ( 4) NoD mismatch      */
				...
				break;
			case OHA_DIAG_GATEWAY_INACTIVE:   /* ( 5) gateway inactive  */
				pHDBUser->TopoLastIndStati[PortId].LastSpecifier_Gateway_Inactive = Specifier;
				break;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_TopoIndicateEvent(
...
)
{
        ...
        (!LSA_HOST_PTR_ARE_EQUAL(pThisInd, LSA_NULL)) )
    {
        ...
        if ((pThisInd->PortId != 0 && pThisInd->DiagValid[OHA_DIAG_MAU-1]) || /* port diag */
			...
			(pThisInd->PortId == 0 && pThisInd->DiagValid[OHA_DIAG_GATEWAY_INACTIVE - 1]) ||
...
/*+  Description: Indicates a TOPO-Event to one user                        +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_TopoIndicateEmptyEvent(
    LSA_UINT16                      PortId,
    OHA_HDB_USER_PTR_TYPE           pHDBUser
)
{
    ...
    if (pHDBUser->TopoState == OHA_TOPO_STATE_START)
    {
		...
		{
			 OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_TopoIndicateEmptyEvent(): Invalid PortControl.PortId (%Xh)",PortId);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_IsRemoteCheck(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        if (OHA_IS_NOT_NULL(pTopoDB))
        {
            ...
            {
                RetVal = LSA_TRUE;
                OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsRemoteCheck(RetVal: 0x%X)", RetVal);
                return(RetVal);
            }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#ifndef OHA_CFG_NO_MRP
static LSA_BOOL OHA_IsRemoteMrpPeer_None(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId)
{
    LSA_BOOL        RetVal  = LSA_FALSE;
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
		...
		if (!(Instance >= 1 && Instance <= EDD_CFG_MAX_MRP_INSTANCE_CNT))
		{
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsRemoteMrpPeer_None(No Instance: 0x%X)", Instance);
        ...
        switch (pHDBMrp->MrpStateInd[Instance-1].topoCtrlMode)
        {
            ...
            case MRP_USR_ENUM_TOPO_CTRL_ADD:   /* do not build diagnosis */
                ...
                OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsRemoteMrpPeer_None(Mode: 0x%X)", pHDBMrp->MrpStateInd[Instance-1].topoCtrlMode);
                break;

            default:    /* no MRP data written */
                OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsRemoteMrpPeer_None(Mode: Initial)");
                break;
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_TopoIndicatePort(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL                CheckPeers,
...
)
{
    ...
    if (pHDBUser && pHDBUser->TopoState == OHA_TOPO_STATE_START) /* topo check for this user is started ? */
    {
        ...
        {
            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, IsPulled: %x",PortId,pDB->IsPulled[PortId-1]);
            return(RetVal);
        }
        ...
        if (!LSA_HOST_PTR_ARE_EQUAL(pTopoDB, LSA_NULL))
        {
            ...
            if (CheckPeers)
            {
                ...
                if (OHA_IsRemoteCheck(pHDBUser, PortId))
                {
                    ...
                    if (RetVal == OHA_OK)
                    {
                        ...
                        OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_NONE: %d",PortId,RemoteMatch);
                ...
                {
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort (Mrp-Special): PortId: %d, OHA_DIAG_PEER_NONE: %d",PortId,RemoteMatch);
                ...
                if (pTopoDB->NumberOfPeers != 0)
                {
                    ...
                    if (RetVal == OHA_OK)
                    {
                        ...
                        OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_CHASSIS: %d",PortId,RemoteMatch);
                    ...
                    if (RetVal == OHA_OK)
                    {
                        ...
                        OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_PORT: %d",PortId,RemoteMatch);
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_MRP_UUID: %d",PortId,RemoteMatch);
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_PTCP: %d",PortId,RemoteMatch);
                ...
                (pTopoDB->DelayMode & OHA_DELAY_DIFFERENCE_CHECK_ON))
            {
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_CABLEDELAY: %d",PortId,LocalOrRemoteMatch);
                ...
                (pTopoDB->MAUTypeMode & OHA_MAU_TYPE_DIFFERENCE_CHECK_ON))
            {
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_MAU: %d",PortId,LocalOrRemoteMatch);
            ...
            if (CheckLinkStatus)
            {
                ...
                if (pTopoDB->LinkStateExisting || pTopoDB->MAUType != 0 || pTopoDB->DelayExisting)
                {
                    ...
                    if (! LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
                    {
                        ...
                        if (pHDBEdd->LinkStatus[PortId-1].LinkStatus != EDD_LINK_UNKNOWN)
                        {
                            ...
                            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_LINK: %d",PortId,LinkState);
                ...
                if (pTopoDB->MAUType != 0)
                {
                    ...
                    if (RetVal == OHA_OK)
                    {
                        ...
                        OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_MAU: %d",PortId,LocalMatch);
                ...
                if (pTopoDB->DelayExisting)
                {
                    ...
                    if (RetVal == OHA_OK)
                    {
                        ...
                        OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_DELAY: %d",PortId,LocalMatch);
            ...
            /* check only if local IRT is supported (IRTPortStatus != OHA_PORT_STATUS_CLASS3_OFF) */
            {
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_DIAG_PEER_RT_CLASS3: %d",PortId,LocalOrRemoteMatch);
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_PEER_PREAMBLE_LENGTH_MISMATCH: %d",PortId,LocalOrRemoteMatch);
                ...
                if (RetVal == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicatePort: PortId: %d, OHA_PEER_FRAGMENTATION_MISMATCH: %d",PortId,LocalOrRemoteMatch);
            ...
            else
            {
                ...
                if (CheckPeers && CheckLinkStatus)
                {
					...
					if (RetVal != LSA_RET_OK)	/* could not confirm the user */
					{
						...
						{
							OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "Could not allocate for empty PRM diag: PortId: 0x%x", PortId);
						}
					}
                }
            }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+  Description: indicates interface TOPO indications to every user        +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_UpdateTopoInterfaces(LSA_VOID)
{
	LSA_UINT16              i;

	/* Multiple Interface Mismatch indication */
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_TopoIndicateInterface(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL                BuildEmptyEvent)
{
    ...
    if (pHDBUser && pHDBUser->TopoState == OHA_TOPO_STATE_START) /* topo check for this user is started ? */
    {
        ...
        if (RetVal == OHA_OK)
        {
			...
			LSA_UINT32  ExtCHannelAddValue = pHDBSock->Router.ExpectedGW;

            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicateInterface: NICId: %d, OHA_DIAG_INTERFACE_GATEWAY: %d",NICId,IfMatch);
        ...
        if (RetVal == OHA_OK)
        {
            ...
            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicateInterface: NICId: %d, OHA_DIAG_INTERFACE_NOS: %d",NICId,IfMatch);
        ...
        if (RetVal == OHA_OK)
        {
            ...
            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicateInterface: NICId: %d, OHA_DIAG_INTERFACE_IP: %d",NICId,IfMatch);
        ...
        if (RetVal == OHA_OK)
        {
			...
			OHA_ASSERT_NULL_PTR(pDB);
			ExtCHannelAddValue  = (pDB->MultipleIfModeValid & OHA_NAME_OF_DEVICE_MASK) ? 2 : 1;

            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicateInterface: NICId: %d, OHA_DIAG_INTERFACE_NOD: %d",NICId,IfMatch);
		...
		/* (v) compare the interface gateways for inactive if existing */
		RetVal = OHA_CompareTopo_GatewayInactive(pHDBUser, &IfMatch);

		if (RetVal == OHA_OK)
		{
			LSA_UINT16  ExtCHannelErrorType = OHA_STANDARD_GATEWAY_INACTIVE;
			LSA_UINT16  ChannelProperties = (IfMatch) ? (LSA_UINT16)OHA_TOPO_ERROR_DISAPPEARS_AND_OKAY : (LSA_UINT16)OHA_TOPO_ERROR_APPEARS;
			LSA_UINT32  ExtCHannelAddValue = pHDBSock->Router.ExpectedGW;

			OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_TopoIndicateInterface: NICId: %d, OHA_DIAG_GATEWAY_INACTIVE: %d", NICId, IfMatch);

			/* indicate only the changes */
			if (pHDBUser->TopoLastIndStati[0].LastSpecifier_Gateway_Inactive != ChannelProperties)
			{
				(LSA_VOID)OHA_PrepareTopoIndication(pHDBUser, 0,
													OHA_MULTIPLE_INTERFACE_MISMATCH,
													ExtCHannelErrorType,
        ...
        else
        {
            ...
            if (BuildEmptyEvent)
            {
				...
				if (RetVal != LSA_RET_OK)	/* could not confirm the user */
				{
					...
					if (RetVal != LSA_RET_OK)	/* could not alloc */
					{
						OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "Could not allocate for empty interface PRM diag: NICId: 0x%x", NICId);
					}
                }
            }
        }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_None(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_Chassis(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
...
/*+  Description: compares the peer (real with expected)                    +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_Ports(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#ifndef OHA_CFG_NO_MRP
LSA_UINT16 OHA_CompareTopoPeer_MRP_domain(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        *pCompare = LSA_FALSE;
		...
		{
            RetVal = OHA_ERR_RESOURCE;
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_MRP_domain(No Instance: 0x%X)", Instance);
        ...
        switch (pHDBMrp->MrpStateInd[Instance-1].topoCtrlMode)
        {
            ...
            case MRP_USR_ENUM_TOPO_CTRL_ADD:   /* do not build diagnosis */
                ...
                OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_MRP_domain(Mode: 0x%X)", pHDBMrp->MrpStateInd[Instance-1].topoCtrlMode);
                return(RetVal);

            default:    /* no MRP data written */
                OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_MRP_domain(Mode: OHA_MRP_REMOVE)");
                RetVal = OHA_ERR_RESOURCE;
                return(RetVal);
        }

        /* TFS 1152052, 1152053: No "Peer MRP domain mismatch" alarm is sent (if MRP_DomainUUID is ffffffff-ffff-ffff-ffff-ffffffffffff) */
        ...
        if (CountOfRealPeers == 0)
        {
            ...
            *pCompare = LSA_TRUE;
            ...
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_MRP_domain(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
...
/*+  Description: compares the peer (real with expected)                    +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_PTCP_Status(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        if (pHDBUser->PtcpTimerInUse[PortId-1])
        {
            ...
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_PTCP_Status(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
        ...
        {
            RetVal = OHA_ERR_PARAM;
            OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "PtcpStatus diag not activated");
        ...
        *pCompare = LSA_FALSE;
        ...
        if (IsWireless)
        {
            ...
            *pCompare = LSA_TRUE;
            ...
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "Peer_PTCP_Status: port (0x%X) is wireless - no ptcp diagnosis", PortId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_PTCP_Status(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
                    ...
                    if (OHA_IS_NOT_NULL(pRemVar) && (RemLen == OHA_LLDP_PNO_PTCP_STATUS_SIZE))
                    {
                            ...
                            ((pTopoDB->AdjustTopoData.DomainBoundaryEgress & OHA_SYNC_BOUNDARY_MASK) == OHA_SYNC_BOUNDARY_OFF) ) /* False: do not block the multicast MAC */
                        {
                            ...
                            if (Equal)
                            {
                                ...
                                {
                                    *pCompare = LSA_TRUE;
                                    OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "no local sync master (PTCP_MasterSourceAddress = {0})");
                                    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_PTCP_Status(RetVal: 0x%X)", RetVal);
                                    return(RetVal);
                                }
                                ...
                                {
                                    *pCompare = LSA_TRUE;
                                    OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "no remote sync master (PTCP_MasterSourceAddress = {0})");
                                    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_PTCP_Status(RetVal: 0x%X)", RetVal);
                                    return(RetVal);
                                }
                                ...
                                else
                                {
                                    ...
                                    *pCompare = LSA_FALSE;
                                    OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "Non blocking the multicast MAC: PTCP-MasterSourceAddresses are different");
                                }
                            }
                            else
                            {
                                ...
                                OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "Non Blocking the multicast MAC: PTCP_SubdomainUUID's are different - no mismatch");
                                *pCompare = LSA_TRUE;
                            }
                        }
...
/*+              FIX	    AUTONEG	   => Error                             +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_MAU_Type(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
        ...
        /* is the Link Up ? */
        {
            ...
            if (CountOfRealPeers == 0)
            {
                ...
                *pCompare = LSA_TRUE;
                ...
                OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_MAU_Type(RetVal: 0x%X)", RetVal);
                return(RetVal);
            }
        }
            ...
            if (SenderId && (SenderId <= LLDP_MAX_PORT_SENDER)) /* get it */
            {
                    ...
                    SenderStatus == OHA_STATUS_DATA_INVALID)
                {
                         ...
                         LLDP_TLV_IEEE_802_3_MAC_PHY_CFG_STATUS_MAUT_TYPE_SIZE))
                    {
                        ...
                        {
                            OHA_FUNCTION_TRACE_03(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,
                        ...
                        {
                            OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_CableDelay(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare,
    LSA_UINT32          *   pExtCHannelAddValue)
{
    ...
    OHA_ASSERT_NULL_PTR(pHDBUser);
    OHA_ASSERT_NULL_PTR(pCompare);
    OHA_ASSERT_NULL_PTR(pExtCHannelAddValue);
    ...
    *pExtCHannelAddValue = 0;
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        *pCompare = LSA_FALSE;  /* not o.k. */
        ...
        if  (!OHA_EDD_DELAY_SUPPORTED(pDB->HardwareType) )
        {
            RetVal = OHA_ERR_PARAM;
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_CableDelay(NICId: 0x%X, RetVal: 0x%X)", NICId, RetVal);
            return(RetVal);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoDelay(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
            ...
            if(pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER)
            {
                ...
                if (LinkState != OHA_PORT_LINK_STATE_UP)
                {
                    ...
                    *pCompare = LSA_TRUE;
                    ...
                    OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoDelay(RetVal: 0x%X)", RetVal);
                    return(RetVal);
                }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoSpeed(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoMode(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_PrepareTopoIndication(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_IndicateEventAllUsers(
    LSA_UINT16                  NicID,
    LSA_UINT16                  PortId
)
{
    ...
    if (OHA_IS_NOT_NULL(pHDBEdd) && OHA_IS_NOT_NULL(pHDBEdd->pHDBUserPrm))
    {
        ...
        if (RetVal != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_WARN,"OHA_IndicateEventAllUsers(NicID: 0x%X, RetVal: 0x%X). OHA_DoRemotePortState() failed", NicID, RetVal);
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#ifndef OHA_CFG_NO_MRP
LSA_BOOL oha_update_mrp_port_status(
    ...
    LSA_UINT16                  PortID)
{
	...
	if(!(Instance >= 1 && Instance <= EDD_CFG_MAX_MRP_INSTANCE_CNT))
	{
		OHA_PROGRAM_TRACE_01(pHDBMrp->TraceIdx, LSA_TRACE_LEVEL_FATAL,"oha_update_mrp_port_status(): Invalid Instance (%Xh)",Instance);
		 OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		return(UpdateSend);
	}
    ...
    if (pHDBMrp->MrpStateInd[Instance-1].mrpStatus != MRP_OPERATION_OFF) /* OHA_PORTSTATE_MRP_OFF */
    {
            ...
            (!OHA_MEMCMP(VarPtr, pDomainUUID, sizeof(OHA_UUID_TYPE)))  )
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDBMrp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"oha_update_mrp_port_status: OHA_LLDPSet failed (Status: 0x%X).", Status);
            }
            UpdateSend = LSA_TRUE;
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_RT_Class3(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        *pCompare = LSA_FALSE;
        ...
        {
            RetVal = OHA_ERR_RESOURCE;
			OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_RT_Class3(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
        ...
        /* is the Link Up ? */
        {
                ...
                /* && pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_RT_Class3 == OHA_ALARM_SPECIFIER_INITIAL_VALUE)*//* AP00571466 */
            {
                ...
                *pCompare = LSA_TRUE;
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,"OHA_CompareTopoPeer_RT_Class3() - IRT not supported (0x%X) !", Local_IRTPortStatus);
            ...
            if (SenderId && (SenderId <= LLDP_MAX_PORT_SENDER)) /* get it */
            {
                    ...
                    SenderStatus == OHA_STATUS_DATA_INVALID)
                {
                    ...
                    if (OHA_IS_NOT_NULL(pVar) && (Len == OHA_LLDP_PNO_PORT_STATUS_SIZE))
                    {
                        ...
                        {
                            OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_CompareTopoPeer_RT_Class3() - Remote_C3_PortState (0x%X) and Local_C3_PortState (0x%X) different!",Remote_IRTPortStatus,Local_IRTPortStatus);
                        }
                    }
                    else
                    {
                        OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_CompareTopoPeer_RT_Class3() - Remote PortStatus-Tlv not existing!");
                    }
                }
            }
        }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestPtcpWriteData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
         ...
         LSA_HOST_PTR_ARE_EQUAL(pHDBGsy,  LSA_NULL) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_WRITE_PTCP_DATA failed. No LLDP- or GSY-Channel present (NIC-ID: 0x%X)",NICId);
    ...
    if (pHDBGsy->SyncDataWrite.Mode == 0)
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_WRITE_PTCP_DATA failed. Got no PDSyncData record by PrmWrite (NIC-ID: 0x%X)",NICId);
        OHA_UserRequestFinish(pHDBUser,pRQB,Response);
        return;
    }
#endif
    ...
    if (NICId != pHDBLldp->pDB->NicId) /* AP01022057 */
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_WRITE_PTCP_DATA failed. No valid NicID for this interface (0x%X)",pHDBLldp->pDB->InterfaceID);

        OHA_UserRequestFinish(pHDBUser,pRQB,Response); /*an error occurred */
        OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestPtcpWriteData(Response: 0x%X)", Response);
        return;
    }
    ...
    if (pHDBGsy->SyncDataCommit.Mode == 0)
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_WRITE_PTCP_DATA failed. No PrmCommit of a (written) PDSyncData record is done (NIC-ID: 0x%X)",NICId);
        OHA_UserRequestFinish(pHDBUser,pRQB,Response);
        return;
    }

    if (pPtcpData->DoSync != OHA_DO_SYNC_NO && pPtcpData->DoSync != OHA_DO_SYNC_YES)
    {
        Response = OHA_ERR_PARAM;  /* parameter is wrong */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP, "DoSync(%u) out of range", pPtcpData->DoSync);
        OHA_UserRequestFinish(pHDBUser,pRQB,Response);
        return;
    }

    if (pPtcpData->IsSync != OHA_IS_SYNC_NO && pPtcpData->IsSync != OHA_IS_SYNC_YES)
    {
        Response = OHA_ERR_PARAM;  /* parameter is wrong */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"IsSync(%u) out of range", pPtcpData->IsSync);
        OHA_UserRequestFinish(pHDBUser,pRQB,Response);
        return;
    }

    /* if there's no record written, no TLV is to send => if there was a record written, the TLV is to remove */
    if (pPtcpData->DoSync == OHA_DO_SYNC_YES)
    {
        /*********************************************************************/
        /* first the check (SubdomainID, MasterSourceAddress, ...)           */
        /*********************************************************************/
        if (pPtcpData->IsSync == OHA_IS_SYNC_YES)
        {
#if 0
            /* check the SubdomainID */
            if (!OHA_CheckDomainUUID(pPtcpData->SubdomainID))  /* is there a valid UUID ( <> 0) ? */
            {
                OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_OPC_WRITE_PTCP_DATA-Write: IRDataID failed.");
            ...
            {
                OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_OPC_WRITE_PTCP_DATA-Write: MasterSourceAddress failed.");
                Response = LSA_RET_ERR_PARAM;
                OHA_UserRequestFinish(pHDBUser,pRQB,Response);
                OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestPtcpWriteData(Response: 0x%X)", Response);
                return;
    ...
    pHDBGsy->SyncDataCommit.Mode = (pPtcpData->DoSync == OHA_DO_SYNC_YES) ? (LSA_UINT16)OHA_SYNC_ADD: (LSA_UINT16)OHA_SYNC_REMOVE;
...
LSA_VOID oha_update_sync_status(LSA_UINT16 NICId)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_update_sync_status(NicID: 0x%X)",
                          NICId);
    ...
    for (PortID = 1; PortID <= OHA_CFG_MAX_PORTS && PortID <= pHDBLldp->pDB->PortCnt; PortID++)
    {
        ...
        if (PtcpDifference) /* update LLDP data by LLDPSet() */
        {
                ...
                pHDBLldp->RequestState == OHA_LLDP_STATE_RE_START) /* LLDP-Set is possible ? */
            {
                ...
                if (Status != LSA_RET_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"oha_update_sync_status: OHA_LLDPSet failed (NICId: 0x%X, Status: 0x%X).", Status,NICId);
                }
            }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL oha_update_ptcp_status(
    LSA_UINT16                  NICId,
    LSA_UINT16                  PortID,
    LSA_BOOL                    OnlySyncForwarding)
{
    ...
    if(!(PortID && (PortID <= OHA_CFG_MAX_PORTS)))
	{
		 OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL,"oha_update_ptcp_status(NICId: 0x%X): Invalid PortID (%Xh)",NICId,PortID);
		 OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		return(UpdateSend);
	}
    ...
    if (pHDBGsy->SyncDataCommit.Mode == OHA_SYNC_ADD)
    {
        ...
        if (IsWireless)
        {
            ...
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "RtClass3_TopoState (NicID: 0x%X, PortId: 0x%X): IsWireless port, UUIDs are set to Null (=OK)!", NICId, PortID);
        }
        else
        {
        ...
        {
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "RtClass3_TopoState (NICId: 0x%X, PortId: 0x%X): OnlySyncForwarding port, Remote_IRDataUUID is set to Null (=OK)!", NICId,PortID);
            pIRDataUUID          = NullIRDataID;
        }
		...
		if (OHA_IS_NOT_NULL(pHDBEdd->pHDBUserPrm))	/* PRM-User existing ? */
		{
			if (pHDBEdd->pHDBUserPrm->PrmState == OHA_PRM_STATE_END)
			{
				if (!OHA_MEMCMP(pHDBGsy->SyncDataWrite.IRDataID, /* written by PrmWrite (= temp buffer) */
								...
								OHA_UUID_SIZE))
				{
					OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "PRM_END (NICId: 0x%X, PortId: 0x%X) is done and the new IRDataUUID is different, Remote_IRDataUUID is set to Null (=OK)!", NICId,PortID);
					pIRDataUUID = NullIRDataID;
				}
			}
		}
        ...
        if (UpdateSend)
        {
            ...
            if (Status != LSA_RET_OK)
            {
                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"oha_update_ptcp_status(NICId: 0x%X): OHA_LLDPSet failed (Status: 0x%X).", NICId,Status);
            }
        }
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_RemoteChassisIdIsIdentical(LSA_UINT16 NICId, LSA_UINT16 PortId, LSA_UINT16 SenderId,
										OHA_DB_TOPO_PTR_TYPE pTopoDB)
{
	...
	{
		OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "No (valid) Sender present(NICId: 0x%X, SenderId: 0x%X, PortId: 0x%X)", NICId,SenderId, PortId);
	...
	{
		pRemotePeer = &pTopoDB->PeerSender;
	}
	else
	{
	...
	{
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemoteChassisIdIsIdentical (NICId: 0x%X, PortId: 0x%X): No valid Tlv LLDPChassisID present", NICId,PortId);
			 ...
			 pRemotePeer->LengthPeerChassisID != 0)
    {
        if (OHA_IS_NOT_NULL(pRemoteChassisID))
        {
            ...
            if (VarLen == pRemotePeer->LengthPeerChassisID)
            {
                ...
                if (!IsIdentical)
                {
                    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemoteChassisIdIsIdentical (NICId: 0x%X, PortId: 0x%X): Remote_NAME and Expected_NAME not identical", NICId,PortId);
                }
            }
            else
            {
        ...
        {
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemoteChassisIdIsIdentical (NICId: 0x%X, PortId: 0x%X): No Tlv LLDPChassis present", NICId,PortId);
        }
    }
    else
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemoteChassisIdIsIdentical (NICId: 0x%X, PortId: 0x%X): No (valid) Expected_NAME present!", NICId,PortId);
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_RemotePortIdIsIdentical(LSA_UINT16 NICId, LSA_UINT16 PortId, LSA_UINT16 SenderId,
										    OHA_DB_TOPO_PTR_TYPE pTopoDB)
{
	         ...
	         pRemotePeer->LengthPeerPortID != 0)
    {
        if (OHA_IS_NOT_NULL(pRemotePortID))
        {
            ...
            if (VarLen == pRemotePeer->LengthPeerPortID)
            {
                ...
                if (IsIdentical)
                {
                    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemotePortIdIsIdentical (NICId: 0x%X, PortId: 0x%X): Remote_PORT and Expected_PORT identical", NICId, PortId);
                }
            }
            else
            {
                OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemotePortIdIsIdentical (NICId: 0x%X, PortId: 0x%X): Remote_PORT and Expected_PORT not identical", NICId, PortId);
            }
        }
        else
        {
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemotePortIdIsIdentical (NICId: 0x%X, PortId: 0x%X): No (valid) Tlv LLDPPort present", NICId, PortId);
        }
    }
    else
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_RemotePortIdIsIdentical (NICId: 0x%X, PortId: 0x%X): No (valid) Expected_PORT present!", NICId, PortId);
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID OHA_GetRemotePortState(
	LSA_UINT16              NICId,
    LSA_UINT16              PortId,
	OHA_DB_TOPO_PTR_TYPE    pTopoDB,
    EDD_UPPER_SET_REMOTE_PORT_STATE_PTR_TYPE pRemotePortState)
{
	...
	{
		OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): No (valid) Sender present(SenderId: %u)", NICId, PortId, SenderId);
		SenderId = 0; /* not present */
	}
	else if (pDB->LldpData[PortId-1].LldpSenderStatus == OHA_STATUS_MULTIPLE_SENDER)
	{
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): multiple Sender", NICId, PortId);
	...
	OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE
		, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): Local_IRTPortStatus(0x%X)"
		, NICId, PortId, Local_IRTPortStatus
		);
	...
	if (SenderId != 0)
	{
		...
		{
			OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): No (valid) Tlv LLDP_PNIO_PTCPSTATUS present (PtcpLen: %u)", NICId, PortId, VarLen);
		}
	}
	...
	if (SenderId != 0)
	{
		...
		{
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): No (valid) Tlv LLDP_PNIO_DELAY present", NICId, PortId);
		}
	}
	...
	if (SenderId != 0)
	{
		...
		{
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): No (valid) Tlv LLDP_MAC_PHY_CFG_STATUS present", NICId,PortId);
		}
	}
	...
	if (SenderId != 0)
	{
        ...
        {
            OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): No (valid) Tlv LLDP_PNIO_PORTSTATUS present", NICId,PortId);
        }
	}
	...
	OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_NOTE
		, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): RemoteMAUTypeIsValid(%u) [Remote_MAUType(%u)]"
		, NICId, PortId, RemoteMAUTypeIsValid, Remote_MAUType
		);
	...
	if (OHA_RemoteChassisIdIsIdentical(NICId, PortId, SenderId, pTopoDB)) /* Remote_NAME == Expected_NAME */
	{
		...
		{
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u) RtClass3_TopoState: Remote_PORTID not existing or not the Expected_PORTID", NICId, PortId);
		}
	}
	else
	{
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u) RtClass3_TopoState: Remote_CHASSISID not existing or not the Expected_CHASSISID", NICId, PortId);
	}
    ...
    switch (Remote_IRTPortStatus & OHA_PORT_STATUS_CLASS3_MASK) /* only the bits RTClass3_PortStatus.RTClass3 */
    {
        ...
        case OHA_PORT_STATUS_CLASS3_RUNNING:
            ...
            break;
        default:
            OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u) RtClass3_RemoteState: Not a (valid) Remote_IRTPortStatus(0x%X) received!", NICId, PortId, Remote_IRTPortStatus);
            RtClass3_RemoteState = EDD_SET_REMOTE_PORT_STATE_RT_CLASS3_OFF;
            break;
    }

	OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE
		, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u): RtClass3_RemoteState(%u) 0=off 1=tx 2=rxtx"
		, NICId, PortId, RtClass3_RemoteState
		);
	...
	if (OHA_RemoteChassisIdIsIdentical(NICId, PortId, SenderId, pTopoDB)) /* Remote_NAME == Expected_NAME */
	{
		...
		{
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u) Neighbour_TopoState: Remote_PORTID not existing or not the Expected_PORTID", NICId, PortId);
		}
	}
	else
	{
		OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OHA_GetRemotePortState (NICId: 0x%X, PortId: %u) Neighbour_TopoState: Remote_CHASSISID not existing or not the Expected_CHASSISID", NICId, PortId);
	}

	OHA_FUNCTION_TRACE_05(0, LSA_TRACE_LEVEL_NOTE
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DoRemotePortState(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId
)
{
    ...
    if (pHDBUser->TopoState == OHA_TOPO_STATE_START) /* topo check for this user is started ? */
    {
        ...
        {
			OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_DoRemotePortState(): No TopoDataBase existing!");
            RetVal = OHA_ERR_RESOURCE;
        ...
        {
			RetVal = OHA_OK;
            OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,"No difference between actual and last port state - O.K.");
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DoRemotePortState(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
        ...
        *pLastPortState = ActualPortState;
        ...
        if (RetVal != OHA_OK)
        {
            OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_DoRemotePortState(): SetRemotePortState failed.");
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoInterface_Gateway(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare)
{
    LSA_UINT16  RetVal  = OHA_OK;
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "IN : OHA_CompareTopoInterface_Gateway(pHDBUser: 0x%X, pCompare: 0x%X)", pHDBUser, pCompare);
    ...
    *pCompare = LSA_TRUE;
	...
	if (pHDBSock->Router.RemoveGatewayDiags)
	{
		OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_CompareTopoInterface_Gateway: Gateway(0x%X) diag removal", pHDBSock->Router.ActivatedGW);
		return(RetVal);
	}
    ...
    {
		OHA_COMMON_MEM_U8_PTR_TYPE FirstGatewayPtr = LSA_NULL;
		...
		while (i < OHA_CFG_MAX_NICS)
		{
			if ( pHDBEdd->State == OHA_HANDLE_STATE_OPEN )
			{
				...
				if (OHA_IS_NOT_NULL(IpAddrPtr) && (!OHA_MEMCMP(IpAddrPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE)))
				{
					/* is the gateway address valid ? */
					if (OHA_IS_NOT_NULL(GatewayPtr) && (!OHA_MEMCMP(GatewayPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE)))
					{
						...
						if (!OHA_MEMCMP(GatewayPtr, IpAddrPtr, OHA_IP_ADDR_SIZE))
						{
							...
							if (OHA_IS_NOT_NULL(FirstGatewayPtr))
							{
								if (!OHA_MEMCMP(GatewayPtr, FirstGatewayPtr, OHA_IP_ADDR_SIZE))
								{
									OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface: Different Gateways (ThisNICId: 0x%X, NICId: 0x%X)", pHDBUser->Params.pDetail->Params.User.NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
									*pCompare = LSA_FALSE;
									break;
								}
							}
							FirstGatewayPtr = GatewayPtr; /* (first) gateway found */
						}
					}
				}
			}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_CompareTopo_GatewayInactive             +*/
/*+                             OHA_HDB_USER_PTR_TYPE        pHDBUser       +*/
/*+-------------------------------------------------------------------------+*/
/*+                                                                         +*/
/*+  Input/Output:                                                          +*/
...
/*+                                                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: get the inactive gateway for mismatch diagnosis           +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopo_GatewayInactive(
		OHA_HDB_USER_PTR_TYPE   pHDBUser,
		LSA_BOOL            *   pCompare)
{
	LSA_UINT16  RetVal = OHA_OK;
	...
	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
						  "IN : OHA_CompareTopo_GatewayInactive(pHDBUser: 0x%X, pCompare: 0x%X)",
						  pHDBUser, pCompare);
	...
	*pCompare = LSA_TRUE;
	...
	if(pHDBSock->Router.RemoveGatewayDiags)
	{
		OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_CompareTopo_GatewayInactive: Gateway(0x%X) diag removal", pHDBSock->Router.ExpectedGW);
		return(RetVal);
	}

	/* If the standard gateway could not be set */
	if (pHDBSock->Router.IsNotSet)
	{
		OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_CompareTopo_GatewayInactive: Gateway(0x%X) not set!", pHDBSock->Router.ExpectedGW);
		*pCompare = LSA_FALSE;
		return(RetVal);
	}

	if (pHDBSock->Router.ActivatedGW == 0) /* no gateway activated -> O.K. */
	{
		OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OHA_CompareTopo_GatewayInactive(): No gateway activated");
		return(RetVal);
	}

#if 0
	/* If the standard gateway is not located in an IP-subnet of one of the interfaces */
	{
		OHA_HDB_EDD_PTR_TYPE  pHDBEdd = OHA_GET_HEDD_PTR(0);
		LSA_UINT16            LocatedSubnets = 0;
		LSA_UINT32            i = 0;

		for (; i < OHA_CFG_MAX_NICS; i++, pHDBEdd++)
		{
			if (pHDBEdd->State == OHA_HANDLE_STATE_OPEN)
			{
				OHA_COMMON_MEM_U8_PTR_TYPE IpAddrPtr = LSA_NULL;
				...
				if (OHA_IS_NOT_NULL(IpAddrPtr) && OHA_IS_NOT_NULL(SubnetPtr) &&
					(!OHA_MEMCMP(SubnetPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE)))
				{
					LSA_UINT32 ThisIpAddr     = OHA_GET32_NTOH(IpAddrPtr, 0);
					LSA_UINT32 ThisSubnetMask = OHA_GET32_NTOH(SubnetPtr, 0);

					if ((pHDBSock->Router.ActivatedGW & ThisSubnetMask) == (ThisIpAddr & ThisSubnetMask)) /* in same subnet */
					{
						OHA_FUNCTION_TRACE_03(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "Subnet (0x%X) for Gateway (0x%X) found (NICId: 0x%X)", ThisSubnetMask, pHDBSock->Router.ActivatedGW, pHDBEdd->Params.pDetail->Params.Edd.NICId);
						LocatedSubnets++;
					}
				}
			}
		...
		if (LocatedSubnets == 0)
		{
			OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_CompareTopo_GatewayInactive: Gateway(0x%X) not located in any subnet!", pHDBSock->Router.ExpectedGW);
			*pCompare = LSA_FALSE;
			return(RetVal);
		}
	}
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopo_GatewayInactive(RetVal: 0x%X", RetVal);
	return(RetVal);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoInterface_NoS(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare)
{
    LSA_UINT16  RetVal  = OHA_OK;
    ...
    *pCompare = LSA_TRUE;
	...
	if (CountOfInterfaces > 1)
    {
        ...
        {
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoInterface_NoS(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
			...
			while (i < OHA_CFG_MAX_NICS)
			{
				if ( pHDBEdd->State == OHA_HANDLE_STATE_OPEN )
				{
					...
					if (NICId != pHDBEdd->Params.pDetail->Params.Edd.NICId) /* not for this If */
					{
						...
						{
							if(OHA_MEMCMP(NamePtr, ThisNamePtr, (LSA_UINT32)Len))
							{
								OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface: Same NoS (ThisNICId: 0x%X, NICId: 0x%X)", NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
							    *pCompare = LSA_FALSE;
								break;
							}
						}
...
/*------------------------------------------------------------------------------
count the network prefix length
*
* The network prefix length is given by the number of most-significant bits
* of the subnetmask that are consecutive set to 1

----------------------------------------------------------------------------*/
LSA_UINT16 oha_network_prefix_length(LSA_UINT32 mask)
{
	LSA_UINT16 i = 0;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoInterface_IP(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare)
{
    LSA_UINT16  RetVal  = OHA_OK;
    ...
    *pCompare = LSA_TRUE;
	...
	if (CountOfInterfaces > 1)
    {
        ...
        {
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoInterface_IP(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
		...
		/* now we compare with the other interfaces */
		{
			...
			LSA_UINT32 this_addr = OHA_GET32_NTOH(ThisIpAddrPtr,0);
			LSA_UINT32 this_mask  = OHA_GET32_NTOH(ThisSubnetPtr,0);
			...
			while (i < OHA_CFG_MAX_NICS)
			{
				if ( pHDBEdd->State == OHA_HANDLE_STATE_OPEN )
				{
					...
					if (NICId != pHDBEdd->Params.pDetail->Params.Edd.NICId) /* not for this If */
					{
						...
						{
							LSA_UINT32 addr = OHA_GET32_NTOH(IpAddrPtr,0);
							LSA_UINT32 mask = OHA_GET32_NTOH(SubnetPtr,0);

							/* AP01145324: different interfaces must be in different subnets (necessary for IP stack receive path) */
							/* TFS 2520391, TFS 3108288: CDIR mode (overlapping subnets allowed) */
							/* TFS 3108288: using the rule set of IP forwarding on a longest-match basis (CDIR mode) */
							if (pHDBUser->Params.pDetail->Params.User.AllowOverlappingSubnet)
							{
								/* 1) check subnets: if same subnet and same network prefix length => error */
								/* if (IF1_Subnet == IF2_Subnet) && (network prefix length IF1_SubnetMask == network prefix length IF2_SubnetMask) then error */
								if ((this_addr & this_mask) == (addr & mask))
								{
									if (oha_network_prefix_length(this_mask) == oha_network_prefix_length(mask))
									{
										OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
											NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
										*pCompare = LSA_FALSE; /* the length of the network prefixes (given by the number of most-significant bits of the subnetmask that are consecutive set to 1) are equal */
										break;
									}
								}

								/* check subnets "more" */
								/* 2a) if (IF1_IPAddress & IF2_SubnetMask == IF2_IPAddress & IF2_SubnetMask) && (network prefix length IF1_SubnetMask <= network prefix length IF2_SubnetMask) then error */
								if ((this_addr & mask) == (addr & mask))
								{
									if (oha_network_prefix_length(this_mask) <= oha_network_prefix_length(mask))
									{
										OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check more): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
											NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
										*pCompare = LSA_FALSE; /* the length of the network prefixes (given by the number of most-significant bits of the subnetmask that are consecutive set to 1) are equal or smaller */
										break;
									}
								}

								/* 2b) if (IF2_IPAddress & IF1_SubnetMask == IF1_IPAddress & IF1_SubnetMask) && (network prefix length IF2_SubnetMask <= network prefix length IF1_SubnetMask) then error */
								if ((addr & this_mask) == (this_addr & this_mask))
								{
									if (oha_network_prefix_length(mask) <= oha_network_prefix_length(this_mask))
									{
										OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check more): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
											NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
										*pCompare = LSA_FALSE; /* the length of the network prefixes (given by the number of most-significant bits of the subnetmask that are consecutive set to 1) are equal or smaller */
							...
							else /* if subnets are overlapping - old behavior (TFS 2520391) */
							{
								/* 1) if (IF1_Subnet == IF2_Subnet) then error */
								if ((this_addr & this_mask) == (addr & mask))
								{
									OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check 1): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
									NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
									*pCompare = LSA_FALSE; /* network prefixes are the same */
									break;
								}

								/* 2a) if (IF1_Subnet == (IF2_Subnet & IF1_SubnetMask)) then error */
								if ((this_addr & this_mask) == ((addr & mask) & this_mask))
								{
									OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check 2): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
										NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
									*pCompare = LSA_FALSE;
									break;
								}

								/* 2b) if (IF2_Subnet  == (IF1_Subnet & IF2_SubnetMask)) then error */
								if ((addr & mask) == ((this_addr & this_mask) & mask))
								{
									OHA_FUNCTION_TRACE_06(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface(check 3): Overlapping (ThisNICId: 0x%X, IP: 0x%X, Subnet: 0x%X, NICId: 0x%X, IP: 0x%X, Subnet: 0x%X)",
										NICId, this_addr, this_mask, pHDBEdd->Params.pDetail->Params.Edd.NICId, addr, mask);
									*pCompare = LSA_FALSE;
									break;
								}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_PreambleLength(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        *pCompare = LSA_FALSE;
        ...
        {
            RetVal = OHA_ERR_RESOURCE;
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_PreambleLength(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
        ...
        /* is the Link Up ? */
        {
            ...
            if (Local_IRTPortStatus == OHA_PORT_STATUS_CLASS3_OFF)
            {
                ...
                *pCompare = LSA_TRUE;
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,"OHA_CompareTopoPeer_PreambleLength() - IRT not supported (0x%X) !", Local_IRTPortStatus);
                    ...
                    SenderStatus == OHA_STATUS_DATA_INVALID)
                {
                    ...
                    if (OHA_IS_NOT_NULL(pVar) && (Len == OHA_LLDP_PNO_PORT_STATUS_SIZE))
                    {
						...
						else
						{
							...
							{
								OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_CompareTopoPeer_PreambleLength() - Remote_PreambleShort (0x%X) and Local_PreambleShort (0x%X) different!",Remote_IRTPreambleShort,Local_IRTPreambleShort);
							}
						}
                    }
                    else
                    {
                        OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_CompareTopoPeer_PreambleLength() - Remote PortStatus-Tlv not existing!");
                    }
                }
                else
                {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoPeer_Fragmentation(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              PortId,
    LSA_BOOL            *   pCompare)
{
    ...
    if (PortId && PortId <= OHA_CFG_MAX_PORTS)
    {
        ...
        *pCompare = LSA_FALSE;
        ...
        {
            RetVal = OHA_ERR_RESOURCE;
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoPeer_Fragmentation(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
        ...
        /* is the Link Up ? */
        {
            ...
            if (Local_IRTPortStatus == OHA_PORT_STATUS_CLASS3_OFF)
            {
                ...
                *pCompare = LSA_TRUE;
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,"OHA_CompareTopoPeer_Fragmentation() - IRT not supported (0x%X) !", Local_IRTPortStatus);
                    ...
                    SenderStatus == OHA_STATUS_DATA_INVALID)
                {
                    ...
                    if (OHA_IS_NOT_NULL(pVar) && (Len == OHA_LLDP_PNO_PORT_STATUS_SIZE))
                    {
						...
						else
						{
							...
							{
								OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_CompareTopoPeer_Fragmentation() - Remote_Fragmentation (0x%X) and Local_Fragmentation (0x%X) different!",Remote_IRTFragmentation,Local_IRTFragmentation);
							}
						}
                    }
                    else
                    {
                        OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_CompareTopoPeer_Fragmentation() - Remote PortStatus-Tlv not existing!");
                    }
                }
                else
                {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT32 OHA_DbGetGlobalDiagMode(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CompareTopoInterface_NoD(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_BOOL            *   pCompare)
{
    LSA_UINT16  RetVal  = OHA_OK;
