/*
 * v3.1 August 30, 2011 net/udp_open.c
...
#define LTRC_ACT_MODUL_ID 25 /* INICHE_TCIP_ITGR added for LSA traces */
   ...
   void *   data)       /* random data, returned on upcalls to aid demuxing */
{
   ...
   if (NDEBUG & INFOMSG)
			...
			TCIP_INICHE_TRACE_07(0, LSA_TRACE_LEVEL_ERROR
				, "udp_open: fhost %u.%u.%u.%u, lsock %u, fsock %u, callback data %04x"
				, (unsigned)(fhost&0xff),(unsigned)((fhost>>8)&0xff),(unsigned)((fhost>>16)&0xff),(unsigned)(fhost>>24),lsock, fsock, data
				);
#else
      dprintf("udp_open: fhost %u.%u.%u.%u, lsock %u, fsock %u, callback data %04x\n",
...
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
			TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_ERROR
				, "UDP: Couldn't allocate conn storage."
				);
#else
