/*
 * v3.1 August 30, 2011 ip/ipnet.c
...
#define LTRC_ACT_MODUL_ID 14 /* INICHE_TCIP_ITGR added for LSA traces */
...
Netinit()
{
   ...
   for (i = 0; i < (int)ifNumber; i++)
   {
      ...
      MaxLnh = MAX(MaxLnh, nets[i]->n_lnh); /* replaced max by MAX */
   ...
   for (i = 0; i < (int)ifNumber; i++)
   {
      if (nets[i]->n_init != NULL)  /* If iface has init routine... */
      {
         ...
         TCIP_INICHE_TRACE_02(0,LSA_TRACE_LEVEL_WARN,"init error %d on net[%d]", e, i);
#else
...
netclose()
{
   ...
   for (ifp = (NET)netlist.q_head; ifp; ifp = ifp->n_next)
   {
      ...
      {
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
        TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_WARN,"netclose: closing iface XX"); /*AP00990756, AP01366119 */
#else
      ...
      else
      {
         ...
         if (NDEBUG & INFOMSG) 
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
			TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_WARN
				, "net XX: no close routine!"
				);
#else
			 dprintf("net %s: no close routine!\n", ifp->name);
#endif
#endif
         index++;
      }
...
pktdemux()
{
   ...
   while (rcvdq.q_len)
   {
...
#if 1 /* INICHE_TCIP_ITGR AP00901717: ad drop all pakets from a sending mulicast MAC */
      if((*(pkt->nb_buff + ETHHDR_BIAS + 6) & 0x01) == 1) /* if sender has Multicast MAC drop this packet */
      {
         LOCK_NET_RESOURCE(FREEQ_RESID);
         pk_free(pkt);  /* dump packet from downed interface */
         UNLOCK_NET_RESOURCE(FREEQ_RESID);
...
#ifdef NPDEBUG
      if( pkt->pRQB == LSA_NULL) /* send packets have pRQB==0 */
      {
      ...
      if((ifc->n_flags & NF_NBPROT) == 0)
      {
         ...
         default:    /* driver bug? */
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
         TCIP_INICHE_TRACE_01(0,LSA_TRACE_LEVEL_WARN,"pktdemux: bad Iface type %ld",ifc->n_mib->ifType); 
#else
      ...
      default:
         ...
         if (NDEBUG & UPCTRACE)
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
			TCIP_INICHE_TRACE_01(0, LSA_TRACE_LEVEL_WARN
