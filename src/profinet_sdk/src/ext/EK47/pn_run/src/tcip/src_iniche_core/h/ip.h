...
#ifndef IP_PMTU
#define  IP_FLGS  0          /* Clear fragmentation field */
#else
#define  IP_FLGS  IP_FLG_DF  /* Set don't fragment flag */
#endif
...
#define  IP_TTL   30    /* overridable from ipport.h_h (INICHE_TCIP_ITGR change 64->30) */
#endif
...
#ifdef USE_IGMPV3
     int     inm_state;          /* state of membership */
...
#ifdef USE_IGMPV3
#define  __IPADDR(x)       ((u_long)(x))
#define  IN_LOCAL_GROUP(i) (((u_long)(i) & __IPADDR(0xffffff00)) == \
                           __IPADDR(0xe0000000))
#define  IN_BADCLASS(i)    (((u_long)(i) & __IPADDR(0xf0000000)) == \
                           __IPADDR(0xf0000000))
#define  IN_CLASSA_NET     __IPADDR(0xff000000)
#define  IP_IGMPv3_TOS      0xc0
#endif
...
#ifndef  IP_MAX_MEMBERSHIPS /* INICHE_TCIP_ITGR added #ifndef */ 
 ...
 */
START_PACKED_STRUCT(ip_mreq_source)
     ip_addr imr_multiaddr;     /* IP multicast address of group */
     ip_addr imr_sourceaddr;    /* source address of group */
     ip_addr imr_interface;     /* local IP address of interface */
END_PACKED_STRUCT(ip_mreq_source)

/*
 * Protocol-Independent Multicast Source Filter APIs
 */
START_PACKED_STRUCT(group_req)
     u_long gr_interface;              /* interface index */
     struct sockaddr_storage gr_group; /* group address */
END_PACKED_STRUCT(group_req)


START_PACKED_STRUCT(group_source_req)
     u_long gsr_interface;               /* interface index */
     struct sockaddr_storage gsr_group;  /* group address */
     struct sockaddr_storage gsr_source; /* source address */
END_PACKED_STRUCT(group_source_req)


START_PACKED_STRUCT(group_filter)
     u_long gf_interface;                 /* interface index */
     struct sockaddr_storage gf_group;    /* multicast address */
     u_long gf_fmode;                     /* filter mode */
     u_long gf_numsrc;                    /* number of sources */
     struct sockaddr_storage gf_slist[1]; /* source address */
END_PACKED_STRUCT(group_filter)
...
void IPADDR_TO_NETP(ip_addr addr, NET* netp);
...
#define     ESP_PROTOCOL 0x32 /* Encapsulating Security Payload (ESP) Protocol */
#define     AH_PROTOCOL  0x33 /* Authentication Header (AH) Protocol */
...
#ifdef USE_IGMPV3
int setipv4sourcefilter(int s, struct in_addr iface, struct in_addr multaddr, 
                        unsigned int fmode, unsigned int numsrc, struct in_addr *slist);

int setsourcefilter(int s, unsigned int iface, struct sockaddr *multaddr, int multaddrlen,
                    unsigned int fmode, unsigned int numsrc, struct sockaddr_storage *slist);

int getipv4sourcefilter(int s, struct in_addr iface, struct in_addr multaddr, unsigned int *fmode,
                        unsigned int *numsrc, struct in_addr *slist);

int getsourcefilter(int s, unsigned int iface, struct sockaddr *multaddr, int multaddrlen,
                    unsigned int *fmode, unsigned int *numsrc, struct sockaddr_storage *slist);
#endif   /* USE_IGMPV3  */
