...
#include "compiler.h"
	...
	/* ------------------------------------------------------ */
	OS_BSS_STACK_FAST __attribute__((aligned(32))) PNIO_UINT8 TaskStack[MAXNUM_OF_TASKS][OS_CFG_STACK_SIZE];
#endif
    ...
    PNIO_UINT32 OsInit (void)
    {
		...
		{
			// **** create semaphore ***
			cyg_semaphore_init(&Semaph[i].Mutex,1);

			// *** occupy semaphore ***
			if (!cyg_semaphore_wait(&Semaph[i].Mutex))
			{
							    ...
							    PNIO_UINT16			delay	)
    {
        ...
        cyg_alarm_initialize(OsTimer[timer_id].TxTimer,cyg_current_time()+OsTimer[timer_id].TimeVal_Ticks,OsTimer[timer_id].timer_typ == LSA_TIMER_TYPE_CYCLIC ? OsTimer[timer_id].TimeVal_Ticks : 0);
...
#if(PNIOD_PLATFORM & PNIOD_PLATFORM_ECOS_EB200P)	/* Faster stack for EB200P */
    	TaskProp[Index].pTaskStack = &TaskStack[Index][TASK_STACKSIZE_DEFAULT];
#else
    ...
    PNIO_UINT32 PNIO_snprintf (PNIO_UINT8* str, PNIO_UINT32 count, PNIO_UINT8* fmt, ...)
    {
        ...
        static PNIO_VOID PrintRS232PostTask (PNIO_VOID)
        {
            ...
            PNIO_INT8* pMsg = NULL;            // pointer to Requestblock
            LSA_UINT32 taskID;
    ...
    // *--------------------------------------------*
		...
		void OsReboot (void)
		{
			...
			void OsRebootService (void)
			{
				...
				cyg_interrupt_disable();
				lsas_com_shutdown_network();
				*((unsigned int*)(ERTEC_SCRB_RES_CTRL_REG)) |= (ERTEC_XRES_SOFT /* | WD_SOFT_RES_FREI */);
			}
		#endif
        ...
        void OsReboot (void)
        {
        	HAL_WRITE_UINT32(AT91_RST + AT91_RST_RCR,	/* Reset Control Register */
 							 AT91_RST_RCR_PROCRST |     /* Processor Reset  */
 							 AT91_RST_RCR_ICERST  |     /* ICE Reset        */
 							 AT91_RST_RCR_PERRST  |     /* Peripheral Reset */
 							 AT91_RST_RCR_EXTRST  |     /* External Reset   */
 							 AT91_RST_RCR_KEY     );    /* Write-Key        */
        }
    #else
