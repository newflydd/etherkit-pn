    ...
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    ...
    if (OHA_IS_NOT_NULL(pDB) && pDB->Existing)
    {
    ...
    else
    {
        ...
        pRQB->args.Config.InterfaceID = 0;  /* invalid */
        pRQB->args.Config.NicId     = 0;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestGetAddress(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_GET_ADDRESS failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else if ( (!(address->Options)) || (OHA_IS_NULL(address->pStationData)) )
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_GET_ADDRESS failed. No Parameter to write or read (Options: 0x%X)",pRQB->args.Address.Options);
    }
    ...
    {           /* is no open/close user-channel runnning ? */
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_GET_ADDRESS failed. User Channel is closing or not open for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else
    {
        ...
        if (pStation->pStationType && pStation->StationTypeLen <= OHA_MAX_STATION_TYPE_LEN)
        {
            ...
            if (len && OHA_IS_NOT_NULL(pName))
            {
                ...
                OHA_MEMCPY(pStation->pStationType, pName, (LSA_UINT32)len);
        ...
        if (OHA_IS_NOT_NULL(pStation->pStationName) && pStation->StationNameLen <= OHA_MAX_STATION_NAME_LEN)
        {
            ...
            if (len && pName)
            {
                ...
                OHA_MEMCPY(pStation->pStationName, pName, (LSA_UINT32)len);
        ...
        if ((len == (2*(LSA_INT)sizeof(LSA_UINT16))) && pName)
        {
            address->Options  |= OHA_CTRL_STATION_ID;
        ...
        if ((len == (LSA_INT)(sizeof(LSA_UINT16))) && pName)
        {
            address->Options  |= OHA_CTRL_DEVICE_INSTANCE;
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestGetAddress(Response: 0x%X)", Response);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestSetAddress(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE    pHDBEdd;
         ...
         LSA_HOST_PTR_ARE_EQUAL(pHDBDcp,  LSA_NULL ) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. No Lower Channel for this NicId (NIC-ID: 0x%X)",NICId);
    }
    else if (!(pHDB->State == OHA_HANDLE_STATE_OPEN &&
               LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) ) )
    {           /* is no open/close user-channel runnning ? */
        Response = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. User Channel is closing or not open for this NicId (NIC-ID: 0x%X)",NICId);
    }
    else if ( (!(pAddress->Options)) || (OHA_IS_NULL(pAddress->pStationData)) )
    {
        ...
        Response = OHA_ERR_PARAM;  /* ParameterStatus is wrong */
        /* Error on getting Options. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. No Parameter to write or read (Options: 0x%X)",pRQB->args.Address.Options);
    }
               ...
               pAddress->AccessMode == OHA_QUERY_WRITE))
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. Invalid AccessMode (0x%X)", pRQB->args.Address.AccessMode);
    }
    else
    {
        ...
        if (pAddress->Options & OHA_CTRL_MAC_ADDR)
        {
            ...
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. Set MacAddress in not supported!");
        }
        ...
        if (pAddress->Options & OHA_CTRL_RESET_TO_FACTORY)
        {
            ...
            if (pAddress->Options & OHA_CTRL_STATION_NAME)
            {
                ...
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. ResetToFactory allows no additional setting of NoS!");
            }
            if (pAddress->Options & OHA_CTRL_IP_SUITE)
            {
                ErrorOptions |= OHA_CTRL_RESET_TO_FACTORY;
                ErrorOptions |= OHA_CTRL_IP_SUITE;
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. ResetToFactory allows no additional setting of IP!");
            }
		}
        ...
        if (pAddress->Options & OHA_CTRL_RESET_COMMUNICATION)
        {
            ...
            if (pAddress->Options & OHA_CTRL_STATION_NAME)
            {
                ...
                ErrorOptions |= OHA_CTRL_STATION_NAME;
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. ResetCommunication allows no additional setting of NoS!");
            }
            if (pAddress->Options & OHA_CTRL_IP_SUITE)
            {
                ErrorOptions |= OHA_CTRL_RESET_COMMUNICATION;
                ErrorOptions |= OHA_CTRL_IP_SUITE;
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. ResetCommunication allows no additional setting of IP!");
            }
		}
        ...
        if (pAddress->Options & OHA_CTRL_IP_SUITE)
        {
            ...
            if (OHA_ERR_PARAM == oha_check_ip_address(OHA_GET32_NTOH(&pAddress->pStationData->LocalIpAddr, 0),
                                                      ...
                                                      OHA_IP_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE) /* 0.0.0.0 is allowed by a local set */
            {
                ...
                ErrorOptions |= OHA_CTRL_IP_SUITE;
                OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_ADDRESS failed. Invalid IP-Suite (IP: 0x%X, SubNet: 0x%X, GateWay: 0x%X)",
        ...
        if (pAddress->Options & OHA_CTRL_STATION_NAME)
        {
			...
			{
				if (OHA_ERR_PARAM == oha_check_station_name(pStation->StationNameLen,
		...
		{
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"The initial value of VendorID/DeviceID must be set via OHA_DB_WRITE_STATION_ID()!");
		...
		{
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"The initial value of InstanceID must be set via OHA_DB_WRITE_DEVICE_INSTANCE()!");
		...
		if(ErrorOptions) /* 'error' options for param error existing ? */
		{
            ...
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_RequestSetAddress(Response: 0x%X)", Response);
            return;
		}
		...
		if(ErrorOptions) /* 'error' options for sequence error existing ? */
		{
			...
			pAddress->Options = ErrorOptions;
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"No queueing, Request (Opcode: 0x%X) is already active", OHA_RQB_GET_OPCODE(pHDB->SockUserReq.pBottom));
        ...
        if (pAddress->Options & OHA_CTRL_IP_SUITE)
        {
            ...
            if (OHA_IS_NULL(pHDBDcp->pHDBUserCtrl) && OHA_IS_NOT_NULL(pHDBDcp->pLowerDcpSetRqb))
            {
                ...
                Response = OHA_OK_ACTIVE;
                OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"An OHA controlled Dcp-Set is active: DcpState (0x%X), NicID (0x%X)", pHDBDcp->QueryState, NICId);
                OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestSetAddress(Response: 0x%X)", Response);
                return;
            }
...
/*+               Writes station data to database and confirms request.     +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DoSetAddressRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_DCP_PTR_TYPE      pHDBDcp;
    ...
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_MAC_ADDR) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_IP_SUITE)
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_IP_SUITE) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_STATION_TYPE)
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_STATION_TYPE) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_STATION_NAME)
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_STATION_NAME) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_STATION_ID)
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_STATION_ID) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_DEVICE_INSTANCE)
    {
        LSA_BOOL Remanent = (pAddress->RemaOptions & OHA_CTRL_DEVICE_INSTANCE) ? LSA_TRUE : LSA_FALSE;
    ...
    if (pAddress->Options & OHA_CTRL_RESET_TO_FACTORY)
    {
		...
		(LSA_VOID)OHA_DoRtfRequest(NICId);
    }
    if (pAddress->Options & OHA_CTRL_RESET_COMMUNICATION)
    {
        /* a ResetToFactory is done, now SetIp (0.0.0.0), SetName (""),... */
    ...
    if (pAddress->Options && pAddress->AccessMode == OHA_QUERY_WRITE)
    {
        ...
        if (!pHDBDcp->pHDBUserCtrl) /* store the control-user */
        {
            ...
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SetAddress User-Set successful");
        }
    }
    ...
    if (RetVal != OHA_OK)
    {
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoSetAddressRequest - DCPUpdateFilter(RetVal: 0x%X)", RetVal);
    ...
    if (pAddress->Options & OHA_CTRL_IP_SUITE)
    {
		...
		switch(OHA_RQB_GET_OPCODE(pRQB))
		{
			case OHA_OPC_SET_ADDRESS:   /* ExpectedIp := CurrentIp, EmergencyIp := 0.0.0.0 */
				...
				{
					 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_DoSetAddressRequest: Write Expected IP failed (RetVal=%u).", RetVal);
				}
				...
				if (Remanent)	/* AP00909635 */
				{
					...
					if (RetVal != LSA_RET_OK)
					{
						 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_DoSetAddressRequest: Write REMA IP failed (RetVal=%u).", RetVal);
					}
				}
				else
				{
			...
			case OHA_OPC_SET_EMERGENCY: /* EmergencyIp := CurrentIp */
                RetVal = oha_db_store_emergency_ip_suite(NICId, IpSuite, OHA_IP_SUITE_SIZE);
					if (RetVal != LSA_RET_OK)
					{
						 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_DoSetAddressRequest: Write Emergency IP failed (RetVal=%u).", RetVal);
					}
        ...
        if (RetVal != OHA_OK)
        {
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoSetAddressRequest - EDDSetArpFilter(RetVal: 0x%X)", RetVal);
        }
        RetVal = OHA_EDDSetIp(NICId, OHA_HANDLE_TYPE_DCP);
        if (RetVal != OHA_OK)
        {
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoSetAddressRequest - EDDSetIp(RetVal: 0x%X)", RetVal);
        }
    }
    ...
    if (RetVal != OHA_OK)
    {
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoSetAddressRequest - LLDPUpdateSend(RetVal: 0x%X)", RetVal);
    }
    ...
    RetVal = OHA_NARECheckAddress(NICId, (OHA_CheckIpEnabled(NICId)) ? OHA_CTRL_IP_SUITE : 0,
                                  pAddress->Options);
    if (RetVal != OHA_OK)
    {
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoSetAddressRequest - OHA_NARECheckAddress failed (RetVal: 0x%X)", RetVal);
    }
#endif

	pAddress->Options = 0;	/* no error, all params written */

    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DoSetAddressRequest(Response: 0x%X)", Response);
    return (Response);
}
...
/*+               Fills RQB with station data and confirms request.         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestReadDcpElement(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE      pHDBEdd;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;  /* Mode is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_READ_ELEMENT failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else
    {
        ...
        if (Response == LSA_RET_OK)
        {
            ...
            pDcpElement->Remanent = (Remanent ? OHA_REMANENT_YES : OHA_REMANENT_NO);
        }
    }

    OHA_UserRequestFinish(pHDB,pRQB,Response);

    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestReadDcpElement(Response: 0x%X)", Response);
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DoWriteAddressRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    LSA_UINT16                Response;
    ...
    Response = OHA_DB_WRITE_DCP_ELEMENT(NICId,
                                        ...
                                        pDcpElement->AccCtrl,  /* access control for variable */
                                        (pDcpElement->Remanent == OHA_REMANENT_YES ? LSA_TRUE : LSA_FALSE));/* storage of variable in memory */

	if (Response != OHA_OK)
	{
		OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_DoWriteAddressRequest: Write DCP ELEMENT failed (RetVal=%u).", Response);
	    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DoWriteAddressRequest(Response: 0x%X)", Response);
	    return Response;
	}
    ...
    if (pDcpElement->Type == OHA_DCP_OPTION_TCP && pDcpElement->SubType == DCP_DSID_TCPIP)
    {
		...
		if (pDcpElement->Remanent == OHA_REMANENT_YES)	/* AP00909635 */
		{
			LSA_UINT16 RetVal;
			...
			if (RetVal != LSA_RET_OK)
			{
				 OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "OHA_DoWriteAddressRequest: Write REMA IP failed (RetVal=%u).", RetVal);
			}
		}
		else
		{
	...
	if (Options)
    {
       /* update the LLDP-send data if LLDP-Send is active */
        ...
        if (RetVal != OHA_OK)
        {
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoWriteAddressRequest - LLDPUpdateSend(RetVal: 0x%X)", RetVal);
        }
    ...
    /* Set DHCP ? */
	if (pDcpElement->Type == OHA_DCP_OPTION_DHCP && 
		(pDcpElement->SubType == 61 || pDcpElement->SubType == 255))
    {
   ...
   LSA_UINT16               NICId)
{
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
    	                   "IN : OHA_DoRtfRequest(NICId: 0x%X)", NICId);
	...
	if (pDB->DcpData.DHCPRtFParams.AccCtrl != OHA_NO_ACCESS) /* DHCP RtF parameters written? */
	{
		RetVal = oha_write_dhcp_rtf(NICId);
		if (RetVal != OHA_OK)
		{
			Response = OHA_ERR_FAILED;
			OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoRtfRequest - DHCPParameters(RetVal: 0x%X)", RetVal);
		}
	}
    ...
    {
        Response = OHA_ERR_FAILED;
        OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoRtfRequest - DCPUpdateFilter(RetVal: 0x%X)", RetVal);
    }
    ...
    {
        Response = OHA_ERR_FAILED;
        OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoRtfRequest - EDDSetArpFilter(RetVal: 0x%X)", RetVal);
    }
    RetVal = OHA_EDDSetIp(NICId, OHA_HANDLE_TYPE_DCP);
    if (RetVal != OHA_OK)
    {
        Response = OHA_ERR_FAILED;
        OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DoRtfRequest - EDDSetIp(RetVal: 0x%X)", RetVal);
    }

#ifndef OHA_CFG_NO_NARE
    /* is the IP-Suite to uncheck (ethernet) */
    RetVal = OHA_NARECheckAddress(NICId, 0, OHA_CTRL_IP_SUITE);
    if (RetVal != OHA_OK)
    {
    ...
    (LSA_VOID)OHA_LLDPSetGlobals(NICId, OHA_HANDLE_TYPE_LLDP, LLDP_DEFAULT_MSG_DELAY,
								 LLDP_DEFAULT_TX_REINIT_DELAY, LLDP_DEFAULT_TX_DELAY, OHA_LLDP_TTL_FACTOR);

	RetVal = OHA_SOCKUpdateMib2Params(OHA_HANDLE_TYPE_LLDP,
...
#ifndef OHA_CFG_NO_REMA
	(LSA_VOID)oha_rema_snmp_update(NICId); /* Write to REMA (global NIC) */
	(LSA_VOID)oha_rema_bus_update(NICId);
#endif

    OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DoRtfRequest(Response: 0x%X)", Response);
    return Response;
}
...
/*+               Fills RQB with station data and confirms request.         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestWriteDcpElement(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE      pHDBEdd;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;  /* Mode is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_RequestWriteDcpElement failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else if (!(pHDB->State == OHA_HANDLE_STATE_OPEN &&
               LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) ) )
    {           /* is no open/close user-channel runnning ? */
        Response = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_RequestWriteDcpElement failed. User Channel is closing or not open for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else if (oha_db_read_user_lock(NICId, pHDB->ThisHandle, pDcpElement->Type, pDcpElement->SubType))  /* locked by any user ? */
    {
        ...
        OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_RequestWriteDcpElement failed. Any user has locked (Option: 0x%X, SubOption: 0x%X)", pDcpElement->Type, pDcpElement->SubType);
    }
    else if (pDcpElement->Remanent != OHA_REMANENT_NO && pDcpElement->Remanent != OHA_REMANENT_YES)
    {
        Response = OHA_ERR_PARAM;  /* parameter is wrong */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Remanent(%u) out of range", pDcpElement->Remanent);
    }
    else if (pDcpElement->Type == OHA_DCP_OPTION_TCP && pDcpElement->SubType == DCP_DSID_TCPIP) /* Set IpAddress */
    {
        ...
        else if (OHA_ERR_PARAM == oha_check_ip_address(OHA_GET32_NTOH(pDcpElement->pVariable, 0),
                                                       ...
                                                       OHA_IP_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE) /* 0.0.0.0 is allowed by a local set */
        {
            Response = OHA_ERR_PARAM;  /* ParameterStatus is wrong */
            /* Error on getting IpAddress. Abort. */
            OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_RequestWriteDcpElement failed. Invalid IP-Suite (IP: 0x%X, SubNet: 0x%X, GateWay: 0x%X)",
        ...
        if (Response == OHA_OK) /* ip address is o.k. */
        {
            ...
            {
                Response = OHA_ERR_SEQUENCE;  /* already one request active */
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_WRITE_ELEMENT(IpAddr) failed. Already one User-Request active for this NicId (NIC-ID: 0x%X)",NICId);
            }
            else
            {
                /* SetIp from any other user running ? */
                ...
                if (Response == OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestWriteDcpElement(Response: 0x%X)", Response);
                    return;
                }
            }
        }
    ...
    else if (pDcpElement->Type == OHA_DCP_OPTION_IDENT && pDcpElement->SubType == DCP_DSID_IDNAME)  /* NameOfStation */
    {
        if (pDcpElement->Length != 0 && pDcpElement->Length <= OHA_MAX_STATION_NAME_LEN)
        {
            ...
            if (OHA_ERR_PARAM == oha_check_station_name(pDcpElement->Length, pDcpElement->pVariable,
                                                        OHA_UPPER_CASES_ALLOWED(NICId) ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO,
                                                        OHA_UPPER_CASES_ALLOWED(NICId) ? OHA_NAME_INTERPRETATION_YES : OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE)
            {
                Response = OHA_ERR_PARAM;
	...
	{
		 /* 936497: initial value must be set via OHA_DB_WRITE_STATION_ID(), a runtime value is set via PDIdentificationRecord */
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"The initial value of VendorID/DeviceID is to set via OHA_DB_WRITE_STATION_ID()!");
        Response = OHA_ERR_PARAM;
    }
    else if (pDcpElement->Type == OHA_DCP_OPTION_IDENT && pDcpElement->SubType == DCP_DSID_IDDEVINST)  /* DeviceInstance */
	{
		 /* 936497: initial value must be set via OHA_DB_WRITE_DEVICE_INSTANCE(), a runtime value is set via PDIdentificationRecord */
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"The initial value of InstanceID is to set via OHA_DB_WRITE_DEVICE_INSTANCE()!");
        Response = OHA_ERR_PARAM;
	}
    else if (pDcpElement->Type == OHA_DCP_OPTION_IDENT && pDcpElement->SubType == DCP_DSID_IDOEMID)  /* OEMDeviceID */
	{
        ...
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"The initial value of OEMDeviceID is to set via OHA_OPC_SET_IDENT!");
        Response = OHA_ERR_PARAM;
	}
    ...
    if (OHA_IS_NOT_NULL(pDB))
    {
        switch (AccessControl)
        {
            ...
            case OHA_QUERY_WRITE: /* query the user for a Dcp-SET */
                ...
                break;
            default:
                OHA_PROGRAM_TRACE_00(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Control failed, no such option supported");
...
/*****************************************************************************/
static OHA_DB_ELEM_PTR_TYPE oha_create_DbElement(
...
/*****************************************************************************/
static OHA_DB_ELEM_PTR_TYPE oha_find_DbElement(
    const OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) * pAnchor,   /* start of a list  */
    LSA_UINT8                   Type,       /* type of variable */
...
/*****************************************************************************/
static LSA_UINT16 oha_update_DbElement(
...
/*****************************************************************************/
static LSA_VOID oha_remove_DbElement  (OHA_DB_ELEM_PTR_TYPE pElement)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
...
/* Frees the database-element                                                */
/*****************************************************************************/
static LSA_VOID oha_delete_DbElement (OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) * pAnchor)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
...
/*****************************************************************************/
static LSA_VOID oha_unlock_DbElement (OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) * pAnchor, LSA_UINT16 UserHandle)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
 ...
 *===========================================================================*/
static OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) * oha_get_db_element(
    LSA_UINT16  NicId,
    LSA_UINT8   Type,
    LSA_UINT8   SubType
)
{
    ...
    if (OHA_IS_NOT_NULL(pElement) && OhaListIsNotInitialized(pElement))
    {
		...
		OHA_PROGRAM_TRACE_03(0, LSA_TRACE_LEVEL_FATAL,"oha_get_db_element(pElement: 0x%X, Type: 0x%X, SubType: 0x%X): List not initalized",pElement,Type,SubType);
...
/*****************************************************************************/
static LSA_UINT16 OHA_GetDbElements(OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) * pAnchor,
							 ...
							 LSA_UINT32 Length)
{
    ...
    if (OHA_IS_NOT_NULL(pAnchor))       /* get the element */
    {
        ...
        while (OHA_IS_NOT_NULL(pElement))
        {
			if (pElement->Type == OHA_DCP_OPTION_DHCP && pElement->Length == 0) /* DHCP options not valid */
			{
				pElement = OhaListNext(pAnchor, &pElement->ListElem, OHA_DB_ELEM_PTR_TYPE);
				continue;
			}
			if (pElement->Type == OHA_DCP_OPTION_DHCP && pElement->SubType != 255)
			{
				if (oha_db_is_dhcp_disabled(pAnchor)) /* DHCP client disabled */
				{
					pElement = OhaListNext(pAnchor, &pElement->ListElem, OHA_DB_ELEM_PTR_TYPE);
					continue;
				}
...
)
{
	...
	while (1)
	{
		...
		Count += OHA_GetDbElements(&pDbIf->DcpData.pDhcpOptions, pOptionList, Count, Length); /* DHCP options 61, 255 */
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_WRITE_DCP_ELEMENT(
    LSA_UINT16                  NicId,
...
)
{
    ...
    OHA_FUNCTION_TRACE_06(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_WRITE_DCP_ELEMENT(NicId: 0x%X, Type: 0x%X, SubType: 0x%X, Length: 0x%X, AccCtrl: 0x%X, Remanent: 0x%X)",
                          NicId, Type, SubType, Length, AccCtrl, Remanent);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType) ) /* invalid (sub)option */
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_DCP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
	...
	{
		if (SubType != 61 && SubType != 255)
		{
			RetVal = LSA_RET_ERR_PARAM;
			OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH,"DHCP suboption (0x%X) not supported (NicId: 0x%X)!", SubType, NicId);
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_DCP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
			return RetVal;
		}
	}
	...
	if (Type == OHA_DCP_OPTION_DHCP)    /* e.g. OHA_DB_WRITE_DHCP_OPTION() */
	{
		OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NicId);
		if (OHA_IS_NOT_NULL(pDB))
		{
			pDB->DcpData.DHCPRtFParamsActive = LSA_FALSE; /* a DHCP option was written */
		}
	}
...
)
{
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DB_WRITE_ACCESS_CONTROL(NicId: 0x%X, Type: 0x%X, SubType: 0x%X, AccCtrl: 0x%X)",
                          NicId, Type, SubType, AccCtrl);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_WRITE_ACCESS_CONTROL(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
...
)
{
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_write_remanence(NicId: 0x%X, Type: 0x%X, SubType: 0x%X, RemaStatus: 0x%X)",
                          NicId, Type, SubType, RemaStatus);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        RetVal = LSA_RET_ERR_PARAM;
...
)
{
    ...
    OHA_FUNCTION_TRACE_05(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_DbWriteUserControl(NicId: 0x%X, UsrHandle: 0x%X, Type: 0x%X, SubType: 0x%X, UserControl: 0x%X)",
                          NicId, UserHandle, Type, SubType, UserControl);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DbWriteUserControl(NicId: 0x%X, Type: 0x%X, SubType: 0x%X)", NicId, Type, SubType);
        return RetVal;
    }
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DbWriteUserControl(NicId: 0x%X, UserHandle: 0x%X)", NicId, UserHandle);
        return RetVal;
    }
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DbWriteUserControl(NicId: 0x%X, UserControl: 0x%X)", NicId, UserControl);
        return RetVal;
    }
        ...
        if (OHA_IS_NOT_NULL(pElement))     /* element existing now ? */
        {
			switch (UserControl)
			{
				case OHA_LOCK_SET:
					...
					{
						OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP, "OUT: OHA_DbWriteUserControl(NicId: 0x%X, LockCnt: 0x%X)", NicId, (pElement)->LockCnt[UserHandle]);
    ...
    {
        RetVal = LSA_RET_ERR_RESOURCE;
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DbWriteUserControl(NicId: 0x%X, no such element in the base)", NicId);
    }
                                 ...
                                 LSA_UINT8   SubType)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_read_set_access(NicId: 0x%X, Type: 0x%X, SubType: 0x%X)",
                          NicId, Type, SubType);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_set_access(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    if (OHA_IS_NOT_NULL(pAnchor))
    {
        ...
        else
        {
            ...
            {
                OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "Only the query user has locked (NicId: 0x%X, UserHandle: 0x%X)", NicId, usr_handle);
            }
    ...
    LSA_UINT8 SubType)
{
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_read_user_lock(NicId: 0x%X, UserHandle: 0x%X, Type: 0x%X, SubType: 0x%X)",
                          NicId, UserHandle, Type, SubType);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_user_lock(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    if (OHA_IS_NOT_NULL(pAnchor))
    {
        ...
        if (OHA_IS_NOT_NULL(pElement))  /* element existing ? */
        {
            ...
            if (lock_cnt == 0)                 /* no user has locked       */
            {
				OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_LOW, "No user has locked (NicId: 0x%X, UserHandle: 0x%X)", NicId, UserHandle);
            ...
            if (lock_cnt == 1)                 /* only one user has locked... */
			{
				...
				{
					OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_LOW, "Only the actual user has locked (NicId: 0x%X, UserHandle: 0x%X)", NicId, UserHandle);
					OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_db_read_user_lock(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
					return RetVal;
				}
				...
				{
					OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_LOW, "Only the query user has locked (NicId: 0x%X, UserHandle: 0x%X)", NicId, UserHandle);
					OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_db_read_user_lock(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
					return RetVal;
				}
			}
    ...
    LSA_UINT8   SubType)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_read_query_set_access(NicId: 0x%X, Type: 0x%X, SubType: 0x%X)",
                          NicId, Type, SubType);
    /* ++ check the params ++ check the params ++ */
    if ((!Type) || (!SubType)) /* invalid (sub)option or no element */
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_read_query_set_access(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
    ...
    if (OHA_IS_NOT_NULL(pAnchor))
    {
        ...
        if (OHA_IS_NOT_NULL(pElement))            /* element existing ? */
        {
            ...
            if (lock_cnt == 0)                  /* no user has locked       */
            {
                     ...
                     usr_handle == pHDBDcp->pHDBUserCtrl->ThisHandle)   /* and it's the query user */
            {
                OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "Only the query user has locked (NicId: 0x%X, UserHandle: 0x%X)", NicId, usr_handle);
            }
            else
            {
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_ACCESS_CONTROL(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
...
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT, "IN: OHA_DB_READ_LOCK_STATUS(NicId: 0x%X, Type: 0x%X, SubType: 0x%X)", NicId, Type, SubType);
...
)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT, "IN: OHA_DB_RESET_LOCK_STATUS(NicId: 0x%X, Type: 0x%X, SubType: 0x%X)", NicId, Type, SubType);
    for (i = 0; i < OHA_CFG_MAX_CHANNELS; i++)
    {
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_DB_READ_DCP_ELEMENT(
    LSA_UINT16                  NicId,
    LSA_UINT8                   Type,        /* type of variable, option */
    LSA_UINT8                   SubType,     /* subtype of variable, suboption */
...
)
{
    ...
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DB_READ_DCP_ELEMENT(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID OHA_FreeDbInterface(OHA_DB_IF_PTR_TYPE   pDbIf)
{
    LSA_UINT16           i, j;
	...
	if (OHA_IS_NOT_NULL(pDbIf->DcpData.DHCPRtFParams.ClientIDValue))
	{
		LSA_UINT16   FreeStat = LSA_RET_OK;
		OHA_FREE_LOCAL_MEM(&FreeStat, pDbIf->DcpData.DHCPRtFParams.ClientIDValue);
		OHA_MEM_FREE_FAIL_CHECK(FreeStat);
		OHA_MEMSET(&pDbIf->DcpData.DHCPRtFParams, 0, sizeof(pDbIf->DcpData.DHCPRtFParams));
	}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_FreeDataBase(LSA_VOID)
{
    ...
    {
        if (pDB->Existing)   /* used interface */
        {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_InitRemoteSender                        +*/
/*+  Input/Output          :    ---                                         +*/
/*+  Result                :    ---                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Frees OHA Database elements if allocated.                 +*/
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_InitRemoteSender(
    OHA_LLDP_SENDER_PTR_TYPE pRemoteSender)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_InitRemoteSender(pRemoteSender: 0x%X)", pRemoteSender);

    if (OHA_IS_NOT_NULL(pRemoteSender))
    {
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pChassisId);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pPortId);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pTTL);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pPortDescr);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pSystemName);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pSystemDescr);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pSystemCap);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pMgmAddr);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pOrgSpec_Others);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pOrgSpec_802_1);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pOrgSpec_802_3);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pOrgSpec_Pno);
        OhaListInitialize(&pRemoteSender->DbLLDPDU.pUnknown_Optionals);
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_InitRemoteSender()");
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_InitDBinterface                         +*/
/*+  Input/Output          :    ---                                         +*/
/*+  Result                :    ---                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Frees OHA Database elements if allocated.                 +*/
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID OHA_InitDBinterface(OHA_DB_IF_PTR_TYPE   pDbIf)
{
    LSA_UINT16           i, j;

    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_InitDBinterface(pDbIf: 0x%X)", pDbIf);

	OHA_ASSERT_NULL_PTR(pDbIf);

    OhaListInitialize (&pDbIf->pStationName);

    /* init LLDP-data entries */
    OhaListInitialize(&pDbIf->pTTL);
    OhaListInitialize(&pDbIf->pMgmAddr);
    OhaListInitialize(&pDbIf->pIfDescr_0);

    /* init DCP-data entries */
    OhaListInitialize (&pDbIf->DcpData.pLocalMacAddr);
    OhaListInitialize (&pDbIf->DcpData.pLocalIpSuite);
    OhaListInitialize (&pDbIf->DcpData.pDbaseList);
    OhaListInitialize (&pDbIf->DcpData.pControlOptions);
    OhaListInitialize (&pDbIf->DcpData.pDhcpOptions);

    OhaListInitialize (&pDbIf->DcpData.pStationType);
    OhaListInitialize (&pDbIf->DcpData.pStationId);
    OhaListInitialize (&pDbIf->DcpData.pProfinetRole);
    OhaListInitialize (&pDbIf->DcpData.pDevInstance);
    OhaListInitialize (&pDbIf->DcpData.pOEMDeviceID);

    OhaListInitialize (&pDbIf->DcpData.pPB_Common);
    OhaListInitialize (&pDbIf->DcpData.pPB_Addition);
    OhaListInitialize (&pDbIf->DcpData.pPB_Others);

    OhaListInitialize (&pDbIf->DcpData.pDeviceInitiative);

    for (i = 0; i < OHA_CFG_MAX_PORTS; i++)  /* over all ports */
    {
        OhaListInitialize(&pDbIf->LldpData[i].pPortId);
        OhaListInitialize(&pDbIf->LldpData[i].pIfDescr);
        OhaListInitialize(&pDbIf->LldpData[i].pOrgSpec_Others);
        OhaListInitialize(&pDbIf->LldpData[i].pOrgSpec_802_1);
        OhaListInitialize(&pDbIf->LldpData[i].pOrgSpec_802_3);
        OhaListInitialize(&pDbIf->LldpData[i].pOrgSpec_Pno);
        OhaListInitialize(&pDbIf->LldpData[i].pUnknown_Optionals);

        for (j = 0; j < LLDP_MAX_PORT_SENDER; j++)  /* over all ports */
        {
            OHA_LLDP_SENDER_PTR_TYPE pRemoteSender = &pDbIf->LldpData[i].RemoteSender[j];
            OHA_InitRemoteSender(pRemoteSender);
        }
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_InitDBinterface()");
    return;
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_InitDatabase                            +*/
/*+  Input/Output          :    ---                                         +*/
/*+  Result                :    ---                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Inits OHA Database (list) elements.                       +*/
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_InitDatabase(LSA_VOID)
{
    OHA_DB_IF_PTR_TYPE   pDB  = &oha_database.IfData[0];
    LSA_UINT32           i    = 0;
    ...
    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "IN : OHA_InitDatabase()");

    OhaListInitialize(&oha_database.StationData.pChassisId); /* NameOfDevice */
    OhaListInitialize(&oha_database.StationData.pSysName);  /* system - name */
    OhaListInitialize(&oha_database.StationData.pSysDescr); /* system - description */
    OhaListInitialize(&oha_database.StationData.pSysCap);   /* system - capabilities */

    OhaListInitialize(&oha_database.StationData.pSysContact); /* system - contact */
    OhaListInitialize(&oha_database.StationData.pSysLocation); /* system - location */

    while ( i < OHA_CFG_MAX_NICS )
    {
        OHA_InitDBinterface(pDB);
        pDB++;
        i++;
    }
    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_InitDatabase()");
    return;
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_UnlockDbUser(OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
    ...
    if (OHA_IS_NOT_NULL(pHDBUser))
    {
        ...
        {
            if (pDbIf->Existing)   /* used interface */
            {
 ...
 *===========================================================================*/
LSA_UINT16 oha_db_store_temp_ip_address(
    LSA_UINT16                    NicId,
    ...
    LSA_INT                       AddrLen)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_store_temp_ip_address(NicId: 0x%X, IpSuitePtr: 0x%X, AddrLen: 0x%X)",
                          NicId, IpSuitePtr, AddrLen);

    if (OHA_IS_NOT_NULL(pDB) && (AddrLen == OHA_IP_ADDR_SIZE) &&
        OHA_IS_NOT_NULL(IpSuitePtr) && OHA_IS_NOT_NULL(NetMaskPtr) && OHA_IS_NOT_NULL(RouterPtr))
    {
        /* IpAddr, SubnetMask, RouterAddr */
        if (OHA_OK == oha_check_ip_address(OHA_GET32_NTOH(IpSuitePtr,0),
                                           OHA_GET32_NTOH(NetMaskPtr,0),
                                           OHA_GET32_NTOH(RouterPtr,0),
                                           OHA_IP_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE)
        {
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: oha_db_store_temp_ip_address(RetVal: 0x%X)", RetVal);
    return RetVal;
}
 ...
 *===========================================================================*/
LSA_UINT16 oha_db_read_temp_ip_address(
    LSA_UINT16                      NicId,
    ...
    LSA_BOOL                      * pRemanent )   /* storage of variable in memory */
{
	...
	OHA_ASSERT_NULL_PTR(IpSuitePtrPtr);
 ...
 *===========================================================================*/
LSA_INT oha_db_read_temp_station_name(
    LSA_UINT16  NicId,
    ...
    LSA_BOOL                          * pRemanent )   /* storage of variable in memory */
{
    ...
    OHA_ASSERT_NULL_PTR(pDbIf);
    OHA_ASSERT_NULL_PTR(NamePtrPtr);
 ...
 *===========================================================================*/
LSA_UINT16 oha_db_delete_temp_ip_address(
    LSA_UINT16                      NicId)
{
    OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NicId);
    LSA_UINT16 RetVal = LSA_RET_OK;
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_db_delete_temp_ip_address(NicId: 0x%X)", NicId);

    /* element exists */
    if (OHA_IS_NULL(pDB))
    {
        RetVal = LSA_RET_ERR_RESOURCE;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE,
                              "OUT: oha_db_delete_temp_ip_address(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return (RetVal);
    }
 ...
 * return value:   LSA_UINT16:              LSA_RET_OK, LSA_RET_ERR_RESOURCE
 *===========================================================================*/
LSA_UINT16 oha_db_delete_temp_station_name(
    LSA_UINT16                      NicId)
{
    OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NicId);
    LSA_UINT16 RetVal = LSA_RET_OK;
    ...
    if (OHA_IS_NULL(pDB))
    {
        RetVal = LSA_RET_ERR_RESOURCE;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE,
                              "OUT: oha_db_delete_temp_station_name(NicId: 0x%X, RetVal: 0x%X)", RetVal, NicId);
        return (RetVal);
    }
 ...
 *                                              cannot be changed
 *===========================================================================*/
LSA_UINT16 oha_db_store_temp_station_name(
    LSA_UINT16                    NicId,
    LSA_UINT8                   * NamePtr,
    LSA_UINT16                    NameLen)
{
    ...
    if (NameLen > OHA_MAX_STATION_NAME_LEN)
    {
        RetVal = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE,
                              "OUT: oha_db_store_temp_station_name(NicId: 0x%X, RetVal: 0x%X)", RetVal, NicId);
        return RetVal;
    }
    ...
    {
        /* check, if it's a RFC-name */
        if (OHA_ERR_PARAM == oha_check_station_name((LSA_UINT16)NameLen, NamePtr,
                                                    OHA_UPPER_CASES_ALLOWED(NicId) ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO,
                                                    OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE)
        {
            RetVal = LSA_RET_ERR_PARAM;
        }
    }
    ...
    LSA_UINT16 DcpStatus)
{
    ...
    if (Response != LSA_RET_OK)
    {
        ...
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_db_compare_remanence(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return RetVal;
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestAddressIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    OHA_HDB_EDD_PTR_TYPE      pHDBEdd;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;  /* Mode is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_ADDR_INFO_IND failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else if (!(pHDB->State == OHA_HANDLE_STATE_OPEN &&
               LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) ) )
    {           /* is no open/close user-channel runnning ? */
        Response = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP," OHA_OPC_ADDR_INFO_IND failed. User Channel is closing or not open for this NicId (NIC-ID: 0x%X)", NICId);
    }
    else
    {   /* only one RQB for one NICId the same time */
        ...
        {
            Response = OHA_ERR_SEQUENCE;
            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP, "SequenceError: only one RQB for one NICId (0x%X) allowed!", NICId);
        }
        else
        {
            /* store the IndRQB */
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestAddressIndication(Response: 0x%X)", Response);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_AddressIndicateEventAllUser(
    LSA_UINT16                              NICId
)
{
    LSA_UINT16 i;
    ...
    for (i=0; i < OHA_CFG_MAX_CHANNELS; i++)
    {
        ...
        if (OHA_IS_NOT_NULL(pHDBIfUser) && (pHDBIfUser->State == OHA_HANDLE_STATE_OPEN) ) /* user channel is opened ? */
        {
			if(pHDBIfUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER)
			{
				...
				if(NICId == pHDBIfUser->Params.pDetail->Params.User.NICId)
				{
					(LSA_VOID)OHA_AddressIndicateEvent(NICId, pHDBIfUser);/* address changed - indication */
				}
			}
		}
    }
...
/*+  Description: Indicates a DIAG-Event to one user                        +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_AddressIndicateEvent(
    LSA_UINT16                              NICId,
    OHA_HDB_USER_PTR_TYPE                   pHDB
)
{
    ...
    {
        RetVal = LSA_RET_ERR_RESOURCE;
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_AddressIndicateEvent(RetVal: 0x%X)", RetVal);
        return (RetVal);
    }
          ...
          LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) ) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_AddressIndicateEvent failed. User Channel is closing or not open for this Interface (If-ID: 0x%X)", pDB->InterfaceID);
    }
    else
    {
...

        {
            ...
            if (IpIsTheSame)
            {
                if ((!(pAddress->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE) && pDB->DcpData.IpConflictDetected) ||
                    ((pAddress->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE) && !pDB->DcpData.IpConflictDetected))
                {
			...
			/* AP01270055: OHA_ADDR_INFO_TYPE: Missing information by AddressConflict of an EmergencyIP */
			CurrentIPSuiteIsEmergency = pHDBEdd->SetAddrInfo.CurrentIPSuiteIsEmergency ? LSA_TRUE : LSA_FALSE;

            if (IpIsTheSame)
            {
                if ((!(pAddress->CurrentIPSuiteIsEmergency == OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES ? LSA_TRUE : LSA_FALSE) && CurrentIPSuiteIsEmergency) ||
                    ((pAddress->CurrentIPSuiteIsEmergency == OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES ? LSA_TRUE : LSA_FALSE) && !CurrentIPSuiteIsEmergency))
                {
                    IpIsTheSame = LSA_FALSE;
                }
            }
            ...
            {
				IpIsTheSame = oha_db_compare_remanence(NICId, DCP_DBID_TCP, DCP_DSID_TCPIP, (pAddress->IpSuiteRemanent == OHA_REMANENT_YES) ? (LSA_UINT16)DCP_MASK_DS_REMANENT : (LSA_UINT16)0);
            }
            ...
            if (!IpIsTheSame)
            {
				...
				{
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,
				...
				if ((!(pAddress->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE) && pDB->DcpData.IpConflictDetected) ||
					 ((pAddress->CurrentIPSuiteConflictDetected == OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES ? LSA_TRUE : LSA_FALSE) && !pDB->DcpData.IpConflictDetected))
				{
					OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,
                ...
                pAddress->CurrentIPSuiteConflictDetected = (pDB->DcpData.IpConflictDetected ? OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES : OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_NO);
                ...
                pAddress->CurrentIPSuiteIsEmergency = (CurrentIPSuiteIsEmergency ? OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES : OHA_CURRENT_IPSUITE_IS_EMERGENCY_NO);

				IsTheSameForTrace = oha_db_compare_remanence(NICId, DCP_DBID_TCP, DCP_DSID_TCPIP, (pAddress->IpSuiteRemanent == OHA_REMANENT_YES) ? (LSA_UINT16)DCP_MASK_DS_REMANENT : (LSA_UINT16)0);
                ...
                pAddress->IpSuiteRemanent = (oha_db_compare_remanence(NICId, DCP_DBID_TCP, DCP_DSID_TCPIP, DCP_MASK_DS_REMANENT) ? OHA_REMANENT_YES : OHA_REMANENT_NO); /* AP01462749 */
        ...
        if (OHA_IS_NOT_NULL(pAddress->NoS.pName))
        {
            ...
            if (NoSIsTheSame)
            {
                if (OHA_IS_NOT_NULL(pAddress->NoS.pName))
                {
                    NoSIsTheSame = OHA_MEMCMP(pAddress->NoS.pName, NamePtr, (LSA_UINT32)str_len);
                }
            }
            ...
            {
				NoSIsTheSame = oha_db_compare_remanence(NICId, DCP_DBID_IDENT, DCP_DSID_IDNAME, (pAddress->NoS.Remanent == OHA_REMANENT_YES) ? (LSA_UINT16)DCP_MASK_DS_REMANENT : (LSA_UINT16)0);
            }
            ...
            if (!NoSIsTheSame)
            {
                if (OHA_IS_NOT_NULL(pAddress->NoS.pName))
                {
                    OHA_MEMCPY(pAddress->NoS.pName, NamePtr, (LSA_UINT32)str_len);
                }
                pAddress->NoS.Length  = (LSA_UINT16)str_len;
                pAddress->NoS.Remanent = (oha_db_compare_remanence(NICId, DCP_DBID_IDENT, DCP_DSID_IDNAME, DCP_MASK_DS_REMANENT) ? OHA_REMANENT_YES : OHA_REMANENT_NO); /* AP01462749 */

                NameOfStationChanged  = LSA_TRUE;
            }
        }

        pAddress->FactoryReset = (pDB->DcpData.FactoryReset ? OHA_FACTORY_RESET_YES : OHA_FACTORY_RESET_NO);
        if (pDB->DcpData.FactoryReset) /* AP01565375 */
        {
			OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,
								 "OHA-AddrInfoInd(NICId=0x%X): FactoryReset(0x%X) is done (0x%X).",
								 NICId, pDB->DcpData.FactoryReset, pAddress->FactoryReset);
        }

        if (IpAddrChanged || NameOfStationChanged || pDB->DcpData.FactoryReset)
        {
...
)
{
    ...
    pAddress->CurrentIPSuiteConflictDetected = pDB->DcpData.IpConflictDetected ? OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_YES : OHA_CURRENT_IPSUITE_CONFLICT_DETECTED_NO;
	pAddress->CurrentIPSuiteIsEmergency = pHDBEdd->SetAddrInfo.CurrentIPSuiteIsEmergency ? OHA_CURRENT_IPSUITE_IS_EMERGENCY_YES : OHA_CURRENT_IPSUITE_IS_EMERGENCY_NO;
    pAddress->IpSuiteRemanent = oha_db_compare_remanence(NICId, DCP_DBID_TCP, DCP_DSID_TCPIP, DCP_MASK_DS_REMANENT) ? OHA_REMANENT_YES : OHA_REMANENT_NO; /* AP01462749 */
	...
	if (OHA_IS_NOT_NULL(pAddress->NoS.pName))
    {
		...
		{
			OHA_MEMCPY(pAddress->NoS.pName, pName, (LSA_UINT32)str_len);
		}
		else
		{
 ...
 * return value:   LSA_UINT16:                          Error
 *===========================================================================*/
LSA_UINT16 oha_db_store_expected_ip_suite(
    LSA_UINT16                    NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr,
    LSA_INT                       AddrLen)
{
 ...
 * return value:   LSA_UINT16:              Length
 *===========================================================================*/
LSA_UINT16 oha_db_read_expected_ip_suite(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE      pAccCtrl,    /* access control for variable */
    LSA_BOOL                      * pRemanent )   /* storage of variable in memory */
 ...
 *===========================================================================*/
LSA_VOID oha_db_delete_expected_ip_suite(
    LSA_UINT16                    NicId)
{
    OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
 ...
 * return value:   LSA_UINT16:                          Error
 *===========================================================================*/
LSA_UINT16 oha_db_store_emergency_ip_suite(
    LSA_UINT16                    NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr,
    LSA_INT                       AddrLen)
{
 ...
 * return value:   LSA_UINT16:              Length
 *===========================================================================*/
LSA_UINT16 oha_db_read_emergency_ip_suite(
    LSA_UINT16                      NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    * IpSuitePtrPtr,
    OHA_COMMON_MEM_U8_PTR_TYPE      pAccCtrl,    /* access control for variable */
    LSA_BOOL                      * pRemanent )   /* storage of variable in memory */
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_VOID oha_db_delete_emergency_ip_suite(
    LSA_UINT16                    NicId)
{
    OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
    LSA_UINT16 RetVal = LSA_RET_ERR_RESOURCE;
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_VOID oha_db_delete_profibus_params(
    LSA_UINT16                    NicId)
{
    OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
    LSA_UINT16 RetVal = LSA_RET_ERR_RESOURCE;
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_VOID oha_db_write_ip_conflict_status(
    LSA_UINT16                  NicId,
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_UINT16 oha_db_read_ip_conflict_status(
    LSA_UINT16                  NicId,
    LSA_BOOL *                  ConflictStatus)
{
    OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
	...
	OHA_ASSERT_NULL_PTR(ConflictStatus);
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_VOID oha_db_write_factory_reset_status(
    LSA_UINT16                  NicId,
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_UINT16 oha_db_delete_real_ip_suite(
    LSA_UINT16                    NicId)
{
    LSA_UINT16 RetVal = LSA_RET_ERR_PARAM;
    OHA_COMMON_MEM_U8_PTR_TYPE  IpAddrPtr     = LSA_NULL;
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_UINT16 oha_db_set_real_ip_suite(
    LSA_UINT16                  NicId,
 ...
 * return value:   LSA_VOID
 *===========================================================================*/
LSA_UINT16 oha_db_delete_station_name(
    LSA_UINT16                    NicId)
{
    LSA_BOOL   Remanent = LSA_FALSE;
    ...
    {
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH,
                              "oha_db_delete_station_name: NameOfStation not existing (NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
    }
 ...
 *===========================================================================*/
LSA_UINT16 oha_db_write_snmp_variable (
	...
	LSA_BOOL                      IsRema)
{
	...
	if (OHA_IS_NOT_NULL(pAnchor))
	{
		...
		if (OhaListIsNotInitialized(pAnchor)) /* only initialized with NULL */
		{
			...
			OHA_PROGRAM_TRACE_03(0, LSA_TRACE_LEVEL_FATAL,"oha_db_write_snmp_variable(pAnchor: 0x%X, VarType: 0x%X, VarLen: 0x%X): List not initalized",pAnchor,VarType,VarLen);
			OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
		}
 ...
 * return value:   LSA_UINT16:                          Error
 *===========================================================================*/
LSA_UINT16 oha_db_store_rema_ip_address(
    LSA_UINT16                    NicId,
    OHA_COMMON_MEM_U8_PTR_TYPE    IpSuitePtr)
{
    OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
 ...
 * return value:   LSA_UINT16:              Length
 *===========================================================================*/
LSA_UINT16 oha_db_read_rema_ip_address(
    LSA_UINT16                      NicId,
 ...
 * return value:   LSA_UINT16:              LSA_RET_OK, LSA_RET_ERR_RESOURCE
 *===========================================================================*/
LSA_UINT16 oha_db_delete_rema_ip_address(
    LSA_UINT16                      NicId)
{
    OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NicId);
    LSA_UINT16 RetVal = LSA_RET_OK;
    ...
    if (OHA_IS_NULL(pDB))
    {
        RetVal = LSA_RET_ERR_RESOURCE;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE,
                              "OUT: oha_db_delete_rema_ip_address(NicId: 0x%X, RetVal: 0x%X)", NicId, RetVal);
        return (RetVal);
    }
 ...
 * return value:   LSA_UINT16:              LSA_RET_OK, LSA_RET_ERR_RESOURCE
 *===========================================================================*/
LSA_UINT16 oha_db_update_rema_ip_address(
    LSA_UINT16                      NICId)
{
    LSA_UINT16 RetVal = LSA_RET_OK;
    OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NICId);
	...
	if (oha_db_compare_remanence(NICId, DCP_DBID_TCP, DCP_DSID_TCPIP, DCP_MASK_DS_REMANENT))
	{
		...
		if (OHA_IS_NOT_NULL(pIpAddr))
		{
			...
			if (RetVal != LSA_RET_OK)
			{
				OHA_PROGRAM_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "oha_db_update_rema_ip_address: Write REMA IP failed (RetVal=%u).", RetVal);
			}
		}
	}
	else
    ...
    LSA_UINT16 NICId)
{
	...
	{
		if (pDB->Existing && pDB->NicId == NICId)
		{
			return(pDB->TraceIdx);
		}
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 oha_undo_init_interface(
    LSA_UINT16 NICId)
{
    OHA_DB_IF_PTR_TYPE  pDB = OHA_DB_GET_INTERFACE(NICId);
    ...
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    ...
    LSA_UINT16              Response;
    LSA_UINT16              NICId;
    OHA_UPPER_IDENT_PTR_TYPE pIdent;
         ...
         LSA_HOST_PTR_ARE_EQUAL(pHDBDcp,  LSA_NULL ) )
    {
        Response = OHA_ERR_RESOURCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_IDENT failed. No Lower Channel for this NicId (NIC-ID: 0x%X)",NICId);
    }
    else if (!(pHDB->State == OHA_HANDLE_STATE_OPEN &&
               LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) ) )
    {           /* is no open/close user-channel runnning ? */
        Response = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_IDENT failed. User Channel is closing or not open for this NicId (NIC-ID: 0x%X)",NICId);
    }
    else if (pIdent->Options == 0)   /* 'nothing to set ? */
    {
        Response = OHA_ERR_PARAM;  /* ParameterStatus is wrong */
        /* Error on getting Options. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_SET_IDENT failed. No Parameter to write (Options: 0x%X)",pRQB->args.Ident.Options);
    }
    else
    {
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestSetIdent(Response: 0x%X)", Response);
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DoSetIdentRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    LSA_UINT16                Response = OHA_OK;
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DoSetIdentRequest(Response: 0x%X)", Response);
    return (Response);
}


/*****************************************************************************/
/* Local function: OHA_IsDHCPDisabled()                                      */
/* if DHCP is disabled, the DHCP options (e.g. 61) should'nt be readable     */
/* the response should be „Suboption not supported“                          */
/*****************************************************************************/
LSA_BOOL oha_db_is_dhcp_disabled(const OHA_LIST_TYPED_ENTRY_TYPE(oha_db_elem_tag) *pAnchor)
{
	OHA_DB_ELEM_PTR_TYPE pElement;
	LSA_BOOL DHCPDisabled = LSA_FALSE;

	OHA_ASSERT_NULL_PTR(pAnchor);

	pElement = oha_find_DbElement(pAnchor, OHA_DCP_OPTION_DHCP, 255);

	if (OHA_IS_NOT_NULL(pElement))
	{
		if (pElement->Length >= 3 && OHA_IS_NOT_NULL(pElement->pVariable))
		{
			OHA_COMMON_MEM_U8_PTR_TYPE pOpt255 = pElement->pVariable;

			if (*(pOpt255) == 255/*code*/ && *(pOpt255 + 1) == 1/*len*/)
			{
				OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE,
					"OUT: oha_db_is_dhcp_disabled(), usage: 0x%X", *(pOpt255 + 2));

				if (*(pOpt255 + 2) == 0 || *(pOpt255 + 2) == 1) /*not use DHCP*/
				{
					DHCPDisabled = LSA_TRUE;
				}
			}
		}
	}
