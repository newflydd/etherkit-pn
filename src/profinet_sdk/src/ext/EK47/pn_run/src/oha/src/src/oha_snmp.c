...
static int  oha_goodValue(U_CHAR * value, U_CHAR inType, int inLen,
                       U_CHAR actualType, int actualLen);
static int  oha_setVariable(U_CHAR * var_val, U_CHAR var_val_type, int var_len,
    ...
    LSA_INT32     version)
{
...

    {
		...
		else
		{
			...
			if(oha_goodValue(var_val, *var_type, (int)(*var_len),
							   stat_type, (int)stat_len) /*!= TRUE*/)
			{
			...
			if (msg_type == OHA_SNMP_SET_REQ)
			{
				...
				if (oha_set_parms.access_method)
				{
					...
					err = (*oha_set_parms.access_method)(OHA_HANDLE_TYPE_USER, var_val, *var_type, (int)(*var_len),
														 oid_name, *oid_len);
				}
				else     /* generic set of variable */
				{
					err = oha_setVariable(var_val, *var_type, (int)(*var_len),
												element, (int)stat_len);
                      ...
                      unsigned * listlength)    /* IN/OUT - number of valid bytes left in var_op_list */
{
	...
	OHA_ASSERT_NULL_PTR(listlength);
	var_op_len = *listlength;
    ...
    *var_val = data;     /* save pointer to this object */
    ...
    OHA_ASSERT_NULL_PTR(var_val_len);
    OHA_ASSERT_NULL_PTR(var_op_start);
    ...
    *listlength -= headerLen;
    ...
    switch (var_val_type)
    {
        case ASN_INTEGER:
            ...
            break;
        case OHA_SNMP_GAUGE: /* 1444139 */
        case OHA_SNMP_COUNTER:
        case OHA_SNMP_TIMETICKS:
            data = oha_asn_build_unsigned_int(data, listlength, var_val_type,
                                              (LSA_UINT32 *) var_val, var_val_len);
      break;
        case ASN_BIT_STR:
    ...
    int         version)
{
	...
	if (vp == LSA_NULL)  /* OHA_ASSERT_NULL_PTR(vp); */
	{
	...
	OHA_ASSERT_NULL_PTR(type);
	OHA_ASSERT_NULL_PTR(acl);
    *type = (U_CHAR)(vp->type);
    *acl = vp->acl;
...
LSA_BOOL  oha_snmp_build_adjust_len(
    ...
    unsigned *  len_final)
{
    int   len_pkt_encoding;
    U_CHAR   hdr_type;           /* Retain the hdr_type */
	...
	OHA_ASSERT_NULL_PTR(start_data);
	OHA_ASSERT_NULL_PTR(start_hdr);

	len_pkt_encoding  =  start_data  -  start_hdr   ;
	hdr_type       = *   start_hdr   ;           /* Retain the hdr_type */

    diff = len_pkt_encoding - oha_snmp_sizeof_int((LSA_INT32)len_data);
    ...
    if ( len_final != LSA_NULL )      /* Return the final len starting from header */
        *len_final = len_data + (unsigned)oha_snmp_sizeof_int((LSA_INT32)len_data);
...
static int oha_is_us_ascii(const char * str,int len)
{
    int match;
    int i;

	OHA_ASSERT_NULL_PTR(str);
    ...
    for (i=0; i < len; i++)
    {
        ...
        switch (str[i])
        {
            ...
            case 13:   /* CR  - carriage return */
                ...
                if ((len == i+1) || (str[i+1] == 0) || (str[i+1] == 10)) /* lint: already last char */
                    match=1;
                break;
            default:
    ...
    int      actualLen)
{
    ...
    switch (actualType)
    {
        ...
        case OHA_SNMP_TIMETICKS:
            ...
            if (oha_parse_int(value, (unsigned)inLen, inType, &int_value, sizeof(LSA_INT32) ))
                return V3_VB_WRONGLENGTH;
            if (actualType == OHA_SNMP_IPADDRESS)  /* No range checking required for IP Address */
                break;
        ...
        case OHA_SNMP_OPAQUE:
            ...
            if ((actualType == ASN_OCTET_STR) && (!(oha_set_parms.vbflags & SF_NOASCIICHK)))
            {
			    ...
			    OHA_MEMCPY((char *)buf, (char *)value, (LSA_UINT32)inLen);
    ...
    int      statLen)    /* var_len set by var_routine */
{
    ...
    switch (var_val_type)
    {
        ...
        case OHA_SNMP_GAUGE: /* valid for GAUGE32 and UNSIGNED32 too */
        case OHA_SNMP_TIMETICKS:
            if (oha_parse_int(var_val, (unsigned)var_len, var_val_type,
        ...
        case ASN_BIT_STR:
            ...
            OHA_MEMCPY((char *)statP, (char *)var_val, set_var_size);
            break;
        case ASN_OCTET_STR:
        case OHA_SNMP_IPADDRESS:
        case OHA_SNMP_OPAQUE:
            ...
            break;
        case ASN_OBJECT_ID:
            set_var_size /= sizeof(OHA_OID);  /* oids in field to set */
...
#endif
		default:
            OHA_SNMPERROR("oha_setVariable(): bad type");
            break;
    }
    ...
    int      statLen)    /* var_len set by var_ routine */
{
    ...
    switch (var_val_type)
    {
        ...
        case ASN_OBJECT_ID:
            {
            OHA_OID  set_var_buffer[OHA_MAX_NAME_LEN];
            set_var_size = OHA_MAX_OID_LEN;  /* oids in field to set */
            if (!oha_asn_parse_objid(var_val, &asn_buf_size, &var_val_type,
                                     set_var_buffer, &set_var_size))
            {
                return V3_VB_BADVALUE;
            }
            }
    ...
    int count)
{
	...
	OHA_ASSERT_NULL_PTR(bytebuf);
	OHA_ASSERT_NULL_PTR(oidbuf);
    ...
    LSA_UINT *  datalength)   /* IN/OUT - number of valid bytes left in buffer */
{
    ...
    if (OHA_IS_NULL(data) || OHA_IS_NULL(datalength) || *datalength == 0 || bp == LSA_NULL)
    {
    ...
    LSA_UINT *  objidlength)   /* IN/OUT - number of sub-id's in objid */
{
    ...
    if (OHA_IS_NULL(bufp) || datalength == 0 || datalength > LLDP_TLV_MGM_ADDR_MAX_OID_STRING_LEN ||
    ...
    *objidlength = (LSA_UINT)(oidp - objid);
