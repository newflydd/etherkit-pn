...
static LSA_UINT16  RecLenPeerDataFailed(LSA_UINT32 err_offset)
{
    LSA_UINT16 RetVal = OHA_ERR_PRM_BLOCK;
    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers: Length of PeerData failed (err_offset: 0x%X).", err_offset);
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
    return(RetVal);
}

static LSA_UINT16  RecLenTopoPeersFailed(LSA_UINT32 err_offset)
{
    LSA_UINT16 RetVal = OHA_ERR_PRM_BLOCK;
    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CopyTopoPeersToDB: Length of TopoPeers failed (err_offset: 0x%X).", err_offset);
    ...
    LSA_UINT16                Response)
{
    ...
    /* 1361434: Unexpected return value with SFB52 RDREC [reading PDInterfaceDataReal, e.g., from PDev] */
    /* BufferOverflow is not a defined error of the Prm interface (PrmRead), so we map to ErrPrmData    */
    if ((Response == OHA_ERR_BUFFER_OVERFLOW) && OHA_RQB_GET_OPCODE(pRQB) == OHA_OPC_PRM_READ)
    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_UserRequestFinish(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_USER_PTR_TYPE OHA_GetUserHDBFromHandle(
...
/*+               Handle has to be a User-Channel-Handle!!                  +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_EDD_PTR_TYPE OHAGetEddHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
...
/*+               Handle has to be a User-Channel-Handle!!                  +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_DCP_PTR_TYPE OHAGetDcpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
...
OHA_HDB_NARE_PTR_TYPE OHAGetNareHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
...
/*+               Handle has to be a User-Channel-Handle!!                  +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_LLDP_PTR_TYPE OHAGetLldpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
...
/*+               Handle has to be a User-Channel-Handle!!                  +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_SOCK_PTR_TYPE OHAGetSockHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHAGetDnsHDBFromHandle                      +*/
/*+  Input/Output          :    LSA_HANDLE_TYPE     Handle                  +*/
...
/*+                                                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Gets Pointer to DNS-HDB for Handle.                       +*/
/*+               Handle has to be a User-Channel-Handle!!                  +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#ifdef OHA_CFG_USE_DNS
OHA_HDB_DNS_PTR_TYPE OHAGetDnsHDBFromHandle(
    LSA_HANDLE_TYPE     Handle)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHAGetDnsHDBFromHandle(Handle: 0x%X)",
                          Handle);
    if ( OHA_HANDLE_IS_DNS(Handle))
    {
        Idx = (LSA_UINT16) OHA_HANDLE_IDX(Handle);   /* Get DNS-Idx */

        if ( Idx == 0 ) /* one SOCK channel only */
        {
            pHDB = OHA_GET_HDNS_PTR();
        }
        else
        {
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Invalid DNS handle (0x%X). Out of range.",Handle);
            pHDB = LSA_NULL;
        }
    }
    else
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Invalid DNS handle (0x%X). No DNS handle.",Handle);
        pHDB = LSA_NULL;
    }
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: OHAGetDnsHDBFromHandle(pHDB: 0x%X)",
                          pHDB);
    return(pHDB);
}
#endif
...
/*+               this entry.                                               +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_CancelDcpControlUser(
...
)
{
    ...
    {
		OHA_HDB_DCP_PTR_TYPE pHDBDcp  = OHA_GET_HDCP_PTR(i);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_CheckDcpControlUser(
    OHA_HDB_USER_PTR_TYPE         pHDBUser
)
{
    LSA_UINT16                  i        = 0;
    ...
    while ( i < OHA_CFG_MAX_NICS)
    {
        ...
        if (OHA_IS_NOT_NULL(pHDBDcp) && pHDBDcp->DcpState == OHA_HANDLE_STATE_OPEN && /* dcp-channel is opened ? */
            (LSA_HOST_PTR_ARE_EQUAL(pHDBUser,pHDBDcp->pHDBUserCtrl)) )
        {
            /* 2347675 - OHA fatal during closing EB200P */
    ...
    LSA_UINT32 InterfaceID, OHA_COMMON_MEM_U16_PTR_TYPE pNicID)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: OHA_TOPOGetNicByInterface(NicId: 0x%X)", *pNicID);
    return(ret);
}
				...
				NicID == pHDBUser->Params.pDetail->Params.User.NICId)
			{
				switch (OHA_RQB_GET_OPCODE(pRQB))
				{
					case OHA_OPC_PRM_COMMIT:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_PRM_COMMIT) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_SET_ADDRESS:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_SET_ADDRESS) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_SET_EMERGENCY:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_SET_EMERGENCY) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_DCP_RESPONSE:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_DCP_RESPONSE) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_DCP_WRITE_ELEMENT:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_DCP_WRITE_ELEMENT) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_PRM_WRITE:
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Local Set (OHA_OPC_PRM_WRITE) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					case OHA_OPC_DCP_CONTROL:  /* AP01335332. Lock IP */
						OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetActive: Lock IP (OHA_OPC_DCP_CONTROL) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
						Found = LSA_TRUE;
					break;
					default:
						OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_IsAnyLocalSetActive: Invalid Opcode (0x%X)", OHA_RQB_GET_OPCODE(pRQB));
						OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
					break;
				}
			}
            ...
            switch (OHA_RQB_GET_OPCODE(pRQB))
            {
                case OHA_OPC_PRM_COMMIT:
                    if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
						NicID == pHDBUser->Params.pDetail->Params.User.NICId)
                    {
                        OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetMib2Active: Local Set (OHA_OPC_PRM_COMMIT) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
                        Found = LSA_TRUE;
                    }
                    break;
                case OHA_OPC_PRM_WRITE:
                    if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
						NicID == pHDBUser->Params.pDetail->Params.User.NICId)
                    {
                        OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetMib2Active: Local Set (OHA_OPC_PRM_WRITE) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
                        Found = LSA_TRUE;
                    }
                    break;
                case OHA_OPC_LLDP_WRITE_OPTION:
                    if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
						NicID == pHDBUser->Params.pDetail->Params.User.NICId)
                    {
                        OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalSetMib2Active: Local Set (OHA_OPC_LLDP_WRITE_OPTION) is active by the user (0x%X) NicID (0x%X)", (i+1), NicID);
                        Found = LSA_TRUE;
                    }
                    break;
                default:
                    OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_IsAnyLocalSetMib2Active: Invalid Opcode (0x%X)", OHA_RQB_GET_OPCODE(pRQB));
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                    break;
            }
        }
...
/*+-------------------------------------------------------------------------+*/
/*+  Description:                                                           +*/
/*+  Control TimeParams (SOCK_OPC_CNTRL_TIMEPARAMS) can be requested by     +*/
/*+  the service (OHA_OPC_TCIP_CONTROL) or record (TCPKeepAliveRecord).     +*/
/*+  - LSA_FALSE: no local control (TimeParams) is active by any user       +*/
/*+  - LSA_TRUE: local control is active by any user (global or if-spec.)   +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
	...
	LSA_HANDLE_TYPE UserHandle)
{
	...
	OHA_UPPER_RQB_PTR_TYPE   pRQB  = pHDBSock->TcIpUserReq.pBottom;

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_IsAnyLocalCtrlTcIpActive()");
    ...
    if (OHA_IS_NOT_NULL(pRQB))
    {
        switch (OHA_RQB_GET_OPCODE(pRQB))
        {
            ...
            case OHA_OPC_PRM_WRITE:
				...
				{
					OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalCtrlTcIpActive: Local Set (OHA_OPC_PRM_READ/WRITE) is active by the user, Handle (0x%X)", UserHandle);
					Found = LSA_TRUE;
				}
				else
				{
					OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "OHA_IsAnyLocalCtrlTcIpActive: Invalid RecordIndex queued (0x%X)", pRQB->args.PrmData.record_index);
					OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
				}
                break;
            case OHA_OPC_TCIP_CONTROL:
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"OHA_IsAnyLocalCtrlTcIpActive: Local Set (OHA_OPC_READ/WRITE_TCIP_DATA) is active by the user, Handle (0x%X)", UserHandle);
                    Found = LSA_TRUE;
                }
                break;
            default:
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_IsAnyLocalCtrlTcIpActive: Invalid Opcode queued (0x%X)", OHA_RQB_GET_OPCODE(pRQB));
                OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                break;
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_IsAnyLocalCtrlClusterIpActive           +*/
/*+  Input/Output          :    LSA_UINT16      NicID                       +*/
/*+  Result                :    LSA_BOOL                                    +*/
/*+-------------------------------------------------------------------------+*/
/*+                                                                         +*/
...
/*+-------------------------------------------------------------------------+*/
/*+  Description:                                                           +*/
/*+  Write or Read ClusterIPv4SuiteRecord or ClusterIPControlRecord         +*/
/*+  - LSA_FALSE: no user request is active by any user                     +*/
/*+  - LSA_TRUE: local control is active by any user (if-specific)          +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_IsAnyLocalCtrlClusterIpActive(
	LSA_HANDLE_TYPE UserHandle)
{
	LSA_BOOL              Found = LSA_FALSE;
	OHA_HDB_SOCK_PTR_TYPE pHDBSock = OHA_GET_HSOCK_PTR();
	OHA_UPPER_RQB_PTR_TYPE   pRQB = pHDBSock->ClusterRQBs.pBottom;
	...
	if (OHA_IS_NOT_NULL(pRQB))
	{
		...
		case OHA_OPC_PRM_READ:
		case OHA_OPC_PRM_WRITE:
			if (pRQB->args.PrmData.record_index == OHA_RECORD_INDEX_CLUSTER_IP_V4_SUITE)
			{
				OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "OHA_IsAnyLocalCtrlClusterIpActive: Local Set or Get (OHA_OPC_PRM_READ/WRITE) is active by the user, Handle (0x%X)", UserHandle);
				Found = LSA_TRUE;
			}
			else
			{
				OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "OHA_IsAnyLocalCtrlClusterIpActive: Invalid RecordIndex queued (0x%X)", pRQB->args.PrmData.record_index);
				OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
			}
			break;
		default:
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "OHA_IsAnyLocalCtrlClusterIpActive: Invalid Opcode queued (0x%X)", OHA_RQB_GET_OPCODE(pRQB));
			OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
			break;
		}
	}
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
		"OUT: OHA_IsAnyLocalCtrlClusterIpActive(Found: 0x%X)", Found);
	return(Found);
}
    ...
    for (i=0; i < OHA_CFG_MAX_CHANNELS; i++, pHDBUser++)
    {
        if (OHA_IS_NOT_NULL(pHDBUser->SockUserReq.pBottom))
        {
			...
			if(pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
				NicID == pHDBUser->Params.pDetail->Params.User.NICId)
			{
				if(OHA_IS_NOT_NULL(pHDBUser->SockUserReq.pBottom))  /* 5085999 */
				{
					...
					switch (OHA_RQB_GET_OPCODE(pRQB))
					{
						...
						break;
						default:
							OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_DoLocalSetAddress: Invalid Opcode (0x%X)", OHA_RQB_GET_OPCODE(pRQB));
							OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
						break;
					}
				}
...
The following syntax is reserved by PNIO:
 i) remote SET (name_interpertation = OHA_NAME_INTERPRETATION_NO):  default (norm convention), no further interpretation
ii) local  SET (name_interpertation = OHA_NAME_INTERPRETATION_YES): station-name is valid if
    the last (rightmost) label match with "--ab-cd-ef-gh-ij-kl"  with a ...l consisting of [a-f0-9-]
//----------------------------------------------------------------------------*/
LSA_UINT8 oha_check_station_name(
    LSA_UINT16 length,
    OHA_COMMON_MEM_U8_PTR_TYPE name,
    LSA_UINT8 allow_upper_case,
    LSA_UINT8 name_interpretation
)
{
    LSA_UINT16 idx;
    LSA_UINT16 lbl;
    ...
    OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_check_station_name(length: 0x%X, name: 0x%X, allow_upper_case:  0x%X, name_interpretation:  0x%X)",
    ...
    {
        OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(invalid length, 1...240): LSA_FALSE");
        return OHA_ERR_PARAM;
    }

    if ( allow_upper_case != OHA_ALLOW_UPPER_CASE_NO && allow_upper_case != OHA_ALLOW_UPPER_CASE_YES )
    {
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "allow_upper_case(%u) is out of range", allow_upper_case);
        return OHA_ERR_PARAM;
    }

    if (name_interpretation != OHA_NAME_INTERPRETATION_NO && name_interpretation != OHA_NAME_INTERPRETATION_YES )
    {
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "name_interpretation(%u) is out of range", name_interpretation);
        return OHA_ERR_PARAM;
    }
    ...
    for ( idx = 0; idx <= length; ++idx )
    {
        ...
        if ( ch == '.' )
        {
            ...
            if ( len < 1 || len > 63 )
            {
                OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(label has either 0 or more than 63 chars): LSA_FALSE");
                return OHA_ERR_PARAM; /* label has either 0 or more than 63 chars */
            }
            ...
            if ( name[idx - 1] == '-' )
            {
                OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(label ends with '-'): LSA_FALSE");
                return OHA_ERR_PARAM; /* label ends with '-' */
            }
            ...
            if ( len > 3 )
            {
                ...
                if ( nr_of_labels == 0 )
                {
                    if ( len == 8 || len == 14 )
					{
                        if (OHA_OK == oha_check_port_name(len, name) ? LSA_TRUE : LSA_FALSE) /* lint critical 661 removed */
                        {
                            /* the first (leftmost) label match with a port-name */
                            OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(the first (leftmost) label match with a port-name): LSA_FALSE");
                            return OHA_ERR_PARAM;
                        }
					}
                }
            ...
            if (name_interpretation == OHA_NAME_INTERPRETATION_YES)
            {
                ...
                if ((length > lbl) && ((length - lbl) == (2 + 17)))
                {
                    ...
                    {
                        if (OHA_OK == oha_check_mac_string((LSA_UINT16)(length - lbl - 2), &name[lbl+2], allow_upper_case ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO) ? LSA_TRUE : LSA_FALSE)
                        {
                            /* the last (rightmost) label match with a valid mac-string - o.k. */
                            OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(the last (rightmost) label match with a mac-string: LSA_TRUE");
                            return OHA_OK;
                        }
                    }
                }
        ...
        if ( allow_upper_case == OHA_ALLOW_UPPER_CASE_YES )
        {
            if ( ch >= 'A' && ch <= 'Z' )
            {
                continue;
        ...
        if ( idx == lbl )
        {
            OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(label does not start with letter or digit): LSA_FALSE");
            return OHA_ERR_PARAM; /* label does not start with letter or digit */
        }
        ...
        OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(illegal char): LSA_FALSE");
        return OHA_ERR_PARAM; /* illegal char */
    }

    if ( looks_like_ipaddr && nr_of_labels == 4 )
    {

        OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(looks like an ip-addr): LSA_FALSE");
        return OHA_ERR_PARAM; /* looks like an ip-addr */
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_station_name(): LSA_TRUE");
    return OHA_OK; /* ok */
}
...
// validate port-name
     ...
     *
//----------------------------------------------------------------------------*/
LSA_UINT8 oha_check_port_name(
    LSA_UINT16 length,
    OHA_COMMON_MEM_U8_PTR_TYPE name
	)
{
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_check_port_name(length: 0x%X, name: 0x%X)",
                          length, name);
    ...
    if (OHA_IS_NOT_NULL(name))
    {
             ...
             && name[7] >= '0' && name[7] <= '9' )
        {
            ...
            if (length == 8)
            {               /* the first (leftmost) label match with "port-xyz" with x, y, z = 0...9 */
                OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_port_name(the first (leftmost) label match with 'port-xyz' with x, y, z = 0...9): LSA_TRUE");
                return OHA_OK;
            }
            ...
            {
                /* the first (leftmost) label match with "port-xyz-abcde" with x, y, z, a, b, c, d, e = 0...9 */
                OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_port_name(the first (leftmost) label match with 'port-xyz-abcde' with x, y, z, a, b, c, d, e = 0...9): LSA_TRUE");
                return OHA_OK;
            }
        }
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_port_name(): LSA_FALSE");
    return OHA_ERR_PARAM; /* not ok */
}
...
// validate mac-string
     ...
     *
//----------------------------------------------------------------------------*/
LSA_UINT8 oha_check_mac_string(
    LSA_UINT16 length,
    OHA_COMMON_MEM_U8_PTR_TYPE name,
    LSA_UINT8 allow_upper_case)
{
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_check_mac_string(length: 0x%X, name: 0x%X, , allow_upper_case: 0x%X)",
    ...
    if (length == 17 && OHA_IS_NOT_NULL(name))
    {
        if (!allow_upper_case)
        {
            if ((name[0]  >= 'A' && name[0]  <= 'F') ||
                ...
                (name[16] >= 'A' && name[16] <= 'F'))
            {
                OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_mac_string (no upper case letters allowed): LSA_FALSE");
                return OHA_ERR_PARAM;
            }
        }
           ...
           )
        {
            ...
            OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_mac_string(the last (rightmost) label match with 'ab-cd-ef-gh-ij-kl' with a...l consists of [a-f,0-9]): LSA_TRUE");
            return OHA_OK;
        }

        OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_mac_string (not a mac string): LSA_FALSE");
        return OHA_ERR_PARAM; /* not ok */
    }

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_mac_string (no buffer or invalid length): LSA_FALSE");
    return OHA_ERR_PARAM; /* not ok */
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_FreeTopoPeers(OHA_DB_TOPO_PTR_TYPE pTopoDataBase)
{
    ...
    {
        /* free allocated TOPO-data entry (TIA 741027) */
        {
            OHA_UPPER_PEER_PTR_TYPE pPeer = &pTopoDataBase->PeerSender;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_FreeAllTopoDBs(OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    LSA_UINT16   i;
    ...
    for (i = 0; i <= OHA_CFG_MAX_PORTS; i++)  /* interface (port 0) and all ports */
	{
		...
		pHDBUser->TopoLastIndStati[i].LastSpecifier_Gateway_Inactive = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_AllocTopoPortInd(
    OHA_HDB_USER_PTR_TYPE pHDBUser,
    LSA_UINT16   PortId)
{
    ...
    OHA_DB_TOPO_PTR_TYPE pSrcTopoData)
{
    ...
    if (pSrcTopoData->NumberOfPeers)
    {
        /* copy TOPO-data entry (TIA 741027) */
    ...
    LSA_UINT16 PortId)
{
    ...
    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_CopyAdjustData(pHDBGsy: 0x%X, pHDBUser: 0x%X, PortId: 0x%X)", pHDBGsy, pHDBUser, PortId);
    ...
    OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CopyAdjustData()");
    return(RetVal);
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_AdjustAdminStatus(
    LSA_UINT16 NICId,
    LSA_UINT16 PortId,
    LSA_UINT32 PeerToPeerBoundary)
{
    ...
    {
        RetVal = OHA_ERR_PARAM;
        OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_AdjustAdminStatus(NICId: 0x%X, RetVal: 0x%X)", NICId, RetVal);
        return (RetVal);
    }
    ...
    switch (PeerToPeerBoundary & OHA_PTP_SEND_LLDP_MASK)
    {
        ...
        case OHA_PTP_SEND_LLDP_OFF:
            ...
            break;
        default:
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_AdjustAdminStatus: Invalid PeerToPeerBoundary (0x%X)", PeerToPeerBoundary);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
            break;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_UpdateFrameOffset(
    LSA_UINT16 NICId,
    LSA_UINT16 PortId,
    LSA_BOOL OnlySyncForwarding)
{
    ...
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL,"OHA_UpdateFrameOffset(NICId: 0x%X): Invalid PortId (%Xh)",NICId,PortId);
    ...
    if (pHDBGsy->SyncDataCommit.Mode == OHA_SYNC_ADD)
    {
        ...
        {
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OHA_UpdateFrameOffset (PortId: 0x%X): OnlySyncForwarding port, REDperiod = OFF (=OK)!", PortId);
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
					...
					(OHA_COMMON_MEM_PTR_TYPE)OHA_ZERO_UUID, sizeof(OHA_UUID_TYPE)))
	{
		...
		{
			OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OHA_CopySyncData(Mode: 0x%X, IfID: 0x%X): The PTCP_SubdomainUUID's are equal, therfore the SyncMaster-MAC Address is not deleted!",
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CopySyncData(RetVal: 0x%X)", RetVal);
    return(RetVal);
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_FreeTopoDB(OHA_DB_TOPO_PTR_TYPE pTopoDataBase)
{
    LSA_UINT16   FreeStat = LSA_RET_OK;    /* free the buffer for var. */
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_TopoInitPort(OHA_HDB_USER_PTR_TYPE pHDBUser, LSA_UINT16 PortId)
{
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_TopoInitPort(pHDBUser: 0x%X, PortId: 0x%X)", pHDBUser, PortId);

    OHA_ASSERT_NULL_PTR(pHDBUser);

    /* (re)init the Topo-DB for one port module */
    OHA_ASSERT(PortId <= OHA_CFG_MAX_PORTS);

    pHDBUser->TopoLastIndStati[PortId].LastSpecifierMau      = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierDelay    = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierLink     = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_Chassis = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_Port = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_MRP_domain = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_None = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_MAU  = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_CableDelay  = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_RT_Class3  = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_PTCP_status = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_PreambleLength = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierPeer_Fragmentation = OHA_ALARM_SPECIFIER_INITIAL_VALUE;

    pHDBUser->TopoLastIndStati[PortId].LastSpecifierInterface_Gateway = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierInterface_NoS     = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierInterface_IP      = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
    pHDBUser->TopoLastIndStati[PortId].LastSpecifierInterface_NoD     = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
	pHDBUser->TopoLastIndStati[PortId].LastSpecifier_Gateway_Inactive = OHA_ALARM_SPECIFIER_INITIAL_VALUE;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_InitTopo(OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    LSA_UINT16 i;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT8 OHA_Get_Peers_PdPortDataExpected(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    OHA_UPPER_MEM_U8_PTR_TYPE * ppNextPeers,
    OHA_COMMON_MEM_U16_PTR_TYPE Port_Len,
    OHA_COMMON_MEM_U16_PTR_TYPE Chassis_Len,
    OHA_UPPER_MEM_U8_PTR_TYPE * ppPort,
...
)
{
	...
	OHA_ASSERT_NULL_PTR(ppPort);
	OHA_ASSERT_NULL_PTR(Port_Len);
	OHA_ASSERT_NULL_PTR(Chassis_Len);
	OHA_ASSERT_NULL_PTR(ppNextPeers);
    ...
    *ppChassis = pRecord;
    ...
    RetVal = OHA_OK;  /* the peer record is ok */
    *Port_Len = PortLen;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT8 OHA_Get_Peers_PdPortDataReal(
    OHA_UPPER_MEM_U8_PTR_TYPE   pRecData,
    OHA_UPPER_MEM_U8_PTR_TYPE * ppNextPeers,
    OHA_COMMON_MEM_U16_PTR_TYPE Port_Len,
    OHA_COMMON_MEM_U16_PTR_TYPE Chassis_Len,
    ...
    LSA_UINT32                  *err_offset)
{
    ...
    *err_offset = 0;
        ...
        BlockType != OHA_TOPO_CHECK_PEERS)
    {
        ...
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): BlockHeader failed.", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
    ...
    index += 1;
    if (index>RecordLen) {*err_offset = index - 1; return RecLenPeerDataFailed(*err_offset);}
    ...
    if (NumberOfPeers)
    {
        ...
        {
            RetVal = OHA_ERR_PRM_DATA;
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): LengthPeerPortName failed.", NICId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
            *err_offset = index - 1;
            return(RetVal);
        }
        ...
        if (OHA_ERR_PARAM == oha_check_port_name((LSA_UINT16)PortLen, PortNamePtr) ? LSA_TRUE : LSA_FALSE)
        {
            RetVal = OHA_ERR_PRM_DATA;
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): PeerPortName failed.", NICId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
            *err_offset = index;
            return(RetVal);
        }
        ...
        pRecord += PortLen;                 /* PeerPortName        */
        index += PortLen;
        if (index>RecordLen) {*err_offset = index - PortLen; return RecLenPeerDataFailed(*err_offset);}
        ...
        {
            RetVal = OHA_ERR_PRM_DATA;
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): LengthPeerStationName failed.", NICId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
            *err_offset = index - 1;
            return(RetVal);
        }
        ...
        /* check, if it's a RFC-name */
        if (OHA_ERR_PARAM == oha_check_station_name((LSA_UINT16)ChassisLen, ChassisNamePtr,
        ...
        {
            RetVal = OHA_ERR_PRM_DATA;
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): PeerStationName failed.", NICId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
            *err_offset = index;
            return(RetVal);
        }
        ...
        pRecord += ChassisLen;              /* PeerStationName     */
        index += ChassisLen;
        if (index>RecordLen) {*err_offset = index - ChassisLen; return RecLenPeerDataFailed(*err_offset);}
    }
    ...
    while ( (index & 3) != 0 )
    {
        ...
        {
            RetVal = OHA_ERR_PRM_DATA;
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"OHA_CheckTopoPeers(NICId: 0x%X) - wrong Padding !", NICId);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
            *err_offset = index - 1;
            return(RetVal);
        }
    ...
    if (!(BlockLength == index - 2*sizeof(LSA_UINT16)))
    {
        ...
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CheckTopoPeers(NICId: 0x%X): Length of PeerData failed (err_offset: 0x%X).", NICId, index);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CheckTopoPeers(RetVal: 0x%X)", RetVal);
        *err_offset = index;
        return(RetVal);
    }
...
LSA_BOOL OHA_CheckMRP_PortStatus(LSA_UINT32 MRP_PortStatus)
{
    LSA_BOOL  RetVal;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetSpeedFromMauType(
    LSA_UINT16                  MauType)
{
    LSA_UINT16     Speed;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetModeFromMauType(
    LSA_UINT16                  MauType)
{
    LSA_UINT16     Mode;
    ...
    switch (MauType)    /* for PNIO */
    {
        ...
        case LLDP_TLV_MAU_TYPE_100BASET4:
            ...
            OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_GetModeFromMauType: duplex mode unknown.");
            break;

        default:        /* other types */
            Mode = EDD_LINK_UNKNOWN;    /* no valid MauType for PNIO */
            OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_GetModeFromMauType: MauType failed.");
            break;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetAdvertisedCapFromParams(LSA_UINT32 AutonegCapAdvertised)
{
...
/*+  Result                :    ---                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: get the LinkState from the EDD (Link)Status, PortStatus   +*/
/*+               and PhyStatus                                             +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetLinkStatus(LSA_UINT16 LinkStatus,
                             LSA_UINT16 PhyStatus,
                             LSA_UINT16 PortStatus)
{
    LSA_UINT16 LinkStateLink;

    OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_GetLinkStatus(Status: 0x%X, PhyStatus: 0x%X, PortStatus: 0x%X)", LinkStatus, PhyStatus, PortStatus);

    switch (PortStatus)
    {
        case EDD_PORT_PRESENT: /* continue */
            break;
        case EDD_PORT_NOT_PRESENT:
            return OHA_PORT_LINK_STATE_NOT_PRESENT;
        default:
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "OHA_GetLinkState: Invalid PortStatus (0x%X)", PortStatus);
            return OHA_PORT_LINK_STATE_UNKNOWN;
    }

    switch (PhyStatus)
    {
        case EDD_PHY_STATUS_ENABLED: /* continue */
            break;
        case EDD_PHY_STATUS_DISABLED:
            return OHA_PORT_LINK_STATE_NOT_PRESENT;
        case EDD_PHY_STATUS_UNKNOWN:
            return OHA_PORT_LINK_STATE_UNKNOWN;
        default:
            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "OHA_GetLinkState: Invalid PhyStatus (0x%X)", PhyStatus);
            return OHA_PORT_LINK_STATE_NOT_PRESENT;
    }
    ...
    switch (LinkStatus)
    {
        ...
        case EDD_LINK_UP_DISABLED:
            LinkStateLink = OHA_PORT_LINK_STATE_UP;
            break;
        case EDD_LINK_DOWN:
            LinkStateLink = OHA_PORT_LINK_STATE_DOWN;
            break;
        case EDD_LINK_UNKNOWN:
            LinkStateLink = OHA_PORT_LINK_STATE_UNKNOWN;
            break;
        default:
            LinkStateLink = OHA_PORT_LINK_STATE_UNKNOWN;
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_GetLinkStatus(PortState: 0x%X)", LinkStateLink);
    return(LinkStateLink);
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_GetPortStatus                           +*/
/*+  Input/Output          :    ---                                         +*/
/*+  Result                :    ---                                         +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: get the LinkState.Port from the EDD (Link)Status,         +*/
/*+               PhyStatus, PortStatus and PortState                       +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_EXTERN LSA_UINT16 OHA_GetPortStatus(
	LSA_UINT16              LinkStatus,
	LSA_UINT16              PhyStatus,
	LSA_UINT16              PortStatus,
	LSA_UINT16              PortState)
{
	LSA_UINT16 LinkStatePort;

	OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_CHAT, "IN : OHA_GetPortStatus(LinkStatus: 0x%X, PhyStatus: 0x%X, PortStatus: 0x%X, PortState: 0x%X)", 
							LinkStatus, PhyStatus, PortStatus, PortState);
	...
	switch (PhyStatus)
	{
		...
		case EDD_PHY_STATUS_DISABLED:
		case EDD_PHY_STATUS_UNKNOWN:
			return OHA_PORT_STATE_DISABLED;
		default:
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "OHA_GetPortStatus: Invalid PhyStatus (0x%X)", PhyStatus);
			return OHA_PORT_STATE_DISABLED;
	}
	...
	switch (PortStatus)
	{
		...
		case EDD_PORT_NOT_PRESENT:
			return OHA_PORT_STATE_DISABLED;
		default:
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "OHA_GetPortStatus: Invalid PortStatus (0x%X)", PortStatus);
			return OHA_PORT_STATE_DISABLED;
	}
	...
	{
		case EDD_LINK_UP:
		case EDD_LINK_UP_CLOSED: /* continue */
		case EDD_LINK_UP_DISABLED:
			break;
		case EDD_LINK_DOWN:
		case EDD_LINK_UNKNOWN:
			return OHA_PORT_STATE_UNKNOWN;
		default:
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "OHA_GetPortStatus: Unknown LinkStatus (0x%X)", LinkStatus);
			return OHA_PORT_STATE_UNKNOWN;
	}
	...
	switch (PortState)
	{
		case EDD_PORT_STATE_DISABLE:
			LinkStatePort = OHA_PORT_STATE_DISABLED;
			break;
		case EDD_PORT_STATE_BLOCKING:
			LinkStatePort = OHA_PORT_STATE_BLOCKING;
			break;
		case EDD_PORT_STATE_LISTENING:
			LinkStatePort = OHA_PORT_STATE_LISTENING;
			break;
		case EDD_PORT_STATE_LEARNING:
			LinkStatePort = OHA_PORT_STATE_LEARNING;
			break;
		case EDD_PORT_STATE_FORWARDING:
		case EDD_PORT_STATE_FORWARDING_WITHOUT_LEARNING:
			LinkStatePort = OHA_PORT_STATE_FORWARDING;
			break;
		case EDD_PORT_STATE_UNCHANGED: /* not appearing at EDD_SRV_LINK_STATUS_IND_PROVIDE_EXT */
		default:
			LinkStatePort = OHA_PORT_STATE_UNKNOWN;
			OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "OHA_GetPortStatus: Invalid PortState (0x%X)", PortState);
			OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
			break;
	}

	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_GetPortStatus(PortState: 0x%X)", LinkStatePort);
	return(LinkStatePort);
}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetLinkStatusChange(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_CopyTopoPeersToDB(
    ...
    OHA_UPPER_MEM_U8_PTR_TYPE   pPeerData)
{
    ...
    index += 1;
    if (index>RecordLen) {return RecLenTopoPeersFailed(index - 1);}
    OHA_PDU_GET_U8 (NumberOfPeers, pRecord);    /* NumberOfPeers       */

    /* copy TOPO-peer entry (TIA 741027) */
    if (NumberOfPeers)
    {
        LSA_UINT8 PortLen, ChassisLen;
        OHA_COMMON_MEM_U8_PTR_TYPE PortNamePtr;
        ...
        pRecord += PortLen;                 /* PeerPortName        */
        index += PortLen;
        if (index>RecordLen) {return RecLenTopoPeersFailed(index - PortLen);}
        ...
        pRecord += ChassisLen;              /* PeerStationName       */
        index += ChassisLen;
        if (index>RecordLen) {return RecLenTopoPeersFailed(index - ChassisLen);}
    ...
    {
        RetVal = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_CopyTopoPeersToDB: Length of PeerData failed (err_offset: 0x%X).", index);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CopyTopoPeersToDB(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
...
LSA_UINT16 oha_db_write_TopoPeer(
    OHA_COMMON_MEM_U8_PTR_TYPE * ppElement,
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetCountOfRealPeers(
    LSA_UINT16              NICId,
    LSA_UINT16              PortId)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_GetCountOfRealPeers(RealPeers: 0x%X)", RealPeers);
    return(RealPeers);
}
...
/*                                                                           */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetPortDelays(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
...
/*+  Input/Output      OHA_UPPER_MEM_PTR_TYPE     *ppTLV                    +*/
/*+                    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength                 +*/
/*+                    LLDP_TLV_SET_INFO_TYPE     *pTLVInfo                 +*/
/*+                                                                         +*/
/*+  Result            LSA_UINT16                                           +*/
/*+-------------------------------------------------------------------------+*/
/*+  Input/Output:                                                          +*/
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetChassisPortTTL(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
    LLDP_TLV_SET_INFO_TYPE     *pTLVInfo,
...
/*                                                                           */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetPortStatus(
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength,
...
/*                                                                           */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetPortStatusMrp(
    OHA_UPPER_MEM_PTR_TYPE         *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE      pTLength,
    OHA_UPPER_MEM_U8_PTR_TYPE       pDomainUUID,
...
/*  +----------------+ +------------+  +---------------+  +--------------+   */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetPtcpStatus(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
...
)
{
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(ppTLV,LSA_NULL)    ||
         LSA_HOST_PTR_ARE_EQUAL(*ppTLV,LSA_NULL)   ||
         LSA_HOST_PTR_ARE_EQUAL(pTLength,LSA_NULL) ||
         LSA_HOST_PTR_ARE_EQUAL(pMasterSourceAddress,LSA_NULL) ||
         LSA_HOST_PTR_ARE_EQUAL(pSubdomainUUID,LSA_NULL) ||
         LSA_HOST_PTR_ARE_EQUAL(pIRDataUUID,LSA_NULL) ||
         LSA_HOST_PTR_ARE_EQUAL(pFrameOffset,LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;
    }
    else
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetUnknownOptionals(
    LSA_UINT16                   NicId,
    LSA_UINT16                   PortId,
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength)
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetPNOSpecifics(
    LSA_UINT16                   NicId,
    LSA_UINT16                   PortId,
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength)
{
    ...
    {
        Response = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: OhaLldpSetPNOSpecifics(Response: 0x%X)", Response);
        return(Response);
    }
    ...
    if (TxEnable == OHA_LLDP_TX_ENABLE &&
		OHA_EDD_DELAY_SUPPORTED(pDbIf->HardwareType))
    {
        ...
        if ((Length == OHA_LLDP_PNO_DELAY_SIZE) && (!LSA_HOST_PTR_ARE_EQUAL(pLocDelays, LSA_NULL)))
        {
            ...
            if (RetVal != OHA_OK)
            {
                OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE,
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetOrgDefOthers(
    LSA_UINT16                     NicId,
    LSA_UINT16                     PortId,
    OHA_UPPER_MEM_PTR_TYPE        *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE     pTLength)
{
    ...
    {
        Response = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: OhaLldpSetOrgDefOthers(Response: 0x%X)", Response);
        return(Response);
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetIEEE802_1Extensions(
    LSA_UINT16                   NicId,
    LSA_UINT16                   PortId,
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength)
{
    ...
    {
        Response = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: OhaLldpSetIEEE802_1Extensions(Response: 0x%X)", Response);
        return(Response);
    }
	...
	if(TxEnable == OHA_LLDP_TX_ENABLE)
	{
			...
			(Len == LLDP_TLV_IEEE_802_1_PORT_VLAN_IDENTIFIER_SIZE))
		{
			...
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetPortVLANID: Invalid Response (0x%X)", RetVal);
			}
		}
	}
	...
	if(TxEnable == OHA_LLDP_TX_ENABLE)
	{
			...
			(Len == LLDP_TLV_IEEE_802_1_PPVID_FLAGS_SIZE + LLDP_TLV_IEEE_802_1_PPVID_REFERENCE_NUMBER_SIZE ))
		{
			...
			if (RetVal != LLDP_RSP_OK)
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetPPVID: Invalid Response (0x%X)", RetVal);
			}
		}
	}
	...
	if(TxEnable == OHA_LLDP_TX_ENABLE)
	{
			 ...
			 LLDP_TLV_IEEE_802_1_PORT_VLAN_NAME_MAX_VLAN_NAME_LEN ))
		{
				...
				LLDP_TLV_IEEE_802_1_PORT_VLAN_NAME_VLAN_NAME_LEN_SIZE)
			{
				...
				if (RetVal != LLDP_RSP_OK)
				{
					OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetVLANName: Invalid Response (0x%X)", RetVal);
				}
			}
			else
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"VLANNameLength (0x%X) does not match with size of element", TLVParams.VLANNameLength);
			}
		}
	}
	...
	if(TxEnable == OHA_LLDP_TX_ENABLE)
	{
			 ...
			 LLDP_TLV_IEEE_802_1_PROTOCOL_IDENTITY_MAX_LEN ))
		{
			...
			if (Len == TLVParams.Length + LLDP_TLV_IEEE_802_1_PROTOCOL_IDENTITY_LENGTH_SIZE)
			{
				...
				if (RetVal != LLDP_RSP_OK)
				{
					OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetProtIdentity: Invalid Response (0x%X)", RetVal);
				}
			}
			else
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"ProtocolIdentity Length (0x%X) does not match with size of element", TLVParams.Length);
			}
		}
	}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetIEEE802_3Extensions(
    LSA_UINT16                   NicId,
    LSA_UINT16                   PortId,
    OHA_UPPER_MEM_PTR_TYPE      *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE   pTLength)
{
    ...
    {
        Response = LSA_RET_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: OhaLldpSetIEEE802_3Extensions(Response: 0x%X)", Response);
        return(Response);
    }
	...
	if(TxEnable & OHA_DOT3_MAC_PHY_CONFIG_STATUS_TX)
	{
					 ...
					 LLDP_TLV_IEEE_802_3_MAC_PHY_CFG_STATUS_MAUT_TYPE_SIZE)) )
		{
			...
			if (RetVal != LLDP_RSP_OK)
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetMACPHYConfigStatus: Invalid Response (0x%X)", RetVal);
			}
		}
	}
	...
	if(TxEnable & OHA_DOT3_POWER_VIA_MDI_TX)
	{
			 ...
			 LLDP_TLV_IEEE_802_3_POWER_VIA_MDI_POWER_CLASS_SIZE))
		{
			...
			if (RetVal != LLDP_RSP_OK)
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetPowerViaMDI: Invalid Response (0x%X)", RetVal);
			}
		}
	}
	...
	if(TxEnable & OHA_DOT3_LINK_AGGREGATION_TX)
	{
			 ...
			 LLDP_TLV_IEEE_802_3_LINK_AGGREGATION_PORT_ID_SIZE ))
		{
			...
			if (RetVal != LLDP_RSP_OK)
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetLinkAgg: Invalid Response (0x%X)", RetVal);
			}
		}
	}
	...
	if(TxEnable & OHA_DOT3_MAX_FRAME_SIZE_TX)
	{
			...
			(Len == LLDP_TLV_IEEE_802_3_MAX_FRAME_SIZE_FRAME_SIZE_SIZE) )
		{
			...
			if (RetVal != LLDP_RSP_OK)
			{
				OHA_PROGRAM_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"lldp_TLVSetMaxFrameSize: Invalid Response (0x%X)", RetVal);
			}
		}
	}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT16 OHA_LLDP_TLVValidateMgmAddrLen(
                                              ...
                                              LSA_UINT16   SenderId)
{
    ...
    if (LSA_HOST_PTR_ARE_EQUAL(pHDBLldp, LSA_NULL) ||
        LSA_HOST_PTR_ARE_EQUAL(pHDBLldp->pDB, LSA_NULL))
    {
        OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_TopoGetPeerMAC - no LLDP existing");
        return LSA_NULL;
    }
    ...
    LSA_UINT16     RRT_State)
{
    ...
    OHA_UPPER_MEM_PTR_TYPE pMRP_PortStatusTlv = MRP_PortStatusTlv;
    ...
    if (RetVal != LLDP_RSP_OK)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"lldp_TLVSetMACPHYConfigStatus: Invalid Response (0x%X)", RetVal);
    ...
    if (RetVal != LSA_RET_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OHA_DB_WRITE_LLDP_ELEMENT(LLDP_PNIO_MRPPORTSTATUS) failed (NICId: 0x%X, RetVal: 0x%X).", NICId, RetVal);
    }
    ...
    OHA_FRAME_OFFSET_PTR_TYPE pFrameOffset)
{
    ...
    OHA_UPPER_MEM_PTR_TYPE pPTCP_StatusTlv = PTCP_StatusTlv;
    ...
    if (RetVal != LSA_RET_OK)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OhaLldpSetPtcpStatus: Invalid Response (0x%X)", RetVal);
        OHA_FATAL(OHA_FATAL_ERR_LLDP);
    }
    ...
    if (RetVal != LSA_RET_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"oha_write_ptcp_status: OHA_DB_WRITE_LLDP_ELEMENT(LLDP_PNIO_PTCPSTATUS) failed (NICId: 0x%X, RetVal: 0x%X).", NICId, RetVal);
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByNIC(
    LSA_UINT16 NICId)
{
...
/*+               LSA_NULL is returned                                      +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_REMA_PTR_TYPE OHA_REMAGetHandleByNIC(
    LSA_UINT16 NICId)
{
    OHA_DB_IF_PTR_TYPE    pDB  = &oha_database.IfData[0];
...
/*+               LSA_NULL is returned                                      +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_GSY_PTR_TYPE OHA_GSYGetHandleByNIC(
    LSA_UINT16 NICId)
{
    OHA_HDB_EDD_PTR_TYPE  pHDBEdd  = OHA_GET_HEDD_PTR(0);
...
/*                                                                           */
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OhaLldpSetChassisMacAdr(
    OHA_UPPER_MEM_PTR_TYPE     *ppTLV,
    OHA_UPPER_MEM_U32_PTR_TYPE  pTLength,
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DbStoreLocalTTL(
    LSA_UINT16 NICId,
...
)
{
    ...
    if ( OHA_IS_NULL(pHDBLldp) || OHA_IS_NULL(pHDBLldp->pDB))
    {
        ...
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Request OHA_DbStoreLocalTTL failed. No LLDP-Channel or no database present (NIC-ID: 0x%X)", NICId);
    }
    else
    {
        ...
        {
            OHA_PROGRAM_TRACE_02(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"TxInterval(0x%X) or TxHoldMultiplier(0x%X) out of range!", TxInterval, TxHoldMultiplier);
            Response = OHA_ERR_PARAM;
        }
        else
        {
            LSA_UINT32 TimeToLive = (LSA_UINT32)TxInterval * (LSA_UINT32)TxHoldMultiplier;
            if (TimeToLive > 65535) { /* TFS 2975949, TTL = min(65535, (TxInterval * TxHoldMultiplier)) */
                TimeToLive = 65535;
            }

            OHA_PUT16_HTON(pLldpTTL, 0, (LSA_UINT16)TimeToLive);
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_DbStoreLocalTTL: OHA_DB_WRITE_LLDP_ELEMENT(TTL) failed (Status: 0x%X).", Status);
                Response = OHA_ERR_PARAM;
            }
        }
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DbStoreMgmAddress(
    LSA_UINT16 NICId,
...
)
{
    ...
    if (OHA_IS_NULL(pDB))
    {
        Response = OHA_ERR_RESOURCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Request OHA_DbStoreMgmAddress failed. No LLDP-Channel or no database present (NIC-ID: 0x%X)", NICId);
    }
    else if (OHA_IS_NULL(pIpaddr))  /* update Management Address (only InterfaceNumber) */
    {
        ...
        else
        {
            Response = OHA_ERR_RESOURCE;
            /* Error on getting handle. Abort. */
            OHA_PROGRAM_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request OHA_DbStoreMgmAddress failed. No LLDP-Channel or no database present (NIC-ID: 0x%X)", NICId);
        }
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetSpeedFromSpeedMode(
    LSA_UINT8                   SpeedMode)
{
    LSA_UINT16     Speed;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetModeFromSpeedMode(
    LSA_UINT8                   SpeedMode)
{
    LSA_UINT16     Mode;
    ...
    OHA_COMMON_MEM_U16_PTR_TYPE pPtcpDelay)
{
    ...
    *pPtcpDelay = 0;
    ...
    if (pHDBGsy->SyncDataCommit.Mode == OHA_SYNC_ADD)
    {
        ...
        {
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CalculatePtcpTimeDelay(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_AdminStatusToSendMode(
    LSA_UINT32                  AdminStatus)
{
    ...
    switch (AdminStatus)    /* for PNIO */
    {
        ...
        case OHA_LLDP_RECV_ONLY:          /* enables only receive of lldp-data, rxOnly(2) */
            SendMode = OHA_LLDP_TX_ENABLE;  /* enables the sending of lldp-data  */
            break;
        case OHA_LLDP_DISABLED:           /* disables lldp, disabled(4) */
            SendMode = OHA_LLDP_TX_DISABLE;  /* disables the sending of lldp-data */
            break;
        default:        /* other types */
...
/*------------------------------------------------------------------------------
  validate ip-address, subnet-mask and gateway-ip
    ...
    *
  ----------------------------------------------------------------------------*/
LSA_UINT8 oha_check_ip_address (
	...
	LSA_UINT8 ip_interpretation /* OHA_IP_INTERPRETATION_YES ... ip_address 0.0.0.0 is an error too */
)
{

	OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
		"IN : oha_check_ip_address(ip_addr: 0x%X, subnet_mask: 0x%X, gateway: 0x%X)",
		ip_addr, subnet_mask, gateway);

	if (ip_interpretation != OHA_IP_INTERPRETATION_NO && ip_interpretation != OHA_IP_INTERPRETATION_YES)
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "ip_interpretation(%u) is out of range", ip_interpretation);
		return OHA_ERR_PARAM;
	}
	...
	if (ip_addr == 0) /* delete ip-suite */
	{
		...
		{
			OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address(): ip-addr 0.0.0.0 but subnet-mask or gateway not 0.0.0.0!");
			return OHA_ERR_PARAM;
		}

		if (ip_interpretation == OHA_IP_INTERPRETATION_YES)
		{
			OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address(): ip-address 0.0.0.0 not allowed!");
			return OHA_ERR_PARAM;
		}
	}
	else
	{
		...
		// - the subnet address: the host part of the address is “0”
		// - the directed broadcast address: the host part of the address is a series of consecutive binary “1”
		// - the limited broadcast address (i.e. 255.255.255.255)
		// - the subnet part of the address is “0”
		// - an address in the address range from 0.0.0.0 up to 0.255.255.255 (highest 8 bit of the address are 400 set to “0000 0000”)
		// - a loopback address: an address in the address range from 127.0.0.0 up to 127.255.255.255 (highest 8 bit of the address are set to “0111 1111”)
		// - a multicast address: an address in the address range from 224.0.0.0 up to 239.255.255.255 (highest 4 bit of the address are “1110”)
		// - a reserved address: an address in the address range from 240.0.0.0 up to 255.255.255.255 (highest 4 bit of the address are “1111”)
		//
		// Subnet mask
		// - consecutive binary “1”, followed by a series of consecutive binary “0”
		// - not   0.0.0.0 (/0)
		// - not 128.0.0.0 (/1)
		// - not 192.0.0.0 (/2)
		// - not 224.0.0.0 (/3)
		...
		*/
		...
		switch (subnet_mask)
		{
			...
			case 0xFFFFFFFF: /* /32 ... invalid because of SPH IP Configuration */
				OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address (0x%X): not a valid subnet_mask!", subnet_mask);
				return OHA_ERR_PARAM;
		...
		{
			OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address (ip: 0x%X, snm: 0x%X): is not valid!", ip_addr, subnet_mask);
			return OHA_ERR_PARAM;
		}
		}
		...
		else
		{
			...
			{
				OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address (gateway: 0x%X): not valid!", gateway);
				return OHA_ERR_PARAM;
			}

#if 0
			/* former OHA_CFG_NO_GATEWAY_CHECK *//* AP01314551 */
			if ((gateway & subnet_mask) != (ip_addr & subnet_mask))
			{
				OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address (ip: 0x%X, gateway: 0x%X, subnet: 0x%X): not in the same subnet!", ip_addr, gateway, subnet_mask);
				return OHA_ERR_PARAM;
			}
#endif
			if ((gateway & subnet_mask) == (ip_addr & subnet_mask)) /* in same subnet */
			{
				...
				if (gateway == host0_addr
					|| gateway == host1_addr)
				{
					OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_NOTE, "OUT: oha_check_ip_address (gateway: 0x%X, host0: 0x%X, host1: 0x%X): ip_addr not valid!", gateway, host0_addr, host1_addr);
					return OHA_ERR_PARAM;
				}
			}
		}
	...
	OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_check_ip_address(): LSA_TRUE");
	return OHA_OK;
}


/*------------------------------------------------------------------------------
* check if this router is to set (checking rules see OHA docu)
*
* LSA_FALSE: this router is not to set
* LSA_TRUE:  this router is to set
*
----------------------------------------------------------------------------*/
LSA_BOOL oha_is_this_router_to_set(
	LSA_UINT16 NICId,
	LSA_UINT32 gateway          /* ip-address of the gateway in host byte order */
)
{
	...
	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
		"IN : oha_is_this_router_to_set(NICId:  0x%X, gateway: 0x%X)",
		NICId, gateway);

	Count = OHA_CountGateways(NICId, 1, LSA_NULL); /* count existing routers, but not at this inferface */
	if (!Count) /* no gateway set at another interface (only this existing) */
	{
		OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_this_router_to_set(True), no gateway");
		return(LSA_TRUE); /* gateway (intial) is to set or to remove */
	}

	Count = OHA_CountGateways(NICId, 2, LSA_NULL); /* count existing routers at inferfaces, only at former initalized to this */
	if (gateway) /* gateway is to set */
	{
		/* no gateways of former initalized interfaces like this */
		if (!Count)
		{
			OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_this_router_to_set(True), no former gateway");
			return(LSA_TRUE);
		}
		else
		{
			OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_this_router_to_set(True), former gateway");
			return(LSA_FALSE); /* do not set, the former gateway is to use */
		}
	}

	OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_is_this_router_to_set(False)");
	return LSA_FALSE;
}

/*------------------------------------------------------------------------------
* Count gateways of interfaces
*
* What = 0: count all existing routers at inferfaces
* What = 1: count existing routers at inferfaces, but not at this inferface
* What = 2: count existing routers at inferfaces, only at former initalized to this
*           (the working standard gateway is typically at the first registered interface)
*
* used for first set or delete of a global router
* Gateway returns the first found one, otherwise 0
----------------------------------------------------------------------------*/
LSA_UINT16 OHA_CountGateways(LSA_UINT16 NICId, LSA_UINT16 What,
							 OHA_COMMON_MEM_U32_PTR_TYPE pGateway)
{
	OHA_DB_IF_PTR_TYPE    pDB = &oha_database.IfData[0];
	LSA_UINT32            i = 0;
	LSA_UINT16            Count = 0;

	if (OHA_IS_NOT_NULL(pGateway))
	{
		*pGateway = 0;
	}

	for (; i < OHA_CFG_MAX_NICS; pDB++, i++)
	{
		if (pDB->Existing)   /* active interface */
		{
			OHA_HDB_EDD_PTR_TYPE pHDBEdd = OHA_EDDGetHandleByNIC(pDB->NicId);
			...
			if (OHA_IS_NOT_NULL(pHDBEdd))
			{
				...
				(LSA_VOID)OHA_DB_READ_IP_SUITE(pHDBEdd->Params.pDetail->Params.Edd.NICId, &IpAddrPtr, LSA_NULL, &GatewayPtr);

				if (pDB->NicId == NICId)
				{
					if (What == 1) /* do not count this interface */
					{
						continue;
					}
					else if (What == 2) /* stop counting former initalized interface at this interface */
					{
						return(Count);
					}
				}
				...
				{
					if(!OHA_MEMCMP(GatewayPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE) &&
						!OHA_MEMCMP(GatewayPtr, IpAddrPtr, OHA_IP_ADDR_SIZE)) /* no gateway */
					{
						Count++;
						...
						if (OHA_IS_NOT_NULL(pGateway))
						{
							if (*pGateway == 0)
							{
								*pGateway = OHA_GET32_NTOH(GatewayPtr, 0);
							}
						}
					}
				}
...
/*+  Description: checks and writes TimeToLive to the local LLDP DataBase   +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DbGetTxInterval(
    LSA_UINT16                  NICId,
...
)
{
    ...
    *pTxInterval = 0;
    ...
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"OHA_DbGetTxInterval: oha_db_read_loc_lldp_element(TTL) failed (NICId: 0x%X).", NICId);
        Response = OHA_ERR_RESOURCE;
    }
...
)
{
	...
	OHA_ASSERT_NULL_PTR(IpAddrPtrPtr);
	OHA_ASSERT_NULL_PTR(IpMaskPtrPtr);
	OHA_ASSERT_NULL_PTR(GatewayPtrPtr);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+  Functionname          :    OHA_IsIpLocalPending                        +*/
/*+  Input                 :    LSA_UINT16       NICId                      +*/
/*+                             OHA_COMMON_MEM_U8_PTR_TYPE IpAddrPtr        +*/
/*+  Result                :    -                                           +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Returns False if LocalPendingIP is 0.0.0.0 or a given IP  +*/
/*                (IpAddrPtr) is not in the database (LocalPendingIP),      +*/
/*+---------------otherwise returns True  ----------------------------------+*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_IsIpLocalPending(
	LSA_UINT16                     NicId,
	OHA_COMMON_MEM_U8_PTR_TYPE     IpAddrPtr
)
{
	...
	/* IP 0.0.0.0 has no local conflict */
	if (OHA_MEMCMP((OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, &pDB->LocalPendingIP, OHA_IP_ADDR_SIZE))
	{
		return (LSA_FALSE); /* not pending */
	}
	else if(OHA_IS_NULL(IpAddrPtr)) /* No IP given to compare */
	{
		IsPending = LSA_TRUE; /* any IP is pending */
	}
	else /* IP given to compare */
	{
		IsPending = OHA_MEMCMP((OHA_COMMON_MEM_U8_PTR_TYPE)&pDB->LocalPendingIP, IpAddrPtr, OHA_IP_ADDR_SIZE);
	}

	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_LOW, "OHA_IsIpLocalPending(NicId: 0x%X, IsPending: 0x%X)", NicId, IsPending);
	return (IsPending);
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+  Functionname          :    OhaSetLocalPendingIp                        +*/
/*+  Input                 :    LSA_UINT16                     NICId        +*/
/*+                             SOCK_UPPER_IP_SUITE_PTR_TYPE   PendingIp    +*/
/*+  Result                :    -                                           +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Writes a local conflicting IP to the OHA database         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OhaSetLocalPendingIp(
	LSA_UINT16                      NicId,
	SOCK_UPPER_IP_SUITE_PTR_TYPE    PendingIp
)
{
	OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
	OHA_COMMON_MEM_U8_PTR_TYPE pIpSuite;

	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
		"IN : OhaSetLocalPendingIp(NicId: 0x%X, PendingIp: 0x%X)",
		NicId, PendingIp);

	OHA_ASSERT_NULL_PTR(pDB);
	OHA_ASSERT_NULL_PTR(PendingIp);

	pIpSuite = (OHA_COMMON_MEM_U8_PTR_TYPE)&pDB->LocalPendingIP; /* write pending IP to database */
	*pIpSuite       = PendingIp->IpAddr.S_un.S_un_b.s_b1;
	*(pIpSuite + 1) = PendingIp->IpAddr.S_un.S_un_b.s_b2;
	*(pIpSuite + 2) = PendingIp->IpAddr.S_un.S_un_b.s_b3;
	*(pIpSuite + 3) = PendingIp->IpAddr.S_un.S_un_b.s_b4;
	OHA_PROGRAM_TRACE_05(0, LSA_TRACE_LEVEL_NOTE_LOW, "OhaSetLocalPendingIp: NicID: (0x%X), Ip: (%u.%u.%u.%u)",
						 NicId, *pIpSuite, *(pIpSuite + 1), *(pIpSuite + 2), *(pIpSuite + 3));

	pIpSuite += OHA_IP_ADDR_SIZE;
	*pIpSuite       = PendingIp->NetMask.S_un.S_un_b.s_b1;
	*(pIpSuite + 1) = PendingIp->NetMask.S_un.S_un_b.s_b2;
	*(pIpSuite + 2) = PendingIp->NetMask.S_un.S_un_b.s_b3;
	*(pIpSuite + 3) = PendingIp->NetMask.S_un.S_un_b.s_b4;

	return;
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+  Functionname          :    OhaResetLocalPendingIp                      +*/
/*+  Input                 :    LSA_UINT16       NICId                      +*/
/*+  Result                :    -                                           +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: Set a local conflicting IP in the OHA database to 0.0.0.0 +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OhaResetLocalPendingIp(
	LSA_UINT16                      NicId
)
{
	OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(NicId);
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
		"IN : OhaResetLocalPendingIp(NicId: 0x%X)", NicId);

	OHA_ASSERT_NULL_PTR(pDB);

	pIpSuite = (OHA_COMMON_MEM_U8_PTR_TYPE)&pDB->LocalPendingIP; /* delete pending IP */
	*pIpSuite = 0;
	*(pIpSuite + 1) = 0;
	*(pIpSuite + 2) = 0;
	*(pIpSuite + 3) = 0;
	...
	*(pIpSuite + 3) = 0;
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW, "OUT: OhaResetLocalPendingIp(NicId: 0x%X)", NicId);
	return;
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_IsIpOnAnyOtherIfLocalPending            +*/
/*+-------------------------------------------------------------------------+*/
/*+  Input/Output:                                                          +*/
/*+                                                                         +*/
/*+           NICId          Interface (this)                               +*/
/*+           pIpAddr:       Pointer to IP                                  +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: compare the interfaces IpAddresses for pending            +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_IsIpOnAnyOtherIfLocalPending(
	LSA_UINT16                      NICId,
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr)
{
	LSA_UINT16  CountOfInterfaces = OHA_GetCountOfInterfaces();
	LSA_BOOL    IpMatch = LSA_FALSE; /* not existing */

	OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
		"IN : OHA_IsIpOnAnyOtherIfLocalPending(NICId: 0x%X, pIpAddr: 0x%X, CountOfIfs: 0x%X)", NICId, pIpAddr, CountOfInterfaces);

	OHA_ASSERT_NULL_PTR(pIpAddr);

	if (OHA_GET32_NTOH(pIpAddr, 0) == 0) /* IP is 0.0.0.0 */
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsIpOnAnyOtherIfLocalPending - No IP(IpMatch: 0x%X)", IpMatch);
		return(IpMatch);
	}

	/* check the interfaces if more than one existing */
	if (CountOfInterfaces > 1)
	{
		/* we compare IP with the other interfaces */
		{
			OHA_HDB_EDD_PTR_TYPE  pHDBEdd = OHA_GET_HEDD_PTR(0);
			LSA_UINT32            i = 0;
			while (i < OHA_CFG_MAX_NICS)
			{
				if (pHDBEdd->State == OHA_HANDLE_STATE_OPEN)
				{
					...
					{
						OHA_DB_IF_PTR_TYPE pDB = OHA_DB_GET_INTERFACE(pHDBEdd->Params.pDetail->Params.Edd.NICId);

						if (OHA_IS_NOT_NULL(pDB))
						{
							OHA_COMMON_MEM_U8_PTR_TYPE IpPtr = (OHA_COMMON_MEM_U8_PTR_TYPE)&pDB->LocalPendingIP; /* read pending IP */
							OHA_PROGRAM_TRACE_05(0, LSA_TRACE_LEVEL_NOTE_LOW, "OhaSetLocalPendingIp: NicID: (0x%X), Ip: (%u.%u.%u.%u)",
												 NICId, *IpPtr, *(IpPtr + 1), *(IpPtr + 2), *(IpPtr + 3));

							if (OHA_MEMCMP(IpPtr, pIpAddr, OHA_IP_ADDR_SIZE))
							{
								OHA_FUNCTION_TRACE_02(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_IsIpOnAnyOtherIfLocalPending: Same Ip Pending (ThisNICId: 0x%X, NICId: 0x%X)", NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
								IpMatch = LSA_TRUE;
								break;
							}
						}
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsIpOnAnyOtherIfLocalPending(IpMatch: 0x%X)", IpMatch);
	return(IpMatch);
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_IsIpOnAnyOtherIfActive                  +*/
/*+-------------------------------------------------------------------------+*/
/*+  Input/Output:                                                          +*/
/*+                                                                         +*/
/*+           NICId          Interface (this)                               +*/
/*+           pIpAddr:       Pointer to IP                                  +*/
/*+-------------------------------------------------------------------------+*/
/*+  Description: compare the interfaces IpAddresses for mismatch           +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_IsIpOnAnyOtherIfActive(
	LSA_UINT16                      NICId,
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr)
{
	LSA_UINT16  CountOfInterfaces = OHA_GetCountOfInterfaces();
	...
	OHA_FUNCTION_TRACE_03(0, LSA_TRACE_LEVEL_CHAT,
		"IN : OHA_IsIpOnAnyOtherIfActive(NICId: 0x%X, pIpAddr: 0x%X, CountOfIfs: 0x%X)", NICId, pIpAddr, CountOfInterfaces);

	OHA_ASSERT_NULL_PTR(pIpAddr);

	if (OHA_GET32_NTOH(pIpAddr, 0) == 0) /* IP is 0.0.0.0 */
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsIpOnAnyOtherIfActive - No IP(IpMatch: 0x%X)", IpMatch);
		return(IpMatch);
	}
		...
		/* we compare IP with the other interfaces */
		{
			...
			while (i < OHA_CFG_MAX_NICS)
			{
				if (pHDBEdd->State == OHA_HANDLE_STATE_OPEN)
				{
					...
					if (NICId != pHDBEdd->Params.pDetail->Params.Edd.NICId) /* not for this If */
					{
						OHA_COMMON_MEM_U8_PTR_TYPE IpPtr = LSA_NULL;
						OHA_DB_READ_IP_SUITE(pHDBEdd->Params.pDetail->Params.Edd.NICId, &IpPtr, LSA_NULL, LSA_NULL);

						if (OHA_IS_NOT_NULL(IpPtr))
						{
							OHA_PROGRAM_TRACE_05(0, LSA_TRACE_LEVEL_NOTE_LOW, "OhaSetLocalPendingIp: NicID: (0x%X), Ip: (%u.%u.%u.%u)",
												 NICId, *IpPtr, *(IpPtr + 1), *(IpPtr + 2), *(IpPtr + 3));
							if (OHA_MEMCMP(IpPtr, pIpAddr, OHA_IP_ADDR_SIZE))
							{
								if (OHA_IsIpLocalPending(pHDBEdd->Params.pDetail->Params.Edd.NICId, pIpAddr)) /* local IP conflict is already set for this IF */
								{
									OHA_FUNCTION_TRACE_02(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "OHA_IsIpOnAnyOtherIfActive: Pending Ip (ThisNICId: 0x%X, OtherNICId: 0x%X)", NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
								}
								else
								{
									OHA_FUNCTION_TRACE_02(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "OHA_IsIpOnAnyOtherIfActive: Active Ip (ThisNICId: 0x%X, OtherNICId: 0x%X)", NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
									IpMatch = LSA_TRUE;
									break;
								}
							}
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsIpOnAnyOtherIfActive(IpMatch: 0x%X)", IpMatch);
	return(IpMatch);
}

/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*+                                                                         +*/
/*+  Functionname          :    OHA_GetIfLocalPending                       +*/
/*+-------------------------------------------------------------------------+*/
/*+  Input/Output:                                                          +*/
/*+                                                                         +*/
/*+           pIpAddr:       Pointer to IP                                  +*/
...
/*+  Description:                                                           +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT32 OHA_GetIfLocalPending(
	OHA_COMMON_MEM_U8_PTR_TYPE      pIpAddr)
{
	LSA_UINT16  CountOfInterfaces = OHA_GetCountOfInterfaces();
	LSA_UINT32  InterfaceID = 0;

	OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
		"IN : OHA_GetIfLocalPending(pIpAddr: 0x%X, CountOfIfs: 0x%X)", pIpAddr, CountOfInterfaces);

	OHA_ASSERT_NULL_PTR(pIpAddr);

	if (OHA_GET32_NTOH(pIpAddr, 0) == 0) /* IP is 0.0.0.0 */
	{
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_GetIfLocalPending - No IP(InterfaceID: 0x%X)", InterfaceID);
		return(0);
	}
			...
			if (pHDBEdd->State == OHA_HANDLE_STATE_OPEN)
			{
				OHA_ASSERT_NULL_PTR(pHDBEdd->Params.pDetail);
				{
					...
					if (OHA_IS_NOT_NULL(pDB))
					{
						OHA_COMMON_MEM_U8_PTR_TYPE IpPtr = (OHA_COMMON_MEM_U8_PTR_TYPE)&pDB->LocalPendingIP; /* read pending IP */
						OHA_PROGRAM_TRACE_05(0, LSA_TRACE_LEVEL_NOTE_LOW, "OhaSetLocalPendingIp: NicID: (0x%X), Ip: (%u.%u.%u.%u)",
											 pHDBEdd->Params.pDetail->Params.Edd.NICId, *IpPtr, *(IpPtr + 1), *(IpPtr + 2), *(IpPtr + 3));

						if (OHA_MEMCMP(IpPtr, pIpAddr, OHA_IP_ADDR_SIZE))
						{
							if (InterfaceID == 0 || pDB->InterfaceID < InterfaceID) /* return lowest IfID */
							{
								InterfaceID = pDB->InterfaceID;
								OHA_FUNCTION_TRACE_02(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW, "OHA_GetIfLocalPending: Ip Pending (NICId: 0x%X, InterfaceID: 0x%X)", pHDBEdd->Params.pDetail->Params.Edd.NICId, InterfaceID);
							}
						}
					}
				}
	...
	OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_GetIfLocalPending(InterfaceID: 0x%X)", InterfaceID);
	return(InterfaceID);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_UserSetAddrPending (
    OHA_HDB_USER_PTR_TYPE pHDBUser,
    LSA_UINT16 Status)
{
    ...
    switch (OHA_RQB_GET_OPCODE(pRQBUser))
    {
        ...
        case OHA_OPC_PRM_COMMIT:
            if (Status != OHA_OK)
            {
                ...
                OHA_UserRequestFinish(pHDBUser,pRQBUser,OHA_ERR_FAILED);

                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"SetIpAddress for Prm-Commit (Rtf) failed (Status: 0x%X)!", Status);
                OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
            }
            else
            {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_GetCountOfInterfaces( LSA_VOID )
{
    ...
    LSA_UINT16 NicID)
{
    ...
    for (i=0; i < OHA_CFG_MAX_CHANNELS; i++, pHDBUser++)
    {
...

		{
			...
			{
				if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
					NicID == pHDBUser->Params.pDetail->Params.User.NICId) /* has this NIC */
				{
					UserCount++;
				}
			}
...
/*+  Description: counts opened user channels of a NIC                      +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_USER_PTR_TYPE OHA_USERIsAgentActive(LSA_HANDLE_TYPE  Agent,
										    LSA_BOOL        *pIsRema)
{
    LSA_UINT32            i;
    LSA_UINT16            UserCount = 0;
			...
			if(Agent == OHA_HANDLE_TYPE_LLDP)
			{
				...
				if (OHA_RQB_GET_OPCODE(pUserRqb) == OHA_OPC_PRM_WRITE)
				{
						...
						pUserRqb->args.PrmData.record_index == OHA_RECORD_INDEX_MULTIPLE_SNMP)
					{
						UserCount++;
						pHDBUserRet = pHDBUser;
						IsRema      = (pUserRqb->args.PrmData.mk_rema == OHA_REMANENT_YES ? LSA_TRUE : LSA_FALSE);
					}
				}
			}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_AGNT_PTR_TYPE OHA_SOCKIsAgentActive(LSA_HANDLE_TYPE Agent)
{
                           ...
                           LSA_SYS_PTR_TYPE       pSys)
{
    ...
    while (pSubAgnt->SnmpRQBs.pBottom)
    {
        ...
        OHA_SOCK_FREE_LOWER_MEM(&RetVal,pRxRQB->args.snmp_recv.buffer, pSys);
        OHA_MEM_FREE_FAIL_CHECK(RetVal);

        OHA_SOCK_FREE_LOWER_RQB(&RetVal,pRxRQB, pSys);
        OHA_MEM_FREE_FAIL_CHECK(RetVal);
	}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT32 OHA_DbGetGlobalIfMode(
...
/* FUNCTION: OHA_CheckIpEnabled()
 *
 *  Input/Output: LSA_UINT16        NICId
 *
 * Description:   Returns the Detail setting of CheckIpEnabled
 *
 * RETURNS:       LSA_TRUE
 *                LSA_FALSE
 */
#ifndef OHA_CFG_NO_NARE
LSA_BOOL OHA_CheckIpEnabled(LSA_UINT16 NICId)
{
	LSA_BOOL CheckIpEnabled = LSA_FALSE;
	OHA_HDB_NARE_PTR_TYPE pHDBNare = OHA_NAREGetHandleByNIC(NICId);

	if (OHA_IS_NOT_NULL(pHDBNare) && OHA_IS_NOT_NULL(pHDBNare->Params.pDetail))
	{
		CheckIpEnabled = pHDBNare->Params.pDetail->Params.Nare.CheckIpEnabled;
		OHA_ASSERT (CheckIpEnabled == LSA_FALSE || CheckIpEnabled == LSA_TRUE); /* sanity */
	}
	return(CheckIpEnabled);
}
#else
LSA_BOOL OHA_CheckIpEnabled(LSA_UINT16 NICId)
{
