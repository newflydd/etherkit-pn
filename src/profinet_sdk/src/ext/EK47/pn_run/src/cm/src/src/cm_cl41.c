					...
					for (CmListForeach (rec, & sub->cl.prm_record_list, CM_PD_RECORD_PTR_TYPE))
					{
						...
						if (cm_pd_mpa_prm_prealloc_tailor_data (LSA_NULL/*usecase cmcl*/, rec) != CM_OK)
						{
							CM_CL_TRACE_02 (argr->trace_idx, LSA_TRACE_LEVEL_UNEXP
								, "dev(%u): tailor data pre-allocation error: record_index(0x%x)"
...
{
	if ((mtd->properties & CM_PD_MTD_PROP_NOS) && ((dev->device_mode & CM_CL_DEVICE_MODE_NOS_OVERWRITE_MASK) != CM_CL_DEVICE_MODE_NOS_OVERWRITE_YES))
	{
		CM_CL_TRACE_01 (argr->trace_idx, LSA_TRACE_LEVEL_ERROR
			, "dev(%u): MasterProject needs property CM_CL_DEVICE_MODE_NOS_OVERWRITE_YES", argr->device_nr
			);
			...
			for (CmListForeach (mod, & ap->mod_list, CM_AR_GRAPH_MODULE_PTR_TYPE))
			{
				...
				for (CmListForeach (sub, & mod->submod_list, CM_AR_GRAPH_SUBMODULE_PTR_TYPE))
				{
					if (CM_IS_PDEV_SUBSLOT_NR (sub->subslot_nr))
					{
						...
						{
							if (argr->ar.ar_type == CM_AR_TYPE_SINGLE_RTC3) /* see Series and Modular Machines 4.4.7.5 Modular switches */
							{
								/* see cm_pd_mtd_lookup_mtdp_from_index() */
								CM_CL_TRACE_01 (argr->trace_idx, LSA_TRACE_LEVEL_ERROR
									, "dev(%u): all Pdev.Subslots of an MasterProject must located on the same slot"
									, argr->device_nr
									);
	...
	)
{
	...
	CM_AR_GRAPH_SUBMODULE_PTR_TYPE sub;
	CM_PD_RECORD_PTR_TYPE          rec;
	CM_PD_RECORD_PTR_TYPE          rec_8052 = LSA_NULL;
	...
	)
{
	...
	{
		alias->name_length = 0;
		while (index < mtd->port_cnt)
		{
			CM_CL_DEVICE_PTR_TYPE	peer_dev;
			CM_PD_MTD_PORT_PTR_TYPE  mtdp = &mtd->ports[index];
			index += 1;

			cm_pd_mtdp_get_peer (dev->channel, mtdp);

			if (mtdp->real_station_nr == 0xFFFF)
			{
				continue;
			}
			if (cm_is_null (mtdp->real_mtdp))
			{
				continue;
			}

			alias->name_length = cm_pd_mpa_put_port_name (alias->name, mtdp->real_properties, mtdp->real_mtdp->local_subslot_nr, mtdp->real_mtdp->local_slot_nr);

			if (mtdp->real_station_nr > 0)
			{
				peer_dev = cm_cl_dev_from_device_nr (dev->channel, mtdp->real_station_nr);
				if (cm_is_null(peer_dev))
				{
					CM_CL_TRACE_01 (dev->channel->trace_idx , LSA_TRACE_LEVEL_UNEXP
						, "required dev(%u) not yet added", mtdp->real_station_nr
						);
					return LSA_FALSE;
				}
			...
			if (mtdp->real_station_nr == 0 || mtd->properties & CM_PD_MTD_PROP_NOS)
			{
				if (alias->name_length + 1 + ioc_nos_len  > OHA_MAX_STATION_NAME_LEN)
				{
					CM_CL_TRACE_02 (dev->channel->trace_idx , LSA_TRACE_LEVEL_UNEXP
						, "dev(%u) length of tailored Alias of IOD(%u) >255", dev->device_nr, mtdp->real_station_nr
						);
					return LSA_FALSE;
				}
	...
	)
{
	...
	// - an address in the address range from 0.0.0.0 up to 0.255.255.255 (highest 8 bit of the address are set to “0000 0000”)
	...
	*/
	...
	else
	{
		...
		else /* note: iod_ipaddr > ioc_ipaddr */
		{
			...
			{
				dev->dev_addr.device_ip = CM_HTON32(iod_ipaddr);
				return LSA_TRUE;
			}
		}
	}
	...
	)
{
	...
	/* CM_ERROR_CAUSE_MP_IN_PROGRESS		17 MasterProject: Machine- or AddressTailoring in progress */
	{
		...
		{
			CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) the PDev is not stable, tailoring in progress", argr->device_nr);
	...
	/* CM_ERROR_CAUSE_MP_NOS_MISSING		18 MasterProject: NameOfStation adaption missing (PDev with invalid NoS) */
	{
		...
		{
			CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) NameOfStation adaption missing (PDev with invalid NoS)", argr->device_nr);
		...
		{
			CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) NameOfStation for PDev is invalid", argr->device_nr);
			CM_CL_DINFO_SET (dinfo, CM_AR_REASON_DCP_NAME, CM_ERROR_CAUSE_MP_NOS_MISSING, CM_PNIO_FAULTY_RTA(CM_AR_REASON_DCP_NAME));
			return LSA_FALSE;
		}
	}
	...
	/* CM_ERROR_CAUSE_MP_IPv4_MISSING		19 MasterProject: IPv4Suite adaption missing (PDev with invalid IPv4Suite) */
	{
		...
		{
			CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) IPv4Suite adaption missing (PDev with invalid IPv4Suite)", argr->device_nr);
		...
		{
			CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) ip-suite not valid, must wait", argr->device_nr);
			CM_CL_DINFO_SET (dinfo, CM_AR_REASON_IPSUITE, CM_ERROR_CAUSE_MP_IPv4_MISSING, CM_PNIO_FAULTY_RTA(CM_AR_REASON_IPSUITE));
			return LSA_FALSE;
		}
		...
		{
			if (ioc_netmask != argr->cm.cl.dev->dev_addr.device_subnet_mask)
			{
				CM_CL_TRACE_03(channel->trace_idx, LSA_TRACE_LEVEL_NOTE
					, "dev(%u) local_subnet_mask(0x%x) not identical with device_subnet_mask(0x%x), must wait"
					, argr->device_nr, ioc_netmask, argr->cm.cl.dev->dev_addr.device_subnet_mask
					);
	...
	/* CM_ERROR_CAUSE_MP_TAILORING_MISSING	22 MasterProject: MachineTailoring missing */
	{
			...
			if (pd_mtd->properties & (CM_PD_MTD_PROP_I_ANY_PEER_OPT | CM_PD_MTD_PROP_I_ANY_PEER_PROG))
			{
				if (! (pd_mtd->set_by_itd & CM_PD_MTD_SET_IS_TAILORED))
				{
					CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE,
						"dev(%u) MachineTailoring missing, pd has optional/programable peer", argr->device_nr);
		...
		if (mtd->properties & (CM_PD_MTD_PROP_DEVICE_OPTIONAL | CM_PD_MTD_PROP_I_ANY_PEER_OPT | CM_PD_MTD_PROP_I_ANY_PEER_PROG))
		{
			...
			{
				CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE,
					"dev(%u) MachineTailoring missing, device is optional or has optional/programable peer", argr->device_nr);
				CM_CL_DINFO_SET (dinfo, CM_AR_REASON_ABORT, CM_ERROR_CAUSE_MP_TAILORING_MISSING, CM_PNIO_FAULTY_RTA(CM_AR_REASON_ABORT));
				return LSA_FALSE;
			}
		}
	...
	/* CM_ERROR_CAUSE_MP_DEVICE_DISABLED	24	*/
	{
		if (mtd->properties & CM_PD_MTD_PROP_DEVICE_OPTIONAL)
		{
			...
			{
				CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) MachineTailoring: do not activate a disabled device", argr->device_nr);
	...
	/* CM_ERROR_CAUSE_MP_IPv4_INVALID		21 MasterProject: tailored IPv4Suite of IOD is invalid */
	{
		if (mtd->properties & CM_PD_MTD_PROP_IP_SUITE)
		{
			...
			{
				CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) tailored IPv4Suite of IOD is invalid", argr->device_nr);
	...
	{
		CM_CL_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_NOTE, "dev(%u) the PDev has ARP, must wait", argr->device_nr);
