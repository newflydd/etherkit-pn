...
static LSA_UINT16 OHA_SetMacPhyConfigurationStatus(
    OHA_HDB_EDD_PTR_TYPE        pHDB,
    LSA_UINT16                  PortId,
    LSA_BOOL                  * PHY_Difference);

static LSA_UINT16 OHA_DeleteMacPhyConfigurationStatus(
    OHA_HDB_EDD_PTR_TYPE        pHDB,
    LSA_UINT16                  PortId,
    LSA_BOOL                  * PHY_Difference);

static LSA_UINT16 OHA_SetMaxFrameSize(
    OHA_HDB_EDD_PTR_TYPE        pHDB,
    LSA_UINT16                  PortId,
    LSA_BOOL                  * FrameSize_Difference);

static LSA_UINT16 OHA_DeleteMaxFrameSize(
    OHA_HDB_EDD_PTR_TYPE        pHDB,
    LSA_UINT16                  PortId,
    LSA_BOOL                  * FrameSize_Difference);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_edd_request_lower_done(
        ...
        case OHA_HANDLE_STATE_OPEN_EDD_RUNNING:
            if ( Response == OHA_OK)
            {
                ...
                if ( Response != OHA_OK)
                {
                    ...
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD Issuing GetParamsRequest failed.");

                    pHDB->State  = OHA_HANDLE_STATE_CLOSE_ERROR_EDD_RUNNING;

                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-Closing Channel because of error.");

                    if ( OHA_EDDCloseChannel(pHDB->EDDHandle,pHDB->Params.pSys) != OHA_OK)
                    {
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close EDD Channel (Handle: 0x%X).",pHDB->ThisHandle);
        ...
        case OHA_HANDLE_STATE_GET_MAC_EDD_RUNNING:
            if ( Response == OHA_OK)
            {
                ...
                if (Response != OHA_OK)                     /* an error occurred */
                {
                    ...
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD Issuing GetPortParamsRequest failed.");
            ...
            else
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD GetParams request failed. Response: 0x%X", Response);
                ...
                if ( OHA_EDDCloseChannel(pHDB->EDDHandle,pHDB->Params.pSys) != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close EDD Channel().");
                    OHA_FATAL(OHA_FATAL_ERR_EDD);
                }
            }
            break;
        ...
        case OHA_HANDLE_STATE_GET_LINK_STATUS_EDD_RUNNING:
            if ( Response == OHA_OK)
            {
				...
				if (OHA_IS_NULL(IpAdrPtr))
				{
					...
					{
						OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"SOCK-OHA_DB_WRITE_IP_SUITE failed.[0x%X]",RetVal);
					}
				...
				else
				{
					...
					if (RetVal != LSA_RET_OK)
					{
						OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"SOCK-oha_db_store_expected_ip_suite failed.[0x%X]",IpAdrPtr);
					}
				}
				...
				if (OHA_IS_NOT_NULL(OHA_SOCKGetHandleGlobal())) /* a (global) SOCK channel is open? */
				{
					if (OHA_IS_NOT_NULL(IpAdrPtr))
					{
						...
						else
						{
							...
							OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"SOCK-SetIpAddress failed.[0x%X]",Response);
							Response = OHA_ERR_LL;
						}
					}
				}
				else /* TFS1728432: The global SOCK channel must have been opened first ! */
				{
					Response = OHA_ERR_SEQUENCE;
					OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,
								 "WRONG SYSTEM ADAPTION(0x%X): Open the global SOCK channel first!", Response);
				}
			}
            if ( Response == OHA_OK)
            {
            ...
            {
                /* GetParams or Ind-Request failed. Cancel Recv-Requests */
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD GetParams, EDD LinkIndProvide request failed. Response: 0x%X", Response);
        ...
        case OHA_HANDLE_STATE_SET_ADDRESS_EDD_RUNNING:
			...
			if ( Response == OHA_OK)
			{
                ...
                if (RetVal != OHA_OK)
                {
                    ...
                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_EDDChannelDone - EDDSetArpFilter(RetVal: 0x%X)", RetVal);
                }
                RetVal = OHA_EDDSetIp(NICId, OHA_HANDLE_TYPE_DCP);
                if (RetVal != OHA_OK)
                {
                    Response = OHA_ERR_FAILED;
                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_EDDChannelDone - EDDSetIp(RetVal: 0x%X)", RetVal);
                }
                ...
                if (RetVal != OHA_OK)
                {
                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OHA_EDDChannelDone - LLDPUpdateSend(RetVal: 0x%X)", RetVal);
                }
            ...
            else
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"SOCK SetIp request failed. Response: 0x%X", Response);
    ...
    LSA_UINT16                  Orginator)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            OHA_EDD_RQB_SET_HANDLE(pRQB,0); /* TIA 2709127 */
            EDD_RQB_SET_HANDLE_LOWER(pRQB,pHDB->EDDHandle); /* TIA 2709127 */
            OHA_EDD_RQB_SET_USERID_UVAR16(pRQB,Orginator);

            pRQB->pParam       = pGetParams;

            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_GET_PARAMS (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_EDD_REQUEST_LOWER(pRQB,pSys);
        }
        else
        {
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating lower-memory failed!");
    ...
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_SRV_GET_PARAMS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
    {
        case EDD_STS_OK:
            ...
            OHA_PROGRAM_TRACE_06(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-GET PARAMS MAC: %2X-%2X-%2X-%2X-%2X-%2X",
            ...
            OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-GET PARAMS PortCnt: %d, InterfaceID: %d",PortCnt, InterfaceID);
            break;

        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-ERROR: GET PARAMS failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
	...
	switch(OHA_HANDLE_GET_TYPE(Orginator))
	{
        ...
        break;
		default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_EDDGetParamsDone: Invalid HandleType (0x%X).", Orginator);
            OHA_FATAL(OHA_FATAL_ERR_HANDLE_INVALID);
		break;
	}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_EDDLinkIndProvide(
    OHA_HDB_EDD_PTR_TYPE     pHDB,
    LSA_UINT16               PortCnt)
{
    ...
    while ((Status == OHA_OK) && (PortCnt))
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
        {
            ...
            if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
            {
                ...
                pLink->SyncId0_TopoOk = EDD_PORT_SYNCID0_TOPO_NOT_OK;
                pLink->PhyStatus      = EDD_PHY_STATUS_UNKNOWN;
                pLink->ShortPreamble  = EDD_PORT_LONG_PREAMBLE;
                pLink->TxFragmentation = EDD_PORT_NO_TX_FRAGMENTATION;

                OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_LINK_STATUS_IND_PROVIDE_EXT (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            ...
            {
                Status = OHA_ERR_RESOURCE;
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating lower EDD memory failed!");
                OHA_EDD_FREE_LOWER_RQB(&RetVal,pRQB,pSys);
                OHA_MEM_FREE_FAIL_CHECK(RetVal);
            }
        }
        ...
        {
            Status = OHA_ERR_RESOURCE;
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating lower EDD RQB-memory failed!");
        }
    ...
    OHA_HDB_EDD_PTR_TYPE            pHDB)
{
        ...
        switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
        {
            case EDD_STS_OK:
                ...
                if (( pLink->PortID > 0 ) && (pLink->PortID <= pDB->PortCnt ))
                {
                    ...
                    pHDB->LinkStatus[pLink->PortID-1].ShortPreamble   = pLink->ShortPreamble == EDD_PORT_SHORT_PREAMBLE ? LSA_TRUE : LSA_FALSE;
                    pHDB->LinkStatus[pLink->PortID-1].TxFragmentation = pLink->TxFragmentation == EDD_PORT_TX_FRAGMENTATION ? LSA_TRUE : LSA_FALSE;
                    ...
                    pHDB->LinkStatus[pLink->PortID-1].SyncTopoOk = pLink->SyncId0_TopoOk;/* for info record */
                    ...
                    if (pLink->PortStatus == EDD_PORT_NOT_PRESENT)
                    {
                        (LSA_VOID)OHA_DeleteMaxFrameSize(pHDB, pLink->PortID, LSA_NULL);
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_EDDLinkIndDone: Port (0x%X) not present!", pLink->PortID);
                    }
                    else
                    {
                        (LSA_VOID)OHA_SetMaxFrameSize(pHDB, pLink->PortID, LSA_NULL);
                    }

                    {
                        ...
                        LinkState = OHA_GetLinkStatus(pHDB->LinkStatus[pLink->PortID-1].LinkStatus,
                                                      pHDB->LinkStatus[pLink->PortID-1].PhyStatus,
                                                      pHDB->LinkStatus[pLink->PortID-1].PortStatus);
                            ...
                            (LinkState == OHA_PORT_LINK_STATE_UP)) /* update LLDP data by LLDPSet() */
                        {
                            ...
                            if (!LSA_HOST_PTR_ARE_EQUAL(pHDBLldp, LSA_NULL))
                            {
                                OHA_LOWER_TRACE_02(pDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH
                                                   ...
                                                   , pLink->PortID, pHDBLldp->RequestState);
                        ...
                        if (LinkStatusChange == EDD_LINK_UP)
                        {
                            ...
                            {
                                OHA_PROGRAM_TRACE_02(pDB->TraceIdx, LSA_TRACE_LEVEL_WARN, "OHA_EDDLinkIndDone: Changed to UP but PHY disabled (LinkState: 0x%X, LinkStatusChange: 0x%X).", LinkState, LinkStatusChange);
                            }
#endif
                        }
                    }
    ...
    if ( Reprovide )
    {
        ...
        OHA_EDD_RQB_SET_HANDLE(pRQB,0); /* TIA 2709127 */
        EDD_RQB_SET_HANDLE_LOWER(pRQB,pHDB->EDDHandle); /* set handle TIA 2709127 */
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_EDDGetPortParams(
    LSA_UINT16                  NICId,
    LSA_UINT16                  PortId,
    LSA_UINT16                  Orginator)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_GET_PORT_PARAMS (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_EDD_REQUEST_LOWER(pRQB,pSys);
        }
        else
        {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDGetPortParamsDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16               Response  = OHA_OK;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_SRV_GET_PORT_PARAMS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
    {
        case EDD_STS_OK:
            ...
            else if (pGetPortParams->PortStatus == EDD_PORT_NOT_PRESENT)
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_EDDGetPortParamsDone: Port (0x%X) not present!", PortId);
            }
            else
            {
                ...
                switch (LinkSpeedModeConfigured)
                {
                    ...
                    case EDD_LINK_10GBIT_FULL:
                        ...
                        break;
                    default:
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"EDD GetPortParams - invalid LinkSpeedMode configured (0x%X).", LinkSpeedModeConfigured);
                        OHA_FATAL(OHA_FATAL_ERR_EDD);
                        break;
                }
                ...
                OHA_ASSERT (pGetPortParams->IsWireless == EDD_PORT_IS_NOT_WIRELESS ||
                            pGetPortParams->IsWireless == EDD_PORT_IS_WIRELESS); /* sanity check */
                pHDB->LinkStatus[PortId-1].IsWireless = (EDD_PORT_IS_WIRELESS == pGetPortParams->IsWireless) ? LSA_TRUE : LSA_FALSE;
                ...
                if (pHDB->LinkStatus[PortId-1].IsWireless)
                {
                    ...
                    if (RetVal != LSA_RET_OK)
                    {
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_EDDGetPortParamsDone: OHA_DeleteMacPhyConfigurationStatus failed (RetVal: 0x%X).", RetVal);
                    }
                }
                else
                {
                    ...
                    if (RetVal != LSA_RET_OK)
                    {
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_EDDGetPortParamsDone: OHA_SetMacPhyConfigurationStatus failed (RetVal: 0x%X).", RetVal);
                    }
                }

                /* update LLDP frame only  if the link is up */
                LinkState = OHA_GetLinkStatus(pHDB->LinkStatus[PortId-1].LinkStatus,
                                              pHDB->LinkStatus[PortId-1].PhyStatus,
                                              pHDB->LinkStatus[PortId-1].PortStatus);
                if ((PHY_Difference) && LinkState == OHA_PORT_LINK_STATE_UP) /* update LLDP data by LLDPSet() */
                {
                        ...
                        (pHDBLldp->RequestState == OHA_LLDP_STATE_START || pHDBLldp->RequestState == OHA_LLDP_STATE_RE_START)) /* LLDP-Set is possible ? */
                    {
                        ...
                        if (RetVal != LSA_RET_OK)
                        {
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_EDDGetPortParamsDone: OHA_LLDPSet failed (RetVal: 0x%X).", RetVal);
                        }
                    }
                }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDOpenChannelDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16               RetVal   = LSA_RET_OK;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_OPC_OPEN_CHANNEL done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));

    Orginator = OHA_EDD_RQB_GET_HANDLE(pRQB);

    if (OHA_EDD_RQB_GET_RESPONSE(pRQB) == EDD_STS_OK)
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-Open Channel successful.");
        /* save channel */
        pHDB->EDDHandle = ((OHA_EDD_LOWER_RQB_OPEN_CHANNEL_PTR_TYPE)pRQB->pParam)->HandleLower; /* TIA 2709127 */

    }
    else
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Open Channel failed.[0x%X].",OHA_EDD_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
...
/*+  Input                      EDD_LOWER_HANDLE_TYPE EDDHandle             +*/
/*+                             LSA_SYS_PTR_TYPE      pSys                  +*/
/*+                                                                         +*/
/*+                                                                         +*/
/*+  Result                :    LSA_UNIT16                                  +*/
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_EDDCloseChannel(
    ...
    LSA_SYS_PTR_TYPE      pSys)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        OHA_EDD_RQB_SET_HANDLE(pRQB,0); /* TIA 2709127 */
        EDD_RQB_SET_HANDLE_LOWER(pRQB,EDDHandle); /* TIA 2709127 */
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDCloseChannelDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16           RetVal   = LSA_RET_OK;
    ...
    if ( OHA_EDD_RQB_GET_RESPONSE(pRQB) == EDD_STS_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-Close Channel successful.");
    }
    else
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Close Channel failed.[0x%X].",OHA_EDD_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_EDD_PTR_TYPE OHA_EDDGetHandleByNIC(
    LSA_UINT16 NICId)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_EDDGetHandleByNIC(pHDB:  0x%X)", pHDB);
    return(pHDB);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EddIndicateEventAllUsers(
    OHA_HDB_EDD_PTR_TYPE        pHDBEdd,
    LSA_UINT16                  PortId
)
{
    ...
    if (OHA_IS_NOT_NULL(pHDBEdd->pHDBUserPrm))
    {
        ...
        if (RetVal != OHA_OK)
        {
            OHA_PROGRAM_TRACE_01(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_EddIndicateEventAllUsers(RetVal: 0x%X). OHA_DoRemotePortState() failed", RetVal);
        }
    }
    ...
    OHA_FUNCTION_TRACE_00(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_EddIndicateEventAllUsers()");
    return;
}
 ...
 *===========================================================================*/
LSA_UINT16 OHA_EDDControlSignal(
    LSA_UINT16  NicId,
    LSA_UINT8  Frequency
)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_LED_BLINK (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_EDD_REQUEST_LOWER(pRQB,pHDB->Params.pSys);
        }
        else
        {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDControlSignalDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16         RetVal = LSA_RET_OK;
    ...
    {
        case EDD_STS_OK:
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-LED BLINK o.k.");
            break;

        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-ERROR: LED BLINK failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            break;
    }
    ...
    LSA_BOOL                  * PHY_Difference)
{
    ...
    OHA_ASSERT_NULL_PTR(PHY_Difference);
    ...
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"lldp_TLVSetMACPHYConfigStatus: Invalid Response (0x%X)", RetVal);
    ...
    *PHY_Difference = LSA_TRUE;
    ...
    if (RetVal != LSA_RET_OK)
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_SetMacPhyConfigurationStatus: OHA_DB_WRITE_LLDP_ELEMENT(MAC/PHY Configuration/Status) failed (RetVal: 0x%X).", RetVal);
    }
    ...
    LSA_BOOL                  * Size_Difference)
{
    ...
    if (RetVal != LLDP_RSP_OK)
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"lldp_TLVSetMaxFrameSize: Invalid Response (0x%X)", RetVal);
        OHA_FATAL(OHA_FATAL_ERR_LLDP);
    }
    ...
    if (RetVal != LSA_RET_OK)
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_SetMaxFrameSize: OHA_DB_WRITE_LLDP_ELEMENT(MaxFrameSize) failed (RetVal: 0x%X).", RetVal);
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL OHA_EDDIsAtLeastOnePortUp(
    LSA_UINT16              NICId)
{
        ...
        (! LSA_HOST_PTR_ARE_EQUAL(pHDBLldp->pDB,  LSA_NULL)))
    {
        ...
        for (PortId = 1; PortId <= OHA_CFG_MAX_PORTS && PortId <= pHDBLldp->pDB->PortCnt; PortId++)
        {
            ...
            {
                RetVal = LSA_TRUE;
                OHA_FUNCTION_TRACE_01(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_IsAtLeastOnePortUp(RetVal: 0x%X)", RetVal);
                return(RetVal);
            }
        }
    }
 ...
 *===========================================================================*/
LSA_UINT16 OHA_EDDSetRemotePortState(
    LSA_UINT16  NicId,
...
)
{
    ...
    {
        OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "ParamError (PortId): RemotePortState.PortID()");
        Response = OHA_ERR_PARAM; /* no valid PortId */
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_EDDSetRemotePortState(Response: 0x%X)", Response);
        return(Response);
    }
	...
	else /* Set(s) already active, do not update */
	{
        ...
        OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_EDDSetRemotePortState already pending (PortId: 0x%X, PendingCnt: 0x%X).", RemotePortState.PortID, pHDB->Set_PendingCnt[RemotePortState.PortID-1]);
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_EDDSetRemotePortState(Response: 0x%X)", Response);
		return(Response);
	}
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            OHA_LOWER_TRACE_07(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,
            ...
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_SET_REMOTE_PORT_STATE (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_EDD_REQUEST_LOWER(pRQB,pHDB->Params.pSys);
        }
        else
        {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDSetRemotePortStateDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16         RetVal = LSA_RET_OK;
	...
	OHA_ASSERT_NULL_PTR(pRQB);
	OHA_ASSERT_NULL_PTR(pHDB);
	pRemotePortState = (EDD_UPPER_SET_REMOTE_PORT_STATE_PTR_TYPE)pRQB->pParam;
	PortID = pRemotePortState->PortID;

    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_SRV_SET_REMOTE_PORT_STATE done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));
    ...
    {
        case EDD_STS_OK:
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD-SET REMOTE_PORT_STATE o.k.");
            break;

        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-ERROR: SET REMOTE_PORT_STATE failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            break;
    }
	...
	if(!pHDB->Set_IsActual[PortID-1]) /* AP01001236 */
	{
		if(pHDB->Set_PendingCnt[PortID-1] == 0) /* update now */
		{
			...
			if (RetVal != OHA_OK)
			{
				OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_EDDSetRemotePortStateDone(): SetRemotePortState on Port %u failed.", PortID);
				OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
			}
		}
	}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL oha_update_port_status(
    LSA_UINT16                  NICId,
    LSA_UINT16                  PortID,
    LSA_BOOL                    OnlySyncForwarding)
{
    ...
    {
        StatusClass3 = OHA_PORT_STATUS_CLASS3_OFF;
        OHA_FUNCTION_TRACE_01(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_CHAT, "oha_update_port_status (PortId: 0x%X): OnlySyncForwarding port, RClass3_PortState is OFF (=OK)!", StatusClass3);
    }
    else
    {
        switch (pHDBEdd->LinkStatus[PortID-1].IRTPortStatus)
        {
            ...
            case OHA_PORT_STATUS_CLASS3_RUNNING:
                ...
                break;
            default:
                OHA_PROGRAM_TRACE_01(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_FATAL,"oha_update_port_status() - invalid IRTPortStatus (0x%X).",pHDBEdd->LinkStatus[PortID-1].IRTPortStatus);
                StatusClass3 = OHA_PORT_STATUS_CLASS3_OFF;
                OHA_FATAL(OHA_FATAL_ERR_EDD);
                break;
        }
...

    {
        ...
        if ((!LSA_HOST_PTR_ARE_EQUAL(NamePtr, LSA_NULL)) && (Len == OHA_LLDP_PNO_PORT_STATUS_SIZE))
        {
            if (!(OHA_MEMCMP(NamePtr, LldpPortStatus, OHA_LLDP_PNO_PORT_STATUS_SIZE)))
            {
                ...
                if (Status != LSA_RET_OK)
                {
                    OHA_PROGRAM_TRACE_01(pHDBEdd->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"oha_update_port_status(): OHA_DB_WRITE_LLDP_ELEMENT(PortStatus) failed (Status: 0x%X).", Status);
                }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_EDDSetArpFilter(
    LSA_UINT16                  NicID,
    LSA_UINT16                  Orginator)
{
    OHA_HDB_EDD_PTR_TYPE                    pHDB = OHA_EDDGetHandleByNIC(NicID);
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            pSetArpFilter->UserIndex = 0; /* we have one IP address */
            ...
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_SET_ARP_FILTER (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_EDD_REQUEST_LOWER(pRQB,pSys);
        }
        else
        {
    ...
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating lower-rqb failed!");
        Status = OHA_ERR_RESOURCE;
	}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDDSetArpFilterDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16               Response  = OHA_OK;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_SRV_SET_ARP_FILTER done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:  /* must not happen */
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"EDD-ERROR: SET ARP-FILTER failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            /* Response = OHA_ERR_LL; */
			OHA_FATAL(OHA_FATAL_ERR_EDD);
            break;
    }
    ...
    LSA_UINT16                  Orginator)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
        {
            ...
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_SET_IP (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
            OHA_LOWER_TRACE_04(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,
    ...
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: EDD_SRV_SET_IP done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_EDD_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-ERROR: SET IP failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            break;
    }
    ...
    LSA_UINT16                  Orginator)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        switch (Mode)
        {
            ...
            case OHA_RESET_FILTER:
            {
				...
				{
					OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating lower-DcpSetMemory failed!");
					OHA_EDD_FREE_LOWER_RQB(&RetVal,pRQB,pSys);
					OHA_MEM_FREE_FAIL_CHECK(RetVal);
					Status = OHA_ERR_RESOURCE;
				}
        ...
        if (Status == OHA_OK)
        {
			...
			if (pHDB->SetFilter_PendingCnt == 0) /* send the request */
			{
				...
				OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_NRT_SET_DCP_FILTER (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
				OHA_EDD_REQUEST_LOWER(pRQB,pSys);
			}
			else /* queue the request */
			{
				LSA_UINT32 BlockCount;
				OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Queue: EDD_SRV_NRT_SET_DCP_FILTER (Handle: 0x%X, EDDHandle: 0x%X, pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
				...
				OHA_LOWER_TRACE_04(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Queue-Details (pHDB: 0x%X, Mode: 0x%X, Orginator: 0x%X, QueuekCount: 0x%X)",pHDB,Mode,Orginator,BlockCount);
			}
        }
        else
        {
    ...
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-Allocating RQB-memory failed!");
        Status = OHA_ERR_RESOURCE;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_EDD_SetFilterDone(
    OHA_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16      Response;
    ...
    switch (OHA_EDD_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"EDD-ERROR: SET FILTER failed.(0x%X).",OHA_EDD_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
        break;
    }
        ...
        case OHA_HANDLE_TYPE_SYS:
			...
			switch(OHA_DCP_RQB_GET_OPCODE(pHDBDcp->pOCReq))
			{
				...
				if ( Response != OHA_OK)
				{
					...
					{
						OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close DCP Channel().");
			...
			OHA_IS_NOT_NULL(pHDB->LowerDcpFilterRQBs.pBottom))	/* queued rqb's */
    {
		...
		switch (OHA_EDD_RQB_GET_USERID_UVAR16(pNextRQB))
		{
			...
			case OHA_HANDLE_TYPE_USER:	/* DCP-Control */
				pHDB->SetFilter_PendingCnt++;
				OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: DCP-NextRequest(user): EDD_SRV_NRT_SET_ARP_FILTER (Handle: 0x%X, EDDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pNextRQB);
			...
			case OHA_HANDLE_TYPE_LLDP:  /* AliasNameOfStation changed */
				if (OHA_IS_NOT_NULL(pHDBDcp) && pHDBDcp->RequestState == OHA_DCP_STATE_FILTER_START)	/* execute the request */
				{
					...
					OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: DCP-NextRequest(local): EDD_SRV_NRT_SET_ARP_FILTER (Handle: 0x%X, DCPHandle: 0x%X,Response: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,Response);
				}
				/* free the queued RQB */
