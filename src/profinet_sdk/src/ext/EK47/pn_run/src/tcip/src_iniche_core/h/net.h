...
struct IfMib
{
   ...
   u_long   SupportedCounters; /* see edd_usr.h */
#endif
};
...
#if 1 /* sado -- ANYIP: added sendqueue here */
typedef struct tcip_packet_hold_tag
{
	void*    first_out;
	void*    last_pkt;

	unsigned int max_len; /* maximum packet queue length */
	unsigned int cur_len; /* current packet queue length */

} TCIP_PACKET_HOLD;

typedef struct tcip_packet_hold_tag * TCIP_PACKET_HOLD_PTR_TYPE;
#endif
...
#ifdef USE_IGMPV3
struct in_multi *    in_addmulti2(ip_addr *ap, struct net *netp, int addrtype, 
                                  u_short numsrc, ip_addr *sap,  int mode, 
                                  int init, int *error);
void                 in_delmulti2(struct in_multi *inm, u_short numsrc, ip_addr *sap,  
                                  int mode, int final, int *error);
struct in_multi *    in_modmulti(ip_addr *ap, struct net *netp, u_short numsrc, 
                                 ip_addr *sap, int mode, u_short old_num, 
                                 ip_addr *old_sap, int old_mode, int init, int *error);
#endif /* USE_IGMPV3 */
...
int      ni_set_ipsuite(NET ifp, u_long Ip_addr, u_long Subnet_mask, u_long Gateway); /* INICHE_TCIP_ITGR added */
#endif  /* DYNAMIC_IFACES */
 ...
 * by the system. 
 */
#ifndef NDEBUG /* INICHE_TCIP_ITGR added #ifndef, MS Visual C++ defines NDEBUG for release versions */
...
#if 0 /* INICHE_TCIP_ITGR: made 'panic' a #define in ipport.h */
