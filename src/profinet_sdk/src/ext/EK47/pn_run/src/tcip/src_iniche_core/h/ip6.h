...
#define IN6_IS_ADDR_LOOPBACK(ipaddr) \
     ((IP6EQ(ipaddr, &ip6loopback)) || (IP6EQ(ipaddr, &ip6llloopback)))
...
int   ip6_send2(PACKET pkt, struct in6_addr * router);
...
#define  IP6F_PERFORM_IPSEC 0x10 /* perform (1) or skip (0) IPSec processing in egress path */
...
int   nd_getPrePMTU(ip6_addr * ip, NET ifp);
...
#define NUM_GLBS_IF 5

typedef struct
{
   short      active;     /* 1 => there are active addresses in this IPV6ADDR_ONIF entry */
   short      cntGlb;     /* count of globals on this IF*/
   ip6_addr local;      /* link-local address for an IF */
   ip6_addr glbs[NUM_GLBS_IF];   /* array of globals on this IF */
} IPV6ADDR_ONIF;
...
ip6_addr * ip6_myAddrSrc(ip6_addr * addrP, int ifn);
int ip6_addrsList(IPV6ADDR_ONIF * addrP);
