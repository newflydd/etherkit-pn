...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_mrp_request_lower_done(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_MRP_PTR_TYPE OHAGetMrpHDBFromHandle(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByIndex(
    LSA_UINT32 Index)
{
    OHA_HDB_MRP_PTR_TYPE pHDB;
...
/*+               LSA_NULL is returned                                      +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_MRP_PTR_TYPE OHA_MRPGetHandleByIf(
    LSA_UINT32 IfId)
{
    ...
    if (IfId >= EDD_INTERFACE_ID_MIN && IfId <= EDD_INTERFACE_ID_MAX) /* 0 is not valid */
    {
		...
		if(ret == LSA_RET_OK)
		{
			...
			if(OHA_IS_NULL(pHDB))
			{
				OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_MRPGetHandleByIf(pHDB: 0x%X, NicID: 0x%X)", pHDB, NicID);
			}
		}
	}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#if 0
LSA_UINT16 OHA_MRPGetRingPortState(
    LSA_UINT16 NICId,
	LSA_UINT16 RingPortId)
{
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_MRPGetRingPortState(NICId: 0x%X, RingPortId: 0x%X)",
                          NICId, RingPortId);

	if(OHA_IS_NULL(pHDBEdd))
	{
		/* Error on getting handle. Abort. */
		OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_HIGH,"Request OHA_MRPGetRingPortState failed. No EDD Channel for this NicId (NIC-ID: 0x%X)", NICId);
    ...
    if (RingPortId && (RingPortId <= OHA_CFG_MAX_PORTS))
	{
		...
		else
		{
			...
			case EDD_MRP_PORTSTATE_ACTIVATED:
				...
				break;
			default:
	            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_MRPGetRingPortState(): MRP_PortStatus 0x%x failed (NICId: 0x%X).", pHDBEdd->LinkStatus[RingPortId-1].MRP_PortState,NICId);
				break;
			}
		}
	}
    ...
    LSA_UINT16                 Response)
{
    ...
    OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"<<<: OHA_MRPChannelDone done (NICId: 0x%X,Rsp: 0x%X,State: 0x%X)",pHDB->Params.pDetail->Params.Mrp.NICId,Response,pHDB->State);
        ...
        case OHA_HANDLE_STATE_OPEN_MRP_RUNNING:
            if ( Response == OHA_OK)
            {
				...
				else
				{
                    ...
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP Receive Provide request failed. Response: 0x%X", Response);

                    pHDB->OpenResponse = Response; /* save for later */

                    pHDB->State = OHA_HANDLE_STATE_CLOSE_ERROR_MRP_RUNNING;

                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"MRP-Closing Channel because of error");

                    if ( OHA_MRPCloseChannel(pHDB->MrpHandle,pHDB->Params.pSys) != OHA_OK)
                    {
                        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close MRP Channel().");
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_MRPOpenChannelDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: MRP_OPC_OPEN_CHANNEL done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_MRP_RQB_GET_RESPONSE(pRQB));

    if ( OHA_MRP_RQB_GET_RESPONSE(pRQB) == MRP_RSP_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"MRP-Open Channel successful");
        /* save MRP-Channel in HDB */
        pHDB->MrpHandle = pRQB->args.channel.open.handle;
    }
    else
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-Open Channel failed.[0x%X]",OHA_MRP_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_MRPCloseChannel(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_MRPCloseChannelDone(
    OHA_MRP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
    LSA_UINT16               Response = OHA_OK;
    ...
    OHA_HDB_MRP_PTR_TYPE    pHDB)
{
	...
	for(Instance = 0; (Instance < pDB->MaxMRP_Instances) && (Instance < EDD_CFG_MAX_MRP_INSTANCE_CNT); Instance++)
	{
		...
		if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
		{
			...
			OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: MRP-Request: MRP_OPC_RINGSTATE_INDICATION (Handle: 0x%X,MRPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->MrpHandle,pRQB);
		...
		{
			Status = OHA_ERR_RESOURCE;
			OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-Allocating lower RQB-memory failed!");
		}
	}
    ...
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: MRP_OPC_RINGSTATE_INDICATION done (Handle: 0x%X,Rsp: 0x%X,State: 0x%X)",
    ...
    if ( pHDB->State == OHA_HANDLE_STATE_OPEN )
    {
        switch (OHA_MRP_RQB_GET_RESPONSE(pRQB))
        {
            ...
            case MRP_RSP_OK_CANCEL:
                ...
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"MRP Receive canceled (0x%X).",OHA_MRP_RQB_GET_RESPONSE(pRQB));
                Reprovide = LSA_FALSE;
                break;

            default:
				Reprovide = LSA_FALSE;
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"MRP-ERROR: MRP_OPC_RINGSTATE_INDICATION failed.(0x%X).",OHA_MRP_RQB_GET_RESPONSE(pRQB));
                OHA_FATAL(OHA_FATAL_ERR_MRP);
                break;
        }
    }
    ...
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"MRP Receive while not in open-state. discard and dont reprovide.");
        Reprovide = LSA_FALSE;
    }
    ...
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
	...
	if ( OHA_IS_NULL(pHDBLldp) || OHA_IS_NULL(pHDBLldp->pDB))
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request MRP_OPC_RINGSTATE_INDICATION failed. No LLDP-Channel or DataBase present (NIC-ID: 0x%X)",NICId);
    }
    else if (pMrpInd->ohaStateInd.rPort1 > OHA_CFG_MAX_PORTS)  /* pHDB->PortCnt */
    {
        Response = OHA_ERR_PARAM;  /* Portid is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request MRP_OPC_RINGSTATE_INDICATION failed. rPortId1 is wrong (PORT-ID1: 0x%X)",pMrpInd->ohaStateInd.rPort1);
    }
    else if (pMrpInd->ohaStateInd.rPort2 > OHA_CFG_MAX_PORTS)  /* pHDB->PortCnt */
    {
        Response = OHA_ERR_PARAM;  /* Portid is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Request MRP_OPC_RINGSTATE_INDICATION failed. rPortId2 is wrong (PORT-ID2: 0x%X)",pMrpInd->ohaStateInd.rPort2);
    }
    else
    {
		...
		if(RingPortID_1 == 0 || RingPortID_2 == 0)	/* both must be 0 */
		{
			...
			{
		        Response = OHA_ERR_PARAM;  /* Portid is wrong */
                OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"MRP-Write (Add,Check) failed (CtrlMode: 0x%X, RingPort1: 0x%X, RingPort2: 0x%X)",
        ...
        {
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"MRP_OPC_RINGSTATE_INDICATION(Add): MRP_RingPortStatus(0x%X) failed.", pMrpInd->ohaStateInd.mrpStatus);
        ...
        switch (pMrpInd->ohaStateInd.topoCtrlMode)
        {
			...
			case MRP_USR_ENUM_TOPO_CTRL_ADD_CHECK:  /* former OHA_MRP_ADD_CHECK: add (expected) and build diagnosis */
            {
                ...
                if (!OHA_CheckDomainUUID(pMrpInd->ohaStateInd.domainUUID))  /* is there a valid UUID ? */
                {
                    ...
                    {
                        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"MRP_OPC_RINGSTATE_INDICATION-Write: DomainUUID failed.");
                        Response = LSA_RET_ERR_PARAM;
                        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_MrpWriteData(Response: 0x%X)", Response);
                        return;
                    }
                ...
                OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"MRP-Write (Add,Check) successful (CtrlMode: 0x%X, RingPort1: 0x%X, RingPort2: 0x%X)",
									 pMrpInd->ohaStateInd.topoCtrlMode, RingPortID_1, RingPortID_2);
                ...
                if (MRP_RingPortDifference_1) /* update LLDP data by LLDPSet() */
                {
						...
						pHDBLldp->RequestState == OHA_LLDP_STATE_RE_START) /* LLDP-Set is possible ? */
                    {
                        ...
                        {
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_MrpWriteData: OHA_LLDPSet(RingPortID_1) failed (Status: 0x%X).", Status);
                        }
                    }
                }
                if (MRP_RingPortDifference_2) /* update LLDP data by LLDPSet() */
                {
						...
						pHDBLldp->RequestState == OHA_LLDP_STATE_RE_START) /* LLDP-Set is possible ? */
                    {
                        ...
                        if (Status != LSA_RET_OK)
                        {
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_MrpWriteData: OHA_LLDPSet(RingPortID_2) failed (Status: 0x%X).", Status);
                        }
                    }
                }
            ...
            case MRP_USR_ENUM_TOPO_CTRL_NONE: /* former OHA_MRP_REMOVE: remove all peers of this port */
            {
                ...
                OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"MRP-Write (Delete) successful (RingPort1: 0x%X, RingPort2: 0x%X)", RingPortID_1, RingPortID_2);
            ...
            break;
            default:
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-Write failed.[topoCtrlMode: 0x%X]",pMrpInd->ohaStateInd.topoCtrlMode);
    ...
    LSA_UINT16                  Instance)
{
    ...
    if (OHA_IS_NULL(pHDB))
    {
        Status = OHA_ERR_RESOURCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Request MRP_OPC_GET_STATUS failed. No MRP Channel!");
    ...
    if (!(pHDB->State == OHA_HANDLE_STATE_OPEN) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Request MRP_OPC_GET_STATUS failed. MRP Channel is closing or not open (State: 0x%X)", pHDB->State);
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_MRPGetStatus(Status: 0x%X)",Status);
		return(Status);
    }
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
		...
		if(!LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL))
		{
			...
			pMrpGet->domainNameLength = MRP_MIB_DEFAULT_DOMAIN_NAME_SIZE_MAX; /* TFS 1414087 */

	        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: MRP-Request: MRP_OPC_GET_STATUS (Handle: 0x%X,MRPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->MrpHandle,pRQB);
		...
		else
		{
			...
			OHA_MEM_FREE_FAIL_CHECK(RetVal);
			Status = OHA_ERR_RESOURCE;
			OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-Allocating lower memory failed!");
		}
    }
    else
    {
    ...
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: MRP_OPC_GET_STATUS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_MRP_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_MRP_RQB_GET_RESPONSE(pRQB))
    {
        case MRP_RSP_OK:
...

			{
				...
				{
			        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Allocating local memory for MrpGet failed!");
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-ERROR: MRP_OPC_GET_STATUS failed.(0x%X).",OHA_MRP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
        ...
        case OHA_HANDLE_TYPE_SOCK:  /* from cable */
        {
            ...
            if (OHA_IS_NOT_NULL(pHDBSock))  /* not closed */
            {
                ...
                if (pSubAgnt->StatsPendCnt == 0)    /* got all necessary statistics ? */
                {
					...
					{
						OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "No active state anymore, late LldpGetStats.Con, state %Xh", pSubAgnt->RequestState);
					}
                }
            }
        }
...
)
{
    ...
    if (!(pHDB->State == OHA_HANDLE_STATE_OPEN) )
    {
        Status = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"Request MRP_OPC_SET_STATUS failed. MRP Channel is closing or not open (pHDB: 0x%X)", pHDB);
    ...
    if (ResetDelays != 1)
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"Request MRP_OPC_SET_STATUS failed. Invalid params (ResetDelays: 0x%X)", ResetDelays);
		OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_MRPSetStatus(Status: 0x%X)",Status);
		return(Status);
    }
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: MRP-Request: MRP_OPC_SET_STATUS (Handle: 0x%X,MRPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->MrpHandle,pRQB);
    ...
    OHA_HDB_MRP_PTR_TYPE        pHDB)
{
    ...
    switch (OHA_MRP_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MRP-ERROR: MRP_OPC_SET_STATUS failed.(0x%X).",OHA_MRP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
    ...
    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_MRPSetStatusDone(Response: 0x%X)", Response);
}
