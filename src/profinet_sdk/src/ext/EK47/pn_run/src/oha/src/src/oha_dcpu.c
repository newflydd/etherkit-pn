    ...
    OHA_HDB_DCP_PTR_TYPE pHDB)
{
    ...
    /* free the response RQBs for DCP Set */
    oha_dcp_free_rsp_rqb(pHDB, pHDB->pLowerDcpRspRqb); /* DcpGet.res RQB      */
    pHDB->pLowerDcpRspRqb = LSA_NULL;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestDcpIndication(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DcpIndicateEvent(
    OHA_HDB_USER_PTR_TYPE               pHDBUser,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE   pEvent
)
{
    ...
    if (pHDBUser->Params.pDetail->Params.User.DcpIndicationFilter != 0)	/* filter set ? */
    {
            ...
            pEvent->Event == OHA_EVENT_CONTROL) /* only a DCP-Set, not a Query-Set ! */
        {
            ...
            {
                OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,
                     ...
                     pEvent->Event == OHA_EVENT_REMOTE_SET)
            {
                ...
                if (pEvent->Options & OHA_CTRL_STATION_NAME)
                {
                    ...
                    if (OHA_IS_NOT_NULL(NamePtr) && length && (length <= OHA_MAX_STATION_NAME_LEN))
                    {
                        ...
                        if (OHA_IS_NOT_NULL(pStationData->pStationName))
                        {
                            OHA_MEMCPY(pStationData->pStationName, NamePtr, (LSA_UINT32)length);
                        }
                    }
                    else    /* no valid name */
                    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DcpWriteAccessControl(
    LSA_UINT16 NicId,
    LSA_UINT32 options,
    LSA_UINT8 control)
{
	...
	if (options & OHA_CTRL_DHCP_USAGE)
	{
		RetVal = OHA_DB_WRITE_ACCESS_CONTROL(NicId, OHA_DCP_OPTION_DHCP, 255, control);
	}
...
/*+  Description: Sets the access control options in the database           +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DcpWriteUserControl(
	LSA_UINT16      NicId,
    ...
    OHA_UPPER_DCP_CONTROL_PTR_TYPE pControl)
{
    ...
    if (pControl->Options & OHA_CTRL_DHCP_CLIENT_ID)
    {
        RetVal |= OHA_DbWriteUserControl(NicId, UserHandle, OHA_DCP_OPTION_DHCP, 61, pControl->Mode);
    }
	if (pControl->Options & OHA_CTRL_DHCP_USAGE)
	{
		RetVal |= OHA_DbWriteUserControl(NicId, UserHandle, OHA_DCP_OPTION_DHCP, 255, pControl->Mode);
	}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DcpIndicateEventAllUsers(
    OHA_HDB_DCP_PTR_TYPE                pHDBDcp,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE   pEvent
)
{
    ...
    OHA_FUNCTION_TRACE_02(pHDBDcp->TraceIdx, LSA_TRACE_LEVEL_CHAT,
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
#if 0
LSA_VOID OHA_DcpImmediateEventAllUsers(
    OHA_HDB_DCP_PTR_TYPE                pHDBDcp,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE   pEvent
)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DcpIndicateEventAllOtherUsers(
    OHA_UPPER_RQB_PTR_TYPE              pRQB,
    LSA_UINT16                          NicId
)
{
    ...
    switch ( OHA_RQB_GET_OPCODE(pRQB))
    {
        ...
        case OHA_OPC_SET_EMERGENCY:
            ...
            Event.Type     = 0;
            Event.SubType  = 0;
            Event.RemaOptions = pRQB->args.Address.RemaOptions;
            Event.Count    = 0;
        ...
        case OHA_OPC_DCP_WRITE_ELEMENT:         /* fill the event-tag */
            ...
            Event.RemaOptions = (pRQB->args.DcpElem.Remanent == OHA_REMANENT_YES) ? OHA_CTRL_ANY_OTHER_OPTION : 0;
            Event.Count    = 1;
            break;
        case OHA_OPC_PRM_WRITE:
        {
            ...
            switch (pRQB->args.PrmData.record_index)
            {
                ...
                case OHA_RECORD_INDEX_DCP_NAME_OF_STATION:
                    ...
                    Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? OHA_CTRL_STATION_NAME : (LSA_UINT16)0;
                    break;
                ...
                case OHA_RECORD_INDEX_DCP_IP_SUITE:
                    ...
                    Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? OHA_CTRL_IP_SUITE : (LSA_UINT16)0;
                    break;
                case OHA_RECORD_INDEX_MULTIPLE_STATION:
                {
                    ...
                    Event.Type     = 0;
                    Event.SubType  = 0;
                    Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? Options : (LSA_UINT16)0;
                }
                break;
                case OHA_RECORD_INDEX_DHCP_PARAMS:
					Event.Options = OHA_CTRL_DHCP_CLIENT_ID + OHA_CTRL_DHCP_USAGE;
					Event.Count = 2;
                    Event.Type = DCP_DBID_DHCP;
                    Event.SubType = 61;
					Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? Event.Options : (LSA_UINT16)0;
                    break;
                ...
                case OHA_RECORD_INDEX_BUS_COMMON:
                    ...
                    Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? OHA_CTRL_PROFIBUS_COMMON : (LSA_UINT16)0;
                    break;
                case OHA_RECORD_INDEX_BUS_ADDITION:
                    Event.Options  = OHA_CTRL_PROFIBUS_ADDITION;
                    Event.Type     = DCP_DBID_PBPARAM;
                    Event.SubType  = DCP_DSID_PBADDITION;
                    Event.RemaOptions = (pRQB->args.PrmData.mk_rema == OHA_REMANENT_YES) ? OHA_CTRL_PROFIBUS_ADDITION : (LSA_UINT16)0;
                    break;
                default:
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                    break;
    ...
    {
        OHA_MEMCPY(&Event.SenderMACAddr, AddrPtr, (LSA_UINT32)Len);
    }
    else
    {
...
/*+               we queue the request.                                     +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestDcpResponse(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        ...
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_RESPONSE failed. No Lower Channel for this NicId (NIC-ID: 0x%X)",NICId);
        OHA_UserRequestFinish(pHDBUser, pRQB, Response);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
         ...
         (pHDBDcp->DcpState != OHA_HANDLE_STATE_OPEN) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_RESPONSE failed. No DCP-Channel opened or DCP-Indication present (NIC-ID: 0x%X)",NICId);
        OHA_UserRequestFinish(pHDBUser, pRQB, Response);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                              "OUT: OHA_RequestDcpResponse(Response: 0x%X)", Response);
        return;
    ...
    if (OHA_IS_NOT_NULL(pHDBUser->SockUserReq.pBottom))
    {
        ...
        OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_RESPONSE failed. Already one User-Request (Opcode=0x%X) active for this NicId (NIC-ID: 0x%X)",OHA_RQB_GET_OPCODE(pRQB),NICId);
        OHA_UserRequestFinish(pHDBUser, pRQB, Response);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                              "OUT: OHA_RequestDcpResponse(Response: 0x%X)", Response);
        return;
    ...
    if (indication->Event == OHA_MODE_NO_SET)
    {
        ...
        if (RetVal == OHA_OK)
        {
            ...
            OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_RequestDcpResponse. Negative default value for Reason code (0x%X) is set!", indication->Reason);
        }
    }

    OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,
    ...
    switch (indication->Event)
    {
        ...
        case OHA_MODE_EMERGENCY_SET:
        {
				 ...
				 (pHDBDcp->CheckSetInd.Options & OHA_CTRL_RESET_COMMUNICATION) )
			{
				...
				else
				{
					...
					OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN,"OHA_OPC_DCP_RESPONSE(ResetToFactory) setting of IP to 0.0.0.0 failed!");
        ...
        default:
            ...
            OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_RESPONSE failed. Option (0x%X) is invalid !", indication->Options);
            OHA_UserRequestFinish(pHDBUser, pRQB, Response);
            OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                                  "OUT: OHA_RequestDcpResponse(Response: 0x%X)", Response);
            return;
...
/*+  Result                :    LSA_VOID                                    +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_FinishDcpResponse(
    OHA_HDB_DCP_PTR_TYPE     pHDBDcp,
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    LSA_UINT16               Response)
{
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DoDcpResponseRequest(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    LSA_UINT16              RetVal;
    ...
    OHA_LOWER_TRACE_03(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: DCP-Request: OHA_OPC_RECV_IND (Handle: 0x%X,DCPHandle: 0x%X,pRQB: 0x%X)",pHDBDcp->ThisHandle,pHDBDcp->DcpHandle,pHDBDcp->pLowerDcpSetRqb);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestDcpControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    LSA_UINT16                Response = OHA_OK;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBEdd,  LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;  /* NICid is wrong */
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_CONTROL failed. No Lower Channel for this NicId (NIC-ID: 0x%X)",NICId);
    }
    else if ( LSA_HOST_PTR_ARE_EQUAL(pHDBDcp,  LSA_NULL))
    {
        Response = OHA_ERR_SEQUENCE;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_CONTROL failed. No EDD-Channel present (NIC-ID: 0x%X)",NICId);
    }
    else
    {
        ...
        switch (control->Mode)
        {
            case OHA_QUERY_WRITE:  /* only this user controls a DCP-Set ? */
                ...
                {
                    OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"This Server is already controlled by another User!");
                    Response = OHA_ERR_PARAM;
                    break;
                }
                else if (!pHDBDcp->pHDBUserCtrl)    /* store the control-user */
                {
                    ...
                    OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Control User-Set successful");
                }
            ...
            case OHA_NO_ACCESS:   /* no Dcp-GET and no Dcp-SET */
                ...
                OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Control Auto-Set successful");
            ...
            case OHA_UNLOCK_SET:  /* this user unlocks a DCP-Set */
				if (control->Options & OHA_CTRL_IP_SUITE) /* AP01335332. Lock IpAddress ? */
				{
					...
					if (OHA_IsAnyLocalSetActive(NICId))
					{
						...
						OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,
											  "OUT: OHA_RequestDcpControl queued (Mode: 0x%X)", control->Mode);
						return;
					}
				}
            ...
            case OHA_RESET_FILTER:
                ...
                {
                    OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_CONTROL failed. Already one User-Request active for this NicId (NIC-ID: 0x%X)",NICId);
                    Response = OHA_ERR_SEQUENCE;
                }
                else
                {
			...
			case OHA_SHOW_LOCATION:	/* LED flashing */
                ...
                OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Control Show-Location");
				break;
            case OHA_WRITE_ONLY:  /* the user controls a DCP-Get */
                OHA_PROGRAM_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Control OHA_NO_ACCESS or OHA_WRITE_ONLY not implemented");
                Response = OHA_ERR_NOT_IMPLEMENTED;
                break;
            default:
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP-Control failed.[0x%X]",OHA_DCP_RQB_GET_RESPONSE(pRQB));
    ...
    {
        OHA_UserRequestFinish(pHDBUser,pRQB,Response);
        OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,
...
OHA_HDB_DCP_PTR_TYPE OHA_DCPGetHandleByNIC(
    LSA_UINT16 NICId)
{
    OHA_HDB_DCP_PTR_TYPE  pHDB  =   OHA_GET_HDCP_PTR(0);
    LSA_UINT32            i     =   0;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID OHA_DCPUserCheckPending(
    OHA_HDB_DCP_PTR_TYPE    pHDBDcp,
    LSA_UINT16              Response)
{
    OHA_HDB_USER_PTR_TYPE     pHDBUser;
    ...
    if ( pHDBDcp->UserRQBCnt ) /* User-RCTRL-Blocks pending ? */
    {
	    ...
	    OHA_LOWER_TRACE_02(pHDBDcp->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,">>>: DCPUserCheckPending (NICId: 0x%X,UserRQBCnt: 0x%X)",
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DCPRequestHandler(
    OHA_HDB_DCP_PTR_TYPE            pHDBDcp,
    OHA_UPPER_RQB_PTR_TYPE          pRQBUser,
    LSA_UINT16                      Response)
{
    ...
    /* we have to wait for.                                                   */
    /* -----------------------------------------------------------------------*/
    OHA_PROGRAM_TRACE_03(pHDBDcp->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,
...
/*+               but not to a control-user.                                +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DcpIndicateEventExtUsers(
    LSA_UINT16                          NicId,
    OHA_UPPER_DCP_INDICATION_PTR_TYPE   pEvent
)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT16 OHA_DcpHelloEvent(
    OHA_HDB_USER_PTR_TYPE           pHDBUser,
...
/*+               but not to a control-user.                                +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DcpHelloEventAllUsers(
    OHA_UPPER_HELLO_PTR_TYPE        pEvent,
	LSA_UINT16                      NicId
)
{
