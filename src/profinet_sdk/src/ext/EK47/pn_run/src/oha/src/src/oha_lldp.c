...
static LSA_BOOL   OHA_LldpIsSinglePortRequestActive(
    const OHA_HDB_LLDP_TYPE * const pHDB)
{
	LSA_UINT16 PortId;
	...
	for(PortId = 1; PortId <= OHA_CFG_MAX_PORTS; PortId++)
	{
		...
		{
			OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"OHA_LldpIsSinglePortRequestActive (Handle: 0x%X,LLDPHandle: 0x%X, PortId 0x%X is active!)",pHDB->ThisHandle,pHDB->LldpHandle,PortId);
...
static LSA_BOOL oha_RemIndex_exists(LSA_UINT32 RemIndex) /* helper for ip2pn_RemIndex_alloc() */
{
	...
	for (inter_face = 1; inter_face <= OHA_CFG_MAX_NICS; ++inter_face)
	{
		...
		{
			LSA_UINT16 port_id;
			for (port_id = 0; port_id < pDB->PortCnt; ++port_id)
			{
				LSA_UINT16 sender_id;
				for (sender_id = 0; sender_id < LLDP_MAX_PORT_SENDER; ++sender_id)
				{
					OHA_LLDP_SENDER_PTR_TYPE pRemoteSender = &pDB->LldpData[port_id].RemoteSender[sender_id];

					if (pRemoteSender->SnmpSenderStatus == OHA_STATUS_DATA_VALID ||
						pRemoteSender->SnmpSenderStatus == OHA_STATUS_DATA_INVALID)
					{ /* "RemIndex" of sender valid */
						if (pRemoteSender->OidRemEntry.RemIndex == RemIndex)
						{
							return LSA_TRUE;
						}
					}
...
static LSA_UINT32 oha_RemIndex_alloc(LSA_VOID){ /* TIA 4814314 */

	do {
		++oha_database.NextRemIndex;
	} while ((oha_database.NextRemIndex == 0) || oha_RemIndex_exists(oha_database.NextRemIndex));

	return oha_database.NextRemIndex;
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_lldp_request_lower_done(
    ...
    LSA_UINT16                 Response)
{
    ...
    OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"<<<: OHA_EDDChannelDone done (NICId: 0x%X,Rsp: 0x%X,State: 0x%X)",NICId,Response,pHDB->State);
    ...
    switch (pHDB->State)
    {
        ...
        case OHA_HANDLE_STATE_OPEN_LLDP_RUNNING:
            if ( Response == OHA_OK)
            {
                ...
                if ( Response != OHA_OK)
                {
                    ...
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP Issuing GetConfigRequest failed.");

                    pHDB->State  = OHA_HANDLE_STATE_CLOSE_ERROR_LLDP_RUNNING;

                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-Closing Channel because of error.");

                    if ( OHA_LLDPCloseChannel(pHDB->LldpHandle,pHDB->Params.pSys) != OHA_OK)
                    {
                        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close LLDP Channel (Handle: 0x%X).",pHDB->ThisHandle);
        ...
        case OHA_HANDLE_STATE_GET_CONFIG_LLDP_RUNNING:
            if ( Response == OHA_OK)
            {
				...
				if ( Response != OHA_OK)
                {
                    ...
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP Issuing SetGlobalsRequest failed.");

                    pHDB->State  = OHA_HANDLE_STATE_CLOSE_ERROR_LLDP_RUNNING;

                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-Closing Channel because of set error.");
            ...
            else
            {
                ...
                if ( OHA_LLDPCloseChannel(pHDB->LldpHandle,pHDB->Params.pSys) != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Close LLDP Channel().");
                    OHA_FATAL(OHA_FATAL_ERR_LLDP);
                }
            }
            break;
        ...
        case OHA_HANDLE_STATE_STOP_LLDP_RUNNING:
            ...
            {
                /* is this a fatal error ?                      */
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Stop LLDP. Response: 0x%X", Response);
            }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_LLDPOpenChannel(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_LLDPOpenChannelDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_OPEN_CHANNEL done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));

    if ( OHA_LLDP_RQB_GET_RESPONSE(pRQB) == LLDP_RSP_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-Open Channel successful");
    ...
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-Open Channel failed.[0x%X]",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_LLDPCloseChannel(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_LLDPCloseChannelDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    LSA_UINT16               Response = OHA_OK;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_CLOSE_CHANNEL done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));

    if ( OHA_LLDP_RQB_GET_RESPONSE(pRQB) == LLDP_RSP_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-Close Channel successful");
    }
    else
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-Close Channel failed.[0x%X]",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_LLDP_PTR_TYPE OHA_LLDPGetHandleByNIC(
    LSA_UINT16 NICId)
{
    ...
    LSA_UINT16 IfId)
{
    ...
    if (IfId >= EDD_INTERFACE_ID_MIN && IfId <= EDD_INTERFACE_ID_MAX) /* 0 is not valid */
    {
		...
		LSA_UINT16 ret = OHA_TOPOGetNicByInterface(IfId, &NicID);
		...
		if(ret == LSA_RET_OK)
		{
			...
			if(OHA_IS_NULL(pHDB))
			{
				OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_LLDPGetHandleByIf(pHDB: 0x%X, NicID: 0x%X)", pHDB, NicID);
			}
		}
	}
...
/*+                     within LLDP!                                        +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_LLDPRecvProvide(
    OHA_HDB_LLDP_PTR_TYPE    pHDB,
    LSA_UINT32               Cnt)
{
    OHA_LLDP_LOWER_RQB_PTR_TYPE pRQB = LSA_NULL;
    ...
    while ((Status == OHA_OK) && (Cnt))
    {
        ...
        {
            Status = OHA_ERR_RESOURCE;
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvProvide(). Allocation TLV memory failed.");
        }
        else
        {
            /* allocate RQB */
            ...
            if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
            {
                ...
                OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_INDICATION_PROVIDE (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
            ...
            else
            {
                ...
                Status = OHA_ERR_RESOURCE;
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-Allocating lower RQB-memory failed!");
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT8 OHA_LLDPGetSenderStatus(
    OHA_HDB_LLDP_PTR_TYPE                       pHDB,
    ...
    LSA_UINT8                                   OhaSenderStatus)
{
    ...
    OHA_FUNCTION_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT,
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT,
                          "OUT: OHA_LLDPGetSenderStatus(SenderStatus: 0x%X)",SenderStatus);
    return(SenderStatus);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_UINT16 OHA_LLDPGetSenderID(
    OHA_HDB_LLDP_PTR_TYPE       pHDB,
    LSA_UINT16                  PORTId)
{
    ...
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : OHA_LLDPGetSenderID(pHDB: 0x%X,PORTId: 0x%X)",
                          pHDB,PORTId);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_IsEqualAliasName(
    LSA_UINT16                  NICId,
    LSA_UINT16                  PortID,
    LSA_UINT16                  SenderId,
	LLDP_TLV_GET_INFO_TYPE *pTLVInfo
)
{
	...
	{
		OHA_PROGRAM_TRACE_04(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE, "~~~No (valid) actual alias present PORTId: 0x%X, SenderId: 0x%X, PortID-Subtype: 0x%X,, ChassisID-Subtype: 0x%X",
    ...
    if (SenderStatus == OHA_STATUS_DATA_VALID || SenderStatus == OHA_STATUS_DATA_INVALID)
    {
        ...
        if (ChassisLen != 0 && OHA_IS_NOT_NULL(ChassisPtr))
        {
			...
			if (PortLen != 0 && OHA_IS_NOT_NULL(PortPtr))
			{
				...
				{
					OHA_PROGRAM_TRACE_03(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE, "~~~No (valid) previous alias present PORTId: 0x%X, SenderId: 0x%X, ChassisID-Subtype: 0x%X",
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_BOOL OHA_IsValidNeighbour(
	LLDP_TLV_GET_INFO_TYPE *pTLVInfo,
...
)
{
	...
	if (OHA_ERR_PARAM == oha_check_port_name(PortLen, (OHA_COMMON_MEM_U8_PTR_TYPE)(&pTLVInfo->PortIDString)) ? LSA_TRUE : LSA_FALSE)
	{
	...
	if (OHA_ERR_PARAM == oha_check_station_name(ChassisLen, pChassis,
											allow_upper_case ? OHA_ALLOW_UPPER_CASE_YES : OHA_ALLOW_UPPER_CASE_NO,
											allow_upper_case ? OHA_NAME_INTERPRETATION_YES : OHA_NAME_INTERPRETATION_NO) ? LSA_TRUE : LSA_FALSE)
	{
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_INDICATION_PROVIDE done (Handle: 0x%X,Rsp: 0x%X,State: 0x%X)",
    ...
    if ( pHDB->State == OHA_HANDLE_STATE_OPEN )
    {
        switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
        {
                ...
                if (pRecv->PORTId && (pRecv->PORTId <= pHDB->pDB->PortCnt)) /* pHDB->PortCnt */
                {
                                    ...
                                    if (pRecv->Event == LLDP_EVENT_DATA_VALID)
                                    {
                                        ...
                                        if ( Resp == LLDP_RSP_OK)
                                        {
                                            ...
                                            if (TLVInfo.ChassisIDLen <= OHA_MAX_CHASSIS_STRLEN)
                                            {
                                                ...
                                                {
                                                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvDone: oha_db_write_rem_lldp_element(ChassisID) failed (RetVal: 0x%X).", RetVal);
                                                }
                                            }
                                            ...
                                            if (TLVInfo.PortIDLen <= OHA_MAX_PORT_STRLEN)
                                            {
                                                ...
                                                if (RetVal != LSA_RET_OK)
                                                {
                                                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvDone: oha_db_write_rem_lldp_element(PortID) failed (RetVal: 0x%X).", RetVal);
                                                }
                                            }
                                            ...
                                            if (RetVal != LSA_RET_OK)
                                            {
                                                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvDone: oha_db_write_rem_lldp_element(TimeToLive) failed (RetVal: 0x%X).", RetVal);
                                            }
                                            if (ValidNeighbour)
                                            {   /* check sum of the strings <= 255 */
                                                if ((TLVInfo.ChassisIDLen + TLVInfo.PortIDLen) <= OHA_MAX_CHASSIS_STRLEN)
                                                {                                                     /* the 3rd (last) mandatory Option   */
                                                    SenderStatus = OHA_STATUS_DATA_VALID;  /* entry of data in the database */
                                                }
                                            }

                                            if (pRemoteSender->SnmpSenderStatus == 0  /* initial value */ ||
                                                pRemoteSender->SnmpSenderStatus == OHA_STATUS_NOT_PRESENT) /* new connection */
                                            {
												pRemoteSender->OidRemEntry.RemIndex = oha_RemIndex_alloc();
...

                                            {
                                                ...
                                                if (Resp != LSA_RET_OK)
                                                {
                                                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvDone: OhaLldpGetParams failed (Resp: 0x%X).", Resp);
                                                }
                                            }
                                        }
                                    }
                    ...
                    if (pRemoteSender)
                    {
							...
							(!IsEqualAliasName))
						{
							...
							if (RetVal != OHA_OK)
							{
								OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_WARN, "OUT: OHA_LLDPRecvDone - DCPUpdateFilter(RetVal: 0x%X)", RetVal);
											  ...
											  pHDB->RequestState == OHA_LLDP_STATE_WF_START))  /* AP01070629, AP01137963 */
                        {
                            ...
                            if (RetVal != LSA_RET_OK)
                            {
                                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPRecvDone: OHA_LLDPSet() failed (RetVal: 0x%X).", RetVal);
                            }
                        }
                    }
            ...
            case LLDP_RSP_OK_CANCEL:
                ...
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"LLDP Receive canceled (0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
                Reprovide = LSA_FALSE;
                break;

            default:
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"LLDP-ERROR: LLDP_OPC_INDICATION_PROVIDE failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
                OHA_FATAL(OHA_FATAL_ERR_LLDP);
                break;
        }
    }
    ...
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"LLDP Receive while not in open-state. discard and dont reprovide.");
        Reprovide = LSA_FALSE;
    }
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_GET_CONFIG (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
        OHA_LLDP_REQUEST_LOWER(pRQB,pSys);
    }
    else
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"LLDP-Allocating RQB-memory failed!");
        Status = OHA_ERR_RESOURCE;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_LLDPGetConfigDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_GET_CONFIG done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
    {
        case LLDP_RSP_OK:
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"PortCnt from LLDP (0x%X) > OHA_CFG_MAX_PORTS.",pRQB->args.GetConfig.PORTCnt);
            ...
            if (Response == OHA_OK)
            {
                ...
                OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-GET CONFIG PortCnt: %d PORTMaxSender: %d",pDB->PortCnt,pDB->PortMaxSender);
            }
        }
        break;

        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-ERROR: GET CONFIG failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
...
)
{
    ...
    if (TxInterval < OHA_LLDP_TX_INTERVAL_MIN || TxInterval > OHA_LLDP_TX_INTERVAL_MAX)  /* TxInterval (5..32768 seconds) */
	{
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"TxInterval (0x%X) must be 5..32768 seconds!", TxInterval);
		OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT,
	...
	{
		Status = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"TxHoldMultiplier (0x%X) must be 2..10 seconds!", TxHoldMultiplier);
		OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT,
			                  "OUT: OHA_LLDPSetGlobals(Status: 0x%X)",Status);
	    return(Status);
	}

	/* Time To Live (2 0ctets) = min(65535,(lldpMessageTxInterval * lldpMessageTxHoldMultiplier)) */
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_SET_GLOBALS (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
    ...
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"LLDP-Allocating RQB-memory for global set failed!");
        Status = OHA_ERR_RESOURCE;
        OHA_FATAL(OHA_FATAL_ERR_NULL_PTR);
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_LLDPSetGlobalsDone(
    OHA_LLDP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    LSA_UINT16      Orginator;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_SET_GLOBALS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));
    OHA_ASSERT_NULL_PTR(pHDB->pDB);
    ...
    switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
    {
        case LLDP_RSP_OK:
        {
            ...
            /* Successfully done the request.               */
            /* ---------------------------------------------*/
            OHA_PROGRAM_TRACE_04(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"LLDP-SET GLOBALS done, NICId: 0x%X, TxInterval: %d, ReInitDelay: %d, TxDelay: %d",
			...
			/* write the TTL to the local LLDP DB */
			{
				...
				LSA_UINT32 TimeToLive =  pRQB->args.SetGlobals.MsgTxInterval * pHDB->pDB->TempTxHoldMultiplier;
				...
				if (RetVal != LSA_RET_OK)
				{
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSetGlobalsDone: LLDP_OPC_SET_GLOBALS(TTL) failed (RetVal: 0x%X).", RetVal);
				...
				if (RetVal != LSA_RET_OK)
				{
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSetGlobalsDone: LLDP_OPC_SET_GLOBALS(UpdateSend) failed (RetVal: 0x%X).", RetVal);
					Response = OHA_ERR_PARAM;
				}
			}
        }
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-ERROR: SET GLOBALS failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
    ...
    switch (OHA_HANDLE_GET_TYPE(Orginator))
    {
        ...
        case OHA_HANDLE_TYPE_LLDP:   /* ResetToFactory: no further action */
        break;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestLldpIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    ...
    LSA_UINT16                   Orginator)
{
    ...
    else
    {
		switch (Orginator) /* AP01001236 */
		{
			...
			case OHA_HANDLE_TYPE_DCP:
				...
				else /* Set(s) already active, do not update */
				{
                    ...
                    OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA_LLDPSet already pending (PortId: 0x%X, PendingCnt: 0x%X).", PortId, pHDB->Set_PendingCnt[PortId-1]);
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
        {
            ...
            if ( ! LSA_HOST_PTR_ARE_EQUAL(MemPtr, LSA_NULL) )
            {
                    ...
                    TLVSetInfo.PortIDLen    && (!LSA_HOST_PTR_ARE_EQUAL(TLVSetInfo.pPortIDString, LSA_NULL)))
                {
                    ...
                    if ( Response == LSA_RET_OK)
                    {
						...
						if(TxEnable & OHA_LLDP_PORT_DESC_TX)
						{
							...
							if ((!LSA_HOST_PTR_ARE_EQUAL(pVar, LSA_NULL)) && Len && Len <= LLDP_TLV_PORT_DESCR_MAX_SIZE)
							{
								...
								{
									OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet: lldp_TLVSetPortDescr() failed (RetVal: 0x%X).", RetVal);
								}
							}
						}
						...
						if(TxEnable & OHA_LLDP_SYS_NAME_TX)
						{
							...
							if ((!LSA_HOST_PTR_ARE_EQUAL(pVar, LSA_NULL)) && Len && Len <= LLDP_TLV_SYSTEM_NAME_MAX_SIZE)
							{
								...
								if (RetVal != LLDP_RSP_OK)
								{
									OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet: lldp_TLVSetSystemName() failed (RetVal: 0x%X).", RetVal);
								}
							}
						}
						...
						if(TxEnable & OHA_LLDP_SYS_DESC_TX)
						{
							...
							if ((!LSA_HOST_PTR_ARE_EQUAL(pVar, LSA_NULL)) && Len && Len <= LLDP_TLV_SYSTEM_DESCR_MAX_SIZE)
							{
								...
								if (RetVal != LLDP_RSP_OK)
								{
									OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet: lldp_TLVSetSystemDescr() failed (RetVal: 0x%X).", RetVal);
								}
							}
						}
						...
						if(TxEnable & OHA_LLDP_SYS_CAP_TX)
						{
							...
							/* set the System Capabilities for the lldp send request */
							{
								...
								if (RetVal != LLDP_RSP_OK)
								{
									OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet: lldp_TLVSetSystemCap() failed (RetVal: 0x%X).", RetVal);
								}
							}
						}
						...
						if(TxEnable == OHA_LLDP_TX_ENABLE)
						{
							...
							if ((!LSA_HOST_PTR_ARE_EQUAL(pVar, LSA_NULL)) && (Len == OHA_IP_ADDR_SIZE))
							{
								...
								if(TLVParams.MgmtAddrStringLen)	/* mngmt address written ? */
								{
									...
									if (RetVal != LLDP_RSP_OK)
									{
										OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet: lldp_TLVSetMgmAddr() failed (RetVal: 0x%X).", RetVal);
									}
								}
							}
						}
                        ...
                        if (RetVal != LSA_RET_OK)
                        {
                            OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,
                    ...
                    if ( Response == LSA_RET_OK)
                    {
                        ...
                        OHA_LOWER_TRACE_04(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH
            ...
            {
                OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSet(). Allocation TLV memory failed.");
                OHA_LLDP_FREE_LOWER_RQB(&RetVal, pRQB, pSys);
                OHA_MEM_FREE_FAIL_CHECK(RetVal);
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_SET done (Handle: 0x%X,Rsp: 0x%X, PortId: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB),PortId);
	...
	if(!pHDB->Set_IsActual[PortId-1]) /* AP01001236 */
	{
		if(pHDB->Set_PendingCnt[PortId-1] == 0) /* update now */
		{
				...
				pHDB->RequestState == OHA_LLDP_STATE_WF_START)  /* AP01070629, AP01137963 */
			{
				...
				if (RetVal != LSA_RET_OK)
				{
					OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPSetDone: OHA_LLDPSet failed (RetVal: 0x%X).", RetVal);
				}
			}
		}
	}
    ...
    LSA_UINT16                   Orginator)
{
    ...
    for (i = 0; i < pHDBLldp->pDB->PortCnt; i++)
    {
        ...
        LinkState = OHA_GetLinkStatus(pHDBEdd->LinkStatus[i].LinkStatus,
                                      pHDBEdd->LinkStatus[i].PhyStatus,
                                      pHDBEdd->LinkStatus[i].PortStatus);
        ...
        if (SendMode == OHA_LLDP_TX_ENABLE)
        {
            ...
            if (LinkState == OHA_PORT_LINK_STATE_UP)
            {
                ...
                if (RetVal != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPStart: OHA_LLDPStart() failed (RetVal: 0x%X).", RetVal);
                    break;
                }
        ...
        else    /* only LLDP-SET (passivate lldp send) */
        {
            ...
            if (RetVal != LSA_RET_OK)
            {
                OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPStart: OHA_LLDPSet() failed (RetVal: 0x%X).", RetVal);
            }
    ...
    OHA_FUNCTION_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_CHAT,
    ...
    LSA_UINT16                   Orginator)
{
    ...
    if (PortId == 0 || PortId > OHA_CFG_MAX_PORTS)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"OHA_LLDPStart(): Invalid PortId (%Xh)",PortId);
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        {
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_START (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
            pHDB->StartStopActive[PortId] = LSA_TRUE;
            OHA_LLDP_REQUEST_LOWER(pRQB,pSys);
        }
        else    /* no send */
        {
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Queue: LLDP_OPC_START (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_SET done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-ERROR: LLDP SET failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
    ...
    {
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_START (Handle: 0x%X,LLDPHandle: 0x%X,pBottomRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pBottomRQB);
    ...
    else
    {
		...
		if (!OHA_LldpIsSinglePortRequestActive(pHDB))
		{
			...
			{
				OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,">>>: LLDP-Request: LLDP_OPC_STOP-AllPorts (Handle: 0x%X,LLDPHandle: 0x%X,pBottomRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pBottomRQB);
				OHA_LLDP_REQUEST_LOWER(pBottomRQB, pHDB->Params.pSys);
			}
		}
    }
    ...
    LSA_UINT16                   Orginator)
{
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        if (PortId == LLDP_PORT_ID_ALL)
        {
			...
			{
				OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,">>>: LLDP-Queue: LLDP_OPC_STOP-AllPorts (Handle: 0x%X,LLDPHandle: 0x%X, pRQB 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
        ...
        if (!pHDB->StartStopActive[PortId]) /* request is inactive ? */
        {
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_STOP (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
            pHDB->StartStopActive[PortId] = LSA_TRUE;
            OHA_LLDP_REQUEST_LOWER(pRQB,pSys);
        }
        else    /* no send */
        {
            OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Queue: LLDP_OPC_STOP (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
            OHA_RQB_PUT_BLOCK_TOP(pHDB->LldpRQBs[PortId].pBottom,
                                  pHDB->LldpRQBs[PortId].pTop,
                                  pRQB);
        }
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_STOP done (Handle: 0x%X,Rsp: 0x%X, Orginator: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB),Orginator);
    ...
    switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-ERROR: LLDP_OPC_STOP failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
    ...
    if (OHA_IS_NOT_NULL(pBottomRQB))
    {
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_STOP (Handle: 0x%X,LLDPHandle: 0x%X,pBottomRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pBottomRQB);
        OHA_LLDP_REQUEST_LOWER(pBottomRQB, pHDB->Params.pSys);
    }
    else
    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_LLDPUpdateSendGlobal(LSA_VOID)
{
	LSA_UINT16  i;
		...
		if(OHA_IS_NOT_NULL(pHDBLldp))
		{
			...
			if(OHA_IS_NOT_NULL(pDetail))
			{
...

				{
                	...
                	if (RetVal != OHA_OK)
                	{
                    	OHA_FUNCTION_TRACE_02(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OHA_LLDPUpdateSendGlobal - LLDPUpdateSend(RetVal: 0x%X, NicID: 0x%X)", RetVal, pDetail->Params.Lldp.NICId);
                	}
            	}
			}
		}
...
)
{
    ...
    /* check parameters */
    OHA_ASSERT_NULL_PTR(pRemoteChanges);
                    ...
                    switch (TLVParams.Type)
                    {
                        ...
                        case LLDP_PARAM_TYPE_PORT_DESCR:
                                 ...
                                 TLVParams.Params.PortDescr.PortDescriptionLen && TLVParams.Params.PortDescr.PortDescriptionLen <= LLDP_TLV_PORT_DESCR_MAX_SIZE)
                            {
								...
								{
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PortDescription) failed (RetVal: 0x%X).", NicId, RetVal);
                                }
                            }
                            break;
                        case LLDP_PARAM_TYPE_SYSTEM_NAME:
                                 ...
                                 TLVParams.Params.SystemName.SystemNameLen && TLVParams.Params.SystemName.SystemNameLen <= LLDP_TLV_SYSTEM_NAME_MAX_SIZE)
                            {
                                ...
                                {
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(SystemName) failed (RetVal: 0x%X).", NicId, RetVal);
                                }
                            }
                            break;
                        case LLDP_PARAM_TYPE_SYSTEM_DESCR:
                                 ...
                                 TLVParams.Params.SystemDescr.SystemDescrLen && TLVParams.Params.SystemDescr.SystemDescrLen <= LLDP_TLV_PORT_DESCR_MAX_SIZE)
                            {
                                ...
                                {
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(SystemDescription) failed (RetVal: 0x%X).", NicId, RetVal);
                                }
                            }
                            break;
                        case LLDP_PARAM_TYPE_SYSTEM_CAP:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = SysCap;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == (LLDP_TL_SIZE + LLDP_TLV_SYSTEM_CAP_SIZE))
                                {
                                    ...
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParam(NicId: 0x%X)s: oha_db_write_rem_lldp_element(SystemCapabilities) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetSystemCap() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_MGM_ADDR:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = MgmAddr;
                            LSA_UINT32  Len = 0;

                            // Added for TEDCheck CISCO.
                           if(TLVParams.Params.MgmAddr.MgmtAddrSubtype != OHA_ADDR_FAMILY_IPV4)
                           {
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len > LLDP_TL_SIZE)
                                {
                                    ...
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(MgmAddress) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetMgmAddr() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        ...
                        case LLDP_PARAM_TYPE_IEEE_802_1_PORT_VLAN_ID:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = VlanID;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_1_PORT_VLAN_ID_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PortVLANID) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetPortVLANID() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_1_PPVID:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = PpvID;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_1_PPVID_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PPVID) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetPPVID() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_1_VLAN_NAME:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = VLanName;
                            LSA_UINT32  Len = 0;
                                ...
                                TLVParams.Params.VLANName.VLANNameLength <= LLDP_TLV_IEEE_802_1_PORT_VLAN_NAME_MAX_VLAN_NAME_LEN)
                            {
                                ...
                                {   /* LLDP adds the TL-header */
                                    if (Len > LLDP_TL_SIZE + LLDP_TLV_ORG_SPECIFIC_MIN_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(VLANName) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }
                                else
                                {
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetVLANName() failed (RetVal: 0x%X).", NicId, RetVal);
                                }
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_1_PROTOCOL_IDENTITY:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = ProtocolID;
                            LSA_UINT32  Len = 0;
                            ...
                            if (TLVParams.Params.ProtIdentity.Length)
                            {
                                ...
                                {   /* LLDP adds the TL-header */
                                    if (Len > LLDP_TL_SIZE + LLDP_TLV_ORG_SPECIFIC_MIN_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(ProtIdentity) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }
                                else
                                {
                                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetProtIdentity() failed (RetVal: 0x%X).", NicId, RetVal);
                                }
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_3_MAC_PHY_CFG_STATUS:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = MacPhyCfgStatus;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_3_MAC_PHY_CFG_STATUS_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(MACPHYConfigStatus) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetMACPHYConfigStatus() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_3_POWER_VIA_MDI:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = PowerViaMDI;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_3_POWER_VIA_MDI_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PowerViaMDI) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetPowerViaMDI() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_3_LINK_AGGREGATION:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = LinkAggregation;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_3_LINK_AGGREGATION_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(LinkAggregation) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetLinkAgg() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_IEEE_802_3_MAX_FRAME_SIZE:
                        {
                            ...
                            LLDP_UPPER_MEM_PTR_TYPE pVar = MaxFrameSize;
                            LSA_UINT32  Len = 0;
                            ...
                            {   /* LLDP adds the TL-header */
                                if (Len == LLDP_TL_SIZE + LLDP_TLV_IEEE_802_3_MAX_FRAME_SIZE_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(MaxFrameSize) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): lldp_TLVSetMaxFrameSize() failed (RetVal: 0x%X).", NicId, RetVal);
                            }
                        }
                        break;
                        case LLDP_PARAM_TYPE_ORG_SPECIFIC:
                        {
                                ...
                                (Oui == OHA_LLDP_TYPE_ORG_OUI_PNO_3))   /* receive "03-0e-cf"as valid PNIO_OUI "00-0e-cf" */
                            {
                                ...
                                if (TLVParams.Params.OrgUnknown.Subtype == OHA_LLDP_SUB_TYPE_PNO_DELAY)
                                {
                                         ...
                                         TLVParams.Params.OrgUnknown.GroupDefInfoStringLen == OHA_LLDP_PNO_DELAY_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PortDelays) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }           /* port status */
                                else if (TLVParams.Params.OrgUnknown.Subtype == OHA_LLDP_SUB_TYPE_PNO_PORT_STATUS)
                                {
                                         ...
                                         TLVParams.Params.OrgUnknown.GroupDefInfoStringLen == OHA_LLDP_PNO_PORT_STATUS_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PortStatus) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }           /* MRT port status */
                                ...
                                else if (TLVParams.Params.OrgUnknown.Subtype == OHA_LLDP_SUB_TYPE_PNO_CHASSIS_MAC_ADR)
                                {
                                         ...
                                         TLVParams.Params.OrgUnknown.GroupDefInfoStringLen == OHA_LLDP_PNO_CHASSIS_MAC_ADR_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(ChassisMacAddr) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }           /* PTCP status */
                                else if (TLVParams.Params.OrgUnknown.Subtype == OHA_LLDP_SUB_TYPE_PNO_PTCP_STATUS)
                                {
                                         ...
                                         TLVParams.Params.OrgUnknown.GroupDefInfoStringLen == OHA_LLDP_PNO_PTCP_STATUS_SIZE)
                                    {
                                        ...
                                        if (RetVal != LSA_RET_OK)
                                        {
                                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(PtcpStatus) failed (RetVal: 0x%X).", NicId, RetVal);
                                        }
                                    }
                                }
                            ...
                            else    /* organizationally defined TLV's (OUI) */
                            {
                                     ...
                                     (TLVParams.Params.OrgUnknown.GroupDefInfoStringLen <= LLDP_TLV_ORG_SPECIFIC_MAX_GROUP_DEFINED_ISTRING_SIZE))
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(OrgUnknown) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                                }
                            }
                        }
                        ...
                        case LLDP_PARAM_TYPE_UNKNOWN_OPTIONAL:
                        {
                                ...
                                TLVParams.Params.UnknownOptional.Type < LLDP_TLV_TYPE_ORG_SPECIFIC) /* Type 9 ...126, unknown optional TLV's */
                            {
                                     ...
                                     TLVParams.Params.UnknownOptional.ValueLen <= LLDP_LEN_MAX_SIZE)
                                {
                                    ...
                                    if (RetVal != LSA_RET_OK)
                                    {
                                        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): oha_db_write_rem_lldp_element(UnknownOptional) failed (RetVal: 0x%X).", NicId, RetVal);
                                    }
                            ...
                            {
                                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): Invalid Type occurred, (Type = %d) !", NicId,
                                                     TLVParams.Params.UnknownOptional.Type);
                            }
                        }
                        break;
                        ...
                        default:    /* continue */
                            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): Invalid ParamsType occurred, (TLVParams.Type = %d) !", NicId, TLVParams.Type);
                            break;
                    }
                }
            }
                     ...
                     Resp == LLDP_RSP_ERR_TLV_PARAM_ERROR)
            {
                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"OhaLldpGetParams(NicId: 0x%X): TlvUnknown or TlyParamError occurred, (Resp = %d) !", NicId, Response);
            }
            else    /* invalid frame */
            {
    ...
    LSA_UINT16                   Orginator)
{
    ...
    {
        Status = OHA_ERR_PARAM;
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_LLDPGetStats(Status: 0x%X)",Status);
        return(Status);
    }
    ...
    if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
    {
        ...
        pRQB->args.Stats.Reset     = 0;

        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: LLDP-Request: LLDP_OPC_STATS (Handle: 0x%X,LLDPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->LldpHandle,pRQB);
    ...
    OHA_HDB_LLDP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: LLDP_OPC_STATS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_LLDP_RQB_GET_RESPONSE(pRQB));
    ...
    switch (OHA_LLDP_RQB_GET_RESPONSE(pRQB))
    {
        ...
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"LLDP-ERROR: GET STATISTICS failed.(0x%X).",OHA_LLDP_RQB_GET_RESPONSE(pRQB));
            Response = OHA_ERR_LL;
            break;
    }
 ...
 *===========================================================================*/
static LSA_BOOL OHA_LLDPCompareGlobals(
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_LLDPUpdateGlobals(
    LSA_UINT16                   NicID,
	LSA_UINT32                   TmpSetOptions,
    LSA_UINT16                   Orginator)
{
    ...
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_LLDPUpdateGlobals(0x%X)", RetVal);
    return(RetVal);
}
 ...
 *========================================================================================*/
static LSA_UINT16 OHA_LLDPUpdatePorts(
    LSA_UINT16                   NicID,
    LSA_UINT16                   PortID,
    LSA_UINT16                   Orginator)
{
	...
	OHA_PROGRAM_TRACE_02(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"OHA_LLDPUpdatePorts (NewAdminStatus: 0x%X, OldAdminStatus: 0x%X).", NewAdminStatus, OldAdminStatus);
	...
	if(NewAdminStatus != OldAdminStatus)	/* otherwise no action */
	{
		switch(NewAdminStatus)
		{
			...
			break;
			default:
				OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"MIB_DB-ERROR: oha_db_read_temp_lldp_mib_value failed.(AdminStatus: 0x%X).",RetVal);
				RetVal = OHA_ERR_FAILED;
			break;
		}
	}

	OHA_PROGRAM_TRACE_03(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"OHA_LLDPUpdatePorts (RequestState: 0x%X, StartAdminStatus: 0x%X, StopdminStatus: 0x%X).", pHDBLldp->RequestState, StartAdminStatus, StopAdminStatus);
	...
	if(StopAdminStatus)
	{
		...
		if (RetVal != OHA_OK)
		{
			OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPUpdatePorts: OHA_LLDPStop failed (Status: 0x%X).", RetVal);
		}
		else
		{
	...
	if(StartAdminStatus)
	{
		...
		if (RetVal != OHA_OK)
		{
			OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPUpdatePorts: OHA_LLDPSart failed (RetVal: 0x%X).", RetVal);
		}
		else
		{
			RetVal = OHA_OK_ACTIVE;
    ...
    OHA_FUNCTION_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_LLDPUpdatePorts(0x%X)", RetVal);
    return(RetVal);
}
 ...
 *===========================================================================*/
LSA_VOID OHA_LLDPSetMibOptions(
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
static LSA_UINT16 OHA_LLDPSetMibPortOptions(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
	...
	LSA_BOOL					IsRema)
{
    ...
    if (PortId == 0 || (PortId > OHA_CFG_MAX_PORTS))
    {
        ...
        OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_LLDPSetMibPortOptions(Invalid PortId:  0x%X)", PortId);
        return (RetVal);
    }
	...
	{
		pPortConfig->TLVsTxEnable.Value = pPortConfig->TLVsTxEnable.TmpOctStr[0];
	...
	{
		pPortConfig->Dot3TLVsTxEnable.Value = pPortConfig->Dot3TLVsTxEnable.TmpOctStr[0];
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
static LSA_BOOL OHA_LLDPComparePorts(
    LSA_UINT16                  NicId,
    LSA_UINT16                  PortId,
	LSA_UINT32                  PortSetOptions,
    LSA_BOOL                    MakeRema,
    LSA_BOOL                    CheckRema)
{
	...
	if (PortSetOptions & OHA_CTRL_LLDP_TLVS) /* lldpPortConfigTLVsTxEnable  */
	{
		if(pPortConfig->TLVsTxEnable.Value != pPortConfig->TLVsTxEnable.TmpOctStr[0])
		{
			return(IsDifferent = LSA_TRUE);
		}
	...
	if (PortSetOptions & OHA_CTRL_DOT3_TLVS) /* lldpXdot3PortConfigTLVsTxEnable */
	{
		if(pPortConfig->Dot3TLVsTxEnable.Value != pPortConfig->Dot3TLVsTxEnable.TmpOctStr[0])
		{
			return(IsDifferent = LSA_TRUE);
		}
    ...
    OHA_FUNCTION_TRACE_01(pDbIf->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_LLDPComparePorts(0x%X)", IsDifferent);
    return(IsDifferent);
}
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_LLDPPrepareStats(
    LSA_UINT16                   Orginator,
				...
				case OHA_SNMP_SET_REQ:
				{
					...
					if(GlobOptions & OHA_CTRL_LLDP_SET_GLOBALS) /* async call to do ? */
					{
						...
						if (OHA_LLDPCompareGlobals(GlobOptions & OHA_CTRL_LLDP_SET_GLOBALS, IsRema, LSA_FALSE))
						{
							...
							{
								OHA_FUNCTION_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_LLDPDoStats: OHA_LLDPUpdateGlobals failed (RetVal 0x%X)", RetVal);
								break;
							}
						}
					}
							...
							if (OHA_LLDPComparePorts(NicID, (LSA_UINT16)(ports+1), PortOptions, IsRema, LSA_FALSE))
							{
								...
								if(PortOptions & OHA_CTRL_ADMIN_STATUS) /* execute first */
								{
									...
									if(pHDBLldp->RequestState == OHA_LLDP_STATE_START) /* LLDP-Start is possible ? */
									{
										...
										{
											OHA_FUNCTION_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "OHA_LLDPDoStats: OHA_LLDPUpdatePorts failed (RetVal 0x%X)", RetVal);
											break;
										}
									}
								}
							...
							if (OHA_LLDPComparePorts(NicID, (LSA_UINT16)(ports+1), PortOptions, IsRema, LSA_TRUE))
							{
								...
								if(PortOptions & OHA_CTRL_LLDP_TX_PORTS) /* TxEnable's: LldpSet(ThisPort) */
								{
										...
										pHDBLldp->RequestState == OHA_LLDP_STATE_RE_START) /* LLDP-Set is possible ? */
									{
										...
										if (RetVal != LSA_RET_OK)
										{
											OHA_PROGRAM_TRACE_01(pHDBLldp->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_LLDPDoStats: OHA_LLDPSet() failed (RetVal: 0x%X).", RetVal);
										}
									}
								}
							}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_LLDPUpdateStats(
...
)
{
        ...
        SenderId > LLDP_MAX_PORT_SENDER)
    {
        ...
        OHA_FUNCTION_TRACE_01(pDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_LLDPUpdateStats(Status: 0x%X)", Status);
        return(Status);
    }
 ...
 * return value:   LSA_UINT16:      LSA_RET_OK or LSA_RET_ERR_PARAM
 *===========================================================================*/
LSA_UINT16 OHA_LLDPSetTempIfOptions(
    LSA_UINT16                  NicId,
	LSA_UINT32                  IfSetOptions)
{
    OHA_DB_IF_PTR_TYPE           pDbIf  = OHA_DB_GET_INTERFACE(NicId);
