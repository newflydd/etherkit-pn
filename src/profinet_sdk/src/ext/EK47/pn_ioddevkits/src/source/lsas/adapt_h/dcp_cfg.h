...
/*  2011-07-06  S05.01.00.00_00.03.03.02 lrg: AP01017495 Index-Trace         */
...
/********************************
 * Client functions are compiled
 * if defined: DCP_CLIENT_MODE
*/

#define DCP_CLIENT_MODE

/**************************************
 * Server functions are compiled
 * if defined: DCP_SERVER_MODE
*/
#define DCP_SERVER_MODE

/* DCP-Dataset IDs (Suboptions) - missing in dcp_usr.h
*/
#define DCP_DSID_RESETFACT  5       /* CONTROL: Reset to factory*/


/*---------------------------------------------------------------------------*/
/* Advanced protection and function handling settings.                       */
/*                                                                           */
/* DCP is designed to use the input functions dcp_timeout() without          */
/* DCP_ENTER() and DCP_EXIT() even if called from a priority above standard  */
/* handling 1). The requirement for this is, that both functions can not be  */
...
/* DCP_DO_TIMEOUT_EXTERN is the more general case, if a context-change for   */
/* timeout handling is necessary.                                            */

/* DCP_DO_TIMEOUT_EXTERN:                                                    */
/*      undef:   DCP directly calls the extern input function dcp_timeout()  */
/*               and does the neccessary standard handling within            */
/*               dcp_timeout(). This can be set if no                        */
/*               context change is neccessary or the function is protected   */
/*               with DCP_ENTER() and DCP_EXIT().                            */
/*      defined: DCP uses DCP_DO_TIMER_REQUEST within dcp_timeout_sys() to   */
/*               do a context change. Within dcp_timeout_sys() very little is*/
...
#define DCP_PB_RQB_TYPE         LSA_UINT8
#define DCP_PB_PAR_TYPE         LSA_UINT8
 ...
 */
#define DCP_MAX_LOW_CHANNELS    1

/* Maximum number of client channels on one lower channel (256)
 */
#define DCP_MAX_CLIENT_CHANNELS 1

/* Maximum number of upper client and server channels:
 * Max. one server channel and 256 client channels are based
 * on one lower channel. 120307lrg001: FSU-Channel
*/
#ifdef DCP_CLIENT_MODE
#ifdef DCP_SERVER_MODE
#define DCP_MAX_CHANNELS        (DCP_MAX_CLIENT_CHANNELS + 2) * DCP_MAX_LOW_CHANNELS
#else                           /* Only DCP_CLIENT_MODE: */
#define DCP_MAX_CHANNELS        DCP_MAX_CLIENT_CHANNELS * DCP_MAX_LOW_CHANNELS
#endif
#else                           /* Only DCP_SERVER_MODE: */
#define DCP_MAX_CHANNELS        DCP_MAX_LOW_CHANNELS * 2
#endif
 ...
 * Access to upper RQB
*/
#define DCP_UPPER_RQB_ERR_COMPID_GET(_RbuP)
...
#define DCP_REQUEST_UPPER_DONE(dcp_request_upper_done_ptr_, upper_rqb_ptr_, sys_ptr_) { LSA_UNUSED_ARG(sys_ptr_); \
 ...
 *
 *===========================================================================*/
#if (DCP_CFG_TRACE_MODE == 1)

/* Take these macros out of comment to have NO trace */
/* MH
#define DCP_UPPER_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_LOWER_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_SYSTEM_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_ERROR_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_FUNCTION_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_SERVER_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
#define DCP_SRVERR_TRACE_04(level_, msg_, para1_, para2_, para3_, para4_)  \
    if(level_ < LSA_TRACE_LEVEL_CHAT)       \
    trace_add_entry_04(APMA_PDB_FW_MODULE_ID_DCP, LTRC_ACT_MODUL_ID, __LINE__, level_, (TRACE_UINT32)para1_, (TRACE_UINT32)para2_, (TRACE_UINT32)para3_, (TRACE_UINT32)para4_)
MH */
#endif
...
#define DCP_ERROR_TRACE_BYTE_ARRAY(idx, level_, msg_, ptr_, len_)
