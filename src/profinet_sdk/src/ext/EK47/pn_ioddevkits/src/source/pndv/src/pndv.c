...
--- INTERNAL FUNCTIONS ------------------------
        ...
        - pndv_in_check_indication_timeout
        - pndv_in_so_locked_delay_timeout
        - pndv_in_set_so_state
        - pndv_in_get_ar_idx
        - pndv_in_get_api_idx
        - pndv_in_compress_api_list
...
void pndv_init( PNDV_IFACE_STRUCT_PTR  iface_ptr )
{
        ...
        pndv_data.stop_req              = FALSE;
        pndv_data.block_ar_while_rtf    = FALSE;
        ...
        for( async_cnt = 0; async_cnt <= PNDV_CM_AR_NO; async_cnt++ )   /* <= so that too much AR can be terminated cleanly */
        {
                ...
                pndv_data.ar[async_cnt].no_prm_end_ind      = FALSE;
                pndv_data.ar[async_cnt].ar_abort_requested  = FALSE;
                pndv_data.ar[async_cnt].empty_prm_end_ind   = FALSE;
                pndv_data.ar[async_cnt].pndv_ar_type        = PNDV_AR_TYPE_UNKNOWN;
                pndv_data.ar[async_cnt].pdev_8000_locked    = FALSE;
                pndv_data.ar[async_cnt].pdev_9000_locked    = FALSE;
        ...
        {
                pndv_data.ar_so_locked_state[async_cnt] = FALSE;
        }
        ...
        pndv_data.cfg.all_modul_running       = TRUE;
        ...
        pndv_data.akt_clear_state = TRUE;
...
void pndv_open( PNDV_SYS_PARAMETER_PTR parameter_ptr)
{
        ...
        if ( PNDV_SM_CLOSED == pndv_data.sm )
        {
                ...
                pndv_data.annotation_ptr = parameter_ptr->cm.annotation_ptr;
                ...
                {
                    pndv_in_fatal_error(PNDV_MODULE, __LINE__, tmp_ret_val);
                }

                //LSA_RQB_SET_METHOD_PTR(&pndv_data.rqb.connect_indication_timeout_rqb, pndv_ar_sys_connect_ind_timeout);
                /* Hardwareumgebung */
                /* ------------------------------------------------------------- */
                ...
                for ( tmp_count = 0; PNDV_MAX_CHANNEL > tmp_count; tmp_count++ )
                {
...
void pndv_sm(PNDV_SM_EVENT_E_T event)
{
    ...
    switch(pndv_data.sm)
    {
        case PNDV_SM_CLOSED:
        {
            /* state: closed, need to open pndv */
            switch (event)
            {
        ...
        case PNDV_SM_OPEN:
        {
            /* state: open, pndv needs to be startet */
            switch (event)
            {
                case PNDV_SM_EVENT_START:
                {
                    /* initiate start now */
                    pndv_data.sm = PNDV_SM_W_OPEN_CHANNELS_DONE;

                    /* activity */

                    pndv_in_cm_open_channel(0);
        ...
        case PNDV_SM_W_OPEN_CHANNELS_DONE:
        {
            /* state: wait on start to be finished */
            switch (event)
            {
                case PNDV_SM_EVENT_CHANNELS_OPEN_DONE:
                {
                    /* start process has finished */
        ...
        case PNDV_SM_W_CREATE_PD_SERVER_CNF:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_STOP:
                ...
                {
                    /* PD Server has now been created, create the CM server now */
        ...
        case PNDV_SM_W_CREAT_SERVER_CNF:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_STOP:
        ...
        case PNDV_SM_W_ADD_IM_SUBMODULES_DONE:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_ADD_IM_SUBMODULES_DONE:
                {
                    ...
                    pndv_in_cm_pd_prm_begin(); // initiates a chain of _PD_PRM_PREPARE, _PD_PRM_WRITE and finaly PD_PRM_END
        ...
        case PNDV_SM_W_REMOVE_DEVICE_CNF:
        {
            switch (event)
            {
        ...
        case PNDV_SM_W_DELETE_SERVER_CNF:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_CM_SV_DELETE_DONE:
                {
                    pndv_data.sm = PNDV_SM_W_CLOSE_ACP_CHANNEL_CNF;
                    /* activity */
                    pndv_in_cm_close_channel(PNDV_INDEX_PATH_ACP_CM);
                    break;
                }
                default:
                {
        ...
        case PNDV_SM_W_CLOSE_ACP_CHANNEL_CNF:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_ACP_CLOSE_CHANNEL_DONE:
                {
        ...
        case PNDV_SM_W_DELETE_PD_SERVER_CNF:
        {
            ...
            switch (event)
            {
                ...
                case PNDV_SM_EVENT_ACP_CLOSE_CHANNEL_DONE:
                    /*when PD server is not created and PNDV_SM_STOP_W_CM_CNF was called */
                case PNDV_SM_EVENT_DELETE_PD_SERVER_DONE:
                {
#ifndef PNDV_USE_PSI
                    /* pd server has been deleted, close CMPDSV channel now */
                    pndv_data.sm = PNDV_SM_W_CLOSE_PD_SV_CHANNEL_CNF;
#else
                    /* cmpdsv channel has been closed, close EDD_CLRPC_CM channel now */
                    pndv_data.sm = PNDV_SM_W_CLOSE_CHANNELS_DONE;
#endif

                    /* activity */
                    pndv_in_cm_close_channel(PNDV_INDEX_PATH_CMPDSV);
                    break;
                }
                default:
                {
        ...
        case PNDV_SM_W_CLOSE_PD_SV_CHANNEL_CNF:
        {
            switch (event)
            {
#ifndef PNDV_USE_PSI
                case PNDV_SM_EVENT_PD_SV_CLOSE_CHANNEL_DONE:
                {
                    /* cmpdsv channel has been closed, close EDD_CLRPC_CM channel now */
                    pndv_data.sm = PNDV_SM_W_CLOSE_CHANNELS_DONE;

                    /* activity */
                    pndv_in_cm_close_channel(PNDV_INDEX_PATH_EDD_CLRPC_CM);
                    break;
                }
#endif
                default:
        ...
        case PNDV_SM_W_CLOSE_CHANNELS_DONE:
        {
            switch (event)
            {
                case PNDV_SM_EVENT_STOP_DONE:
                {
...
Unsigned16 pndv_version( Unsigned16 version_len, PNDV_SYS_VERSION_PTR version_ptr)
{
        ...
        {
                UnsignedOpt i;
                Unsigned8   tmp_prefix[LSA_PREFIX_SIZE] = PNDV_LSA_PREFIX;
...
void pndv_debug_get_info (PNDV_SYS_DEBUG_INFO_PTR debug_info)
{
        ...
        debug_info->trace_data_ptr = (Unsigned8 *)&pndv_debug_buffer;
        debug_info->trace_data_len = sizeof(pndv_debug_buffer);

        debug_info->local_data_ptr = (Unsigned8 *)&pndv_data;
        debug_info->local_data_len = sizeof(pndv_data);


#else

        debug_info->local_data_ptr = (Unsigned8 *)&pndv_deb_str_not_supp;
...
void pndv_in_check_led_info( void )
{
        ...
        )
    {
        ...
        if ( TRUE != pndv_glob_data.sf_on )
        {
            pndv_glob_data.sf_on = TRUE;
    ...
    {
        if ( FALSE != pndv_glob_data.sf_on )
        {
            pndv_glob_data.sf_on = FALSE;
...
{
    UnsignedOpt sync_led_local_on = 0;
...
{
    #ifdef PNDV_TRACE_MSG
        //Unsigned8* p_deb_code_str;
        PNDV_TRACE_MSG_GET_STR(LSA_TRACE_LEVEL_NOTE_HIGH, &deb_code_str[0], deb_code, detail_1, detail_2, detail_3);
        PNDV_TRACE_MSG(0,      LSA_TRACE_LEVEL_NOTE_HIGH, &deb_code_str[0], detail_1, detail_2, detail_3);
    #else
...
void pndv_in_fatal_error( Unsigned8 error_module, Unsigned16 error_line, Unsigned32 error_code )
{
        ...
        pndv_errcb.lsa_component_id  = PNDV_LSA_COMPONENT_ID;
        ...
        pndv_errcb.error_data_length = (Unsigned16)sizeof( pndv_data);
        pndv_errcb.error_data_ptr    =                    &pndv_data;

        PNDV_FATAL_ERROR( &pndv_errcb);
        /*NOTREACHED*/
        while(1); /* make sure that this function can never return */
}
