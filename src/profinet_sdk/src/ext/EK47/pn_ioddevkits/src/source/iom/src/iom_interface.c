...
/*  descend into the "KRAM module driver" in case of the Ertec200 or the     */
/*  "PERIF module driver" in case of the Ertec200p, respectively.            */
/*                                                                           */
/*****************************************************************************/
...
/**
 * @name common functions for both KRAM and PERIF
 * @{ */
 ...
 *
 * @param init_data     structure containing PERIF/KRAM specific configuration parameters.
 *                      See iom_perif_init / iom_kram_init for details.
 */
LSA_VOID iom_init(iom_init_struct_t *init_data)
{
#if defined IOM_CFG_PNIP
    iom_perif_init(init_data);
#elif defined IOM_CFG_IRTE
    iom_kram_init(init_data);
#elif defined IOM_CFG_SWIF
    iom_swif_init(init_data);
#endif
}
 ...
 *
 * @param   ar_idx      index of the AR to be set up. PERIF : [0 - (IOM_CFG_PERIF_NUM_AR-1) (7)] , KRAM [0 - 1]
 * @param   ar_set_nr
 * @param   session_key
...
#elif defined IOM_CFG_IRTE
    return iom_kram_allocate_iocr_memory(ar_idx, ar_set_nr, session_key, iocr_ptr);
#elif defined IOM_CFG_SWIF
    return iom_swif_allocate_iocr_memory(ar_idx, ar_set_nr, session_key, iocr_ptr);
#endif
}
...
#elif defined IOM_CFG_IRTE
    iom_kram_free_iocr_memory(ar_idx);
#elif defined IOM_CFG_SWIF
    iom_swif_free_iocr_memory(ar_idx);
#endif
}
 ...
 *          In case of the Perif submodule, data consistency is always guaranteed; in case of the KRAM submodule, data consisteny
 *          is guaranteed by software, if the define IOM_KRAM_PROP_USE_CONS_EX_BUFFERS is set (see iom_init())
...
{
#if defined IOM_CFG_PNIP
    return iom_perif_consumer_lock(buffer, (LSA_VOID**)apdu_status_ptr, ar_idx);
#elif defined IOM_CFG_IRTE
    return iom_kram_consumer_lock(buffer, apdu_status_ptr, ar_idx);
#elif defined IOM_CFG_SWIF
    return iom_swif_consumer_lock(buffer, apdu_status_ptr, ar_idx);
#endif
}
...
#elif defined IOM_CFG_IRTE
        return iom_kram_consumer_unlock(ar_idx);
#elif defined IOM_CFG_SWIF
    return iom_swif_consumer_unlock(ar_idx);
#endif
}
...
IOM_ATTRIBUTE_ITCM LSA_VOID iom_provider_lock(LSA_VOID **buffer, LSA_UINT32 ar_idx)
{
    ...
    #else
        iom_kram_provider_lock(buffer, ar_idx);
    #endif
#elif defined IOM_CFG_SWIF
    iom_swif_provider_lock(buffer, ar_idx);
#endif
}
 ...
 * @param   apdu_status_ptr returns a pointer to the APDU status data. Enter LSA_NULL, if you don't need it.
 * @param   ar_idx          specifies the AR, whose output CR buffer shall be acquired
 * @param   offset          output CR offset
 * @param   length          output CR data length
 * @return  IOM_INVALID_BUFFER if the output CR is not initialized,
 *          IOM_BUFFER_LOCKED if the CR memory is already locked,
 *          IOM_OK_NEW_BUFFER or IOM_OK_OLD_BUFFER if locking was successful
 */
IOM_ATTRIBUTE_ITCM LSA_VOID iom_provider_lock_section(LSA_VOID** buffer, LSA_UINT32 ar_idx, LSA_UINT32 offset, LSA_UINT32 length)
{
#ifdef IOM_CFG_IRTE400
    iom_kram_provider_lock_section(buffer, ar_idx, offset, length);
#endif
}
...
IOM_ATTRIBUTE_ITCM LSA_UINT16 iom_provider_unlock(LSA_UINT32 ar_idx)
{
...
#elif defined IOM_CFG_IRTE
    #ifdef IOM_CFG_IRTE400
        return iom_kram_provider_unlock_section(ar_idx);
    #else
        return iom_kram_provider_unlock(ar_idx);
    #endif
#elif defined IOM_CFG_SWIF
    return iom_swif_provider_unlock(ar_idx);
#endif
}

#ifdef IOM_CFG_RTC3_UNBUFFERED
    /**
     * @brief   This function locks a buffer similar to iom_consumer_lock().
     *          In contrast to the standard lock-function, here the IRTE-memory is accessed directly (KRAM); no
     *          external SDRAM is accessed in case of the KRAM module. The PERIF submodule behaves exactly
     *          the same, compared to iom_consumer_lock().
     * @param   buffer          returns the address of the current user buffer
     * @param   apdu_status_ptr returns a pointer to the APDU status data
     * @param   ar_idx          specifies the AR, whose input CR memory shall be locked
     */
	IOM_ATTRIBUTE_ITCM LSA_UINT16 iom_consumer_lock_unbuffered(LSA_VOID **buffer, iom_apdu_status_t **apdu_status_ptr, LSA_UINT32 ar_idx)
    {
    #if defined IOM_CFG_PNIP
        return iom_perif_consumer_lock(buffer, (LSA_VOID**)apdu_status_ptr, ar_idx);
    #elif defined IOM_CFG_IRTE
        ...
        #else
            return iom_kram_consumer_lock_unbuffered(buffer, apdu_status_ptr, ar_idx);
        #endif
	#elif defined IOM_CFG_SWIF
        return iom_swif_consumer_lock(buffer, apdu_status_ptr, ar_idx);
    #endif
	...
	IOM_ATTRIBUTE_ITCM LSA_UINT16 iom_consumer_unlock_unbuffered(LSA_UINT32 ar_idx)
    {
    #if defined IOM_CFG_PNIP
        return iom_perif_consumer_unlock(ar_idx);
    #elif defined IOM_CFG_IRTE
            ...
            return IOM_INTERNAL_ERROR;
        #else
            return iom_kram_consumer_unlock_unbuffered(ar_idx);
        #endif
	#elif defined IOM_CFG_SWIF
        return iom_swif_consumer_unlock(ar_idx);
    #endif
    ...
    /**
     * @brief   This function locks a buffer similar to iom_provider_lock().
     *          In contrast to the standard lock-function, here the IRTE-memory is accessed directly (KRAM, only RT Class 3); no
     *          external SDRAM is accessed in case of the KRAM module. The PERIF submodule behaves exactly
     *          the same, compared to iom_provider_lock().
     * @param   buffer  returns the address of the current user buffer
     * @param   ar_idx  specifies the AR, whose input CR memory shall be locked
     */
	IOM_ATTRIBUTE_ITCM LSA_VOID iom_provider_lock_unbuffered(LSA_VOID **buffer, LSA_UINT32 ar_idx)
    {
    #if defined IOM_CFG_PNIP
        iom_perif_provider_lock(buffer, ar_idx);
    #elif defined IOM_CFG_IRTE
            ...
            *buffer = NULL;
        #else
            iom_kram_provider_lock_unbuffered(buffer, ar_idx);
        #endif
	#elif defined IOM_CFG_SWIF
        iom_swif_provider_lock(buffer, ar_idx);
    #endif
	...
	IOM_ATTRIBUTE_ITCM LSA_UINT16 iom_provider_unlock_unbuffered(LSA_UINT32 ar_idx)
    {
    #if defined IOM_CFG_PNIP
        return iom_perif_provider_unlock(ar_idx);
    #elif defined IOM_CFG_IRTE
            ...
            return IOM_INTERNAL_ERROR;
        #else
            return iom_kram_provider_unlock_unbuffered(ar_idx);
        #endif
	#elif defined IOM_CFG_SWIF
        return iom_swif_provider_unlock(ar_idx);
    #endif
 ...
 *          -> KRAM : performs internal 4 byte rounding
 *          -> SWIF : no internal rounding
...
{
#if defined IOM_CFG_PNIP
    return iom_perif_get_iocr_buffer_sizes(ar_idx, input_cr_buffer_size, output_cr_buffer_size);
#elif defined IOM_CFG_IRTE
    return iom_kram_get_iocr_buffer_sizes(ar_idx, input_cr_buffer_size, output_cr_buffer_size);
#elif defined IOM_CFG_SWIF
    return iom_swif_get_iocr_buffer_sizes(ar_idx, input_cr_buffer_size, output_cr_buffer_size);
#endif
}

/**
 * @brief   This function gets new output data from the ertec200 and is called in interrupt context.
 *          The function is specific to the KRAM module and available in the PNIP case only for
 *          compatibility reasons.
 */
LSA_VOID iom_update_consumer_data()
{
#if defined IOM_CFG_IRTE
    iom_kram_update_consumer_data();
#endif
}

/**
 * @brief   This function passes new input data to the ertec200 and is called in interrupt context.
 *          The function is specific to the KRAM module and available in the PNIP case only for
 *          compatibility reasons.
 */
LSA_VOID iom_update_provider_data()
{
#if defined IOM_CFG_IRTE
...
#elif defined IOM_CFG_IRTE
    return iom_kram_provider_set_data_state(ar_idx, value, mask);
#endif
}
...
#elif defined IOM_CFG_IRTE
    iom_kram_set_session_key_primary_arid(ar_idx);
#endif
}
...
#elif defined IOM_CFG_IRTE
    return iom_kram_get_session_key_primary_arid(ARSetID);
#endif
}
