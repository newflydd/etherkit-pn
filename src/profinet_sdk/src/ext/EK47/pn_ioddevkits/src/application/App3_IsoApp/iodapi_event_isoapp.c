...
#include "usrapp_cfg.h"   // example application spec. configuration settings
    ...
    // *=======================================================
        ...
        static PNIO_UINT32          IsoNewCycleCounter     = 0UL;
        static PNIO_UINT16          IsoTimerId             = 0U;
        static PNIO_ISO_OBJ_HNDL    IsoIsrNewCycleHndl     = 0;
        #if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
            #define ISO_GPIO_TI         7
            #define ISO_GPIO_TO         5
        #else
          #ifdef _RENESAS_RZN_
            #define ISO_GPIO_TI         1
          #else
            #define ISO_GPIO_TI         0
          #endif
            #define ISO_GPIO_TO         3
        #endif
    #endif
    ...
    void PrintAllUsedIoData (void)
    {
	    ...
	    for (SlotNum = 0; SlotNum < NUMOF_SLOTS; SlotNum++)			// loop over all slots
	    {
		    for (SubNum = 0; SubNum < NUMOF_SUBSLOTS; SubNum++)	// loop over all subslots
		    {
		      ...
		      {
			    DataAvailable = 1;
			    PNIO_printf ( (PNIO_INT8*) "##DatIn Slot=%d  Sub=%d length=%d : ",
				        SlotNum,					// slot num
					    SubNum,						// subslot num
			    ...
			    for (IoInd=0; IoInd < InDatLen[SlotNum][SubNum]; IoInd++)
			    {
				    PNIO_printf ( (PNIO_INT8*) "0x%02x ", InData[SlotNum][SubNum][IoInd]);
			    }
	    ...
	    for (SlotNum = 0; SlotNum < NUMOF_SLOTS; SlotNum++)			// loop over all slots
	    {
		    for (SubNum = 0; SubNum < NUMOF_SUBSLOTS; SubNum++)	// loop over all subslots
		    {
		      ...
		      {
			    DataAvailable = 1;
			    PNIO_printf ( (PNIO_INT8*) "##DatOut Slot=%d  Sub=%d length=%d : ",
				        SlotNum,					// slot num
					    SubNum,						// subslot num
			    ...
			    {
				    PNIO_printf ( (PNIO_INT8*) "0x%02X ", OutData[SlotNum][SubNum][IoInd]);
			    }
			    PNIO_printf ( (PNIO_INT8*) "\n");
		      }
		    }
	    ...
	    if (DataAvailable == 0)
		    PNIO_printf ( (PNIO_INT8*) "no io data available in any active IOCR\n");

    }

    PNIO_VOID IsoNewCycleIsr(void)
    {
        IsoNewCycleCounter++;
    }

    void PrintIsoNewCycleCounter(void)
    {
        PNIO_printf("IRT NewCycle counter: %lu.\n", IsoNewCycleCounter);
    }
            ...
            )
    {
		...
		{
			 PNIO_printf ( (PNIO_INT8*) "new IO controller input consumer status (ICS) = 0x%x in slot %d, subslot %d\n",
            ...
            )
    {
		...
		{
			PNIO_printf ( (PNIO_INT8*) "new IO controller output provider status (OPS) = 0x%x in slot %d, subslot %d\n",
...

    {
        ...
        { // ****param end for all submodules ****
    	    PNIO_printf ( (PNIO_INT8*) "##PARAM END  for all submodules, ArNum=%d, Session=%d \n",
                         ArNum, SessionKey);
        }
        else
        { // ****param end for one submodule ****
    	    PNIO_printf ( (PNIO_INT8*) "##PARAM END for Api=%d, Slot=%d, Sub=%d, ArNum=%d, Session=%d \n",
								      ...
								      PNIO_AR_REASON ReasonCode)
    {
        ...
        if (NumOfAr)
			...
			if ((IsoGpiosActivated == PNIO_TRUE) && (ArNum == IsoArNum))
			{
                ...
                if (PNIO_IsoObjCheck(IsoIsrNewCycleHndl) == PNIO_OK)
                {
                    PNIO_printf ("deactivate ISR for NewCycle....");
                    Status = PNIO_IsoFreeObj(IsoIsrNewCycleHndl);
                    if (Status == PNIO_OK)
                         PNIO_printf ("OK\n");
                     else
                         PNIO_printf ("ERROR PNIO_IsoFreeObj (Event=0x%x) occured\n", IsoGpioToHndl );
                ...
                if (IsoTimerId != 0)
                {
                    Status = OsStopTimer(IsoTimerId);
                    IsoNewCycleCounter = 0UL;
                }
			}
	#endif
    }
	...
	)
    {
        ...
        switch (RecordIndex)
        {
            ...
            // *----------------------------------------------*
            case 0x8030: // **** IM 4 *****
                    PNIO_printf ( (PNIO_INT8*) "##READ IsoMData, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                                 Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
                    *pBufLen = 0; // no date read.
                 break;
            ...
            case 2: // **** user specific demo record *****
		            PNIO_printf ( (PNIO_INT8*) "##READ Demo-Record , Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
					             Api, slot_nr, subslot_nr, *pBufLen, SequenceNum);
            ...
            case 1: // **** user specific startup record *****
                    PNIO_printf ( (PNIO_INT8*) "##READ startup-Record (here: demo record is returned), Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                ...
                case EXAMPL_IPAR_REC_IND: // **** read example data for the iPar Server *****
	                     ...
	                     *pBufLen = 4;
                         OsMemCpy ( pBuffer,
				                    ...
				                    *pBufLen);
		                 PNIO_printf ( (PNIO_INT8*) "##READ iPar example data, Api=%d Slot=%d Subslot=%d Len=%d, RecordIndex=%d\n",
		        ...
		        // *----------------------------------------------*
	             if ( RecordIndex <= 0x7fff)
	             {
                    ...
                    else
                    {
		                ...
		                PNIO_printf ( (PNIO_INT8*) "##READ_REC RQ,  Api =%d Slot=%d Subslot=%d Index=0x%x, Len=%d, Sequ_nr=%d\n",
        ...
        if (DoHexdumpData)
        {
            ...
            PNIO_printf ( (PNIO_INT8*) "##REC_DATA = ");
            for (i=0; i < *pBufLen; i++)
            {
                PNIO_UINT32  Val;
                Val = (PNIO_UINT32 ) *(p8+i);;
                PNIO_printf ( (PNIO_INT8*) "0x%02x ", Val);
            }
...

    {
        ...
        switch (RecordIndex)
        {
                ...
                case 0x8030:
...

                    {
                        ...
                        PNIO_UINT32 TiDelay_ns = 0;
							...
							if (Slot == 1)
							{
								// *** slot with input data ****

								// ** aktivate Ti on GPIO 7 ***
								if (Status == PNIO_OK)
								{
								    if (IsoGpioTiHndl != 0)
								    {
									...
									Status = PNIO_IsoActivateGpioObj(ISO_GPIO_TI, TiDelay_ns,  ISO_GPIO_LOW_ACTIVE, &IsoGpioTiHndl);
									if (Status != PNIO_OK)
									{
							...
							if ((Slot == 2) && (Status == PNIO_OK))
							{
								// *** slot with outnput data ****
								// ** aktivate To on GPIO 5 ***
							    if (IsoGpioToHndl != 0)
                                {
							    	if(PNIO_IsoObjCheck(IsoGpioTiHndl) == PNIO_OK)
							    	{
                                ...
                                if(To_ns == Tcyc_ns)
                                {
                                    Status = PNIO_IsoActivateGpioObj(ISO_GPIO_TO, 0,  ISO_GPIO_LOW_ACTIVE, &IsoGpioToHndl);
                                }
                                else
                                {
								    Status = PNIO_IsoActivateGpioObj(ISO_GPIO_TO, To_ns,  ISO_GPIO_LOW_ACTIVE, &IsoGpioToHndl);
                                }
								if (Status != PNIO_OK)
								{
									PNIO_printf ("Error PNIO_IsoActivateGpioObj Ti=%d\n", To_ns);
                            ...
                            if (Status == PNIO_OK && IsoIsrNewCycleHndl == 0)
                            {
                                Status = PNIO_IsoActivateIsrObj(IsoNewCycleIsr, 0, &IsoIsrNewCycleHndl);
                            }

                            if (Status == PNIO_OK && IsoTimerId == 0)
                            {
                                Status = OsAllocTimer(&IsoTimerId, LSA_TIMER_TYPE_CYCLIC, LSA_TIME_BASE_100MS, (PNIO_VOID *)PrintIsoNewCycleCounter);
                            }

                            if (Status == PNIO_OK)
                            {
                                Status = OsResetTimer(IsoTimerId);
                                Status = OsStartTimer(IsoTimerId, 0, 1);
                            }
                        ...
                        PNIO_printf ( (PNIO_INT8*) "##WRITE IsoM-Data, Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                                     Api, pAddr->Geo.Slot, pAddr->Geo.Subslot, *pBufLen, SequenceNum);
                                {
                                    PNIO_printf ( (PNIO_INT8*) "\n=========== decode ISOM record with index 0x8030 =============\n");
                                    PNIO_printf ( (PNIO_INT8*) "Slotnumber          = %10d\n", Slot);
                                    PNIO_printf ( (PNIO_INT8*) "SubSlotnumber       = %10d\n", Sub);
                                    PNIO_printf ( (PNIO_INT8*) "ConApplCycleFactor  = %10d\n", OsHtons (pIsoM->ControllerApplicationCycleFactor));
                                    PNIO_printf ( (PNIO_INT8*) "TimeDataCycle       = %10d nsec\n", Tcyc_ns);
                                    PNIO_printf ( (PNIO_INT8*) "TimeIOInput         = %10d nsec\n", Ti_ns);
                                    PNIO_printf ( (PNIO_INT8*) "TimeIOOutput        = %10d nsec\n", To_ns);
                                    PNIO_printf ( (PNIO_INT8*) "TimeIOInputValid    = %10d nsec\n", OsHtonl (pIsoM->TimeIOInputValid));
                                    PNIO_printf ( (PNIO_INT8*) "timeIOOutputValid   = %10d nsec\n", OsHtonl (pIsoM->TimeIOOutputValid));
                                    PNIO_printf ( (PNIO_INT8*) "\n");
            ...
            #if EXAMPLE_IPAR_SERVER
                case EXAMPL_IPAR_REC_IND: // **** read example data for the iPar Server *****
		                PNIO_printf ( (PNIO_INT8*) "##download iPar example, Api=%d Slot=%d Subslot=%d Len=%d, RecordIndex=%d\n",
            ...
            #endif
            case 2: // **** user specific demo record *****
		            PNIO_printf ( (PNIO_INT8*) "##WRITE Demo-Record , Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
					             Api, pAddr->Geo.Slot, pAddr->Geo.Subslot, *pBufLen, SequenceNum);
                    break;

            case 1: // **** user specific startup record *****
                    PNIO_printf ( (PNIO_INT8*) "##WRITE startup-Record from GSD file , Api=%d Slot=%d Subslot=%d Len=%d, Sequ_nr=%d\n",
                                 Api, pAddr->Geo.Slot, pAddr->Geo.Subslot, *pBufLen, SequenceNum);
                    break;
            default:
                 Status = PNIO_NOT_OK;
        ...
        if (Status == PNIO_OK)
        {
            ...
            if (DoHexdumpData)
            {
	            PNIO_printf ( (PNIO_INT8*) "##WRITE_REC RQ, Api=%d Slot=%d Subslot=%d Index=0x%x, Len=%d, Sequ_nr=%d\n",
