...
{
    OsAlloc( (void **) &data, 0, sizeof (AMRBlock) );

    if ( !data )
    {
        PNIO_printf ("ERROR: AMR data is not initialized => zero Len !\n");
    }
    ...
    )
{
    ...
    PNIOext_cbf_amr_response_handler(pAddr, pBufLen, pBuffer);
...
PNIO_VOID   AMRData_AddAssetManagementBlock ( AMRBlock_ptr block )
{
    ...
    if ( !data )
    {
        PNIO_printf ("ERROR: AMR_CreateAssetManagementObject() => zero Len !\n");
        return;
    }
...
AMRBlock_ptr AMRData_CreateAMRObject ( AssetType type )
{
    ...
    switch ( type )
    {
        ...
        case Asset_FirmwareInformation:
            break;
        default:
            PNIO_printf ( "ERROR ( AMR ) : Not enough memory for creation of asset management data\n", block);
            return block;
    }

    OsAlloc( (void **) &block, 0, sizeof (AMRBlock) );
...
PNIO_UINT8 * AMRObject_GenerateIMUniqueIdentifier( PNIO_UINT32 instance, PNIO_UINT8 vendor_id, PNIO_UINT8 device_id )
{
    ...
    OsAlloc ( (PNIO_VOID **) &uuid, 0, sizeof (AMR_UUID) );

    //get 100ns time from OS
    time = OsGetUuidTime();

    // clock seq
    clock_seq = ( PNIO_UINT16 ) OsRand();

    // setup UUID version 1
    uuid->UUID_part_1 = time & 0xffffffff;
    uuid->UUID_part_2 = ( time >> 32 ) & 0xffff;
    uuid->UUID_part_3 = (1 << 12) | ( ( time >> 48 ) & 0xfff );
...
PNIO_VOID AMRObject_SetIMUniqueIdentifier(AMRBlock_ptr block, PNIO_UINT8 * uuid, PNIO_UINT32 size )
{
    ...
    if ( (*tmp == 0) && ( OsMemCmp(tmp, tmp + 1, size - 1) == 0 ) )
    {
        PNIO_printf ( "ERROR ( AMR ) : zero value for UUID is reserved \n", block);
        return;
    }

    OsMemCpy (& block->IMUniqueIdentifier.UUID_part_1, tmp, sizeof(block->IMUniqueIdentifier.UUID_part_1));
    tmp += sizeof(block->IMUniqueIdentifier.UUID_part_1);

    OsMemCpy (& block->IMUniqueIdentifier.UUID_part_2, tmp, sizeof(block->IMUniqueIdentifier.UUID_part_2));
    tmp += sizeof(block->IMUniqueIdentifier.UUID_part_2);

    OsMemCpy (& block->IMUniqueIdentifier.UUID_part_3, tmp, sizeof(block->IMUniqueIdentifier.UUID_part_3));
    tmp += sizeof(block->IMUniqueIdentifier.UUID_part_3);

    OsMemCpy (& block->IMUniqueIdentifier.UUID_part_4, tmp, sizeof(block->IMUniqueIdentifier.UUID_part_4));
    tmp += sizeof(block->IMUniqueIdentifier.UUID_part_4);

    OsMemCpy (& block->IMUniqueIdentifier.UUID_part_5_array[0], tmp, 6 * sizeof ( PNIO_UINT8) );
...
PNIO_VOID AMRObject_SetAMLocation(AMRBlock_ptr block, LocationType type, PNIO_VOID *nodes, PNIO_UINT16 num )
{
    ...
    if ( !block )
    {
        PNIO_printf ("ERROR: (AMR) - NULL output asset management block for AM Location \n");
    }

    if ( !nodes )
    {
        PNIO_printf ("ERROR: (AMR) - NULL input data for AM Location\n");
    }

    if ( ( type != Asset_LocationLevelTree ) && (type != Asset_LocationSlotNumber) )
    {
        PNIO_printf ("ERROR: (AMR) - wrong type of AM Location \n");
    }

    if ( ( type == Asset_LocationLevelTree) && ( num > 12 ) )
    {
        PNIO_printf ("ERROR: (AMR) - max number of nodes is 12 \n");
    }
    if ( ( type == Asset_LocationSlotNumber) && ( num != 4 ) )
    {
        PNIO_printf ("ERROR: (AMR) - BeginSlotNumber BeginSubslotNumber EndSlotNumber EndSubslotNumber must be sent -> 4 \n");
    }
    ...
    switch ( type )
    {
        ...
        case Asset_LocationSlotNumber:
            ...
            {
                *(dst - src_off ) = OsHtons( src[src_off] );
            }
...
PNIO_VOID AMRObject_IMAnnotation(AMRBlock_ptr block, PNIO_INT8 *InputData, PNIO_UINT32 length )
{
    ...
    if ( !block )
    {
        PNIO_printf ("ERROR: (AMR) - asset management block is NULL \n");
    }

    if ( !InputData )
    {
        PNIO_printf ("ERROR: (AMR) - data is NULL \n");
    }

    if (length > 64 )
    {
        PNIO_printf ("ERROR: (AMR) - length has to be shorter then 64B \n");
    }
    ...
    for ( i = 0; i < length ; i ++)
    {
        ...
        {
            PNIO_printf ("ERROR: (AMR) - character is not visible string \n");
            return;
        }
    }
...
PNIO_VOID AMRObject_SetIMSerialNumber(AMRBlock_ptr block, PNIO_INT8 *InputData, PNIO_UINT32 length )
{
    ...
    if ( length > 16 )
    {
        PNIO_printf ("ERROR: (AMR) - length has to be shorter then 16B \n");
    }
...
PNIO_VOID AMRObject_SetAMDeviceIdentification ( AMRBlock_ptr block, PNIO_UINT16 DeviceID, PNIO_UINT16 VendorID, OrganizationType Organization, PNIO_UINT16 DeviceSubID )
{
    ...
    switch ( Organization )
    {
        case Asset_IEC_61158_6_10:
            if (DeviceSubID != 0)
            {
                PNIO_printf ("ERROR: (AMR) - wrong device sub ID \n");
                return;
            }
            break;
        ...
        case Asset_IEC_61158_Type_8:
            break;
        default:
            PNIO_printf ("ERROR: (AMR) - wrong organization ID \n");
            return;
    }
...
PNIO_VOID AMRObject_SetAMTypeIdentification ( AMRBlock_ptr block, PNIO_UINT16 AMTypeIdentification )
{
    ...
    {
        PNIO_printf ("ERROR: (AMR) - AMTypeIdentification in reserved range \n");
        return;
    }
...
PNIO_VOID AMRObject_SetAMSoftwareRevision( AMRBlock_ptr block, PNIO_UINT8 *InputData, PNIO_UINT32 size )
{
    ...
    switch ( block->type )
    {
        ...
        case Asset_HardwareInformation:
        default:
            PNIO_printf ("ERROR: (AMR) - wrong type of Asset Management Data \n");
            return;
    }

    if (dst->IMSoftwareRevision.SWRevisionPrefix != 0)
    {
        PNIO_printf ("ERROR: (AMR) - IM SoftwareRevision is used \n");
        return;
    }
...
PNIO_VOID AMRObject_SetAMHardwareRevision( AMRBlock_ptr block, PNIO_UINT8 *InputData, PNIO_UINT32 size )
{
    ...
    {
        PNIO_printf ("ERROR: (AMR) - IM HardwareRevision is used \n");
        return;
    }
...
PNIO_VOID AMRObject_SetIMSoftwareRevision( AMRBlock_ptr block, PNIO_UINT8 prefix, PNIO_UINT8 revision, PNIO_UINT8 bugfix, PNIO_UINT8 internal )
{
    ...
    {
        PNIO_printf ("ERROR: (AMR) - SW prefix is wrong \n");
        return;
    }
...
PNIO_VOID ASSET_PUT8_HTON(PNIO_VOID * base, PNIO_UINT32 offset, PNIO_UINT8 val)
{
    ...
    OsMemCpy ((void*)ptr, (void*) & val, sizeof(val));
}
