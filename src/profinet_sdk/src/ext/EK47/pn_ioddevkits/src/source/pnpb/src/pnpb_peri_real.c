...
// * public functions
// *=================================================================
void PnpbRealCfgInit (void)
{
    OsMemSet (&RealPlugList[0], 0, sizeof (RealPlugList));
}
...
{
    OsMemSet (&pPnpbIf->real_cfg[Ind], 0, sizeof (pPnpbIf->real_cfg[0]));
                            ...
                            PNIO_BOOL            more_follows)   // more follows PNIO_TRUE/PNIO_FALSE
{
    ...
    if (iface_real_cfg_ptr->elem.state == PNDV_IFACE_SERVICE_IDLE)
    {
        ...
        if(more_follows == PNIO_FALSE)
        {
            ...
            OsExitX (OS_MUTEX_PNPB);                 // free mutex temporarily during wainting on semaphore SEM_PNPB_PLUG, to avoid deadlock
            PnpbReqSync  (cmd,                       // command
                           0,                        // add value 1
                           (PNIO_UINT16)Ind,         // add value 2
                           Pnpb.SemId[SEM_PNPB_PLUG],
                           NULL);    // not used yet
            OsEnterX (OS_MUTEX_PNPB);                // reoccupy mutex after semaphore SEM_PNPB_PLUG has been set
