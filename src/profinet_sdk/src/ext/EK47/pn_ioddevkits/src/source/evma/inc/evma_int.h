...
/* hardware properties */
    ...
    #define EVMA_NUM_COMPARATORS   5        /**< 5 comparators in IRTE IP */
    #define EVMA_NUM_OUTPUTS       6        /**< 6 MUX outputs for 6 GPIOs */

    /* EVMA_IRTE_IRQ_INDEX specifies the interrupt to be used on the Ertec200 (see config file) */
#elif defined(EVMA_USE_RZT2)
    #define EVMA_NUM_COMPARATORS   4        /**< No comparator, Number of comparators = number of PTPOUTs */
    #define EVMA_NUM_OUTPUTS       4        /**< 4 MUX outputs for 4 GPIOs */
...
#elif defined(EVMA_USE_IRTE)
    /** IRTE instance descriptor */
typedef struct evma_instance_s
{
    LSA_UINT32 magic_value;             /** magic value that is used to verify the validity of the instance structure */
    evma_instance_state_t state;
    evma_application_type_t app_type;   /** is set according to the evma_alloc_event() - input parameter */
    LSA_VOID (*isr)(LSA_VOID);          /** callback function, that is called at timer event */
    LSA_UINT8 comparator_index;         /** index of the allocated comparator inside the application timer */
    LSA_UINT32 trigger_time_ns;         /** is set according to the evma_alloc_event() input parameter */
    LSA_UINT8 gpio_index;               /** specifies the GPIO index that is to be used if application type EVMA_APPLICATION_GPIO is requested. -> output_index is defined that way */
}evma_instance_t;
#elif defined(EVMA_USE_RZT2)
    /** RZT2 instance descriptor */
typedef struct evma_instance_s
{
    LSA_UINT32 magic_value;             /** magic value that is used to verify the validity of the instance structure */
    evma_instance_state_t state;
...
void evma_in_fatal_error( LSA_UINT8 error_module, LSA_UINT16 error_line, LSA_UINT16 error_code);
