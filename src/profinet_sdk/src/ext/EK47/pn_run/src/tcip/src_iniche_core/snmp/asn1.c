 ...
 * MODULE: SNMP
 *
 * ROUTINES: asn_parse_int(), asn_build_int(), asn_build_unsigned_int(),
...
#if 1 /*INICHE_TCIP_ITGR check valid buffer length, see 2374779 */
	if (0 == *datalength)
	{
		return NULL;
	}
...
#if 1 /*INICHE_TCIP_ITGR check valid buffer length, see 2374779 */
   bufp = asn_parse_length(bufp, *datalength - 1, &asn_length);
#else
...
u_char * asn_build_unsigned_int(u_char* data,
                           unsigned* datalength,
                           u_char type, const u_long* intp, unsigned intsize)
{
   /*
    * ASN.1 integer ::= 0x02 asnlength byte {byte}*
    */
...
#if 1 /*INICHE_TCIP_ITGR check valid buffer length, see 2374779 */
   bufp = asn_parse_length(bufp + 1, *datalength -1, &asn_length);
#else
...
#if 1
	unsigned datalength, /*INICHE_TCIP_ITGR pass valid buffer length to parse function, see 2374779 */
#endif
     u_long  *   length)        /* OUT - value of length field */
{
   ...
   if (lengthbyte & ASN_LONG_LEN)
   {
	  ...
	  {
		SNMPERROR("insufficient valid buffer length");
		SNMP_MIB_CTR(snmpInASNParseErrs);
		return NULL;
	  }
#endif
...
#if 1 /*INICHE_TCIP_ITGR length cannot be longer than maximum packet length, see 2374779 */
		if (*length >= SNMPSIZ)
		{
			SNMPERROR("length exceeds max SNMP packet length");
			SNMP_MIB_CTR(snmpInASNParseErrs);
			return NULL;
		}
#endif
 ...
 */
   ...
   *datalength -= (int)asn_length + (bufp - data);
   ...
   while (length > 0)
   {
      ...
      do 
      {
#if 1 /*INICHE_TCIP_ITGR prevent reading beyond OID end, see 2374779 */
		if (0 == length)
		{
			return NULL; /* bufp has passed end of malformed OID, see while clause below */
		}
#endif
 ...
 * Returns NULL on any error.
 */
#if 0 /*INICHE_TCIP_ITGR, see 2374779 */
...
#endif /* SNMP_32BIT_OID */
#endif  /*INICHE_TCIP_ITGR, see 2374779 */
