...
/*  2016-08-01  P05.03 - P07.00 1818081: Xid - Abfrage der TimerID...        */
/*                                                                           */
/*****************************************************************************/
#endif /* DCP_MESSAGE */
...
DCP_ETH_RQB_PTR_TYPE  RblPtr)
{
	...
	*/
	...
	DCP_LOWER_TRACE_03(0 /*TraceIdx*/, LSA_TRACE_LEVEL_NOTE, ">>> dcp_request_lower_eth_done() RblPtr=%X UserPtr=%X Response=0x%x",
		...
		case DCP_ETH_OPC_REQUEST:
			...
			*/
			...
			case DCP_ETH_RECV:
				...
				{
					RcvPtr = (DCP_ETH_RECV_PTR)(void*)DCP_ETH_RQB_RECV_PTR_GET(RblPtr);
					...
					PduPtr = (DCP_PDU*)(void*)((LSA_UINT8*)RcvPtr + DCP_ETH_RQB_RECV_DATA_OFFSET_GET(RblPtr) + 2);
					...
					*/
					...
					else
					{
						...
						{
							//DCP_SWAPD(&PduPtr->Rsp.Hdr.XidHigh, &Xid);      //This line produces Lint msg 415: Likely access of out-of-bounds pointer
							Xid = DCP_DWORD(DCP_SWAP16(PduPtr->Rsp.Hdr.XidLow), DCP_SWAP16(PduPtr->Rsp.Hdr.XidHigh));
							...
							*/
							if (LSA_HOST_PTR_ARE_EQUAL(ChPtr, LSA_NULL)
							|| (DCP_LOBYTE(DCP_HIWORD(Xid)) > ChPtr->TimerCount)		// 1818081: was >= because of HELLO timer
...
//						else
			...
			case DCP_ETH_SEND:
				SndPtr = (DCP_ETH_SEND_PTR)(void*)DCP_ETH_RQB_SEND_PTR_GET(RblPtr);
				PduPtr = &SndPtr->Pdu;
				if (DCP_IS_REQUEST(PduPtr))
 					ChPtr = dcp_ChGet(UserId.uvar8_array[DCP_HANDLE_UID]);
				else
				...
				else
				{
					...
					if (ChLowPtr->State == DCP_CHA_STATE_CLOSING)
					{
						if (SndPtr->Pdu.Req.Hdr.ServiceId == DCP_HELLO) /* TIA 2445114 */
						{
							/* case: close-channel between send.req and send.cnf; timer not freed in dcp_TimerCleanAll() because ChPtr->TimerPtr is NULL */
							if (LSA_HOST_PTR_ARE_EQUAL(ChPtr->TimerPtr, LSA_NULL)
								&& ChPtr->Hello.Active
								&& ChPtr->Hello.TimerQueue.Count != 0)
							{
								DCP_TIMER_PTR_TYPE TimerPtr = ChPtr->Hello.TimerQueue.FirstPtr;
					...
					else if (!LSA_HOST_PTR_ARE_EQUAL(ChPtr, ChLowPtr->ChServerPtr))
					{
						...
						*/
						//DCP_SWAPD(&SndPtr->Pdu.Req.Hdr.XidHigh, &Xid);      //This line produces Lint msg 415: Likely access of out-of-bounds pointer
						Xid = DCP_DWORD(DCP_SWAP16(SndPtr->Pdu.Req.Hdr.XidLow), DCP_SWAP16(SndPtr->Pdu.Req.Hdr.XidHigh));
	...
	DCP_LOWER_TRACE_04(TraceIdx, LSA_TRACE_LEVEL_NOTE, "<<< dcp_request_lower_eth_done() RblPtr=%X, ChPtr=%X Service=0x%x Response0x%x", 
					RblPtr, ChPtr, Service, Response);
    DCP_EXIT();
}
...
DCP_CHA_PTR  			ChPtr)
{
	...
	||  (ChPtr->DetailPtr->DevId == DCP_DEV_FSU))
	{
		...
		*/
		...
		DCP_ETH_RQB_HANDLE_SET(RblPu.RbEthPtr, 0);
		EDD_RQB_SET_HANDLE_LOWER(RblPu.RbEthPtr, 0);
	...
	||  (ChPtr->DetailPtr->DevId == DCP_DEV_FSU))
	{
		if (DCP_ETH_RQB_RESPONSE_GET(RblPu.RbEthPtr) == DCP_ETH_RSP_OK)
		{
			...
			*/
			...
			ChPtr->ChLowPtr->HandleLower = DCP_ETH_RQB_HANDLE_LOWER_GET(RblPu.RbEthPtr); /* TIA 2709127 */

			/* Fill Lower Requstblock for REQUEST, Service: GetParams
			*/
			DCP_ETH_RQB_OPCODE_SET(RblPu.RbEthPtr, DCP_ETH_OPC_REQUEST);
			DCP_ETH_RQB_HANDLE_SET(RblPu.RbEthPtr, 0); /* TIA 2709127 */
			EDD_RQB_SET_HANDLE_LOWER(RblPu.RbEthPtr, ChPtr->ChLowPtr->HandleLower); /* TIA 2709127 */
...
LSA_UINT8				Uid)
{
	...
	*/
	...
	if(RbuRespVal == DCP_RSP_OK)    //lint !e774 Boolean within 'if' always evaluates to True, because dcp_ErrorInternal() is an exit, HM 13.01.2016
	{
...
DCP_ETH_RQB_PTR_TYPE	RbPtr)
{
	...
	*/
	...
	if(LSA_HOST_PTR_ARE_EQUAL(RbPtr, LSA_NULL))     //Avoid Lint msg 413: Likely use of null pointer
	{
		/* Allocate Requestblock
		*/
		DCP_ALLOC_LOWER_RQB(&NewPu, UserId, sizeof(DCP_LOWER_RQB_TYPE), ChPtr->SysPtr);
...
LSA_UINT16 				RbuRespVal)
{
	...
	LSA_UINT16 RetVal = DCP_RSP_ERR_UNKNOWN;
	...
	||  (ChPtr->DetailPtr->DevId == DCP_DEV_FSU))
	{
		if (ChPtr->ChLowPtr->State == DCP_CHA_STATE_OPEN)
		{
			...
			*/
			...
			RetVal = dcp_CloseLower(ChPtr, RblPu);
		}
		else
		{
			/* On error:
	...
	/* else: currently only Ethernet is supported
	*/
	DCP_FUNCTION_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_OpenLowerError() ChPtr=%X RbuPtr=%X RetVal=%X", 
						ChPtr, RbuPtr, RetVal);
}
