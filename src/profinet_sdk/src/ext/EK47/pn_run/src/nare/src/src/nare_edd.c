...
static const NARE_MAC_TYPE NARE_EDDBroadcastMac = {0xff,0xff,0xff,0xff,0xff,0xff};
static const NARE_MAC_TYPE NARE_EDDNullMac      = {0,0,0,0,0,0};
...
/*                                 functions                                 */
/*===========================================================================*/
static LSA_VOID NARE_EDDNrtSendDone(
    ...
    LSA_UINT16                      FrameLen)
{
    ...
    /* --------------------------------------------------------------------------*/
    if (( FrameLen >= Length ) && (!LSA_HOST_PTR_ARE_EQUAL(pArpContent, LSA_NULL)))
    {
            ...
            (pArpContent->Op  == NARE_HTONS(NARE_ARP_OP_RESPONSE)) )
        {
            ...
            {   /* if this RCTRL is in use and sniffer active..*/
                ...
                {   /* if this is a IP we waiting for answer...*/
                    ...
                    {
                        if ( pRCtrl->Sniffer.Mode == NARE_SNIFF_MODE_MULTIPLE )
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_EDDFinishARP(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_EDD_RCTRL_PTR_TYPE      pRCtrl)
{
    NARE_RSP                 Response;
    ...
    switch ( NARE_RQB_GET_OPCODE(pRCtrlUser->pRQB))
    {
        case NARE_OPC_TEST_IP:
            ...
            {   /* copy errorneous MAC to RQB */
                NARE_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"EDD[%X:%u]-ARP finished. IP in use, SniffMode(%u)!!"
                    ,pHDB->ThisHandle,pRCtrl->ThisIdx,pRCtrlUser->pRQB->args.TestIP.SniffMode);
                if (pRCtrlUser->pRQB->args.TestIP.SniffMode == NARE_SNIFF_MODE_MULTIPLE)
                {
            ...
            {
                NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD[%X:%u]-ARP finished. No multiple/test-IP usage.",pHDB->ThisHandle,pRCtrl->ThisIdx);
            }
            break;
        ...
        case NARE_OPC_RESOLVE_IP:
            ...
            {   /* copy MAC to RQB */
                ...
                {
                    Response = NARE_RSP_ERR_IP_IN_USE;
                    NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH,"EDD[%X:%u]-ARP finished. Resolve IP multiple use!!",pHDB->ThisHandle,pRCtrl->ThisIdx);
                }
                else
                {
                    NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD[%X:%u]-ARP finished. Resolve IP success.",pHDB->ThisHandle,pRCtrl->ThisIdx);
                }
            }
            else
            {
                NARE_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"EDD[%X:%u]-ARP finished. Resolve IP no answer.",pHDB->ThisHandle,pRCtrl->ThisIdx);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID NARE_EDDTimerEventARP(LSA_VOID)
{
    LSA_UINT32                 i,j;
    NARE_HDB_EDD_PTR_TYPE      pHDB;
    NARE_EDD_RCTRL_PTR_TYPE    pRCtrl;
            ...
            for (j=0; j<pHDB->EDDRCtrl.Cnt; j++)
            {
                ...
                {   /* first check whether to send the next ARP */
                    if ((pRCtrl->Sniffer.TimerEndCounter == Counter) && (pRCtrl->pARP_RQB != LSA_NULL))
                    {
                        ...
                        {
                            if(( pRCtrl->Sniffer.Mode == NARE_SNIFF_MODE_SINGLE ) && (pRCtrl->Sniffer.MultipleRsp == LSA_TRUE))
                            {   /* terminate sequence  */
                    ...
                    else
                    {
                        ...
                        if (( pRCtrl->Sniffer.TimerEndCounter == Counter ) ||
                            (( pRCtrl->Sniffer.Mode == NARE_SNIFF_MODE_SINGLE ) && ( pRCtrl->Sniffer.MultipleRsp ))
                            )
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID NARE_EDDAbortSniffing(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_BOOL NARE_EDDFindAndFinishSniff(
    NARE_HDB_EDD_PTR_TYPE       pHDB,
    NARE_USER_RCTRL_PTR_TYPE    pRCtrl)
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_HDB_EDD_PTR_TYPE NARE_EDDGetHandleByNIC(
    LSA_UINT16 NICId)
{
    NARE_HDB_EDD_PTR_TYPE pHDB;
    LSA_UINT32            i;
    ...
    LSA_SYS_PTR_TYPE          pSys)
{
    ...
    if ( Found )
    {
        ...
        NARE_ALLOC_LOCAL_MEM((NARE_LOCAL_MEM_PTR_TYPE *)&pHDB->EDDRCtrl.pTable,DSize);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static NARE_EDD_RCTRL_PTR_TYPE NARE_EDDGetRCTRLByIdx(
    NARE_HDB_EDD_CONST_PTR_TYPE pHDB,
    LSA_UINT32                  Idx)
{
    NARE_EDD_RCTRL_PTR_TYPE  pRCtrl;
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
NARE_EDD_RCTRL_PTR_TYPE NARE_EDDGetFreeRCTRL(
    NARE_HDB_EDD_PTR_TYPE pHDB)
{
    NARE_EDD_RCTRL_PTR_TYPE  pRCtrl;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_EDDOpenChannelDone(
    NARE_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16               RetVal;
    ...
    if ( NARE_EDD_RQB_GET_RESPONSE(pRQB) == EDD_STS_OK)
    {
        ...
        /* save channel */
        pHDB->EDDHandle = ((NARE_EDD_LOWER_RQB_OPEN_CHANNEL_PTR_TYPE)pRQB->pParam)->HandleLower; /* TIA 2709127 */
    }
    else
    {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_EDDCloseChannelDone(
    NARE_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_EDD_PTR_TYPE        pHDB)
{
    LSA_UINT16               RetVal;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_EDDGetMACDone(
    NARE_EDD_LOWER_RQB_PTR_TYPE  pRQB,
    NARE_HDB_EDD_PTR_TYPE        pHDB)
{
    ...
    NARE_HDB_EDD_PTR_TYPE        pHDB)
{
    NARE_RSP                 Response         = NARE_RSP_OK;
    NARE_EDD_RCTRL_PTR_TYPE  pRCtrl;
    LSA_BOOL                 CallStateMachine = LSA_FALSE;
    LSA_BOOL                 Resend           = LSA_FALSE;
    NARE_USER_RCTRL_PTR_TYPE pRCtrlUser;
    LSA_UINT16               RetVal;
    ...
    switch (NARE_EDD_RQB_GET_RESPONSE(pRQB))
    {
        case EDD_STS_OK:
            ...
            if (( pRCtrl->BurstCnt > 1 ) && ( ! pRCtrl->pRCtrl->DoCancel ))
            {
                ...
                NARE_EDD_RQB_SET_HANDLE(pRQB,0); /* TIA 2709127 */
                EDD_RQB_SET_HANDLE_LOWER(pRQB,pHDB->EDDHandle); /* Prepare for sending TIA 2709127 */
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static LSA_VOID NARE_EDDRQBCopyLLErrorLocal(
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID nare_edd_request_lower_done(
    NARE_EDD_LOWER_RQB_PTR_TYPE  pRQB)
{
    NARE_HDB_EDD_PTR_TYPE   pHDB;
    ...
    LSA_UINT8                    Mode)
{
    ...
    pRCtrl->Sniffer.CurMACValid = LSA_FALSE; /* only used for Mode == NARE_SNIFF_MODE_MULTIPLE */
    ...
    Status = NARE_EDDSendARP(pHDB,
                             pRCtrl,
                             (NARE_LOCAL_MEM_IP_CONST_PTR_TYPE) &pRCtrl->Sniffer.DestIP.b.IPAddr,
                             (NARE_LOCAL_MEM_IP_CONST_PTR_TYPE) &pRCtrl->Sniffer.LocalIP.b.IPAddr);
    if ( Status == NARE_RSP_OK )
    {   /* Setup EndTime for sniffing. We add 1 tick to   */
        ...
        if((NARE_RQB_GET_OPCODE(pRCtrl->pRCtrl->pRQB) == NARE_OPC_TEST_IP))
        {
            ...
            if(pRCtrl->pRCtrl->pRQB->args.TestIP.UseDelay == NARE_USE_DELAY_YES)
            {   /* This would be the first one wait 2..3.seconds   */
                ...
                pRCtrl->pRCtrl->pRQB->args.TestIP.UseDelay = NARE_USE_DELAY_NO;
            }
            else
            {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
static NARE_RSP NARE_EDDTrigger(
    NARE_HDB_EDD_PTR_TYPE     pHDB,
    NARE_EDD_RCTRL_PTR_TYPE   pRCtrl)
{
    NARE_USER_RCTRL_PTR_TYPE  pRCtrlUser;
    ...
    switch (*pRCtrlUser->pRState)
    {
        case NARE_RCTRL_RSTATE_EDD_TEST_IP_ARP:
            ...
            {
                pRCtrl->BurstCnt = 1;
                Status = NARE_EDDStartSniffer(pHDB,pRCtrl,NARE_SNIFF_MODE_MULTIPLE);
            }
            else if (pRCtrlUser->pRQB->args.TestIP.SniffMode == NARE_SNIFF_MODE_SINGLE)
            {
                ...
                pRCtrl->SentARPs = 0;
                Status = NARE_EDDStartSniffer(pHDB,pRCtrl,NARE_SNIFF_MODE_SINGLE);
            }
            else
            {
...
/*+               pending-queue.                                            +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID NARE_EDDCheckAndStartPending(
    NARE_HDB_EDD_PTR_TYPE     pHDB)
{
    NARE_USER_RCTRL_PTR_TYPE  pRCtrlUser;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
/*lint -save -esym(818, pOwnMacAddr) Request 1596366*/
static NARE_RSP  NARE_EDDFillARPRequest(
        ...
        if ( Result == NARE_RSP_OK)
        {
            ...
            if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
            {
                ...
                if ( ! LSA_HOST_PTR_ARE_EQUAL(pSend, LSA_NULL) )
                {
                    ...
                    pRCtrl->ARP_IO_DONE = LSA_FALSE;
                    NARE_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Request: EDD_SRV_NRT_SEND (Handle: 0x%X,EDDHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->EDDHandle,pRQB);
                    /* AP01248034 : use an initial delay If opc = NARE_OPC_TEST_IP and UseDelay == NARE_USE_DELAY_YES */
                    /* so we skip sending in this case ! */
                    if((ReqOpcode == NARE_OPC_TEST_IP) && (pRCtrl->pRCtrl->pRQB->args.TestIP.UseDelay == NARE_USE_DELAY_YES))
                    {
