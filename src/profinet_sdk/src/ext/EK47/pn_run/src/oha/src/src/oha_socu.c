...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestSockControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    LSA_UINT16                Response = OHA_OK_ACTIVE;
    OHA_HDB_SOCK_PTR_TYPE     pHDBSock;
    LSA_UINT32                ControlMode;

#ifdef OHA_SOCK_SNMP_OFF
    OHA_HDB_USER_PTR_TYPE     pHDBSnmpUser = OHA_SnmpGetEventUser();
#endif
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBSock,  LSA_NULL) )
    {
        ...
        OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_UNEXP,"Request Activate/Deactivate SNMP-Agent failed. No Lower Channel for SOCK");
    }
#ifdef OHA_SOCK_SNMP_OFF
    else if(OHA_IS_NULL(pHDBSnmpUser)) /* we have no user for SNMP.Activate */
    {
        Response = OHA_ERR_PRM_INDEX;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_UNEXP,"Writing SnmpControlRecord failed. No user for SNMP");
    }
#endif
    else
    {
        ...
        switch ( ControlMode )
        {
            ...
            case OHA_CTRL_SNMP_DEACTIVATE:
                ...
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"SNMP-Control (0x%X) SNMP-Agent failed. Already one User-Request active", ControlMode);
                    Response = OHA_ERR_SEQUENCE;
                    break;
                }
#ifdef OHA_SOCK_SNMP_OFF
                Response = OHA_SnmpIndicateEvent(pHDBSnmpUser, ControlMode);/* SnmpMode to change - indication */
                if ( Response == OHA_OK )  /* no error, no differences for SNMP indication */
                {
                    oha_database.SNMPActivation      = ControlMode;
                }
#endif
                break;
            default:
                OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_UNEXP,"SNMP-Control-Mode(0x%X) failed.[0x%X]",ControlMode, OHA_ERR_SYNTAX);
                Response = OHA_ERR_SYNTAX;
                break;
        }
    }

    if ( Response != OHA_OK_ACTIVE)  /* finish the request ? */
    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
OHA_HDB_SOCK_PTR_TYPE OHA_SOCKGetHandleGlobal(
    LSA_VOID)
{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_SOCKUserCheckPending(
    OHA_HDB_SOCK_PTR_TYPE   pHDBSock,
    LSA_UINT16              Response)
{
    OHA_HDB_USER_PTR_TYPE     pHDBUser;
...
/*+               we start the handling for this RQB.                       +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_SOCKUserSetMib2ParamsPending(
	OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response)
{
    OHA_FUNCTION_TRACE_02(0, LSA_TRACE_LEVEL_CHAT,
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_SOCKRequestHandler(
    OHA_HDB_SOCK_PTR_TYPE           pHDBSock,
    OHA_OID                         MibNode,
    LSA_UINT16                      Response)
{
    ...
    while (! Ready)
    {
        ...
        switch (pSubAgnt->RequestState)
        {
            case OHA_SNMP_STATE_STOP:   /* initial, after OpenChannel: Agent is in Deact */
				OHA_PROGRAM_TRACE_02(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SOCK-SM [%X]: STATE_STOP (Resp: %Xh)",pHDBSock->ThisHandle,Response);
            ...
            case OHA_SNMP_STATE_WF_START:   /* OHA_OPC_START_SNMP.Req is active    */
                OHA_PROGRAM_TRACE_02(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SOCK-SM [%X]: STATE_WF_START (Resp: %Xh)",pHDBSock->ThisHandle,Response);
                ...
                else
                {
                    ...
                    if (!pSubAgnt->RxPendingCnt) /* no Recv-Requests pending within SOCK (first Activate-Call ?) */
                    {
                        ...
                        LSA_UINT16 RetVal = OHA_SOCKRecvProvide(pHDBSock, MibNode, OHA_SNMP_NUM_OF_RECV);
                        if ( RetVal != OHA_OK )
                        {
                            /* Any Recv-Request failed. Cancel Recv-Requests (by DeactAgent) */
                            OHA_PROGRAM_TRACE_01(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"Any SOCK Receive Provide request failed. Response: 0x%X", Response);
                        }
                    }
            ...
            case OHA_SNMP_STATE_WF_STOP:     /* OHA_OPC_STOP_SNMP.Req is active   */
                OHA_PROGRAM_TRACE_02(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SOCK-SM [%X]: STATE_WF_STOP (Resp: %Xh)",pHDBSock->ThisHandle,Response);
                ...
                else
                {
                    ...
                    OHA_SOCKFreeAgent(pSubAgnt, pHDBSock->Params.pSys);
                    pSubAgnt->RequestState = OHA_SNMP_STATE_STOP;
                }
                /* confirm to the user, if exist */
                /* check for pending user-requests   */
            ...
            case OHA_SNMP_STATE_WF_STATISTICS:
                OHA_PROGRAM_TRACE_02(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SOCK-SM [%X]: STATE_WF_STAT (Resp: %Xh)",pSubAgnt->RequestState,Response);
                /*FALLTHROUGH*/
			case OHA_SNMP_STATE_WF_USER:
                /* no break, intentionally left ! */

            case OHA_SNMP_STATE_START:      /* SNMP-agent is active     */
                OHA_PROGRAM_TRACE_02(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"SOCK-SM [%X]: STATE_START (Resp: %Xh)",pSubAgnt->RequestState,Response);
            ...
            default:
                OHA_PROGRAM_TRACE_01(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA_SockRequestHandler(): Invalid RState (%Xh)",pSubAgnt->RequestState);
                OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                break;
        }
    }
...
/*+               we start the handling for this RQB.                       +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_SOCKUserSetIpPending(
    OHA_HDB_USER_PTR_TYPE   pHDBUser,
    LSA_UINT16              Response)
{
    OHA_UPPER_RQB_PTR_TYPE  pRQBUser;
	...
	if (OHA_IS_NULL(pHDBUser->SockUserReq.pBottom)) /* 5085999 */
	{
		OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "An OHA controlled DCP_SET was pending (User: 0x%X)", pHDBUser->ThisHandle);
		return;
	}
    ...
    switch (OHA_RQB_GET_OPCODE(pRQBUser))
    {
        ...
        {
			OHA_PROGRAM_TRACE_03(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"<<<: OHA_SOCKUserSetIpPending done (NICId: 0x%X,Resp: 0x%X,Opcode: 0x%X)",NicId,Response,OHA_RQB_GET_OPCODE(pRQBUser));
			...
			if(OHA_CheckIpEnabled(NicId)) /* AP01522573: ip only */
			{
        ...
        case OHA_OPC_DCP_RESPONSE:
        {
			...
			if(OHA_CheckIpEnabled(NicId)) /* IP to check ? */
			{
                CancelOptions |= OHA_CTRL_IP_SUITE;
			}
			if ( (CancelOptions & OHA_CTRL_RESET_TO_FACTORY) ||
...
#else
        if(pHDBSock->ControlUser == pHDBUser->ThisHandle) /* no event user anymore */
        {
            OHA_SOCKUserCheckPending(pHDBSock, OHA_ERR_SEQUENCE);
        }
#endif
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestControlTcipData(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    LSA_UINT16                Response;
    ...
    if ( LSA_HOST_PTR_ARE_EQUAL(pHDBSock,  LSA_NULL) )
    {
        Response = OHA_ERR_PARAM;
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_TCIP_CONTROL failed. No such Lower Channel for SOCK");
    }
    else if (!(pHDBUser->State == OHA_HANDLE_STATE_OPEN &&  /* is no open/close user-channel runnning ? */
               LSA_HOST_PTR_ARE_EQUAL(pHDBUser->pOCReq, LSA_NULL) ) )
    {
        Response = OHA_ERR_SEQUENCE;
		OHA_PROGRAM_TRACE_01(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_TCIP_CONTROL failed. User Channel is closing or not open for this Handle (NIC-ID: 0x%X)", pHDBUser->ThisHandle);
    }
	...
	{
        Response = OHA_ERR_PARAM;
        OHA_PROGRAM_TRACE_01(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_TCIP_CONTROL failed. Invalid parameter TimeOption (0x%X)",pRQB->args.TcipData.TimeOption);
	}
    else
    {
		...
		if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQBSock, LSA_NULL) )
		{
			...
			if (pHDBSock->TcIpRQBCnt != 0) /* SockCntrlTimeParams already active ? */
			{
				...
				OHA_RQB_PUT_BLOCK_TOP(pHDBSock->TcIpUserReq.pBottom,
									  ...
									  pRQB);
				Response = OHA_OK;
				OHA_FUNCTION_TRACE_01(pHDBSock->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestControlTcipData: 0x%X)", Response);
				return;
			}
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestControlTcipDataDone(
    ...
    OHA_HDB_SOCK_PTR_TYPE        pHDB)
{
	...
	OHA_ASSERT(pHDB->TcIpRQBCnt==1); /* no queueing to SOCK */
    ...
    OHA_ASSERT_NULL_PTR(pHDBSock->TcIpUserReq.pBottom); /* User-RCTRL-Blocks pending ? */

	OHA_RQB_REM_BLOCK_BOTTOM(pHDBSock->TcIpUserReq.pBottom,
							 pHDBSock->TcIpUserReq.pTop,
							 pRQBUser);

	OHA_LOWER_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: SOCK_OPC_CNTRL_TIMEPARAMS done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_SOCK_RQB_GET_RESPONSE(pRQB));
    ...
    if (Response != OHA_OK)
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"SOCK-ERROR: SOCK_OPC_CNTRL_TIMEPARAMS failed.(0x%X).",OHA_SOCK_RQB_GET_RESPONSE(pRQB));
    }
	...
	while (OHA_IS_NOT_NULL(pHDBSock->TcIpUserReq.pBottom) && (pHDB->TcIpRQBCnt == 0))
	{
		...
		case OHA_OPC_PRM_WRITE:
			OHA_PrmControlTCP(pNextRQB, pNextHDB);
			break;
		case OHA_OPC_TCIP_CONTROL:
			OHA_RequestControlTcipData(pNextRQB, pNextHDB);
			break;
		default:
			OHA_PROGRAM_TRACE_01(pNextHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_RequestControlTcipDataDone: Invalid Opcode in queue (0x%X)", OHA_RQB_GET_OPCODE(pNextRQB));
			OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
			break;
		}
	}
