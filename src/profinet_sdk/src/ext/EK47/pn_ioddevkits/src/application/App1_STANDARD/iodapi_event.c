    ...
    void PrintAllUsedIoData (void)
    {
	    ...
	    for (SlotNum = 0; SlotNum < NUMOF_SLOTS; SlotNum++)			// loop over all slots
	    {
		    for (SubNum = 0; SubNum < NUMOF_SUBSLOTS; SubNum++)	// loop over all subslots
		    {
		      ...
		      {
			    DataAvailable = 1;
			    PNIO_ConsolePrintf ( (PNIO_INT8*) "##DatIn Slot=%d  Sub=%d length=%d : ",
				        SlotNum,					// slot num
					    SubNum,						// subslot num
			    ...
			    for (IoInd=0; IoInd < InDatLen[SlotNum][SubNum]; IoInd++)
			    {
			        PNIO_ConsolePrintf ( (PNIO_INT8*) "0x%02x ", InData[SlotNum][SubNum][IoInd]);
			    }
	    ...
	    for (SlotNum = 0; SlotNum < NUMOF_SLOTS; SlotNum++)			// loop over all slots
	    {
		    for (SubNum = 0; SubNum < NUMOF_SUBSLOTS; SubNum++)	// loop over all subslots
		    {
		      ...
		      {
			    DataAvailable = 1;
			    PNIO_ConsolePrintf ( (PNIO_INT8*) "##DatOut Slot=%d  Sub=%d length=%d : ",
				        SlotNum,					// slot num
					    SubNum,						// subslot num
			    ...
			    {
			        PNIO_ConsolePrintf ( (PNIO_INT8*) "0x%02X ", OutData[SlotNum][SubNum][IoInd]);
			    }
			    PNIO_ConsolePrintf ( (PNIO_INT8*) "\n");
		      }
		    }
