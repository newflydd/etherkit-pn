	...
	)
{
	...
	ACP_ASSERT (acp_is_not_null (edd_nrt));
	...
	if (acp_is_null (edd_nrt->pBuffer)) {
	...
	)
{
	...
	ACP_ASSERT (acp_is_not_null (lower));
	ACP_ASSERT (acp_is_not_null (*lower));
	...
	)
{
	...
	ACP_ASSERT (edd_send->Length <= EDD_IEEE_FRAME_LENGTH);
	...
	)
{
	...
	ACP_ASSERT (acp_is_not_null(rtasm->rta));
	ACP_ASSERT (acp_is_not_null(rtasm->rta->channel));
...
/*----------------------------------------------------------------------------*/
static LSA_VOID
acp_nrt_recv_check_and_queue (
	ACP_LOWER_PTR  channel_lower,
	ACP_LIST_ENTRY_TYPE *lower_list,
	ACP_LOWER_RQB_PTR_TYPE	nrt,
	ACP_RTA_HEADER_TYPE rta_header
	)
{
	ACP_LOWER_RQB_PTR_TYPE last_nrt;
	ACP_RTA_HEADER_TYPE last_rta_header;

	if(!AcpListIsEmpty(lower_list)) {
		LSA_BOOL rc;
		ACP_LL_NRT_RECV_PTR_TYPE edd_recv;

		last_nrt = AcpListPeekTail(lower_list, ACP_LOWER_RQB_PTR_TYPE);

		ACP_ASSERT(acp_is_not_null(last_nrt));

		edd_recv = ACP_LL_GET_PPARAM(last_nrt, ACP_LL_NRT_RECV_PTR_TYPE);
		rc = acp_rta_header_parse(channel_lower, edd_recv->pBuffer, edd_recv->UserDataOffset, edd_recv->UserDataLength, &last_rta_header);
		LSA_UNUSED_ARG(rc); /* rc checked when added to queue */

		if(rta_header.type == ACP_RTA_PDU_TYPE_DATA
			&& rta_header.type == last_rta_header.type
			&& rta_header.send_seq_nr == last_rta_header.send_seq_nr
			&& rta_header.ack_seq_nr == last_rta_header.ack_seq_nr) {

			ACP_RTA_TRACE_01(channel_lower->trace_idx, LSA_TRACE_LEVEL_NOTE_HIGH
				, "nrt(0x%x) duplicate: reprovide", nrt
				);
	...
	AcpListInsertTailT(lower_list, nrt);
}
	...
	)
{
	...
	ACP_ASSERT (acp_is_not_null (edd_recv)); /* Lower-RQB-Error */
...
ACP_FATAL (ACP_ERR_UNSUPPORTED);
...
#if ACP_FRAG_MODE
	case ACP_RTA_ST_WAIT_ACK:
	case ACP_RTA_ST_WAIT_DATA:
#endif
		break;
	...
	case ACP_RTA_ST_ERROR_CNF:
		...
		acp_nrt_recv_check_and_queue(channel_lower, &rtasm->lower_list, nrt, rta_hdr);
		return;
...
#if ACP_FRAG_MODE
		if(rtasm->state == ACP_RTA_ST_WAIT_DATA)
		{
			ACP_RTA_TRACE_04 (channel_lower->trace_idx, LSA_TRACE_LEVEL_WARN
				, "rta: aref(%u/%u) dst_alarm_ref(%d) != rta_hdr.src_alarm_ref(%d)"
				, rtasm->rta->alarm_ref, rtasm->prio
...
#if ACP_FRAG_MODE
		if(rtasm->prio == ACP_ASRT_PRIORITY_FRAG)
		{
			acp_frag_recv_ack (rta->channel, rtasm, nrt, & rta_hdr);
			break;
		}
#endif
...
#if ACP_FRAG_MODE
	case ACP_RTA_PDU_TYPE_FRAG:
		acp_frag_recv_data(rta->channel, rtasm, nrt, & rta_hdr);
		break;
#endif
	...
	)
{
	...
	if (rtasm->prio == ACP_ALARM_PRIORITY_HIGH) {
		frame_id = EDD_SRT_FRAMEID_ASRT_PNIO_ALARM_HIGH;
	}
	else {
		frame_id = EDD_SRT_FRAMEID_ASRT_PNIO_ALARM_LOW;
	}

#if ACP_FRAG_MODE
if (rtasm->prio == ACP_ASRT_PRIORITY_FRAG) {
	frame_id = EDD_SRT_FRAMEID_ASRT_PNIO_FRAG;
}
#endif
...
#if ACP_FRAG_MODE
	case ACP_RTA_PDU_TYPE_FRAG:
		add_flags = ACP_RTA_ADD_FLAGS_WINDOWS_SIZE;
		ACP_ASSERT (var_part_len > 0);
		ACP_ASSERT (rtasm->frag_offset+(var_part_len - ACP_PDU_FRAG_HEADER_LENGTH)<= rtasm->data_length);

		if(rtasm->frag_offset+rtasm->payload_sent == rtasm->data_length)
		{
			add_flags &= ~ACP_RTA_ADD_FLAGS_MORE_FRAG;
		}
		else
		{
			add_flags |= ACP_RTA_ADD_FLAGS_MORE_FRAG;
		...
		add_flags |= ACP_RTA_ADD_FLAGS_TACK;
	...
	)
{
	...
	switch (frame_id) {
	case EDD_SRT_FRAMEID_ASRT_PNIO_ALARM_HIGH:		hdr->prio = ACP_ALARM_PRIORITY_HIGH; break;
	case EDD_SRT_FRAMEID_ASRT_PNIO_ALARM_LOW:		hdr->prio = ACP_ALARM_PRIORITY_LOW; break;

#if ACP_FRAG_MODE
	case EDD_SRT_FRAMEID_ASRT_PNIO_FRAG:			hdr->prio = ACP_ASRT_PRIORITY_FRAG; break;
#endif
...
#if ACP_FRAG_MODE
	case ACP_RTA_PDU_TYPE_FRAG:
		if (1 > hdr->var_part_len  ||  hdr->var_part_len > 1432) { //frag open use define
			ACP_RTA_TRACE_02 (channel_lower->trace_idx, LSA_TRACE_LEVEL_UNEXP
				, "rta: FRAG-PDU rta.type(%u), with wrong VarPartLen(%u)"
				, hdr->type, hdr->var_part_len
				);
			return LSA_FALSE;
		}
	...
	case ACP_RTA_PDU_TYPE_ERROR:
		if (hdr->var_part_len == sizeof(LSA_UINT32))
		{
			...
			{
				LSA_UINT32 vendor_device, pn_version, dd1, dd2, dd3, dd4;
				acp_dp_get_u32 (vendor_device, dp);
				acp_dp_get_u32 (pn_version, dp);
				...
				ACP_RTA_TRACE_07 (channel_lower->trace_idx, LSA_TRACE_LEVEL_WARN
					...
					, hdr->pnio_status, vendor_device, pn_version, dd1, dd2, dd3, dd4
					);
			}
		}
		else {
	...
	)
{
	...
	AcpListInitialize(&rtasm->lower_list);
	rtasm->wf_ack			= 0;
	rtasm->wf_timeout		= 0;
...
#if ACP_FRAG_MODE
	rtasm->last_recv_frag	= ACP_FRAG_RECV_IN_WORK;
	rtasm->frag_offset		= 0;
	rtasm->data_length		= 0;
#endif
	...
	)
{
	...
	while(!AcpListIsEmpty(&rta->send_list[rtasm->prio])) {
		AcpListRemoveHead(&rta->send_list[rtasm->prio], rb, ACP_UPPER_RQB_PTR_TYPE);
		acp_callback_async(channel, ACP_OK_CANCEL, &rb);
	}

#if ACP_FRAG_MODE
	while(!AcpListIsEmpty(&rta->frag_provide_list)) {
		AcpListRemoveHead(&rta->frag_provide_list, rb, ACP_UPPER_RQB_PTR_TYPE);
	...
	if ((rtasm->upper)) {
		...
		ACP_ASSERT ( /* AP00294144 */
			...
			||	ACP_RQB_GET_OPCODE (rtasm->upper) == ACP_OPC_FRAG_RECV
			);
#else
		ACP_ASSERT ( /* AP00294144 */
				ACP_RQB_GET_OPCODE(rtasm->upper) == ACP_OPC_ALARM_INDICATION
	...
	while(!AcpListIsEmpty(&rtasm->lower_list)) {
		AcpListRemoveHead(&rtasm->lower_list, nrt, ACP_LOWER_RQB_PTR_TYPE);
	...
	)
{
	...
	edd_send = ACP_LL_GET_PPARAM (nrt, ACP_LL_NRT_SEND_PTR_TYPE);
	ACP_ASSERT (acp_is_not_null (edd_send));
	...
	ACP_ASSERT (alarm_offset < EDD_IEEE_FRAME_LENGTH);

	alarm_length = acp_alarm_build_data (channel, alarm, rtasm->prio,
			edd_send->pBuffer + alarm_offset,
			ACP_CAST_U16 (EDD_IEEE_FRAME_LENGTH - alarm_offset)
		);
	...
	)
{
	ACP_LOCAL_ALARM_PTR  alarm;
	ACP_LL_NRT_SEND_PTR_TYPE  edd_send;
	LSA_UINT16  frag_length;
	LSA_UINT32  offset;
	LSA_UINT16  frag_header_offset;
	LSA_UINT16  payload_length;

	ACP_ASSERT (rtasm->state == ACP_RTA_ST_DATA_Q);
	ACP_ASSERT (rtasm->rta->on_closing == 0);
	ACP_ASSERT (rtasm->prio == ACP_ASRT_PRIORITY_FRAG);
	...
	frag_header_offset = ACP_PDU_ETHERNET_HEADER_LENGTH;
	offset = frag_header_offset + ACP_PDU_RTA_HEADER_LENGTH;

	ACP_ASSERT (offset < EDD_IEEE_FRAME_LENGTH);

	payload_length = acp_frag_build (channel, alarm, edd_send->pBuffer+ offset);
	frag_length = payload_length + ACP_PDU_FRAG_HEADER_LENGTH;
	rtasm->payload_sent = payload_length;

	acp_rta_header_build_2 (channel, rtasm, edd_send->pBuffer + ACP_PDU_ETHERNET_HEADER_LENGTH,	ACP_RTA_PDU_TYPE_FRAG, frag_length);

	acp_ethernet_header_build (channel, rtasm, edd_send->pBuffer, ACP_ETHERTYPE_RT);

	edd_send->Length = offset + frag_length;
	...
	)
{
	...
	if (rtasm->rta->on_closing) {
		rtasm->wf_ack = 0;
		ACP_ASSERT(AcpListIsEmpty(&rtasm->rta->send_list[rtasm->prio])); /* see acp_rta_sm_stop */
	}
	else {
		if (rsp == ACP_OK) {
			rtasm->wf_ack = 1;
...
#if ACP_FRAG_MODE
	if(rtasm->prio == ACP_ASRT_PRIORITY_FRAG && rtasm->last_recv_frag == ACP_FRAG_RECV_IN_WORK)
	{
		rtasm->state = ACP_RTA_ST_WAIT_ACK;
	}
#endif
	...
	)
{
	...
	if(rtasm->prio == ACP_ASRT_PRIORITY_FRAG)
	{
		ACP_ASSERT(acp_is_not_null (rtasm->upper));

		if(rtasm->last_recv_frag == ACP_FRAG_RECV_IN_WORK)
		{
			rtasm->state = ACP_RTA_ST_WAIT_DATA;

			//timer because we are waiting for another fragment to come
			acp_oneshot_trigger (& rtasm->oneshot_timer, rtasm->rta->timeout_ms*3);
		}
		else
		{
			rtasm->state = ACP_RTA_ST_OPEN;

			AcpListRemoveEntry ((ACP_LIST_ENTRY_PTR_TYPE) rtasm->upper);
			acp_callback_async(channel,ACP_OK,&rtasm->upper);
		}
	...
	if (acp_is_not_null (rtasm->upper)) {
		if (rsp != ACP_OK) {
	...
	ACP_ASSERT (acp_is_null (rtasm->upper));
	...
	)
{
	...
	if (! rta->is_udp)
	{
		...
		if (acp_is_not_null (rb) &&	ACP_RQB_GET_OPCODE(rb) == ACP_OPC_ALARM_CLOSE  /* sanity */
		&&	rb->args->alarm.close.error_info.pnio_version != 0)
		{
			...
			pn_version = rb->args->alarm.close.error_info.pnio_version;
			...
			ACP_RTA_TRACE_08 (channel->trace_idx, LSA_TRACE_LEVEL_WARN
				...
				, rta->alarm_ref, rtasm->prio, pnio_status, pn_version, dd1, dd2, dd3, dd4
				);
			}
		}
	}
...
#if ACP_FRAG_MODE
		if (rta->rta_sm[ACP_ALARM_PRIORITY_LOW].state == ACP_RTA_ST_FREE
		 && rta->rta_sm[ACP_ALARM_PRIORITY_HIGH].state == ACP_RTA_ST_FREE
		 && rta->rta_sm[ACP_ASRT_PRIORITY_FRAG].state == ACP_RTA_ST_FREE) {
#else
	...
	)
{
	...
	case ACP_RTA_ST_OPEN:
		...
		upper = AcpListIterFirst (&channel->alarm_provide[hdr->prio], ACP_UPPER_RQB_PTR_TYPE);
		...
		edd_recv = ACP_LL_GET_PPARAM (nrt, ACP_LL_NRT_RECV_PTR_TYPE);
		ACP_ASSERT (acp_is_not_null (edd_recv));
...
acp_frag_recv_data (
	...
	ACP_ASSERT (alarm->on_closing == 0);
	ACP_ASSERT(hdr->prio == ACP_ASRT_PRIORITY_FRAG);

	switch (rtasm->state) {
	case ACP_RTA_ST_OPEN:
	{
		upper = AcpListIterFirst(&alarm->frag_provide_list, ACP_UPPER_RQB_PTR_TYPE);
		...
		rtasm->data_length = 0; //not set until now
		rtasm->state = ACP_RTA_ST_WAIT_DATA;
		rtasm->prio = ACP_ASRT_PRIORITY_FRAG;
		rtasm->upper_buffer_len =  upper->args->alarm.frag_recv.data_max_length;
		rtasm->state = ACP_RTA_ST_WAIT_DATA;
	}
	case ACP_RTA_ST_WAIT_DATA:
		...
		if (rtasm->wf_ack) {
			...
			if (hdr->ack_seq_nr == rtasm->tx_seq_num) {
				...
				if(!(hdr->add_flags & ACP_RTA_ADD_FLAGS_MORE_FRAG))
				{
					acp_rta_send_done (channel, rtasm->rta, rtasm->prio, ACP_OK, ACP_PNIO_STATUS_OK);
				}
		...
		// is stopping if it isn't the first one.
		acp_oneshot_stop (& rtasm->oneshot_timer);
		...
		/* Check Frag-Frame*/
		rc = acp_frag_indication_check_data(alarm, upper, edd_recv->pBuffer+edd_recv->UserDataOffset+ACP_PDU_RTA_HEADER_LENGTH, hdr, rtasm);

		acp_nrt_recv_provide (channel->lower, nrt);

		/* keep upper and send ack, upper will send to acp_user on acp_rta_sm_send_ack_done */
		rtasm->upper = upper;

		if (rc != ACP_OK || !(hdr->add_flags  & ACP_RTA_ADD_FLAGS_TACK))
		{
			ACP_RTA_TRACE_03 (channel->trace_idx, LSA_TRACE_LEVEL_UNEXP
				 , "rta: wrong frag payload parameters of indication aref(%u) astate(%u) wf_ack(%u)"
				 , alarm->alarm_ref, rtasm->state, rtasm->wf_ack
				 );
			acp_rta_sm_state_enter (channel, rtasm);
			return;
		}
		...
		if(!(hdr->add_flags & ACP_RTA_ADD_FLAGS_MORE_FRAG))
		{
			rtasm->last_recv_frag = ACP_FRAG_RECV_COMPLETED; //is the last frag element
			upper->args->alarm.frag_recv.data_length = rtasm->data_length;
			ACP_RTA_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_UNEXP
				, "rta: No more frags will come.(%u)"
				, alarm->alarm_ref
				);
		}
		else
		{
			rtasm->last_recv_frag = ACP_FRAG_RECV_IN_WORK; //is not the last frag element
		}

		rtasm->state = ACP_RTA_ST_ACK_Q;
		ACP_RTA_TRACE_01 (channel->trace_idx, LSA_TRACE_LEVEL_UNEXP
			, "rta: TACK was set in incoming frag rqb with alarm_ref(%u)"
			, alarm->alarm_ref
			);
		acp_rta_sm_q_or_send (channel, rtasm); // ==> acp_rta_sm_q_or_send_wakeup()
...
{
	ACP_RTA_TRACE_05 (channel->trace_idx, LSA_TRACE_LEVEL_CHAT
		, "rta-frag: aref(%u/%u)  wf_ack(%d) hdr.ack_seq_nr(0x%04x) tx_seq_num(0x%04x)"
		, rtasm->rta->alarm_ref
		, rtasm->prio
		, rtasm->wf_ack
		, hdr->ack_seq_nr
	...
	case ACP_RTA_ST_WAIT_ACK:
		...
		if (hdr->ack_seq_nr == rtasm->tx_seq_num) {
			acp_oneshot_stop (& rtasm->oneshot_timer);
			rtasm->frag_offset += rtasm->payload_sent;

			if(rtasm->frag_offset == rtasm->data_length)
			{
				/* callback */
				rtasm->state = ACP_RTA_ST_OPEN;
				acp_rta_send_done (channel, rtasm->rta, rtasm->prio, ACP_OK, ACP_PNIO_STATUS_OK);
			...
			acp_rta_sm_q_or_send(channel,rtasm);
			return;
		}
...
#if ACP_FRAG_MODE
		if (ACP_RQB_GET_OPCODE(upper) == ACP_OPC_FRAG_RECV)
		{
			acp_rta_sm_state_enter(channel, rtasm);
			break;
		}
	...
	)
{
	...
	ACP_ASSERT (acp_are_equal (& rtasm->oneshot_timer, oneshot));
...
#if ACP_FRAG_MODE
				rtasm->state = ACP_RTA_ST_OPEN; //do not wait for ack anymore
#endif
	...
	)
{
	...
	case ACP_RTA_ST_WAIT_DATA:
		...
		if (!AcpListIsEmpty(&rtasm->lower_list)) {
			ACP_LOWER_RQB_PTR_TYPE  nrt;
		...
		if(!AcpListIsEmpty(&rtasm->rta->send_list[rtasm->prio])) {
			if (rtasm->wf_ack) {
				ACP_RTA_TRACE_02 (channel->trace_idx, LSA_TRACE_LEVEL_CHAT
					, "rta: aref(%u/%u) wf_ack"
					, rtasm->rta->alarm_ref
	...
	)
{
	...
	case ACP_RTA_ST_DATA_Q:
		...
		if(rtasm->prio == ACP_ASRT_PRIORITY_FRAG)
		{
			acp_frag_send_data (channel,rtasm,nrt);
			return;
		}
#endif
	...
	)
{
	...
	ACP_ASSERT (acp_is_not_null (rtasm));
...
{
	ACP_ASSERT (acp_is_not_null (nrt));
	ACP_ASSERT (acp_is_not_null (nrt->pParam));
...
#if ACP_FRAG_MODE
	acp_rta_sm_init (channel, & rta->rta_sm[ACP_ALARM_PRIORITY_LOW], rta, ACP_ALARM_PRIORITY_LOW, retry_count);

	acp_rta_sm_init (channel, & rta->rta_sm[ACP_ALARM_PRIORITY_HIGH], rta, ACP_ALARM_PRIORITY_HIGH, retry_count);

	acp_rta_sm_init (channel, & rta->rta_sm[ACP_ASRT_PRIORITY_FRAG], rta, ACP_ASRT_PRIORITY_FRAG, retry_count);
#else
...
#if ACP_FRAG_MODE
	rtasm = acp_rta_sm_from_rta (rta, ACP_ASRT_PRIORITY_FRAG);
	acp_rta_sm_stop (channel, rtasm);
#endif
	...
	)
{
	...
	ACP_ASSERT (!AcpListIsEmpty(&rta->send_list[prio]));
...
/* more cond ... */
...
#if ACP_FRAG_MODE
		ACP_ASSERT(rtasm->frag_offset > 0);
		ACP_ASSERT (rtasm->wf_ack == 0);
	...
	case ACP_RTA_ST_WAIT_DATA:
		ACP_ASSERT(prio == ACP_ASRT_PRIORITY_FRAG);
		return;
	case ACP_RTA_ST_WAIT_ACK:
#endif
	...
	)
{
	...
	for (i = 0; i < ACP_CFG_MAX_CHANNELS; ++i)
	{
		...
		ACP_ASSERT (acp_are_equal (channel->lower, channel_lower));
