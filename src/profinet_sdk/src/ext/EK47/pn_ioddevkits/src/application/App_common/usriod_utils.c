...
PNIO_UINT32 PrintDevkitVersion (void)
{
    ...
    PNIO_ConsolePrintf ( (PNIO_INT8*) "PNIO DEVKIT appl.-example no.%d, %c %d.%d.%d.%d, %s - %s\n",
...
void InputAndStoreMacAddress (void)
 {
	 ...
	 {
	     PNIO_printf ( (PNIO_INT8*) "Mac address was not stored - wrong MAC address or same MAC address \n");
	 }
	 else
	 {
    ...
    if (0 == NumOfAr)
    {
        ...
        if ( USR_ADDR_TEST_OK == TestResult)
        {
            ...
            else
            {
                ...
                PNIO_change_ip_suite( ( *( Unsigned32* )IpSuite ), ( *( Unsigned32* )( IpSuite + 4 ) ), ( *( Unsigned32* )( IpSuite + 8 ) ) );
            }
...
PNIO_VOID PrintSubmodules( PNIO_VOID )
{
	...
	for (i = 0; i < PnpbMod.NumOfPluggedSub; i++)
	{
		if (PnpbMod.Sub[i].PlugState == PNPB_PLUGGED)
		{
			PNIO_ConsolePrintf ( (PNIO_INT8*) "%02d Slot=%02d Sub=%04x ModID=%02x \n",
					              i,
    ...
    if( 0 == NumOfAr)
    {
        ...
        if( USR_ADDR_TEST_OK == TestResult )
        {
            ...
            else
            {
                ...
                PNIO_change_device_name(( PNIO_INT8* )DevName, DevNameLen);
            }
