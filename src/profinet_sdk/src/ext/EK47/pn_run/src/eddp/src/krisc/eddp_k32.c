    ...
    EDDP_UPPER_DSB_PTR_TYPE            pDSB)
{
    ...
    LSA_UINT16  Idx;
    LSA_UINT32  tcmValue = 0;   /* only DTCM */
    EDD_RSP     Status = EDD_STS_OK;
    LSA_UINT8   kriscFWVersion = 0; /* store what FW we'll use */
    ...
    if ((EDDP_HW_ERTEC200P == pDDB->Hardware) || (EDDP_HW_ERTEC200P_FPGA == pDDB->Hardware))
    {
        ...
        EDDP_HAL_REG32_WRITE(pDDB,PNIP_REG_KRISC_TCM_MAP,tcmValue);
        /* hold reset */
        EDDP_K32_RESET(pDDB->hSysDev,LSA_TRUE);
    }
#endif

    /* prepare I-TCM / D-TCM ratio
    */
    ...
    switch (EDDP_HALGetPnipRevision(pDDB))
    {
        ...
        case EDDP_HAL_HW_PNIP_REV1:
            {
                kriscFWVersion = K32_FIRMWARE_REV1;
                EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "[H:--] --- EDDP_K32DeviceSetup(): Downloading Firmware Rev1");
                // Load K32 Rev1 FW
                tcmValue = (k32_code_rev1_bin_len + (((pDDB->K32.tileSize))-1)) / ((pDDB->K32.tileSize));
                if (K32_TCM_MAP_SIZE_REV1 < tcmValue)
                {
                    /* Instruction size is too big
                    */
                    EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] --- EDDP_K32DeviceSetup(): KRISC32 firmware code size is too big :-O ");
                    Status = EDD_STS_ERR_HW;
                    break;
                }
                /* Load firmware into KRISC32 and start it
                */
                Addr = EDDP_K32_FW_DOWNLOAD(pDDB,k32_code_rev1_bin,k32_code_rev1_bin_len,k32_data_rev1_bin,k32_data_rev1_bin_len);
            }
            break;
        ...
        case EDDP_HAL_HW_PNIP_REV2:
            {
                kriscFWVersion = K32_FIRMWARE_REV2;
                    EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "[H:--] --- EDDP_K32DeviceSetup(): Downloading Firmware Rev2");
                // Load K32 Rev2 FW
                tcmValue = (k32_code_rev2_bin_len + (((pDDB->K32.tileSize))-1)) / ((pDDB->K32.tileSize));
                if (K32_TCM_MAP_SIZE_REV2 < tcmValue)
                {
                    /* Instruction size is too big
                    */
                        EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_ERROR, "[H:--] --- EDDP_K32DeviceSetup(): KRISC32 firmware code size is too big :-O ");
                ...
                /* Load firmware into KRISC32 and start it
                */
                Addr = EDDP_K32_FW_DOWNLOAD(pDDB,k32_code_rev2_bin,k32_code_rev2_bin_len,k32_data_rev2_bin,k32_data_rev2_bin_len);
            }
            break;
#endif // EDDP_CFG_HW_PNIP_REV2_SUPPORT    
        default:
            {
                EDDP_K32_TRACE_01(pDDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "[H:--] EDDP_K32DeviceSetup(): FATAL -> Version(%d) of PNIP is unknown", 
...
//#error This does not work with K32_OPC_LOG at the moment! Please set K32_OPC_NO_OPERATION to zero!
    ...
    for(index=0;index<(int)(NUMBER_OF_ELEMENTS(queues));++index)
            ...
            /* LSA_TRACE_LEVEL_OFF: do not log (bit0 always zero)
             * LSA_TRACE_LEVEL_XXX: log level XXX (bitXXX downto bit1 is set)
             */
            {
                LSA_UINT32 tmp_mask = 0;
                LSA_INT ii;
                for (ii = 1; ii <= pDDB->K32.RRQBTraceLevel; ii++)
                {
                    tmp_mask |= 1 << ii;
                }
                /* set RRQB Trace Log Mask */
                EDDP_HAL_MEM32_WRITE(pDDB,&(partPtr->config.logMask), tmp_mask);
            }

            EDDP_HAL_MEM32_WRITE(pDDB,&(partPtr->config.partitionSize),partSize);
            EDDP_K32_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "[H:--] --- EDDP_K32DeviceSetup(): K32logMask=0x%08x K32partSize=%u",
                              EDDP_HAL_MEM32_READ(pDDB,&(partPtr->config.logMask)), partSize);
            ...
            if ((EDDP_HW_ERTEC200P == pDDB->Hardware) || (EDDP_HW_ERTEC200P_FPGA == pDDB->Hardware))
            {
                ...
                */
                ...
                /* release reset of KRISC32 cpu
                */
                EDDP_K32_RESET(pDDB->hSysDev,LSA_FALSE);
            }

            /** @note: since EDDP initializes RAM (CmdRAM/DTCM) we didn't wait here till KRISC32 is finished with startup. */
            if (K32_FIRMWARE_REV1 == kriscFWVersion)
            {
                EDDP_K32_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "[H:--] --- EDDP_K32DeviceSetup(): running KRISC32 (ITCM:DTCM = %d:%d)",
                    tcmValue, (K32_TCM_MAP_SIZE_REV1 - tcmValue));
            }
            else if (K32_FIRMWARE_REV2 == kriscFWVersion)
            {
                EDDP_K32_TRACE_02(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "[H:--] --- EDDP_K32DeviceSetup(): running KRISC32 (ITCM:DTCM = %d:%d)",
                    tcmValue, (K32_TCM_MAP_SIZE_REV2 - tcmValue));
            }
            else if (K32_FIRMWARE_REV3 == kriscFWVersion)
            {
                 EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "[H:--] --- EDDP_K32DeviceSetup(): running KRISC32 with HERA ");
            }
            else
            {
                 EDDP_K32_TRACE_00(pDDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_HIGH, "[H:--] --- EDDP_K32DeviceSetup(): running KRISC32 with DLL ");
            }
    ...
    K32_RQB_PTR_TYPE         pK32RQB)
{
    ...
    LSA_UINT8   InterfaceID; // = EDDP_HAL_MEM8_READ(pDDB, &(pK32RQB->Hdr.InterfaceID));
    ...
    InterfaceID = EDDP_HAL_MEM8_READ(pDDB, &(pK32RQB->Hdr.InterfaceID));
    ...
    case K32_OPC_LOG:
        {
            LSA_UINT    i;
            K32_TRACE_BUFFER* logBuffer = LSA_NULL;
#ifdef EDDP_USE_K32_DLL
            // 0211lrg:  pK32TCMBase + logBufferOffset ==> bad ptr
            logBuffer = (K32_TRACE_BUFFER*) EDDP_HAL_MEM32_READ(pDDB,&((pK32RQB->Params.Logging.logBufferOffset)));
#else
            if ((EDDP_HW_ERTEC200P == pDDB->Hardware) || (EDDP_HW_ERTEC200P_FPGA == pDDB->Hardware))
            {
                logBuffer = (K32_TRACE_BUFFER*) (((LSA_UINT8*)pDDB->K32.pK32TCMBase) + EDDP_HAL_MEM32_READ(pDDB,&(pK32RQB->Params.Logging.logBufferOffset)));
            }
#endif
#ifdef K32_LOG_TO_TRACE
            {
    ...
    K32_RQB_PTR_TYPE         pK32RQB)
{
    ...
    */
    ...
    EDDP_K32_TRACE_05(pDDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "[H:--] IN :EDDP_K32AckRQB(pK32RQB=0x%X Opcode=%u IfId=%u UserId=0x%X Response=0x%x)",
        pK32RQB, Opcode, InterfaceID, UserId.uvar32, Response);
