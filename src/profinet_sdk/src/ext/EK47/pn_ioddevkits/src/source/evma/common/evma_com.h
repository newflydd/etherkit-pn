...
/** reads the cycle length of the application timer (32 bit value [ns]) */
#define EVMA_GET_CYCLE_LENGTH()             *((volatile LSA_UINT32*) EVMA_IRTE_REG_ADDR_CLKCY_PERIOD) * *((volatile LSA_UINT32*) EVMA_IRTE_REG_ADDR_APLCLK_DIVIDER) * 10
/** reads the current time of the application timer (32 bit value, [ns]) */
#define EVMA_GET_CYCLE_TIME()               *((volatile LSA_UINT32*) EVMA_IRTE_REG_ADDR_APLCLK_TIMER) * 10

/** defines the number of interrupt vectors that are managed/used by EVMA */
#define EVMA_NUM_ISR_VECTORS                5

#elif defined(EVMA_USE_RZT2)

/** reads the cycle length of the application timer (32 bit value [ns]) */
#define EVMA_GET_CYCLE_LENGTH()             R_ETHSW->TDMA_CYCLE
/** reads the current time of the application timer (32 bit value, [ns]) */
#define EVMA_GET_CYCLE_TIME()               R_ETHSW->ATIME0

/** defines the number of interrupt vectors that are managed/used by EVMA */
#define EVMA_NUM_ISR_VECTORS                4
