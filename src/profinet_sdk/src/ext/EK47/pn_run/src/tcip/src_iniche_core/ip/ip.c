...
#define LTRC_ACT_MODUL_ID 20 /* INICHE_TCIP_ITGR added for LSA traces */
...
#include "icmp.h"
#include "ip_reasm.h"
#include "ether.h" /* INICHE_TCIP_ITGR added #include */
#include "nptcp.h" /* INICHE_TCIP_ITGR added #include */
...
#if 0 /* INICHE_TCPIP_ITGR: remove double definition, see include nptcp.h above */
#ifndef MIN
...
ip_init(void)
{
   ...
   {   
		...
		TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_ERROR
			, "IP_INIT: unable to register type with MAC driver"
			);
#else
...
#if 0 /* INICHE_TCIP_ITGR AP01335199 don't be a router */
...
#if defined(IP_V4) && defined(IP_PMTU)
   if (pmtucache_init ())
   {
#ifdef NPDEBUG
      dprintf ("IP_INIT: pmtucache_init() failed\n");
#endif   /* NPDEBUG */
      return(1);
   }
#endif   /* IP_V4 and IP_PMTU */
...
ip_bldhead(PACKET p, unsigned pid, u_char prot, unshort fragword)
{
   ...
   pip->ip_flgs_foff = htons(fragword); /* fragment flags and offset */
   ...
   if ((sopts = p->soxopts) != 0) /* INICHE_TCIP_ITGR added != 0 (warning: assignment within conditional expression) */
   {
	  /* yup */
	  if (sopts->ip_ttl)
...
ip_write_internal(PACKET p)
{
   ip_addr firsthop;
#ifdef IP_MULTICAST
#if 0 /*see below: INICHE_TCIP_ITGR mcast loopback disabled */
   PACKET pkt2;
   struct in_multi * inm;
#endif
   unsigned i;
   ...
   if (IN_MULTICAST(ntohl(p->fhost)))
   {
      ...
      if ((p->net == NULL) || (p->net->n_mcastlist) == NULL)
      {
         ...
         {
            goto cantroute;  /*INICHE_TCIP_ITGR has referenced cantroute again */
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
			TCIP_INICHE_TRACE_03(0, LSA_TRACE_LEVEL_ERROR
								, "ip_write_internal: pkt:%p len%u to 0x%x, can't route"
								, p, p->nb_plen, p->fhost
								);
#else
       ...
       * received queue.
       */
#if 0 /* INICHE_TCIP_ITGR mcast loopback disabled (implementation of SignalPktDemux() directly calls pktdemux()) */
          ...
          ((p->imo == NULL) || p->imo->imo_multicast_loop)) 
      {
         ...
         if (pkt2)
         {
#if 1 /* INICHE_TCIP_ITGR added initialisation of the etherheader in dest. buffer */
            char *ethhdr = pkt2->nb_prot - ETHHDR_SIZE + ETHHDR_BIAS;
...
#endif
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
	TCIP_INICHE_TRACE_06(0, LSA_TRACE_LEVEL_NOTE
		,"ip_write_internal: pkt:%p len%u to %u.%u.%u.%u, can't route"
		, p, p->nb_plen, (unsigned)(p->fhost&0xff),(unsigned)((p->fhost>>8)&0xff),(unsigned)((p->fhost>>16)&0xff),(unsigned)(p->fhost>>24)
		);
#else
            dprintf("ip_write_internal: pkt:%p len%u to %u.%u.%u.%u, can't route\n",
                    p, p->nb_plen, PUSH_IPADDR(p->fhost));
...
sendit:  /* label used for multicast packets to skip routing logic */
   ...
   if (NDEBUG & IPTRACE)
   {
	...
	TCIP_INICHE_TRACE_05(0, LSA_TRACE_LEVEL_CHAT
		,"ip_write: pkt[%u] to %u.%u.%u.%u,"
		, p->nb_plen, (unsigned)(p->fhost&0xff),(unsigned)((p->fhost>>8)&0xff),(unsigned)((p->fhost>>16)&0xff),(unsigned)(p->fhost>>24)
		);
	TCIP_INICHE_TRACE_04(0, LSA_TRACE_LEVEL_CHAT
		," route %u.%u.%u.%u"
		, (unsigned)(firsthop&0xff),(unsigned)((firsthop>>8)&0xff),(unsigned)((firsthop>>16)&0xff),(unsigned)(firsthop>>24)
		);
   ...
   {
#if 1 /* INICHE_TCIP_ITGR always prevent send to the same interface due to vulnerabilty, see AP01278476 */
      LOCK_NET_RESOURCE(FREEQ_RESID);
      pk_free(p);
      UNLOCK_NET_RESOURCE(FREEQ_RESID);
      IN_PROFILER(PF_IP, PF_EXIT);
    ...
    * both constraints) */
   if (!intrsafe_buf)
       limit = MIN(maxbuflen,(unsigned)p->net->n_mtu); /* INICHE_TCIP_ITGR added cast (unsigned) */
   else
       limit = p->net->n_mtu;
   ...
   if ((p->nb_plen + p->net->n_lnh) > limit)
   {
      ...
      return(err);
#else
#if 0 /* INICHE_TCIP_ITGR do not stop if IP_FRAGMENTS is not defined, see 1935252 */
   ...
   PACKET   p)
{
    ...
    */
   pip->ip_ver_ihl = 0x45;       /* 2 nibbles; VER:4, IHL:5. */
   pip->ip_flgs_foff = htons(IP_FLGS);  /* also clears the frag info field */
...
int ip_write2 (u_char prot, PACKET p, u_char * optp)
{
   ...
   for (tmpp = (u_char *)(p->nb_prot + sizeof(struct ip)); *optp != EOL_OPT; ++optp)
   {
      ...
      {
         case IP_RTR_ALERT_OPT:
#if 1 /* INICHE_TCIP_ITGR see Request ID 908657 */
			 {
			 union {
				u_long longVal;
				u_char charVal[sizeof(u_long)];
			} tmpLocal;
			tmpLocal.longVal = htonl (IP_RTR_ALERT_OPT_DATA);
			memcpy(tmpp, &tmpLocal.charVal, sizeof(u_long));
			}
#else
...
ip_fragment_lc(PACKET p, ip_addr firsthop)
{
   ...
   while (left > 1)  /* more data left to send? */
   {
      ...
      p->nb_plen = MIN((int)fragsize, left) + iphlen; /* replaced min by MIN */
#else
...
ip_dump(PACKET p)
{
...
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
   TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_CHAT
	   , "IP packet header:"
	   );
#else
...
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
   TCIP_INICHE_TRACE_04(0, LSA_TRACE_LEVEL_CHAT
	   , "ver/hlen: %02x, TOS: %02x, len: %04x, id: %04x"
   ...
   if(osum==xsum)
	TCIP_INICHE_TRACE_04(0, LSA_TRACE_LEVEL_CHAT
		, "flags/offs: %04x, TTL %02x, protocol: %02x, cksum: %04x (ok)"
		, htons(*(unshort*)(cp+6)), pip->ip_time, pip->ip_prot, htons(osum) );
   else
	TCIP_INICHE_TRACE_04(0, LSA_TRACE_LEVEL_CHAT
		, "flags/offs: %04x, TTL %02x, protocol: %02x, cksum: %04x (bad)"
		, htons(*(unshort*)(cp+6)), pip->ip_time, pip->ip_prot, htons(osum) );
#else
...
iproute(ip_addr host, ip_addr * hop1)
{
    ...
    * host we're trying to route to (thereby allowing this code to 
    * work with arp routing). otherwise, we would try to return 
...
ip_copypkt(PACKET p)
{
   ...
   if (np == NULL)
   {
	...
	TCIP_INICHE_TRACE_01(0, LSA_TRACE_LEVEL_NOTE
		, "ip_copypkt(): unable to obtain packet (len %d)"
		, len
		);
#else
