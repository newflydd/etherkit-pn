...
/*                            If wrong handle  OHA_FATAL(OHA_FATAL_ERR_RQB)  */
/*                          - oha_request()                                  */
/*                          - oha_close_channel()                            */
/*                                                                           */
/*****************************************************************************/
...
static LSA_UINT16 OHA_UserCancelReceive(
...
/*+               case we call OHA_FATAL(OHA_FATAL_ERR_RQB)                 +*/
...
LSA_VOID oha_open_channel(OHA_UPPER_RQB_PTR_TYPE pRQB)
{
    ...
    if ( OHA_RQB_GET_OPCODE(pRQB) != OHA_OPC_OPEN_CHANNEL )
    {
        ...
        OHA_FATAL(OHA_FATAL_ERR_RQB);
    }
    else
    {
        OHA_UPPER_TRACE_01(0, LSA_TRACE_LEVEL_NOTE,">>>: Request: OHA_OPC_OPEN_CHANNEL (Handle: 0x%X)",OHA_RQB_GET_HANDLE(pRQB));

        if ( pRQB->args.channel.oha_request_upper_done_ptr == 0 ) /* lint warning 611 “suspicious cast” */
        {
            OHA_RQB_SET_RESPONSE(pRQB, OHA_ERR_PARAM);
            Status = OHA_ERR_PARAM;
    ...
    if (Status == OHA_OK)
    {
        ...
        if ( Status == OHA_OK )
        {
            ...
            switch (pDetail->PathType)
            {
                    ...
                    /* Global User-Channel.                                           */
                    /* ---------------------------------------------------------------*/
                case OHA_PATH_TYPE_GLOBAL:
                {
                    OHA_HDB_USER_PTR_TYPE pHDBUser = LSA_NULL;

					OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_LOW,"Opening global USER-Channel");
                    ...
                    else
                    {
                        ...
                        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Open global OHA USER-channel failed. Cannot get USER handle! (Rsp: 0x%X).",Status);
                    }
                }
                break;
                ...
                case OHA_PATH_TYPE_USER:
                {
                    ...
                    if(!(OHA_EDDCheckNICUsed(pDetail->Params.User.NICId) ||
                         OHA_IS_NOT_NULL(OHA_SOCKGetHandleGlobal()) /* a (global) SOCK channel is open? */))
                    {
                        Status = OHA_ERR_SEQUENCE;
                        /* Error on getting Lower-Channel. Abort. */
                        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Open OHA USER-channel failed. No Lower-Channel present for this NicID (0x%X)", pDetail->Params.User.NICId);
                    }
                    else
                    {
                        OHA_HDB_USER_PTR_TYPE pHDBUser = LSA_NULL;
                        ...
                        if ( Status == OHA_OK )
                        {
                            ...
                            pHDBUser->Params.SysPath       = pRQB->args.channel.sys_path;
                            pHDBUser->Params.HandleUpper   = pRQB->args.channel.handle_upper;
                            pHDBUser->TraceIdx             = OHA_DbGetTrcIdx(pDetail->Params.User.NICId);
                            ...
                            if (OHA_CheckIpEnabled(pDetail->Params.User.NICId))
                            {
                ...
                case OHA_PATH_TYPE_DCP:
                    ...
                    else if (!(OHA_EDDCheckNICUsed(pDetail->Params.Dcp.NICId)))
					{
						Status = OHA_ERR_SEQUENCE;
						/* Error on getting Lower-Channel. Abort. */
						OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "Open OHA DCP-channel failed. No Lower EDD-Channel present for this NicID (0x%X)", pDetail->Params.Dcp.NICId);
					}
					else
                    {
                        ...
                        if ( Status == OHA_OK )
                        {
                            ...
                            pHDBDcp->TraceIdx             = OHA_DbGetTrcIdx(pDetail->Params.Dcp.NICId);
                ...
                case OHA_PATH_TYPE_LLDP:
					...
					else if (!(OHA_EDDCheckNICUsed(pDetail->Params.Lldp.NICId)))
					{
						Status = OHA_ERR_SEQUENCE;
						/* Error on getting Lower-Channel. Abort. */
						OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "Open OHA LLDP-channel failed. No Lower EDD-Channel present for this NicID (0x%X)", pDetail->Params.Lldp.NICId);
					}
                    else
                    {
                        ...
                        if ( Status == OHA_OK )
                        {
                            ...
                            pHDBLldp->TraceIdx             = OHA_DbGetTrcIdx(pDetail->Params.Lldp.NICId);
                ...
                case OHA_PATH_TYPE_SOCK:
                    ...
                    /* Check if already used the SOCK-Channel. If so abort */
					...
					else
                    {
						...
						/* TFS1728432: Not allowed! Open this global SOCK channel first! ! */
						if (IfCount)
						{
                    ...
                    /* DNS-Channel.                                                  */
                    /* --------------------------------------------------------------*/
                case OHA_PATH_TYPE_DNS:
                    OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_LOW,"Opening DNS-SystemChannel");
#ifndef OHA_CFG_USE_DNS
                    OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_UNEXP,"Open channel failed. DNS not supported yet!");
                    Status = OHA_ERR_PARAM;
#else
                    /* Check if NIC-ID already used by a DNS-Channel. If so abort */
                    {
                        OHA_HDB_DNS_PTR_TYPE pHDBDns;
                        /* Get a free, initialized HDB */
                        Status = OHA_DNSGetFreeHDB(&pHDBDns);

                        if ( Status == OHA_OK )
                        {
                            OHA_ASSERT_NULL_PTR(pHDBDns);

                            pHDBDns->Params.pSys          = pSys;
                            pHDBDns->Params.pDetail       = pDetail;
                            pHDBDns->Params.Cbf           = pRQB->args.channel.oha_request_upper_done_ptr;
                            pHDBDns->Params.SysPath       = pRQB->args.channel.sys_path;
                            pHDBDns->Params.HandleUpper   = pRQB->args.channel.handle_upper;

                            /* set our handle in RQB */
                            pRQB->args.channel.handle     = pHDBDns->ThisHandle;

                            /* Do a DNS-open channel request */
                            Status = OHA_DNSOpenChannel(pHDBDns->ThisHandle,pHDBDns->Params.SysPath,pHDBDns->Params.pSys);

                            if ( Status == OHA_OK )
                            {
                                /* Request is underway now.. */
                                pHDBDns->DnsState  = OHA_HANDLE_STATE_OPEN_DNS_RUNNING;
                                pHDBDns->pOCReq = pRQB; /* save pointer to request for later use */
                                Finish           = LSA_FALSE;
                            }
                            else
                            {
                                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"Open OHA-DNS channel failed. (Rsp: %Xh)",Status);
                                OHA_DNSReleaseHDB(pHDBDns);
                            }
                        }
                        else
                        {
                            /* Error on getting handle. Abort. */
                            OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Open OHA-DNS channel failed. Cannot get DNS handle! (Rsp: %Xh).",Status);
                        }
                    }
#endif
                    break;
                ...
                case OHA_PATH_TYPE_NARE:
					...
					else if (!(OHA_EDDCheckNICUsed(pDetail->Params.Nare.NICId)))
					{
						Status = OHA_ERR_SEQUENCE;
						/* Error on getting Lower-Channel. Abort. */
						OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "Open OHA NARE-channel failed. No Lower EDD-Channel present for this NicID (0x%X)", pDetail->Params.Nare.NICId);
					}
                    else
                    {
                        ...
                        if ( Status == OHA_OK )
                        {
                            ...
                            pHDBNare->TraceIdx             = OHA_DbGetTrcIdx(pDetail->Params.Nare.NICId);
                ...
                case OHA_PATH_TYPE_MRP:
					...
					else if (!(OHA_EDDCheckNICUsed(pDetail->Params.Mrp.NICId)))
					{
						Status = OHA_ERR_SEQUENCE;
						/* Error on getting Lower-Channel. Abort. */
						OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP, "Open OHA MRP-channel failed. No Lower EDD-Channel present for this NicID (0x%X)", pDetail->Params.Mrp.NICId);
					}
                    else
                    {
                        ...
                        if ( Status == OHA_OK )
                        {
                            ...
                            pHDBMrp->TraceIdx             = OHA_DbGetTrcIdx(pDetail->Params.Mrp.NICId);
    ...
    else
    {
        ...
        switch ( OHA_HANDLE_GET_TYPE(Handle))
        {
                ...
                if (! LSA_HOST_PTR_ARE_EQUAL(pHDBUser, LSA_NULL) )
                {
...
#if 1 /* 2347675 - OHA fatal during closing EB200P */
...
#endif
                    else if (OHA_IsAnyLocalCtrlTcIpActive(pHDBUser->ThisHandle))
                    {
                        OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel(): Close channel failed. SOCK-TCIP-User requests pending");
                        Status = OHA_ERR_SEQUENCE;
                    }
					else if (OHA_IsAnyLocalCtrlClusterIpActive(pHDBUser->ThisHandle))
					{
						OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR, "oha_close_channel(): Close channel failed. ClusterIP-User requests pending");
						Status = OHA_ERR_SEQUENCE;
					}
					else if (OHA_IsAnyLocalIpForwardingActive(pHDBUser->ThisHandle))
					{
						OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR, "oha_close_channel(): Close channel failed. ClusterIP-User requests pending");
						Status = OHA_ERR_SEQUENCE;
					}
                    else if (OHA_IS_NOT_NULL(pHDBUser->DnsUserReq.pBottom))
                    {
                        OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel(): Close channel failed. DNS-Set-User requests pending");
                        Status = OHA_ERR_SEQUENCE;
                    }
                    else if (OHA_CheckDcpControlUser(pHDBUser))
                    {
                        OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel(): User OHA_OPC_DCP_RESPONSE pending");
                        Status = OHA_ERR_SEQUENCE;
                    }
                    ...
                    else
                    {
                        ...
                        if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER &&
							OHA_CheckIpEnabled(pHDBUser->Params.pDetail->Params.User.NICId))
                        {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ( Status == OHA_OK )
                            {
                                ...
                                oha_snmp_undo_init(pHDBEdd->Params.pDetail->Params.Edd.NICId);
#endif
                                /* Request is underway now.. */
            ...
            case OHA_HANDLE_TYPE_SOCK: /* SOCK not supported yet */
                ...
                /* DNS-Channel.                                                      */
                /* ------------------------------------------------------------------*/
            case OHA_HANDLE_TYPE_DNS:

#ifndef OHA_CFG_USE_DNS
                OHA_PROGRAM_TRACE_00(0, LSA_TRACE_LEVEL_UNEXP,"Close channel failed. DNS not supported!");
#else
                pHDBDns = OHAGetDnsHDBFromHandle(Handle);

                if (! LSA_HOST_PTR_ARE_EQUAL(pHDBDns, LSA_NULL) )
                {
                    Finish      = LSA_TRUE;
                    Cbf         = pHDBDns->Params.Cbf;
                    HandleUpper = pHDBDns->Params.HandleUpper;
                    pSys        = pHDBDns->Params.pSys;

                    OHA_RQB_SET_HANDLE(pRQB, HandleUpper );

                    if ( OHA_GET_GLOB_PTR()->UsedUserCnt || pHDBDns->UserCnt ) /* some user-channel uses this DNS-channel ?*/
                    {
                        OHA_UPPER_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel() failed. Channel still in use by user-channels, UsedUserCnt (0x%X), UserCnt (0x%X)", OHA_GET_GLOB_PTR()->UsedUserCnt, pHDBDns->UserCnt);
                        Status = OHA_ERR_SEQUENCE;
                    }
                    else
                    {
                        if (! LSA_HOST_PTR_ARE_EQUAL(pHDBDns->pOCReq, LSA_NULL))
                        {
                            OHA_UPPER_TRACE_00(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel(): Close channel failed. Close already running.");
                            Status = OHA_ERR_SEQUENCE;
                        }
                        ...
                        {
                            Status = OHA_DNSCloseChannel(pHDBDns->DnsHandle,pHDBDns->Params.pSys);
                            if ( Status == OHA_OK )
                            {
                                /* Request is underway now.. */
                                pHDBDns->DnsState  = OHA_HANDLE_STATE_CLOSE_DNS_RUNNING;
                                pHDBDns->pOCReq = pRQB; /* save pointer to request for later use */
                                /* pHDBDns->State  = OHA_HANDLE_STATE_STOP_DNS_RUNNING; */
                                Finish = LSA_FALSE;
                            }
                            else
                            {
                                /* is this a fatal error ?                      */
                                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL,"Cannot Close DNS. Status: 0x%X", Status);
                            }
                        }
                    }
...
#endif
                {
                    OHA_UPPER_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"oha_close_channel(): Invalid OHA-DNS Handle! (0x%X)",Handle);
                    Status = OHA_ERR_PARAM;
                    OHA_RQB_SET_RESPONSE(pRQB, Status);
					OHA_FATAL(OHA_FATAL_ERR_RQB);
                }
...
/*+               OHA_ERR_PARAM                                             +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_request(OHA_UPPER_RQB_PTR_TYPE pRQB)
{
    LSA_HANDLE_TYPE          Handle;
    OHA_HDB_USER_PTR_TYPE    pHDB;
    ...
    if (! LSA_HOST_PTR_ARE_EQUAL(pHDB, LSA_NULL) )
    {
             ...
             LSA_HOST_PTR_ARE_EQUAL(pHDB->pOCReq, LSA_NULL) )
        {
            switch ( OHA_RQB_GET_OPCODE(pRQB))
            {
                ...
                case OHA_OPC_SNMP_INDICATION:  /* global requests can be requested on an interface channel too */
#endif
                ...
                default:  /* if-specific requests can be requested only on an interface channel */
					if(pHDB->Params.pDetail->PathType != OHA_PATH_TYPE_USER)
					{
			            OHA_UPPER_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,">>>: oha_request(): Invalid PathType(0x%X) for Opcode(0x%X)",pHDB->Params.pDetail->PathType, OHA_RQB_GET_OPCODE(pRQB));
            ...
            switch ( OHA_RQB_GET_OPCODE(pRQB))
            {
                ...
                case OHA_OPC_ADDR_INFO_IND:       /* EDD */
                    ...
                    break;
#ifdef OHA_SOCK_SNMP_OFF
                case OHA_OPC_SNMP_INDICATION:       /* EDD */
                    OHA_UPPER_TRACE_01(0, LSA_TRACE_LEVEL_NOTE,">>>: Request: OHA_OPC_SNMP_INDICATION (Handle: 0x%X)",OHA_RQB_GET_HANDLE(pRQB));
                    OHA_RequestSnmpIndication(pRQB, pHDB);
                    break;
#endif
                ...
                case OHA_OPC_PRM_WRITE:
                    ...
                    break;
                case OHA_OPC_PRM_CHANGE_PORT:
                    OHA_UPPER_TRACE_01(0, LSA_TRACE_LEVEL_NOTE,">>>: Request: OHA_OPC_PRM_CHANGE_PORT (Handle: 0x%X)",OHA_RQB_GET_HANDLE(pRQB));
                    OHA_RequestPrmChangePort(pRQB, pHDB);   /* EDD */
                    break;
    ...
    OHA_HDB_USER_PTR_TYPE       pHDB)
{
    ...
    /* cancel the pending user-indication resource for SNMP */
    {
        pRQBHelp = pHDB->pSnmpInd;
        if (OHA_IS_NOT_NULL(pRQBHelp))
        {
            OHA_UserRequestFinish(pHDB,pRQBHelp,OHA_OK_CANCEL);
    ...
    LSA_UINT16                 Response)
{
    ...
    if (OHA_IS_NOT_NULL(pHDB->Params.pDetail) &&
        ...
        OHA_CheckIpEnabled(pHDB->Params.pDetail->Params.User.NICId))
    {
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_USERCloseChannelDone(
    OHA_HDB_USER_PTR_TYPE       pHDB,
    LSA_UINT16                 Response)
{
    OHA_UPPER_RQB_PTR_TYPE      pRQB;
