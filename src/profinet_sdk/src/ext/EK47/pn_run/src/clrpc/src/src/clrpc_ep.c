...
} CLRPC_EPM_CTX_TYPE, *CLRPC_EPM_CTX_PTR_TYPE;
	...
	)
{
    ...
    CLRPC_ASSERT(clrpc_data.count_contexts > 0);       /* AP00830411: count context */
    clrpc_data.count_contexts--;
    ...
    )
{
    ...
    /* AP00830411: do we have resources for new context? */
    if (clrpc_data.count_contexts < CLRPC_CFG_MAX_CONTEXTS)
    {
        ...
        if (CLRPC_PTR_IS_NULL (ctx)) {
            CLRPC_EPM_TRACE_00 (0, LSA_TRACE_LEVEL_UNEXP
                , "no local mem for new context"
                );
        }
        else
        {
            clrpc_data.count_contexts++;        /* AP00830411: count new context */
    ...
    else
    {
        /* AP00830411 */
        CLRPC_EPM_TRACE_00 (0, LSA_TRACE_LEVEL_UNEXP
            , "no resource for new context, max number is used"
            );
        ctx = LSA_NULL;
    }
    return ctx;
	...
	)
{
	...
	else
	{
		...
		/* adjust body len */
	call->ndr_len = (LSA_UINT32)(dp - call->ndr_data);
}
...
//----------------------------------------------------------------------------*/
static LSA_UINT32 clrpc_get_strlen(
    CLRPC_COMMON_MEM_U8_CONST_PTR_TYPE    str,
...
clrpc_epm_callback (
	...
	LSA_UINT16  response,
	CLRPC_UPPER_RQB_PTR_TYPE *rb_ptr)
{
	...
	LSA_UINT8  annotation[CLRPC_MAX_ANNOTATION_SIZE] = {0};
...
/*----------------------------------------------------------------------------*/
		case 2: /* ept_lookup */
			...
			if (CLRPC_PTR_IS_NULL (ctx))
			{
				CLRPC_RQB_SET_RESPONSE (rb, CLRPC_ERR_SERVER_BUSY);
				CLRPC_EPM_TRACE_00 (scall->trace_idx, LSA_TRACE_LEVEL_WARN
					, "epm server overload, check CLRPC_CFG_MAX_CONTEXTS. ->epm query aborted before end?"
					);
				break; /* case */
			}
