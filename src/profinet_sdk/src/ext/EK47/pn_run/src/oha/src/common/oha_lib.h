...
{
    struct oha_list_entry_tag * Flink; /* forward link */
    struct oha_list_entry_tag * Blink; /* backward link */
};

typedef struct oha_list_entry_tag  OHA_LIST_ENTRY_TYPE;

typedef struct oha_list_entry_tag *OHA_LIST_ENTRY_PTR_TYPE;
...
{                                       \
LSA_UINT32 SetValue = (LSA_UINT32)(Value); /* lint warning 572 removed */ \
*(pValue)++ = (LSA_UINT8) (SetValue >> 24 );  \
*(pValue)++ = (LSA_UINT8) (SetValue >> 16 );  \
*(pValue)++ = (LSA_UINT8) (SetValue >> 8  );  \
*(pValue)++ = (LSA_UINT8) (SetValue);         \
}
