...
typedef struct iom_data_s
{
    ...
    Unsigned16          session_key_prim_ar;                         /**< bypassed AR-session_key, needed for setting data state  */
...
iom_data_t iom_data	IOM_ATTRIBUTE_TCM ;
//extern iom_data_t *g_pIoMem;
...
static LSA_BOOL iom_perif_find_free_mem_block(LSA_UINT8 page, LSA_UINT16* size, LSA_UINT16 search_start_index, LSA_UINT32 *offset_region_start)
{
    ...
    *size = 0;
    ...
    if(size64)
    {
        for(offset = (search_start_index >> 3); offset <= (32 - size64); offset++)
        {
            if((iom_data.perif_page_mem_desc[page].memory_map[offset >> 5] & (1 << (offset % 32))) == 0)
            {
                const LSA_UINT32 offset_end_max = offset + size64;

                for(offset_end = offset + 1; offset_end < offset_end_max; offset_end++)
                {
                    if((iom_data.perif_page_mem_desc[page].memory_map[offset_end >> 5] & (1 << (offset_end % 32))) != 0)
                    {
                        break;
                    }
                }
                ...
                if((offset_end - offset) >= size64)
                {
                    ...
                    while(   (offset_end < 32)
                    	  && ((iom_data.perif_page_mem_desc[page].memory_map[offset_end >> 5] & (1 << (offset_end % 32))) == 0))
                    {
                        offset_end++;
                    }
...
static LSA_BOOL iom_perif_add_used_mem_block(iom_perif_cr_data_t *cr, LSA_UINT8 page_index, LSA_UINT32 buffer_size, LSA_UINT32 offset)
{
    ...
    if(cr->num_pages >= IOM_PERIF_MAX_NUM_PAGES)
    {
        return LSA_FALSE;
    }
    ...
    cr->used_pages[cr->num_pages].page_offset_low = offset & 0xfffffff8;                        /* 8 byte aligned */
    cr->used_pages[cr->num_pages].page_offset_high = (offset + buffer_size + 7) & 0xfffffff8;   /* 8 byte aligned, specifies the index of the
    ...
    while(index < (cr->used_pages[cr->num_pages].page_offset_high >> 3))
    {
        ...
        {
            iom_data.perif_page_mem_desc[page_index].memory_map[index >> 5] |= (1 << (index % 32));
            index++;
        }
    }
    if(( cr->num_pages < IOM_PERIF_MAX_NUM_PAGES ) && (page_index < IOM_PERIF_MAX_PAGE_INDEX))
	{
        ...
        if(256 - cr->used_pages[cr->num_pages].page_offset_high < iom_data.perif_page_mem_desc[page_index].bytes_free_from_end)
        {
            iom_data.perif_page_mem_desc[page_index].bytes_free_from_end = 256 - cr->used_pages[cr->num_pages].page_offset_high;
        }
...
static LSA_VOID iom_perif_delete_cr_used_mem_blocks(iom_perif_cr_data_t *cr)
{
    ...
    for(i = 0; i < cr->num_pages; i++)
    {
        ...
        {
            iom_data.perif_page_mem_desc[page_index].memory_map[index >> 5] &= ~(1UL << (index % 32));
            index++;
        }
        ...
        if(cr->used_pages[i].page_offset_low == iom_data.perif_page_mem_desc[page_index].bytes_free_from_start)
        {
            ...
            while(   (index < 32)
            	  && ((iom_data.perif_page_mem_desc[page_index].memory_map[index >> 5] & (1UL << (index % 32))) == 0))
            {
                index++;
            }
        ...
        if(256 - cr->used_pages[i].page_offset_high == iom_data.perif_page_mem_desc[page_index].bytes_free_from_end)
        {
            ...
            while(   (index >= 0)
            	  && ((iom_data.perif_page_mem_desc[page_index].memory_map[index >> 5] & (1 << (index % 32))) == 0) )
            {
                index--;
            }
            iom_data.perif_page_mem_desc[page_index].bytes_free_from_end = 256 - ((index + 1) << 3);
        }
    ...
    if(iocr_type == IOM_PERIF_INPUT_CR || iocr_type == IOM_PERIF_ALL_CRS)
    {
        if(iom_data.perif_page_ar_data[ar_idx].input_cr)
        {
            if(iom_data.perif_page_ar_data[ar_idx].input_cr->locked == 0)       /* if the CR is locked we cannot deallocate the memory */
            {
                ...
                IOM_PERIF_CR_ADDRESS_N(iom_data.perif_page_ar_data[ar_idx].input_cr->cr_index)->zero_data = 0;
                IOM_PERIF_CR_ADDRESS_N(iom_data.perif_page_ar_data[ar_idx].input_cr->cr_index)->new_data_int = 0;         /* no data interrupts shall be triggered */
            ...
            if(iom_data.perif_page_ar_data[ar_idx].is_sysred_provider != FALSE)
            {
                iom_data.perif_page_ar_data[ar_idx].is_sysred_provider = FALSE;

                if(eddp_SysRed_Provider_Remove(EDD_INTERFACE_ID_MIN-1, iom_data.perif_page_ar_data[ar_idx].prov_id) != EDD_STS_OK)
                {
                    iom_perif_in_fatal_error(IOM_LOCAL_ERR_MODULE, __LINE__, 0);
                }
...
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
                /* reset CR_Address_x register */
                IOM_PERIF_CR_ADDRESS_N(iom_data.perif_page_ar_data[ar_idx].output_cr->cr_index)->zero_data = 0;
                IOM_PERIF_CR_ADDRESS_N(iom_data.perif_page_ar_data[ar_idx].output_cr->cr_index)->new_data_int = 0;        /* no data interrupts shall be triggered */
...
LSA_VOID iom_perif_init(iom_init_struct_t *init_data)
{
   ...
   // g_pIoMem = &iom_data;
    ...
    for(i = 0; i < IOM_CFG_PERIF_NUM_AR; i++)
    {
        ...
        iom_data.perif_page_ar_data[i].free_request_issued = LSA_FALSE;
        iom_data.perif_page_ar_data[i].is_sysred_provider = FALSE;
    ...
    {
        iom_data.perif_page_mem_desc[i].bytes_free = 256;
        iom_data.perif_page_mem_desc[i].bytes_free_from_start = 256;
        iom_data.perif_page_mem_desc[i].bytes_free_from_end = 256;

        /* each bit of the memory map corresponds to 8 bytes of the page, memory_map is a 32 bit variable */
        for(j = 0; j < 256 / 8 / 32; j++)
        {
    ...
    *IOM_PERIF_PN_IRQACK_HIGH   = 0xFFFFFFFF;
    ...
    eddp_SysRed_Init();
    if( EDD_STS_OK != eddp_SysRed_Setup(EDD_INTERFACE_ID_MIN-1, 0, init_data->pnip_base_address, init_data->gsharedmem_address, EDDP_HW_ERTEC200P) )
    {
        iom_perif_in_fatal_error(IOM_LOCAL_ERR_MODULE, __LINE__, 0);
    }
...
LSA_BOOL iom_perif_allocate_iocr_memory(LSA_UINT32 ar_idx, LSA_UINT16 ar_set_nr, LSA_UINT16 session_key, CM_UPPER_SV_AR_CONNECT_IOCR_PTR_TYPE iocr_ptr)
{
    ...
    IOM_ASSERT(iocr_ptr->io_buffer_size < 1792, LSA_FALSE);     /* 512 Bytes data, 256 Subslotsx2 Byte, 768 Bytes gap due to 4-byte-alignment */

    buffer_size = iocr_ptr->io_buffer_size;
    buffer_size = (buffer_size + 7) & 0xfffffff8;   /* alignment to 8 byte, no APDU status in Ertec200p */
    ...
    * ***************************************************************** */
     ...
     * ****************************************** */
    {
        ...
        }   /* endif : size_to_alloc <= 256*/

        if(size_to_alloc)
        {
            /* buffer to be allocated is larger than 256 bytes OR
            ...
            for(page_index = 0; page_index < IOM_PERIF_MAX_PAGE_INDEX; page_index++)
            {
                ...
                if(iom_data.perif_page_mem_desc[page_index].bytes_free_from_end > 0)
                {
                    if(size_to_alloc > iom_data.perif_page_mem_desc[page_index].bytes_free_from_end)     /* the end of the first page shall be completely filled up */
                    {
                        ...
                        complete_pages_needed = space_left_to_alloc >> 8;
            ...
            if(index_best_first_page < IOM_PERIF_MAX_PAGE_INDEX)
            {
                ...
                {
					if(iom_data.perif_page_mem_desc[page_index].bytes_free == 256)
					{
						/* mark the used memory area of the page in the memory map and update
						 * the page- and CR descriptor. Data is allocated from the page beginning */
						if(iom_perif_add_used_mem_block(cr, page_index, 256, 0) == LSA_FALSE)
						{
        ...
        cr->buffer_size_no_padding = iocr_ptr->io_buffer_size;
        ...
        if(iocr_ptr->iocr_type == CM_IOCR_TYPE_OUTPUT || iocr_ptr->iocr_type == CM_IOCR_TYPE_MULTICAST_CONSUMER)
        {
            ...
            IOM_ASSERT(iocr_ptr->eddi_apdu_status_offset != LSA_NULL && iocr_ptr->eddi_apdu_status_offset != EDD_DATAOFFSET_INVALID, LSA_FALSE);
            cr->apdu_status_address = (LSA_VOID*) (U_PNIP__BASE + iocr_ptr->eddi_apdu_status_offset);
        }
        ...
        iocr_ptr->eddx_data_offset = (cr->cr_index << 11) + cr->used_pages[0].page_offset_low;
        #ifdef IOM_USE_PROFINET_STACK_V5_3
        iocr_ptr->eddx_cr_number = cr->cr_index;
        #else
        iocr_ptr->eddp_cr_number = cr->cr_index;
        #endif
        iocr_ptr->eddp_forward_mode = EDD_FORWARDMODE_SF;       /* store and forward mode */
    }
    ...
    if( (iocr_ptr -> iocr_type ==CM_IOCR_TYPE_INPUT) || (iocr_ptr -> iocr_type ==CM_IOCR_TYPE_MULTICAST_PROVIDER) )
    {
        ...
        {
            ret_value = eddp_SysRed_Provider_Add(EDD_INTERFACE_ID_MIN-1, &local_provider_id,
                                                 iocr_ptr->eddx_data_status_offset /* DataStatusOffset  */,
                                                 local_prov_type);
            ...
            iom_data.perif_page_ar_data[ar_idx].is_sysred_provider = TRUE;
     ...
     * ************************* */
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
    IOM_PERIF_CR_ADDRESS_N(cr->cr_index)->cr_start_address = 0;     /* not applicable to paged mode */
    IOM_PERIF_CR_ADDRESS_N(cr->cr_index)->cr_end_address = 0;       /* not applicable to paged mode */
    IOM_PERIF_CR_ADDRESS_N(cr->cr_index)->zero_data = 0;            /* zero data after overflow of data-hold-timer watchdog / PNIP */
    IOM_PERIF_CR_ADDRESS_N(cr->cr_index)->new_data_int = 0;         /* new data interrupt to be triggered : default = off. See beneath for potential activation*/
#endif
    /* ************************************************************************************
...
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
        IOM_PERIF_CR_ADDRESS_N(cr->cr_index)->new_data_int = ar_idx + 1;    /* generate new data events */
#endif
 ...
 */
IOM_ATTRIBUTE_ITCM LSA_UINT16 iom_perif_consumer_lock(LSA_VOID** consumer_buffer, LSA_VOID** ppv_apdu_status_ptr, LSA_UINT32 ar_idx)
{
    iom_perif_command_if_control_reg_t cmd_if_reg;
    iom_apdu_status_t** apdu_status_ptr = (iom_apdu_status_t**)ppv_apdu_status_ptr;
    ...
    IOM_ASSERT(iom_data.perif_page_ar_data[ar_idx].output_cr != LSA_NULL, IOM_INVALID_BUFFER);
    IOM_ASSERT(iom_data.perif_page_ar_data[ar_idx].output_cr->locked == LSA_NULL, IOM_BUFFER_LOCKED);
    ...
    *consumer_buffer = iom_data.perif_page_ar_data[ar_idx].output_cr->start_address;
    ...
    #endif
    {
        ...
        cmd_if_reg.cr_number = iom_data.perif_page_ar_data[ar_idx].output_cr->cr_index;
        cmd_if_reg.conf_request = 0;
        cmd_if_reg.f_code = 1;          /* ConsRD -> get new user buffer, previously filled by PNIP */
        cmd_if_reg.user_id = 0;
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
 ...
 */
IOM_ATTRIBUTE_ITCM LSA_VOID iom_perif_provider_lock(void **provider_buffer, LSA_UINT32 ar_idx)
{
     IOM_ENTER_IO();
...
#endif
    {
        cmd_if_reg.cr_number = iom_data.perif_page_ar_data[ar_idx].input_cr->cr_index;
        cmd_if_reg.conf_request = 0;
        cmd_if_reg.f_code = 2;              /* ConsWR -> user data is handed over to PNIP */
        cmd_if_reg.user_id = 0;
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200P)
        *IOM_PERIF_COMMAND_IF_CONTROL = cmd_if_reg;
#endif
...
LSA_UINT32 iom_perif_provider_set_data_state  (LSA_UINT32 ar_idx, LSA_UINT8 value, LSA_UINT8 mask)
{
	...
	if(ar_idx < IOM_CFG_PERIF_NUM_AR)
	{
        ...
        ret_val = eddp_SysRed_ProviderDataStatus_Set(EDD_INTERFACE_ID_MIN-1, iom_data.perif_page_ar_data[ar_idx].prov_id, value, mask);
