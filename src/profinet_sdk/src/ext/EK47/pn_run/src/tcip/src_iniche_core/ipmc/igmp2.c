/*
 * v3.1 August 30, 2011 ipmc/igmp2.c
...
int igmpv2_input (PACKET p)
{
   ...
   USE_ARG(igmplen); /* INICHE_TCPIP_ITGR eliminate MIPS compiler warning "variable 'igmplen' set but not used", see 1911686 */
...
int igmpv2_process_report (PACKET p)
{
    ...
    * Scenario# IGMPv1 rtr present Type of rcvd report Cancel timer
    * =============================================================
    * 1         No                 IGMPv1              Yes
    * 2         No                 IGMPv2              Yes
    * 3         Yes                IGMPv1              Yes
   ...
   {
      /* since the Reports are sent to the group address, we should
...
int igmpv2_process_query (PACKET p)
{
   ...
   if (igmp->igmp_code == 0)
   {
      ...
      netp->igmp_timer1 = IGMPv1_RTR_PRESENT_TMO * PR_FASTHZ;
      igmp_del_all_existing_igmp_timers (netp);
      igmp_upd_oper_mode_transition_stats (netp, IGMP_MODE_V1);
      netp->igmp_oper_mode = IGMP_MODE_V1;
   ...
   USE_ARG(mcgrp_addr); /* INICHE_TCPIP_ITGR eliminate MIPS compiler warning "variable 'mcgrp_addr' set but not used", see 1911686 */
...
#if 1 /* INICHE_TCIP_ITGR prevent passing 0 to IGMPv2_RANDOM_DELAY, otherwise division by zero */
	  if (max_resp_time == 0)
	  {
		  max_resp_time = 1;
	  }
