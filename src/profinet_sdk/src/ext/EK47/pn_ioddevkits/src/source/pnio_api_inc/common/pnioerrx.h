...
#define PNIO_APP_CHECK(_cond,_text)  																\
{																								\
	if (!(_cond)) { PNIO_printf(_text); }		\
}
...
#define PNIO_ASSERT(_cond,_text)  																\
{																								\
	if (!(_cond)) { LSA_TRACE_00  (TRACE_SUBSYS_IOD_API, LSA_TRACE_LEVEL_FATAL, _text); }		\
}
