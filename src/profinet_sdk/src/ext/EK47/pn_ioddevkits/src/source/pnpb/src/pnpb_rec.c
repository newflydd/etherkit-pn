        ...
        if (CM_OPC_SV_AR_READ_IND == LSA_RQB_GET_OPCODE(pPndvIf->ds_rw.ptr))
        {
            ...
            { // *** default:  synchronous record read ***
                ...
                { // *** ERROR occured: copy PnioState (IEC61158-6)
                    pDsPeri->event_type.u.sv.ar_rec.cm_pnio_err = OsHtonl (PnioStat.vu32);
         ...
         )
{
    ...
    { // *** copy detailed PnioState (see IEC61158-6), maybe with or without error
    	pDsPeri->event_type.u.sv.ar_rec.cm_pnio_err = OsHtonl (pUnionPnioStat->vu32);
    ...
    { // *** OK, no error ****
    	pDsPeri->event_type.u.sv.ar_rec.data_length = CM_RECORD_OFFSET + NettoDatLength;    // ok
    	// **** copy data ***
    	OsMemCpy ( ((PNIO_UINT8*)pDsPeri->record_data)+ CM_RECORD_OFFSET, pSrcDat, NettoDatLength);
...
// *---------------------------------------------------------------------------
PNIO_VOID*  pnpb_rec_set_rsp_async (void)
{
	OsAlloc ((PNIO_VOID**) &pAsyncRecReq, 0, sizeof (ASYNC_REC_DATA));
    return (pAsyncRecReq);
}
		...
		)
{
	...
	FSUHeader = OsNtohs( ( PNIO_UINT16 )( *( PNIO_UINT16* )( NettoDatP8 + shift_ptr ) ) );
	FSULen = OsNtohs( ( PNIO_UINT16 )( *( PNIO_UINT16* )( NettoDatP8 + shift_ptr + 2 ) ) );
	if( ( 0x0609 == FSUHeader ) && ( 0x0020 == FSULen ) )
	{
		...
		if( ( 0x0601 == FSUHeader ) && ( 0x0018 == FSULen ) )
		{
			...
			shift_ptr += 8;
			ARFSUMode = OsNtohl( ( PNIO_UINT32 )( *( PNIO_UINT32* )( NettoDatP8 + shift_ptr ) ) );
			...
			PNIO_UINT8 * TmpBuff;
			OsAlloc ( ( void** )&TmpBuff, 0xff, ARFSU_LEN );
			pnpb_ARFSU_UUID_read_Handler( &FSULen, TmpBuff );
			if( 0 == OsMemCmp( TmpBuff, ( PNIO_VOID* )( NettoDatP8 + shift_ptr ), ARFSU_LEN ) )
			{
		...
		)
{
	...
	OsMemCpy ( pBuffer, TmpDataPtr, ARFSU_LEN );
