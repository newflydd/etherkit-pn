	...
	)
{
	...
	if (opcode == SOCK_OPC_UDP_SEND || opcode == SOCK_OPC_UDP_RECEIVE) {
		...
		if (opcode == SOCK_OPC_UDP_SEND || opcode == SOCK_OPC_UDP_RECEIVE) {
			(*lower_rqb_ptr_ptr)->args.data.flags = SOCK_STANDARD_IP_ONLY; /* see Cluster IP user story */
		}
	...
	)
{
	...
	lower_rqb_ptr->args.data.flags = SOCK_STANDARD_IP_ONLY; /* sanity; see Cluster IP user story */
