/*
 * v3.1 August 30, 2011 net/pktalloc.c
...
#define LTRC_ACT_MODUL_ID 19 /* INICHE_TCIP_ITGR added for LSA traces */
...
#if 1 /* INICHE_TCIP_ITGR allow tracing of allocated packets(size)                     */
/* TCPIP_PK_ALLOC_SIZE is called each time a packet is allocated through pk_alloc */
#ifndef TCPIP_PK_ALLOC_SIZE
    #define TCPIP_PK_ALLOC_SIZE(uPaketSize)
#endif
/* TCPIP_PK_ALLOC_INFO is called each time a packet is allocated through pk_alloc */
#ifndef TCPIP_PK_ALLOC_INFO
    #define TCPIP_PK_ALLOC_INFO(pkt_ptr)
#endif
/* TCPIP_PK_FREE_INFO is called each time a packet is freed through pk_free */
#ifndef TCPIP_PK_FREE_INFO
    #define TCPIP_PK_FREE_INFO(pkt_ptr)
#endif
#endif
...
#ifdef ALIGN_BUFS
#error "INICHE_TCIP_ITGR: will not work as expected. see NPDEBUG. alloc-functions must do it right."
...
no_pkt_buf:
			...
			TCIP_INICHE_TRACE_01(0, LSA_TRACE_LEVEL_ERROR
				, "Netinit: calloc failed getting buffer %d"
				, i
				);
#else
...
PACKET pk_alloc(unsigned len)
{
   ...
   else
   {
      ...
      if (!p)
#if 1 /* INICHE_TCIP_ITGR error trace if pkalloc fails */
		{
			TCIP_INICHE_TRACE_03(0,LSA_TRACE_LEVEL_ERROR
								, "(!) pk_alloc: was not able to alloc buffer len(%d), bigfreeq.q_len(%d), lilfreeq.q_len(%d)"
								, len, bigfreeq.q_len, lilfreeq.q_len
								);
			return NULL;
		}
#else
   ...
   p->net = NULL;
#if 1 /* INICHE_TCIP_ITGR AP00557635 */
#ifdef IP_MULTICAST
   p->imo = NULL;
#endif /* IP_MULTICAST */
#endif
   ...
   p->nb_pmtu = IPV6_MINMTU;      /* Set minimum IPv6 Path MTU */
#endif   /* IP_V6 */
#endif /* LINKED_PKTS */
   p->soxopts = (struct ip_socopts *)NULL;

   p->inuse = 1;  /* initially buffer in use by 1 user */

   /* reset all non-sticky bits in the 'flags' field */
   p->flags &= ~(PKF_MCAST | PKF_BCAST | PKF_IPOPTS | PKF_IPV6_GBL_PING);

   p->pRQB = LSA_NULL; /* INICHE_TCIP_ITGR */

   TCPIP_PK_ALLOC_INFO(p);/* INICHE_TCIP_ITGR allow action with allocated packet */
...
#endif /* HEAPBUFS */
   {
      ...
      if (pkt->nb_blen == bigbufsiz)  /* check in bigfreeq */
      {
         ...
         for (p=(PACKET)bigfreeq.q_head; p; p = p->next)
            ...
            {
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
               TCIP_INICHE_TRACE_01(0,LSA_TRACE_LEVEL_WARN,"pk_free: buffer %p already in bigfreeq", pkt);
#else
      ...
      else if (pkt->nb_blen == lilbufsiz)  /* check in lilfreeq */
      {
         ...
         {
#if 1 /* INICHE_TCIP_ITGR added LSA trace */
               TCIP_INICHE_TRACE_01(0,LSA_TRACE_LEVEL_WARN,"pk_free: buffer %p already in lilfreeq", pkt);
#else
...
{
#ifdef NPDEBUG /* INICHE_TCIP_ITGR variable is only used in case of NPDEBUG */
   int e;
#endif
...
#ifdef NPDEBUG /* INICHE_TCIP_ITGR begin: to improve performance according to AP00791882 */
...
#endif /* INICHE_TCIP_ITGR end: to improve performance according to AP00791882 */
...
#endif /* HEAPBUFS */
      {
         TCPIP_PK_FREE_INFO(pkt); /* INICHE_TCIP_ITGR allow action before freeing packet */
