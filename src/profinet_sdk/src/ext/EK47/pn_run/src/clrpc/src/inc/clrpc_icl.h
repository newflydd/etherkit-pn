...
typedef struct clrpc_cl_channel_tag {
	...
	LSA_UINT16  entered; /* refactor: as explained in the comment above enterd/again may be removed (TIA 212675) */
	LSA_UINT16  again;
...
typedef struct clrpc_client_tag {
	...
	LSA_UINT16  entered; /* refactor: as explained in the comment in CLRPC_CL_CHANNEL_TYPE enterd/again may be removed (TIA 212675) */
	LSA_UINT16  again;
...
} CLRPC_CLIENT_TYPE, *CLRPC_CLIENT_PTR_TYPE;

typedef CLRPC_CLIENT_TYPE const * CLRPC_CLIENT_CONST_PTR_TYPE;
/*-----------------------------------------------------------------------------
...
LSA_VOID clrpc_cl_udp_recv_done(
	CLRPC_CHANNEL_PTR_TYPE channel,
...
LSA_VOID clrpc_cl_provide(
	CLRPC_CHANNEL_PTR_TYPE channel,
	CLRPC_LOWER_RQB_PTR_TYPE lower
);

LSA_VOID clrpc_cl_event(
	CLRPC_CLIENT_PTR_TYPE client
);

/*---------------------------------------------------------------------------*/

LSA_UINT16 clrpc_cl_query_epm_bind(
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);

LSA_UINT16 clrpc_cl_query_epm_bind_done(
	LSA_UINT16 response,
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);

LSA_VOID clrpc_cl_query_epm_next(
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);

LSA_UINT16 clrpc_cl_query_epm_next_done(
	LSA_UINT16 response,
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);

LSA_UINT16 clrpc_cl_query_epm_break(
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);

LSA_UINT16 clrpc_cl_query_epm_break_done(
	LSA_UINT16 response,
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper
);
