...
static LSA_UINT16 OHA_GetMultiplePDEVData(
    LSA_UINT16 NICId,
    OHA_UPPER_MEM_U8_PTR_TYPE pData,
    OHA_COMMON_MEM_U32_PTR_TYPE pDataLen);

static LSA_VOID OHA_IndicateRemaPDEV(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaStation(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaNameOfStation(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaIPSuite(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaDHCP(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaSnmp(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);

static LSA_VOID OHA_IndicateRemaBus(
    LSA_UINT16 NICId,
    OHA_HDB_USER_PTR_TYPE pHDBUser);
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestRemaIndication(
    ...
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
	...
	if ((OHA_IS_NULL(pHDBRema)) || (!OHA_EDDCheckNICUsed(NICId)))
    {
		...
		OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_REMA_INDICATION failed. No valid NicID for this interface (0x%X)",NICId);

		OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestRemaIndication()");
		...
		RemaStationFormat != OHA_REMA_STATION_FORMAT_ADVANCED)
	{
        ...
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_REMA_INDICATION failed. No valid RemaStationFormat for this user (0x%X)", RemaStationFormat);
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_OPC_REMA_INDICATION failed. No valid Handle of this user (0x%X)", Handle);
    ...
    if (pHDBRema->PDEV_Last_Ind.data_tag)
    {
        ...
        if (!pHDBRema->Ind_Locked[Handle])
        {
            ...
            if (!pHDBRema->PDEV_Ind_Done[Handle])
            {
                ...
                data_length = pRQB->args.RemaData.data_size;

                Response = OHA_GetMultiplePDEVData(NICId,
                                                     pRQB->args.RemaData.data,
                                                     &data_length);

                pRQB->args.RemaData.data_length = data_length;
                pHDBRema->PDEV_Ind_Done[Handle] = LSA_TRUE; /* rema indicated */

                OHA_UserRequestFinish(pHDB,pRQB,Response);

                OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestRemaIndication(Resp: 0x%X)", Response);
                return;
            }
        }
        else
        {
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(PDEV) are locked (pHDB: 0x%X)", pHDB);
        }
    }
		...
		RemaStationFormat == OHA_REMA_STATION_FORMAT_STANDARD)
    {
        /* AP00958658, if not indicated */
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(MultipleStation) are locked (pHDB: 0x%X)", pHDB);
            }
        }
    }
		...
		RemaStationFormat == OHA_REMA_STATION_FORMAT_ADVANCED)
    {
        /* AP00958658, if not indicated */
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(NameOfStation) are locked (pUser: 0x%X)", pHDB);
            }
        }
    }
		...
		RemaStationFormat == OHA_REMA_STATION_FORMAT_ADVANCED)
    {
        /* AP00958658, if not indicated */
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(IPSuite) are locked (pUser: 0x%X)", pHDB);
            }
        }
    }
    ...
    {
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(DHCP) are locked (pUser: 0x%X)", pHDB);
            }
        }
    }
    ...
    if (pHDBRema->Snmp_Last_Ind.data_tag)
    {
        /* AP00958658, if not indicated */
        {
            ...
            {
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(SNMP) are locked (pHDB: 0x%X)", pHDB);
            }
        }
    }
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestRemaControl(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
    LSA_UINT16      NICId, RemaStationFormat;
    ...
    {
        OHA_UserRequestFinish(pHDB,pRQB,OHA_ERR_PARAM);
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_WARN, "ParamError - Mode (0x%X): OHA_RequestRemaControl()", pRQB->args.RemaControl.Mode);
        return;
    }
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL, "REMA-Control failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    switch (pRQB->args.RemaControl.Mode)
    {
        case OHA_HOLD_REMA_IND:  /* send no rema indications to the user */
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Control HOLD successful");
            pHDBRema->Ind_Locked[Handle] = LSA_TRUE;
            break;
        case OHA_UNHOLD_REMA_IND:  /* send rema indications to the user */
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Control UNHOLD successful");
        ...
        default:
            ...
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"REMA-Control failed.[0x%X]", Response);
            break;
    }

    OHA_UserRequestFinish(pHDB,pRQB,Response);

    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestRemaControl (Response: 0x%X)", Response);
}
    ...
    OHA_COMMON_MEM_U32_PTR_TYPE pDataLen)
{
    ...
    if (!OhaListIsEmpty(&pHDBRema->set_a))
    {
        ...
        for (OhaListForeach (rec, &pHDBRema->set_a, OHA_PRM_RECORD_PTR_TYPE))
        {
            ...
            {
                OHA_PROGRAM_TRACE_03 (0, LSA_TRACE_LEVEL_ERROR
                                      , "too much records in Set_A or too much data, NICId: 0x%X, list_cnt(%u) block_length(%u)"
                                      , NICId, list_cnt, block_length);
    ...
    if (OHA_IS_NOT_NULL(pData))
    {
        if (block_length > *pDataLen)
        {
            ...
            OHA_FUNCTION_TRACE_04(0, LSA_TRACE_LEVEL_NOTE
                                  , "OUT: OHA_GetMultiplePDEVData(NICId: 0x%X, RetVal: 0x%X)  got DataLen(%u) but need block_length(%u)"
    ...
    OHA_HDB_USER_PTR_TYPE    pHDBUser)
{
    ...
    {
        /* Error: no such interface. Abort. */
		OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_REMA_READ failed. No valid NicID for this interface (0x%X)",NICId);

		OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_RequestRemaRead()");
		OHA_UserRequestFinish(pHDBUser,pRQB,OHA_ERR_RESOURCE);
        return;
    }

    if ((LSA_HOST_PTR_ARE_EQUAL(pRQB->args.RemaData.data, LSA_NULL)) && (pRQB->args.RemaData.data_size))
    {
        OHA_FUNCTION_TRACE_00(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN, "ParamError, no buffer: OHA_RequestRemaRead()");
    ...
    switch (pRQB->args.RemaData.index)
    {
        ...
        default:
            ...
            OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_WARN, "ParamError, invalid index (0x%X)", pRQB->args.RemaData.index);
            break;
    }
    ...
    OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT,
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaPDEV failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    if (OHA_IS_NOT_NULL(pRQBBottom))
    {
        ...
        if (NICId == pHDBUser->Params.pDetail->Params.User.NICId)
        {
            ...
            if (!pHDBRema->Ind_Locked[Handle])
            {
                ...
                {
                    OHA_PROGRAM_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Bottom RQB is not the removed one (pRQBBottom: 0x%X, pRQB: 0x%X) - why?!", pRQBBottom, pRQB);
            ...
            else
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications are locked (pHDBUser: 0x%X)", pHDBUser);
            }
        }
        else
        {
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaStation failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaNameOfStation failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaIPSuite failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaDHCP(NICId: 0x%X) failed. No valid Handle of this user (0x%X)", NICId, Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaSnmp failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    OHA_HDB_USER_PTR_TYPE pHDBUser)
{
    ...
    if (!(Handle < OHA_CFG_MAX_CHANNELS))
    {
        OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_FATAL, "OHA_IndicateRemaBus failed. No valid Handle of this user (0x%X)", Handle);
        OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);  /* NOTREACHED */
        return;
    }
    ...
    if (OHA_IS_NOT_NULL(pRQBBottom))
    {
        ...
        if (NICId == pHDBUser->Params.pDetail->Params.User.NICId)
        {
            ...
            else
            {
                ...
                OHA_PROGRAM_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"REMA-Indications(BUS) are locked (pHDBUser: 0x%X)", pHDBUser);
            }
        }
        else
        {
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdatePDEVData(
    LSA_UINT16 NICId,
    LSA_BOOL  *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    LSA_UINT16 RetVal;
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the Multiple PDEV Record failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pPDEV_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pData),
                                     (LSA_UINT16)DataLength);
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication Multiple PDEV Record data failed (NICId: 0x%X) !", NICId);
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"Multiple PDEV Record data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdatePDEVData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pPDEV_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pPDEV_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated Multiple PDEV Record data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated Multiple PDEV Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateStationData(
    LSA_UINT16 NICId,
    LSA_BOOL  *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the Multiple Station Record failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pStation_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication Multiple Station Record data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of Multiple Station Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"Multiple Station Record data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateStationData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pStation_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pStation_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated Multiple Station Record data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated Multiple Station Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateNameOfStationData(
    LSA_UINT16 NICId,
    LSA_BOOL *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the NameOfStationRecord failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pNameOfStation_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication NameOfStationRecord data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of NameOfStationRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"NameOfStationRecord data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateNameOfStationData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pNameOfStation_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pNameOfStation_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated NameOfStationRecord data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated NameOfStationRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateIPSuiteData(
    LSA_UINT16 NICId,
    LSA_BOOL  *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the IPSuiteRecord failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pIPSuite_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication IPSuiteRecord data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of IPSuiteRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"IPSuiteRecord data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateIPSuiteData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pIPSuite_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pIPSuite_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated IPSuiteRecord data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated IPSuiteRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateDHCPData(
    LSA_UINT16 NICId,
    LSA_BOOL *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the DHCPParametersRecord failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pDHCPData_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication DHCPParametersRecord data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of DHCPParametersRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"DHCPParametersRecord data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateDHCPData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pDHCPData_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pDHCPData_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated DHCPParametersRecord data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated DHCPParametersRecord data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateSnmpData(
    LSA_UINT16 NICId,
    LSA_BOOL  *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the Multiple SNMP Record failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pSnmp_Last_Ind->data_length  != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR, "Alloc for the indication Multiple SNMP Record data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of Multiple SNMP Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"Multiple SNMP Record data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateSnmpData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pSnmp_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL (pSnmp_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated Multiple SNMP Record data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated Multiple SNMP Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_UINT16 OHA_UpdateBusData(
    LSA_UINT16 NICId,
    LSA_BOOL  *pDataChanged)
{
    OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_REMAGetHandleByNIC(NICId);
    ...
    *pDataChanged = LSA_FALSE;
    ...
    if (RetVal != OHA_OK)
    {
        OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Get length of the Multiple BUS Record failed (NICId: 0x%X, RetVal: 0x%X) !", NICId, RetVal);
    ...
    if (pBus_Last_Ind->data_length != 0 && DataLength != 0)
    {
        if (!DataChanged)   /* compare the rema data */
        {
            ...
            if (DataLength)
            {
                ...
                if (OHA_IS_NULL (pData))
                {
                    OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the indication Multiple BUS Record data failed (NICId: 0x%X) !", NICId);
                    return OHA_ERR_RESOURCE;
                }
                ...
                if (rc != OHA_OK)
                {
                    OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Temporary write of Multiple BUS Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                }
    ...
    if (!DataChanged)
    {
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE_LOW,"Multiple BUS Record data are the same (NICId: 0x%X)", NICId);
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_UpdateBusData(RetVal: 0x%X)", RetVal);
        return(RetVal);
    }
    ...
    if (DataChanged)
    {
        ...
        if (DataLength != 0)
        {
            OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(&pBus_Last_Ind->data),
                                 (LSA_UINT16)DataLength);
            if (OHA_IS_NULL(pBus_Last_Ind->data))
            {
                OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,"Alloc for the last indicated Multiple BUS Record data failed (NICId: 0x%X) !", NICId);
                return OHA_ERR_RESOURCE;
            }
        }
        ...
        if (rc != OHA_OK)
        {
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_ERROR,"Storage of the last indicated Multiple BUS Record data failed (NICId: 0x%X, rc: 0x%X) !", NICId, rc);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
        }
    }
...
/*----------------------------------------------------------------------------*/
static LSA_VOID OHA_IndicateRemaPDEVUsers(
    LSA_UINT16 NICId,
    LSA_HANDLE_TYPE Orginator)
{
    ...
    switch (OHA_HANDLE_GET_TYPE(Orginator))
    {
        ...
        case OHA_HANDLE_TYPE_DCP:
            break;
        default:
            OHA_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL,"Unknown Orginator (NICId: 0x%X, 0x%X)!", NICId, Orginator);
            OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
            break;
    }
...
LSA_VOID oha_rema_init(LSA_VOID) /* AP01039458 */
{
    LSA_UINT32 i;
    ...
    {
        OHA_HDB_REMA_PTR_TYPE pHDBRema = OHA_GET_HREMA_PTR(i);
...
static LSA_VOID oha_rema_prm_free_list (
...
/*----------------------------------------------------------------------------*/
LSA_VOID oha_station_undo_init(
    LSA_UINT16               NICId
)
{
    OHA_HDB_REMA_PTR_TYPE    pHDBRema;
...
/*----------------------------------------------------------------------------*/
LSA_VOID oha_snmp_undo_init_global( LSA_VOID )
{
    OHA_HDB_REMA_PTR_TYPE    pHDBRema;
    LSA_UINT32               i;

    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT,
                          "IN : oha_snmp_undo_init_global()");

	for ( i=0;( i < OHA_CFG_MAX_NICS); i++ )
	{
		OHA_UPPER_REMA_DATA_PTR_TYPE pSnmp_Last_Ind;

		pHDBRema = OHA_GET_HREMA_PTR(i);
        pSnmp_Last_Ind = &pHDBRema->Snmp_Last_Ind;
    ...
    /***/
    OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_snmp_undo_init_global() done");
    return;
}

/*----------------------------------------------------------------------------*/
LSA_VOID oha_bus_undo_init(
    LSA_UINT16               NICId
)
{
    OHA_HDB_REMA_PTR_TYPE    pHDBRema;
...
/*----------------------------------------------------------------------------*/
LSA_VOID oha_rema_prm_free_recordset_a(
    LSA_UINT16               NICId
)
{
    OHA_HDB_REMA_PTR_TYPE    pHDBRema;
...
static LSA_VOID oha_rema_prm_add_or_replace (
    LSA_UINT16              NICId,
    OHA_PRM_RECORD_PTR_TYPE  prm
)
{
...
LSA_UINT16 oha_rema_prm_prepare(
    LSA_UINT16 NICId,
    LSA_BOOL is_commit
)
{
    ...
    switch (pHDBRema->rema_state)
    {
        ...
        case OHA_PRM_STATE_COMMIT:
            ...
            OHA_FATAL (pHDBRema->rema_state);  /* NOTREACHED */
            return OHA_ERR_SEQUENCE;

        default:
            OHA_FUNCTION_TRACE_01 (0, LSA_TRACE_LEVEL_FATAL, "wrong state(%u)", pHDBRema->rema_state);
            OHA_FATAL (pHDBRema->rema_state);  /* NOTREACHED */
            return OHA_ERR_SEQUENCE;
    }
...
)
{
    ...
    switch (pHDBRema->rema_state)
    {
        ...
        default:
            OHA_FUNCTION_TRACE_01 (0, LSA_TRACE_LEVEL_FATAL, "sequence.error within state(%u)", pHDBRema->rema_state);
            OHA_FATAL (pHDBRema->rema_state);  /* NOTREACHED */
            return OHA_ERR_SEQUENCE;
    }
    ...
    {
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_ERROR,
    ...
    OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(& prm), sizeof(*prm));

    if (OHA_IS_NULL (prm))
    {
        OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_WARN, "OUT: oha_rema_prm_write(NICId: 0x%X) done (OHA_ERR_RESOURCE)", NICId);
        return OHA_ERR_RESOURCE;
    }
    ...
    OHA_ALLOC_LOCAL_MEM ((OHA_UPPER_MEM_PTR_TYPE *)(& prm->rec.record_data),
...
)
{
    ...
    /***/
    {
        ...
        OHA_FUNCTION_TRACE_00 (0, LSA_TRACE_LEVEL_CHAT, "rema-store: BEGIN-OF-DUMP");

        for ( OhaListForeach(prm, &pHDBRema->set_b, OHA_PRM_RECORD_PTR_TYPE) )
        {
            OHA_FUNCTION_TRACE_05 (0, LSA_TRACE_LEVEL_CHAT
                                   , "rema-store: NICId(0x%X) index(%#x) slot(%u) subslot(%#x) data_length(%u)"
                                   , NICId
        ...
        OHA_FUNCTION_TRACE_00 (0, LSA_TRACE_LEVEL_CHAT, "rema-store: END-OF-DUMP");
    }

    /***/
    OHA_FUNCTION_TRACE_01(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_rema_prm_end() done (%u)", rc);
    return rc;
}
...
LSA_UINT16 oha_rema_prm_commit(
    LSA_UINT16      NICId,
...
)
{
    ...
    switch (pHDBRema->rema_state)
    {
        ...
        default:
            OHA_FUNCTION_TRACE_01 (0, LSA_TRACE_LEVEL_ERROR, "sequence.error within state(%u)", pHDBRema->rema_state);
            rc = OHA_ERR_SEQUENCE;
            return (rc);
    }
...
LSA_UINT16 oha_rema_station_update(
    LSA_UINT16      NICId
)
{
    LSA_BOOL DataChanged = LSA_FALSE;
...
LSA_UINT16 oha_rema_dhcp_update(
    LSA_UINT16      NICId
)
{
    LSA_BOOL DataChanged = LSA_FALSE;
...
LSA_VOID oha_rema_snmp_update_global( LSA_VOID )
{
    OHA_DB_IF_PTR_TYPE    pDB;
	LSA_UINT32 i;

	OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT,
		"IN : oha_rema_snmp_update_global()");

	for(i=0, pDB=&oha_database.IfData[0]; i < OHA_CFG_MAX_NICS; i++, pDB++)
	{
		if (pDB->Existing)
		{
			(LSA_VOID)oha_rema_snmp_update(pDB->NicId);
		}
	}

	OHA_FUNCTION_TRACE_00(0, LSA_TRACE_LEVEL_CHAT, "OUT: oha_rema_snmp_update() done");
	return;
}

/*----------------------------------------------------------------------------*/

LSA_UINT16 oha_rema_snmp_update(
    LSA_UINT16      NICId
)
{
    LSA_BOOL DataChanged = LSA_FALSE;
    ...
    if (DataChanged)
    {
        ...
        for (i=0; i < OHA_CFG_MAX_CHANNELS; i++)
        {
			...
			if (OHA_IS_NOT_NULL(pHDBUser) && (pHDBUser->State == OHA_HANDLE_STATE_OPEN) ) /* user channel is opened ? */
			{
				...
				if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER)
				{
					OHA_IndicateRemaSnmp(pHDBUser->Params.pDetail->Params.User.NICId, pHDBUser);
				}
			}
        }
    }
...
LSA_UINT16 oha_rema_bus_update(
    LSA_UINT16      NICId
)
{
    LSA_BOOL DataChanged = LSA_FALSE;
    ...
    if (DataChanged)
    {
        ...
        for (i=0; i < OHA_CFG_MAX_CHANNELS; i++)
        {
			...
			if (OHA_IS_NOT_NULL(pHDBUser) && (pHDBUser->State == OHA_HANDLE_STATE_OPEN) ) /* user channel is opened ? */
			{
				...
				if (pHDBUser->Params.pDetail->PathType == OHA_PATH_TYPE_USER)
				{
		            OHA_IndicateRemaBus(pHDBUser->Params.pDetail->Params.User.NICId, pHDBUser);
				}
			}
        }
    }
