...
/*  2003-05-16  Neuer Identify mit Faltungsfaktor wird unterst�tzt           */
...
/*              mehreren Filtern gleichzeitig ist m�glich.                   */
/*              Filter 04 DetailedTypeOfStation wird unterst�tzt             */
/*              R�ckgabe des TCP-Datensatzes in der Identify.Responese.      */
...
/*              Ueberpr�fen, ob ein Timer mit gleicher Transaction-ID und    */
...
/*  2014-11-21  P06.00.00.00_00.01.48.01 1126404: SET-Statemachine gem�� IEC-*/
/*              Automat DCPUCR ununterbrechbar machen (Redo ServerSetWait)   */
/*  2014-11-25  P05.03.00.00_00.03.24.01 1157678: DCPBlockLength ist gr��er  */
/*              als die Frame-L�nge                                          */
/*  2014-12-02  P06.00.00.00_00.01.48.01 1126404: SET-Statemachine Teil 2    */
/*              SET-TLV-Pr�fungen aus OHA �bernehmen --> dcp_SetCheck()      */
...
DCP_UPPER_RQB_PTR_TYPE	RbuPtr)
{
	...
	else if (ChPtr->ChNextPtr->DetailPtr->AllocOnRequest == DCP_CDB_ALLOC_LOWER_RQBS_AT_REQUEST)
	{
		...
		*/
			...
			DCP_NUM_SERVER_GET_RESOURCES <= ChPtr->QueueInd.Count))
		{
			...
			*/
			...
			{
				DCP_CHA_PTR UpperChPtr;
				/* Not enough lower Receive RQBs
				*/
				RespVal = DCP_RSP_ERR_RESOURCE;
				dcp_GetUpperQ(&ChPtr->QueueInd, RbuPtr);
				UpperChPtr = dcp_EnqueUpper(RbuPtr);
				DCP_ERROR_TRACE_04(ChPtr->TraceIdx, LSA_TRACE_LEVEL_ERROR, "*** dcp_RecvIndReq() ChPtr=%X RbuPtr=%X Lower QueueRecv.Count=%u UpperChPtr=%X", 
								ChPtr, RbuPtr, ChPtr->ChLowPtr->QueueRecv.Count, UpperChPtr);
			}
			else
			{
...
DCP_UPPER_RQB_PTR_TYPE	RbuPtr)
{
	...
	LSA_BOOL		SendResponse = LSA_TRUE;
	...
	{
		ChPtr->ServerSetWait = LSA_FALSE;	//1126404: leave WACK state
		if (0 == OptCnt)
		{
			/* 1126404: only leave WACK state if no options in SET response
			*/
			SendResponse = LSA_FALSE;
		}
	}
	...
	if (SendResponse)
	{
		...
		if ((ChPtr->QueueLower.Count != 0) && (DCP_RSP_OK == RespVal))
		{
			...
			*/
			...
			{
				DsPtr = (DCP_DATASET *)(void*)(RbuPtr->args.SendRsp.DataPtr + DataLen);
				...
				*/
				...
				else
				{
					...
					*/
					...
					if (Unlock)     //lint !e774 Boolean within 'if' always evaluates to False, depends on DCP_UNLOCK_ON_TLV, HM 13.01.2016
					{
	...
	DCP_FUNCTION_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  < dcp_SendRspReq() ChPtr=%X ServerSetWait=%u RespVal=0x%x", 
			ChPtr, ChPtr->ServerSetWait, RespVal);
...
DCP_UPPER_RQB_PTR_TYPE	RbuPtr)
{
	...
	if (LSA_HOST_PTR_ARE_EQUAL(ChPtr->CancelRbu, LSA_NULL))
	{
		if (0 == RbuPtr->args.Cancel.Count)
		{
			...
			*/
			CancelCount = (LSA_INT)(ChPtr->QueueIdentify.Count + ChPtr->QueueGet.Count + ChPtr->QueueSet.Count);
			if (CancelCount != 0)
			{
			...
			else if(ChPtr->QueueHello.Count != 0)
			{
				...
				*/
				CancelCount = (LSA_INT)(ChPtr->QueueHello.Count);
		...
		else
		{
			...
			*/
			if (RbuPtr->args.Cancel.Count < (LSA_INT)ChPtr->QueueInd.Count)
				...
				CancelCount = (LSA_INT)(ChPtr->QueueInd.Count);
...
DCP_CHA_PTR 			ChPtr,
DCP_LOWER_RQB_PTR_TYPE  RblPu,
const DCP_USR_BYTE_PTR	AdrPtr,
LSA_UINT8 				ServiceId,
LSA_UINT32 				TransactId,
const DCP_USR_BYTE_PTR	DataPtr,
LSA_INT					DataLen)
{
	...
	if (ChPtr->DetailPtr->DevId == DCP_DEV_ETH)
	{
		...
		/* Ethernetheader
		*/
		DCP_ETH_FRAME_MACADR_SET(&EthFramePtr->Hdr.DstMacAdr[0], AdrPtr);
...
/*                                                                           */
/*****************************************************************************/
static LSA_INT dcp_SetCheck(
DCP_CHA_PTR 			ChPtr,
LSA_UINT8			*	DataPtr, 
LSA_INT					DataLen)
{
	LSA_UINT8	*	ReadPtr = DataPtr; 
	LSA_INT			RestLen = DataLen;
	LSA_INT			OptCount = 0;
    LSA_UINT16		RetVal = DCP_RET_OK;
    ...
    DCP_FUNCTION_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"  > dcp_SetCheck() ChPtr=%X DataPtr=%X DataLen=%u",
    					ChPtr, DataPtr, DataLen);

    /* check TLV-chain
    */
    while ((RestLen > 0) && (DCP_RET_OK == RetVal))
    {
        DCP_DATASET	*	DsPtr = (DCP_DATASET *)(void*)ReadPtr;
        LSA_INT			ReadLen = DCP_DS_HDR_SIZE + DCP_SWAP16(DsPtr->Hdr.Len);

		if (RestLen < ReadLen)
		{
			/* 1157678: Truncated dataset
			*/
			DCP_ERROR_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() Truncated block (%u)", OptCount);
		...
		else
		{
			...
			{
				/* TFS 852764/1126404: Check coding of the DCP-Set-ReqPDU, see PNIO draft
				*/
				switch (DsPtr->Hdr.Id)
				{
					case DCP_DSID_START:
					{
						/* StartTransactionBlock must be the first and only once
						*/
						if (OptCount != 1)
						{
							DCP_ERROR_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() StartTransactionBlock not first block (%u)", 
									OptCount);
							RetVal = LSA_RET_ERR_PARAM;
						}
					...
					case DCP_DSID_END:
					{
						/* StopTransactionBlock must be the last and only once
						*/
						if (0 != RestLen)
						{
							DCP_ERROR_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() StopTransactionBlock not last block (%u)",
									OptCount);
							RetVal = LSA_RET_ERR_PARAM;
						}
					...
					case DCP_DSID_RESETFACT:
					case DCP_DSID_RESET:
					{
						/* ResetToFactory or FactoryReset only once
						*/
						if(FactoryResetBlock)
						{
							DCP_ERROR_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() more than one FactoryResetBlock(%u)",
									OptCount);
							RetVal = LSA_RET_ERR_PARAM;
						}
	...
	if ((DCP_RET_OK == RetVal) && (FactoryResetBlock))
	{
		if (AnyOtherBlock)
		{
			DCP_ERROR_TRACE_00(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() FactoryResetBlock and other block existing");
			RetVal = LSA_RET_ERR_PARAM;
		}
		if (StartTransBlock && !StopTransBlock)
		{
			DCP_ERROR_TRACE_00(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"*** dcp_SetCheck() missing StopTransBlock");
			RetVal = LSA_RET_ERR_PARAM;
		}
	}

	if (DCP_RET_OK != RetVal)
	{
		OptCount = 0;
	}

    DCP_FUNCTION_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE,"  < dcp_SetCheck() OptCount=%u", OptCount);
    return(OptCount);
}
...
DCP_CHA_PTR 		ChPtr)
{
	DCP_UPPER_RQB_PTR_TYPE	RbPtr;
	LSA_UINT8			*	WritePtr;
	LSA_UINT8			*	ReadPtr = (LSA_UINT8*)&ReqPtr->Data.Set.Dataset;
	...
	*/
	...
	{
		/* 1126404: check option count fitting for response
		*/
		OptCount = dcp_SetCheck(ChPtr, ReadPtr, RestLen);
		if ((0 == OptCount) || (DCP_MAX_SET_OPT_COUNT < OptCount))
		{
			RetVal = LSA_RET_ERR_PARAM;
		}
	}
	if (LSA_RET_OK == RetVal)
	{
		...
		*/
		ChPtr->ServerSetWait = LSA_TRUE;	//1126404: enter WACK state
		...
		while (RestLen > 0)
		{
			...
			else
			{
				...
				if (RestLen != 0)
				{
					...
					*/
					ReadPtr += ReadLen;
					DsPtr = (DCP_DATASET_IDENT *)ReadPtr;       //lint !e826 Suspicious pointer-to-pointer conversion, on purpose, HM 13.01.2016
				}
			}
		}
		if (0 == OptCount)
		...
		*/
		...
		if (LSA_RET_OK == RetVal)
		{
			...
			*/
			...
			if  (  (LSA_RET_OK == RetVal) 
                && (Faltung != 0)  )        //Avoid Lint msg 414: Possible division by 0
			{
		...
		while (RestLen > 0)
		{
			...
			else
			{
				...
				if (RestLen != 0)
				{
					...
					*/
					ReadPtr += ReadLen;
					DsPtr = (DCP_DATASET *)ReadPtr;     //lint !e826 Suspicious pointer-to-pointer conversion, on purpose, HM 13.01.2016
				}
			}
		}
		if (0 == OptCount)
...
/*****************************************************************************/
static LSA_UINT16 	DCP_LOCAL_FCT_ATTR dcp_UnknownService(
DCP_REQ_PTR			ReqPtr,
LSA_INT				ReqLen,
const DCP_ETHDR	*	HdrPtr,
DCP_CHA_PTR 		ChPtr)
{
	LSA_UINT16				RetVal = LSA_RET_OK;
...
/*****************************************************************************/
static LSA_BOOL 	DCP_LOCAL_FCT_ATTR dcp_LockOnClient(
DCP_ETHDR		*	HdrPtr,
DCP_CHA_PTR 		ChPtr)
{
	DCP_LOWER_RQB_PTR_TYPE  RblPu;
	...
	DCP_FUNCTION_TRACE_03(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, "  > dcp_LockOnClient() ChPtr=%X HdrPtr=%X ServerSetWait=%u", 
			ChPtr, HdrPtr, ChPtr->ServerSetWait);

	if (!ChPtr->ServerSetWait)	//1126404: ignore any SET/GET in WACK state
	{
...
DCP_CHA_PTR 		ChPtr)
{
	     ...
	     &&  (DataLen <= (PduLen - DCP_HEADER_SIZE)))
	{
		...
		&&  (DCP_FRAMEID_0 == FrameId0))    //lint !e774 Boolean within (DCP_FRAMEID_0 == FrameId0) always evaluates to True, keep it for better readability, HM 13.01.2016
		{
...
/*****************************************************************************/
static LSA_VOID 	DCP_LOCAL_FCT_ATTR dcp_ServerClose(
DCP_CHA_PTR 			ChPtr)
{
	DCP_UPPER_RQB_PTR_TYPE	RbPtr;
			...
			else
			{
				...
				*/
				...
				if (0 != ChPtr->QueueHello.Count)
				{
					...
					*/
					if (DCP_ETH_RSP_OK != DCP_ETH_RQB_RESPONSE_GET (RblPu.RbEthPtr))
					{
						dcp_ErrorLower (DCP_MODULE_ID, __LINE__, DCP_ERR_FATAL,
							RblPu.RbEthPtr, sizeof (DCP_ETH_RQB_TYPE));
					}
					ChPtr->CancelRbu->args.Cancel.Count += (LSA_INT)ChPtr->QueueHello.Count;
			...
			else
			{
				...
				*/
				...
				else
				{
					...
					*/
					Response = DCP_RSP_OK;
					MaxCount = (LSA_INT)(ChPtr->ChNextPtr->QueueHello.Count);
...
//040811lrg		RbPtr->err = DCP_ETH_RQB_ERR_GET(RblPu.RbEthPtr);
		...
		else
		{
			...
			*/
			...
			FrameId0 = *BytePtr++;
			FrameId1 = *BytePtr++;
			ReqPtr = (DCP_REQ_PTR)(void*)BytePtr;
			...
			*/ 
			default:
				...
				if (ChPtr->DetailPtr->DevId == DCP_DEV_ETH)
                {
                    LSA_UINT16 USRetVal = dcp_UnknownService(ReqPtr, ReqLen, HdrPtr, ChPtr);
                    DCP_FUNCTION_TRACE_01(ChPtr->TraceIdx, LSA_TRACE_LEVEL_NOTE, " -  dcp_ServerInd() dcp_UnknownService() RetVal==%X", 
                                          USRetVal);
                }
		...
		if (!LSA_HOST_PTR_ARE_EQUAL(RblPu.RbEthPtr, LSA_NULL))
		{
			...
			*/
			...
			else if (ChPtr->DetailPtr->AllocOnRequest == DCP_CDB_ALLOC_LOWER_RQBS_AT_OPEN_CHANNEL)
			{
