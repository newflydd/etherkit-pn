	...
	)
{
	...
	{
	LSA_UINT32  alloc_size = sizeof(*channel->usr.edd.ports) * (edd_params->MaxPortCnt + 1);
	CM_ALLOC_LOCAL_MEM (CM_CAST_LOCAL_MEM_PTR_PTR (& channel->usr.edd.ports), alloc_size);
	if (cm_is_null (channel->usr.edd.ports))
	{
		CM_FATAL();
	}
	}
...
# error "EDD_PORT_ID_AUTO not 0"
	...
	{
	LSA_UINT16  port_id;
	for (port_id = 0/*auto*/; port_id <= edd_params->MaxPortCnt; ++port_id)
	{
		...
		edd = cm_edd_alloc_rqb (channel, EDD_SRV_LINK_STATUS_IND_PROVIDE_EXT);
		if (cm_is_null(edd))
		{
		...
		{
			cm_edd_request_lower (channel, EDD_SRV_LINK_STATUS_IND_PROVIDE, 0, edd);
		}
		else
		{
			cm_edd_request_lower (channel, EDD_SRV_LINK_STATUS_IND_PROVIDE_EXT, 0, edd);
		}
		}
	}
	}
	...
	)
{
	...
	CM_ASSERT (cm_is_not_null (edd_channel->usr.edd.ports));
	...
	)
{
	...
	CM_EDD_TRACE_03(channel->trace_idx, LSA_TRACE_LEVEL_NOTE
		, "edd-status(%u) edd-speed(%u) edd-mode(%u)"
		, ps->status, ps->speed, ps->mode
		);

	/***/

	if( ! (ps->status == EDD_LINK_UP || ps->status == EDD_LINK_UP_CLOSED) ) { /* TIA 1103363: "closed" is ok too (lasts < 100ms) */
	...
	if(
		ps->speed == EDD_LINK_UNKNOWN /* note: always false, see cm_edd_link_indication() */
		|| ps->speed == EDD_LINK_SPEED_10
	) {
