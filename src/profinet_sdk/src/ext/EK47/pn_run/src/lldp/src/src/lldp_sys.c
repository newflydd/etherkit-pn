...
static struct LLDP_STATIC_TYPE_
{
...
static LLDP_RSP LLDP_FreeMgm(LSA_VOID)
{
...
LLDP_RSP lldp_init(LSA_VOID)
{
    ...
    {
        g_pLLDPInfo->HUser.HandleCnt = LLDP_MAX_CHANNELS;
        g_pLLDPInfo->HUser.UsedCnt   = 0;

        for ( i=0; i<LLDP_MAX_CHANNELS; i++)
        {
...
LLDP_RSP lldp_undo_init(LSA_VOID)
{
