/*
 * v3.1 August 30, 2011 allports/allports.c
...
#define LTRC_ACT_MODUL_ID 10 /* INICHE_TCIP_ITGR added for LSA traces */
...
#endif /* IP multicast and (IGMPv1 or IGMPv2 or IGMPv3) */
...
#ifdef IN_MENUS /* INICHE_TCIP_ITGR added #ifdef */
#include "menu.h"
#endif
...
#endif
#ifdef OSPORT_H
#if 0 /* INICHE_TCIP_ITGR not used */
#include OSPORT_H
#endif
#endif
...
#ifdef INCLUDE_TRAFTEST
extern   int   prep_traftest(void);
#endif
...
#ifdef  IP_MULTICAST_TEST
extern   void  u_mctest_init(void);
#endif   /* IP_MULTICAST_TEST */
...
extern   void  ssh_cleanup(void);
extern   int   prep_ssh(void);
#endif

#ifdef USE_SCP
extern   int   scp_init(void);
#endif
...
#if (defined (DHCP_CLIENT) || defined (USE_AUTOIP))
int addr_setup (void);
#endif /* DHCP_CLIENT or USE_AUTOIP */
...
netmain_init(void)
{
...
#ifdef SYS_SHUTDOWN
   printf("\n*** Execution limited to %d minutes. ***\n\n", SYS_SHUTDOWN/60);
#endif
#endif
#ifdef IN_MENUS
   ...
   be overwritten from command line parms or DHCP later. */
   {
      ...
      for (i = 0; i < STATIC_NETS; i++)
      {
         ...
         if ((inet_nvparms.ifs[i].igmp_admin_mode != IGMP_MODE_V1) &&
             (inet_nvparms.ifs[i].igmp_admin_mode != IGMP_MODE_V2) &&
             (inet_nvparms.ifs[i].igmp_admin_mode != IGMP_MODE_V3))
            netstatic[i].igmp_oper_mode = netstatic[i].igmp_admin_mode = IGMP_MODE_DEFAULT;
         else
            netstatic[i].igmp_oper_mode = netstatic[i].igmp_admin_mode = inet_nvparms.ifs[i].igmp_admin_mode;
   ...
   if (msg)
   {
#if 1 /* INICHE_TCIP_ITGR added LSA trace, %s replaced by XX */
      TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_FATAL,
               "inet startup error: XX");
#else
...
#ifdef SUPERLOOP
/* FUNCTION: post_netmain_init()
 * 
 * This function invokes initialization code for modules that require
 * the TCP/IP stack to be fully operational prior to some (or all) of
 * their initialization.  It is intended to be invoked after the basic 
 * TCP/IP stack has come up (as indicated by 'iniche_net_ready' having 
 * a value of 1), but before the code enters the main while (1) {} 
 * loop (in tk_yield).
 *
 * This function provides the SNTPv4 client with its configuration
 * information (which may include a time server's DNS name that needs 
 * to be resolved).  This information is used by the client in 
 * subsequent time synchronization attempts.
 * 
*/
void 
post_netmain_init(void)
{
#ifdef USE_SNTP_V4
   {
      int e;
...
station_state(void * pio)
{
   ...
   int i; /* INICHE_TCIP_ITGR moved i into #ifndef */
   NET ifp;
...
sysuptime()
{
   return (unsigned long)((cticks/TPS)*100);    /* 100ths of a sec since boot time (INICHE_TCIP_ITGR cast added) */
}
...
int e = 0;
   ...
   if (e != 0)
   {
      dprintf("SSH Module prep failed\n");
      panic("prep_modules");
   }
#endif   /* USE_SSH */
   ...
   if (e != 0)
   {
      dprintf("SCP Module prep failed\n");
      panic("prep_modules");
   }
#endif   /* USE_SCP */
...
/* depending upon configuration, wait for either the DHCP or UPNP 
 * state machine(s) to complete their configuration process 
 */
#if (defined (DHCP_CLIENT) || defined (USE_AUTOIP))
int addr_setup (void)
{
   ...
   return dhc_loop ();
#elif (defined (DHCP_CLIENT) && defined (USE_AUTOIP))
   /* run the UPNP state machine */
   return upnp_loop ();
#endif /* DHCP_CLIENT, USE_AUTOIP */
}
#endif /* DHCP_CLIENT or USE_AUTOIP */
