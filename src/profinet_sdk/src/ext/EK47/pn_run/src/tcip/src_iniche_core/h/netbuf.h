/*
 * v3.1 August 30, 2011 h/netbuf.h
...
#if 1 /* INICHE_TCIP_ITGR added pointer to RQB */
   void  *pRQB;
#endif
};
 ...
 */
#define BAD_REGULAR_BUF_LEN_ERR   0 /* number of times when pk_validate () discovered a buffer of length that
...
#define GUARD_BAND_VIOLATED_ERR   1 /* number of instances when pk_validate () discovered that a regular buffer's
                                     * left or right guard band was violated */
#define MULTIPLE_FREE_ERR         2 /* number of instances when pk_validate () discovered that a regular buffer
                                     * (that was being freed via pk_free ()) already existed on the little or big
                                     * free buffer queues */
#define INCONSISTENT_LOCATION_ERR 3 /* number of instances when pk_validate () discovered that a packet's 
