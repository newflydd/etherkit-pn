...
/**
 * This macro can be used to fill the "additional transfers-descriptor" of function
 * gdma_add_transfers_to_job().
 */
#define GDMA_SET_ADDITIONAL_TRANSFERS_DESCR(_trans_desc_ptr, _num_transfers, _transfers)\
{                                                                                                                               \
    ((gdma_add_transfers_descriptor_t*) _trans_desc_ptr)->num_transfers   = _num_transfers;                                     \
    ((gdma_add_transfers_descriptor_t*) _trans_desc_ptr)->transfers       = _transfers;                                         \
}

/**
 * you can use this macro to initialize the structure "gdma_job_descriptor_t", which is used to
 * allocate a new GDMA job.
 * @see gdma_add_job()
 */
#define GDMA_SET_JOB_DESCR(_job_desc_ptr, _num_transfers, _transfers, _trigger, _priority, _hw_flow_src, _job_done_callback, _cbf_arg)\
{                                                                                                                               \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->num_transfers     = _num_transfers;                                               \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->transfers         = _transfers;                                                   \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->trigger           = _trigger;                                                     \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->priority          = _priority;                                                    \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->hw_flow_src       = _hw_flow_src;                                                 \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->job_done_callback = _job_done_callback;                                           \
    ((gdma_job_descriptor_t*) _job_desc_ptr)->cbf_arg           = _cbf_arg;                                                     \
}
...
typedef struct
{
    ...
    LSA_UINT32          transfer_count;     /**< number of elements to be transferred */
...
LSA_VOID  gdma_enable_all_jobs(LSA_VOID);
...
/** GDMA_IS_JOB_RUNNING() returns LSA_TRUE if the the job, referenced by job_handle, is active (= started / running / interrupted)
 *  Attention: the macro doesn't check, if the job belongs to the currently active pool! You have to make sure this by yourself! */
#define GDMA_IS_JOB_RUNNING(job_handle) ((*((volatile LSA_UINT32*) (GDMA_CFG_BASE_ADDRESS + 0x90)) & (1 << (job_handle % 32))) ? LSA_TRUE : LSA_FALSE)
