...
LSA_VOID cm_oneshot_trigger(
	...
	if( oneshot->running ) { /* running */
		...
		if( cm_is_not_null(next) ) {
			next->ticks += oneshot->ticks; /* give the ticks to the successor */
		}
	}

	CM_ASSERT(cm_is_null(oneshot->dlink.Flink) && cm_is_null(oneshot->dlink.Blink)); /* sanity check */
...
//----------------------------------------------------------------------------*/
LSA_VOID cm_timeout_synchronized(
	...
	if( cm_is_not_null(oneshot) && oneshot->ticks > 0 && --oneshot->ticks == 0 ) {
		...
		} while( cm_is_not_null(oneshot) && oneshot->ticks == 0 );
...
LSA_UINT16 cm_oneshot_stop(
	...
	if( oneshot->running ) { /* running */
		...
		CM_ASSERT(cm_is_null(oneshot->dlink.Flink) && cm_is_null(oneshot->dlink.Blink));
