...
/*****************************************************************************/
#if (SOCK_CFG_SNMP_ON) && (SOCK_CFG_INTERNICHE) && (!SOCK_CFG_USE_OBSD_SNMPD) /* SNMP Agent Mulitcast */
/*****************************************************************************/
#include "ipport.h"
#include "tcpport.h"
#include "bsdsock.h"
#include "snmp_imp.h" // snmp_agt_parse()
#include "anyip_edd.h"
...
static sock_snmp_activate_int(
	...
	if ((lower_handle = sock_low_socket(SOCK_LOW_DGRAM,  &native_error)) != SOCK_LOW_INVALID_HANDLE)
	{
		soc = sock_socket_alloc(p_cha, lower_handle, SOCK_LOW_DGRAM, (SOCKET_OPTION*)0 /* socket_specific_options */, (SOCKET_OPTION_VAL_TYPE)0 /* option_value */ );
	}
	...
	soc->loc_addr.sin_family = SOCK_AF_INET;
	...
	if (response != SOCK_RSP_OK)
	{
		SOCK_PROTOCOL_TRACE_03(0, LSA_TRACE_LEVEL_ERROR
			, "lower_handle(0x%x) CLT BIND error(%d) response(%d)"
	...
	SOCK_ALLOC_LOCAL_MEM(&p_tmp, SOCK_SNMP_BUFFER_SIZE );
...
sock_snmp_activate(
	...
	LSA_UINT16 response = SOCK_RSP_OK_ACTIVE;
...
sock_snmp_dispatch_request(
	...
	else   /* pass data to internal agent */
	{
		 ...
		 */
		LSA_BOOL pending = (ANYIP_TCIP_STATISTICS_PENDING_YES == tcip_mib2_statistics_update() ? LSA_TRUE : LSA_FALSE);
...
sock_snmp_dispatch_intern(
	LSA_VOID* p_snmp_data,
	LSA_UINT32 p_snmp_data_length,
	SOCK_SOCKADDR_TYPE const * p_rem_addr
) {
	SOCK_LOG_FCT("sock_snmp_dispatch_intern")

	LSA_UINT16 ret_val;

	ret_val = (LSA_UINT16)snmp_agt_parse((LSA_UINT8*)p_snmp_data
										, (LSA_UINT)p_snmp_data_length
										, (LSA_UINT8*)sock_data.snmp_int_agent_return_buf
	...
	if( ret_val != 0 )
	{
		...
		send.sock_fd = sock_data.snmp_dev->dev_handle;
		send.buffer_length = SOCK_SNMP_BUFFER_SIZE;
...
sock_snmp_dispatch_extern(
	...
	else
	{
		...
		{
			SOCK_FATAL(); /* impossible (see sock_usr.h) */
		}
		else
		{
			...
			recv->snmp_done = SOCK_SNMP_DONE_NO; /* reset done flag */
...
sock_snmp_receive_rsp(
	...
	if( rcv->snmp_done == SOCK_SNMP_DONE_YES )
	{
	...
	else
	{
		...
		LSA_UINT32 next_snmp_handle = p_rqb->args.snmp_recv.snmp_handle + 1;
