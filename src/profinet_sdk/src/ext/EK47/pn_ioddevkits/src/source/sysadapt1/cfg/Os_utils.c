...
/*****************************************************************************/
	...
	#define LOG_LSA_PACK_ID		LSA_COMP_ID_SYSADAPT
	#define LOG_LSA_MOD_ID		LTRC_ACT_MODUL_ID

    // **** external variables ****
    extern  PNIO_UINT32 clock (void);
    
    // **** static variables ****
#if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200_400)
    static volatile PNIO_CYCLE_INFO_PTR  pSwiTim  = {0, 0, 0, 0};    // hardwaretimer in ERTEC switch
#endif
    ...
    #if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200_400)
        #define CIRCUF_HW_BASED   1    // 1:  hw based timer for OsSetCircBufEntry ()
    #else
    ...
    // *--------------------------------------------*
    void OsEnableCircBuf (PNIO_BOOL Enable)
    {
    ...
    // *--------------------------------------------*
    void OsResetMaxMinCircBuf (LSA_UINT32 BufIndex)
    {
    ...
    void OsInitCircBuf (void)
    {
	    ...
	    #if (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200_400)
		    // *** get pointer to ertec hardware timer for time measurement ***
		    //PNIO_get_HwCycInfoPtr ((PNIO_CYCLE_INFO_PTR*)&pSwiTim);
	    #endif
    }
    ...
    // *--------------------------------------------*
    void OsSetEntryCircBuf (LSA_UINT32 Index, LSA_UINT32 func)
	{
    ...
    // *--------------------------------------------*
    void OsSetEntryCircBufX (PNIO_UINT32 Index, PNIO_UINT32 value)
    {
		CIRC_BUF*  pCircBuf = &CircBuf[Index];
		if (CircBufEnabled)
		{
    ...
    // *   initialize circular buffer for debbugging
    // *--------------------------------------------*
    void OsPrintCircBuf (PNIO_UINT32 BufIndex)
    {
        PNIO_UINT32 i;
