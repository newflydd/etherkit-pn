...
in_addmulti(ip_addr *ap, struct net *netp, int addrtype)
{
   ...
   int error = 0; /* INICHE_TCIP_ITGR has initialized to reduce warnings */

#ifdef USE_IGMPV3
   USE_ARG(inm);
   return (in_addmulti2(ap, netp, addrtype, 0, NULL, MCAST_EXCLUDE, 1, &error));
#else
...
in_delmulti(struct in_multi * inm)
{
   ...
   int error = 0; /* INICHE_TCIP_ITGR initialize */

#ifdef USE_IGMPV3
    in_delmulti2(inm, 0, NULL, MCAST_EXCLUDE, 1, &error);
    return;
#endif
   ...
   USE_ARG(error); /* INICHE_TCPIP_ITGR eliminate MIPS compiler warning "variable 'error' set but not used", see 1911686 */
}
