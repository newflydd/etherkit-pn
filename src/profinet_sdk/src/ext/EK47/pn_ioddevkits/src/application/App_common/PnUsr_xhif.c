    ...
    while(PNIO_TRUE)
    {
        ...
        else if( 0 != (pnusr_submodule_params[submodule_no]->Direction & PNUSR_XHIF_DIRECTION_IN))    /* direction IN or INOUT */
        {
            if(pnusr_xhif_cyclic_in[tmp_read_ptr].Data_size !=
                    pnusr_submodule_params[submodule_no]->InData_size)
            {
#if 0
        ...
        else    /* direction OUT or NO_DATA */
        {
            ...
            else
            {
                    ...
                    /* Do not report - problem in TIA project, parameterization will fail and there will be module diff block */
                /* Invalid data size */
                PNPB_SYS_TRACE_00(LSA_TRACE_LEVEL_FATAL,  "\nInvalid no data size, expected 0\n");
#endif
            }
        }
...
PNIO_VOID PnUsr_xhif_gpio_init()
{
    ...
    OsCreateThread ((void  (*)(void))PNIO_XHIF_Write_Task, (PNIO_UINT8*)"Pnio_CycleIO", TASK_PRIO_PNPB, &Task_Acyc_Write);
