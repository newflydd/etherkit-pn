...
# error "NARE_CFG_NO_SOCK is obsolete. Please remove the define from nare_cfg.h"
#endif

#ifdef NARE_NO_SOCK
# error "NARE_NO_SOCK is obsolete. Please remove the define from nare_cfg.h"
#endif

#ifdef NARE_CFG_USE_RQB_PRIORITY
# error "NARE_CFG_USE_RQB_PRIORITY is obsolete. Please remove the define from nare_cfg.h"
#endif

#ifdef NARE_CFG_MAX_USER_CHANNELS
# error "NARE_CFG_MAX_USER_CHANNELS is obsolete. Please remove the define from nare_cfg.h"
#endif

#ifdef NARE_CFG_MAX_DCP_CHANNELS
# error "NARE_CFG_MAX_DCP_CHANNELS is obsolete. Please remove the define from nare_cfg.h"
#endif

#ifdef NARE_CFG_MAX_EDD_CHANNELS
# error "NARE_CFG_MAX_EDD_CHANNELS is obsolete. Please remove the define from nare_cfg.h"
#endif
...
#define NARE_MODULE_ID_NARE_reserved         0x6
...
typedef NARE_IP_TYPE * NARE_LOCAL_MEM_IP_PTR_TYPE;

typedef NARE_IP_TYPE const * NARE_LOCAL_MEM_IP_CONST_PTR_TYPE;

/*===========================================================================*/
/* Maximum number of user communication channels                             */
/*===========================================================================*/

#define NARE_MAX_USER_CHANNELS      (3 * EDD_CFG_MAX_INTERFACE_CNT) /* (CM + OHA + Nare App Ch) * IfCnt */ 
#define NARE_MAX_DCP_CHANNELS       (1 * EDD_CFG_MAX_INTERFACE_CNT)
#define NARE_MAX_EDD_CHANNELS       (1 * EDD_CFG_MAX_INTERFACE_CNT)
...
/*                                                                           */
/* This User-RCTRL-Block will move through the USER, DCP and EDD             */
/* channels depending on the Actions to perform.                             */
/*                                                                           */
/* If a User-Request needs a DCP-Call the RCTRL-Block is queued within the   */
...
/*                                                                           */
/* The same handling is done with EDD-Requests.                              */
/*                                                                           */
/* The max no. of simultaneous requests is set in NARE_CDB_PATH_PARAMS_TYPE. */
/*                                                                           */
/*                                                                           */
/* The RCTRL-Block managed as array of RCTRL-Blocks. So we can use the       */
/* index into the array for reference. For DCP we use this index             */
...
typedef struct _NARE_USER_RCTRL_TYPE * NARE_USER_RCTRL_PTR_TYPE;
...
#define NARE_RCTRL_QTYPE_EDD_OPEN        5 /* Special RCTRL used for EDD      */
...
#define NARE_RCTRL_RSTATE_reserved        1 /* reserved (was SOCK_GHBN)        */
...
typedef struct _NARE_USER_RCTRL_LINK_BASE_TYPE * _NARE_USER_RCTRL_LINK_BASE_PTR_TYPE;
...
typedef NARE_INTERNAL_REQUEST_TYPE * NARE_INTERNAL_REQUEST_PTR_TYPE;
...
typedef volatile struct _NARE_TIMER_INFO_TYPE
{
    ...
    LSA_VOID                        LSA_FCT_PTR(/*ATTR*/, Cbf) (NARE_INTERNAL_REQUEST_PTR_TYPE pIntRQB);
} NARE_TIMER_INFO_TYPE;

typedef volatile struct _NARE_TIMER_INFO_TYPE * NARE_TIMER_INFO_PTR_TYPE;
...
/* forward declaration */
typedef struct _NARE_HDB_USER_TYPE * NARE_HDB_USER_PTR_TYPE;
...
#define NARE_HANDLE_STATE_reserved_4                4
#define NARE_HANDLE_STATE_reserved_5                5
...
typedef struct _NARE_DCP_RCTRL_TYPE * NARE_DCP_RCTRL_PTR_TYPE;

typedef struct _NARE_DCP_RCTRL_TYPE const*   NARE_DCP_RCTRL_CONST_PTR_TYPE;
...
typedef struct _NARE_DCP_RCTRL_LINK_BASE_TYPE * NARE_DCP_RCTRL_LINK_BASE_PTR_TYPE;

/* declaring a pointer to an array */
typedef NARE_DCP_RCTRL_TYPE         NARE_DCP_RCTRL_ARRAY_TYPE[1];
typedef NARE_DCP_RCTRL_ARRAY_TYPE * NARE_DCP_RCTRL_ARRAY_PTR_TYPE;
...
typedef struct _NARE_DCP_HELLO_MGM_TYPE * NARE_DCP_HELLO_MGM_PTR_TYPE;

/* Status */
#define NARE_DCP_HELLO_IDLE              0
#define NARE_DCP_HELLO_IN_PROGRESS       1

typedef struct _NARE_HDB_DCP_TYPE * NARE_HDB_DCP_PTR_TYPE;
typedef struct _NARE_HDB_DCP_TYPE const* NARE_HDB_DCP_CONST_PTR_TYPE;
...
typedef volatile struct _NARE_EDD_ARP_SNIFFER_TYPE
{
    ...
    LSA_UINT16                          TimerEndCounter;/* Time we end with sniffing      */
                                                       /* ARP-Responses                   */
    LSA_UINT32                          DummyAlignment;/* TIA 1295596: compiler warning   */
    ...
    LSA_UINT32                          DummyAlignment2;/* TIA 2829082: compiler bug (ARM gcc 4.6.1) */
...
typedef struct _NARE_EDD_RCTRL_TYPE * NARE_EDD_RCTRL_PTR_TYPE;

typedef struct _NARE_HDB_EDD_TYPE *       NARE_HDB_EDD_PTR_TYPE;
typedef struct _NARE_HDB_EDD_TYPE const * NARE_HDB_EDD_CONST_PTR_TYPE;
...
typedef struct _NARE_EDD_RCTRL_LINK_BASE_TYPE * NARE_EDD_RCTRL_LINK_BASE_PTR_TYPE;

/* declaring a pointer to an array */
typedef NARE_EDD_RCTRL_TYPE         NARE_EDD_RCTRL_ARRAY_TYPE[1];
typedef NARE_EDD_RCTRL_ARRAY_TYPE * NARE_EDD_RCTRL_ARRAY_PTR_TYPE;
...
/* we have NARE_MAX_EDD_CHANNELS EDD-channels. */
...
{
    LSA_UINT16                    HandleCnt;       /* Number of handles supported       */
                                                   /* set to NARE_MAX_EDD_CHANNELS  */
    LSA_UINT16                    UsedCnt;         /* currently used user-channels      */
    NARE_ARP_TIMER_TYPE           ARPTimer;        /* ARP-Timer management              */
    NARE_HDB_EDD_TYPE             HDB[NARE_MAX_EDD_CHANNELS];
                                                   /* to array of MaxHandle             */
                                                   /* NARE_EDD_USER_TYPE types.         */
} NARE_HDB_EDD_INFO_TYPE;

typedef struct _NARE_HDB_EDD_INFO_TYPE * NARE_HDB_EDD_INFO_PTR_TYPE;
...
typedef NARE_USER_RCTRL_ARRAY_TYPE * NARE_USER_RCTRL_ARRAY_PTR_TYPE;
...
typedef struct _NARE_USR_HELLO_MGM_TYPE * NARE_USR_HELLO_MGM_PTR_TYPE;
...
/* we have NARE_MAX_USER_CHANNELS user-channels. */

typedef struct _NARE_HDB_USER_INFO_TYPE
{
    LSA_UINT16                    HandleCnt;       /* Number of handles supported        */
                                                   /* set to NARE_MAX_USER_CHANNELS  */
    LSA_UINT16                    UsedCnt;         /* currently used user-channels       */
    NARE_HDB_USER_TYPE            HDB[NARE_MAX_USER_CHANNELS];
                                                   /* to array of MaxHandle              */
                                                   /* NARE_HDB_USER_TYPE types.          */
...
typedef struct _NARE_NDB_TYPE * NARE_NDB_PTR_TYPE;
...
typedef struct _NARE_MGM_TYPE * NARE_MGM_PTR_TYPE;
...
/* we have three types of handle. A DCP, EDD and a USER-Handle.      */
...
#define NARE_HANDLE_TYPE_0x80  0x80
...
#define NARE_FLIP16(C)      (LSA_UINT16)(((((LSA_UINT16)(C))>>8)&0xFF)|((((LSA_UINT16)(C))<<8)&0xFF00))  /*lint -e572*/
...
((((LSA_UINT8 *)(_DstPtr))[0] ==  ((LSA_UINT8 *)(_SrcPtr))[0]) && \
(((LSA_UINT8 *)(_DstPtr))[1] ==  ((LSA_UINT8 *)(_SrcPtr))[1]) && \
(((LSA_UINT8 *)(_DstPtr))[2] ==  ((LSA_UINT8 *)(_SrcPtr))[2]) && \
(((LSA_UINT8 *)(_DstPtr))[3] ==  ((LSA_UINT8 *)(_SrcPtr))[3]))
#endif
...
/*lint -function(exit, NAREFatalError) */
LSA_EXTERN LSA_VOID NAREFatalError(
    NARE_FATAL_ERROR_TYPE  Error,
    LSA_UINT16             ModuleID,
    LSA_UINT32             Line);

LSA_EXTERN LSA_VOID NARERQBCopyLLErrorLocal(
    LSA_RQB_LOCAL_ERROR_TYPE * pLLError);

/* AP01539314 needs a typedef */
typedef  LSA_VOID  (NareUpperCbf)(NARE_UPPER_RQB_PTR_TYPE pRQB);

LSA_EXTERN LSA_VOID NARECallCbf(
//    LSA_VOID LSA_FCT_PTR(/*ATTR*/, Cbf)(NARE_UPPER_RQB_PTR_TYPE pRQB),
    NareUpperCbf                 Cbf,
    NARE_UPPER_RQB_PTR_TYPE      pRQB,
    LSA_SYS_PTR_TYPE             pSys);

LSA_EXTERN LSA_VOID NAREUserRequestFinish(
    NARE_HDB_USER_PTR_TYPE      pHDB,
    NARE_UPPER_RQB_PTR_TYPE     pRQB,
    NARE_RSP                    Status);

LSA_EXTERN NARE_HDB_USER_PTR_TYPE NAREGetUserHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN NARE_HDB_DCP_PTR_TYPE NAREGetDcpHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

LSA_EXTERN NARE_HDB_EDD_PTR_TYPE NAREGetEddHDBFromHandle(
    LSA_HANDLE_TYPE     Handle);

/* nare_usr.c */

LSA_EXTERN LSA_VOID NARE_UserChannelDone(
    NARE_HDB_USER_PTR_TYPE       pHDB,
    NARE_RSP                     Response);

LSA_EXTERN LSA_VOID NARE_DCPChannelDone(
    NARE_HDB_DCP_PTR_TYPE       pHDB,
    NARE_RSP                    Response);

LSA_EXTERN LSA_VOID NARE_UserCheckPending(
    NARE_HDB_USER_PTR_TYPE     pHDB);

LSA_EXTERN LSA_VOID NARE_UserReady(
    NARE_HDB_USER_PTR_TYPE      pHDB,
    NARE_USER_RCTRL_PTR_TYPE    pRCtrlUser,
    NARE_RSP                    Response);

LSA_EXTERN LSA_VOID NARE_UserStateMachine(
    NARE_USER_RCTRL_PTR_TYPE         pRCtrlUser,
    NARE_RSP                         Response,
...
LSA_EXTERN NARE_RSP NARE_DCPOpenChannel(
    LSA_HANDLE_TYPE       NAREHandle,
    LSA_SYS_PATH_TYPE     SysPath,
    LSA_SYS_PTR_TYPE      pSys);


LSA_EXTERN NARE_RSP NARE_DCPCloseChannel(
    LSA_HANDLE_TYPE       DCPHandle,
    LSA_SYS_PTR_TYPE      pSys);

LSA_EXTERN NARE_RSP NARE_DCPTrigger(
    NARE_HDB_DCP_PTR_TYPE     pHDB,
    NARE_DCP_RCTRL_PTR_TYPE   pRCtrl);

LSA_EXTERN LSA_VOID NARE_DCPCheckAndStartPending(
    NARE_HDB_DCP_PTR_TYPE     pHDB);

LSA_EXTERN LSA_VOID NARE_DCPApplyRCtrl(
    NARE_HDB_DCP_PTR_TYPE     pHDB,
    NARE_USER_RCTRL_PTR_TYPE  pRCtrlUser);

LSA_EXTERN NARE_RSP NARE_DCPGetFreeHDB(
    LSA_UINT16             NoOfParallelRQBs,
    NARE_HDB_DCP_PTR_TYPE *pHDBUser,
    LSA_SYS_PTR_TYPE       pSys);

LSA_EXTERN LSA_VOID NARE_DCPReleaseHDB(
    NARE_HDB_DCP_PTR_TYPE pHDBUser);

LSA_EXTERN NARE_HDB_DCP_PTR_TYPE NARE_DCPGetHandleByNIC(
    LSA_UINT16                   NICId);

LSA_EXTERN NARE_DCP_RCTRL_PTR_TYPE NARE_DCPGetFreeRCTRL(
    NARE_HDB_DCP_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID NARE_DCPReleaseRCTRL(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    NARE_DCP_RCTRL_PTR_TYPE      pRCtrl);

LSA_EXTERN NARE_RSP NARE_DCPIdentify(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
...
LSA_EXTERN NARE_RSP NARE_DCPIdentifyRQBSetup(
...
LSA_EXTERN NARE_RSP NARE_DCPSet(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    NARE_DCP_RCTRL_PTR_TYPE      pRCtrl,
...
LSA_EXTERN LSA_VOID NARE_DCPRemoveRCtrl(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    NARE_USER_RCTRL_PTR_TYPE     pRCtrlUser);

LSA_EXTERN LSA_BOOL NARE_DCPCheckNICUsed(
    LSA_UINT16 NICId);

LSA_EXTERN NARE_RSP NARE_DCPHello(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
...
LSA_EXTERN NARE_RSP NARE_DCPHelloStop(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    NARE_HDB_USER_PTR_TYPE       pHDBUser);

LSA_EXTERN   NARE_RSP NARE_DCPCancel(
    NARE_HDB_DCP_PTR_TYPE        pHDB,
    LSA_UINT32                   RequestID);

/* nare_edd.c */

LSA_EXTERN NARE_RSP NARE_EDDOpenChannel(
    LSA_HANDLE_TYPE              NAREHandle,
    LSA_SYS_PATH_TYPE            SysPath,
    LSA_SYS_PTR_TYPE             pSys);

LSA_EXTERN NARE_RSP NARE_EDDCloseChannel(
    EDD_HANDLE_LOWER_TYPE        EDDHandle,
    LSA_SYS_PTR_TYPE             pSys);

LSA_EXTERN LSA_VOID NARE_EDDReleaseHDB(
    NARE_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN NARE_HDB_EDD_PTR_TYPE NARE_EDDGetHandleByNIC(
    LSA_UINT16                   NICId);

LSA_EXTERN NARE_RSP NARE_EDDGetFreeHDB(
    LSA_UINT16                   NoOfParallelRQBs,
    NARE_HDB_EDD_PTR_TYPE        *pHDBEdd,
    LSA_SYS_PTR_TYPE             pSys);

LSA_EXTERN NARE_RSP NARE_EDDSendARP(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_EDD_RCTRL_PTR_TYPE      pRCtrl,
    NARE_LOCAL_MEM_IP_CONST_PTR_TYPE   pIPAddr,
    NARE_LOCAL_MEM_IP_CONST_PTR_TYPE   pLocalIPAddr);

LSA_EXTERN LSA_VOID NARE_EDDApplyRCtrl(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_USER_RCTRL_PTR_TYPE     pRCtrlUser);

LSA_EXTERN LSA_VOID NARE_EDDCheckAndStartPending(
    NARE_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID NARE_EDDTimerEventARP(LSA_VOID);

LSA_EXTERN NARE_RSP NARE_EDDGetMACAddr(
    NARE_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN LSA_VOID NARE_EDDTimerCB(
    NARE_INTERNAL_REQUEST_PTR_TYPE   pIntRQB);

LSA_EXTERN LSA_VOID NARE_EDDRemoveRCtrl(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_USER_RCTRL_PTR_TYPE     pRCtrlUser);

LSA_EXTERN LSA_BOOL NARE_EDDCheckNICUsed(
    LSA_UINT16                   NICId);

LSA_EXTERN NARE_EDD_RCTRL_PTR_TYPE NARE_EDDGetFreeRCTRL(
    NARE_HDB_EDD_PTR_TYPE        pHDB);

LSA_EXTERN NARE_RSP NARE_EDDStartSniffer(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_EDD_RCTRL_PTR_TYPE      pRCtrl,
    LSA_UINT8                    Mode);

LSA_EXTERN LSA_VOID NARE_EDDChannelDone(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_RSP                     Response);

LSA_EXTERN LSA_VOID NARE_EDDAbortSniffing(
    NARE_HDB_EDD_CONST_PTR_TYPE  pHDB);

LSA_EXTERN LSA_BOOL NARE_EDDFindAndFinishSniff(
    NARE_HDB_EDD_PTR_TYPE        pHDB,
    NARE_USER_RCTRL_PTR_TYPE     pRCtrl);

/* nare_sys.c */

LSA_EXTERN NARE_RSP NARE_UserGetFreeHDB(
    LSA_UINT16              NoOfParallelRQBs,
    NARE_HDB_USER_PTR_TYPE *pHDBUser);

LSA_EXTERN LSA_VOID NARE_UserReleaseHDB(
    NARE_HDB_USER_PTR_TYPE pHDBUser);

LSA_EXTERN NARE_USER_RCTRL_PTR_TYPE NARE_UserGetFreeRCTRL(
    NARE_HDB_USER_PTR_TYPE pHDB);

LSA_EXTERN LSA_VOID NARE_UserReleaseRCTRL(
    NARE_HDB_USER_PTR_TYPE      pHDB,
    NARE_USER_RCTRL_PTR_TYPE    pRCtrl);

LSA_EXTERN LSA_VOID NARE_EDDReleaseRCTRL(
    NARE_HDB_EDD_PTR_TYPE      pHDB,
    NARE_EDD_RCTRL_PTR_TYPE    pRCtrl);
...
LSA_EXTERN NARE_RSP NARETimerAlloc(
...
LSA_EXTERN NARE_RSP NARETimerFree(LSA_UINT16 NARETimerID);
LSA_EXTERN LSA_VOID NARETimerMgmSetup(LSA_VOID);

LSA_EXTERN LSA_VOID NARETimerStop(
    LSA_UINT16 NARETimerID);

LSA_EXTERN LSA_VOID NARETimerStart(
    LSA_UINT16 NARETimerID,
    LSA_UINT16 Time);

LSA_EXTERN LSA_VOID NARETimerRQBFree(
    LSA_UINT16 NARETimerID);
...
#define NARE_FATAL(Error)  NAREFatalError( Error, NARE_MODULE_ID, __LINE__)
