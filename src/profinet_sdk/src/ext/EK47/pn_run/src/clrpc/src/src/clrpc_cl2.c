...
clrpc_cl_bindRebind(
	...
	CLRPC_UPPER_CL_BIND_CONST_PTR_TYPE param /* sanity checked in caller */
)
{
...
clrpc_cl_bind_delayed(
	CLRPC_CHANNEL_PTR_TYPE channel, /* sanity checked in caller */
	CLRPC_UPPER_RQB_CONST_PTR_TYPE upper /* sanity checked in caller */
) {
	CLRPC_CLIENT_PTR_TYPE client;
...
#endif
	CLRPC_ASSERT(CLRPC_OFFS(CLRPC_CLIENT_TYPE, dispatch_tag) == 0); /* must be first in struct */
