/*
 * v3.1 August 30, 2011 ip/et_arp.c
...
#define LTRC_ACT_MODUL_ID 21 /* INICHE_TCIP_ITGR added for LSA traces */
...
int arp_ageout = 60 * TPS;   /* APR table refresh age, in ticks */
#else
...
etainit(void)
{
   ...
   if (reg_type(ET_ARP) != 0)
   {
		...
		TCIP_INICHE_TRACE_00(0, LSA_TRACE_LEVEL_ERROR
			, "ARP: unable to register type with MAC Driver"
			);
#else
...
send_arp(PACKET pkt, ip_addr dest_ip)
{
   ...
   MEMSET(arphdr->ar_tha, 0x00, 6); /* INICHE_TCIP_ITGR AP00932917 (initialize THA) */
...
arpReply(PACKET pkt)
{
   ...
   if (!outpkt)
   {
#if 0 /* INICHE_TCIP_ITGR it may hapen, that no more pakets are available; so we just do not send the arp reply! */
      dtrap();
#endif
      return;
   }
   ...
   if((unshort)ET_TYPE_GET((pkt->nb_buff+ ETHHDR_BIAS)) == 0x8100) /* bufferstart changed to real start of buffer: AP00922404 */
   {
...
arprcv(PACKET pkt)
{
  ...
  if ((arphdr->ar_tpa != arphdr->ar_spa) &&  /* gratuitous arp */
#ifdef IP_MULTICAST
      (arphdr->ar_tpa != pkt->net->n_ipaddr) &&   /* if it's not for me.... */
     (!IN_MULTICAST(ntohl(arphdr->ar_tpa))))
#else
      (arphdr->ar_tpa != pkt->net->n_ipaddr))
#endif /* IP_MULTICAST */
   {
      LOCK_NET_RESOURCE(FREEQ_RESID);
	  ...
	  USE_ARG(eth); /* INICHE_TCPIP_ITGR eliminate MIPS compiler warning "variable 'eth' set but not used", see 1911686 */

      return (ENP_NOT_MINE);
   }

#if 1 /* INICHE_TCIP_ITGR AP00901717: do not add broadcast or multicast addr to arp cache! */
   if (arphdr->ar_spa == INADDR_BROADCAST) goto drop; /* drop broadcast sender addr */

   /* IP-addresses are still in network byte order... (AP00981231) */
   if(IN_MULTICAST(ntohl(arphdr->ar_spa))) goto drop;/* drop sender multicast ip! */

   if ((arphdr->ar_sha[0] & 0x01) == 1) goto drop; /* drop senders multicast MAC */
#endif
   ...
   if (arphdr->ar_op == ARREQ)   /* is it an arp request? */
   {
      ...
      if (arphdr->ar_tpa != arphdr->ar_spa)  /* If not a gratuitous arp */
      {
...
/* INICHE_TCIP_ITGR enable drop for all compilations */
/*#ifdef IEEE_802_3_ONLY    */
drop:
/*#endif IEEE_802_3_ONLY */
...
send_via_arp(PACKET pkt, ip_addr dest_ip)
{
   ...
   if (tp->t_pro_addr == dest_ip)   /* we found our entry */
   {
      ...
      {
#if 1 /* INICHE_TCIP_ITGR take account to bigfreeq  too AP00609189 */
            if ( (lilfreeq.q_len < 2) || (bigfreeq.q_len < 2) )
#else
         if (lilfreeq.q_len < 2)
#endif
         {
