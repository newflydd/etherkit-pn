...
#include "compiler.h"
    ...
    // *
    // *----------------------------------------------------------------*
    static void FillUpStringWithBlanks (PNIO_UINT8* pStr, PNIO_UINT32 StringLen)
    {
        ...
        {
            OsMemSet (pStr + TerminatorOfs+1, ' ', StringLen - TerminatorOfs - 1);
        }
    }
                                 ...
                                 PNIO_IM0_SUPP_ENUM  Im0Support)  // stands for NOTHING / SUBMODULE / MODULE / DEVICE
    {
            ...
            // *--------------------------------------------------------
            OsMemCpy (pIm0Dst, pIm0Src, sizeof (IM0_DATA));
            ...
            {
                OsMemCpy (&Im0Device, pIm0Dst, sizeof (IM0_DATA));
            }
    }
	...
	PNIO_VOID pnpb_save_proxy(PNIO_DEV_ADDR* pAddr)  // geographical address
	{
		...
		// *--------------------------------------------------------
		OsMemCpy(&AddrProxy, pAddr, sizeof(PNIO_DEV_ADDR));
	}
		    ...
		    )
    {
        ...
        if (pSub->Im0Support)
        {
            OsMemCpy (&pIm0->IM0, &pSub->Im0Dat, sizeof (IM0_DATA)); // subslot has own im0 data
        }
        else
        {
            OsMemCpy (&pIm0->IM0, &Im0Device, sizeof (IM0_DATA));    // subslot has not own im0 data --> take from device
        }
        ...
        // *--------------------------------------------------------
        pIm0->IM0.VendorId       = OsHtons (pIm0->IM0.VendorId);
        pIm0->IM0.HwRevision     = OsHtons (pIm0->IM0.HwRevision);
        pIm0->IM0.Revcnt         = OsHtons (pIm0->IM0.Revcnt);
        pIm0->IM0.ProfId         = OsHtons (pIm0->IM0.ProfId);
        pIm0->IM0.ProfSpecTyp    = OsHtons (pIm0->IM0.ProfSpecTyp);
        pIm0->IM0.ImXSupported   = OsHtons (pIm0->IM0.ImXSupported);
       ...
       // **** check if subslot is plugged, else error ****
        ...
        if (pSub == NULL)
        {
            ...
            if ((pSub->Im0Support && ((pSub->Im0Dat.ImXSupported & IM1_SUPPORT_MSK) == 0))
                || (((Im0Device.ImXSupported & IM1_SUPPORT_MSK) == 0) && (pSub->Im0Support == 0)))
            {
                pPnioState->ErrCode   = 0xde;  // IODReadRes with ErrorDecode = PNIORW
                pPnioState->ErrDecode = 0x80;  // PNIORW
		    ...
		    )
    {
        ...
        if (pSub == NULL)
        {
            ...
            if ((pSub->Im0Support && ((pSub->Im0Dat.ImXSupported & IM2_SUPPORT_MSK) == 0))
                || (((Im0Device.ImXSupported & IM2_SUPPORT_MSK) == 0) && (pSub->Im0Support == 0)))
            {
                pPnioState->ErrCode   = 0xde;  // IODReadRes with ErrorDecode = PNIORW
                pPnioState->ErrDecode = 0x80;  // PNIORW
                pPnioState->ErrCode1  = 0xb0;  // example: Error Class 11 = Access, ErrorNr  = 0 "invalid index"
		    ...
		    )
    {
        ...
        if (pSub == NULL)
        {
            ...
            if ((pSub->Im0Support && ((pSub->Im0Dat.ImXSupported & IM3_SUPPORT_MSK) == 0))
                || (((Im0Device.ImXSupported & IM3_SUPPORT_MSK) == 0) && (pSub->Im0Support == 0)))
            {
                pPnioState->ErrCode   = 0xde;  // IODReadRes with ErrorDecode = PNIORW
                pPnioState->ErrDecode = 0x80;  // PNIORW
                pPnioState->ErrCode1  = 0xb0;  // example: Error Class 11 = Access, ErrorNr  = 0 "invalid index"
		    ...
		    )
    {
        ...
        if (pSub == NULL)
        {
            ...
            if ((pSub->Im0Support && ((pSub->Im0Dat.ImXSupported & IM4_SUPPORT_MSK) == 0)) 
				|| (((Im0Device.ImXSupported & IM4_SUPPORT_MSK) == 0) && (pSub->Im0Support == 0)))
            {
                pPnioState->ErrCode   = 0xde;  // IODReadRes with ErrorDecode = PNIORW
                pPnioState->ErrDecode = 0x80;  // PNIORW
                pPnioState->ErrCode1  = 0xb0;  // example: Error Class 11 = Access, ErrorNr  = 0 "invalid index"
