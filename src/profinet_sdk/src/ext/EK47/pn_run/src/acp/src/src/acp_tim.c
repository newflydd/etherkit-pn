...
static struct acp_oneshot_anchor_tag {
...
LSA_VOID acp_oneshot_sys_init(
	...
	case LSA_TIME_BASE_1S:
		acp_oneshot.msec_per_tick *= 1000;
		break;
	case LSA_TIME_BASE_100MS:
		acp_oneshot.msec_per_tick *= 100;
		break;
...
LSA_VOID acp_oneshot_trigger(
	...
	if( oneshot->running ) { /* running */
		...
		if( acp_is_not_null(next) ) {
			next->ticks += oneshot->ticks; /* give the ticks to the successor */
		}
	}

	ACP_ASSERT(acp_is_null(oneshot->dlink.Flink) && acp_is_null(oneshot->dlink.Blink)); /* sanity check */
...
//----------------------------------------------------------------------------*/
LSA_VOID acp_timeout_synchronized(
	...
	if( acp_is_not_null(oneshot) && oneshot->ticks > 0 && --oneshot->ticks == 0 ) {
		...
		} while( acp_is_not_null(oneshot) && oneshot->ticks == 0 );
...
LSA_UINT16 acp_oneshot_stop(
	...
	if( oneshot->running ) { /* running */
		...
		ACP_ASSERT(acp_is_null(oneshot->dlink.Flink) && acp_is_null(oneshot->dlink.Blink));
