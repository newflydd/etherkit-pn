...
// pay attention to section settings in the linker file and the MPU settings for the appropriate region
	...
	static LSA_UINT8    rxtx_mem[RX_TX_MEM_SIZE] __attribute__((used, section("uncached_mem")));
    extern LSA_VOID PNIP_Init(LSA_VOID);
#elif (PNIOD_PLATFORM & PNIOD_PLATFORM_DKSW)
	// rxtx_mem is allocated dynamically
#else
...
PNIO_UINT8 PNIO_get_in_data_size(PNIO_UINT32 ModId);
PNIO_UINT8 PNIO_get_out_data_size(PNIO_UINT32 ModId);
...
void  PNIO_init (void)
{
...
#endif
	 OsInit();		                            // operating system init

    TRACE_INIT (PNIO_TRACE_BUFFER_SIZE);        // do not call before OsInit()
    PnpbNvInit();
    ...
    // *------------------------------------------------*
    #if (PNIOD_PLATFORM & PNIOD_PLATFORM_DKSW)
        OsAlloc ((void**)&pRxTxMem, 0, RX_TX_MEM_SIZE);
    #else
        pRxTxMem = &rxtx_mem[0];	//Allocation based on linker script
        //OsMemSet (pRxTxMem, 0, RX_TX_MEM_SIZE); //Initialised again in XX_MEM_INIT
    #endif

    OsMemSet (&RegCbf_IrtTransEnd, 0, sizeof (RegCbf_IrtTransEnd));
}
                         ...
                         PNIO_UINT32 DefRouterAddr)     // IP Suite: default router address
{
    ...
    StationTypeLen = OsStrLen (pStationType);

    // *** check parameter ***
    if (StationTypeLen >= DCP_MAX_STATION_TYPE_LEN)
    {
    ...
    // *** check parameter ***
    if (StationNameLen >= DCP_MAX_STATION_NAME_LEN)
    {
        #if 1   // fix06.4
                            ...
                            PNIO_UINT32           NumOfSubListEntries)    // number of entries in pPioSubList
{
    ...
    if (PDEVnum)
    {
        ...
        OsAlloc((void**)&PDEVSubList, 0, sizeof(PNIO_SUB_LIST_ENTRY) * PDEVnum);
        ...
        for (i = 0; i < NumOfSubListEntries; i++)
        {
            if (pIoSubList[i].Subslot & 0x8000)
            {
                ...
                OsMemCpy(&PDEVSubList[PDEVnum], &pIoSubList[i], sizeof(PNIO_SUB_LIST_ENTRY));
                PDEVnum++;
            }
        }
...
// *  PNIO_shutdown ()
// *----------------------------------------------------------------*
PNIO_UINT32 PNIO_shutdown (void)
{
							 ...
							 PNIO_UINT32		    *pDevHndl)          // [out] device handle
{
    ...
    #elif  (PNIOD_PLATFORM & PNIOD_PLATFORM_EB200_400)
        XX_MEM_init(RX_TX_MEM_SIZE, 0xee, (void*)pRxTxMem, XX_MEM_DO_NOT_PRESET, 0xDD);
    #elif  (PNIOD_PLATFORM & PNIOD_PLATFORM_DKSW)
        XX_MEM_init(RX_TX_MEM_SIZE, 0xee, (void*)pRxTxMem, XX_MEM_DO_NOT_PRESET, 0xDD);
	#elif(PNIOD_PLATFORM & PNIOD_PLATFORM_RZT2)
        XX_MEM_init(RX_TX_MEM_SIZE, 0xee, pRxTxMem, XX_MEM_DO_NOT_PRESET, 0xDD);
...
PNIO_UINT32  PNIO_trigger_pndv_ds_rw_done (PNIO_ERR_STAT* PnioStat, PNIO_UINT32 bufLen)
{
    ...
    pDsPeri->event_type.u.sv.ar_rec.cm_pnio_err = OsHtonl (*vu32);
...
// *-----------------------------------------------------------------
PNIO_VOID*  PNIO_rec_set_rsp_async (void)
{
...
// *---------------------------------------------------------------*
// *
// *   PnpbSetLastError (void)
// *---------------------------------------------------------------
// *
// *   sets the last error variable to the actual error value
// *---------------------------------------------------------------*
//lint -e{832, 578} Parameter 'Symbol' not explicitly declared, int assumed
void PnpbSetLastError (PNIO_ERR_ENUM LastErr)
{
...
PNPB_CODE_FAST PNIO_UINT32 PNIO_initiate_data_read   (PNIO_UINT32   DevHndl)
{
	...
	else	/*redundancy S2 - */
	{
		...
		{
			if(TRUE == pnpb_data.timer[i].rqb_in_use)
			{
				pnpb_rdht_timeout(i);
			}
		}
...
PNPB_CODE_FAST PNIO_UINT32 PNIO_dbai_buf_lock  (PNIO_BUFFER_LOCK_TYPE*  pLock)
{
    ...
    switch (pLock->IoDir)
    {
        case PNIO_IOCR_TYPE_INPUT:
        		...
        		Status = PnpbProvLock (&(pLock->pBuf), (PNIO_UINT16) (pLock->ArNum - 1), pLock->IocrOffset, 255);
#if 1 // fix07.7
                PnpbCopyRecReadIoData(pLock->pBuf, ( pLock->ArNum - 1 ) , pLock->IocrOffset, 255);
#endif
        	#else
...
// *
// *----------------------------------------------------------------*
// *    init function for the isochronous support.
// *    This function must be called once, before this feature
// *    can be used.
// *
// *----------------------------------------------------------------*
PNIO_UINT32  PNIO_IsoInit (PNIO_VOID)
{
    PNPB_API_TRACE_00(LSA_TRACE_LEVEL_NOTE,  "IsoInit \n");
    return (PNIO_TRUE);
}
    ...
    // *-----------------------------------------------------------------
    PNPB_CODE_FAST void  pnpb_cbf_trigger_io_exchange (void)
    {
    ...
    PNIO_UINT32 PNIO_CP_register_cbf(PNIO_CP_CBE_TYPE CbeType, PNIO_CP_CBF pCbf)
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "PNIO_CP_register_cbf() not implemented on this platform\n");
    ...
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "pnpb_cbf_trigger_io_exchange() not implemented on this platform\n");
    }
    ...
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "PNIO_IsoActivateGpioObj() not implemented on this platform\n");
        return (PNIO_NOT_OK);
    }
    ...
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "PNIO_IsoFreeObj() not implemented on this platform\n");
        return (PNIO_NOT_OK);
    }
    ...
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "PNIO_IsoActivateTransEndObj() not implemented on this platform\n");
        return (PNIO_NOT_OK);
    }
    ...
    {
        PNPB_API_TRACE_00(LSA_TRACE_LEVEL_ERROR,
                          "IsoActivateIsrObj() not implemented on this platform\n");
        return (PNIO_NOT_OK);
    }
    ...
    void  pnpb_cbf_trigger_io_exchange (void)
    {
        if (RegCbf_IrtTransEnd.Active == PNIO_TRUE)
        {
            // *** call user defined Start_OP  CBF ****
            (RegCbf_IrtTransEnd.pCbf) ();
        }
    }
            ...
            PNIO_CP_CBF       pCbf)
    {
        switch(CbeType)
      {
          case PNIO_CP_CBE_TRANS_END_IND:        // transmission end
              if (pCbf)
              {
                  RegCbf_IrtTransEnd.pCbf   = pCbf;
                  RegCbf_IrtTransEnd.Active = PNIO_TRUE;
              }
              else
              {
                  RegCbf_IrtTransEnd.Active = PNIO_FALSE;
                  RegCbf_IrtTransEnd.pCbf   = pCbf;
              }
              break;

          default:
              PNPB_API_TRACE_01(LSA_TRACE_LEVEL_ERROR, "unknown CbeType = %d in PNIO_CP_register_cbf\n", CbeType);
