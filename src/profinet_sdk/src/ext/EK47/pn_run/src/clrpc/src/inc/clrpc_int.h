...
#ifdef CLRPC_LOG_FCT
# error "CLRPC_LOG_FCT is obsolete (TIA 1649473, V6.1i1.4). please remove the define from clrpc_cfg.h"
#endif
...
#define CLRPC_OFFS(type, field)  ((LSA_INT) /*lint -e(413)  Likely use of null pointer */ ( &((type*)0)->field ))
...
#define CLRPC_PTR_IS_NULL(ptr_)             (LSA_HOST_PTR_ARE_EQUAL((ptr_), LSA_NULL))
#define CLRPC_PTR_IS_NOT_NULL(ptr_)         (! LSA_HOST_PTR_ARE_EQUAL((ptr_), LSA_NULL))
#define CLRPC_PTR_ARE_EQUAL(ptr1_, ptr2_)   (LSA_HOST_PTR_ARE_EQUAL((ptr1_), (ptr2_)))
...
# define CLRPC_CAST_UPPER_RQB_PTR_PTR(val_)  ((CLRPC_UPPER_RQB_PTR_TYPE *)(val_))
# define CLRPC_CAST_UPPER_MEM_PTR_PTR(val_)  ((CLRPC_UPPER_MEM_PTR_TYPE *)(val_))

# define CLRPC_CAST_LOCAL_MEM_PTR_PTR(val_)  ((CLRPC_LOCAL_MEM_PTR_TYPE *)(val_))

# define CLRPC_CAST_LOWER_RQB_PTR_PTR(val_)  ((CLRPC_LOWER_RQB_PTR_TYPE *)(val_))
# define CLRPC_CAST_LOWER_MEM_PTR_PTR(val_)  ((CLRPC_LOWER_MEM_PTR_TYPE *)(val_))
...
typedef struct clrpc_channel_tag * CLRPC_CHANNEL_PTR_TYPE; /* forward */

typedef struct clrpc_channel_tag const * CLRPC_CHANNEL_CONST_PTR_TYPE; /* forward */
/*------------------------------------------------------------------------------
...
typedef struct clrpc_list_entry_tag *CLRPC_LIST_ENTRY_PTR_TYPE;
...
} CLRPC_QUEUE_TYPE, *CLRPC_QUEUE_PTR_TYPE;

typedef CLRPC_QUEUE_TYPE const * CLRPC_QUEUE_CONST_PTR_TYPE;
...
typedef struct clrpc_oneshot_tag * CLRPC_ONESHOT_PTR_TYPE;
typedef struct clrpc_oneshot_tag const * CLRPC_ONESHOT_CONST_PTR_TYPE;
...
clrpc_oneshot_is_running(
	CLRPC_ONESHOT_CONST_PTR_TYPE oneshot
);
...
typedef struct {
	...
	LSA_VOID_PTR_TYPE user_ptr;
} CLRPC_HASH_ENTRY_TYPE, * CLRPC_HASH_ENTRY_PTR_TYPE;

typedef CLRPC_HASH_ENTRY_TYPE const * CLRPC_HASH_ENTRY_CONST_PTR_TYPE;
...
} CLRPC_HASH_TYPE, * CLRPC_HASH_PTR_TYPE;

typedef CLRPC_HASH_TYPE const * CLRPC_HASH_CONST_PTR_TYPE;
...
} CLRPC_HASH_ITERATOR_TYPE, * CLRPC_HASH_ITERATOR_PTR_TYPE;

typedef CLRPC_HASH_ITERATOR_TYPE const * CLRPC_HASH_ITERATOR_CONST_PTR_TYPE;
...
typedef LSA_VOID
	LSA_FCT_PTR(/*ATTR*/, CLRPC_DISPATCH_TAG)( /* the tag is a function pointer */
		CLRPC_CHANNEL_PTR_TYPE channel,
...
#define CLRPC_DISPATCH(ptr, channel, instance, lower) { \
		( *(CLRPC_DISPATCH_TAG *)(ptr) )(channel, instance, lower); \
	}
...
typedef struct clrpc_data_tag
{
    ...
    CLRPC_CHANNEL_TYPE  (* channels_ptr)[CLRPC_CFG_MAX_CHANNELS];
#if CLRPC_CFG_MAX_SERVERS
    LSA_UINT16                                  count_contexts;
    CLRPC_SERVER_TYPE      (* servers_ptr)[CLRPC_CFG_MAX_SERVERS];
    CLRPC_HASH_PTR_TYPE                         sc_hash_ptr;
    CLRPC_HASH_PTR_TYPE                         ctx_hash_ptr;
#endif
    CLRPC_ONESHOT_GLOB_TYPE * oneshot_ptr;
}CLRPC_DATA_TYPE;
...
#define CLRPC_UNMARSHAL_UUID(ptr_, bytes_left_, dst_, qry_, jump_label_) { \
	if( (*(bytes_left_)) < sizeof(CLRPC_UUID_TYPE) ) goto jump_label_; \
  	*(dst_) = *(CLRPC_UUID_TYPE *)(CLRPC_TEST_ALIGN4(*(ptr_))); \
	if( CLRPC_DREP_INT_DIFFERS((qry_)->u.call.drep.drep_int) ) { \
...
CLRPC_DATA_TYPE         clrpc_data = {0};
CLRPC_ONESHOT_GLOB_TYPE clrpc_oneshot = {0};
#else
CLRPC_DATA_TYPE         clrpc_data;
CLRPC_ONESHOT_GLOB_TYPE clrpc_oneshot;
#endif

#else
LSA_EXTERN CLRPC_DATA_TYPE clrpc_data;
...
LSA_VOID
clrpc_callback (
	CLRPC_CHANNEL_CONST_PTR_TYPE channel,
	LSA_UINT16 response,
	CLRPC_UPPER_RQB_PTR_TYPE *upper_ptr,
    LSA_UINT16 module,
    LSA_INT line
);
...
clrpc_mac_addr_is_valid (
	CLRPC_COMMON_MEM_U8_CONST_PTR_TYPE mac_addr
);

LSA_BOOL clrpc_alloc_rcv_resources(
	CLRPC_CHANNEL_PTR_TYPE      channel
);
