...
// *
// *-----------------------------------------------------------------
void pnpb_sub_real_init(void)
{
	OsMemSet (&PnpbMod, 0, sizeof (PnpbMod));
}
...
// *-----------------------------------------------------------------
// *  pnpb_sub_real_entity_from_peri
// *  gets the list index in real_cfg for a corresponding Entity in PERI-IF
// *
// *  input:    EntityInd   Entity Index in Perif
// *  output:   return      Index in PnpbMod.Sub[Index] or (-1) (not found)
// *
// *-----------------------------------------------------------------
PNIO_UINT32   pnpb_sub_real_entity_from_peri (PNIO_UINT16 EntityInd)
{
	PNIO_UINT32 i;
	...
	for (i = 0; i < IOD_CFG_MAX_NUMOF_SUBSLOTS; i++)
	{
		if (   (PnpbMod.Sub[i].PlugState)               // submodule is plugged
			&& (PnpbMod.Sub[i].PeriphRealCfgInd == EntityInd))
		{ // subslot found
			return (i);
		}
	}
...
PNPB_REAL_SUB*   pnpb_sub_handle_from_peri (PNIO_UINT16 EntityInd)
{
	PNIO_UINT32 i;
	...
	for (i = 0; i < IOD_CFG_MAX_NUMOF_SUBSLOTS; i++)
	{
		if (PnpbMod.Sub[i].PlugState == PNPB_NOT_PLUGGED)
		{
			...
			{ // **** remove submodule from PNPB, if it could not be plugged in PNDV
				OsMemSet (&PnpbMod.Sub[i], 0, sizeof (PnpbMod.Sub[0]));
			}
						  ...
						  PNIO_UINT32*          pStatusList)           // list of return-Stati[NumOfSublistEntries]
{
	...
	OsAlloc ((void**) &pIndexList,                          // address pointer
	...
	for (i = 0; i < NumOfSubListEntries; i++)
	{
		if (*(pStatusList+i) == PNIO_OK)  // no "early" error found for this subslot
		{
			...
			if (PnpbInd != PNPB_INVALID_INDEX)
			{
				...
				{
					OsMemSet (&PnpbMod.Sub[PnpbInd], 0, sizeof (PnpbMod.Sub[0]));
								...
								PNIO_DEV_ADDR *pAddr)
{
	...
	for (i = 0; i < IOD_CFG_MAX_NUMOF_SUBSLOTS; i++)
	{
		...
		{ // *** subslot found ***
			...
			OsMemSet (&PnpbMod.Sub[i], 0, sizeof (PNPB_REAL_SUB));
            ...
            if (Im0Support != PNIO_IM0_NOTHING)
            {
                ...
                OsAlloc((void**) &IM1_clr, 0x20, sizeof(IM1_DATA));
                OsAlloc((void**) &IM2_clr, 0x20, sizeof(IM2_DATA));
                OsAlloc((void**) &IM3_clr, 0x20, sizeof(IM3_DATA));
                OsAlloc((void**) &IM4_clr, 0x00, sizeof(IM4_DATA));
