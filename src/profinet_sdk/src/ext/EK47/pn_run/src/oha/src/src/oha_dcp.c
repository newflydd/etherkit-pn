...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID oha_dcp_request_lower_done(
    ...
    LSA_UINT16                 Response)
{
    ...
    OHA_PROGRAM_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"<<<: OHA_DCPChannelDone done (NICId: 0x%X,Rsp: 0x%X,State: 0x%X)",NICId,Response,pHDB->DcpState);
    ...
    switch (pHDB->DcpState)
    {
        ...
        case OHA_HANDLE_STATE_OPEN_DCP_RUNNING:
            if ( Response == OHA_OK)
            {
                ...
                if ( Response == OHA_OK )
                {
                    /* preallocate the response RQBs for DCP Set */
                    pHDB->pLowerDcpRspRqb = oha_dcp_alloc_rsp_rqb(pHDB); /* DcpSet.res RQB      */
                    OHA_ASSERT_NULL_PTR(pHDB->pLowerDcpRspRqb);
                ...
                else
                {
                    ...
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP Receive Provide request failed. Response: 0x%X", Response);

                    pHDB->OpenResponse = Response; /* save for later */

                    pHDB->DcpState = OHA_HANDLE_STATE_CLOSE_ERROR_DCP_RUNNING;

                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Closing Channel because of error");
                    ...
                    {
                        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Cancel DCP Receive().");
    ...
    if ( Finish )  /* finish the request ? */
    {
        ...
        OHA_UPPER_TRACE_03(0, LSA_TRACE_LEVEL_NOTE,"<<<: Open/Close-Request finished. Opcode: 0x%X, UpperHandle: 0x%X, Response: 0x%X",OHA_RQB_GET_OPCODE(pRQB),OHA_RQB_GET_HANDLE(pRQB),Response);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DCPOpenChannelDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB)
{
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: DCP_OPC_OPEN_CHANNEL done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_DCP_RQB_GET_RESPONSE(pRQB));

	NICId = pHDB->Params.pDetail->Params.Dcp.NICId;

    if ( OHA_DCP_RQB_GET_RESPONSE(pRQB) == DCP_RSP_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Open Channel successful");
    ...
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP-Open Channel failed.[0x%X]",OHA_DCP_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
	...
	if (Response == OHA_OK)
	{
        ...
        if (Response != OHA_OK)
        {
            ...
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"Cannot Set Frame Filter for DCP Open Channel(Response: 0x%X).", Response);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DCPCloseChannel(
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DCPCloseChannelDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB)
{
    LSA_UINT16               Response = OHA_OK;
    ...
    if ( OHA_DCP_RQB_GET_RESPONSE(pRQB) == DCP_RSP_OK )
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Close Channel successful");
    }
    else
    {
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP-Close Channel failed.[0x%X]",OHA_DCP_RQB_GET_RESPONSE(pRQB));
        Response = OHA_ERR_LL;
    }
        ...
        if ( ! LSA_HOST_PTR_ARE_EQUAL(pRQB, LSA_NULL) )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: DCP-Request: DCP_OPC_RECV_IND (Handle: 0x%X,DCPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->DcpHandle,pRQB);

                    DCP_UPPER_RQB_RECV_IND_SERVICE_ID_SET(pRQB, ServiceId);
    ...
    OHA_HDB_DCP_PTR_TYPE         pHDB)
{
    ...
    OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: DCP_OPC_RECV_IND done (Handle: 0x%X,Rsp: 0x%X,State: 0x%X)",pHDB->ThisHandle,OHA_DCP_RQB_GET_RESPONSE(pRQB),pHDB->DcpState);
        ...
        if (OHA_GET_GLOB_PTR()->UsedUserCnt)
        {
            switch (DCP_UPPER_RQB_RESPONSE_GET(pRQB))
            {
                case DCP_RSP_OK:
                    ...
                    switch (pRecv->ServiceId)
                    {
                        ...
                        case DCP_IDENTIFY:   /* 5 */
                            ...
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"a DCP_IDENTIFY is done (RetVal: 0x%X)", RetVal);
                            break;
                        case DCP_GET:       /* 3 */
                            RetVal = oha_dcp_Get(pRQB, pHDB);
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"a DCP_GET is done (RetVal: 0x%X)", RetVal);
                            break;
                        case DCP_SET:       /* 4 */
                        {
                            ...
                            {                  /* the user has to respond a DcpSet    */
                                /* meanwhile a further DcpSet received -don't REJECT (TFS 1126408) */
                                if(OHA_IS_NULL(pHDB->pLowerDcpSetRqb))
                                {
                                    pHDB->pLowerDcpSetRqb = pRQB;  /* store this LowerRqb */
                                    pHDB->RxPendingCnt--;   /* number of Recv-Request pending within DCP */
                                    OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DCPRecvDone() - Queued Request for DCP_SET (QueryState: 0x%X)", pHDB->QueryState);
                                    return;
                                }
                                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"previous DCP-SETs are to respond by the User (QueryState: 0x%X). Reject and reprovide.", pHDB->QueryState);
                                break;
                            }
							...
							/* TIA 1126408: Every DcpSet has to be responded to DCP */
							(LSA_VOID)oha_dcp_ParseSet(pRQB, pHDB, &pHDB->CheckSetInd); /* are there parameters to set ? */
                            ...
                            switch (pHDB->CheckSetInd.Event)
                            {
                                ...
                                case OHA_MODE_EMERGENCY_SET:        /* an OHA controlled Dcp-Set as emergency Set */
                                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"an OHA controlled DCP_SET is to do (Mode: 0x%X)", pHDB->CheckSetInd.Event);
										 ...
										 (pHDB->CheckSetInd.Options & OHA_CTRL_RESET_COMMUNICATION) )
									{
										...
										if (RetVal != OHA_OK)
										{
											...
											OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_WARN,"DCP-Set(ResetToFactory) of Set-IP to 0.0.0.0 failed!");
											break;
										}
										else
										{
                                            ...
                                            pHDB->RxPendingCnt--;   /* number of Recv-Request pending within DCP */
                                            OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DCPRecvDone() - ResetToFactory active for DCP_SET");
                                            return;
										}
									}
                                    ...
                                    if (pHDB->CheckSetInd.Options & OHA_CTRL_IP_SUITE)  /* ip address is to set */
                                    {
                                        ...
                                        if (OHA_IS_NOT_NULL(IpSuitePtr))
                                        {
                                            ...
                                            if (RetVal != OHA_OK)   /* local SockSetIpAddress fails */
                                            {
												...
												pHDB->CheckSetInd.Event  = OHA_MODE_NO_SET;  /* No set of parameters, only respond the remote dcp-server */
												pHDB->CheckSetInd.Reason = OHA_DCP_REASON_DS_NOT_SET;  /* we take the negative default value, not OK ! */
												OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_WARN,"DCP-Set of Set-IP failed!");
												break;
	                                        }
                                            else
                                            {
                                                pHDB->pLowerDcpSetRqb = pRQB;  /* store this LowerRqb, respond a DcpSet by SockSetIp.Cnf */
                                                pHDB->RxPendingCnt--;   /* number of Recv-Request pending within DCP */
                                                OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DCPRecvDone() - SockSetIp active for DCP_SET");
                                                return;
                                            }
                                        }
                                    }
                                ...
                                case OHA_EVENT_QUERY_SET:   /* user-query for a DcpSet (Query-Set) */
                                    ...
                                    pHDB->RxPendingCnt--;   /* number of Recv-Request pending within DCP */
                                    OHA_FUNCTION_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_DCPRecvDone() - USER Query for DCP_SET");
                                    return;
#if 0
                                case OHA_MODE_SET:          /* a USER controlled Dcp-Set */
                                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"a USER controlled DCP_SET is done");
                                    break;
#endif
                                default:
                                    OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR,"invalid DCP_SET option");
                                    OHA_FATAL(OHA_FATAL_ERR_INCONSISTENZ);
                                    break;
                            }
                        ...
                        case DCP_HELLO:   /* 6 */
                            ...
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"a DCP_HELLO is done (RetVal: 0x%X)", RetVal);
                            break;
                        default:        /* service not supported - ignore   */
                            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"DCP Receive Service not supported (0x%X).",pRecv->ServiceId);
                            break;
                    }
                    break;
                ...
                case DCP_RSP_OK_CANCEL:
                    ...
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"OHA Receive canceled (0x%X).",DCP_UPPER_RQB_RESPONSE_GET(pRQB));
                    Reprovide = LSA_FALSE;
                    break;

                    /* Eine DCP-PDU mit unbekannten Protokollelementen wurde emp-fangen. */
                case DCP_RSP_ERR_PROTOCOL:
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_FATAL,"OHA-ERROR: OHA_OPC_RECV_IND failed.(0x%X).",DCP_UPPER_RQB_RESPONSE_GET(pRQB));
                    break;

                default:
                    /* DCP_RSP_ERR_RESSOURCE    No Timer, lower RQB or memory available */
                    /* DCP_RSP_ERR_LOWER_LAYER  Error occurred at lower Receive */
                    OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_ERROR,"OHA-ERROR: OHA_OPC_RECV_IND failed.(0x%X).",DCP_UPPER_RQB_RESPONSE_GET(pRQB));
                    break;
            }
        }
        else
        {
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"DCP Receive while in open-state but without any user. discard and reprovide.");
        }
    }
    else
    {
        OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"DCP Receive while not in open-state. discard and dont reprovide.");
        Reprovide = LSA_FALSE;
    }

    if ( Reprovide )
    {
        OHA_LOWER_TRACE_03(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: DCP-Request: OHA_OPC_RECV_IND (Handle: 0x%X,DCPHandle: 0x%X,pRQB: 0x%X)",pHDB->ThisHandle,pHDB->DcpHandle,pRQB);
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DCPCancelReceive(
    LSA_HANDLE_TYPE       DCPHandle,
    LSA_SYS_PTR_TYPE      pSys)
{
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB    = LSA_NULL;
...
/*+                                                                         +*/
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_DCPCancelReceiveDone(
    OHA_DCP_LOWER_RQB_PTR_TYPE  pRQB,
    OHA_HDB_DCP_PTR_TYPE        pHDB)
{
    LSA_UINT16       RetVal = LSA_RET_OK;
    ...
    OHA_LOWER_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE,"<<<: DCP_OPC_CANCEL_RECV done (Handle: 0x%X,Rsp: 0x%X)",pHDB->ThisHandle,OHA_DCP_RQB_GET_RESPONSE(pRQB));

    switch (OHA_DCP_RQB_GET_RESPONSE(pRQB))
    {
        case DCP_RSP_OK:
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"DCP-Cancel Receive successful");
            break;
        case DCP_RSP_ERR_PARAM:
            OHA_PROGRAM_TRACE_00(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE_LOW,"No DCP-Cancel Receives found");
            break;
        default:
            OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP-Cancel Receive failed.[0x%X]",OHA_DCP_RQB_GET_RESPONSE(pRQB));
            break;
    }
    ...
    OHA_HDB_DCP_PTR_TYPE        pHDB)
{
    ...
    if (ServiceId == DCP_SET)
    {
        ...
        switch (pHDB->QueryState)
        {
            ...
            default:
                OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DCP_SET send state failed (QueryState: 0x%X).", pHDB->QueryState);
                break;
        }
    }
    ...
    {
        OHA_PROGRAM_TRACE_02(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"DDCP_OPC_SEND_RSP failed (ServiceId: 0x%X, Response: 0x%X).", ServiceId, Response);
    }

    if (ServiceId != DCP_SET) /* 1126408: the response rqb of a SET remains allocated */
    {
        oha_dcp_free_rsp_rqb(pHDB, pRQB);
    }
    pHDB->TxPendingCnt--;

    /* TFS 1126408: get a stored LowerRqb */
    if(OHA_IS_NOT_NULL(pHDB->pLowerDcpSetRqb))
    {
        OHA_DCP_LOWER_RQB_PTR_TYPE  pLowerDcpSetRqb = pHDB->pLowerDcpSetRqb;

        pHDB->pLowerDcpSetRqb = LSA_NULL;
        pHDB->RxPendingCnt++;  /* number of Recv-Request pending within DCP */
        OHA_DCPRecvDone(pLowerDcpSetRqb, pHDB);
        OHA_FUNCTION_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_NOTE, "OUT: OHA_DCPRecvDone() - Execute queued Request for DCP_SET (QueryState: 0x%X)", pHDB->QueryState);
    }
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_UINT16 OHA_DCPUpdateFilter(
    LSA_UINT16 NICId,
    ...
    if (ControlStatus & OHA_CTRL_STATION_NAME)
    {
			...
			(pHDBDcp->RequestState == OHA_DCP_STATE_FILTER_START)) /* DCP-Filter is active ? */
        {
			...
			if(OHA_IS_NOT_NULL(pBlock)) /* AP01022055, AP01256689 */
			{
				...
				OHA_LOWER_TRACE_03(pHDBDcp->TraceIdx, LSA_TRACE_LEVEL_NOTE,">>>: EDD-Filter-Queue-Update-Details (pHDBEdd: 0x%X, Orginator: 0x%X, QueuekCount: 0x%X)",pHDBEdd,Orginator,BlockCount);
			}
			else
			{
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestReadQueryElements(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
    OHA_HDB_USER_PTR_TYPE    pHDB)
{
        ...
        pQueryElement->Count == 0)
    {
        ...
        OHA_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_READ_ELEMENT failed. No Lower Channel for this NicId (NIC-ID: 0x%X)", NICId);
    }
               ...
               LSA_HOST_PTR_ARE_EQUAL(pHDBDcp->pLowerDcpSetRqb, LSA_NULL)) )
    {
        ...
        /* Error on getting handle. Abort. */
        OHA_PROGRAM_TRACE_01(pHDB->TraceIdx, LSA_TRACE_LEVEL_UNEXP,"OHA_OPC_DCP_READ_ELEMENT failed. No DCP-Channel opened or DCP-Indication present (NIC-ID: 0x%X)",NICId);
    }
    else
    {
...
/*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
LSA_VOID OHA_RequestHelloIndication(
    OHA_UPPER_RQB_PTR_TYPE   pRQB,
